...
std::ostream& operator<<(std::ostream& out, GraphOpt const& opts) {
...
////////////////////////////////////////////////////////////////////////////////
int GraphOpt::optionsWithArgs(int argc, char* argv[], GraphOpt& res) {
  char* optArg = nullptr;
  int optIndex = 1;
  ...
  for (optIndex = 1;
       (optIndex < argc) && (argv[optIndex][0] == '-') && (argv[optIndex][0]);
       optIndex++) {
    ...
    if (p == nullptr) {
      std::cerr << "opt " << (char)opt << " not found with " << optionStr
                << std::endl;
      res._opts.push_back('?');
      res.reset();
      return -1;
      ...
      if (p[1] == ':')  // processing param with
      {
        optIndex++;
        if (optIndex >= argc) {
          std::cerr << "optIndex " << optIndex << " is out of bounds " << argc
                    << std::endl;
          res.reset();
          res._opts.push_back('?');
          return -2;
  ...
  if (!res.hasParam('l') && !res.hasParam('x')) {
    std::cerr << "No -l or -x params are provided. At least one of them should "
                 "be used."
              << std::endl;
    res.reset();
    res._opts.push_back('?');
  ...
  if (res._args.empty()) res._args['o'] = defaultOutputName;
...
////////////////////////////////////////////////////////////////////////////////
std::ostream& GraphOpt::help(std::string app, std::ostream& out) {
  out << "Usage: \n"
      << app
      << " [-lxe] [-o outname] filename1 "
         "[filename2 filename3 ... filenameN]"
      << std::endl;
