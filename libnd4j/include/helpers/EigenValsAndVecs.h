...
class EigenValsAndVecs {
 public:
  ...
  // {N,2} matrix of eigenvalues, 2 means real and imaginary part
  NDArray _Vals;
  // {N,N,2} matrix, whose columns are the eigenvectors (complex), 2 means real and imaginary part
  NDArray _Vecs;
 ...
 private:
  ...
  // makes changes both in schurMatrixT(NxN) and schurMatrixU(NxN), also calculates and stores pseudo-eigenvectors (real) in schurMatrixU columns
  void calcPseudoEigenVecs(NDArray& schurMatrixT, NDArray& schurMatrixU);

  // calculates _Vecs
  void calcEigenVecs(const NDArray& schurMatrixU);
};
