...
class RandomHelper {
 ...
 public:
  ...
  inline _CUDA_D uint64_t nextUInt() { return buffer->getNextElement(); }
  ...
  inline _CUDA_D int relativeInt(Nd4jLong index) {
    return (int)(relativeUInt(index) %
                 (sd::DataTypeUtils::max<uint32_t>() + 1));
  }
  ...
  inline _CUDA_D T relativeT(Nd4jLong index) {
    if (sizeof(T) < 4) {
      // FIXME: this is fast hack for short types, like fp16. This should be
      // improved.
      return (T)((float)relativeUInt(index) /
                 (float)sd::DataTypeUtils::max<uint32_t>());
    } else
