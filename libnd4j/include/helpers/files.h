...
// adopted from
// https://stackoverflow.com/questions/2718915/check-if-file-exists-including-on-path
//
...
char **shellpath(void) {
  ...
  char **vector =  // size is overkill
      (char **)malloc_check("hold path elements",
                            strlen(path) * sizeof(*vector));
  const char *p = path;
  int next = 0;
  while (p) {
#ifdef _WIN32
    char *q =
        strchr(const_cast<char *>(p), ';');  // windows uses ; as delimiter
#else
    char *q = strchr(const_cast<char *>(p),
                     ':');  // linux and derivatives use : as delimiter
#endif
