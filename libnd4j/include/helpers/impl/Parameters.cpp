...
Parameters* Parameters::addIntParam(std::initializer_list<std::string> strings,
  ...
  if (s.size() != p.size())
    throw std::runtime_error(
        "addIntParam: number of keys and values should match");

  for (int e = 0; e < s.size(); e++) _intParams[s[e]] = p[e];
...
Parameters* Parameters::addBoolParam(std::initializer_list<std::string> strings,
                                     std::initializer_list<bool> params) {
  std::vector<std::string> s(strings);
  ...
  for (int e = 0; e < s.size(); e++) _boolParams[s[e]] = p[e];

  return this;
}

Parameters* Parameters::addArrayParam(std::string string,
                                      std::initializer_list<int> param) {
  _arrayParams[string] = std::vector<int>(param);
  return this;
}

Parameters* Parameters::addArrayParam(
