...
//////////////////////////////////////////////////////////////////////////
sd::NDArray* sd::MmulHelper::tensorDot(
    const sd::NDArray* A, const sd::NDArray* B,
...
//////////////////////////////////////////////////////////////////////////
sd::NDArray* sd::MmulHelper::tensorDot(const sd::NDArray* a,
                                       const sd::NDArray* b,
                                       const std::vector<int>& axes_0,
                                       const std::vector<int>& axes_1) {
  std::vector<int> permutAt, permutBt;
  std::vector<Nd4jLong> shapeAt, shapeBt;

  auto outShape = ShapeUtils::evalShapeForTensorDot(
      a, b, axes_0, axes_1, permutAt, permutBt, shapeAt, shapeBt);
  ...
  const NDArray* aPR = aP->isSameShape(shapeAt)
                           ? aP
                           : new NDArray(aP->reshape(aP->ordering(), shapeAt));
  const NDArray* bPR = bP->isSameShape(shapeAt)
                           ? bP
                           : new NDArray(bP->reshape(bP->ordering(), shapeBt));
...
//////////////////////////////////////////////////////////////////////////
void sd::MmulHelper::tensorDot(const sd::NDArray* a, const sd::NDArray* b,
                               sd::NDArray* c, const std::vector<int>& axes_a,
                               const std::vector<int>& axes_b,
                               const std::vector<int>& permutForC) {
  std::vector<int> permutAt, permutBt;
  std::vector<Nd4jLong> shapeAt, shapeBt;
  ShapeUtils::evalShapeForTensorDot(a, b, axes_a, axes_b, permutAt, permutBt,
                                    shapeAt, shapeBt);
  ...
  NDArray* cPR =
      cP->isSameShape(requiredCshape)
          ? cP
          : new NDArray(cP->reshape(cP->ordering(), requiredCshape, false));

  mmul(aPR, bPR, cPR, 1.0, 0.0);

  if (cPR->buffer() != cP->buffer() ||
      cPR->specialBuffer() !=
          cP->specialBuffer())  // this means both permute and reshape have been
                                // performed on c, cP always points on
                                // c->buffer()
    cP->assign(cPR);
...
//////////////////////////////////////////////////////////////////////////
void sd::MmulHelper::tensorDot(
    const NDArray* a, const NDArray* b, NDArray* c,
    const std::vector<std::vector<Nd4jLong>>& modifA,
    const std::vector<std::vector<Nd4jLong>>& modifB,
    const std::vector<std::vector<Nd4jLong>>& modifC) {
  NDArray *aPR(const_cast<NDArray*>(a)), *bPR(const_cast<NDArray*>(b));
  std::string whatToDoWithA, whatToDoWithB,
      whatToDoWithC;  // "" - nothing; "p" - permutation; "r" - reshaping; "pr"
                      // - permutation+reshaping; "rp" - reshaping/permutation,
                      // and so on; if another string is produced - throw
                      // exception
  ...
  for (const auto& arr : modifA)
    whatToDoWithA =
        (std::find(arr.begin(), arr.end(), 0) != arr.end())
            ? whatToDoWithA + "p"
            : whatToDoWithA +
                  "r";  // when 0 is present in arr then it is permutation
                        // array, otherwise - it is reshaping array
  ...
  if (!whatToDoWithA.empty())
    aPR = (whatToDoWithA[0] == 'p')
              ? new NDArray(a->permute(modifA[0]))
              : new NDArray(a->reshape(a->ordering(), modifA[0]));
  // first step for b array
  if (!whatToDoWithB.empty())
    bPR = (whatToDoWithB[0] == 'p')
              ? new NDArray(b->permute(modifB[0]))
      ...
      cArrs[i + 1] =
          (whatToDoWithC[i] == 'p')
              ...
              : new NDArray(cArrs[i]->reshape(
                    c->ordering(), modifC[i],
                    false));  // since we ignore first element in cArrs (that is
                              // cArrs[0]) then it is always equal to c
  }
...
//////////////////////////////////////////////////////////////////////////
NDArray* sd::MmulHelper::tensorDot(
    const sd::NDArray* a, const sd::NDArray* b,
    const std::vector<std::vector<Nd4jLong>>& modifA,
    const std::vector<std::vector<Nd4jLong>>& modifB) {
  NDArray *aPR(const_cast<NDArray*>(a)), *bPR(const_cast<NDArray*>(b));
  std::string whatToDoWithA,
      whatToDoWithB;  // "" - nothing; "p" - permutation only; "r" - reshaping
                      // only; "pr" - permutation+reshaping; "rp" -
...
//////////////////////////////////////////////////////////////////////////
sd::NDArray* MmulHelper::mmul(const sd::NDArray* A, const sd::NDArray* B,
                              sd::NDArray* C, const double alpha,
  ...
  if (A->lengthOf() == B->lengthOf() && isAVector && isBVector &&
      (aRank != 2 ||
       aRank == 2 &&
           (A->isSameShape(B) ||
            bRank == 1 && A->sizeAt(1) == 1)))  // (1x1x1 * 1x1) or (1x4 * 1*4)
                                                // or (4x1 * 4x1) or (4x1 * 4)
  ...
  // vector x matrix, A{M} x B{M,N} = C{N} -> reduce to matrix x matrix A2{1,M}
  // x B{M,N} = C2{1,N}, since there is no corresponding blas operation sgevm
  if (isAVector && bRank == 2) {
    NDArray* A2 = new NDArray(
        A->reshape(A->ordering(), {1, A->lengthOf()}));  // A{M} -> A2{1,M}
    NDArray* C2 =
        C ? new NDArray(C->reshape(C->ordering(), {1, C->lengthOf()}, false))
          : nullptr;                                          // C{N} -> C2{1,N}
...
//////////////////////////////////////////////////////////////////////////
void MmulHelper::matmul(const sd::NDArray* x, const sd::NDArray* y,
                        sd::NDArray* z, const bool transX, const bool transY,
                        double alpha, double beta) {
  int xRank = x->rankOf();
  int yRank = y->rankOf();

  auto outShape = ShapeUtils::evalShapeForMatmul(x->shapeInfo(), y->shapeInfo(),
                                                 transX, transY);
  if (!z->isSameShape(outShape)) {
    nd4j_printf(
        "NDArrayFactory::matmul static method: input shape of output array is "
        "wrong, actual is %s and expected is %s ! \n",
  ...
  if (xRank <= 2 &&
      yRank <= 2) {  // dot (1Dx1D), vector-matrix (1Dx2D), matrix-vector
                     // (2Dx1D), matrix-matrix (2Dx2D) product cases

    if (xRank == 1 &&
        yRank == 2) {  // reduce vector-matrix to matrix-matrix case
      xT = new NDArray(
          x->reshape(x->ordering(),
                     {1, x->lengthOf()}));  // please note x is not transposed
                                            // in this case (since xRank=1)
    ...
    const Nd4jLong numOfSubArrs =
        ShapeUtils::getNumOfSubArrs(xT->shapeInfo(), dimsToExclude);
