...
//////////////////////////////////////////////////////////////////////////
// evaluate shape for array resulting from tensorDot operation, also evaluate
// shapes and dimensions permutations for transposition of two input arrays
std::vector<Nd4jLong> ShapeUtils::evalShapeForTensorDot(
    const Nd4jLong* aShapeInfo, const Nd4jLong* bShapeInfo,
    std::vector<int> axesA, std::vector<int> axesB, std::vector<int>& permutAt,
    std::vector<int>& permutBt, std::vector<Nd4jLong>& shapeAt,
    std::vector<Nd4jLong>& shapeBt) {
  ...
  if (axeAsize != axeBsize)
    throw std::runtime_error(
        "ShapeUtils::evalShapeForTensorDot method: the numbers of a axes and b "
        "axes to make dot product along must have identical values !");
  if (axeAsize > aRank || axeBsize > bRank)
    throw std::runtime_error(
        "ShapeUtils::evalShapeForTensorDot method: the length of vector of a "
        "or b axes is larger than array rank !");
  ...
  for (int i = 0; i < axeBsize; i++) {
    ...
    if (aShapeInfo[axesA[i] + 1] != bShapeInfo[axesB[i] + 1])
      throw std::runtime_error(
          "ShapeUtils::evalShapeForTensorDot method: the dimensions at given "
          "axes for both input arrays must be the same !");
  }
  ...
  if ((int)uniqueElems.size() != axeAsize)
    throw std::runtime_error(
        "ShapeUtils::evalShapeForTensorDot method: the vector of a axes "
        "contains duplicates !");
  uniqueElems.clear();
  uniqueElems = std::set<Nd4jLong>(axesB.begin(), axesB.end());
  if ((int)uniqueElems.size() != axeBsize)
    throw std::runtime_error(
        "ShapeUtils::evalShapeForTensorDot method: the vector of b axes "
        "contains duplicates !");
  ...
  // if permut contains something like {0,1,2,..rank-1}, then there is no need
  // to make permutation and we return empty vector in this case
  uint i1, i2;
...
//////////////////////////////////////////////////////////////////////////
std::vector<Nd4jLong> ShapeUtils::evalShapeForTensorDot(
    const NDArray* a, const NDArray* b, const std::vector<int>& axesA,
    const std::vector<int>& axesB, std::vector<int>& permutAt,
    std::vector<int>& permutBt, std::vector<Nd4jLong>& shapeAt,
    std::vector<Nd4jLong>& shapeBt) {
  return evalShapeForTensorDot(a->shapeInfo(), b->shapeInfo(), axesA, axesB,
                               permutAt, permutBt, shapeAt, shapeBt);
}

//////////////////////////////////////////////////////////////////////////
// evaluate output shape for reduce operation when input shape is empty
const Nd4jLong* ShapeUtils::evalReduceShapeInfoEmpty(
    const char order, const std::vector<int>& vdimsToExclude,
    const Nd4jLong* shapeInfo, const sd::DataType dataType, const bool keepDims,
    sd::memory::Workspace* workspace) {
  auto dimsToExclude = vdimsToExclude;

  if (dimsToExclude.size() == 0) {  // return copy of input shape
    Nd4jLong* outShapeInfo = ShapeBuilders::copyShapeInfoAndType(
        shapeInfo, dataType, true, workspace);
  ...
  if (dimsToExclude.size() ==
      rank) {  // return scalar or shape filled with unities

    if (!keepDims)
      outShapeInfo = ShapeBuilders::createScalarShapeInfo(dataType, workspace);
    else
      outShapeInfo = ShapeBuilders::createShapeInfo(
          dataType, order, std::vector<Nd4jLong>(rank, 1), workspace);
  } else {
    ...
    outShapeInfo =
        ShapeBuilders::createShapeInfo(dataType, order, outShape, workspace);
  }
...
const Nd4jLong* ShapeUtils::evalReduceShapeInfo(
    const char order, const std::vector<int>& dimsToExclude, const NDArray& arr,
    const bool keepDims, const bool supportOldShapes,
    sd::memory::Workspace* workspace) {
  return evalReduceShapeInfo(order, dimsToExclude, arr, arr.dataType(),
                             keepDims, supportOldShapes, workspace);
}

const Nd4jLong* ShapeUtils::evalReduceShapeInfo(
    const char order, const std::vector<int>& dimsToExclude,
    const Nd4jLong* shapeInfo, const bool keepDims, const bool supportOldShapes,
    sd::memory::Workspace* workspace) {
  return evalReduceShapeInfo(order, dimsToExclude, shapeInfo,
                             ArrayOptions::dataType(shapeInfo), keepDims,
...
const Nd4jLong* ShapeUtils::evalReduceShapeInfo(
    const char order, const std::vector<int>& dimsToExclude, const NDArray& arr,
    const sd::DataType dataType, const bool keepDims,
    const bool supportOldShapes, sd::memory::Workspace* workspace) {
  return evalReduceShapeInfo(order, dimsToExclude, arr.shapeInfo(), dataType,
                             keepDims, supportOldShapes, workspace);
}
...
const Nd4jLong* ShapeUtils::evalReduceShapeInfo(
  ...
  if (ArrayOptions::arrayType(shapeInfo) == ArrayType::EMPTY)
    return ShapeUtils::evalReduceShapeInfoEmpty(order, dimsToExclude, shapeInfo,
                                                dataType, keepDims, workspace);
  ...
  if (dimsToExclude.size() ==
      0) {  // return scalar or array with len=1 in this case
  ...
  if (keepDims) {
    ...
    for (int i = 0; i < rank; ++i)
      if (std::binary_search(dimsToExclude.begin(), dimsToExclude.end(),
                             i))  // dimsToExclude is already sorted after
                                  // shape::checkDimensions() has been applied
        newShapeInfo[i + 1] = 1;
      else
  ...
  if (newRank == 0 ||
      (dimSize == 1 &&
       dimsToExclude[0] == INT_MAX)) {  // check whether given dimension is
                                        // meant for the whole dimension

    if (supportOldShapes) {
      ALLOCATE(newShapeInfo, workspace, shape::shapeInfoLength(2), Nd4jLong);
      shape::shapeOldScalar(ArrayOptions::dataType(shapeInfo), newShapeInfo,
                            'c');
      ShapeDescriptor descriptor(newShapeInfo, dataType);
      RELEASE(newShapeInfo, workspace);
      return ConstantShapeHelper::getInstance()
  ...
  if (newRank == 1 && supportOldShapes) {
    ...
    RELEASE(newShapeInfo, workspace);
    ALLOCATE(newShapeInfo, workspace, shape::shapeInfoLength(2),
             Nd4jLong);  // set newRank = 2
    newShapeInfo[0] = 2;
...
std::vector<Nd4jLong> ShapeUtils::evalRepeatShape(
    int axis, const std::vector<int>& repeats, const NDArray& arr) {
  if (axis < 0) axis += arr.rankOf();

  if (repeats.size() != 1 && repeats.size() != arr.sizeAt(axis))
    throw std::invalid_argument(
        "ShapeUtils::evalRepeatShape: size of repeats vector must be 1 or "
        "equal to dimension at given axis !");
...
const Nd4jLong* ShapeUtils::evalPermShapeInfo(const int* dimensions,
                                              const int rank,
                                              const NDArray& arr,
                                              sd::memory::Workspace* workspace,
  ...
  if (!arr.nonNull())
    throw std::runtime_error(
        "ShapeUtils::evalPermShapeInfo static method: wrong arguments: array "
        "is nullptr!");

  if (rank != arr.rankOf())
    throw std::runtime_error(
        "ShapeUtils::evalPermShapeInfo static method: wrong arguments: rank is "
        "not suitable!");
...
//////////////////////////////////////////////////////////////////////////
// evaluate shapeInfo of permuted array
const Nd4jLong* ShapeUtils::evalPermShapeInfo(
    const Nd4jLong* dimensions, const int rank, const NDArray& arr,
    sd::memory::Workspace* workspace) {
...
const Nd4jLong* ShapeUtils::evalTranspShapeInfo(
    const NDArray& arr, sd::memory::Workspace* workspace,
    const bool setContigStrides) {
  int rank = arr.rankOf();
...
//////////////////////////////////////////////////////////////////////////
bool ShapeUtils::copyVectorPart(std::vector<int>& target,
                                std::vector<int>& source, int rank,
                                int offset) {
...
//////////////////////////////////////////////////////////////////////////
// return new (shorter) sorted dimensions array without dimensions that are
// present in input vector
std::vector<int> ShapeUtils::evalDimsToExclude(const int rank,
                                               const int dimsLen,
                                               const int* dimensions) {
  std::vector<int> newDimensions;
  if (dimsLen == 0) {  // if input vector is empty then return whole shape range
    newDimensions.resize(rank);
    std::iota(newDimensions.begin(), newDimensions.end(),
              0);  // fill with 0, 1, ... rank-1
  } else {
    bool isAbsent;
    for (int i = 0; i < rank; ++i) {
      isAbsent = true;
...
//////////////////////////////////////////////////////////////////////////
std::vector<int> ShapeUtils::evalDimsToExclude(
    const int rank, const std::vector<int>& dimensions) {
  return ShapeUtils::evalDimsToExclude(rank, dimensions.size(),
                                       dimensions.data());
}
...
bool ShapeUtils::areShapesBroadcastable(const NDArray& arr1,
                                        const NDArray& arr2) {
  return areShapesBroadcastable(arr1.shapeInfo(), arr2.shapeInfo());
}

bool ShapeUtils::areShapesBroadcastable(const Nd4jLong* shapeInfo1,
                                        const Nd4jLong* shapeInfo2) {
  int minRank = shape::rank(shapeInfo1) < shape::rank(shapeInfo2)
                    ? shape::rank(shapeInfo1)
                    : shape::rank(shapeInfo2);

  for (int i = -1; i >= -minRank; --i)
    if (shape::sizeAt(shapeInfo1, i) != shape::sizeAt(shapeInfo2, i) &&
        shape::sizeAt(shapeInfo1, i) != 1 && shape::sizeAt(shapeInfo2, i) != 1)
      return false;

  return true;
}

bool ShapeUtils::areShapesBroadcastable(const std::vector<Nd4jLong>& shape1,
                                        const std::vector<Nd4jLong>& shape2) {
  const auto rank1 = shape1.size();
  const auto rank2 = shape2.size();
  const int minRank = rank1 < rank2 ? rank1 : rank2;

  for (int i = 1; i <= minRank; ++i)
    if (shape1[rank1 - i] != shape2[rank2 - i] && shape1[rank1 - i] != 1 &&
        shape2[rank2 - i] != 1)
      return false;
...
//////////////////////////////////////////////////////////////////////////
// check the possibility of broadcast operation, if true then return shapeInfo
// of resulting array if evalMinMax == false the array with larger rank has to
// be passed as first argument
bool ShapeUtils::evalBroadcastShapeInfo(const NDArray& max, const NDArray& min,
                                        const bool evalMinMax,
                                        const Nd4jLong*& resultShapeInfo,
                                        sd::memory::Workspace* workspace) {
  return evalBroadcastShapeInfo(max.shapeInfo(), min.shapeInfo(), evalMinMax,
                                resultShapeInfo, workspace);
}

bool ShapeUtils::evalBroadcastShapeInfo(const Nd4jLong* max,
                                        const Nd4jLong* min,
                                        const bool evalMinMax,
                                        const Nd4jLong*& resultShapeInfo,
                                        sd::memory::Workspace* workspace) {
  ...
  if (resultShapeInfo != nullptr)
    throw std::runtime_error(
        "std::runtime_error(ShapeUtils::evalBroadcastShapeInfo method: the "
        "input pointer on shapeInfo must be empty (=nullptr) !");
  ...
  for (int i = 0; i < minRank; ++i)
    if ((maxShapeInfo[maxRank - i] != 0 &&
         maxShapeInfo[maxRank - i] < minShapeInfo[minRank - i]) ||
        minShapeInfo[minRank - i] == 0)
      tmpShapeInfo[maxRank - i] = minShapeInfo[minRank - i];

  ShapeUtils::updateStridesAndType(
      tmpShapeInfo,
      DataTypeUtils::pickPairwiseResultType(maxShapeInfo, minShapeInfo),
      shape::order(maxShapeInfo));

  if (shape::isEmpty(max) || shape::isEmpty(min)) {
    ArrayOptions::setPropertyBit(tmpShapeInfo, ARRAY_EMPTY);
    memset(shape::stride(tmpShapeInfo), 0,
           shape::rank(tmpShapeInfo) * sizeof(Nd4jLong));
  }
...
//////////////////////////////////////////////////////////////////////////
// check the possibility of broadcast operation for set of arrays, if true then
// return resulting broadcasted shapeInfo
bool ShapeUtils::evalCommonBroadcastShapeInfo(
    const std::vector<const NDArray*>& arrays, Nd4jLong*& resultShapeInfo,
    memory::Workspace* workspace) {
  if (resultShapeInfo != nullptr)
    throw std::runtime_error(
        "ShapeUtils::evalCommonBroadcastShapeInfo method: the input pointer on "
        "shapeInfo must be empty (=nullptr) !");
...
//////////////////////////////////////////////////////////////////////////
// return sorted vector of dimensions common (same) for two arrays, dimensions
// values corresponds to array with bigger rank for example if arr1{2,7},
// arr2{2,5,4,7} then vector = {0,3}
std::vector<int> ShapeUtils::getDimsWithSameShape(const NDArray& arr1,
                                                  const NDArray& arr2) {
...
const Nd4jLong* ShapeUtils::evalTileShapeInfo(
    const NDArray& arr, const std::vector<Nd4jLong>& reps,
    sd::memory::Workspace* workspace) {
  // check whether reps contains at least one zero (then throw exception) or
  // whether all elements in reps are unities (then simply reshape or do
  // nothing)
  ...
  if (product == 0)
    throw std::runtime_error(
        "NDArray::tile method: one of the elements in reps array is zero !");
  ...
  Nd4jLong* newShapeInfo = nullptr;
  if (diff < 0) {
    ALLOCATE(newShapeInfo, workspace, shape::shapeInfoLength(repsSize),
             Nd4jLong);
    newShapeInfo[0] = repsSize;  // set new rank
    for (int i = 1; i <= -diff; ++i)
      newShapeInfo[i] = 1;  // set unities to be new dimensions at left-hand
                            // side of newShapeInfo shape place
    memcpy(
        newShapeInfo + 1 - diff, arr.shapeInfo() + 1,
        rankOld * sizeof(Nd4jLong));  // copy old dimensions to the right-hand
                                      // side of newShapeInfo shape place
    for (int i = 1; i <= repsSize; ++i)
      newShapeInfo[i] *=
          reps[i - 1];  // set new shape by multiplying old dimensions by
                        // corresponding numbers from reps
  } else {
    ALLOCATE(newShapeInfo, workspace, shape::shapeInfoLength(rankOld),
             Nd4jLong);
    memcpy(newShapeInfo, arr.shapeInfo(),
           shape::shapeInfoByteLength(
               rankOld));  // copy all elements of _shapeInfo to newShapeInfo
    for (int i = 1; i <= repsSize; ++i)
      newShapeInfo[rankOld + 1 - i] *=
          reps[repsSize - i];  // set new shape by multiplying old dimensions by
                               // corresponding numbers from reps
  }
...
std::string ShapeUtils::shapeAsString(const Nd4jLong* shapeInfo) {
  if (!shapeInfo)
    throw std::runtime_error(
        "ShapeUtils::shapeAsString method: input shapeInfo must not be nullptr "
        "!");
...
std::string ShapeUtils::shapeAsString(const int rank,
                                      const Nd4jLong* shapeInfo) {
  if (!shapeInfo)
    throw std::runtime_error(
        "ShapeUtils::shapeAsString method: input shapeInfo must not be nullptr "
...
std::vector<Nd4jLong> ShapeUtils::shapeAsVector(const Nd4jLong* shapeInfo) {
  if (!shapeInfo)
    throw std::runtime_error(
        "ShapeUtils::shapeAsVector method: input shapeInfo must not be nullptr "
        "!");

  std::vector<Nd4jLong> vector(shapeInfo[0]);

  for (uint e = 0; e < shapeInfo[0]; e++) vector[e] = shapeInfo[e + 1];

  return vector;
}

//////////////////////////////////////////////////////////////////////////
// evaluate shapeInfo for diagonal array which is made using input arr elements
// as diagonal
const Nd4jLong* ShapeUtils::evalDiagShapeInfo(
    const Nd4jLong* shapeInfoConst, sd::memory::Workspace* workspace) {
  ...
  if (shape::isVector(shapeInfo) || shape::isScalar(shapeInfo)) {
    ...
    ALLOCATE(outputShapeInfo, workspace, shape::shapeInfoLength(2 * rank),
             Nd4jLong);
  ...
  ShapeUtils::updateStridesAndType(outputShapeInfo, shapeInfo,
...
std::vector<int> ShapeUtils::evalBroadcastBackwardAxis(
    const Nd4jLong* operandShapeInfo, const Nd4jLong* resultShapeInfo) {
  // rRank >= oRank always  !!
  ...
  for (int i = 0; i < rRank; ++i)
    if (i < diff || shape::sizeAt(operandShapeInfo, i - diff) !=
                        shape::sizeAt(resultShapeInfo, i))
      axis.push_back(i);
...
////////////////////////////////////////////////////////////////////////////////
const Nd4jLong* ShapeUtils::matrixProductShape(
    const Nd4jLong* theFirstShape, const Nd4jLong* theSecondShape,
    bool shouldTranspondFirst, bool shouldTranspondSecond, sd::DataType dtype,
    sd::memory::Workspace* workspace) {
  ...
  if (shape::rank(tmpA) == 1 && shape::isMatrix(tmpB)) {
    ...
    shape[1] = tmpB[2];
    Nd4jLong* newShape =
        ShapeBuilders::createShapeInfo(dtype, 'f', 2, shape, workspace);
    ...
    if (shape::rank(tmpB) == 2) {
      ...
      auto newShape =
          ShapeBuilders::createVectorShapeInfo(dtype, tmpA[1], workspace);
    ...
    // element-wise
    shape[0] = 1;
    shape[1] = (int)sd::math::nd4j_max<Nd4jLong>(shape::length(tmpA),
                                                 shape::length(tmpB));
...
////////////////////////////////////////////////////////////////////////////////
std::vector<int> ShapeUtils::evalPermutFromTo(
    const std::vector<Nd4jLong>& shapeFrom,
    const std::vector<Nd4jLong>& shapeTo) {
  auto rank = shapeFrom.size();
  if (rank != shapeTo.size())
    throw std::runtime_error(
        "ShapeUtils::evalPermutFromTo static method: the input shapes are not "
        "suitable for mutual permutation !");

  if (std::equal(begin(shapeFrom), end(shapeFrom),
                 begin(shapeTo)))  // if shapes are identical (permutation is
                                   // unnecessary) then return empty vector
    return std::vector<int>();

  std::vector<int> permutation(rank, -2);  // vector to be returned
  std::vector<Nd4jLong> shapeTo2(
      shapeTo);  // make copy of const vector since we will change the content
                 // of shapeTo
  ...
  for (int i = 0; i < rank; ++i)
    for (int j = 0; j < rank; ++j)
      if (shapeFrom[i] == shapeTo2[j]) {
        permutation[j] = i;
        shapeTo2[j] = -2;  // mark coincidence as -2 in order to not account
                           // index of shapeTo twice
        break;
      }

  if (std::find(begin(permutation), end(permutation), -2) !=
      end(permutation))  // if -2 is still present in vector then permutation is
                         // impossible
    throw std::runtime_error(
        "ShapeUtils::evalPermutFromTo static method: the input shapes are not "
        "suitable for mutual permutation !");
...
////////////////////////////////////////////////////////////////////////////////
std::vector<Nd4jLong> ShapeUtils::composeShapeUsingDimsAndIdx(
    const std::vector<int>& dimsAndIdx) {
  auto size = dimsAndIdx.size();
  if (size % 2 != 0)
    throw std::runtime_error(
        "ShapeUtils::composeShapeUsingDimsAndIdx static method: the size of "
        "input vector must be even !");
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (index > size - 1)
      throw std::runtime_error(
          "ShapeUtils::composeShapeUsingDimsAndIdx static method: input index "
          "is too large !");
...
////////////////////////////////////////////////////////////////////////////////
std::vector<Nd4jLong> ShapeUtils::evalShapeForMatmul(const Nd4jLong* xShapeInfo,
  ...
  if (xRank == 1 && yRank == 1) {  // dot case, output is scalar
    if (xShapeInfo[1] != yShapeInfo[1]) {
      nd4j_printf(
          "ShapeUtils::evalShapeForMatmul method: since input arrays are "
          "vectors they must have the same length, but got x length = %i, y "
          "length = %i !",
  ...
  if (xRank == 1 &&
      yRank ==
          2) {  // vector x matrix, i.e. [4] x [4,5] = [5], output is vector
    if (xShapeInfo[1] != y0Dim) {
      nd4j_printf(
          "ShapeUtils::evalShapeForMatmul method: input arrays have "
          "inconsistent shapes for vector-matrix product: x %s, y %s !",
          ShapeUtils::shapeAsString(xShapeInfo).c_str(),
          ShapeUtils::shapeAsString(yShapeInfo).c_str());
      throw std::invalid_argument("");
    }
  ...
  if (xRank == 2 &&
      yRank ==
          1) {  // matrix x vector , i.e. [4,5] x [5] = [4], output is vector
  ...
  if (xRank != yRank) {
    nd4j_printf(
        "ShapeUtils::evalShapeForMatmul static method: the ranks of arrays "
        "must be the same, but got xRank = %i and yRank = %i ! \n",
        xRank, yRank);
    throw std::invalid_argument("");
  }
  ...
  if (x1Dim != y0Dim) {
    nd4j_printf(
        "ShapeUtils::evalShapeForMatmul static method: input shapes are "
        "inconsistent: xDim %i != yDim %i \n",
        x1Dim, y0Dim);
    throw std::invalid_argument("");
  }
  ...
  for (int i = 0; i < xRank - 2; ++i)
    if (xShapeInfo[i + 1] != yShapeInfo[i + 1]) {
      nd4j_printf(
          "ShapeUtils::evalShapeForMatmul static method: input shapes are "
          "inconsistent: xShape = %s, yShape = %s ! \n",
          ShapeUtils::shapeAsString(xShapeInfo).c_str(),
          ShapeUtils::shapeAsString(yShapeInfo).c_str());
      throw std::invalid_argument("");
    }
...
////////////////////////////////////////////////////////////////////////////////
Nd4jLong ShapeUtils::getNumOfSubArrs(const Nd4jLong* shapeInfo,
                                     const std::vector<int>& dimsToExclude) {
  Nd4jLong numOfSubArrs = 1;

  if (dimsToExclude.size() == shape::rank(shapeInfo) ||
      dimsToExclude.size() == 0)  // means there is only one sub-array and it
                                  // coincides with whole array
    return numOfSubArrs;
...
////////////////////////////////////////////////////////////////////////////////
std::vector<Nd4jLong> ShapeUtils::evalDimsWithoutUnities(
    const Nd4jLong* shapeInfo) {
...
////////////////////////////////////////////////////////////////////////////////
void ShapeUtils::updateStridesAndType(Nd4jLong* dest, const Nd4jLong* source,
                                      const char order) {
...
////////////////////////////////////////////////////////////////////////////////
void ShapeUtils::updateStridesAndType(Nd4jLong* dest, const DataType dtype,
                                      const char order) {
  shape::updateStrides(dest, order);
...
////////////////////////////////////////////////////////////////////////////////
std::vector<int> ShapeUtils::tadAxesForSimpleBroadcast(const NDArray& max,
                                                       const NDArray& min) {
...
void ShapeUtils::copyCertainStridesFromShapeInfo(const Nd4jLong* inShapeInfo,
                                                 const int nRank,
                                                 const int dimsSize,
                                                 const int* dims,
                                                 Nd4jLong* outStrides) {
...
bool ShapeUtils::areShapesEqual(const Nd4jLong* shapeInfo,
...
std::vector<int>& sameDims) {
    ...
    for (int iMax = -1, iMin = -1; iMax >= -max->rankOf() && iMin >=
-min->rankOf(); ) {
