...
void DebugHelper::retrieveDebugStatistics(DebugInfo* info,
                                          NDArray const* input) {
  ...
  if (input->lengthOf() == 1) {  // scalar case
    ...
    info->_zeroCount =
        sd::math::nd4j_abs(input->e<double>(0)) > 0.00001 ? 0 : 1;
    ...
    auto _zeroCount =
        sd::math::nd4j_abs(input->e<double>(0)) > 0.00001 ? 0L : 1L;
    ...
    *info = {_minValue,      _maxValue,  _meanValue / input->lengthOf(),
    ...
    _stdDevValue = 0;  // math::nd4j_sqrt<double, double>(info->_stdDevValue /
                       // (input->lengthOf() - 1));
    ...
    auto func = PRAGMA_REDUCE_DOUBLE {
      ...
      for (auto e = start; e < stop; e++) {
        ...
        _stdDevValue += (info->_meanValue - current) *
                        (info->_meanValue - current);  // info->_minValue;
      }
    ...
    info->_stdDevValue =
        math::nd4j_sqrt<double, double>(_stdDevValue / input->lengthOf());
  }
