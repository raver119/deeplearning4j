...
//////////////////////////////////////////////////////////////////////////
void GradCheck::fillGradArrays(const LossFunc loss,
                               const std::vector<NDArray*>& gradArrs) {
  ...
  switch (loss) {
    ...
    default:
      throw std::invalid_argument(
          "GradCheck::fillGradArrays: invalid type of loss function !");
  }
}

//////////////////////////////////////////////////////////////////////////
bool GradCheck::checkGrad(ops::DeclarableOp& opFF, ops::DeclarableOp& opBP,
                          const OpArgsHolder& argsHolderFF,
                          const OpArgsHolder& argsHolderBP,
                          const std::vector<bool>& whatArrsToCheck,
                          const std::vector<double>& idxRange,
                          const LossFunc loss) {
  const int numInArrsFF =
      argsHolderFF.getNumInArrs();  // at the same time numInArrsFF = number of
                                    // output arrays in opBP
  const int numInGradArrsBP =
      argsHolderBP.getNumInArrs() -
      numInArrsFF;  // because argsHolderBP.getNumInArrs() = numInArrsFF +
                    // numInGradArrsBP
  ...
  fillGradArrays(
      loss, std::vector<NDArray*>(&inArrsBP[numInArrsFF],
                                  &inArrsBP[numInArrsFF + numInGradArrsBP]));

  // back prop pass
  ResultSet outArrsBP = opBP.execute(
      argsHolderBP);  // number of output arrays in back prop = numInArrsFF;

  NDArray tmpScalar(sd::DataType::DOUBLE,
                    inArrsFF[0]->getContext());  // scalar = 0

  for (int i = 0; i < numInArrsFF; ++i) {  // loop through input array

    if (!whatArrsToCheck.empty() &&
        static_cast<bool>(whatArrsToCheck[i]) == false)
      continue;

    const Nd4jLong idxStart =
        static_cast<Nd4jLong>(idxRange[0] * inArrsFF[i]->lengthOf());
    const Nd4jLong idxEnd =
        static_cast<Nd4jLong>(idxRange[1] * inArrsFF[i]->lengthOf());

    for (Nd4jLong j = idxStart; j < idxEnd;
         ++j) {  // loop through all elements for current array
      ...
      if (std::isnan(numericalGrad) || std::isinf(numericalGrad)) {
        printf(
            "GradCheck::checkGrad: got wrong value for numerical gradient for "
      ...
      if (std::isnan(analyticGrad) || std::isinf(analyticGrad)) {
        printf(
            "GradCheck::checkGrad: got wrong value for analytical gradient for "
            "input array # %i and its element at position %lld ! \n",
            i, j);
        throw std::runtime_error("");
      }
      ...
      if (numericalGrad == 0. && analyticGrad == 0.)
        ...
        relError = math::nd4j_abs<double>(analyticGrad - numericalGrad) /
                   (math::nd4j_abs<double>(analyticGrad) +
                    math::nd4j_abs<double>(numericalGrad));
      ...
      if (relError > MAXRELERR || std::isnan(relError)) {
        ...
        printf("numericalGrad = %.15f,  analyticGrad = %.15f \n", numericalGrad,
               analyticGrad);
        printf(
            "GradCheck::checkGrad: got RELERROR = %f > MAXRELERROR(%f) for "
            "input array # %i and its element at position %lld ! \n",
