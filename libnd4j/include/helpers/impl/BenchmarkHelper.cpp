...
namespace sd {
BenchmarkHelper::BenchmarkHelper(unsigned int warmUpIterations,
                                 unsigned int runIterations) {
...
std::string BenchmarkHelper::printHeader() {
  return std::string(
      "TestName\tOpNum\tWarmup\tNumIter\tDataType\tInplace\tShape\tStrides\tAxi"
      "s\tOrders\tavg (us)\tmedian (us)\tmin (us)\tmax (us)\tstdev (us)\n");
}

std::string BenchmarkHelper::benchmarkOperation(OpBenchmark &benchmark) {
  for (uint i = 0; i < _wIterations; i++) benchmark.executeOnce();
  ...
  auto stdev =
      n.varianceNumber(sd::variance::SummaryStatsStandardDeviation, false)
          .e<double>(0);
  ...
  // printing out stuff
  snprintf(
      const_cast<char *>(temp.data()), temp.length(),
      "%s\t%i\t%i\t%i\t%s\t%s\t%s\t%s\t%s\t%s\t%lld\t%lld\t%lld\t%lld\t%.2f\n",
      benchmark.testName().c_str(), benchmark.opNum(), _wIterations,
      _rIterations, t.c_str(), inpl.c_str(), s.c_str(), strides.c_str(),
      a.c_str(), o.c_str(), sd::math::nd4j_floor<double, Nd4jLong>(sumT),
...
void BenchmarkHelper::benchmarkScalarOperation(scalar::Ops op,
                                               std::string testName,
                                               double value, NDArray &x,
                                               NDArray &z) {
  auto y = NDArrayFactory::create(x.dataType(), value);

  // for (uint i = 0; i < _wIterations; i++)
  // NativeOpExecutioner::execScalar(op, x.buffer(), x.shapeInfo(), z.buffer(),
  // z.shapeInfo(), y.buffer(), y.shapeInfo(), nullptr);
  ...
  for (uint i = 0; i < _rIterations; i++) {
    ...
    // NativeOpExecutioner::execScalar(op, x.buffer(), x.shapeInfo(),
    // z.buffer(), z.shapeInfo(), y.buffer(), y.shapeInfo(), nullptr);
  ...
  // printing out stuff
  nd4j_printf(
      "%s\t%i\t%i\t%i\t%s\t%s\t%s\t%s\t%s\tn/a\t%lld\t%lld\t%lld\t%lld\t%.2f\n",
      testName.c_str(), op, _wIterations, _rIterations, t.c_str(), inpl.c_str(),
      s.c_str(), stride.c_str(), o.c_str(),
      sd::math::nd4j_floor<double, Nd4jLong>(sumT), median, min, max, stdev);
}

std::string BenchmarkHelper::runOperationSuit(
    std::initializer_list<OpBenchmark *> benchmarks, const char *msg) {
...
std::string BenchmarkHelper::runOperationSuit(
    std::vector<OpBenchmark *> &benchmarks, bool postHeaders, const char *msg) {
  std::string result;
  ...
  if (postHeaders) result += printHeader();

  for (auto v : benchmarks) result += benchmarkOperation(*v);

  return result;
}

std::string BenchmarkHelper::runOperationSuit(
    DeclarableBenchmark *op, const std::function<Context *(Parameters &)> &func,
    ParametersBatch &parametersBatch, const char *message) {
  ...
  for (auto &p : parameters) {
    ...
    if (x.size() != z.size())
      throw std::runtime_error(
          "ScalarBenchmark: number of X and Z arrays should match");
...
std::string BenchmarkHelper::runOperationSuit(
    ScalarBenchmark *op,
    const std::function<void(ResultSet &, ResultSet &)> &func,
    const char *message) {
  std::string output;
...
std::string BenchmarkHelper::runOperationSuit(
  ...
  for (auto &p : parameters) {
    ...
    if (x.size() != z.size())
      throw std::runtime_error(
          "TransformBenchmark: number of X and Z arrays should match");
...
std::string BenchmarkHelper::runOperationSuit(
  ...
  for (auto &p : parameters) {
    ...
    if (x.size() != z.size())
      throw std::runtime_error(
          "ReductionBenchmark: number of X and Z arrays should match");
...
std::string BenchmarkHelper::runOperationSuit(
  ...
  for (auto &p : parameters) {
    ...
    if (x.size() != z.size())
      throw std::runtime_error(
          "BroadcastBenchmark: number of X and Z arrays should match");
...
std::string BenchmarkHelper::runOperationSuit(
  ...
  for (auto &p : parameters) {
    ...
    if (x.size() != z.size() || x.size() != y.size())
      throw std::runtime_error(
          "PairwiseBenchmark: number of X and Z arrays should match");
