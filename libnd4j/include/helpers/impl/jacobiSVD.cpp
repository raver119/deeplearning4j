...
void JacobiSVD<T>::mulRotationOnLeft(const int i, const int j, NDArray& block,
  ...
  if (i < j) {
    ...
    NDArray temp(block.ordering(), {2, block.sizeAt(1)},
...
void JacobiSVD<T>::mulRotationOnRight(const int i, const int j, NDArray& block,
  ...
  if (i < j) {
    ...
    NDArray temp(block.ordering(), {block.sizeAt(0), 2},
                                       block.dataType(), block.getContext());
...
bool JacobiSVD<T>::isBlock2x2NotDiag(NDArray& block, int p, int q, T& maxElem) {
  NDArray rotation(_m.ordering(), {2, 2}, _m.dataType(),
                                         _m.getContext());
  T n = math::nd4j_sqrt<T, T>(block.t<T>(p, p) * block.t<T>(p, p) +
                              block.t<T>(q, p) * block.t<T>(q, p));
  ...
  maxElem = math::nd4j_max<T>(
      maxElem, math::nd4j_max<T>(math::nd4j_abs<T>(block.t<T>(p, p)),
                                 math::nd4j_abs<T>(block.t<T>(q, q))));
...
void JacobiSVD<T>::svd2x2(const NDArray& block, int p, int q, NDArray& left,
                          NDArray& right) {
  NDArray m(block.ordering(), {2, 2}, block.dataType(),
                                  block.getContext());
  ...
  NDArray rotation(block.ordering(), {2, 2},
                                         block.dataType(), block.getContext());
  ...
  if (math::nd4j_abs<T>(d) < DataTypeUtils::min<T>()) {
    rotation.r<T>(0, 0) =
    rotation.r<T>(1, 1) = (T)1;
    rotation.r<T>(0, 1) =
    rotation.r<T>(1, 0) = (T)0;
  } else {
    T u = t / d;
    T tmp = math::nd4j_sqrt<T, T>((T)1.f + u * u);
    rotation.r<T>(0, 0) =
    rotation.r<T>(1, 1) = u / tmp;
  ...
  while (!stop) {
    ...
    for (int p = 1; p < _diagSize; ++p) {
      for (int q = 0; q < p; ++q) {
        ...
        if (math::nd4j_abs<T>(_m.t<T>(p, q)) > threshold ||
            math::nd4j_abs<T>(_m.t<T>(q, p)) > threshold) {
          stop = false;
          ...
          // if(isBlock2x2NotDiag(_m, p, q, maxDiagElem))
          {
            ...
            maxDiagElem = math::nd4j_max<T>(
                maxDiagElem,
                math::nd4j_max<T>(math::nd4j_abs<T>(_m.t<T>(p, p)),
                                  math::nd4j_abs<T>(_m.t<T>(q, q))));
          }
        }
      }
    }
