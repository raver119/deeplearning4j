...
class SD_EXPORT RandomBuffer {
 ...
 public:
   ...
   */
#ifdef __CUDACC__
  __host__ RandomBuffer(Nd4jLong seed, Nd4jLong size, uint64_t *hostBuffer,
                        uint64_t *devBuffer) {
  ...
  __host__ Nd4jPointer getDevicePointer() {
    return reinterpret_cast<Nd4jPointer>(devHolder);
  }

  __host__ ~RandomBuffer() { cudaFree(devHolder); }

  __host__ void propagateToDevice(sd::random::RandomBuffer *buffer,
                                  cudaStream_t stream) {
    cudaMemcpyAsync(devHolder, buffer, sizeof(sd::random::RandomBuffer),
  ...
  inline _CUDA_HD uint64_t *getBuffer() { return this->buffer; }

  inline _CUDA_HD uint64_t *getDeviceBuffer() { return this->devBuffer; }

#ifdef __CUDACC__
  _CUDA_HD curandGenerator_t *getGeneratorPointer() { return &gen; }

  _CUDA_HD curandGenerator_t getGenerator() { return gen; }

  _CUDA_H void setBuffer(uint64_t *ptr) { this->buffer = ptr; }
#endif

  inline _CUDA_HD Nd4jLong getSize() { return this->size; }

  inline _CUDA_HD Nd4jLong getSeed() { return this->seed; }
  ...
  Nd4jLong _CUDA_HD getAllocatedSize() { return this->size * sizeof(double); }

  inline _CUDA_HD Nd4jLong getOffset() { return this->currentPosition; }

  void _CUDA_HD setOffset(Nd4jLong offset) { this->currentPosition = offset; }

  void _CUDA_HD reSeed(Nd4jLong amplifier) { this->amplifier = amplifier; }
...
//                __syncthreads();
  ...
  uint64_t _CUDA_HD next64(uint64_t shiftedSeed) {
    ...
    auto s1 =
        static_cast<uint64_t>(shiftedSeed) % sd::DataTypeUtils::max<int>() + 11;
    uint64_t r0, r1;
  ...
  void _CUDA_HD incrementGeneration() { this->generation++; }
  ...
  uint64_t _CUDA_HD getNextElement() {
    ...
    return generation == 1 ? buffer[getNextIndex()]
                           : buffer[getNextIndex()] * generation;
  }
  ...
  int _CUDA_D nextInt() {
    auto u = nextUInt64();
    return u <= sd::DataTypeUtils::max<int>()
               ? static_cast<int>(u)
               : static_cast<int>(u % sd::DataTypeUtils::max<int>());
  };

  uint64_t _CUDA_D nextUInt64() { return getNextElement(); }
  ...
  inline _CUDA_D T relativeT(Nd4jLong index) {
     ...
     *
     * FIXME: once we add support for additional datatypes this code must be
     * tweaked
     */
...
class SD_EXPORT IGenerator {
 ...
 public:
  ...
  _CUDA_HD RandomBuffer *getBuffer() { return realBuffer; }
