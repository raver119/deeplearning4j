...
/**
 * This class is a simple wrapper to represent batch arguments as single surface
 * of parameters. So we pass batch parameters as single surface, and then we use
 * this helper to extract arguments for each aggregates.
 *
 ...
 * [2] here we put block of JVM IntArrays by value, batchLimit * maxIntArrays *
 * maxArraySize; [3] then we put real arguments [4] then we put arguments
 * pointers [5] then we put shape pointers
 *
 */
template <typename T>
class PointersHelper {
 private:
  ...
  // we have 5 diff kinds of arguments: arguments, shapeArguments,
  // intArrayArguments, indexArguments, realArguments
  ...
  // since that's pointers (which is 64-bit on 64bit systems), we limit number
  // of maximum arguments to 1/2 of maxIndex arguments
  int maxArguments;
 ...
 public:
  ...
  __host__ __device__
#endif
  PointersHelper(void *ptrToParams, int numAggregates, int maxArgs,
                 int maxShapes, int maxIntArrays, int maxIntArraySize,
  ...
  /**
   * This method returns set of pointers to shape aruments for specified
   * aggregates
   *
   * @param aggregateIdx
   * @return
  ...
  /**
   * This method returns pointer to array of int/index arguments for specified
   * aggregate
   *
   * @param aggregateIdx
   * @return
  ...
  ptr_def int *getIntArrayArguments(int aggregateIdx, int argumentIdx) {
    ...
    return ptr + (aggregateIdx * maxIntArrays * maxArraySize) +
           (argumentIdx * maxArraySize);
  }
  ...
  ptr_def T *getRealArguments(int aggregateIdx) {
    // we get pointer for last batchElement + 1, so that'll be pointer for 0
    // realArgument
