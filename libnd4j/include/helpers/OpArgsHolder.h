...
class SD_EXPORT OpArgsHolder {
 ...
 public:
  ...
  // constructor
  OpArgsHolder(const std::vector<NDArray*>& inArrs,
               const std::vector<double>& tArgs = std::vector<double>(),
               const std::vector<Nd4jLong>& iArgs = std::vector<Nd4jLong>(),
  ...
  const std::vector<Nd4jLong>& getIArgs() const { return _iArgs; }
  ...
  OpArgsHolder createArgsHolderForBP(const std::vector<NDArray*>& inGradArrs,
                                     const bool isInPlace = false) const;
