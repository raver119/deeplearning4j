...
JacobiSVD<T>::JacobiSVD(const NDArray& matrix, const bool calcU,
  ...
  _s = NDArrayFactory::create(matrix.ordering(), {_diagSize, 1},
                              matrix.dataType(), matrix.getContext());

  if (_calcU) {
    if (_fullUV)
      _u = NDArrayFactory::create(matrix.ordering(), {_rows, _rows},
                                  matrix.dataType(), matrix.getContext());
    else
      _u = NDArrayFactory::create(matrix.ordering(), {_rows, _diagSize},
                                  matrix.dataType(), matrix.getContext());
  } else
    _u = NDArrayFactory::create(matrix.ordering(), {_rows, 1},
                                matrix.dataType(), matrix.getContext());

  if (_calcV) {
    if (_fullUV)
      _v = NDArrayFactory::create(matrix.ordering(), {_cols, _cols},
                                  matrix.dataType(), matrix.getContext());
    else
      _v = NDArrayFactory::create(matrix.ordering(), {_cols, _diagSize},
                                  matrix.dataType(), matrix.getContext());
  } else
    _v = NDArrayFactory::create(matrix.ordering(), {_cols, 1},
                                matrix.dataType(), matrix.getContext());

  _m = NDArrayFactory::create(matrix.ordering(), {_diagSize, _diagSize},
                              matrix.dataType(), matrix.getContext());
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void JacobiSVD<T>::mulRotationOnLeft(const int i, const int j, NDArray& block,
                                     const NDArray& rotation) {
  if (i < j) {
    if (j + 1 > block.sizeAt(0))
      throw std::runtime_error(
          "ops::helpers::JacobiSVD mulRotationOnLeft: second arguments is out "
          "of array row range !");
    ...
    if (j + 1 > block.sizeAt(0) || i + 1 > block.sizeAt(0))
      throw std::runtime_error(
          "ops::helpers::JacobiSVD mulRotationOnLeft: some or both integer "
          "arguments are out of array row range !");

    auto temp = NDArrayFactory::create(block.ordering(), {2, block.sizeAt(1)},
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void JacobiSVD<T>::mulRotationOnRight(const int i, const int j, NDArray& block,
                                      const NDArray& rotation) {
  if (i < j) {
    if (j + 1 > block.sizeAt(1))
      throw std::runtime_error(
          "ops::helpers::JacobiSVD mulRotationOnRight: second argument is out "
          "of array column range !");
    ...
    if (j + 1 > block.sizeAt(1) || i + 1 > block.sizeAt(1))
      throw std::runtime_error(
          "ops::helpers::JacobiSVD mulRotationOnRight: some or both integer "
          "arguments are out of array column range !");

    auto temp = NDArrayFactory::create(block.ordering(), {block.sizeAt(0), 2},
                                       block.dataType(), block.getContext());
...
bool JacobiSVD<T>::isBlock2x2NotDiag(NDArray& block, int p, int q, T& maxElem) {
  auto rotation = NDArrayFactory::create(_m.ordering(), {2, 2}, _m.dataType(),
                                         _m.getContext());
  T n = math::nd4j_sqrt<T, T>(block.e<T>(p, p) * block.e<T>(p, p) +
                              block.e<T>(q, p) * block.e<T>(q, p));
  ...
  maxElem = math::nd4j_max<T>(
      maxElem, math::nd4j_max<T>(math::nd4j_abs<T>(block.e<T>(p, p)),
                                 math::nd4j_abs<T>(block.e<T>(q, q))));
...
bool JacobiSVD<T>::createJacobiRotation(const T& x, const T& y, const T& z,
  ...
  if (denom < DataTypeUtils::min<T>()) {
    ...
    rotation.p(0, 1,
               -sign * (y / math::nd4j_abs<T>(y)) * math::nd4j_abs<T>(t) * n);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void JacobiSVD<T>::svd2x2(const NDArray& block, int p, int q, NDArray& left,
                          NDArray& right) {
  auto m = NDArrayFactory::create(block.ordering(), {2, 2}, block.dataType(),
                                  block.getContext());
  ...
  auto rotation = NDArrayFactory::create(block.ordering(), {2, 2},
                                         block.dataType(), block.getContext());
...
void JacobiSVD<T>::evalData(const NDArray& matrix) {
  ...
  if (_rows > _cols) {
    ...
    HHsequence hhSeg(qr._qr, qr._coeffs,
                     'u');  // type = 'u' is not mistake here !
    ...
    _m.assign(
        static_cast<const NDArray&>(matrix({0, _diagSize, 0, _diagSize})) /
        scale);
  ...
  while (!stop) {
    ...
    for (int p = 1; p < _diagSize; ++p) {
      for (int q = 0; q < p; ++q) {
        ...
        if (math::nd4j_abs<T>(_m.e<T>(p, q)) > threshold ||
            math::nd4j_abs<T>(_m.e<T>(q, p)) > threshold) {
          stop = false;

          // if(isBlock2x2NotDiag(_m, p, q, maxDiagElem))
          {
            auto rotLeft = NDArrayFactory::create(
                _m.ordering(), {2, 2}, _m.dataType(), _m.getContext());
            auto rotRight = NDArrayFactory::create(
                _m.ordering(), {2, 2}, _m.dataType(), _m.getContext());
            ...
            maxDiagElem = math::nd4j_max<T>(
                maxDiagElem,
                math::nd4j_max<T>(math::nd4j_abs<T>(_m.e<T>(p, p)),
                                  math::nd4j_abs<T>(_m.e<T>(q, q))));
          }
        }
      }
    }
  ...
  for (int i = 0; i < _diagSize; i++) {
    int pos =
        (_s({i, -1, 0, 0}).indexReduceNumber(indexreduce::IndexMax, nullptr))
            .template e<int>(0);
...
template class SD_EXPORT JacobiSVD<float>;
template class SD_EXPORT JacobiSVD<float16>;
template class SD_EXPORT JacobiSVD<bfloat16>;
template class SD_EXPORT JacobiSVD<double>;
