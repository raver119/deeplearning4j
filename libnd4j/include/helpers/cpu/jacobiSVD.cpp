...
bool JacobiSVD<T>::isBlock2x2NotDiag(NDArray& block, int p, int q, T& maxElem) {
    ...
    T& elemPP = block.t<T>(p, p);
    T& elemQQ = block.t<T>(q, q);
    T& elemQP = block.t<T>(q, p);
    T& elemPQ = block.t<T>(p, q);

    T n = math::nd4j_sqrt<T,T>(elemPP*elemPP  + elemQP*elemQP);
    ...
    if(n == (T)0.f) {
        ...
        rotation.t<T>(0,0) = rotation.t<T>(1,1) = v;

        v = elemQP / n;
        rotation.t<T>(0,1) = v;

        rotation.t<T>(1,0) = -rotation.template t<T>(0,1);
    ...
    maxElem = math::nd4j_max<T>(maxElem, math::nd4j_max<T>(math::nd4j_abs<T>(elemPP), math::nd4j_abs<T>(elemQQ)));
    T threshold = math::nd4j_max<T>(almostZero, precision * maxElem);

    return math::nd4j_abs<T>(elemPQ) > threshold || math::nd4j_abs<T>(elemQP) > threshold;
}
...
bool JacobiSVD<T>::createJacobiRotation(const T& x, const T& y, const T& z, NDArray& rotation) {
    ...
    if(denom < DataTypeUtils::min<T>()) {
        ...
        rotation.t<T>(0,0) = rotation.t<T>(1,1) = (T)1;
        rotation.t<T>(0,1) = rotation.t<T>(1,0) = (T)0;
...
void JacobiSVD<T>::svd2x2(const NDArray& block, int p, int q, NDArray& left, NDArray& right) {
    ...
    m.t<T>(0,0) = block.t<T>(p,p);
    m.t<T>(0,1) = block.t<T>(p,q);
    m.t<T>(1,0) = block.t<T>(q,p);
    m.t<T>(1,1) = block.t<T>(q,q);
    ...
    if(math::nd4j_abs<T>(d) < DataTypeUtils::min<T>()) {
        ...
        rotation.t<T>(0,0) = rotation.t<T>(1,1) = u / tmp;
        rotation.t<T>(0,1) =  1.f / tmp;
...
void JacobiSVD<T>::evalData(const NDArray& matrix) {
    ...
    for(int i = 0; i < _diagSize; ++i) {
        ...
        _s.t<T>(i) = math::nd4j_abs<T>(_m.t<T>(i,i));
    ...
    for(int i = 0; i < _diagSize; i++) {
        ...
        if(pos) {
            ...
            math::nd4j_swap<T>(_s.t<T>(i), _s.t<T>(pos));
