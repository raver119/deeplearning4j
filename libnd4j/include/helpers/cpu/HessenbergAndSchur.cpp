...
void Schur<T>::calcShift(const int ind, const int iter, T& shift, NDArray& shiftVec) {
    ...
    shiftVec.t<T>(0) = _T.t<T>(ind, ind);
    shiftVec.t<T>(1) = _T.t<T>(ind-1, ind-1);
    shiftVec.t<T>(2) = _T.t<T>(ind, ind-1) * _T.t<T>(ind-1, ind);

    if (iter == 10) {
        shift += shiftVec.t<T>(0);

        for (int i = 0; i <= ind; ++i)
            _T.t<T>(i,i) -= shiftVec.t<T>(0);

        T s = math::nd4j_abs<T>(_T.t<T>(ind, ind-1)) + math::nd4j_abs<T>(_T.t<T>(ind-1, ind-2));

        shiftVec.t<T>(0) = T(0.75) * s;
        shiftVec.t<T>(1) = T(0.75) * s;
        shiftVec.t<T>(2) = T(-0.4375) * s*s;
    }
...
void Schur<T>::initFrancisQR(const int ind1,  const int ind2, const NDArray& shiftVec, int& ind3, NDArray& householderVec) {
  ...
  for (ind3 = ind2-2; ind3 >= ind1; --ind3) {
        ...
        householderVec.t<T>(0) = (r * s - shiftVec.t<T>(2)) / _T.t<T>(ind3+1, ind3) + _T.t<T>(ind3, ind3+1);
        householderVec.t<T>(1) = _T.t<T>(ind3+1, ind3+1) - mm - r - s;
        householderVec.t<T>(2) = _T.t<T>(ind3+2, ind3+1);
...
void Schur<T>::calcFromHessenberg() {
    ...
    for (int j = 0; j < numCols; ++j)
        norm += _T({0,math::nd4j_min<int>(numCols,j+2), j,j+1}).reduceNumber(reduce::ASum).t<T>(0);
    ...
    if(norm != T(0)) {
        ...
        while (iu >= 0) {
            ...
            if (il == iu) {
                ...
                _T.t<T>(iu,iu) = _T.t<T>(iu,iu) + shift;
                if (iu > 0)
                    _T.t<T>(iu, iu-1) = T(0);
                iu--;
                iter = 0;
