...
void SVD<T>::deflation1(int col1, int shift, int ind, int size) {
    ...
    if (denom == (T)0.) {
        ...
        _m.t<T>(first+ind, first+ind) = (T)0;
        return;
    }
...
void SVD<T>::deflation2(int col1U , int col1M, int row1W, int col1W, int ind1, int ind2, int size) {
    ...
    if (denom == (T)0.)  {
      _m.t<T>(col1M + ind1, col1M + ind1) = _m.t<T>(col1M + ind2, col1M + ind2);
      return;
    }

    cos /= denom;
    sin /= denom;
    _m.t<T>(col1M + ind1, col1M) = denom;
    _m.t<T>(col1M + ind2, col1M + ind2) = _m.t<T>(col1M + ind1, col1M + ind1);
    _m.t<T>(col1M + ind2, col1M) = (T)0;
...
void SVD<T>::deflation(int col1, int col2, int ind, int row1W, int col1W, int shift)
{
    ...
    for(int i=1; i < len; i++)
        if(diagInterval.template t<T>(i) < epsBig) {
            ...
            for(int i = 0; i < len; ++i)
                diagInterval.t<T>(i) = _m.t<T>(col1+shift+i,col1+shift+i);
        }

    {
        ...
        for(int i = totDefl ? 0 : 1; i < len; i++) {
            ...
            math::nd4j_swap<T>(diagInterval.t<T>(i), diagInterval.t<T>(jac));

            if(i!=0 && jac!=0)
                math::nd4j_swap<T>(colVec0->t<T>(i), colVec0->t<T>(jac));
...
void SVD<T>::calcSingVals(const NDArray& col0, const NDArray& diag, const NDArray& permut, NDArray& singVals, NDArray& shifts, NDArray& mus) {
    ...
    for (Nd4jLong k = 0; k < len; ++k)  {
        ...
        if (col0.t<T>(k) == (T)0.f || curLen==1) {
            ...
            singVals.t<T>(k) = k==0 ? col0.t<T>(0) : diag.t<T>(k);
            mus.t<T>(k) = (T)0;
            shifts.t<T>(k) = k==0 ? col0.t<T>(0) : diag.t<T>(k);
            continue;
        }
...
void SVD<T>::perturb(const NDArray& col0, const NDArray& diag, const NDArray& permut, const NDArray& singVals,  const NDArray& shifts, const NDArray& mus, NDArray& zhat) {
    ...
    for (int k = 0; k < n; ++k) {
        ...
        if (col0.t<T>(k) == (T)0.f)
            ...
            zhat.t<T>(k) = col0.t<T>(k) > (T)0 ? tmp : -tmp;
        }
    }
}
        ...
        *colU = 0.;
        ...
        if (zhat.t<T>(k) == (T)0.f) {
            ...
            for(int l = 0; l < m; ++l) {
                ...
                U.t<T>(i,k) =  zhat.t<T>(i)/(((diag.t<T>(i) - shifts.t<T>(k)) - mus.t<T>(k)) )/( (diag.t<T>(i) + singVals.t<T>(k)));
            }
            U.t<T>(n,k) = (T)0;
            *colU /= colU->reduceNumber(reduce::Norm2);
            ...
            if (_calcV) {
                ...
                for(int l = 1; l < m; ++l){
                    int i = perm.t<T>(l);
                    V.t<T>(i,k) = diag.t<T>(i) * zhat.t<T>(i) / (((diag.t<T>(i) - shifts.t<T>(k)) - mus.t<T>(k)) )/( (diag.t<T>(i) + singVals.t<T>(k)));
                }
                V.t<T>(0,k) = (T)-1;
...
void SVD<T>::calcBlockSVD(int col1, int size, NDArray& U, NDArray& singVals, NDArray& V) {
    ...
    for(int i=0; i<curSize-1; ++i) {
        ...
        if(singVals.t<T>(i) > singVals.t<T>(i+1)) {
            ...
            math::nd4j_swap<T>(singVals.t<T>(i), singVals.t<T>(i+1));
    ...
    for (int e = 0; e < curSize / 2; ++e)
        math::nd4j_swap<T>(temp1.t<T>(e), temp1.t<T>(curSize-1-e));
...
void SVD<T>::DivideAndConquer(int col1, int col2, int row1W, int col1W, int shift) {
    ...
    if (_calcU) {
        ...
        for (int i = col1 + k - 1; i >= col1; --i)
            _u.t<T>(0, i+1) = _u.t<T>(0, i);

        _u.t<T>(0, col1) = q1 * c0;
        _u.t<T>(0, col2+1) = -q1*s0;
        _u.t<T>(1, col1) = _u.t<T>(1, col2+1) * s0;
        _u.t<T>(1, col2+1) =  _u.t<T>(1, col2 + 1) * c0;
        _u({1,2,  col1+1, col1+k+1}) = (T)0.f;
        _u({0,1,  col1+k+1, col1+n}) = (T)0.f;
    }

   _m.t<T>(col1+shift, col1+shift) = r0;
