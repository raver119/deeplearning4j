...
SVD<T>::SVD(const NDArray& matrix, const int switchSize, const bool calcU,
  ...
  _m = NDArray(matrix.ordering(), {_diagSize + 1, _diagSize},
                                matrix.dataType(), matrix.getContext());
  // _m.assign(0.);

  if (_calcU)
    _u = NDArray(
        matrix.ordering(), {_diagSize + 1, _diagSize + 1}, matrix.dataType(),matrix.getContext());
  else
    _u = NDArray(matrix.ordering(), {2, _diagSize + 1},
                                  matrix.dataType(), matrix.getContext());
  // _u.assign(0.);

  if (_calcV) {
    _v = NDArray(matrix.ordering(), {_diagSize, _diagSize},
                                  matrix.dataType(), matrix.getContext());
    //_v.assign(0.);
  }
...
void SVD<T>::deflation(int col1, int col2, int ind, int row1W, int col1W,
...

  {
    ...
    if (totDefl) {
      for (int i = 1; i < len; ++i) {
        int ki = permut[i];
        if (math::nd4j_abs<T>(diagInterval.template t<T>(ki)) < almostZero ||
...

  {
    ...
    while (i > 0 &&
           (math::nd4j_abs<T>(diagInterval.template t<T>(i)) < almostZero ||
            math::nd4j_abs<T>(colVec0.template t<T>(i)) < almostZero))
      --i;

    for (; i > 1; --i) {
      if ((diagInterval.template t<T>(i) - diagInterval.template t<T>(i - 1)) <
          DataTypeUtils::eps<T>() * maxElem) {
        if (math::nd4j_abs<T>(diagInterval.template t<T>(i) -
                              diagInterval.template t<T>(i - 1)) >= epsBig)
          throw std::runtime_error(
...
T SVD<T>::secularEq(const T diff, const NDArray& col0, const NDArray& diag,
  ...
  for (int i = 0; i < len; ++i) {
    ...
    item = col0.t<T>(j) /
           ((diagShifted.t<T>(j) - diff) * (diag.t<T>(j) + shift + diff));
    res += item * col0.t<T>(j);
  }
...
void SVD<T>::perturb(const NDArray& col0, const NDArray& diag,
  ...
  for (int k = 0; k < n; ++k) {
    if (col0.t<T>(k) == (T)0.f)
      ...
      T dk = diag.t<T>(k);
      T prod = (singVals.t<T>(last) + dk) *
               (mus.t<T>(last) + (shifts.t<T>(last) - dk));
      ...
      for (int l = 0; l < m; ++l) {
        ...
        if (i != k) {
          ...
          prod *=
              ((singVals.t<T>(j) + dk) / ((diag.t<T>(i) + dk))) *
  ...
  for (int k = 0; k < n; ++k) {
    ...
    if (zhat.t<T>(k) == (T)0.f) {
      ...
      for (int l = 0; l < m; ++l) {
        ...
        U.r<T>(i, k) =
            zhat.t<T>(i) / (((diag.t<T>(i) - shifts.t<T>(k)) - mus.t<T>(k))) /
      ...
      if (_calcV) {
        for (int l = 1; l < m; ++l) {
          int i = perm.t<T>(l);
          V.r<T>(i, k) =
              diag.t<T>(i) * zhat.t<T>(i) /
                  (((diag.t<T>(i) - shifts.t<T>(k)) - mus.t<T>(k))) /
                  ((diag.t<T>(i) + singVals.t<T>(k)));
        }
        V.r<T>(0, k) = (T)-1;
...
void SVD<T>::calcBlockSVD(int col1, int size, NDArray& U, NDArray& singVals,
  ...
  diag.r<T>(0) = (T)0;
  singVals =
      NDArray(_m.ordering(), {size, 1}, _m.dataType(),_m.getContext());
  U = NDArray(_u.ordering(), {size + 1, size + 1},
                               _u.dataType(), _u.getContext());
  if (_calcV)
  ...
  NDArray permut(
      _m.ordering(), {(int)indices.size()}, _m.dataType(), _m.getContext());
...
void SVD<T>::exchangeUV(const HHsequence& hhU, const HHsequence& hhV,
  ...
  if (_calcU) {
    ...
    NDArray temp1(_u.ordering(), {hhU.rows(), colsU},
                                          _u.dataType(), _u.getContext());
    temp1.setIdentity();
    _u = temp1;
  ...
  if (_calcV) {
    ...
    NDArray temp1(_v.ordering(), {hhV.rows(), colsV},
                                          _v.dataType(), _v.getContext());
    temp1.setIdentity();
    _v = temp1;
