...
void sd::IndexReductionLoops<X, Z>::loopIndexReduce(
  ...
  sd::LoopKind::Kind kindOfLoop =
      sd::LoopKind::deduceKindOfLoopTadXZ(xShapeInfo, zShapeInfo, tadShapeInfo);
  ...
  const Nd4jLong* tadShape =
      shape::shapeOf(const_cast<Nd4jLong*>(tadShapeInfo));
  const Nd4jLong* tadStride =
      shape::stride(const_cast<Nd4jLong*>(tadShapeInfo));
      ...
      auto func = PRAGMA_THREADS_FOR {
        for (auto i = start; i < stop; i++) {
          ...
          for (Nd4jLong i0 = 0; i0 < tadLen; ++i0) {
            functions::indexreduce::IndexValue<X> comp(tad[i0 * tadStride[0]],
                                                       i0);
            indexValue = OpType::update(indexValue, comp, extraParams);
          }
        ...
        for (auto i = start; i < stop; i++) {
          ...
          for (Nd4jLong i0 = 0; i0 < tadShape[0]; ++i0) {
            for (Nd4jLong i1 = 0; i1 < tadShape[1]; ++i1) {
              ...
              functions::indexreduce::IndexValue<X> comp(tad[tadOffset],
                                                         tadIndex);
              indexValue = OpType::update(indexValue, comp, extraParams);
            }
          }
          ...
          for (Nd4jLong i0 = 0; i0 < tadShape[0]; ++i0) {
            for (Nd4jLong i1 = 0; i1 < tadShape[1]; ++i1) {
              for (Nd4jLong i2 = 0; i2 < tadShape[2]; ++i2) {
                const auto tadOffset =
                    i0 * tadStride[0] + i1 * tadStride[1] + i2 * tadStride[2];
                const auto tadIndex =
                    i0 * newStride[0] + i1 * newStride[1] + i2;
                functions::indexreduce::IndexValue<X> comp(tad[tadOffset],
                                                           tadIndex);
                indexValue = OpType::update(indexValue, comp, extraParams);
              }
            ...
            for (Nd4jLong i1 = 0; i1 < tadShape[1]; ++i1) {
              for (Nd4jLong i2 = 0; i2 < tadShape[2]; ++i2) {
                for (Nd4jLong i3 = 0; i3 < tadShape[3]; ++i3) {
                  ...
                  const auto tadIndex = i0 * newStride[0] + i1 * newStride[1] +
                                        i2 * newStride[2] + i3;
                  functions::indexreduce::IndexValue<X> comp(tad[tadOffset],
                                                             tadIndex);
                  indexValue = OpType::update(indexValue, comp, extraParams);
              ...
              for (Nd4jLong i2 = 0; i2 < tadShape[2]; ++i2) {
                for (Nd4jLong i3 = 0; i3 < tadShape[3]; ++i3) {
                  for (Nd4jLong i4 = 0; i4 < tadShape[4]; ++i4) {
                    const auto tadOffset =
                        i0 * tadStride[0] + i1 * tadStride[1] +
                        i2 * tadStride[2] + i3 * tadStride[3] +
                        i4 * tadStride[4];
                    const auto tadIndex =
                        i0 * newStride[0] + i1 * newStride[1] +
                        i2 * newStride[2] + i3 * newStride[3] + i4;
                    functions::indexreduce::IndexValue<X> comp(tad[tadOffset],
                                                               tadIndex);
                    indexValue = OpType::update(indexValue, comp, extraParams);
                  }
    ...
    case sd::LoopKind::Z_EWSNONZERO: {
      ...
      auto func = PRAGMA_THREADS_FOR {
        for (auto i = start; i < stop; i++) {
          ...
          for (Nd4jLong j = 0; j < tadLen; j++) {
            auto tadOffset = shape::indexOffset(j, tadShapeInfo,
                                                castTadShapeInfo, canCastTad);
...
void sd::IndexReductionLoops<X, Y>::wrapIndexReduce(
    const int opNum, const void* vx, const Nd4jLong* xShapeInfo, void* vz,
    const Nd4jLong* zShapeInfo, const Nd4jLong* tadShapeInfo,
    const Nd4jLong* tadOffsets, void* vextraParams) {
  auto x = reinterpret_cast<const X*>(vx);
  auto z = reinterpret_cast<Y*>(vz);
  auto extraParams = reinterpret_cast<X*>(vextraParams);

  DISPATCH_BY_OPNUM_TT(loopIndexReduce,
