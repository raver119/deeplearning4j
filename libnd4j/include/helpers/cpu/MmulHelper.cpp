...
static void usualGemm(const NDArray* vA, const NDArray* vB, NDArray* vC,
                      const int aMaxis, const int aKaxis, const int bKaxis,
                      const int bNaxis, const int cMaxis, const int cNaxis,
...
template <typename T1, typename T2, typename T3>
static void usualGemv(const NDArray* vA, const NDArray* vX, NDArray* vY,
...
// (X*Y) = Z[0]
template <typename T1, typename T2, typename T3>
static void usualDot(const Nd4jLong length, const double alpha, const void* vX,
                     const Nd4jLong incx, const void* vY, const Nd4jLong incy,
  ...
  PRAGMA_OMP_PARALLEL_FOR_ARGS(
      OMP_IF(length > Environment::getInstance().elementwiseThreshold())
          schedule(guided) reduction(OMP_SUMT
                                     : sum))
  for (Nd4jLong i = 0; i < length; ++i) sum += X[i * incx] * Y[i * incy];
...
NDArray* MmulHelper::mmulMxM(const NDArray* A, const NDArray* B, NDArray* C,
  ...
  if (A->dataType() != B->dataType())
    throw datatype_exception::build(
        "mmulMxM expects all data types to be the same", A->dataType(),
        B->dataType());
  ...
  if (A->rankOf() != 2)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: rank of A array is not equal 2 !");
  if (B->rankOf() != 2)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: rank of B array is not equal 2 !");
  ...
  if (C != nullptr && C->rankOf() != 2)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: rank of C array is not equal 2 !");
  if (B->sizeAt(0) != K)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: B array has wrong number of rows !");
  if (C != nullptr && C->sizeAt(0) != M)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: C array has wrong number of rows !");
  if (C != nullptr && C->sizeAt(1) != N)
    throw std::runtime_error(
        "MmulHelper::mmulMxM: C array has wrong number of columns !");
  ...
  if (!typeFloat && !typeDouble) {
    BUILD_SINGLE_SELECTOR_THRICE(aType, usualGemm,
                                 (A, B, C, 0, 1, 0, 1, 0, 1, alpha, beta),
                                 NUMERIC_TYPES);
    // BUILD_TRIPLE_SELECTOR(aType, bType, cType, usualGemm, (A, B, C, 0, 1, 0,
    // 1, 0, 1, alpha, beta), LIBND4J_TYPES, FLOAT_TYPES, FLOAT_TYPES);
  } else {
    ...
    const int lda =
        (aMcont && aKcont) ? M : !aMcont ? pA->strideAt(0) : pA->strideAt(1);
    const int ldb =
        (bKcont && bNcont) ? K : !bKcont ? pB->strideAt(0) : pB->strideAt(1);
    const int ldc =
        (cMcont && cNcont) ? M : !cMcont ? pC->strideAt(0) : pC->strideAt(1);

    if (typeFloat) {
      BlasHelper::getInstance().sgemm()(
          blasOrder, transAblas, transBblas, M, N, K, (float)alpha,
          pA->bufferAsT<float>(), lda, pB->bufferAsT<float>(), ldb, (float)beta,
          pC->bufferAsT<float>(), ldc);
    } else if (typeDouble) {
      BlasHelper::getInstance().dgemm()(
          blasOrder, transAblas, transBblas, M, N, K, (double)alpha,
          pA->bufferAsT<double>(), lda, pB->bufferAsT<double>(), ldb,
          (double)beta, pC->bufferAsT<double>(), ldc);
    }
...
NDArray* MmulHelper::mmulMxV(const NDArray* A, const NDArray* X, sd::NDArray* Y,
  ...
  if (X->dataType() != A->dataType())
    throw datatype_exception::build(
        "mmulMxV expects all data types to be the same", A->dataType(),
        X->dataType());
  ...
  if (A->rankOf() != 2)
    throw std::runtime_error(
        "MmulHelper::mmulMxV: rank of A array is not equal 2 !");
  ...
  if (X->lengthOf() != N)
    throw std::runtime_error(
        "MmulHelper::mmulMxV: X vector has wrong length !");
  ...
  if (!typeDouble && !typeFloat) {
    BUILD_SINGLE_SELECTOR_THRICE(
        aType, usualGemv, (A, X, Y, incx, incy, 0, alpha, beta), NUMERIC_TYPES);
    // BUILD_TRIPLE_SELECTOR(aType, xType, yType, usualGemv, (A, X, Y, incx,
    // incy, 0, alpha, beta), LIBND4J_TYPES, FLOAT_TYPES, FLOAT_TYPES);
  } else {
    ...
    const int lda =
        (aMcont && aNcont) ? M : !aMcont ? pA->strideAt(0) : pA->strideAt(1);

    // choose appropriate cuda gemm api depending on data types
    if (typeDouble) {
      BlasHelper::getInstance().dgemv()(
          blasOrder, CblasNoTrans, M, N, alpha, (double*)pA->buffer(), lda,
          (double*)X->buffer(), incx, beta, (double*)Y->buffer(), incy);
    } else if (typeFloat) {
      BlasHelper::getInstance().sgemv()(
          blasOrder, CblasNoTrans, M, N, (float)alpha, (float*)pA->buffer(),
          lda, (float*)X->buffer(), incx, (float)beta, (float*)Y->buffer(),
          incy);
    }
...
NDArray* MmulHelper::dot(const NDArray* X, const NDArray* Y, sd::NDArray* Z,
  ...
  if (X->dataType() != Y->dataType())
    throw datatype_exception::build("Dot expects all data types to be the same",
  ...
  if (Y->lengthOf() != length)
    throw std::runtime_error(
        "MmulHelper::dot: lengths of input vectors are different !");
  ...
  BUILD_SINGLE_SELECTOR_THRICE(
      ...
      NUMERIC_TYPES);
  // BUILD_TRIPLE_SELECTOR(xType, yType, zType, usualDot, (length, alpha,
  // X->buffer(), incx, Y->buffer(), incy, beta, Z->buffer()), LIBND4J_TYPES,
...
static void batchedGemm(const NDArray* vA, const NDArray* vB, NDArray* vC,
                        const int* aBatchDims, const int* bBatchDims,
                        const int* cBatchDims, const int aMaxis,
                        const int aKaxis, const int bKaxis, const int bNaxis,
                        const int cMaxis, const int cNaxis, const double alpha,
                        const double beta) {
  const T1* A = vA->bufferAsT<T1>();
  const T2* B = vB->bufferAsT<T2>();
  T3* C = vC->bufferAsT<T3>();
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    for (auto i = start; i < stop; ++i) {
      ...
      // evaluate A coordinates
      if (aRank > 2)
        shape::index2coords(batchInd, aShapeInfo, aCoords.data(), aRank - 2,
                            aBatchDims);
      aCoords[aMaxis] = cCoords[cMaxis];
      aCoords[aKaxis] = 0;
      ...
      // evaluate B coordinates
      if (bRank > 2)
        shape::index2coords(batchInd, bShapeInfo, bCoords.data(), bRank - 2,
                            bBatchDims);
      bCoords[bKaxis] = 0;
      bCoords[bNaxis] = cCoords[cNaxis];
...
NDArray* MmulHelper::mmulNxN(const NDArray* A, const NDArray* B, NDArray* C,
  ...
  // batchedGemm, (A, B, C, aBatchDims.data(), bBatchDims.data(),
  // cBatchDims.data(), aMaxis, aKaxis, bKaxis, bNaxis, cMaxis, cNaxis, alpha,
  // beta), LIBND4J_TYPES, FLOAT_TYPES, FLOAT_TYPES);
  BUILD_SINGLE_SELECTOR_THRICE(
      A->dataType(), batchedGemm,
      (A, B, C, aBatchDims.data(), bBatchDims.data(), cBatchDims.data(), aMaxis,
       aKaxis, bKaxis, bNaxis, cMaxis, cNaxis, alpha, beta),
      NUMERIC_TYPES);
...
// MXK x KxN = MxN
template <typename T1, typename T2, typename T3>
static void usualGemm(const char cOrder, const bool transA, const bool transB,
...
ldc) {
    ...
    // PRAGMA_OMP_PARALLEL_FOR_ARGS(OMP_IF(M*N >
Environment::getInstance().elementwiseThreshold()) schedule(guided))
...
betaZ * c[col * ldc];
    ...
    //             for(uint col = 0; col < N; ++col) {
    //                 if(betaZ)
    //                     c[col] += a * b[flagB ? col : col * ldb] + betaZ *
c[col];
    //                 else
...
// MXN x N = M
template <typename T1, typename T2, typename T3>
static void usualGemv(const char aOrder, const int M, const int N, const double
alpha, const void* vA, const int lda, const void* vX, const int incx, const
double beta, void* vY, const int incy) {
...
// BUILD_TRIPLE_TEMPLATE(template void usualGemm, (const char cOrder, const bool
// transA, const bool transB, const int M, const int N, const int K, const
// double alpha, const void* A, const int lda, const void* B, const int ldb,
// const double beta, void* C, const int ldc), LIBND4J_TYPES, FLOAT_TYPES,
// FLOAT_TYPES); BUILD_TRIPLE_TEMPLATE(template void usualGemv, (const char
// aOrder, const int M, const int N, const double alpha, const void* A, const
// int lda, const void* B, const int incx, const double beta, void* C, const int
// incy), LIBND4J_TYPES, FLOAT_TYPES, FLOAT_TYPES);
// BUILD_TRIPLE_TEMPLATE(template void usualDot,  (const Nd4jLong length, const
// double alpha, const void* vX, const Nd4jLong incx, const void* vY, const
// Nd4jLong incy, const double beta, void* vZ), LIBND4J_TYPES, FLOAT_TYPES,
// FLOAT_TYPES);
