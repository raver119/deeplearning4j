...
class SD_EXPORT DebugHelper {
 public:
  ...
  static FORCEINLINE void checkErrorCode(cudaStream_t* stream,
                                         const char* failMessage = nullptr) {
    cudaError_t res = cudaStreamSynchronize(*stream);
    if (res != 0) {
      if (failMessage == nullptr) {
        std::string op = "CUDA call ended with error code [" +
                         StringUtils::valueToString<int>(res) +
                         std::string("]");
        throw std::runtime_error(op);
      } else {
        std::string op =
            std::string(failMessage) + std::string("Error code [") +
            StringUtils::valueToString<int>(res) + std::string("]");
        throw std::runtime_error(op);
      }
    }
  }
  ...
  static void retrieveDebugStatistics(DebugInfo* statistics,
                                      NDArray const* input);
};
}  // namespace sd
