...
class SD_EXPORT BitwiseUtils {
 public:
  ...
  /**
   *  This method returns TRUE if it's called on Big-Endian system, and false
   * otherwise
   */
  static bool isBE();
   ...
   * @return
   */
  static int FORCEINLINE flip_bits(int v) { return ~v; }

  static int8_t FORCEINLINE flip_bits(int8_t v) { return ~v; }

  static int16_t FORCEINLINE flip_bits(int16_t v) { return ~v; }

  static uint8_t FORCEINLINE flip_bits(uint8_t v) { return ~v; }

  static uint16_t FORCEINLINE flip_bits(uint16_t v) { return ~v; }

  static uint32_t FORCEINLINE flip_bits(uint32_t v) { return ~v; }

  static uint64_t FORCEINLINE flip_bits(uint64_t v) { return ~v; }

  static Nd4jLong FORCEINLINE flip_bits(Nd4jLong v) { return ~v; }
};
}  // namespace sd
