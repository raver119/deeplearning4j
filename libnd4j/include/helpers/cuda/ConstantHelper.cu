...
static void *getConstantSpace() {
  ...
  auto dZ = cudaGetSymbolAddress(reinterpret_cast<void **>(&dConstAddr),
...
void *ConstantHelper::replicatePointer(void *src, size_t numBytes,
  ...
  if (constantOffset + numBytes >= CONSTANT_LIMIT) {
    ...
    auto res = cudaMemcpyToSymbol(deviceConstantMemory,
                                  const_cast<const void *>(src), originalBytes,
                                  constantOffset, cudaMemcpyHostToDevice);
