...
ConstantShapeBuffer& ConstantShapeHelper::bufferForShapeInfo(
  ...
  if (_cache[deviceId].count(descriptor) == 0) {
    ...
    auto dPtr = std::make_shared<PointerWrapper>(ConstantHelper::getInstance().replicatePointer(
        hPtr->pointer(), shape::shapeInfoByteLength(hPtr->pointerAsT<Nd4jLong>())), std::make_shared<CudaPointerDeallocator>());
...
bool ConstantShapeHelper::checkBufferExistenceForShapeInfo(
...
Nd4jLong const* ConstantShapeHelper::createShapeInfo(
    const sd::DataType dataType, const char order, const int rank,
    const Nd4jLong* shape) {
  ShapeDescriptor descriptor(dataType, order, shape, rank);
...
Nd4jLong const* ConstantShapeHelper::createShapeInfo(
    const sd::DataType dataType, const Nd4jLong* shapeInfo) {
  return ConstantShapeHelper::createShapeInfo(
      dataType, shape::order(shapeInfo), shape::rank(shapeInfo),
      shape::shapeOf(const_cast<Nd4jLong*>(shapeInfo)));
}
...
Nd4jLong const* ConstantShapeHelper::scalarShapeInfo(
    const sd::DataType dataType) {
  auto descriptor = ShapeDescriptor::scalarDescriptor(dataType);
  return bufferForShapeInfo(descriptor).primary();
}

Nd4jLong const* ConstantShapeHelper::vectorShapeInfo(
...
Nd4jLong const* ConstantShapeHelper::createFromExisting(Nd4jLong* shapeInfo,
...
Nd4jLong const* ConstantShapeHelper::createFromExisting(
    Nd4jLong* shapeInfo, sd::memory::Workspace* workspace) {
  ShapeDescriptor descriptor(shapeInfo);
  auto result = createShapeInfo(descriptor);
...
ConstantShapeBuffer&  ConstantShapeHelper::createShapeInfoWithUnitiesForBroadcast(
  ...
  ALLOCATE(newShapeInfo, workspace,
           shape::shapeInfoLength(shape::rank(maxShapeInfo)), Nd4jLong);

  newShapeInfo[0] = shape::rank(maxShapeInfo);

  sd::ArrayOptions::copyDataType(newShapeInfo, minShapeInfo);  // type
  newShapeInfo[2 * newShapeInfo[0] + 2] =
      shape::elementWiseStride(minShapeInfo);                          // ews
  ...
  if (!dimensions.empty()) {
    for (uint k = 0, j = 0, i = 0; i < shape::rank(maxShapeInfo); ++i) {
      if (j < dimensions.size() && dimensions[j] == i) {
        ...
        if (shape::sizeAt(minShapeInfo, k) == 1 &&
            dimensions.size() != shape::rank(minShapeInfo))
          ++k;
      }
    }
  } else {
    for (int j = shape::rank(minShapeInfo) - 1,
             i = shape::rank(maxShapeInfo) - 1;
         i >= 0; --i) {
      if (j >= 0) {
        shape::shapeOf(newShapeInfo)[i] = shape::shapeOf(minShapeInfo)[j];
        shape::stride(newShapeInfo)[i] = shape::shapeOf(minShapeInfo)[j] == 1
                                             ? 0
