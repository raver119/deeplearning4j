...
class TAD {
 public:
  ...
  // need to track whether we create a new dimension array or not, we could have
  // just moved the pointer forward due to leading ones
  bool createdNewDimension = false;

  // special case for CUDA, we're passing in __shared__ memory pointers to be
  // used instead of new/malloc
  void *ptrManager = nullptr;
  int *ptrOutput = nullptr;

  INLINEDEF bool dimensionsDescending(int rank, int const *dimensions,
                                      int length);
...
#endif
      INLINEDEF void
      setExternalBuffers(void *ptrManager);

#ifdef __CUDACC__
  __host__ __device__
#endif
      INLINEDEF void
      setOutputBuffer(int *ptrOutput);
...
#endif
      INLINEDEF void
      init(Nd4jLong const *shapeInfo, int const *dimension,
           int dimensionLength);
...
#endif
      INLINEDEF void
      init(int index, Nd4jLong const *shapeInfo, int const *dimension,
           int dimensionLength);
...
#endif
      INLINEDEF void
      permuteShapeBufferInPlace(Nd4jLong const *shapeBuffer,
                                int const *rearrange, Nd4jLong *out);

#ifdef __CUDACC__
  __host__ __device__
#endif
      INLINEDEF Nd4jLong *
      permuteShapeBuffer(Nd4jLong const *shapeBuffer, int *rearrange);

#ifdef __CUDACC__
  __host__ __device__
#endif
      INLINEDEF void
      createTadOnlyShapeInfo();

#ifdef __CUDACC__
  __host__ __device__
#endif
      INLINEDEF Nd4jLong
      lengthPerSlice(Nd4jLong const *shapeBuffer);

#ifdef __CUDACC__
  __host__ __device__
#endif
      INLINEDEF Nd4jLong *
      tad2Sub(Nd4jLong index);
...
#endif
      INLINEDEF Nd4jLong *
      tad2Sub(Nd4jLong index, void *ptrManager);
...
#endif
      INLINEDEF Nd4jLong
      tadLength(Nd4jLong const *shapeInfo, int const *dimension,
                int dimensionLength);
...
#endif
      INLINEDEF Nd4jLong
      tensorsAlongDimension(Nd4jLong const *shapeInfo, int const *dimension,
                            int dimensionLength);

#ifdef __CUDACC__
  __host__ __device__ INLINEDEF void createOffsetForBlock(int blockIdx) {
...
/*
    ...
    __host__ __device__
#endif
    INLINEDEF TAD::TAD(int tadIndex,Nd4jLong *shapeInfo,int *dimension,int
dimensionLength) { this->tadIndex = tadIndex; this->init(shapeInfo, dimension,
dimensionLength);
    }
...
INLINEDEF void TAD::initWithExternalTAD(Nd4jLong *existingTAD,
                                        Nd4jLong *originalShape, int *dimension,
                                        int dimensionLength) {
...
INLINEDEF void TAD::init(int tadIndex, Nd4jLong const *shapeInfo,
                         int const *dimension, int dimensionLength) {
  this->tadIndex = tadIndex;
  this->init(shapeInfo, dimension, dimensionLength);
}

INLINEDEF void TAD::init(Nd4jLong const *shapeInfo, int const *dimension,
                         int dimensionLength) {
  ...
  this->numTads = dimensionLength == 0 ? 1
                                       : this->tensorsAlongDimension(
                                             this->shapeInfo, this->dimension,
  ...
  if (dimensionLength == 0) {
    ...
    wholeThing =
        this->numTads ==
            1  // if number of TADs is 1, we just have input shape == TAD shape
        ||
        ((this->dimensionLength ==
              this->rank  // if number of dimensions is the same as input rank,
                          // that'll be wholeTad too, but only if EWS==1 (aka -
                          // not a View)
          || (this->numTads == shape::length(shapeInfo) &&
              shape::order(shapeInfo) ==
                  'c'))  // OR  number of tads equals to shapeInfo length AND
                         // input is in C order. if order is F - we'll have to
                         // calculate offsets
         &&
         ews ==
             1);  // as mentioned above - last 2 rules apply only to non-views
    ...
    // {
    // if(dimension == 0 && ) {
    if (dimensionLength != 0 && dimension != nullptr &&
        shape::shapeOf(shapeInfo)[dimension[0]] == 1) {
      wholeThing = true;
    }
  }
}
...
INLINEDEF void TAD::printTADsND(T *x) {
  if (wholeThing) {
    ...
    for (int i = 0; i < numTads; i++) {
      ...
      T *xPointer = x + offset;
      if (PrepareOneRawArrayIter<T>(rankIter, shape::shapeOf(tadOnlyShapeInfo),
                                    xPointer, shape::stride(tadOnlyShapeInfo),
                                    &rankIter, shapeIter, &xPointer,
                                    xStridesIter) >= 0) {
        ND4J_RAW_ITER_START(dim, shape::rank(tadOnlyShapeInfo), coord,
                            shapeIter);
        {
          /* Process the innermost dimension */
          printf(" %f ", xPointer[0]);
        }
        ND4J_RAW_ITER_ONE_NEXT(dim, rankIter, coord, shapeIter, xPointer,
                               xStridesIter);
        printf("\n");
...
INLINEDEF void TAD::permuteShapeBufferInPlace(Nd4jLong const *shapeBuffer,
                                              int const *rearrange,
                                              Nd4jLong *out) {
  memcpy(out, shapeBuffer,
         sizeof(Nd4jLong) * shape::shapeInfoLength(this->rank));
  doPermuteShapeInfo(out, rearrange);
}

INLINEDEF Nd4jLong *TAD::permuteShapeBuffer(Nd4jLong const *shapeBuffer,
                                            int *rearrange) {
...
INLINEDEF void TAD::createTadOnlyShapeInfo() {
  ...
  sd::ArrayOptions::setDataType(
      this->tadOnlyShapeInfo,
      sd::ArrayOptions::dataType(this->originalShapeInfo));
  ...
  if (shape::order(this->originalShapeInfo) == 'c' &&
      ...
      dimensionsDescending(shape::rank(this->originalShapeInfo),
    ...
    this->tadOnlyShapeInfo[shape::shapeInfoLength(this->tadOnlyShapeInfo) - 2] =
        this->originalShapeInfo[shape::shapeInfoLength(
                                    this->originalShapeInfo) -
                                2];
  }

  // do not swap order if positive elementwise stride preserved
  if (shape::elementWiseStride(this->tadOnlyShapeInfo) >= 1) {
    this->tadOnlyShapeInfo[shape::shapeInfoLength(this->tadOnlyShapeInfo) - 1] =
        shape::order(this->originalShapeInfo);
  }
...
INLINEDEF Nd4jLong TAD::lengthPerSlice(Nd4jLong const *shapeBuffer) {
  int dimension = 0;
  Nd4jLong *remove = shape::removeIndex(shape::shapeOf(shapeBuffer), &dimension,
                                        shape::rank(shapeBuffer), 1);
...
INLINEDEF Nd4jLong *TAD::tad2Sub(Nd4jLong index) {
  ...
  for (int i = 0; i < rank; i++) {
    // look for dimensions NOT found in dimension length (basically compute
    // shape - dimension (set difference)
    bool found = false;
    ...
    // indexes
    if (!found) {
      // accumulate the list of indexes left over used for initializing the
      // return value
      leftOverIndexes[leftOverIndex] = i;
      // accumulate the tad shape
      tadShape[leftOverIndex] = shape[i];
      // accumulate the length (product) of the indexes that will be iterated
      // over
      len *= shape[i];
      leftOverIndex++;
    }
...
INLINEDEF TAD::~TAD() {
  // we may have just moved the pointer forward, we may not need to delete the
  // pointer here
  ...
  if (this->tadOnlyShapeInfo != nullptr &&
      this->tadOnlyShapeInfo != shapeInfo) {
...
INLINEDEF Nd4jLong *TAD::tensorShape() {
  ...
  Nd4jLong *tensorShape = shape::keep(theShape, this->dimension,
                                      dimensionLength, shape::rank(shapeInfo));
...
INLINEDEF Nd4jLong *TAD::shapeInfoOnlyShapeAndStride() {
  ...
  if (dimensionLength > 1 && shape::isVector(shapeInfo))
    return shape::copyOf(shape::shapeInfoLength(shape::rank(shapeInfo)),
                         shapeInfo);

  // case when tad coincides with whole array
  if (this->numTads == 1 &&
      ((shape::rank(originalShapeInfo) == originalDimensionLength) ||
       originalDimensionLength == 0)) {
    // we might have special case here: skipped dimensions might be just full of
    // ones
    Nd4jLong *ret = shape::copyOf(
        shape::shapeInfoLength(shape::rank(shapeInfo)), shapeInfo);
    if (shape::isDimPermuted<int>(
            dimension,
            (Nd4jLong)dimensionLength))  // check whether we need permutation
  ...
  if (dimensionLength == 1) {
    if (dimension[0] == 0 && shape::isVector(shapeInfo) && theShape[1] == 1) {
      ...
      return permutedRet2;
    } else if (dimension[0] == 1 && shape::isVector(shapeInfo) &&
               theShape[0] == 1) {
      Nd4jLong *ret = shape::copyOf(
          shape::shapeInfoLength(shape::rank(shapeInfo)), shapeInfo);
      return ret;
      ...
      scalarInfo[shape::shapeInfoLength(shape::rank(scalarInfo)) - 3] =
          this->tadIndex;
      return scalarInfo;
    }
  }
  ...
  int *remove = shape::removeIndex<int>(rankRange, dimension, (Nd4jLong)rank,
                                        (Nd4jLong)dimensionLength);
  // concat is wrong here with the length
  int *newPermuteDims = shape::concat(remove, rank - dimensionLength,
                                      reverseDimensions, dimensionLength);

  Nd4jLong *permuted = shape::permuteShapeBuffer(shapeInfo, newPermuteDims);

  Nd4jLong sliceIndex = shape::sliceOffsetForTensor(
      shape::rank(permuted), this->tadIndex, shape::shapeOf(shapeInfo),
      tensorShape, dimensionLength, dimension, dimensionLength);

  Nd4jLong *ret2 = shape::sliceOfShapeBuffer(sliceIndex, permuted);
  Nd4jLong tensorLength = shape::prodLong(tensorShape, tadRank);

  Nd4jLong compLength = shape::isVector(ret2)
                            ? shape::length(ret2)
                            : shape::prodLong(tensorShape, tadRank);
  // int temp;
  ...
  // !isLikeVector) {
  if (dimensionLength == tadRank && compLength == shape::length(ret2)) {
    if (dimensionLength == 1 && shape::isVector(ret2) &&
        shape::shapeOf(ret2)[0] == 1) {
    ...
    if (sliceIndex == 0 && length == lengthPerSlice) {
      ...
      while (shape::length(ret2) > length) {
        ...
        sliceIndex = sliceOffsetForTensor(sliceIndex, shape::length(ret2),
                                          lengthPerSlice2);
...
INLINEDEF Nd4jLong TAD::tadLength(Nd4jLong const *shapeInfo,
                                  int const *dimension, int dimensionLength) {
  if (dimensionLength == 1) {
...
INLINEDEF Nd4jLong TAD::tensorsAlongDimension(Nd4jLong const *shapeInfo,
                                              int const *dimension,
                                              int dimensionLength) {
  return shape::length(shapeInfo) /
         this->tadLength(shapeInfo, dimension, dimensionLength);
}
...
INLINEDEF void TAD::collapse() {
  ...
  memcpy(this->dimension, this->originalDimension,
         sizeof(int) * dimensionLength);
    ...
    for (int i = (dimensionLength)-1; i >= 0; i--) {
      if (shape[(dimension)[i]] == 1) {
        ...
        if (!nonOneEncountered) {
          ...
          // intermediary result just needs to have the results copied from
          // dimension since we're just removing the tail
          memcpy(intermediaryResult, dimension, sizeof(int) * dimensionLength);
          changed = true;
          // break the for loop and force it to go back around starting from the
          // new index
          break;
        } else {
