...
class SD_EXPORT BenchmarkHelper {
 ...
 protected:
  ...
  void benchmarkScalarOperation(scalar::Ops op, std::string testName,
                                double value, NDArray &x, NDArray &z);

  void benchmarkDeclarableOp(sd::ops::DeclarableOp &op, std::string testName,
                             Context &context);

  void benchmarkGEMM(char orderA, std::initializer_list<Nd4jLong> shapeA,
                     char orderB, std::initializer_list<Nd4jLong> shapeB,
                     char orderC, std::initializer_list<Nd4jLong> shapeC);

  std::string printHeader();

 public:
  BenchmarkHelper(unsigned int warmUpIterations = 10,
                  unsigned int runIterations = 100);

  std::string runOperationSuit(std::initializer_list<OpBenchmark *> benchmarks,
                               const char *msg = nullptr);
  std::string runOperationSuit(std::vector<OpBenchmark *> &benchmarks,
                               bool postHeaders, const char *msg = nullptr);
  std::string runOperationSuit(OpBenchmark *benchmark);

  std::string runOperationSuit(
      ScalarBenchmark *op,
  ...
  std::string runOperationSuit(
      DeclarableBenchmark *op,
      const std::function<Context *(Parameters &)> &func,
      ParametersBatch &parametersBatch, const char *message = nullptr);
};
}  // namespace sd
