...
static __global__ void simpleIndexReduceGeneric(
    const int op, void const *dx, Nd4jLong const *xShapeInfo, int xRank,
    void *extraParams, void *result, Nd4jLong const *zShapeInfo, int zRank,
  ...
  functions::indexreduce::IndexReduce<X, Z>::transform(
      op, dx, xShapeInfo, extraParams, result, zShapeInfo, dimension,
      dimensionLength, postProcessOrNot, allocationBuffer, reductionBuffer,
...
template <typename X, typename Z>
_CUDA_H void IndexReduce<X, Z>::executeIndexReduceScalar(
    dim3 launchDims, cudaStream_t *stream, const int opNum, void const *dx,
    Nd4jLong const *xShapeInfo, int xRank, void *extraParams, void *result,
    Nd4jLong const *zShapeInfo, int zRank, int *dimension, int dimensionLength,
    int postProcessOrNot, int *allocationBuffer, void *reductionBuffer,
    Nd4jLong const *tadOnlyShapeInfo, Nd4jLong const *tadOffsets) {
  simpleIndexReduceGeneric<X, Z>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          opNum, dx, xShapeInfo, xRank, extraParams, result, zShapeInfo, 0,
          nullptr, 0, 1, allocationBuffer, reductionBuffer, tadOnlyShapeInfo,
          tadOffsets);
}

template <typename X, typename Z>
_CUDA_H void IndexReduce<X, Z>::executeIndexReduce(
    dim3 launchDims, cudaStream_t *stream, const int opNum, void const *dx,
    Nd4jLong const *xShapeInfo, int xRank, void *extraParams, void *result,
    Nd4jLong const *zShapeInfo, int zRank, int *dimension, int dimensionLength,
    int postProcessOrNot, int *allocationBuffer, void *reductionBuffer,
  ...
  simpleIndexReduceGeneric<X, Z>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          opNum, dx, xShapeInfo, xRank, extraParams, result, zShapeInfo, zRank,
          dimension, dimensionLength, 1, allocationBuffer, reductionBuffer,
          tadOnlyShapeInfo, tadOffsets);
}
...
// int, uint, char, uchar, short, ushort, long long, ulong long, bool, float,
// and double One could also specialize it for user-defined types.
...
__device__ void IndexReduce<X, Z>::aggregatePartials(
    IndexValue<X> **sPartialsRef, Nd4jLong tid, Nd4jLong numElements,
    void *vextraParams) {
...
__device__ void IndexReduce<X, Y>::transform(
    const int opNum, void const *x, Nd4jLong const *xShapeInfo,
    void *extraParams, void *result, Nd4jLong const *zShapeInfo, int *dimension,
    int dimensionLength, int postProcessOrNot, int *allocationBuffer,
    void *reductionBuffer, Nd4jLong const *tadShapeInfo,
    Nd4jLong const *tadOffset) {
  DISPATCH_BY_OPNUM_TT(
      transform,
      PARAMS(x, xShapeInfo, extraParams, result, zShapeInfo, dimension,
             dimensionLength, postProcessOrNot, allocationBuffer,
             reductionBuffer, tadShapeInfo, tadOffset),
      INDEX_REDUCE_OPS);
}

template <typename X, typename Z>
template <typename OpType>
__device__ void IndexReduce<X, Z>::transform(
    void const *vdx, Nd4jLong const *xShapeInfo, void *vextraParams, void *vz,
  ...
  if (sd::ArrayOptions::arrayType(xShapeInfo) == sd::ArrayType::EMPTY) {
    ...
    for (uint i = blockIdx.x * blockDim.x + threadIdx.x; i < zLen;
  ...
  if (!resultScalar) {
    ...
    if (dimensionLength > 1 || tadEWS < 1) {
      for (int r = blockIdx.x; r < numTads; r += gridDim.x) {
        ...
        for (int i = threadIdx.x; i < tadLength; i += blockDim.x) {
          ...
          sPartials[threadIdx.x] =
              OpType::update(sPartials[threadIdx.x], comp, extraParams);
        }
      ...
      for (int i = blockIdx.x; i < numTads; i += gridDim.x) {
        ...
        if (threadIdx.x == 0) {
          z[i] =
              (Z)sPartials[threadIdx.x]
                  .index;  // postProcess(sPartials[0],tadLength ,extraParams);
        }
        __syncthreads();
      }
    }
    ...
    aggregatePartials<OpType>(&sPartials, threadIdx.x,
                              sd::math::nd4j_min<int>(blockDim.x, (int)n),
                              extraParams);
    __syncthreads();
    ...
    if (gridDim.x > 1) {
      ...
      if (amLast) {
        ...
        for (Nd4jLong i = threadIdx.x; i < gridDim.x; i += blockDim.x) {
          sPartials[threadIdx.x] =
              OpType::update(sPartials[threadIdx.x], pBuffer[i], extraParams);
        }

        __syncthreads();
        aggregatePartials<OpType>(
            &sPartials, threadIdx.x,
            sd::math::nd4j_min<int>(gridDim.x, blockDim.x), extraParams);
...
BUILD_DOUBLE_TEMPLATE(template class SD_EXPORT IndexReduce, , LIBND4J_TYPES,
                      INDEXING_TYPES);
}  // namespace indexreduce
}  // namespace functions
