...
__global__ static void pairwiseSimpleShaped(
...
void _CUDA_H PairWiseBoolTransform<X, Z>::intermediateShaped(
    dim3& launchDims, cudaStream_t* stream, void const* vx,
    Nd4jLong const* xShapeInfo, void const* vy, Nd4jLong const* yShapeInfo,
    void* vz, Nd4jLong const* zShapeInfo, void* vextraParams) {
...
void PairWiseBoolTransform<X, Y>::executeCudaShaped(
  ...
  DISPATCH_BY_OPNUM_TT(intermediateShaped,
                       PARAMS(launchDims, stream, vx, xShapeInfo, vy,
                              yShapeInfo, vz, zShapeInfo, vextraParams),
                       PAIRWISE_BOOL_OPS);
}
