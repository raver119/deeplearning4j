...
void TypeCast::convertGenericCuda(Nd4jPointer *extras, void *dx, Nd4jLong N,
                                  void *dz) {
...
__device__ void convertKernelGeneric(S *x, Nd4jLong N, T *z) {
  ...
  // Define this to more rigorously avoid bank conflicts, even at the lower
...
template <bool isNP2>
__device__ void loadSharedChunkFromMem(int *s_data, const int *g_idata, int n,
                                       int baseIndex, int &ai, int &bi,
...
template <bool isNP2>
__device__ void storeSharedChunkToMem(int *g_odata, int *s_data, int n, int ai,
...
__device__ void clearLastElement(int *s_data, int *g_blockSums,
                                 int blockIndex) {
  if (threadIdx.x == 0) {
...
__global__ void prescan(int *g_odata, const int *g_idata, int *g_blockSums,
  ...
  loadSharedChunkFromMem<isNP2>(
      reinterpret_cast<int *>(s_data), g_idata, n,
...
__global__ void uniformAdd(int *g_data, int *uniforms, int n, int blockOffset,
                           int baseIndex) {
...
//////////////////////////////////////////////////////////////////////////
/*
 * PLEASE NOTE: This kernel doesn't allow loop for data. Basically: grid will be
 * huge.
 */
template <typename T>
__global__ static void execEncoderKernelP1(const void *dx, Nd4jLong N, void *dz,
                                           float threshold) {
  auto x = reinterpret_cast<const T *>(dx);
  auto z = reinterpret_cast<int *>(dz);

  // basically, for phase One we want do calculation: how many eligible values
  // we have, and which blocks will be holding data
  Nd4jLong tid = blockIdx.x * blockDim.x + threadIdx.x;

  int pass =
      tid < N && sd::math::nd4j_abs<T>(x[tid]) >= static_cast<T>(threshold) ? 1
                                                                            : 0;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void encoderKernelP1Generic(dim3 &launchDims, cudaStream_t *stream,
                                     const void *dx, Nd4jLong N, void *dz,
                                     float threshold) {
  execEncoderKernelP1<T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
      dx, N, dz, threshold);
  sd::DebugHelper::checkErrorCode(stream, "encoderP1(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT encoderKernelP1Generic,
                      (dim3 & launchDims, cudaStream_t *stream, const void *dx,
                       Nd4jLong N, void *dz, float threshold),
                      FLOAT_TYPES);
 ...
 * huge.
 *
 * Based on: https://github.com/knotman90/cuStreamComp <-- efficient CUDA stream
 * compaction algorithm
 */
template <typename T>
__global__ static void execEncoderKernelP3(void *dx, int *offsets, Nd4jLong N,
                                           void *dz) {
  ...
  auto pred =
      tid >= N
          ? 0
          : sd::math::nd4j_abs<T>(value) >= static_cast<T>(threshold) ? 1 : 0;
  auto w_i = threadIdx.x / warpSize;  // warp index (or, warp number) - index of
                                      // the Warp within TOTAL_WARPS
  auto t_i = threadIdx.x % warpSize;  // thread index within a warp
  unsigned int t_m =
      INT_MAX >> (warpSize - t_i -
                  1);  // thread mask (ERROR IN THE PAPER minus one is required)

  int b = __ballot_sync(
      t_m, pred);  // balres = number whose ith bit isone if the ith's thread
                   // pred is true masked up to the current index in warp
  auto t_u = __popc(b);  // popc count the number of bit one. simply count the
                         // number predicated true BEFORE MY INDEX
  ...
  int w_i_u = 0;
  for (int j = 0; j <= 5; j++) {
    unsigned int b_j = __ballot_sync(
        t_m,
        warpTotals[t_i] &
  ...
  if (pred) {
    ...
    if (idx < limit + 4) {
      ...
      x[tid] = value > static_cast<T>(0.0f) ? x[tid] - threshold
                                            : x[tid] + threshold;
    }
  }
}
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void encoderKernelP3Generic(dim3 &launchDims, cudaStream_t *stream,
                                     void *dx, int *offsets, Nd4jLong N,
                                     void *dz) {
  execEncoderKernelP3<T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
      dx, offsets, N, dz);
  sd::DebugHelper::checkErrorCode(stream, "encoderP3(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT encoderKernelP3Generic,
                      (dim3 & launchDims, cudaStream_t *stream, void *dx,
                       int *offsets, Nd4jLong N, void *dz),
                      FLOAT_TYPES);
...
__global__ static void execDecoderKernel(const void *dx, Nd4jLong N, void *dz) {
  ...
  for (int e = tid; e < limit; e += blockDim.x * gridDim.x) {
    ...
    // TODO: investigate, if += would work better here, as in "decoded
    // accumulation"
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void decoderKernelGeneric(dim3 &launchDims, cudaStream_t *stream,
                                   const void *dx, Nd4jLong N, void *dz) {
  execDecoderKernel<T>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(dx, N, dz);
  sd::DebugHelper::checkErrorCode(stream, "execDecoder(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT decoderKernelGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, const void *dx,
                       Nd4jLong N, void *dz),
                      FLOAT_TYPES);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void execCudaEncodeBitmapKernel(void *vdx, Nd4jLong N,
                                                  int *dz, int *scalar,
                                                  int *reductionBuffer,
                                                  float threshold) {
  ...
  for (Nd4jLong i = tid; i < loopLimit; i += blockDim.x * gridDim.x) {
    ...
    } else if (abs >= static_cast<T>(threshold) / static_cast<T>(2.0f) &&
               val < static_cast<T>(0.0f) && i < N) {
      atomicAdd(&counter, 1);
      shmem[threadIdx.x] = 1 << (bitId + 16);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void cudaEncodeBitmapGeneric(dim3 &launchDims, cudaStream_t *stream,
                                      void *vdx, Nd4jLong N, int *dz,
                                      int *scalar, int *reductionBuffer,
                                      float threshold) {
  execCudaEncodeBitmapKernel<T>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          vdx, N, dz, scalar, reductionBuffer, threshold);
  sd::DebugHelper::checkErrorCode(stream, "encodeBitmap(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT cudaEncodeBitmapGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, void *vdx,
                       Nd4jLong N, int *dz, int *scalar, int *reductionBuffer,
                       float threshold),
                      FLOAT_TYPES);

//////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void execCudaDecodeBitmapKernel(const void *dx, Nd4jLong N,
                                                  void *vdz) {
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void cudaDecodeBitmapGeneric(dim3 &launchDims, cudaStream_t *stream,
                                      const void *dx, Nd4jLong N, void *vdz) {
  execCudaDecodeBitmapKernel<T>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(dx, N, vdz);
  sd::DebugHelper::checkErrorCode(stream, "cudeDecodeBitmap(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT cudaDecodeBitmapGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, const void *dx,
                       Nd4jLong N, void *vdz),
                      FLOAT_TYPES);

template <bool storeSum, bool isNP2>
__host__ void prescanLauncher(dim3 &blocks, dim3 &threads, int shmem,
                              cudaStream_t *stream, int *g_odata,
                              const int *g_idata, int *g_blockSums, int n,
  ...
  //printf("Prescan grid: <%i/%i/%i>; threads: <%i/%i/%i>; shareMemSize: %i\n", blocks.x, blocks.y, blocks.z,   threads.x, threads.y, threads.z,    shmem);
        shmem = sd::math::nd4j_max<int>(shmem, 16384);prescan<storeSum, isNP2><<<blocks, threads, shmem, *stream>>>(
      g_odata, g_idata, g_blockSums, n, blockIndex, baseIndex);
...
BUILD_DOUBLE_TEMPLATE(template void TypeCast::convertGenericCuda,
                      (Nd4jPointer * extras, void *dx, Nd4jLong N, void *dz),
                      LIBND4J_TYPES_EXTENDED, LIBND4J_TYPES_EXTENDED);
BUILD_DOUBLE_TEMPLATE(template void prescanLauncher,
                      (dim3 & blocks, dim3 &threads, int shmem,
                       cudaStream_t *stream, int *g_odata, const int *g_idata,
