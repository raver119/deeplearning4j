...
__global__ void transformSameSimple(const void *x, const Nd4jLong *xShapeInfo,
...
template <typename X>
_CUDA_H void TransformSame<X>::executeTransformShaped(
...
template <typename X>
template <typename OpType>
_CUDA_H void TransformSame<X>::intermediateShaped(
    dim3 launchDims, cudaStream_t *stream, const void *x,
    const Nd4jLong *xShape, int xRank, void *extraParams, void *z,
    const Nd4jLong *zShape, int zRank, int *allocationPointer,
    void *reductionPointer, const Nd4jLong *tadShapeInfo,
    const Nd4jLong *tadOffsets) {
  transformSameSimple<X, OpType>
