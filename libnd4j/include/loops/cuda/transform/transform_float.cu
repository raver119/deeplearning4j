...
__global__ void transformFloatSimple(const void *x, const Nd4jLong *xShapeInfo,
...
_CUDA_H void TransformFloat<X, Y>::executeTransformShaped(
...
__device__ void TransformFloat<X, Z>::transformCuda(
  ...
  if (OpType::requiresSpecial) {
    OpType::execSpecialCuda(x, xShapeInfo, z, zShapeInfo, params,
                            allocationPointer, reductionPointer, tadShapeInfo,
                            tadOffsets);
    return;
  } else {
...
__device__ void TransformFloat<X, Y>::transformCudaLegacy(
    const int opNum, const void *x, const Nd4jLong *xShapeInfo, void *params,
    void *z, const Nd4jLong *zShapeInfo, int *allocationPointer,
    void *reductionPointer, const Nd4jLong *tadShapeInfo,
    const Nd4jLong *tadOffsets) {
  DISPATCH_BY_OPNUM_TT(
      transformCuda,
      PARAMS(x, xShapeInfo, params, z, zShapeInfo, allocationPointer,
             reductionPointer, tadShapeInfo, tadOffsets),
      TRANSFORM_FLOAT_OPS);
}
...
_CUDA_H void TransformFloat<X, Z>::intermediateShaped(
    ...
    void *reductionPointer, const Nd4jLong *tadShapeInfo,
    const Nd4jLong *tadOffsets) {
  transformFloatSimple<X, Z, OpType>
