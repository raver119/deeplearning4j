...
__device__ void ReduceBoolFunction<X, Z>::aggregatePartials(
...
template <typename X, typename Z>
template <typename OpType>
__device__ void ReduceBoolFunction<X, Z>::transformCudaXD(
...
template <typename X, typename Z>
template <typename OpType>
__device__ void ReduceBoolFunction<X, Z>::execScalarCuda(
    const void *vx, const Nd4jLong *xShapeInfo, void *vextraParams, void *vz,
...
template <typename X, typename Z>
template <typename OpType>
__host__ void ReduceBoolFunction<X, Z>::intermediateXD(
  ...
  if (shape::isEmpty(hXShapeInfo)) {
    ...
    if (res != 0)
      throw sd::cuda_exception::build(
          "ReduceBoolFunction<X,Z>::intermediateXD: failed to copy temporary "
          "scalar",
          res);
    ...
    functions::scalar::ScalarTransform<Z, Z, Z>::executeCudaShaped(
        launchDims, stream, 14, z, zShapeInfo, hZShapeInfo, z, zShapeInfo,
...
template <typename X, typename Z>
template <typename OpType>
__host__ void ReduceBoolFunction<X, Z>::intermediateScalar(
    dim3 launchDims, cudaStream_t *stream, const void *x,
    const Nd4jLong *xShapeInfo, const Nd4jLong *hXShapeInfo, void *extraParams,
    void *z, const Nd4jLong *zShapeInfo, const Nd4jLong *hZShapeInfo,
  ...
  if (shape::isEmpty(hXShapeInfo)) {
    ...
    if (res != 0)
      throw sd::cuda_exception::build(
          "ReduceBoolFunction<X,Z>::intermediateScalar: failed to copy "
          "resulting scalar",
          res);

    sd::DebugHelper::checkErrorCode(stream,
                                    "reduceBoolScalar empty(...) failed");
...
_CUDA_H void ReduceBoolFunction<X, Y>::execReduceScalar(
...
////////////////////////////////////////////////////////////////////////
template <typename X, typename Y>
_CUDA_H void ReduceBoolFunction<X, Y>::execReduceXD(
    dim3 launchDims, cudaStream_t *stream, const int opNum, const int rank,
...
BUILD_DOUBLE_TEMPLATE(template class SD_EXPORT ReduceBoolFunction, ,
