...
////////////////////////////////////////////////////////////////////////
template <typename T>
__device__ void fillDimensionalIsMax(const void *vdX, void *vdZ,
  ...
  for (int r = blockIdx.x; r < numTads; r += gridDim.x) {
    ...
    if (dimensionLength > 1 || tadEWS < 1) {
      for (Nd4jLong e = threadIdx.x; e < tadLength; e += blockDim.x) {
        auto xOffset =
            tadOffsetForBlock + shape::getIndexOffset(e, tadOnlyShapeInfo);
...
////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ void execfillDimensionalIsMax(const void *dX, void *dZ,
                                         const Nd4jLong *zShapeInfo,
                                         const Nd4jLong *tadOnlyShapeInfo,
                                         int *dimension, int dimensionLength,
                                         const Nd4jLong *tadOffsets) {
  fillDimensionalIsMax<T>(dX, dZ, zShapeInfo, tadOnlyShapeInfo, dimension,
                          dimensionLength, tadOffsets);
}

////////////////////////////////////////////////////////////////////////
template <typename T>
__host__ void fillDimensionalIsMaxGeneric(dim3 &launchDims,
                                          cudaStream_t *stream, const void *dX,
  ...
  execfillDimensionalIsMax<T>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          dX, dZ, zShapeInfo, tadOnlyShapeInfo, dimension, dimensionLength,
          tadOffsets);
  sd::DebugHelper::checkErrorCode(stream, "fillDimensionalIsMax(...) failed");
}
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT fillDimensionalIsMaxGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, const void *dX,
                       void *dZ, const Nd4jLong *zShapeInfo,
                       const Nd4jLong *tadOnlyShapeInfo, int *dimension,
                       int dimensionLength, const Nd4jLong *tadOffsets),
                      LIBND4J_TYPES);
}  // namespace sd