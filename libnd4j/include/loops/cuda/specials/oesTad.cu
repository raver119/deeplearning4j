    ...
    for (int i = 0; i < iterations; i++) {
      if (i % 2 == 0) {
        for (int tid = threadIdx.x; tid < xTadLength; tid += blockDim.x) {
          ...
          if (top < xTadLength) {
            auto t0 =
                cached ? top - 1 : shape::getIndexOffset(top - 1, tadShapeInfo);
...
__host__ void oesTadGeneric(dim3 &launchDims, cudaStream_t *stream, void *vx,
                            ...
                            Nd4jLong const *tadOffsets, bool descending) {
  execOesTadKernel<T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
      vx, xShapeInfo, dimension, dimensionLength, tadShapeInfo, tadOffsets,
      descending);
}
...
__host__ void oesTadGenericKey(dim3 &launchDims, cudaStream_t *stream, void *vx,
                               ...
                               Nd4jLong const *tadShapeInfo,
                               Nd4jLong const *tadOffsets, bool descending) {
  execOesTadKernelKey<X, Y>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          vx, xShapeInfo, vy, yShapeInfo, dimension, dimensionLength,
          tadShapeInfo, tadOffsets, descending);
}

BUILD_SINGLE_TEMPLATE(template void SD_EXPORT oesTadGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, void *vx,
                       Nd4jLong const *xShapeInfo, int *dimension,
                       int dimensionLength, Nd4jLong const *tadShapeInfo,
                       Nd4jLong const *tadOffsets, bool descending),
                      LIBND4J_TYPES);
BUILD_DOUBLE_TEMPLATE(template void SD_EXPORT oesTadGenericKey,
                      (dim3 & launchDims, cudaStream_t *stream, void *vx,
                       Nd4jLong const *xShapeInfo, void *vy,
                       Nd4jLong const *yShapeInfo, int *dimension,
                       int dimensionLength, Nd4jLong const *tadShapeInfo,
