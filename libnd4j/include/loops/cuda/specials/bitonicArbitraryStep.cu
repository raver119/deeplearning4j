...
__global__ void bitonicArbitraryStepKernelKey(
    void *vx, Nd4jLong const *xShapeInfo, void *vy, Nd4jLong const *yShapeInfo,
  ...
  /*
      ...
      on second iteration we'll have: 0 + (3 * 4) = 12;  4 + (3 * 4) = 16; 8 +
     (3 * 4) = 20
  */
  int firstPosition;
  int firstStep;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ void execBitonicArbitraryStepKernel(void *vx,
                                               Nd4jLong const *xShapeInfo,
                                               int window, int length,
  ...
  for (int i = firstPosition; i < length; i += firstStep) {
    for (int j = secondPosition; j < half; j += secondStep) {
      ...
      if (it < length && ij < length) {
        ...
        if (!descending ==
            (shmem[threadIdx.x] > shmem[threadIdx.x + blockDim.x])) {
...
__host__ void bitonicArbitraryStepGeneric(dim3 &launchDims,
  ...
  execBitonicArbitraryStepKernel<T>
      <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
          vx, xShapeInfo, window, length, reverse, descending);
}
...
BUILD_SINGLE_TEMPLATE(template void SD_EXPORT bitonicArbitraryStepGeneric,
                      (dim3 & launchDims, cudaStream_t *stream, void *vx,
                       Nd4jLong const *xShapeInfo, int window, int length,
                       int reverse, bool descending),
                      LIBND4J_TYPES);
BUILD_DOUBLE_TEMPLATE(template void SD_EXPORT bitonicArbitraryStepGenericKey,
                      (dim3 & launchDims, cudaStream_t *stream, void *vx,
                       Nd4jLong const *xShapeInfo, void *vy,
                       Nd4jLong const *yShapeInfo, int window, int length,
                       int reverse, bool descending),
