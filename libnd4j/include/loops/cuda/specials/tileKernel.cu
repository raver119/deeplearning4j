...
namespace sd {
static Nd4jLong __device__ __noinline__
getIndexOffset_(Nd4jLong index, Nd4jLong const* shapeInfo) {
  return shape::getIndexOffset(index, shapeInfo);
}

static Nd4jLong __device__ __noinline__ subArrayOffset(
    Nd4jLong index, Nd4jLong const* shapeInfoA, Nd4jLong const* shapeInfoB) {
...
template <typename T>
static __global__ void tileKernel(void const* inputBuffer,
  ...
  auto tid =
      ...
      threadIdx.x;  // copy linear sequence of elements, so one-level threading
  ...
  if (shape::order(outputShape) == 'c') {  //  ews == 1 always here
    for (int i = tid; i < resultLength; i += totalThreads) {
      ...
      *(reinterpret_cast<T*>(outputBuffer) + i) =
          *(reinterpret_cast<T const*>(inputBuffer) + yOffset);
    }
  } else {
    for (int i = tid; i < resultLength; i += totalThreads) {
      auto xOffset = getIndexOffset_(i, outputShape);
      auto yOffset = subArrayOffset(i, outputShape, inputShape);
      *(reinterpret_cast<T*>(outputBuffer) + xOffset) =
          *(reinterpret_cast<T const*>(inputBuffer) + yOffset);
    }
  }
}
...
BUILD_SINGLE_TEMPLATE(template __global__ void tileKernel,
                      (void const* inputBuffer, Nd4jLong const* inputShape,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
void tileKernelH(void const* inputBuffer, Nd4jLong const* inputShape,
                 void* outputBuffer, Nd4jLong const* outputShape,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// enhancement for tileKernel to different input and output data types: X -
// output type, Y - input type
template <typename X, typename Y>
static __global__ void tileKernelDouble(void const* inputBuffer,
                                        Nd4jLong const* inputShape,
                                        void* outputBuffer,
                                        Nd4jLong const* outputShape,
  ...
  if (ordering == 'c' && ews == 1) {  //  ews == 1 always here
    for (int i = tid; i < resultLength; i += totalThreads) {
      auto yOffset = subArrayOffset(i, outputShape, inputShape);
      *(reinterpret_cast<X*>(outputBuffer) + i) =
...
BUILD_SINGLE_TEMPLATE_TWICE(template __global__ void tileKernelDouble,
                            (void const* inputBuffer,
                             Nd4jLong const* inputShape, void* outputBuffer,
                             Nd4jLong const* outputShape, Nd4jLong resultLength,
                             Nd4jLong ews),
                            LIBND4J_TYPES);

template <typename X, typename Y>
void tileKernelHH(void const* inputBuffer, Nd4jLong const* inputShape,
                  void* outputBuffer, Nd4jLong const* outputShape,
...
BUILD_SINGLE_TEMPLATE_TWICE(template void tileKernelHH,
                            (void const* inputBuffer,
                             Nd4jLong const* inputShape, void* outputBuffer,
                             Nd4jLong const* outputShape, Nd4jLong resultLength,
                             Nd4jLong ews, cudaStream_t* stream),
                            LIBND4J_TYPES);
}  // namespace sd