...
Nd4jLong IndexReduce<X, Y>::execScalar(const int opNum, const void *x,
...
////////////////////////////////////////////////////////////////////////
template <typename X, typename Y>
void IndexReduce<X, Y>::exec(const int opNum, const void *x,
...
Nd4jLong IndexReduce<X, Y>::execScalar(const void *vx,
                                       const Nd4jLong *xShapeInfo,
                                       void *vextraParams) {
  ...
  if (xEws == 1 && shape::order(xShapeInfo) == 'c') {
    auto func = PRAGMA_THREADS_FOR {
      ...
      for (auto i = start; i < stop; i++) {
        ...
        intermediatery[thread_id] =
            OpType::update(intermediatery[thread_id], curr, extraParams);
      }
    };
    ...
    for (int e = 0; e < maxThreads; e++)
      startingIndex =
          OpType::update(startingIndex, intermediatery[e], extraParams);
...
void IndexReduce<X, Z>::exec(const void *vx, const Nd4jLong *xShapeInfo,
  ...
  if (sd::ArrayOptions::arrayType(xShapeInfo) == sd::ArrayType::EMPTY) {
    ...
    for (Nd4jLong i = 0; i < zLen; i++) z[i] = (Z)indexValue.index;
  ...
  sd::IndexReductionLoops<X, Z>::template loopIndexReduce<OpType>(
