...
Y SummaryStatsReduce<X, Y>::execScalar(const int opNum,
                                       const bool biasCorrected, const void *x,
                                       const Nd4jLong *xShapeInfo,
                                       void *extraParams) {
  RETURNING_DISPATCH_BY_OPNUM_TT(
      execScalar, PARAMS(biasCorrected, x, xShapeInfo, extraParams),
      SUMMARY_STATS_OPS);
}
...
void SummaryStatsReduce<X, Y>::execScalar(const int opNum,
  ...
  DISPATCH_BY_OPNUM_TT(
      execScalar,
      PARAMS(biasCorrected, x, xShapeInfo, extraParams, z, zShapeInfo),
      SUMMARY_STATS_OPS);
}

template <typename X, typename Y>
void SummaryStatsReduce<X, Y>::exec(const int opNum, const bool biasCorrected,
                                    const void *x, const Nd4jLong *xShapeInfo,
                                    void *extraParams, void *z,
                                    const Nd4jLong *zShapeInfo, int *dimension,
                                    int dimensionLength) {
  DISPATCH_BY_OPNUM_TT(exec,
                       PARAMS(biasCorrected, x, xShapeInfo, extraParams, z,
                              zShapeInfo, dimension, dimensionLength),
                       SUMMARY_STATS_OPS);
}
...
template <typename X, typename Z>
template <typename OpType>
Z SummaryStatsReduce<X, Z>::execScalar(const bool biasCorrected, const void *vx,
                                       const Nd4jLong *xShapeInfo,
                                       void *vextraParams) {
...
template <typename X, typename Z>
template <typename OpType>
void SummaryStatsReduce<X, Z>::exec(const bool biasCorrected, const void *vx,
                                    const Nd4jLong *xShapeInfo,
                                    void *vextraParams, void *vz,
                                    const Nd4jLong *zShapeInfo, int *dimension,
                                    int dimensionLength) {
  ...
  const bool canCast = tadEWS == 1 && tadOrder == 'c'
                           ? false
                           : sd::DataTypeUtils::castShapeInfo<uint>(
                                 tadShapeShapeInfo, tadShapeShapeInfoCast);
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto r = start; r < stop; r++) {
      ...
      if (tadEWS == 1 && tadOrder == 'c') {
        ...
        for (Nd4jLong i = 1; i < tadLength; i++) {
          auto xOffset = shape::indexOffset(i, tadShapeShapeInfo,
                                            tadShapeShapeInfoCast, canCast);
