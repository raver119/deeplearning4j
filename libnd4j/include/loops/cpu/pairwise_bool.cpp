...
void PairWiseBoolTransform<X, Y>::exec(const int opNum, const void *x,
                                       Nd4jLong xEws, const void *y,
                                       Nd4jLong yEws, void *z, Nd4jLong zEws,
                                       void *extraParams, Nd4jLong n,
                                       const uint64_t start,
                                       const uint64_t stop) {
  DISPATCH_BY_OPNUM_TT(
      exec, PARAMS(x, xEws, y, yEws, z, zEws, extraParams, n, start, stop),
      PAIRWISE_BOOL_OPS);
};
...
void PairWiseBoolTransform<X, Z>::exec(const void *vx, Nd4jLong xEws,
                                       const void *vy, Nd4jLong yEws, void *vz,
                                       Nd4jLong zEws, void *vextraParams,
                                       const Nd4jLong n, const uint64_t start,
                                       const uint64_t stop) {
...
void PairWiseBoolTransform<X, Y>::exec(
  ...
  DISPATCH_BY_OPNUM_TT(exec,
                       PARAMS(x, xShapeInfo, y, yShapeInfo, z, zShapeInfo,
...
template <typename X, typename Z>
template <typename OpType>
void PairWiseBoolTransform<X, Z>::exec(
  ...
  const sd::LoopKind::Kind kindOfLoop =
      sd::LoopKind::deduceKindOfLoopXYZ(xShapeInfo, yShapeInfo, zShapeInfo);
  ...
  } else if ((kindOfLoop == sd::LoopKind::EWS1 ||
              kindOfLoop == sd::LoopKind::EWSNONZERO) &&
             !sameShapesXY) {  // not same shape
    exec<OpType>(x, xEws, y, yEws, z, zEws, extraParams,
                 shape::length(yShapeInfo), start, stop);
  } else {
...
BUILD_DOUBLE_TEMPLATE(template class SD_EXPORT PairWiseBoolTransform, ,
