 ...
 *
 * The non recursive implementation is important for being able to run on cuda
 * as well as host.
 *
...
#endif
    void
    quickSort(StridePermutation *arr, int elements);
...
#define ND4J_RAW_ITER_TWO_NEXT(idim, ndim, coord, shape, dataA, stridesA, \
                               dataB, stridesB)                           \
  for ((idim) = 0; (idim) < (ndim); (idim)++) {                           \
    if (++(coord)[idim] == (shape)[idim]) {                               \
      (coord)[idim] = 0;                                                  \
...
#define ND4J_RAW_ITER_THREE_NEXT(idim, ndim, coord, shape, dataA, stridesA, \
...
#define ND4J_RAW_ITER_FOUR_NEXT(idim, ndim, coord, shape, dataA, stridesA, \
...
#endif
    inline void
    SortStrideArray(int ndim, int strides[],
...
#endif
    inline int
    PrepareOneRawArrayIter(int ndim, Nd4jLong shape[], T data[],
                           Nd4jLong strides[], int *out_ndim,
                           Nd4jLong outShape[], T **out_data,
                           Nd4jLong *outStrides) {
...
class BlockInformation {
 public:
  ...
  BlockInformation(Nd4jLong length, int threshold) {
    ...
    threads =
        (1 < threads) ? threads : 1;  // sd::math::nd4j_max<int>(1, threads);
    threads = (threads < omp_get_max_threads())
                  ? threads
                  : omp_get_max_threads();  // sd::math::nd4j_min<int>(threads,
                                            // omp_get_max_threads());
...
int _CUDA_HD PrepareTwoRawArrayIter(int ndim, Nd4jLong *shape, X *dataA,
                                    Nd4jLong *stridesA, Y *dataB,
                                    Nd4jLong *stridesB, int *out_ndim,
                                    Nd4jLong *outShape, X **out_dataA,
                                    Nd4jLong *outStridesA, Y **out_dataB,
                                    Nd4jLong *outStridesB) {
  int i;
...
#endif
    int
    PrepareThreeRawArrayIter(int ndim, Nd4jLong shape[], X *dataA,
                             Nd4jLong *stridesA, Y *dataB, Nd4jLong *stridesB,
                             Z *dataC, Nd4jLong *stridesC, int &out_ndim,
                             Nd4jLong *outShape, X **out_dataA,
                             Nd4jLong outStridesA[], Y **out_dataB,
                             Nd4jLong outStridesB[], Z **out_dataC,
                             Nd4jLong outStridesC[]) {
