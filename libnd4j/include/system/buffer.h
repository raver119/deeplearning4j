...
// Question: Should the indexes here really be int? Isn't size_t or Nd4jLong
// more appropriate?
namespace sd {
namespace buffer {
/**
...
#endif
    void
    allocBuffer(Buffer<T> **buffer, int length);
...
__host__ void copyDataToGpu(Buffer<T> **buffer, cudaStream_t stream) {
  ...
  checkCudaErrors(cudaMemcpyAsync(bufferRef->gData, bufferRef->data,
                                  bufferSize(bufferRef), cudaMemcpyHostToDevice,
                                  stream));
...
__host__ void copyDataFromGpu(Buffer<T> **buffer, cudaStream_t stream) {
  ...
  checkCudaErrors(cudaMemcpyAsync(bufferRef->data, bufferRef->gData,
                                  bufferTotalSize, cudaMemcpyDeviceToHost,
                                  stream));
  checkCudaErrors(cudaStreamSynchronize(stream));
}
#endif
...
#ifdef __CUDACC__
template <typename T>
__host__ Buffer<T> *createBuffer(T *data, int length, cudaStream_t stream) {
  Buffer<T> *ret = createBuffer(data, length);

  T *gData;
  T **gDataRef = &(gData);
  checkCudaErrors(
      cudaMalloc(reinterpret_cast<void **>(gDataRef), sizeof(T) * length));
  ret->gData = gData;
  checkCudaErrors(cudaMemcpyAsync(ret->gData, ret->data, sizeof(T) * length,
