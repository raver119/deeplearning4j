...
#ifndef SD_OPENMP_PRAGMAS_H
#define SD_OPENMP_PRAGMAS_H
...
#define PRAGMA_OMP_SIMD_SUM(args) \
  _Pragma(OMP_STRINGIFY(omp simd reduction(sumT : args)))
#define PRAGMA_OMP_SIMD_MAX(args) \
  _Pragma(OMP_STRINGIFY(omp simd reduction(maxTF : args)))
#define PRAGMA_OMP_PARALLEL _Pragma(OMP_STRINGIFY(omp parallel default(shared)))
#define PRAGMA_OMP_PARALLEL_REDUCTION(args) \
  _Pragma(OMP_STRINGIFY(omp parallel reduction(args) default(shared)))
#define PRAGMA_OMP_PARALLEL_ARGS(args) \
  _Pragma(OMP_STRINGIFY(omp parallel args default(shared)))
#define PRAGMA_OMP_PARALLEL_THREADS(args) \
  _Pragma(OMP_STRINGIFY(                  \
      omp parallel num_threads(args) if (args > 1) default(shared)))
#define PRAGMA_OMP_PARALLEL_THREADS_IF(threads, condition) \
  _Pragma(OMP_STRINGIFY(                                   \
      omp parallel num_threads(threads) if (condition) default(shared)))
...
#define FUNC_2D                                                             \
  std::function<void(uint64_t, int64_t, int64_t, int64_t, int64_t, int64_t, \
                     int64_t)>
#define FUNC_3D                                                             \
  std::function<void(uint64_t, int64_t, int64_t, int64_t, int64_t, int64_t, \
...
#define PRAGMA_REDUCE_LONG                             \
  [&](uint64_t thread_id, int64_t start, int64_t stop, \
      int64_t increment) mutable -> int64_t
#define PRAGMA_REDUCE_DOUBLE                           \
  [&](uint64_t thread_id, int64_t start, int64_t stop, \
      int64_t increment) mutable -> double
...
#define PRAGMA_THREADS_FOR_2D                                             \
  [&](uint64_t thread_id, int64_t start_x, int64_t stop_x, int64_t inc_x, \
      int64_t start_y, int64_t stop_y, int64_t inc_y) -> void
#define PRAGMA_THREADS_FOR_3D                                             \
  [&](uint64_t thread_id, int64_t start_x, int64_t stop_x, int64_t inc_x, \
      int64_t start_y, int64_t stop_y, int64_t inc_y, int64_t start_z,    \
      int64_t stop_z, int64_t inc_z) -> void

#endif  // SD_OPENMP_PRAGMAS_H
