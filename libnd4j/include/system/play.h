...
// BUILD_PAIRWISE_SELECTOR(xType, yType, zType, functionName, (signature),
// DATA_TYPES, Y_TYPES);

// BUILD_SINGLE_UNCHAINED_TEMPLATE(functionName , (signature), Y_TYPES);

// BUILD_TRIPLE_SELECTOR(xType, yType, zType, functionName, (signature),
// DATA_TYPES, Y_TYPES, Z_TYPES)

// BUILD_TRIPLE_TEMPLATE(functionName, (signature), DATA_TYPES, Y_TYPES,
// Z_TYPES)

// BUILD_ENUMERATION(DATA_TYPES)

// BUILD_SINGLE_SELECTOR(xType, functions::IndexReduce, ::op(a, b, c, d, e),
// DATA_TYPES) BUILD_DOUBLE_SELECTOR(xType, yType, functions::IndexReduce,
// ::op(a, b, c, d, e), DATA_TYPES, DATA_TYPES)

// BUILD_SINGLE_TEMPLATE(template class Alpha, (signature), DATA_TYPES);

// BUILD_DOUBLE_TEMPLATE(template class Alpha, (signature) , DATA_TYPES,
// DATA_TYPES);
...
// BUILD_CALL_1(template void sd::NDArray<float16>::applyTransform, float16,
// (NDArray<float16>* a, float16* b), TRANSFORM_OPS)

// BUILD_CALL_1(template void sd::NDArray<float16>::applyPairwiseTransform,
// float16, (NDArray<float16>* other, float16* extraParams),
// PAIRWISE_TRANSFORM_OPS) BUILD_TRACKER(TRANSFORM, ACTIVATIONS)

// BUILD_CALL_1(template void sd::NDArray<float16>::applyScalar, float16,
// (float16 scalar, NDArray<float16>* target, float16 *extraParams) ,
// ACTIVATIONS);

/*
#define DECLARE_OP(NAME, NIN, NOUT)   DECLARE_OP_UNIQ(__COUNTER__, NAME, NIN,
NOUT)
#define DECLARE_OP_UNIQ(CTR, NAME, NIN, NOUT)   template <typename T> \
                                                class NAME: public
sd::ops::DeclarableOp<T> { \
                                                public:\
                                                NAME() :
sd::ops::DeclarableOp<T>(NIN, NOUT, #NAME) { } \
                                                protected: \
                                                    Nd4jStatus
validateAndExecute(Block<T>& block); \
                                                };\
                                                template <typename T> \
                                                Nd4jStatus
...
// DISPATCH_SIMPLE(scalarAlongDimension_, float, PARAMS(x, xShapeInfo,
// extraParamx, z, zShapeInfo, scalars, tadShapeInfo, tadOffsets, tadShapeInfoZ,
// tadOffsetsZ), OPS_A(SCALAR_OPS))
...
// original version
//    DISPATCH_METAOP(functions::pairwise_transforms::PairWiseTransform<T>::template
//    transformCuda, PARAMS(N, dx, dy, xStride, yStride, paramsPtr, dz, zStride,
//    nullptr, nullptr, nullptr), InvertedMetaOp, OPS_A(SCALAR_OPS),
//    OPS_B(PAIRWISE_TRANSFORM_OPS))

/*
DISPATCH_METAOP(invertedMetaPairwiseShaped_Pairwise_Scalar, PARAMS(opTypeA,
opTypeB, N, x, xShape, y, yShape, z, zShape, extrasA, extrasB, scalarA,
...
/*
 ...
 invertedMetaOpKernel_Pairwise_Scalar_16_1_float(const int opTypeA, const int
 opTypeB, long N, float *dx, int *xShapeInfo, float *dy, int *yShapeInfo, float
 *dz, int *zShapeInfo, float *extraA, float *extraB, float scalarA, float
 scalarB) { invertedMetaPairwiseShapedGeneric<float,
 simdOps::InvertedMetaOp<float, simdOps::Copy<float>,
 simdOps::Multiply<float>>>(opTypeA, opTypeB, N, dx, xShapeInfo, dy, yShapeInfo,
 dz, zShapeInfo, extraA, extraB, scalarA, scalarB);
 }
 */
