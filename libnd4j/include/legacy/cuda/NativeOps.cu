...
/*
 * This message returns shared memory threshold value. default overflow ratio is
 * 0.3
 */
...
sd::buffer::Buffer<Nd4jLong> *createScalarBuffer(cudaStream_t stream) {
  ...
  auto buff = sd::buffer::createBuffer(scalarShapeInfo,
                                       shape::shapeInfoLength(2), stream);
...
class ScalarShapeInformation {
 ...
 public:
  ScalarShapeInformation(cudaStream_t stream) {
    auto scalarDimensionBuff =
        reinterpret_cast<Nd4jLong *>(malloc(sizeof(Nd4jLong)));
  ...
  Nd4jLong *getShapeInfoHostPointer() { return scalarShapeInfo->data; }

  Nd4jLong *getShapeInfoGpuPointer() { return scalarShapeInfo->gData; }

  Nd4jLong *getDimensionHostPointer() { return scalarDimension->data; }

  Nd4jLong *getDimensionGpuPointer() { return scalarDimension->gData; }
};
...
class ScalarInfo {
 ...
 public:
   ...
   */
  Nd4jLong *getDeviceShapeInfo() { return shapeInfo->getShapeInfoGpuPointer(); }
...
void execPairwiseTransform(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    LaunchContext lc(extraPointers[1], extraPointers[4], extraPointers[5],
                     extraPointers[3]);
    NativeOpExecutioner::execPairwiseTransform(
        &lc, opNum, dbX->primary(), hXShapeInfo, dbX->special(),
        ConstantShapeHelper::getInstance()
            ->bufferForShapeInfo(hXShapeInfo)
            .specialAsT<Nd4jLong>(),
        dbY->primary(), hYShapeInfo, dbY->special(),
        ConstantShapeHelper::getInstance()
            ->bufferForShapeInfo(hYShapeInfo)
            .specialAsT<Nd4jLong>(),
        dbZ->primary(), hZShapeInfo, dbZ->special(),
        ConstantShapeHelper::getInstance()
            ->bufferForShapeInfo(hZShapeInfo)
            .specialAsT<Nd4jLong>(),
        extraParams);
...
void execBroadcastBool(Nd4jPointer *extraPointers, int opNum,
                       OpaqueDataBuffer *dbX, Nd4jLong const *hXShapeInfo,
                       Nd4jLong const *dXShapeInfo, OpaqueDataBuffer *dbY,
                       Nd4jLong const *hYShapeInfo, Nd4jLong const *dYShapeInfo,
                       OpaqueDataBuffer *dbZ, Nd4jLong const *hZShapeInfo,
                       Nd4jLong const *dZShapeInfo, void *extraParams,
...
void execReduceSame2(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    auto tadPack = sd::ConstantTadHelper::getInstance()->tadForDimensions(
        hXShapeInfo, dimension, shape::length(hDimensionShape));
    ...
    NativeOpExecutioner::execReduceSame(
        ...
        ConstantShapeHelper::getInstance()
            ->bufferForShapeInfo(hZShapeInfo)
            .specialAsT<Nd4jLong>(),
        dimension, dimensionLength, tadPack.specialShapeInfo(),
...
void execReduceLong(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    if (zType != sd::DataType::INT64)
      throw datatype_exception::build("execReduceLong wrong Z data type",
                                      sd::DataType::INT64, zType);
    ...
    BUILD_DOUBLE_SELECTOR(
        ...
        ::execReduceScalar(launchDims, stream, opNum, dbX->special(),
                           ConstantShapeHelper::getInstance()
                               ->bufferForShapeInfo(hXShapeInfo)
                               .specialAsT<Nd4jLong>(),
                           hXShapeInfo, extraParams, dbZ->special(),
                           ConstantShapeHelper::getInstance()
                               ->bufferForShapeInfo(hZShapeInfo)
                               .specialAsT<Nd4jLong>(),
                           hXShapeInfo, nullptr, 0, reductionPointer,
                           dTADShapeInfo),
        LIBND4J_TYPES, LONG_TYPES);
  ...
  try {
    ...
    if (zType != sd::DataType::BOOL)
      throw std::runtime_error(
          "execReduceBool requires Z operand to have BOOL type");
    ...
    BUILD_DOUBLE_SELECTOR(
        ...
        ::execReduceScalar(launchDims, stream, opNum, dbX->special(),
                           ...
                           ConstantShapeHelper::getInstance()
                               ->bufferForShapeInfo(hZShapeInfo)
                               .specialAsT<Nd4jLong>(),
                           hZShapeInfo, nullptr, 0, reductionPointer,
                           dTADShapeInfo),
...
void execTransformSame(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    auto tadShapeInfo = reinterpret_cast<Nd4jLong *>(
        extraPointers != nullptr ? extraPointers[0] : nullptr);
    auto tadOffsets = reinterpret_cast<Nd4jLong *>(
        extraPointers != nullptr ? extraPointers[1] : nullptr);
...
void execTransformStrict(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    auto tadShapeInfo = reinterpret_cast<Nd4jLong *>(
        extraPointers != nullptr ? extraPointers[10] : nullptr);
    auto tadOffsets = reinterpret_cast<Nd4jLong *>(
        extraPointers != nullptr ? extraPointers[11] : nullptr);
...
void checkP2P() {
  ...
  if (devCnt > 1) {
    ...
    // if we have only 1 device - we say that we support P2P, since all data
    // will be on 1 device
    supportedP2P = true;
  }
}
...
Nd4jPointer mallocHost(Nd4jLong memorySize, int flags) {
  ...
  auto res = cudaHostAlloc(reinterpret_cast<void **>(&pointer), memorySize + 8,
...
Nd4jPointer createEvent() {
  ...
  auto dZ = cudaEventCreateWithFlags(
      reinterpret_cast<cudaEvent_t *>(&nativeEvent), cudaEventDisableTiming);
  if (dZ != 0) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(dZ);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
...
int memcpySync(Nd4jPointer dst, Nd4jPointer src, Nd4jLong size, int flags,
  ...
  auto dZ = cudaMemcpy(reinterpret_cast<void *>(dst),
                       const_cast<const void *>(reinterpret_cast<void *>(src)),
...
int memcpyAsync(Nd4jPointer dst, Nd4jPointer src, Nd4jLong size, int flags,
  ...
  // auto dZ = cudaMemcpy(reinterpret_cast<void *>(dst), const_cast<const void
  // *>(reinterpret_cast<void *>(src)), static_cast<size_t>(size), kind);
  if (dZ != 0) {
    printf("Failed on [%p] -> [%p], size: [%i], direction: [%i], dZ: [%i]\n",
           src, dst, size, flags, static_cast<int>(dZ));
    fflush(stdout);
...
int memsetSync(Nd4jPointer dst, int value, Nd4jLong size, int flags,
               Nd4jPointer reserved) {
  auto dZ = cudaMemset(reinterpret_cast<void *>(dst), value,
...
int memsetAsync(Nd4jPointer dst, int value, Nd4jLong size, int flags,
  ...
  auto dZ = cudaMemsetAsync(reinterpret_cast<void *>(dst), value,
                            static_cast<size_t>(size), *pStream);
  if (dZ != 0) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(dZ);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
...
void specialConcat(Nd4jPointer *extraPointers, int dimension, int numArrays,
                   Nd4jPointer *data, Nd4jPointer *inputShapeInfo, void *dZ,
                   Nd4jLong const *dZShapeInfo, Nd4jPointer *tadPointers,
                   Nd4jPointer *offsetPointers) {
  try {
    BUILD_SINGLE_SELECTOR(ArrayOptions::dataType(dZShapeInfo),
                          sd::SpecialMethods,
                          ::concatCpuGeneric(dimension, numArrays, data,
                                             inputShapeInfo, dZ, dZShapeInfo),
                          LIBND4J_TYPES);
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
 ...
 * This method saves
 */
sd::TadPack *tadOnlyShapeInfo(Nd4jLong const *dXShapeInfo, int *dimension,
                              int dimensionLength) {
  try {
    auto pack = new TadPack();
    *pack = sd::ConstantTadHelper::getInstance()->tadForDimensions(
        dXShapeInfo, dimension, dimensionLength);
    return pack;
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
...
int memcpyConstantAsync(Nd4jLong dst, Nd4jPointer src, Nd4jLong size, int flags,
  ...
  auto dZ = cudaMemcpyToSymbolAsync(deviceConstantMemory,
                                    const_cast<const void *>(src), size, dst,
                                    kind, *pStream);
  if (dZ != 0) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(dZ);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
        "cudaMemcpyToSymbolAsync failed");
  }
...
Nd4jPointer getConstantSpace() {
  ...
  cudaError_t dZ = cudaGetSymbolAddress(reinterpret_cast<void **>(&dConstAddr),
                                        deviceConstantMemory);
...
void pullRows(Nd4jPointer *extraPointers, OpaqueDataBuffer *dbX,
  ...
  try {
    ...
    BUILD_SINGLE_SELECTOR(
        ...
        (launchDims, stream, dbX->special(), dbZ->special(), n, indexes,
...
void average(Nd4jPointer *extras, Nd4jPointer *x, Nd4jLong const *xShapeInfo,
             Nd4jPointer *dx, Nd4jLong const *dXShapeInfo, void *z,
             Nd4jLong const *zShapeInfo, void *dz, Nd4jLong const *dzShapeInfo,
  ...
  try {
    ...
    if (mode == 0) {
      ...
      BUILD_SINGLE_SELECTOR(xType, averagingKernelGeneric,
                            (launchDims, stream, dX, dz, n, length, propagate),
                            LIBND4J_TYPES);
      ...
      BUILD_SINGLE_SELECTOR(
          ...
          ::averageGeneric(x, z, zShapeInfo, n, length, propagate),
          LIBND4J_TYPES);
    }
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
...
void accumulate(Nd4jPointer *extras, Nd4jPointer *x, Nd4jLong const *xShapeInfo,
                Nd4jPointer *dx, Nd4jLong const *dXShapeInfo, void *z,
  ...
  try {
    ...
    if (mode == 0) {
      ...
      BUILD_SINGLE_SELECTOR(xType, accumulateKernelGeneric,
                            (launchDims, stream, dX, dz, n, length),
                            LIBND4J_TYPES);
      sd::DebugHelper::checkErrorCode(stream, "AccumulateFloat(...) failed");
    } else {
      // launching on host memory
      BUILD_SINGLE_SELECTOR(xType, sd::SpecialMethods,
                            ::accumulateGeneric(x, z, zShapeInfo, n, length),
                            LIBND4J_TYPES);
    }
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
...
void shuffle(Nd4jPointer *extras, Nd4jPointer *x, Nd4jPointer *xShapeInfo,
             Nd4jPointer *dx, Nd4jPointer *dXShapeInfo, Nd4jPointer *z,
             Nd4jPointer *zShapeInfo, Nd4jPointer *dz, Nd4jPointer *dZShapeInfo,
             int N, int *shuffleMap, Nd4jPointer *tadShapeInfo,
  ...
  try {
    ...
    BUILD_SINGLE_SELECTOR(xType, shuffleKernelGeneric,
                          (launchDims, stream, dX, dxShape, dZ, N, shuffleMap,
                           tadOnlyShapeInfo, tadOffset),
                          LIBND4J_TYPES);
...
void execSummaryStatsTad(Nd4jPointer *extraPointers, int opNum,
                         ...
                         OpaqueDataBuffer *dbDimension,
                         Nd4jLong const *hDimensionShape,
                         Nd4jLong const *dDimensionShape, bool biasCorrected,
  ...
  try {
    ...
    NativeOpExecutioner::execSummaryStats(
        ...
        tadShapeInfo, tadOffsets, biasCorrected);
...
void execScalarBool(Nd4jPointer *extraPointers, int opNum,
  ...
  try {
    ...
    NativeOpExecutioner::execScalarBool(
        ...
        ConstantShapeHelper::getInstance()
            ->bufferForShapeInfo(hScalarShapeInfo)
            .specialAsT<Nd4jLong>(),
        extraParams);
...
void execScalarTad(Nd4jPointer *extraPointers, int opNum, OpaqueDataBuffer *dbX,
  ...
  try {
    ...
    if (yType != xType && yType != sd::DataType::BOOL &&
        !isExperimentalEnabled())
      throw sd::datatype_exception::build(
          "execScalar both operands must have same data type", xType, yType);
    ...
    BUILD_PAIRWISE_SELECTOR(
        ...
        ::executeCudaAlongDimension(launchDims, stream, opNum, dX, dXShapeInfo,
                                    dZ, dZShapeInfo, dScalars, extraParams,
    ...
    BUILD_SINGLE_SELECTOR_THRICE(
        ...
        ::executeCudaAlongDimension(launchDims, stream, opNum, dbX->special(),
                                    ConstantShapeHelper::getInstance()
                                        ->bufferForShapeInfo(hXShapeInfo)
                                        .specialAsT<Nd4jLong>(),
                                    dbZ->special(),
...
void execAggregate(Nd4jPointer *extraPointers, int opNum, void **arguments,
                   int numArguments, Nd4jLong **shapes, int numShapes,
                   int *indexArguments, int numIndexArguments, int **intArrays,
                   int numIntArrays, void *realArguments, int numRealArguments,
                   sd::DataType dtype) {}
...
////////////////////////////////////////////////////////////////////////
void execRandom(Nd4jPointer *extraPointers, int opNum, Nd4jPointer stateHost,
                OpaqueDataBuffer *dbZ, Nd4jLong const *hZShapeInfo,
                Nd4jLong const *dZShapeInfo, void *extraArguments) {
  try {
    InteropDataBuffer::prepareSpecialUse({dbZ}, {});

    LaunchContext lc(extraPointers[1], extraPointers[4], extraPointers[5],
                     extraPointers[3]);
    NativeOpExecutioner::execRandom(&lc, opNum, stateHost, dbZ->primary(),
                                    hZShapeInfo, dbZ->special(),
                                    ConstantShapeHelper::getInstance()
                                        ->bufferForShapeInfo(hZShapeInfo)
                                        .specialAsT<Nd4jLong>(),
                                    extraArguments);
...
void execRandom2(Nd4jPointer *extraPointers, int opNum, Nd4jPointer stateHost,
  ...
  try {
    ...
    LaunchContext lc(extraPointers[1], extraPointers[4], extraPointers[5],
                     extraPointers[3]);
    NativeOpExecutioner::execRandom(&lc, opNum, stateHost, dbX->primary(),
                                    hXShapeInfo, dbX->special(),
                                    ConstantShapeHelper::getInstance()
                                        ->bufferForShapeInfo(hXShapeInfo)
                                        .specialAsT<Nd4jLong>(),
                                    dbZ->primary(), hZShapeInfo, dbZ->special(),
...
Nd4jPointer initRandom(Nd4jPointer *extraPointers, long seed, long bufferSize,
  ...
  unsigned long long *ptrHost =
      reinterpret_cast<unsigned long long *>(extraPointers[0]);
  cudaStream_t *stream = reinterpret_cast<cudaStream_t *>(extraPointers[1]);
  ...
  auto buffer = new sd::random::RandomBuffer(
      seed, bufferSize, reinterpret_cast<uint64_t *>(ptrHost),
  ...
  // and copy it to gpu
  cudaMemcpyAsync(ptrDev, ptrHost, bufferSize * 8, cudaMemcpyHostToDevice,
                  *stream);
...
void destroyRandom(Nd4jPointer ptrBuffer) {
  sd::random::RandomBuffer *buffer =
      reinterpret_cast<sd::random::RandomBuffer *>(ptrBuffer);

  // FIXME: it's bad thing, but we can't know in advance, which stream(s) where
  // using this generator in practice
...
void refreshBuffer(Nd4jPointer *extraPointers, long seed,
  ...
  sd::random::RandomBuffer *buffer =
      reinterpret_cast<sd::random::RandomBuffer *>(ptrRandom);
  ...
  // copy back to gpu
  cudaMemcpyAsync(ptrDev, ptrHost, buffer->getSize() * 8,
...
void tear(Nd4jPointer *extras, OpaqueDataBuffer *dbX,
          Nd4jLong const *xShapeInfo, Nd4jLong const *dXShapeInfo,
  ...
  try {
    ...
    BUILD_SINGLE_SELECTOR(xType, tearKernelGeneric,
                          (launchDims, stream, dbX->special(), dXShapeInfo,
                           targets, zShapeInfo, tadShapeInfo, tadOffsets),
                          LIBND4J_TYPES);
...
void prescanArrayRecursive(Nd4jPointer *extras, int *dZ, int *dX,
  ...
  int numBlocks = sd::math::nd4j_max<int>(
      1, static_cast<int>(
             ceil(static_cast<float>(numElements) / (2.f * blockSize))));
  int numThreads;
  ...
  // execute the scan
  if (numBlocks > 1) {
    sd::prescanLauncher<true, false>(grid, threads, sharedMemSize, stream, dZ,
                                     dX, g_scanBlockSums[level], numThreads * 2,
                                     0, 0);
    if (np2LastBlock) {
      sd::prescanLauncher<true, true>(gridOnes, threadsOnes, sharedMemLastBlock,
                                      stream, dZ, dX, g_scanBlockSums[level],
    ...
    // recursive (CPU) call
    prescanArrayRecursive(extras, g_scanBlockSums[level],
                          g_scanBlockSums[level], numBlocks, level + 1);

    sd::uniformAdd<<<grid, threads, 1024, *stream>>>(
        dZ, g_scanBlockSums[level], numElements - numEltsLastBlock, 0, 0);

    if (np2LastBlock) {
      sd::uniformAdd<<<1, numThreadsLastBlock, 1024, *stream>>>(
          dZ, g_scanBlockSums[level], numEltsLastBlock, numBlocks - 1,
          numElements - numEltsLastBlock);
    }
    ...
    sd::prescanLauncher<false, true>(grid, threads, sharedMemSize, stream, dZ,
...
void sort(Nd4jPointer *extraPointers, void *x, Nd4jLong const *xShapeInfo,
          void *dX, Nd4jLong const *dXShapeInfo, bool descending) {
  try {
    cudaStream_t *stream = reinterpret_cast<cudaStream_t *>(extraPointers[1]);
    ...
    // check if xLength is a power of 2, and use bitonic sort, if that's the
    // case
    if ((xLength != 0) && ((xLength & (xLength - 1)) == 0) &&
        (xLength <= 1024 * 1024 * 10)) {
      ...
      for (int window = 2; window < max; window <<= 1) {
        ...
        do {
          ...
          BUILD_SINGLE_SELECTOR(xType, bitonicArbitraryStepGeneric,
                                (launchDims, stream, dX, dXShapeInfo, n,
                                 xLength, rev, descending),
                                LIBND4J_TYPES);
          n >>= 1;
          rev = 1;
        } while (n > 1);
...
void sortByKey(Nd4jPointer *extraPointers, void *x, Nd4jLong const *xShapeInfo,
               void *dX, Nd4jLong const *dXShapeInfo, void *y,
  ...
  try {
    ...
    if (xLength != yLength)
      throw std::runtime_error(
          "sortByKey: keys and values must have the same size");
      ...
      for (int k = 2; k <= xLength; k = 2 * k) {
        for (int j = k >> 1; j > 0; j = j >> 1) {
          BUILD_DOUBLE_SELECTOR(xType, yType, bitonicSortStepGenericKey,
                                (launchDims, stream, dX, dXShapeInfo, dy,
                                 dyShapeInfo, j, k, xLength, descending),
                                LIBND4J_TYPES, LIBND4J_TYPES);
        }
      }
    } else {
      ...
      for (int window = 2; window < max; window <<= 1) {
        ...
        do {
          ...
          BUILD_DOUBLE_SELECTOR(xType, yType, bitonicArbitraryStepGenericKey,
                                (launchDims, stream, dX, dXShapeInfo, dy,
                                 dyShapeInfo, n, xLength, rev, descending),
                                LIBND4J_TYPES, LIBND4J_TYPES);
          n >>= 1;
          rev = 1;
        } while (n > 1);
...
void sortByValue(Nd4jPointer *extraPointers, void *x,
  ...
  try {
    ...
    if (xLength != yLength)
      throw std::runtime_error(
          "sortByValue: keys and values must have the same size");
    ...
    if ((xLength != 0) && ((xLength & (xLength - 1)) == 0) &&
      ...
      for (int k = 2; k <= xLength; k = 2 * k) {
        for (int j = k >> 1; j > 0; j = j >> 1) {
          BUILD_DOUBLE_SELECTOR(xType, yType, bitonicSortStepGenericKey,
                                (launchDims, stream, dy, dyShapeInfo, dX,
      ...
      for (int window = 2; window < max; window <<= 1) {
        ...
        do {
          ...
          BUILD_DOUBLE_SELECTOR(xType, yType, bitonicArbitraryStepGenericKey,
                                (launchDims, stream, dy, dyShapeInfo, dX,
                                 dXShapeInfo, n, xLength, rev, descending),
                                LIBND4J_TYPES, LIBND4J_TYPES);
          n >>= 1;
          rev = 1;
        } while (n > 1);
...
void sortTadByKey(Nd4jPointer *extraPointers, void *x,
  ...
  try {
    auto stream = reinterpret_cast<cudaStream_t *>(extraPointers[1]);
    auto context = extraPointers[0] == 0
    ...
    auto yType = sd::ArrayOptions::dataType(yShapeInfo);
    BUILD_DOUBLE_SELECTOR(
        xType, yType, oesTadGenericKey,
        (launchDims, stream, dX, dXShapeInfo, dy, dyShapeInfo, nullptr,
         dimensionLength, tadPack.platformShapeInfo(),
         tadPack.platformOffsets(), descending),
        LIBND4J_TYPES, LIBND4J_TYPES);
...
void sortTadByValue(Nd4jPointer *extraPointers, void *x,
  ...
  try {
    ...
    BUILD_DOUBLE_SELECTOR(
        xType, yType, oesTadGenericKey,
        (launchDims, stream, dy, dyShapeInfo, dX, dXShapeInfo, nullptr,
         dimensionLength, tadPack.platformShapeInfo(),
         tadPack.platformOffsets(), descending),
        LIBND4J_TYPES, LIBND4J_TYPES);
...
void sortTad(Nd4jPointer *extraPointers, void *x, Nd4jLong const *xShapeInfo,
             void *dX, Nd4jLong const *dXShapeInfo, int *dimension,
  ...
  try {
    ...
    BUILD_SINGLE_SELECTOR(
        xType, oesTadGeneric,
        (launchDims, stream, dX, dXShapeInfo, nullptr, dimensionLength,
...
sd::ShapeList *_calculateOutputShapes(
  ...
  for (int e = 0; e < numInputShapes; e++) {
    ...
    void *buffer_ = sd::ArrayOptions::arrayType(shape_) == ArrayType::EMPTY
                        ? nullptr
                        : inputBuffers[e];
    void *bufferD_ = sd::ArrayOptions::arrayType(shape_) == ArrayType::EMPTY
                         ? nullptr
                         : inputBuffers[e + numInputShapes];
...
sd::ShapeList *calculateOutputShapes2(Nd4jPointer *extraPointers, Nd4jLong hash,
  ...
  try {
    ...
    return _calculateOutputShapes(extraPointers, op, inputBuffers, inputShapes,
...
sd::ShapeList *calculateOutputShapes(Nd4jPointer *extraPointers, Nd4jLong hash,
  ...
  try {
    ...
    return _calculateOutputShapes(extraPointers, op, inputShapes,
                                  numInputShapes, tArgs, numTArgs, iArgs,
                                  numIArgs);
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
...
realExec(sd::ops::DeclarableOp *op, Nd4jPointer *extraPointers, Nd4jLong hash,
         Nd4jPointer *inputBuffers, Nd4jPointer *inputShapes, int numInputs,
         Nd4jPointer *outputBuffers, Nd4jPointer *outputShapes, int numOutputs,
         double *tArgs, int numTArgs, Nd4jLong *iArgs, int numIArgs,
         bool *bArgs, int numBArgs, bool isInplace) {
  if (op == nullptr)
  ...
  for (int e = 0; e < numInputs; e++) {
    ...
    void *buffer = sd::ArrayOptions::arrayType(shape) == ArrayType::EMPTY
                       ? nullptr
                       : inputBuffers[e];
    void *bufferD = sd::ArrayOptions::arrayType(shape) == ArrayType::EMPTY
                        ? nullptr
                        : inputBuffers[e + numInputs];
  ...
  auto dZ = op->execute(inputs, outputs, ttArgs, iiArgs, bbArgs,
...
int execCustomOp(Nd4jPointer *extraPointers, Nd4jLong hash,
  ...
  try {
    ...
    return realExec(op, extraPointers, hash, inputBuffers, inputShapes,
...
int execCustomOp2(Nd4jPointer *extraPointers, Nd4jLong hash,
  ...
  try {
    ...
    auto res =
        cudaStreamSynchronize(*context->launchContext()->getCudaStream());
    if (res != 0)
...
static VariablesSet *executeStoredGraphT(Nd4jPointer *extraPointers,
                                         Nd4jLong graphId,
                                         Nd4jPointer *inputBuffers,
                                         Nd4jPointer *inputShapes,
  ...
  for (int e = 0; e < numInputs; e++) {
    ...
    auto array = new sd::NDArray(inputBuffers[e],
                                 reinterpret_cast<Nd4jLong *>(inputShapes[e]));
  ...
  if (dZ == ND4J_STATUS_OK) {
    ...
    for (int e = 0; e < outputs->size(); e++) {
      // we're only getting variable ID/Index from original grap. values will be
      // taken from cloned workspace
...
VariablesSet *executeStoredGraph(Nd4jPointer *extraPointers, Nd4jLong graphId,
                                 Nd4jPointer *inputBuffers,
                                 Nd4jPointer *inputShapes, int *inputIndices,
                                 int numInputs) {
  try {
    return executeStoredGraphT(extraPointers, graphId, inputBuffers,
                               inputShapes, inputIndices, numInputs);
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
        e.what());
...
Nd4jStatus execCustomOpWithScope(Nd4jPointer *extraPointers,
                                 sd::graph::GraphState *state, Nd4jLong opHash,
                                 Nd4jLong *scopes, int numScopes,
                                 Nd4jPointer *inputBuffers,
   ...
   * depending on operation (i.e. while of if), different logic executors could
   * be used
   */
  ...
  // Node is dynamically created, and has nothing beyond it: only inputs and
  // outputs this node has id of 0, and inputs are
  ...
  for (int e = 0; e < numScopes; e++) {
    ...
    if (!state->hasScope(scopeId)) {
      // nd4j_printf("execCustomOpWithScope: referenced scope [%i] doesn't
      // exist\n", scopeId);
      return Status::THROW();
    }
...
Nd4jStatus execCustomOpWithScope(Nd4jPointer *extraPointers, Nd4jPointer state,
                                 ...
                                 Nd4jPointer *outputShapes, int numOutputs) {
  try {
    return execCustomOpWithScope(
        extraPointers, reinterpret_cast<sd::graph::GraphState *>(state), opHash,
        scopes, numScopes, inputBuffers, inputShapes, numInputs, outputBuffers,
        outputShapes, numOutputs);
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
        e.what());
...
int estimateThreshold(Nd4jPointer *extraPointers, Nd4jPointer dX,
                      Nd4jLong const *dXShapeInfo, int N, float threshold) {
  throw std::runtime_error("estimateThreshold: Not implemented yet");
}

/*
 * TypeDef:
 *     void convertTypes(Nd4jPointer *extras, int srcType, Nd4jPointer dX, long
 * N, int dstType, Nd4jPointer dZ);
 */
void convertTypes(Nd4jPointer *extras, int srcType, Nd4jPointer dX, Nd4jLong N,
                  int dstType, Nd4jPointer dZ) {
  try {
    auto dx = reinterpret_cast<void *>(dX);
    auto dz = reinterpret_cast<void *>(dZ);
    ...
    if (srcType == ND4J_FLOAT8) {
      if (dstType == ND4J_FLOAT8) {
        ...
        // sd::TypeCast::convertGenericCuda<sd::float8, sd::int8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_UINT8) {
        // sd::TypeCast::convertGenericCuda<sd::float8, sd::uint8>(extras, dx,
        // N, dz);
      } else if (dstType == ND4J_FLOAT16) {
        // sd::TypeCast::convertGenericCuda<sd::float8, float16>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT16) {
        // sd::TypeCast::convertGenericCuda<sd::float8, sd::int16>(extras, dx,
        // N, dz);
      } else if (dstType == ND4J_UINT16) {
        // sd::TypeCast::convertGenericCuda<sd::float8, sd::uint16>(extras, dx,
        // N, dz);
      } else if (dstType == ND4J_FLOAT24) {
      } else if (dstType == ND4J_FLOAT32) {
        // sd::TypeCast::convertGenericCuda<sd::float8, float>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_DOUBLE) {
        // sd::TypeCast::convertGenericCuda<sd::float8, double>(extras, dx, N,
        // dz);
      } else {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<sd::int8, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<uint8_t, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<float16, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<int16_t, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<float, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
      ...
      if (dstType == ND4J_FLOAT8) {
        // sd::TypeCast::convertGenericCuda<double, sd::float8>(extras, dx, N,
        // dz);
      } else if (dstType == ND4J_INT8) {
...
__global__ static void scatterUpdateCuda(const int opCode,
                                         const int numOfSubArrs, void *vx,
                                         const Nd4jLong *xShapeInfo,
                                         const Nd4jLong *xOffsets, void *vy,
                                         const Nd4jLong *yShapeInfo,
                                         const Nd4jLong *yOffsets,
                                         const void *vindexes) {
  __shared__ T *x, *y;
  ...
  for (int e = 0; e < numOfSubArrs; e++) {
    ...
    const bool isOwner = xIndex < gridDim.x ? blockIdx.x == xIndex
                                            : blockIdx.x == xIndex % gridDim.x;
...
template <typename T, typename I>
__host__ static void scatterUpdateCudaLauncher(
    const cudaStream_t *stream, const int opCode, const int numOfSubArrs,
    void *vx, const Nd4jLong const *xShapeInfo, const Nd4jLong *xOffsets,
    void *vy, const Nd4jLong *yShapeInfo, const Nd4jLong *yOffsets,
    const void *indexes) {
  scatterUpdateCuda<T, I><<<512, 256, MAX_NUM_THREADS, *stream>>>(
      opCode, numOfSubArrs, vx, xShapeInfo, xOffsets, vy, yShapeInfo, yOffsets,
      indexes);
}

//////////////////////////////////////////////////////////////////////////
void scatterUpdate(Nd4jPointer *extraPointers, int opCode, int numOfSubArrs,
                   void *hX, Nd4jLong const *hXShapeInfo,
                   Nd4jLong const *hXOffsets, void *dX,
                   Nd4jLong const *dXShapeInfo, Nd4jLong const *dXOffsets,
                   void *hY, Nd4jLong const *hYShapeInfo,
                   Nd4jLong const *hYOffsets, void *dY,
                   Nd4jLong const *dYShapeInfo, Nd4jLong const *dYOffsets,
                   void *hIindexes, Nd4jLong const *hIndicesShapeInfo,
                   void *dIindexes, Nd4jLong const *dIndicesShapeInfo) {
  try {
    auto stream = reinterpret_cast<cudaStream_t *>(extraPointers[1]);
    ...
    BUILD_DOUBLE_SELECTOR(type, iType, scatterUpdateCudaLauncher,
                          (stream, opCode, numOfSubArrs, dX, dXShapeInfo,
                           dXOffsets, dY, dYShapeInfo, dYOffsets, dIindexes),
...
bool isBlasVersionMatches(int major, int minor, int build) {
  ...
  if (!result) {
    nd4j_printf(
        "CUDA/cuBLAS version mismatch. Expected: %i.%i.%i but got %i.%i.%i "
        "instead\n",
        Environment::getInstance()->_blasMajorVersion,
        Environment::getInstance()->_blasMinorVersion,
...
sd::ConstantDataBuffer *constantBufferLong(sd::DataType dtype,
                                           Nd4jLong const *data, int length) {
  return sd::ConstantHelper::getInstance()->constantBuffer(
      ConstantDescriptor(data, length), dtype);
}
...
Nd4jPointer shapeBufferForNumpy(Nd4jPointer npyArray) {
  try {
    ...
    return (Nd4jPointer)(
        sd::ConstantShapeHelper::getInstance()->createFromExisting(
            shapeBuffer,
            true));  // TO DO: this can lead to unpleasant crash sometimes
  } catch (std::exception &e) {
    sd::LaunchContext::defaultContext()->errorReference()->setErrorCode(1);
    sd::LaunchContext::defaultContext()->errorReference()->setErrorMessage(
        e.what());
