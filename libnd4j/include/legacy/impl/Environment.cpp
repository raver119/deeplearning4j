...
sd::Environment::Environment() {
  ...
  if (_maxMasterThreads.load() > _maxThreads.load()) {
    nd4j_printf(
        "Warning! MAX_MASTER_THREADS > MAX_THREADS, tuning them down to match "
        "each other\n",
        "");
  ...
  /**
   * If this env var is defined - we'll disallow use of platform-specific
   * helpers (mkldnn, cudnn, etc)
   */
  ...
  /**
   * This var defines max amount of special (i.e. device) memory library can
   * allocate on all devices combined
   */
...
void Environment::setDefaultFloatDataType(sd::DataType dtype) {
  if (dtype != sd::DataType::FLOAT32 && dtype != sd::DataType::DOUBLE &&
      dtype != sd::DataType::FLOAT8 && dtype != sd::DataType::HALF)
    throw std::runtime_error(
        "Default Float data type must be one of [FLOAT8, FLOAT16, FLOAT32, "
        "DOUBLE]");
...
void Environment::setMaxThreads(int max) {
  // FIXME: not possible at this moment, since maxThreads is limited by number
  // of threads in pool. however we can allocate more threads if we want
