...
void MemoryTracker::countIn(MemoryType type, Nd4jPointer ptr,
                            Nd4jLong numBytes) {
#if defined(__GNUC__) && !defined(__MINGW64__) &&           \
    !defined(SD_ANDROID_BUILD) && !defined(SD_IOS_BUILD) && \
    !defined(SD_APPLE_BUILD)
  if (Environment::getInstance().isDetectingLeaks()) {
    ...
    std::pair<Nd4jLong, AllocationEntry> pair(
        lptr, AllocationEntry(type, lptr, numBytes, stack));
  ...
  if (!_allocations.empty()) {
    ...
    for (auto &v : _allocations) {
      nd4j_printf("Leak of %i [%s] bytes\n%s\n\n", (int)v.second.numBytes(),
                  v.second.memoryType() == MemoryType::HOST ? "HOST" : "DEVICE",
                  v.second.stackTrace().c_str());
    }
