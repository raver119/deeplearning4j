...
Workspace::Workspace(Nd4jLong primarySize, Nd4jLong secondarySize) {
  if (secondarySize > 0) {
    auto res = cudaHostAlloc(reinterpret_cast<void **>(&_ptrHost),
                             secondarySize, cudaHostAllocDefault);
    if (res != 0)
...
void Workspace::init(Nd4jLong primaryBytes, Nd4jLong secondaryBytes) {
  if (this->_currentSize < primaryBytes) {
    ...
    auto res =
        cudaMalloc(reinterpret_cast<void **>(&_ptrDevice), secondaryBytes);
    if (res != 0)
      throw cuda_exception::build("Can't allocate [DEVICE] memory", res);
  ...
  if (this->_currentSizeSecondary < secondaryBytes) {
    ...
    auto res = cudaHostAlloc(reinterpret_cast<void **>(&_ptrHost),
                             secondaryBytes, cudaHostAllocDefault);
    if (res != 0)
      throw cuda_exception::build("Can't allocate [HOST] memory", res);
...
Nd4jLong Workspace::getUsedSize() { return getCurrentOffset(); }

Nd4jLong Workspace::getCurrentSize() { return _currentSize; }

Nd4jLong Workspace::getCurrentOffset() { return _offset.load(); }
...
void *Workspace::allocateBytes(sd::memory::MemoryType type, Nd4jLong numBytes) {
  switch (type) {
    case HOST: {
      if (numBytes < 1)
        throw allocation_exception::build(
            "Number of [HOST] bytes for allocation should be positive",
            numBytes);
      ...
      if (_offsetSecondary.load() + numBytes > _currentSizeSecondary) {
        ...
        Nd4jPointer p;
        auto res = cudaHostAlloc(reinterpret_cast<void **>(&p), numBytes,
      ...
      nd4j_debug(
          "Allocating %lld bytes from [HOST] workspace; Current PTR: %p; "
          "Current offset: %lld\n",
    ...
    case DEVICE: {
      if (numBytes < 1)
        throw allocation_exception::build(
            "Number of [DEVICE] bytes for allocation should be positive",
            numBytes);
      ...
      nd4j_debug(
          "Allocating %lld bytes from [DEVICE] workspace; Current PTR: %p; "
          "Current offset: %lld\n",
          numBytes, result, _offset.load());
...
Workspace *Workspace::clone() {
  // for clone we take whatever is higher: current allocated size, or allocated
  // size of current loop
  return new Workspace(sd::math::nd4j_max<Nd4jLong>(
      this->getCurrentSize(), this->_cycleAllocations.load()));
}
...
Nd4jLong Workspace::getCurrentSecondarySize() { return _currentSizeSecondary; }

Nd4jLong Workspace::getCurrentSecondaryOffset() {
  return _offsetSecondary.load();
}

Nd4jLong Workspace::getSpilledSecondarySize() { return _spillsSizeSecondary; }
