...
int AffinityManager::currentDeviceId() {
  ...
  if (globalThreadToDevice < 0) {
    ...
    // we need to check if we've got deviceId >= number of actual devices, and
...
void AffinityManager::setCurrentDevice(int deviceId) {
  ...
  if (previousDeviceId >= 0 && LaunchContext::isInitialized()) {
    ...
    res = cudaStreamSynchronize(
        *LaunchContext::defaultContext()->getCudaSpecialStream());
    if (res != 0)
      throw cuda_exception::build("setCurrentDevice -> specialSync failed",
                                  res);

    if (deviceId != previousDeviceId) {
      // discard existing stuff
      // nd4j_printf("AffinityManager::setCurrentDevice() was invoked, releasing
      // buffers\n", "");
