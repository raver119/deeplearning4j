...
// this function executed once per thread, it polls functions from queue, and
// executes them via wrapper
static void executionLoop_(int thread_id,
                           BlockingQueue<CallableWithArguments *> *queue) {
  while (true) {
    ...
    switch (c->dimensions()) {
      ...
      case 2: {
        auto args = c->arguments();
        c->function_2d()(c->threadId(), args[0], args[1], args[2], args[3],
                         args[4], args[5]);
        c->finish();
      ...
      case 3: {
        auto args = c->arguments();
        c->function_3d()(c->threadId(), args[0], args[1], args[2], args[3],
                         args[4], args[5], args[6], args[7], args[8]);
        c->finish();
      } break;
      default:
        throw std::runtime_error(
            "Don't know what to do with provided Callable");
    }
  }
}
...
ThreadPool::ThreadPool() {
  ...
  for (int e = 0; e < _available.load(); e++) {
    ...
    CPU_ZERO(&cpuset);
    CPU_SET(e, &cpuset);
    int rc = pthread_setaffinity_np(_threads[e]->native_handle(),
...
Ticket *ThreadPool::tryAcquire(int numThreads) {
  ...
  bool threaded = false;
  {
    ...
    if (_available >= numThreads) {
      ...
      // ticket must contain information about number of threads for the current
      // session
