...
int ThreadsHelper::numberOfThreads(int maxThreads, uint64_t numberOfElements) {
  ...
  auto optimalThreads =
      sd::math::nd4j_max<uint64_t>(1, numberOfElements / 1024);

  // now return the smallest value
  return sd::math::nd4j_min<int>(optimalThreads, maxThreads);
}

Span3::Span3(int64_t startX, int64_t stopX, int64_t incX, int64_t startY,
             int64_t stopY, int64_t incY, int64_t startZ, int64_t stopZ,
             int64_t incZ) {
  _startX = startX;
  _startY = startY;
  _startZ = startZ;
...
Span3 Span3::build(int loop, uint64_t threadID, uint64_t numThreads,
                   int64_t startX, int64_t stopX, int64_t incX, int64_t startY,
                   int64_t stopY, int64_t incY, int64_t startZ, int64_t stopZ,
                   int64_t incZ) {
  switch (loop) {
    case 1: {
      ...
      auto e = s + span;
      if (threadID == numThreads - 1) e = stopX;
    ...
    case 2: {
      ...
      auto s = span * threadID;
      auto e = s + span;
      if (threadID == numThreads - 1) e = stopY;
    ...
    case 3: {
      ...
      auto s = span * threadID;
      auto e = s + span;
      if (threadID == numThreads - 1) e = stopZ;
...
Span Span::build(uint64_t threadID, uint64_t numThreads, int64_t startX,
                 int64_t stopX, int64_t incX) {
  auto span = (stopX - startX) / numThreads;
  auto s = span * threadID;
  auto e = s + span;
  if (threadID == numThreads - 1) e = stopX;
...
Span2::Span2(int64_t startX, int64_t stopX, int64_t incX, int64_t startY,
             int64_t stopY, int64_t incY) {
  _startX = startX;
  _startY = startY;
  _stopX = stopX;
  _stopY = stopY;
...
Span2 Span2::build(int loop, uint64_t threadID, uint64_t numThreads,
                   int64_t startX, int64_t stopX, int64_t incX, int64_t startY,
                   int64_t stopY, int64_t incY) {
  switch (loop) {
    case 1: {
...
int64_t Span::startX() const { return _startX; }

int64_t Span::stopX() const { return _stopX; }

int64_t Span::incX() const { return _incX; }

int64_t Span2::startX() const { return _startX; }

int64_t Span2::startY() const { return _startY; }

int64_t Span2::stopX() const { return _stopX; }

int64_t Span2::stopY() const { return _stopY; }

int64_t Span2::incX() const { return _incX; }

int64_t Span2::incY() const { return _incY; }

int64_t Span3::startX() const { return _startX; }

int64_t Span3::startY() const { return _startY; }

int64_t Span3::startZ() const { return _startZ; }

int64_t Span3::stopX() const { return _stopX; }

int64_t Span3::stopY() const { return _stopY; }

int64_t Span3::stopZ() const { return _stopZ; }

int64_t Span3::incX() const { return _incX; }

int64_t Span3::incY() const { return _incY; }

int64_t Span3::incZ() const { return _incZ; }

int ThreadsHelper::pickLoop2d(int numThreads, uint64_t itersX,
                              uint64_t itersY) {
  // if one of dimensions is definitely too small - we just pick the other one
  if (itersX < numThreads && itersY >= numThreads) return 2;
  if (itersY < numThreads && itersX >= numThreads) return 1;
  ...
  // if there's no remainder left in some dimension - we're picking that
  // dimension, because it'll be the most balanced work distribution
  if (remX == 0) return 1;
  if (remY == 0) return 2;

  // if there's no loop without a remainder - we're picking one with smaller
  // remainder
  if (remX < remY) return 1;
  if (remY < remX && splitY >= 64)  // we don't want too small splits over last
                                    // dimension, or vectorization will fail
    return 2;
...
static int threads_(int maxThreads, uint64_t elements) {
  if (elements == maxThreads) {
    ...
    // if we have full load across thread, or at least half of threads can be
    // utilized
...
int ThreadsHelper::numberOfThreads2d(int maxThreads, uint64_t iters_x,
                                     uint64_t iters_y) {
  ...
  if ((iters_x >= maxThreads && remX == 0) ||
      (iters_y >= maxThreads && remY == 0))
    return maxThreads;

  // at this point we suppose that there's no loop perfectly matches number of
  // our threads so let's pick something as equal as possible
  if (iters_x > maxThreads || iters_y > maxThreads)
    return maxThreads;
  else
...
int ThreadsHelper::numberOfThreads3d(int maxThreads, uint64_t itersX,
                                     uint64_t itersY, uint64_t itersZ) {
  // we don't want to run underloaded threads
  if (itersX * itersY * itersZ <= 32) return 1;
  ...
  if ((itersX >= maxThreads && remX == 0) ||
      (itersY >= maxThreads && remY == 0) ||
      (itersZ >= maxThreads && remZ == 0))
    return maxThreads;
  ...
  // we want to split as close to outer loop as possible, so checking it out
  // first
...
int ThreadsHelper::pickLoop3d(int numThreads, uint64_t itersX, uint64_t itersY,
                              uint64_t itersZ) {
  auto remX = itersX % numThreads;
  auto remY = itersY % numThreads;
  ...
  if (remX == 0)
    ...
    return 2;
  else if (remZ == 0)  // TODO: we don't want too smal splits over last
                       // dimension? or we do?
    return 3;
...
int Threads::parallel_tad(FUNC_1D function, int64_t start, int64_t stop,
                          int64_t increment, uint32_t numThreads) {
  if (start > stop)
    throw std::runtime_error("Threads::parallel_for got start > stop");

  auto delta = (stop - start);

  if (numThreads > delta) numThreads = delta;
  ...
  if (ticket != nullptr) {
    ...
    // if there were no threads available - we'll execute function right within
    // current thread
    function(0, start, stop, increment);
...
int Threads::parallel_for(FUNC_1D function, int64_t start, int64_t stop,
                          int64_t increment, uint32_t numThreads) {
  if (start > stop)
    throw std::runtime_error("Threads::parallel_for got start > stop");
  ...
  // we decide what's optimal number of threads we need here, and execute it in
  // parallel_tad.
...
int Threads::parallel_for(FUNC_2D function, int64_t startX, int64_t stopX,
                          int64_t incX, int64_t startY, int64_t stopY,
                          int64_t incY, uint64_t numThreads, bool debug) {
  if (startX > stopX)
  ...
  if (debug) {
    for (int e = 0; e < numThreads; e++) {
      auto span = Span2::build(splitLoop, e, numThreads, startX, stopX, incX,
                               startY, stopY, incY);

      function(e, span.startX(), span.stopX(), span.incX(), span.startY(),
               span.stopY(), span.incY());
    }
    ...
    if (ticket != nullptr) {
      for (int e = 0; e < numThreads; e++) {
        ...
        auto span = Span2::build(splitLoop, threadId, numThreads, startX, stopX,
                                 incX, startY, stopY, incY);

        ticket->enqueue(e, numThreads, function, span.startX(), span.stopX(),
                        span.incX(), span.startY(), span.stopY(), span.incY());
      }
      ...
      // if there were no threads available - we'll execute function right
...
int Threads::parallel_for(FUNC_3D function, int64_t startX, int64_t stopX,
                          int64_t incX, int64_t startY, int64_t stopY,
                          int64_t incY, int64_t startZ, int64_t stopZ,
                          int64_t incZ, uint64_t numThreads) {
  if (startX > stopX)
    throw std::runtime_error("Threads::parallel_for got startX > stopX");
  ...
  numThreads =
      ThreadsHelper::numberOfThreads3d(numThreads, itersX, itersY, itersZ);
  if (numThreads == 1) {
  ...
  if (ticket != nullptr) {
    ...
    for (int e = 0; e < numThreads; e++) {
      ...
      auto span = Span3::build(splitLoop, thread_id, numThreads, startX, stopX,
                               incX, startY, stopY, incY, startZ, stopZ, incZ);

      ticket->enqueue(e, numThreads, function, span.startX(), span.stopX(),
                      span.incX(), span.startY(), span.stopY(), span.incY(),
                      span.startZ(), span.stopZ(), span.incZ());
    }
...
int Threads::parallel_do(FUNC_DO function, uint64_t numThreads) {
  ...
  if (ticket != nullptr) {
    ...
    // if there's no threads available - we'll execute function sequentially one
    // by one
    for (uint64_t e = 0; e < numThreads; e++) function(e, numThreads);
...
int64_t Threads::parallel_long(FUNC_RL function, FUNC_AL aggregator,
                               int64_t start, int64_t stop, int64_t increment,
                               uint64_t numThreads) {
  if (start > stop)
    throw std::runtime_error("Threads::parallel_long got start > stop");

  auto delta = (stop - start);
  if (delta == 0 || numThreads == 1) return function(0, start, stop, increment);
  ...
  for (uint32_t e = 0; e < numThreads; e++) {
    ...
    if (e == numThreads - 1)
      ...
      ticket->enqueue(e, numThreads, &intermediatery[e], function, start_,
                      stop_, increment);
  }
...
double Threads::parallel_double(FUNC_RD function, FUNC_AD aggregator,
                                int64_t start, int64_t stop, int64_t increment,
                                uint64_t numThreads) {
  if (start > stop)
    throw std::runtime_error("Threads::parallel_long got start > stop");
...
int Threads::parallel_aligned_increment(FUNC_1D function, int64_t start,
                                        int64_t stop, int64_t increment,
                                        size_t type_size,
  ...
  int adjusted_numThreads = samediff::ThreadsHelper::numberOfThreads(
      req_numThreads, (num_elements * sizeof(double)) / (200 * type_size));

  if (adjusted_numThreads > delta) adjusted_numThreads = delta;
  // shortcut
  ...
  if (ticket != nullptr) {
    ...
    ticket->enqueue(numThreads - 1, numThreads, function, begin, stop,
                    increment);
    // block and wait till all threads finished the job
    ticket->waitAndRelease();
    // we tell that parallelism request succeeded
