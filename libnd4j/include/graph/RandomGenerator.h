  ...
  void* operator new(size_t len) {
    ...
    if (res != 0)
      throw std::runtime_error(
          "CudaManagedRandomGenerator: failed to allocate memory");
...
public:
 ...
 public:
  ...
  RandomGenerator& operator=(const RandomGenerator& other) noexcept;
  ...
  FORCEINLINE _CUDA_HD Nd4jLong rootState() { return _rootState._long; }

  FORCEINLINE _CUDA_HD Nd4jLong nodeState() { return _nodeState._long; }
};

FORCEINLINE RandomGenerator::RandomGenerator(Nd4jLong rootSeed,
                                             Nd4jLong nodeSeed) {
...
FORCEINLINE void RandomGenerator::setStates(Nd4jLong rootSeed,
                                            Nd4jLong nodeSeed) {
  // this seed is used graph-level state
  if (rootSeed == 0) rootSeed = currentMilliseconds();
...
_CUDA_HD FORCEINLINE uint64_t
RandomGenerator::relativeT<uint64_t>(Nd4jLong index) {
  return this->xoroshiro64(index);
}

template <>
_CUDA_HD FORCEINLINE uint32_t
RandomGenerator::relativeT<uint32_t>(Nd4jLong index) {
...
template <>
_CUDA_HD FORCEINLINE Nd4jLong
RandomGenerator::relativeT<Nd4jLong>(Nd4jLong index) {
...
template <typename T>
_CUDA_HD FORCEINLINE T RandomGenerator::relativeT(Nd4jLong index, T from,
                                                  T to) {
...
template <>
_CUDA_HD FORCEINLINE Nd4jLong RandomGenerator::relativeT(Nd4jLong index,
                                                         Nd4jLong from,
                                                         Nd4jLong to) {
...
template <>
_CUDA_HD FORCEINLINE int RandomGenerator::relativeT(Nd4jLong index, int from,
                                                    int to) {
