...
Stack::Stack(const VariableProxy &root) {
  _frames.push_back(StackFrame(const_cast<VariableProxy&>(root), -1));
}
...
void Stack::openFrame(int frameId) {
  _frames.emplace_back(StackFrame(_frames.back().variableProxy(), frameId));
}
