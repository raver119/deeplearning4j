...
NDArray FlatUtils::fromFlatArray(const sd::graph::FlatArray *flatArray) {
  ...
  memcpy(newShape, flatArray->shape()->data(),
         shape::shapeInfoByteLength(rank));
  ...
  if (shape::isEmpty(newShape)) {
    delete[] newShape;
    return NDArrayFactory::empty(dtype);
  }
  // TODO fix UTF16 and UTF32
  if (dtype == UTF8) {
    bool isBe = BitwiseUtils::isBE();
    bool canKeep =
        (isBe && flatArray->byteOrder() == sd::graph::ByteOrder_BE) ||
  ...
  BUILD_SINGLE_SELECTOR(
      dtype, DataTypeConversions,
      ::convertType(newBuffer, (void *)flatArray->buffer()->data(), dtype,
                    ByteOrderUtils::fromFlatByteOrder(flatArray->byteOrder()),
                    length),
      LIBND4J_TYPES);
...
flatbuffers::Offset<FlatArray> FlatUtils::toFlatArray(
    flatbuffers::FlatBufferBuilder &builder, NDArray &array) {
  ...
  return CreateFlatArray(builder, fShape, fBuffer,
                         static_cast<sd::graph::DType>(array.dataType()), bo);
}
}  // namespace graph
}  // namespace sd