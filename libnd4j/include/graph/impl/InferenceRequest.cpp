...
namespace sd {
namespace graph {
InferenceRequest::InferenceRequest(Nd4jLong graphId,
...
void InferenceRequest::appendVariable(int id, int index, const NDArray &array) {
  auto v = std::make_shared<Variable>(std::make_shared<sd::NDArray>(array),
                                      nullptr, id, index);
  insertVariable(v);
}

void InferenceRequest::appendVariable(const std::string &id,
                                      const NDArray &array) {
  auto v = std::make_shared<Variable>(std::make_shared<sd::NDArray>(array),
                                      id.c_str());
  insertVariable(v);
...
void InferenceRequest::appendVariable(const std::string &name, int id,
                                      int index, const NDArray &array) {
  auto v = std::make_shared<Variable>(std::make_shared<sd::NDArray>(array),
                                      name, id, index);
  insertVariable(v);
}
...
flatbuffers::Offset<FlatInferenceRequest>
InferenceRequest::asFlatInferenceRequest(
