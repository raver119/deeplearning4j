...
bool GraphUtils::filterOperations(GraphUtils::OpList& ops) {
  ...
  std::sort(filtered.begin(), filtered.end(),
            [](ops::OpDescriptor a, ops::OpDescriptor b) {
  ...
  for (int e = 1; e < filtered.size(); e++) {
    ...
    if (0 == filtered[e].getOpName()->compare(name)) {
      // there is a match
      auto fi =
          std::find_if(ops.begin(), ops.end(), [name](ops::OpDescriptor a) {
            return a.getOpName()->compare(name) == 0;
          });
      if (fi != ops.end()) ops.erase(fi);
      modified = true;
    }
...
int GraphUtils::runPreprocessor(char const* input, char const* output) {
  int status = 0;
...
#pragma error "Compiler version should be greater then 4.9"
    ...
    if (f == NULL) {
      std::cerr << "Cannot find c++ compiler with 'which' command."
                << std::endl;
      exit(1);
    }
    ...
    char const* cxx = cmd.c_str();  //;getenv("CXX");
                                    //    if (cxx == nullptr) {
    //        nd4j_printf("Cannot retrieve mandatory environment variable 'CXX'.
    //        Please set up the variable and try again.", ""); exit(3);
    //    }
    ...
    // to retrieve c++ version (hardcoded 6): c++ -v 2>&1 | tail -1 | awk '{v =
    // int($3); print v;}'
