...
//            printf("\n");
        ...
        bool   Graph::opGraphProto(std::unordered_map<int, NodeInfo>& collector, std::set<int>& startNodes, std::set<int>& inBranchingNodes) const{
            
            if(_unmapped.empty())
               return false;

            for (const auto& it : _unmapped) {
                ...
                for (auto in = inputs.begin(); in != inputs.end(); ++in) {
                    if (variableSpace()->hasVariable(in->first, 0)) {
                        inExCounts++;
                    }
                    else{
                        inInternalCounts++;
        ...
        bool  Graph::topolSearch(const int startNode,  const std::unordered_map<int, NodeInfo>& collector, 
            ...
            if(itParent != collector.end()){
                ...
                for (const auto& itNodes : itParent->second.connections() ) {
                    ...
                    if(itChild != collector.end()){
                        ...
                        const auto it = _unmapped.find(itNodes);
        ...
        OptimizedGraph Graph::optimizedGraph() const {
            ...
            if(!opGraphProto(collector, startNodes, inBranching))
                throw std::runtime_error("Graph::optimizedGraph() - not prototyped");
            ...
            for (const auto& id : startNodes) {
                ...
                const auto it = _unmapped.find(id);
            ...
            for(auto& vSeq : vOpSeq){
               optGraph.append(vSeq);
            }
            return optGraph;
        }
        
        bool   Graph::initOpSeqContainer(const std::unordered_map<int, NodeInfo>& collection, std::vector<std::vector< OpSequence >>& vOpSeq) const {
            ...
            for(const auto& node : collection){
                if(layer < node.second.getLayer())
                   layer = node.second.getLayer();
            }
            ...
            for(int i = 0; i <= layer; ++i){
                int nSeq = 0;
                for(const auto& node : collection){
                    if(nSeq < node.second.getSequence() && i == node.second.getLayer())
                       nSeq = node.second.getSequence();
                }
                vOpSeq[i].resize(nSeq + 1);
            }
            
            return true;
        }

        bool Graph::layersSeqDefine(std::unordered_map<int, NodeInfo>& collection, int ID, int layer, int startSeq) const {
