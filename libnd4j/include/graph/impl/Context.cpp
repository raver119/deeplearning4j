...
namespace sd {
namespace graph {
Context::Context(const ContextPrototype &prototype,
...
Context::Context(int nodeId, VariableSpace *variableSpace, bool isInplace)
    : Context(nodeId, variableSpace) {
...
sd::memory::Workspace *Context::workspace() const { return _workspace; }

Stash *Context::stash() const { return _variableSpace->stash(); }
...
std::shared_ptr<Variable> Context::getVariable(int idx) const {
  if (idx >= this->_inputs.size()) {
    nd4j_printf(
        "Node %i; Variable [%i] requested, but only %i inputs available\n",
  ...
  if (Environment::getInstance().isDebugAndVerbose() && v != nullptr &&
    ...
    if (!array->isEmpty()) {
      ...
      nd4j_printf(
          "Debug info for node_%i input[%i]; shape: %s; ews: [%i]; order: "
          "[%i]; dtype: [%s]; first values: %s\n",
          this->_nodeId, idx, shape_.c_str(), array->ews(), array->ordering(),
          type.c_str(), values.c_str());
    } else {
      nd4j_printf(
          "Debug info for node_%i input[%i]; shape: %s; ews: [%i]; order: "
          "[%i]; dtype: [%s]; mean value: [%f]\n",
          this->_nodeId, idx, shape_.c_str(), array->ews(), array->ordering(),
          type.c_str(), m);
    }
  }
...
std::shared_ptr<Variable> Context::variable(
    std::initializer_list<int> p) const {
  if (p.size() != 2)
...
void Context::pushNDArrayToVariableSpace(int nodeId, int index,
                                         const NDArray &array) {
  std::pair<int, int> pair(nodeId, index);
  pushNDArrayToVariableSpace(pair, array);
}

void Context::pushNDArrayToVariableSpace(const std::pair<int, int> &pair,
                                         const NDArray &array) {
...
std::shared_ptr<Variable> Context::ensureVariable(const std::string &name,
                                                  int id, int idx) {
  ...
  if (!_variableSpace->hasVariable(pair)) {
    ...
    if (!name.empty()) var->setName(name);
...
NDArray *Context::arrayForOp(int idx) const {
  ...
  if (ptr.get() != nullptr && ptr->undefined()) return nullptr;

  return ptr.get();
}

void Context::setInputArray(int index, void *buffer, void *shapeInfo,
                            void *specialBuffer, void *specialShapeInfo) {
  this->setInputArray(index, buffer, const_cast<const void *>(shapeInfo),
                      specialBuffer,
                      const_cast<const void *>(specialShapeInfo));
}

void Context::setInputArray(int index, void *buffer, void const *shapeInfo,
                            void *specialBuffer, void const *specialShapeInfo) {
  auto array = std::make_shared<NDArray>(
      buffer, specialBuffer, reinterpret_cast<Nd4jLong const *>(shapeInfo));
...
void Context::setOutputArray(int index, void *buffer, void *shapeInfo,
                             void *specialBuffer, void *specialShapeInfo) {
  this->setOutputArray(index, buffer, const_cast<const void *>(shapeInfo),
                       specialBuffer,
                       const_cast<const void *>(specialShapeInfo));
}

void Context::setOutputArray(int index, void *buffer, const void *shapeInfo,
                             void *specialBuffer,
...
void Context::setInputArray(int index, void *vdatabuffer, void const *shapeInfo,
  ...
  if (dataBuffer != nullptr)
    array = std::make_shared<NDArray>(
        dataBuffer->dataBuffer(), reinterpret_cast<Nd4jLong const *>(shapeInfo),
        sd::LaunchContext::defaultContext(),
        dataBuffer->offset() /
            DataTypeUtils::sizeOf(ArrayOptions::dataType(
                reinterpret_cast<Nd4jLong const *>(shapeInfo))));
  else
    array = std::make_shared<NDArray>(
        nullptr, nullptr, reinterpret_cast<Nd4jLong const *>(shapeInfo));
...
void Context::setOutputArray(int index, void *vdatabuffer,
                             void const *shapeInfo,
                             void const *specialShapeInfo) {
  auto dataBuffer = reinterpret_cast<InteropDataBuffer *>(vdatabuffer);
...
void Context::setCudaContext(Nd4jPointer cudaStream,
                             Nd4jPointer reductionPointer,
                             Nd4jPointer allocationPointer) {
#ifdef __CUDABLAS__
  _context = new LaunchContext(cudaStream, reductionPointer, allocationPointer);

  // FIXME: either pass handle from outside, or make sure outside we use the
  // same handle
...
bool Context::helpersAllowed() const { return _helpersAllowed; }
...
bool Context::shapeFunctionOverride() const { return _shapeFunctionOverride; }

samediff::ExecutionMode Context::executionMode() const { return _execMode; }
