...
///////////////////////////////////////////////////////////////////
void OptimizedGraph::sortGraphWithFrames(const VariableSpace& varSpace) {
...
///////////////////////////////////////////////////////////////////
void OptimizedGraph::sortUsualGraph(const VariableSpace& varSpace) {
...
OptimizedGraph::OptimizedGraph(const MAP_IMPL<int, Node>& inMap, const VariableSpace& varSpace): _nodesMap(inMap) {
    ...
    for (const auto& p : _nodesMap) {
        if(p.second.name().find("Enter") != std::string::npos) {
            hasEnter = true;
            break;
        }
    }
    ...
    if(hasEnter)
        sortGraphWithFrames(varSpace);
    else
        sortUsualGraph(varSpace);
}
