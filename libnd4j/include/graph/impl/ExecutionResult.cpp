...
Nd4jLong ExecutionResult::size() { return _variables.size(); }
...
Variable* ExecutionResult::at(int position) {
  if (position >= _variables.size())
    throw std::runtime_error(
        "Position index is higher then number of variables stored");
...
flatbuffers::Offset<FlatResult> ExecutionResult::asFlatResult(
