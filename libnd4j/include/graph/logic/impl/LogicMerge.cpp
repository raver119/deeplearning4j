...
namespace sd {
namespace graph {
Nd4jStatus LogicMerge::processNode(const Node *node) {
  throw std::runtime_error("LogicMerge::processNode - not implemented yet");
  /*
  // at merge node only one of inputs exist if that's just switch and other node
isn't LogicNextItration auto __variableSpace = graph->variableSpace(); auto
__flowPath = __variableSpace->flowPath();
  ...
  // FIXME: we don't need this check. Just last input should survive, IF it
exists if (isWhile){
      ...
      if ( hasVar && __flowPath->wasExecuted(inputAddr1.first)) {
          ...
          if (__variableSpace->hasVariable(node->id(), 0))
              ...
              lvar = new Variable(nullptr, node->getName().c_str(), node->id(),
0);
...
//                        delete lvar->getNDArray();
      ...
      for (int e = 0; e < node->input().size(); e++) {
          ...
          if (__variableSpace->hasVariable(inputAddr)) {
              ...
              if (!var->hasNDArray() ||
!__flowPath->isNodeActive(inputAddr.first)) continue;
