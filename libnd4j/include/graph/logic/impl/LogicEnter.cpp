...
Nd4jStatus LogicEnter::processNode(const Node *node, Stack &stack, const OptimizedGraph& graph) {
  ...
  // ssince we've opened new StackFrame, this Variable will end up in current VariableProxy
  varSpace.putVariable(std::pair<int, int>{node->id(), 0}, *varSpace.getVariable(inputs[0])->getNDArray());
