...
namespace sd {
namespace graph {
Nd4jStatus LogicWhile::processNode(const Node *node) {
  ...
  if (inputs < 3) {
      nd4j_printf("While [%i]: loop should have at least 1 external variable
  announced\n", node->id()); return ND4J_STATUS_BAD_INPUT;
  }

  for (int e = 0; e < inputs - 2; e++) {
      std::pair<int, int> pair(node->id(), e);
      if (!__variableSpace->hasVariable(pair)) {
          __variableSpace->putVariable(pair, new Variable(nullptr, nullptr,
  node->id(), e));
      }
  ...
  node->id(), scope->nodes()->size(), scopeConditionIndex);
      ...
      for (Node* v: *scope->nodes()) {
          ...
          if (v->opType() == OpType_LOGIC) {
              ...
              nd4j_debug("Op [<%s>]\n", v->getName().c_str());
              Nd4jStatus status = GraphExecutioner::executeFlatNode(graph, v,
  __variableSpace); if (status != ND4J_STATUS_OK) return status;
          }
  ...
  scopeBody->nodes()->size(), scopeBodyIndex); for (; e <
  scopeBody->nodes()->size() - 1; e++) { Node* v = scopeBody->nodes()->at(e);
              ...
              if (v->opType() == OpType_LOGIC) {
                  ...
                  nd4j_debug("Op [<%s>]\n", v->getName().c_str());
                  //v->getBlock()->updateVariables();
                  Nd4jStatus status = GraphExecutioner::executeFlatNode(graph,
  v, __variableSpace); if (status != ND4J_STATUS_OK) return status;
              }
