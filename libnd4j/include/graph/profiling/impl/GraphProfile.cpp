...
void GraphProfile::addToTotal(Nd4jLong bytes) { _memoryTotal += bytes; }

void GraphProfile::addToActivations(Nd4jLong bytes) {
  _memoryActivations += bytes;
}

void GraphProfile::addToTemporary(Nd4jLong bytes) { _memoryTemporary += bytes; }

void GraphProfile::addToObjects(Nd4jLong bytes) { _memoryObjects += bytes; }

void GraphProfile::setBuildTime(Nd4jLong nanos) { _buildTime = nanos; }

void GraphProfile::setExecutionTime(Nd4jLong nanos) { _executionTime = nanos; }
...
Nd4jLong GraphProfile::currentTime() {
  ...
  return (Nd4jLong)std::chrono::duration_cast<std::chrono::nanoseconds>(epoch)
      .count();
}
...
void GraphProfile::recordEvent(const char *name) {
  ...
  auto v =
      (Nd4jLong)std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0)
          .count();
...
void GraphProfile::spotEvent(const char *name) {
  auto t = std::chrono::system_clock::now();
  auto d =
      (Nd4jLong)std::chrono::duration_cast<std::chrono::nanoseconds>(t - _last)
          .count();
  std::string k = name;
  _timings[k] = d;
  updateLast();
...
void GraphProfile::printOut() {
  ...
  nd4j_printf("ACT: %lld; TMP: %lld; OBJ: %lld; TTL: %lld;\n", act / _merges,
              tmp / _merges, obj / _merges, ttl / _merges);
  ...
  if (_profiles.size() > 1) {
    ...
    std::sort(sorted.begin(), sorted.end(),
              [](const NodeProfile *a, const NodeProfile *b) -> bool {
