...
class SD_EXPORT Context : public sd::graph::ContextPrototype {
 protected:
  ...
  // in some cases we might be able to skip shape function for validation
  // purposes
 ...
 public:
  Context(const ContextPrototype &prototype, VariableSpace *variableSpace,
          GraphMemoryManager *memoryManager = nullptr);
  ...
  void pushNDArrayListToVariableSpace(int nodeId, int index,
                                      ...
                                      bool track = true);
  void pushNDArrayListToVariableSpace(const std::pair<int, int> &pair,
                                      const NDArrayList &list,
                                      bool track = true);

  bool isValueAvailable(const std::string &name, int id, int idx = 0) const;

  std::shared_ptr<Variable> ensureVariable(const std::string &name, int id,
                                           int idx = 0);
  ...
  void setInputArray(int index, void *buffer, void const *shapeInfo,
                     void *specialBuffer, void const *specialShapeInfo);
  void setInputArray(int index, void *buffer, void *shapeInfo,
                     void *specialBuffer, void *specialShapeInfo);
  void setInputArray(int index, void *databuffer, void const *shapeInfo,
  ...
  void setOutputArray(int index, void *buffer, const void *shapeInfo,
                      void *specialBuffer, const void *specialShapeInfo);
  void setOutputArray(int index, void *buffer, void *shapeInfo,
                      void *specialBuffer, void *specialShapeInfo);
  void setOutputArray(int index, void *databuffer, void const *shapeInfo,
                      void const *specialShapeInfo);
  ...
  void setCudaContext(Nd4jPointer cudaStream, Nd4jPointer reductionPointer,
