...
static std::string transformBenchmark() {
  std::string output;
  output += "transformBenchmark " +
            DataTypeUtils::asString(DataTypeUtils::fromT<T>());

  BenchmarkHelper helper(WARMUP, NUM_ITER);
  IntPowerParameters length("length", 2, 8, 20,
                            4);  // 2^8, 2^12, 2^16, 2^20 - 4MB
...
static std::string scalarBenchmark() {
  std::string output;
  output +=
      "scalarBenchmark " + DataTypeUtils::asString(DataTypeUtils::fromT<T>());
  ...
  output += helper.runOperationSuit(&sbAdd, generator, batch,
                                    "Scalar Addition - x.add(3.14159265359)");
  output += helper.runOperationSuit(&sbDiv, generator, batch,
                                    "Scalar Division - x.div(3.14159265359)");
  output += helper.runOperationSuit(&sbPow, generator, batch,
...
static std::string pairwiseBenchmark() {
  std::string output;
  output +=
      "pairwiseBenchmark " + DataTypeUtils::asString(DataTypeUtils::fromT<T>());

  BenchmarkHelper helper(WARMUP, NUM_ITER);
  IntPowerParameters length(
      "length", 2, 8, 20,
      4);  // 2^4 to 2^20 in steps of 4 - 2^4, 2^8, 2^16, 2^20
  BoolParameters inplace("inplace");
...
static std::string mismatchedOrderAssign() {
  ...
  output += helper.runOperationSuit(&tb, generator, batch,
                                    "C->F and F->C Assign F32");
  ...
  output += helper.runOperationSuit(&tb2, generator2, batch2,
                                    "nchw->nhwc and nhwc->nchw Assign FP32");
  return output;
}
...
static std::string gemmBenchmark() {
  ...
  for (int o = 0; o <= 1; o++) {
    ...
    IntPowerParameters sz("sz", 2, 4, 10,
                          2);  // 2^4=16, ..., 2^10=1024   ->  4 elements
...
static std::string reduceFullBenchmark() {
  ...
  output += helper.runOperationSuit(
      ...
      (const std::function<void(Parameters &, ResultSet &, ResultSet &,
                                ResultSet &)>)(generator),
      batch, "Product - Full Array Reduction");
  output += helper.runOperationSuit(
      &rbMax,
      (const std::function<void(Parameters &, ResultSet &, ResultSet &,
                                ResultSet &)>)(generator),
      batch, "Maximum - Full Array Reduction");
  ...
  auto generator3 = PARAMETRIC_D() {
    ...
    ctx->setInputArray(
        0, NDArrayFactory::create<T>('c', {p.getIntParam("length")}));
...
static std::string conv2d() {
  ...
  auto generator = PARAMETRIC_D() {
    ...
    auto w = NDArrayFactory::create<T>(
        'c', {khw, khw, 3, 3});  // [kH, kW, iC, oC] always
...
static std::string pool2d() {
  ...
  output +=
      helper.runOperationSuit(&benchmark1, generator, batch, "Average Pool 2d");

  sd::ops::maxpool2d maxpool2d;
  DeclarableBenchmark benchmark2(maxpool2d, "maxpool");
  output +=
      helper.runOperationSuit(&benchmark2, generator, batch, "Max Pool 2d");
  return output;
}
...
static std::string lstmBenchmark() {
  ...
  auto generator = PARAMETRIC_D() {
    ...
    if (f == 0) {
      // TNS format
      ctx->setInputArray(
          1, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // x
      ctx->setOutputArray(
          0, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // i
      ctx->setOutputArray(
          1, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // c
      ctx->setOutputArray(
          2, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // f
      ctx->setOutputArray(
          3, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // o
      ctx->setOutputArray(
          4, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // z
      ctx->setOutputArray(
          5, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // h
      ctx->setOutputArray(
          6, NDArrayFactory::create<T>('c', {seqLength, m, n}));  // y
    } else {
      // NST format
      ctx->setInputArray(
          1, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // x
      ctx->setOutputArray(
          0, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // i
      ctx->setOutputArray(
          1, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // c
      ctx->setOutputArray(
          2, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // f
      ctx->setOutputArray(
          3, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // o
      ctx->setOutputArray(
          4, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // z
      ctx->setOutputArray(
          5, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // h
      ctx->setOutputArray(
          6, NDArrayFactory::create<T>('f', {m, n, seqLength}));  // y
    }
...
static std::string broadcast2d() {
  ...
  IntPowerParameters cols(
      "cols", 2, 2, 12,
      4);  // 2^2 to 2^12 in steps of 2 - 2^1=2, ..., 2^10=1024
  ...
  auto generator = PARAMETRIC_D() {
    ...
    auto arr =
        NDArrayFactory::create<float>('c', {rows, p.getIntParam("cols")});
    ...
    if (p.getIntParam("inplace") == 1) {
      ...
      ctx->setOutputArray(
          0, NDArrayFactory::create<float>('c', {rows, p.getIntParam("cols")}));
    }
    return ctx;
  };
