...
namespace blas {
template <typename T>
static void *transpose(int orderSource, int orderTarget, int rows, int cols,
                       void *source);
...
class GEMM {
 protected:
 public:
  static void op(int Order, int TransA, int TransB, int M, int N, int K,
                 double alpha, void *A, int lda, void *B, int ldb, double beta,
                 void *C, int ldc);
};
...
class GEMV : public sd::blas::GEMM<X, Y, Z> {
 public:
  static void op(int TRANS, int M, int N, double alpha, void *vA, int lda,
                 void *vX, int incx, double beta, void *vY, int incy);
};
