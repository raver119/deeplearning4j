...
class Choice {
 public:
  ...
  __device__ static inline void specialOpCuda(
    ...
    // TODO: we probably might want to skip this sum, and state that
    // probabilities array should be real probabilities, i.e. should sum to 1.0
    ...
    if (zEWS >= 1 && xEWS >= 1 && yEWS >= 1 && xOrder == yOrder &&
        xOrder == zOrder) {
      for (Nd4jLong e = tid; e < zLength; e += blockDim.x * gridDim.x) {
        ...
        for (Nd4jLong f = 0; f < yLength; f++) {
          ...
          //                        __syncthreads();  // Eliminated due RTX20xx
          //                        specific
        }
        //                    __syncthreads();  // Eliminated due RTX20xx
        //                    specific
  ...
  static inline void specialOp(Nd4jPointer state, const T *x,
                               const Nd4jLong *xShapeBuffer, const T *y,
                               const Nd4jLong *yShapeBuffer, T *z,
                               const Nd4jLong *zShapeBuffer,
                               T *extraArguments) {
    /**
     * X holds data,
     * Y holds probabilities
    ...
    // reinterpret_cast<sd::random::RandomBuffer *> (state);
    sd::graph::RandomGenerator *rng =
        reinterpret_cast<sd::graph::RandomGenerator *>(state);
    // TODO: we probably might want to skip this sum, and state that
    // probabilities array should be real probabilities, i.e. should sum to 1.0
    // T probSum = extraArguments[0];
...
//////////////////////////////////////////////////////////////////////
/**
 * This Op produces random values within specified boundaries. Distribuion is
 * Gaussian
 */
template <typename T>
 ...
 public:
  ...
  __device__ static inline void specialOpCuda(
    ...
    for (int e = tid; e < middle; e += step) {
      ...
      z[e * zEWS] =
          (sd::math::nd4j_sqrt<T, T>(t * sd::math::nd4j_log<T, T>(r0)) *
           sd::math::nd4j_cos<T, T>(two_pi * r1)) *
              stddev +
          realMean0;
      ...
      if (epm < zLength) {
        ...
        z[epm * zEWS] =
            (sd::math::nd4j_sqrt<T, T>(t * sd::math::nd4j_log<T, T>(r0)) *
             sd::math::nd4j_sin<T, T>(two_pi * r1)) *
                stddev +
            realMean1;
      }
    }
  ...
  static inline void specialOp(Nd4jPointer state, const T *x,
                               ...
                               T *extraArguments) {
    const T two_pi =
        static_cast<T>(2.0f) * static_cast<T>(3.14159265358979323846);
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto e = start; e < stop; e++) {
        ...
        auto z0 = (sd::math::nd4j_sqrt<T, T>(static_cast<T>(-2.0f) *
                                             sd::math::nd4j_log<T, T>(r0)) *
                   sd::math::nd4j_cos<T, T>(two_pi * r1)) *
                      stddev +
                  realMean0;
        z[e * zEWS] = z0;
        ...
        if (epm < zLength) {
          T realMean1 = y == z ? mean : y[epm * yEWS];
          auto z1 = (sd::math::nd4j_sqrt<T, T>(static_cast<T>(-2.0f) *
                                               sd::math::nd4j_log<T, T>(r0)) *
                     sd::math::nd4j_sin<T, T>(two_pi * r1)) *
                        stddev +
                    realMean1;
...
class TruncatedNormalDistribution {
 private:
  static inline _CUDA_HD T step(sd::graph::RandomGenerator *rng, T mean,
                                T stddev, Nd4jLong e, Nd4jLong middle, T &z) {
    auto epm = e + middle;
    const T two_pi =
        static_cast<T>(2.0f) * static_cast<T>(3.14159265358979323846);
 ...
 public:
  ...
  __device__ static inline void specialOpCuda(
    ...
    GaussianDistribution<T>::specialOpCuda(state, x, xShapeBuffer, y,
                                           yShapeBuffer, z, zShapeBuffer,
                                           extraArguments);
    __syncthreads();
    ...
    for (Nd4jLong e = tid; e < zLength; e += step) {
      if (z[e] > mean + ds || z[e] < mean - ds) {
        z[e] = TruncatedNormalDistribution<T>::step(rng, mean, stddev, e,
                                                    middle, z[e]);
  ...
  static inline void specialOp(Nd4jPointer state, const T *x,
                               ...
                               const Nd4jLong *zShapeBuffer,
                               T *extraArguments) {
    GaussianDistribution<T>::specialOp(state, x, xShapeBuffer, y, yShapeBuffer,
                                       z, zShapeBuffer, extraArguments);
    Nd4jLong zLength = shape::length(zShapeBuffer);
    // auto yEWS = shape::elementWiseStride(yShapeBuffer);
    // auto zEWS = shape::elementWiseStride(zShapeBuffer);
    auto rng = reinterpret_cast<sd::graph::RandomGenerator *>(state);
 ...
 public:
  ...
  __device__ static inline void specialOpCuda(
    ...
    for (Nd4jLong e = tid; e < middle; e += step) {
      ...
      z[e * zEWS] = sd::math::nd4j_exp<T, T>(
          (sd::math::nd4j_sqrt<T, T>(static_cast<T>(-2.0f) *
                                     sd::math::nd4j_log<T, T>(r0)) *
           sd::math::nd4j_cos<T, T>(two_pi * r1)) *
              stddev +
          realMean);
      ...
      if (epm < zLength) {
        ...
        z[epm * zEWS] = sd::math::nd4j_exp<T, T>(
            (sd::math::nd4j_sqrt<T, T>(static_cast<T>(-2.0f) *
                                       sd::math::nd4j_log<T, T>(r0)) *
             sd::math::nd4j_sin<T, T>(two_pi * r1)) *
                stddev +
