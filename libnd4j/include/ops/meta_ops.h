...
namespace metaOps {
/**
 * InvertedMetaOp shares the same idea as MetaOp, but op being applied to op.Y
 * in pairwise/broadcast ops
 */
 ...
 *
 * Predicate can be either scalar or transform, to process data before actual op
 * call Postulate will be the scalar/transform, but will be applied to result of
 * broadcast/reduce/reduce3
 */
template <typename T, typename OpTypeA, typename OpTypeB>
class MetaOp {
 public:
  ...
  meta_def static T op(T d1, T *params) {
    /*
     * We assume, that params for MetaOp is a set of pointers to actual op A & B
     * extraArgs
     */
    Nd4jPointer *wrap = reinterpret_cast<Nd4jPointer *>(params);
    T *paramsA = reinterpret_cast<T *>(wrap[0]);
