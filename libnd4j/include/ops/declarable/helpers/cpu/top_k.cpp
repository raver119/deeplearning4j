...
template <typename T>
static int topKFunctor_(const NDArray* input, NDArray* values, NDArray* indices,
                        const uint k, bool needSort) {
  Nd4jLong width = input->sizeAt(-1);
  int lastDim = input->rankOf() - 1;
  // -----------------------------------------------------------------------------------------------
  // // this assumption is right:
  //        if (values->lengthOf() != k * lastDimList->size()) {
  //            nd4j_printf("top_k: something is wrong. %i expected, but %i
  //            given.\n",
  ...
  if (k == 1) {
    ...
    for (Nd4jLong e = 0; e < numOfSubArrs; ++e) {
      ...
      NDArray topValues =
          NDArrayFactory::create<T>('c', {k}, input->getContext());
      NDArray sortedVals =
          NDArrayFactory::create<T>('c', {k}, input->getContext());
      NDArray topIndices =
      ...
      // std::sort(sortedVals.begin(), sortedVals.end()); // sorted in ascending
      // order
      SpecialMethods<T>::sortGeneric(sortedVals.buffer(),
                                     sortedVals.shapeInfo(), false);
      for (Nd4jLong i = static_cast<Nd4jLong>(k); i < width; ++i) {
        ...
        if (minTopVal < val) {  // value should be inserted to top k
          ...
          if (!exists) {
            // exchangePos - a distance between begin and minimal existed to be
            // suppressed by val
      ...
      if (needSort) {
        SpecialMethods<T>::sortGeneric(topValues.buffer(),
                                       topValues.shapeInfo(), true);
        ...
        // std::sort(data.begin(), data.end(), [](std::pair<int, T> const& a,
        // std::pair<int, T> const& b) {
...
template <typename T>
static int inTopKFunctor_(sd::LaunchContext* context, const NDArray* input,
                          const NDArray* target, NDArray* result,
                          const uint k) {
  std::vector<Nd4jLong> shapeI(input->rankOf());
  for (int i = 0; i < input->rankOf() - 1; i++) shapeI[i] = input->sizeAt(i);
  shapeI[input->rankOf() - 1] = k;
  std::unique_ptr<NDArray> indices(
      NDArrayFactory::create_<Nd4jLong>(input->ordering(), shapeI, context));
...
int topKFunctor(sd::LaunchContext* context, const NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), return topKFunctor_,
                        (input, values, indices, k, needSort), NUMERIC_TYPES);
}

int inTopKFunctor(sd::LaunchContext* context, const NDArray* input,
                  const NDArray* target, NDArray* result, const uint k) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return inTopKFunctor_,
                        (context, input, target, result, k), NUMERIC_TYPES);
}

BUILD_SINGLE_TEMPLATE(template int topKFunctor_,
                      (const NDArray* input, NDArray* values, NDArray* indices,
                       const uint k, bool needSort),
                      NUMERIC_TYPES);
BUILD_SINGLE_TEMPLATE(template int inTopKFunctor_,
