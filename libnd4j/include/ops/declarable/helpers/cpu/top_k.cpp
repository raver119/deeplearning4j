        ...
        const Nd4jLong numOfSubArrs = ShapeUtils::getNumOfSubArrs(input->shapeInfo(), dimsToExclude);
            ...
            if (k == 1) {
                ...
                for (Nd4jLong e = 0; e < numOfSubArrs; ++e) {
                    ...
                    NDArray topValues = NDArrayFactory::vector<T>(k, T(0.f), input->getContext());
                    NDArray sortedVals = NDArrayFactory::vector<T>(k, T(0.f), input->getContext());
                    NDArray topIndices = NDArrayFactory::vector<Nd4jLong>(k, 0LL, input->getContext());
...
// ----------------------------------------------------------------------------------------------- //
    ...
    static int inTopKFunctor_(sd::LaunchContext* context, const NDArray* input, const NDArray* target, NDArray* result, const uint k) {
            ...
            auto indices = NDArrayFactory::create<Nd4jLong>(shapeI, std::vector<Nd4jLong>{}, (sd::Order)input->ordering(), context);
