  ...
  if (dimC == rank - 1 && 'c' == input.ordering() && 1 == input.ews() &&
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        z[i] = 0.2989f * x[xStep] + 0.5870f * x[xStep + 1] +
               0.1140f * x[xStep + 2];
      }
    };
...
void transformRgbGrs(sd::LaunchContext* context, const NDArray& input,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), rgbToGrs_, (input, output, dimC),
                        NUMERIC_TYPES);
}

template <typename T, typename Op>
FORCEINLINE static void rgbToFromYuv_(const NDArray& input, NDArray& output,
                                      const int dimC, Op op) {
  const T* x = input.bufferAsT<T>();
  T* z = output.bufferAsT<T>();
  const int rank = input.rankOf();
  bool bSimple =
      (dimC == rank - 1 && 'c' == input.ordering() && 1 == input.ews() &&
       'c' == output.ordering() && 1 == output.ews());
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto i = start; i < stop; i++) {
      ...
      op(xTad[0], xTad[xDimCstride], xTad[2 * xDimCstride], zTad[0],
...
template <typename T>
FORCEINLINE static void rgbYuv_(const NDArray& input, NDArray& output,
                                const int dimC) {
...
void transformRgbYuv(sd::LaunchContext* context, const NDArray& input,
                     NDArray& output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input.dataType(), rgbYuv_, (input, output, dimC),
                        FLOAT_TYPES);
}

template <typename T>
FORCEINLINE static void yuvRgb_(const NDArray& input, NDArray& output,
                                const int dimC) {
...
void transformYuvRgb(sd::LaunchContext* context, const NDArray& input,
                     NDArray& output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input.dataType(), yuvRgb_, (input, output, dimC),
                        FLOAT_TYPES);
}

template <typename T, typename Op>
FORCEINLINE static void tripleTransformer(const NDArray* input, NDArray* output,
                                          const int dimC, Op op) {
  const int rank = input->rankOf();
  ...
  if (dimC == rank - 1 && input->ews() == 1 && output->ews() == 1 &&
      input->ordering() == 'c' && output->ordering() == 'c') {
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i += increment) {
        op(x[i], x[i + 1], x[i + 2], z[i], z[i + 1], z[i + 2]);
      }
...
template <typename T>
FORCEINLINE static void tripleTransformer(const NDArray* input, NDArray* output,
                                          const int dimC, T (&tr)[3][3]) {
  const int rank = input->rankOf();

  const T* x = input->bufferAsT<T>();
  T* z = output->bufferAsT<T>();
  // TODO: Use tensordot or other optimizied helpers to see if we can get better
  // performance.
...
template <typename T>
FORCEINLINE static void hsvRgb(const NDArray* input, NDArray* output,
                               const int dimC) {
...
template <typename T>
FORCEINLINE static void rgbHsv(const NDArray* input, NDArray* output,
                               const int dimC) {
...
template <typename T>
FORCEINLINE static void rgbYiq(const NDArray* input, NDArray* output,
                               const int dimC) {
...
template <typename T>
FORCEINLINE static void yiqRgb(const NDArray* input, NDArray* output,
                               const int dimC) {
  // TODO: this operation does not use the clamp operation, so there is a
  // possibility being out of range. Justify that it  will not be out of range
  // for images data
...
void transformHsvRgb(sd::LaunchContext* context, const NDArray* input,
                     NDArray* output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input->dataType(), hsvRgb, (input, output, dimC),
                        FLOAT_TYPES);
}

void transformRgbHsv(sd::LaunchContext* context, const NDArray* input,
                     NDArray* output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input->dataType(), rgbHsv, (input, output, dimC),
                        FLOAT_TYPES);
}

void transformYiqRgb(sd::LaunchContext* context, const NDArray* input,
                     NDArray* output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input->dataType(), yiqRgb, (input, output, dimC),
                        FLOAT_TYPES);
}

void transformRgbYiq(sd::LaunchContext* context, const NDArray* input,
                     NDArray* output, const int dimC) {
  BUILD_SINGLE_SELECTOR(input->dataType(), rgbYiq, (input, output, dimC),
                        FLOAT_TYPES);
}
