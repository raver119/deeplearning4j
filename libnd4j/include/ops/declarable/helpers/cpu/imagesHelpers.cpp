    ...
    if (dimC == rank - 1 && input->ews() == 1 && output->ews() == 1 && input->ordering() == 'c' && output->ordering() == 'c') {
        ...
        auto func = PRAGMA_THREADS_FOR{
            for (auto i = start; i < stop; i += increment) { 
                ...
                z[i]   = x[i] * tr[0][0] + x[i + 1] * tr[1][0] + x[i + 2] * tr[2][0];
                z[i+1] = x[i] * tr[0][1] + x[i + 1] * tr[1][1] + x[i + 2] * tr[2][1];
                z[i+2] = x[i] * tr[0][2] + x[i + 1] * tr[1][2] + x[i + 2] * tr[2][2];
        ...
        auto func = PRAGMA_THREADS_FOR{
            for (auto i = start; i < stop; i += increment) {
                ...
                T* zTad = z + packZ.platformOffsets()[i];
                //simple M*v //tr.T*v
                zTad[0]               = xTad[0] * tr[0][0] + xTad[xDimCstride] * tr[1][0] + xTad[2 * xDimCstride] * tr[2][0];
                zTad[zDimCstride]     = xTad[0] * tr[0][1] + xTad[xDimCstride] * tr[1][1] + xTad[2 * xDimCstride] * tr[2][1];
                zTad[2 * zDimCstride] = xTad[0] * tr[0][2] + xTad[xDimCstride] * tr[1][2] + xTad[2 * xDimCstride] * tr[2][2];
