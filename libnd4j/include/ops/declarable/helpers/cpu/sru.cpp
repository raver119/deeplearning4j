...
static FORCEINLINE NDArray activation(const NDArray& arr) {
  // return (const_cast<NDArray<T>&>(arr)).template
  // transform<simdOps::Tanh<T>>();
...
//////////////////////////////////////////////////////////////////////////
void sruCell(sd::LaunchContext* context, const NDArray* x, const NDArray* c0,
             const NDArray* w, const NDArray* b, NDArray* h, NDArray* c) {
  ...
  auto r =
      sigmoid(z({0, 0, 2 * inSize, 3 * inSize}) + (*b)({inSize, 2 * inSize}));
...
//////////////////////////////////////////////////////////////////////////
void sruTimeLoop(sd::LaunchContext* context, const NDArray* x,
                 const NDArray* c0, const NDArray* w, const NDArray* b,
                 NDArray* h, NDArray* c) {
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void sruBI_(NDArray* x, const NDArray* w, const NDArray* b,
                   const NDArray* c0, const NDArray* mask, NDArray* ht,
                   NDArray* ct) {
  // x     input 3d tensor [time x bS x 2*K], time - number of time steps, bS -
  // batch size, K - number of features w     2d tensor of weights [2*K x 6*K]
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void sruBIBP_(NDArray* x, const NDArray* w, const NDArray* b,
                     const NDArray* c0, const NDArray* ct,
                     const NDArray* inGradC0, const NDArray* inGradHt,
                     const NDArray* mask, NDArray* gradI, NDArray* gradW,
                     NDArray* gradB, NDArray* gradC0) {
  // x  input 3d tensor [time x bS x 2*K], time - number of time steps, bS -
  // batch size, K - number of features w  2d tensor of weights [2*K x 6*K] b
  // row of biases with twice length 4*K] c0 2d tensor of initial state [bS x
  // 2*K] at time t=0 ct [time x bS x 2*K] inGradC0 [bS x 2*K] inGradHt  [time x
  // bS x 2*K] mask optional,  2d tensor of dropout mask [bS x 2*K]
  ...
  // U = x * w
  NDArray wi =
      mmul(*x, *w);  //  [time x bS x 2*K] * [2*K x 6*K] = [time x bS x 6*K]
  NDArray gradBias(x->ordering(), {bS, 4 * K}, x->dataType(), x->getContext());
  NDArray gradWi(x->ordering(), {time, bS, 6 * K}, x->dataType(),
                 x->getContext());
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto col = start; col < stop; col++) {
      ...
      for (Nd4jLong t = 0; t < time; ++t) {
        // evaluate sigmoids
        T ft =
            ((T)1.) / ((T)1. + sd::math::nd4j_exp<T, T>(-(*(pWiVal + 1) + bF)));
        T rt =
            ((T)1.) / ((T)1. + sd::math::nd4j_exp<T, T>(-(*(pWiVal + 2) + bR)));

        T val = sd::math::nd4j_tanh<T, T>(*pStateVal);
        T prevVal =
            (t < time - 1) ? (*(pStateVal - ncolsRev)) : (*(pInit + col));
        // grad wrt input
  ...
  MmulHelper::mmul(
      x, &gradWi, gradW, 1.,
      0.);  // [time x 2*K x bS ] * [time x bS x 6*K] = [time x 2*K x 6*K]
}

void sruBI(sd::LaunchContext* context, NDArray* x, const NDArray* w,
           const NDArray* b, const NDArray* c0, const NDArray* mask,
           NDArray* ht, NDArray* ct) {
  BUILD_SINGLE_SELECTOR(x->dataType(), sruBI_, (x, w, b, c0, mask, ht, ct),
                        FLOAT_TYPES);
}
void sruBIBP(sd::LaunchContext* context, NDArray* x, const NDArray* w,
             const NDArray* b, const NDArray* c0, const NDArray* ct,
             const NDArray* inGradC0, const NDArray* inGradH,
             const NDArray* mask, NDArray* gradI, NDArray* gradW,
             NDArray* gradB, NDArray* gradC0) {
  BUILD_SINGLE_SELECTOR(
      x->dataType(), sruBIBP_,
      (x, w, b, c0, ct, inGradC0, inGradH, mask, gradI, gradW, gradB, gradC0),
      FLOAT_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void sruBI_,
                      (NDArray * x, const NDArray* w, const NDArray* b,
                       const NDArray* c0, const NDArray* mask, NDArray* ht,
                       NDArray* ct),
                      FLOAT_TYPES);
...
//////////////////////////////////////////////////////////////////////////
// template <typename T>
// void sruCellBP(const std::vector<NDArray<T>*>& inArrs, const
// std::vector<NDArray<T>*>& outArrs) {

//     NDArray<T>* x    = inArrs[0];               // input [bS x inSize], bS -
//     batch size, inSize - number of features NDArray<T>* c0   = inArrs[1]; //
//     previous cell state c  [bS x inSize], that is at previous time step t-1
//     NDArray<T>* w    = inArrs[2];               // weights [inSize x
//     3*inSize] NDArray<T>* b    = inArrs[3];               // biases
//     [2*inSize] NDArray<T>* dLdC = inArrs[4];               // gradient of the
//     loss func with respect to cell output [bS x inSize] NDArray<T>* dLdH =
//     inArrs[5];               // gradient of the loss func with respect to
//     cell state  [bS x inSize]

//     NDArray<T>* dLdX  = outArrs[0];             // gradient of the loss func
//     with respect to input [bS x inSize], so called epsilon NDArray<T>* dLdW
//     = outArrs[1];             // gradient of the loss func with respect to
//     weights [inSize x 3*inSize] NDArray<T>* dLdB  = outArrs[2]; // gradient
//     of the loss func with respect to biases [2*inSize] NDArray<T>* dLdC0 =
//     outArrs[3];             // gradient of the loss func with respect to
...
//     // forget gate = sigmoid(x*Wf + bf)
//     NDArray<T> f = sigmoid<T>(z({{},{inSize,   2*inSize}}) + (*b)({{0,
//     inSize}}));             // [bS, inSize] NDArray<T> oneMinusF = 1. - f;

//     // reset gate = sigmoid(x*Wr + br)
//     NDArray<T> r = sigmoid<T>(z({{},{2*inSize, 3*inSize}}) + (*b)({{inSize,
//     2*inSize}}));      // [bS, inSize] NDArray<T> oneMinusR = 1. - r;

//     // current sell state = f◦c0 + (1 - f)◦(x*Wc)             ---> c->assign(
//     f*(*c0) + ((T)1. - f) * z({{},{0, inSize}}) );
//     // current cell output = r◦activation(c) + (1 - r)◦x      ---> h->assign(
//     r*activation<T>(*c) + ((T)1. - r) * (*x) );
...
//     NDArray<T> dCdX = oneMinusF * (*w)({{},{0, inSize}}) + dCdF *
//     (*w)({{},{inSize, 2*inSize}});

//     // dLdC0 =  dLdC * dCdC0 = dLdC * f
//     dLdC0->assign((*dLdC) * f);

//     // dLdBf = dLdH*dHdBf + dLdC*dCdBf = dLdH*dHdC*dCdBf + dLdC*dCdF*dFdBf =
//     dLdH*dHdC*dCdF*dFdBf + dLdC*dCdF*dFdBf = (dLdH*dHdC + dLdC)*dCdF*dFdBf
//     (*dLdB)({{0, inSize}}).assign(((*dLdH) * dHdC + *dLdC) * dCdF * dFdBf);
//     // dLdBr = dLdH * dHdR * dRdBr
//     (*dLdB)({{inSize, 2*inSize}}).assign((*dLdH) * dHdR * dRdBr)

//     // dLdWc = dLdH*dHdWc + dLdC*dCdWc = dLdH*dHdC*dCdWc + dLdC*dCdWc =
//     (dLdH*dHdC + dLdC) * dCdWc = (dLdH*dHdC + dLdC) * (1-f)*x
//     (*dLdW)({{}, {0, inSize}}).assign(((*dLdH) * dHdC + *dLdC) * oneMinusF *
//     (*x));
//     // dLdWf = dLdBf * x
//     (*dLdW)({{}, {inSize, 2*inSize}}).assign((*dLdB)({{0, inSize}}) * (*x));
//     // dLdWr = dLdBr * x
//     (*dLdW)({{}, {2*inSize, 3*inSize}}).assign((*dLdB)({{inSize, 2*inSize}})
//     * (*x));

//     // dLdX = dLdH*dHdX + dLdC*dCdX = dLdH*(dHdX + dHdR*dRdX + dHdC*dCdX) +
//     dLdC*dCdF*dFdX = dLdH*(1 - r + dHdR*dRdX + dHdC*dCdX) + dLdC*dCdX
//     dLdX->assign((*dLdH) * (oneMinusR + dHdR * (*w)({{},{2*inSize,
//     3*inSize}}) + dHdC * dCdX) + (*dLdC) * dCdX);
// }
