...
static void onehot_(void* voutput, Nd4jLong const* zShapeInfo,
  ...
  auto tLen =
      static_cast<unsigned int>(shape::length(tadPack.primaryShapeInfo()));
  ...
  if (iLen != numTads)
    throw std::runtime_error(
        "OneHot: number of TADs should be equal to number of indices");

  if (shape::elementWiseStride(zShapeInfo) != 1 ||
      shape::elementWiseStride(iShapeInfo) != 1)
    throw std::runtime_error(
        "OneHot: op expects output and indices to have elementWiseStride to be "
        "equal to 1");
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto e = start; e < stop; e++) {
        ...
        if (idx < 0 || idx >= tLen) {
          ...
          PRAGMA_OMP_SIMD
          for (unsigned int t = 0; t < tLen; t++) {
            cO[shape::getIndexOffset(t, tadPack.primaryShapeInfo())] =
                idx == t ? one : zero;
          }
        }
      }
...
void onehot(const sd::LaunchContext* context, const NDArray* indices,
  ...
  BUILD_DOUBLE_SELECTOR(
      zType, iType, onehot_,
      (output->buffer(), output->shapeInfo(), indices->buffer(),
       indices->shapeInfo(), axis, on, off),
