...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void im2col_(sd::LaunchContext& context, const NDArray& input,
  ...
  if (shape::order(imShapeBuffer) == 'c' &&
      shape::order(colShapeBuffer) == 'c' &&
      shape::strideDescendingCAscendingF(imShapeBuffer) &&
      shape::strideDescendingCAscendingF(colShapeBuffer)) {
            ...
            for (int kCol = 0; kCol < kW; ++kCol) {
              for (int colH = 0; colH < oH; ++colH) {
                for (int colW = 0; colW < oW; ++colW) {
                  ...
                  auto col = colBuff + b * colStride0 + c * colStride1 +
                  ...
                  if (static_cast<unsigned>(imRow) >=
                    ...
                    *col = zeroPadVal;
                    ...
                    auto im = imBuff + b * imStride0 + c * imStride1 +
...
void im2col(sd::LaunchContext& context, const NDArray& im, NDArray& col,
            const int kH, const int kW, const int sH, const int sW,
            const int pH, const int pW, const int dH, const int dW,
            const NDArray& arrZeroPadVal) {
  BUILD_SINGLE_SELECTOR(
      im.dataType(), im2col_,
      (context, im, col, kH, kW, sH, sW, pH, pW, dH, dW, arrZeroPadVal),
      FLOAT_TYPES);
}
