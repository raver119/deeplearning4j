...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void pooling2dBP_(sd::graph::Context& block, const NDArray& input,
          ...
          for (int oh = 0; oh < oH; ++oh) {
            for (int ow = 0; ow < oW; ++ow) {
              ...
              if (sameStrides) {
                ...
                gI[b * gIStride0 + c * gIStride1 + maxKH * gIStride2 +
                   maxKW * gIStride3] += valO;
              }
            }
          }
        }
  ...
  /*************************************************************************/
            ...
            for (int ow = 0; ow < oW; ++ow) {
              ...
              if (sameStrides) {
                ...
                valO *= sd::math::nd4j_pow<T, T, T>(
                    sum, ((T)1. - extraParam0) / extraParam0);

                for (Nd4jLong kh = hstart; kh < hend; kh += iStep2)
                  for (Nd4jLong kw = wstart; kw < wend; kw += iStep3)
                    pgI[kh + kw] += valO *
                                    sd::math::nd4j_pow<T, T, T>(
                                        sd::math::nd4j_abs<T>(pIn[kh + kw]),
                                        extraParam0 - 1.f) *
                ...
                for (Nd4jLong kh = hstart; kh < hend; kh += dH) {
                  for (Nd4jLong kw = wstart; kw < wend; kw += dW) {
                    ...
                    pgI[kh * gIStride2 + kw * gIStride3] +=
                        valO *
                        sd::math::nd4j_pow<T, T, T>(
                            sd::math::nd4j_abs<T>(inVal), extraParam0 - 1.f) *
    ...
    nd4j_printf(
        "ConvolutionUtils::pooling2dBP: pooling mode argument can take three "
...
void ConvolutionUtils::pooling2dBP(sd::graph::Context& block,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), pooling2dBP_,
                        (block, input, gradO, gradI, kH, kW, sH, sW, pH, pW, dH,
