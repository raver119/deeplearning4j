...
//////////////////////////////////////////////////////////////////////////
template <typename T>
SVD<T>::SVD(const NDArray& matrix, const int switchSize, const bool calcU,
            const bool calcV, const bool fullUV) {
  if (matrix.rankOf() != 2 || matrix.isScalar())
    throw std::runtime_error(
        "ops::helpers::SVD constructor: input array must be 2D matrix !");
  ...
  if (_transp) math::nd4j_swap<bool>(_calcU, _calcV);

  _s = NDArrayFactory::create<T>(matrix.ordering(), {_diagSize, 1},
                                 matrix.getContext());
  _m = NDArrayFactory::create<T>(matrix.ordering(), {_diagSize + 1, _diagSize},
                                 matrix.getContext());
  _m.assign(0.);

  if (_calcU)
    _u = NDArrayFactory::create<T>(
        matrix.ordering(), {_diagSize + 1, _diagSize + 1}, matrix.getContext());
  else
    _u = NDArrayFactory::create<T>(matrix.ordering(), {2, _diagSize + 1},
                                   matrix.getContext());
  _u.assign(0.);

  if (_calcV) {
    _v = NDArrayFactory::create<T>(matrix.ordering(), {_diagSize, _diagSize},
                                   matrix.getContext());
    _v.assign(0.);
  }
...
template <typename T>
SVD<T>::SVD(const NDArray& matrix, const int switchSize, const bool calcU,
            const bool calcV, const bool fullUV, const char t) {
  if (matrix.rankOf() != 2 || matrix.isScalar())
    throw std::runtime_error(
        "ops::helpers::SVD constructor: input array must be 2D matrix !");
...
void SVD<T>::deflation1(int col1, int shift, int ind, int size) {
  if (ind <= 0)
    throw std::runtime_error(
        "ops::helpers::SVD::deflation1 method: input int must satisfy "
        "condition ind > 0 !");
  ...
  auto rotation =
      NDArrayFactory::create<T>(_m.ordering(), {2, 2}, _m.getContext());
  rotation.p(0, 0, cos);
  rotation.p(0, 1, -sin);
  rotation.p(1, 0, sin);
  rotation.p(1, 1, cos);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::deflation2(int col1U, int col1M, int row1W, int col1W, int ind1,
                        int ind2, int size) {
  if (ind1 >= ind2)
    throw std::runtime_error(
        "ops::helpers::SVD::deflation2 method: input intes must satisfy "
        "condition ind1 < ind2 !");

  if (size <= 0)
    throw std::runtime_error(
        "ops::helpers::SVD::deflation2 method: input size must satisfy "
        "condition size > 0 !");
  ...
  if (_calcU) {
    ...
    JacobiSVD<T>::mulRotationOnRight(col1U + ind1, col1U + ind2, temp,
                                     rotation);
  } else
  ...
  if (_calcV) {
    ...
    JacobiSVD<T>::mulRotationOnRight(col1W + ind1, col1W + ind2, temp,
                                     rotation);
  }
}

//////////////////////////////////////////////////////////////////////////
// has effect on block from (col1+shift, col1+shift) to (col2+shift, col2+shift)
// inclusively
template <typename T>
void SVD<T>::deflation(int col1, int col2, int ind, int row1W, int col1W,
                       int shift) {
  const int len = col2 + 1 - col1;

  auto colVec0 = new NDArray(
      _m({col1 + shift, col1 + shift + len, col1 + shift, col1 + shift + 1},
         true));

  auto diagInterval =
      _m({col1 + shift, col1 + shift + len, col1 + shift, col1 + shift + len},
         true)
          .diagonal('c');
...

  {
    ...
    if (totDefl) {
      for (int i = 1; i < len; ++i) {
        int ki = permut[i];
        if (math::nd4j_abs<T>(diagInterval.template e<T>(ki)) < almostZero ||
            diagInterval.template e<T>(0) < diagInterval.template e<T>(ki))
          permut[i - 1] = permut[i];
        else {
          permut[i - 1] = 0;
          break;
    ...
    for (int i = totDefl ? 0 : 1; i < len; i++) {
      ...
      if (_calcU) {
        ...
        auto temp2 =
            _u({col1, col1 + len + 1, col1 + jac, col1 + jac + 1}, true);
      ...
      if (_calcV) {
        ...
        auto temp2 =
            _v({row1W, row1W + len, col1W + jac, col1W + jac + 1}, true);
        auto temp3 = temp1.dup();
        temp1.assign(temp2);
        temp2.assign(temp3);
      }
...

  {
    ...
    while (i > 0 &&
           (math::nd4j_abs<T>(diagInterval.template e<T>(i)) < almostZero ||
            math::nd4j_abs<T>(colVec0->template e<T>(i)) < almostZero))
      --i;

    for (; i > 1; --i) {
      if ((diagInterval.template e<T>(i) - diagInterval.template e<T>(i - 1)) <
          DataTypeUtils::eps<T>() * maxElem) {
        if (math::nd4j_abs<T>(diagInterval.template e<T>(i) -
                              diagInterval.template e<T>(i - 1)) >= epsBig)
          throw std::runtime_error(
              "ops::helpers::SVD::deflation: diagonal elements are not "
              "properly sorted !");
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
T SVD<T>::secularEq(const T diff, const NDArray& col0, const NDArray& diag,
                    const NDArray& permut, const NDArray& diagShifted,
                    const T shift) {
  ...
  for (int i = 0; i < len; ++i) {
    ...
    item = col0.e<T>(j) /
           ((diagShifted.e<T>(j) - diff) * (diag.e<T>(j) + shift + diff));
    res += item * col0.e<T>(j);
  }
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::calcSingVals(const NDArray& col0, const NDArray& diag,
                          const NDArray& permut, NDArray& singVals,
                          NDArray& shifts, NDArray& mus) {
  auto len = col0.lengthOf();
  auto curLen = len;

  while (curLen > 1 && col0.e<T>(curLen - 1) == (T)0.f) --curLen;
  ...
  for (int k = 0; k < len; ++k) {
    ...
    if (k == curLen - 1)
      ...
      while (col0.e<T>(l) == (T)0.f) {
        ...
        if (l >= curLen)
          throw std::runtime_error(
              "ops::helpers::SVD::calcSingVals method: l >= curLen !");
      }
    ...
    while (fCur != (T).0 &&
           math::nd4j_abs<T>(muCur - muPrev) >
               (T)8. * DataTypeUtils::eps<T>() *
                   math::nd4j_max<T>(math::nd4j_abs<T>(muCur),
                                     math::nd4j_abs<T>(muPrev)) &&
           math::nd4j_abs<T>(fCur - fPrev) > DataTypeUtils::eps<T>() &&
           !useBisection) {
    ...
    if (useBisection) {
      ...
      T fRight =
          secularEq(rightShifted, col0, diag, permut, diagShifted, shift);
      // if(fLeft * fRight >= (T)0.)
      ...
      while (rightShifted - leftShifted >
             (T)2.f * DataTypeUtils::eps<T>() *
                 math::nd4j_max<T>(math::nd4j_abs<T>(leftShifted),
                                   math::nd4j_abs<T>(rightShifted))) {
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::perturb(const NDArray& col0, const NDArray& diag,
                     const NDArray& permut, const NDArray& singVals,
  ...
  for (int k = 0; k < n; ++k) {
    if (col0.e<T>(k) == (T)0.f)
      ...
      T dk = diag.e<T>(k);
      T prod = (singVals.e<T>(last) + dk) *
               (mus.e<T>(last) + (shifts.e<T>(last) - dk));
      ...
      for (int l = 0; l < m; ++l) {
        ...
        if (i != k) {
          ...
          prod *=
              ((singVals.e<T>(j) + dk) / ((diag.e<T>(i) + dk))) *
              ((mus.e<T>(j) + (shifts.e<T>(j) - dk)) / ((diag.e<T>(i) - dk)));
        }
      }
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::calcSingVecs(const NDArray& zhat, const NDArray& diag,
                          const NDArray& perm, const NDArray& singVals,
                          const NDArray& shifts, const NDArray& mus, NDArray& U,
                          NDArray& V) {
  int n = zhat.lengthOf();
  int m = perm.lengthOf();
  ...
  for (int k = 0; k < n; ++k) {
    ...
    *colU = 0.;
    ...
    if (zhat.e<T>(k) == (T)0.f) {
      ...
      if (_calcV) colV->p(k, 1.f);
    } else {
      for (int l = 0; l < m; ++l) {
        int i = perm.e<int>(l);
        U.p(i, k,
            zhat.e<T>(i) / (((diag.e<T>(i) - shifts.e<T>(k)) - mus.e<T>(k))) /
                ((diag.e<T>(i) + singVals.e<T>(k))));
      }
      U.p(n, k, 0.f);
      *colU /= colU->reduceNumber(reduce::Norm2);
      ...
      if (_calcV) {
        for (int l = 1; l < m; ++l) {
          int i = perm.e<T>(l);
          V.p(i, k,
              diag.e<T>(i) * zhat.e<T>(i) /
                  (((diag.e<T>(i) - shifts.e<T>(k)) - mus.e<T>(k))) /
                  ((diag.e<T>(i) + singVals.e<T>(k))));
        }
        V.p(0, k, -1.f);
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::calcBlockSVD(int col1, int size, NDArray& U, NDArray& singVals,
                          NDArray& V) {
  const T almostZero = DataTypeUtils::min<T>();
  auto col0 = _m({col1, col1 + size, col1, col1 + 1}, true);
  auto diag = static_cast<const NDArray&>(
      _m({col1, col1 + size, col1, col1 + size}, true).diagonal('c'));

  diag.p(Nd4jLong(0), T(0));
  singVals =
      NDArrayFactory::create<T>(_m.ordering(), {size, 1}, _m.getContext());
  U = NDArrayFactory::create<T>(_u.ordering(), {size + 1, size + 1},
                                _u.getContext());
  if (_calcV)
    V = NDArrayFactory::create<T>(_v.ordering(), {size, size}, _v.getContext());

  int curSize = size;
  while (curSize > 1 && diag.template e<T>(curSize - 1) == (T)0.f) --curSize;
  ...
  auto permut = NDArrayFactory::create<T>(
      _m.ordering(), {1, (int)indices.size()}, indices, _m.getContext());
  auto shifts =
      NDArrayFactory::create<T>(_m.ordering(), {size, 1}, _m.getContext());
  auto mus =
      NDArrayFactory::create<T>(_m.ordering(), {size, 1}, _m.getContext());
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::DivideAndConquer(int col1, int col2, int row1W, int col1W,
                              int shift) {
  ...
  auto f =
      NDArrayFactory::create<T>(_u.ordering(), {1, n - k - 1}, _u.getContext());

  if (n < _switchSize) {
    JacobiSVD<T> jac(_m({col1, col1 + n + 1, col1, col1 + n}, true), _calcU,
                     _calcV, _fullUV);
    ...
    auto temp =
        _m({col1 + shift, col1 + shift + n + 1, col1 + shift, col1 + shift + n},
           true);
    temp.assign(0.);
    auto diag = _m.diagonal('c');
    diag({col1 + shift, col1 + shift + n, 0, 0}, true)
        .assign(jac._s({0, n, 0, 0}, true));
  ...
  r0 = math::nd4j_sqrt<T, T>((math::nd4j_abs<T>(alphaK * lambda) *
                              math::nd4j_abs<T>(alphaK * lambda)) +
                             math::nd4j_abs<T>(betaK * phi) *
                                 math::nd4j_abs<T>(betaK * phi));
  ...
  if (_calcV) _v.p(row1W + k, col1W, 1.f);
  ...
  if (_calcU) {
    ...
    _u({col1 + k + 1, col1 + n + 1, col1, col1 + 1}, true)
        .assign(static_cast<const NDArray&>(_u(
                    {col1 + k + 1, col1 + n + 1, col2 + 1, col2 + 2}, true)) *
                s0);
    _u({col1 + k + 1, col1 + n + 1, col2 + 1, col2 + 2}, true) *= c0;
  } else {
    T q1 = _u.e<T>(0, col1 + k);

    for (int i = col1 + k - 1; i >= col1; --i) _u.p(0, i + 1, _u.e<T>(0, i));
  ...
  auto temp1 = _m(
      {col1 + shift + 1, col1 + shift + k + 1, col1 + shift, col1 + shift + 1},
      true);
  temp1.assign(l * alphaK);
  auto temp2 = _m(
      {col1 + shift + k + 1, col1 + shift + n, col1 + shift, col1 + shift + 1},
      true);
  ...
  auto blockM = _m(
      {col1 + shift, col1 + shift + n, col1 + shift, col1 + shift + n}, true);
  blockM = 0.f;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void SVD<T>::exchangeUV(const HHsequence& hhU, const HHsequence& hhV,
                        const NDArray& U, const NDArray& V) {
  if (_calcU) {
    int colsU = _fullUV ? hhU.rows() : _diagSize;
    auto temp1 = NDArrayFactory::create<T>(_u.ordering(), {hhU.rows(), colsU},
                                           _u.getContext());
    temp1.setIdentity();
    _u = temp1;
  ...
  if (_calcV) {
    ...
    auto temp1 = NDArrayFactory::create<T>(_v.ordering(), {hhV.rows(), colsV},
                                           _v.getContext());
    temp1.setIdentity();
    _v = temp1;
...
//////////////////////////////////////////////////////////////////////////
// svd operation, this function is not method of SVD class, it is standalone
// function
template <typename T>
static void svd_(const NDArray* x, const std::vector<NDArray*>& outArrs,
  ...
  for (int i = 0; i < listX.size(); ++i) {
    // NDArray<T> matrix(x->ordering(), {listX.at(i)->sizeAt(0),
    // listX.at(i)->sizeAt(1)}, block.getContext()); matrix.assign(listX.at(i));
...
void svd(sd::LaunchContext* context, const NDArray* x,
  ...
  BUILD_SINGLE_SELECTOR(x->dataType(), svd_,
                        (x, outArrs, fullUV, calcUV, switchNum), FLOAT_TYPES);
}
