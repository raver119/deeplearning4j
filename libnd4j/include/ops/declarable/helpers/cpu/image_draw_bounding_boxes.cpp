...
void drawBoundingBoxesFunctor(sd::LaunchContext* context, NDArray* images,
  ...
  // auto imageList = images->allTensorsAlongDimension({1, 2, 3}); // split
  // images by batch
  //        auto boxList = boxes->allTensorsAlongDimension({1, 2}); // split
  //        boxes by batch
  // auto colorSet = colors->allTensorsAlongDimension({0});
  output->assign(
      images);  // fill up all output with input images, then fill up boxes
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto batch = start; batch < stop; ++batch) {  // loop by batch
      ...
      for (auto boxIndex = 0; boxIndex < numBoxes; ++boxIndex) {
        ...
        auto rowStart =
            Nd4jLong((height - 1) * boxes->t<float>(batch, boxIndex, 0));
        auto rowStartBound = sd::math::nd4j_max(Nd4jLong(0), rowStart);
        auto rowEnd =
            Nd4jLong((height - 1) * boxes->t<float>(batch, boxIndex, 2));
        auto rowEndBound = sd::math::nd4j_min(Nd4jLong(height - 1), rowEnd);
        auto colStart =
            Nd4jLong((width - 1) * boxes->t<float>(batch, boxIndex, 1));
        auto colStartBound = sd::math::nd4j_max(Nd4jLong(0), colStart);
        auto colEnd =
            Nd4jLong((width - 1) * boxes->t<float>(batch, boxIndex, 3));
        auto colEndBound = sd::math::nd4j_min(Nd4jLong(width - 1), colEnd);

        if (rowStart > rowEnd || colStart > colEnd) {
          nd4j_debug(
              "helpers::drawBoundingBoxesFunctor: Bounding box (%lld, %lld, "
              "%lld, %lld) is inverted "
