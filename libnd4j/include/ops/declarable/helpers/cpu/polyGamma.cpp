...
template <typename T>
static FORCEINLINE T polyGammaScalar(sd::LaunchContext* context, const int n,
                                     const T x) {
  // if (n < 0)
...
template <typename T>
static void polyGamma_(sd::LaunchContext* context, const NDArray& n,
                       const NDArray& x, NDArray& output) {
  auto func = PRAGMA_THREADS_FOR {
    for (auto i = start; i < stop; i++) {
      const T order = n.e<T>(i);
      if (order !=
          static_cast<int>(order))  // if order has fractional part then do not
                                    // perform calculations and return NAN
        output.p(i, std::numeric_limits<T>::quiet_NaN());
      else if (order ==
               0)  // polygamma function of zero order is digamma function
...
void polyGamma(sd::LaunchContext* context, const NDArray& n, const NDArray& x,
               NDArray& output) {
  BUILD_SINGLE_SELECTOR(x.dataType(), polyGamma_, (context, n, x, output),
                        FLOAT_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void polyGamma_,
                      (sd::LaunchContext * context, const NDArray& n,
                       const NDArray& x, NDArray& output),
                      FLOAT_TYPES);
