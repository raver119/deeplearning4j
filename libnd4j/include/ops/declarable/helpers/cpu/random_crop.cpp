...
static int _randomCropFunctor(graph::Context& context, NDArray* input,
  ...
  // execTransform<randomOps::UniformDistribution<T>>(rng, output->buffer(),
  // output->shapeInfo(), std::vector<T>({T(0.), shape->e(last)}).data());
  // NativeOpExecutioner::execRandom(random::UniformDistribution, rng,
  // output->buffer(), output->shapeInfo(), std::vector<T>({T(0.),
  // shape->e<T>(last)}).data());
  ...
  // functions::random::RandomFunction<T>::template
  // execTransform<randomOps::UniformDistribution<T>>(rng, output->buffer(),
  // output->shapeInfo(), std::vector<T>({T(0.),
  // shape->getScalar(last)}).data());
