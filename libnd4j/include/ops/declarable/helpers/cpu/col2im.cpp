...
template <typename T>
void col2im_(sd::LaunchContext& context, const NDArray& input, NDArray& output,
  ...
  // if (shape::order(colShapeBuffer) == 'c' &&  shape::order(imShapeBuffer) ==
  // 'c' && shape::strideDescendingCAscendingF(colShapeBuffer) &&
  // shape::strideDescendingCAscendingF(imShapeBuffer)) {
  if (false) {
            ...
            for (int kCol = 0; kCol < kW; ++kCol) {
              for (int colH = 0; colH < oH; ++colH) {
                for (int colW = 0; colW < oW; ++colW) {
                  ...
                  im = imBuff + b * imStride0 + c * imStride1 +
                       imRow * imStride2 + imCol * imStride3;

                  if (static_cast<unsigned>(imRow) <
        ...
        for (int colH = 0; colH < oH; ++colH, col4 += colStride4) {
          ...
          for (int colW = 0; colW < oW; ++colW, col5 += colStride5) {
            ...
            for (int c = 0; c < iC; ++c, col1 += colStride1, im1 += imStride1) {
              ...
              for (int kRow = 0; kRow < kH; ++kRow, col2 += colStride2,
                       imRow += dH, im2 += dH * imStride2) {
                int imCol = -pW + colW * sW;
                T const* col3 = col2;
                T* im3 = im2 + imCol * imStride3;
                for (int kCol = 0; kCol < kW; ++kCol, col3 += colStride3,
                         imCol += dW, im3 += dW * imStride3) {
                  if (static_cast<unsigned>(imRow) <
                          static_cast<unsigned>(iH) &&
                      static_cast<unsigned>(imCol) < static_cast<unsigned>(iW))
                    *im3 += *col3;
...
void col2im(sd::LaunchContext& context, const NDArray& input, NDArray& output,
            const int sH, const int sW, const int pH, const int pW,
            const int iH, const int iW, const int dH, const int dW) {
  BUILD_SINGLE_SELECTOR(
      input.dataType(), col2im_,
      (context, input, output, sH, sW, pH, pW, iH, iW, dH, dW), FLOAT_TYPES);
}
