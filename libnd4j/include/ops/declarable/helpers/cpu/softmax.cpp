...
template <typename T>
static void softMaxForVector_(void const* input, Nd4jLong const* inShapeInfo,
...
///////////////////////////////////////////////////////////////////
void softMaxForVector(sd::LaunchContext* context, const NDArray& input,
                      NDArray& output) {
  if (!input.isVector() || !output.isVector())
    throw std::runtime_error(
        "ops::helpers::softMaxForVector function: input and output arrays must "
        "be vectors !");

  auto xType = input.dataType();
  BUILD_SINGLE_SELECTOR(
      xType, softMaxForVector_,
      (input.buffer(), input.shapeInfo(), output.buffer(), output.shapeInfo()),
      FLOAT_TYPES);
}

template <typename T>
void softmax_loop(const T* input, T* output, const Nd4jLong* offsets,
                  Nd4jLong numOfSubArrs, uint32_t tadLen);

#ifdef _OPENMP
template <>
FORCEINLINE void softmax_loop(const float* input, float* output,
                              const Nd4jLong* offsets, Nd4jLong numOfSubArrs,
                              uint32_t tadLen) {
...
#pragma omp simd reduction(+ : sum)
    ...
    for (uint j = 0; j < tadLen; ++j) outBuff[j] /= sum;
  }
}
#else
template <>
...
template <typename T>
FORCEINLINE void softmax_loop(const T* input, T* output,
                              const Nd4jLong* offsets, Nd4jLong numOfSubArrs,
                              uint32_t tadLen) {
  auto func = PRAGMA_THREADS_FOR {
    for (auto i = start; i < stop; i++) {
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void softmax_(sd::LaunchContext* context, const NDArray& input,
                     NDArray& output, const int dimension) {
  const int rank = input.rankOf();

  if (input.isVector()) {
    if (rank == 1 || input.sizeAt(dimension) != 1)
      softMaxForVector_<T>(input.buffer(), input.shapeInfo(), output.buffer(),
                           output.shapeInfo());
    else
      output = 1.;
  } else if (input.isSameShapeStrict(output)) {
    TadPack tadPack = sd::ConstantTadHelper::getInstance().tadForDimensions(
    ...
    if (shape::elementWiseStride(tadShapeInfo) == 1) {
      ...
      bool canCast =
          sd::DataTypeUtils::castShapeInfo(tadShapeInfo, inShapeInfoCast);
      ...
      auto func = PRAGMA_THREADS_FOR {
        for (auto i = start; i < stop; i++) {
          ...
          for (uint j = 0; j < tadLen; ++j) outBuff[offsets[j]] /= sum;
        }
      };
    ...
    NDArray max =
        input.reduceAlongDimension(sd::reduce::Max, {dimension}, true);
    input.applyTrueBroadcast(sd::BroadcastOpsTuple::Subtract(), max, output,
                             false);
    output.applyTransform(sd::transform::Exp, output);
    NDArray sum =
        output.reduceAlongDimension(sd::reduce::Sum, {dimension}, true);
    output /= sum;
  }
}
...
void softmax(sd::LaunchContext* context, const NDArray& input, NDArray& output,
             const int dimension) {
  BUILD_SINGLE_SELECTOR(input.dataType(), softmax_,
                        (context, input, output, dimension), FLOAT_TYPES);
}
