...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void tileBP_(const NDArray& gradO /*input*/, NDArray& gradI /*output*/,
  ...
  if (gradO.ordering() == 'c' && gradOEWS == 1) {
    ...
    for (Nd4jLong i = 0; i < gradOLen; ++i) {
      ...
      gradI.p(fidx, gradI.e<T>(fidx) +
                        gradOBuff[shape::getIndexOffset(i, gradO.shapeInfo())]);
    }
  }
}
...
void tileBP(sd::LaunchContext* context, const NDArray& gradO /*input*/,
  ...
  BUILD_SINGLE_SELECTOR(gradI.dataType(), tileBP_, (gradO, gradI, reps),
                        FLOAT_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void tileBP_,
                      (const NDArray& gradO /*input*/,
