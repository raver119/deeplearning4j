...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void clipByNorm_(NDArray& input, NDArray& output,
  ...
  if (isInplace) {
    if (norm2.lengthOf() == 1) {
      if (normActual > normClip) input *= (normClip / normActual);
    } else {
...
void clipByNorm(sd::LaunchContext* context, NDArray& input, NDArray& output,
  ...
  BUILD_SINGLE_SELECTOR(output.dataType(), clipByNorm_,
                        (input, output, dimensions, clipNorm, isInplace),
                        FLOAT_TYPES);
}

template <typename T>
static void clipByGlobalNorm_(std::vector<NDArray*> const& inputs,
                              double clipNorm, sd::memory::Workspace* workspace,
                              std::vector<NDArray*>& outputs, bool isInplace) {
  T globalNorm = 0;  // NDArrayFactory::create<T>(0, inputs[0]->getContext());
  ...
  // globalNorm.applyTransform(transform::Sqrt, nullptr, nullptr);// =
  // sd::math::nd4j_sqrt(globalNorm);
...
void clipByGlobalNorm(sd::LaunchContext* context,
  ...
  BUILD_SINGLE_SELECTOR(outputs[0]->dataType(), clipByGlobalNorm_,
                        (inputs, clipNorm, workspace, outputs, isInplace),
                        FLOAT_TYPES);
}
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void clipByNormBP_(const NDArray& input, const NDArray& gradO,
                          NDArray& gradI /*output*/,
                          const std::vector<int>& dimensions,
                          const NDArray& clipNorm) {
  const int rank = input.rankOf();
  ...
  if (norm2.lengthOf() == 1) {
    ...
    if (N > cn) {
      const T sumOfProd = (input * gradO)
                              .reduceNumber(reduce::Sum)
                              .e<T>(0);  // reduce to scalar
      ...
      (const_cast<NDArray&>(input))
          .applyPairwiseLambda<T>(const_cast<NDArray&>(gradO), lambda, gradI);
    } else
      gradI.assign(gradO);
  } else {
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        if (N > cn) {
          ...
          const T sumOfProd = (inputSubArr * gradOSubArr)
                                  .reduceNumber(reduce::Sum)
                                  .e<T>(0);  // reduce to scalar
          const T factor1 = static_cast<T>(1.f) / N;
          const T factor3 = factor1 / (N * N);  // 1 / (N*N*N)
          ...
          auto lambda =
              LAMBDA_TT(elem1, elem2, cn, sumOfProd, factor1, factor3) {
            return cn * (factor1 * elem2 - factor3 * elem1 * sumOfProd);
          };
...
void clipByNormBP(sd::LaunchContext* context, const NDArray& input,
  ...
  BUILD_SINGLE_SELECTOR(gradI.dataType(), clipByNormBP_,
                        (input, gradO, gradI, dimensions, clipNorm),
                        FLOAT_TYPES);
}
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void clipByAveraged_(NDArray& input, NDArray& output,
                            const std::vector<int>& dimensions,
                            const NDArray& clipNorm, const bool isInplace) {
  auto cn = clipNorm.e<T>(0);
...
void clipByAveraged(sd::LaunchContext* context, NDArray& input, NDArray& output,
                    const std::vector<int>& dimensions, const NDArray& clipNorm,
                    const bool isInplace) {
  BUILD_SINGLE_SELECTOR(input.dataType(), clipByAveraged_,
                        (input, output, dimensions, clipNorm, isInplace),
                        FLOAT_TYPES);
}
...
template <typename T>
static void clipByValue_(NDArray& input, double leftBound, double rightBound,
                         NDArray& output) {
...
void clipByValue(sd::LaunchContext* context, NDArray& input, double leftBound,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), clipByValue_,
                        (input, leftBound, rightBound, output), FLOAT_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void clipByValue_,
                      (NDArray & input, double leftBound, double rightBound,
                       NDArray& output);
                      , FLOAT_TYPES);
