...
template <typename T>
static void __depthToSpace(const NDArray &input, NDArray *output,
                           int block_size, bool isNHWC) {
  ...
  if (isNHWC) {
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto out_idx = start; out_idx < stop; out_idx++) {
        ...
        const int inp_idx =
            in_d +
            input_depth * (in_w + input_width * (in_h + input_height * b));
        ...
        const int output_idx =
            bX + block_size *
                     (iX + input_width *
                               (bY + block_size *
                                         (oC_iY +
                                          n * output_depth_by_input_height)));
...
void _depthToSpace(sd::LaunchContext *context, const NDArray &input,
  ...
  BUILD_SINGLE_SELECTOR(xType, __depthToSpace,
                        (input, output, block_size, isNHWC), LIBND4J_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void __depthToSpace,
                      (const NDArray &input, NDArray *output, int block_size,
                       bool isNHWC);
                      , LIBND4J_TYPES);
