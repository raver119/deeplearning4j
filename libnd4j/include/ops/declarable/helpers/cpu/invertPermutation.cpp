...
void invertPermutation(sd::LaunchContext* context, const NDArray& input,
  ...
  for (int i = 0; i < length; ++i) {
    ...
    if (!uniqueElems.insert(elem)
             .second)  // this operation forbids us to use #pragma omp
      throw std::runtime_error(
          "helpers::invertPermutation function: input array contains "
          "duplicates !");

    if (elem < 0 || elem > length - 1)
      throw std::runtime_error(
          "helpers::invertPermutation function: element of input array is out "
          "of range (0, length-1) !");
