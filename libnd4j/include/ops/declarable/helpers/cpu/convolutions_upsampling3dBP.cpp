...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void upsampling3dBP_(const NDArray& gradO, NDArray& gradI,
                            const bool isNCDHW) {
    ...
    for (uint b = start_x; b < stop_x; b += inc_x) {
      for (uint c = start_y; c < stop_y; c += inc_y) {
        for (uint d = start_z; d < stop_z; d += inc_z) {
          for (uint h = 0; h < iH; ++h) {
            for (uint w = 0; w < iW; ++w) {
              const auto zOffset = b * zStride0 + c * zStride1 + d * zStride2 +
                                   h * zStride3 + w * zStride4;
              ...
              for (uint xd = d * factorD; xd < d * factorD + factorD; ++xd)
                for (uint xh = h * factorH; xh < h * factorH + factorH; ++xh)
                  for (uint xw = w * factorW; xw < w * factorW + factorW; ++xw)
                    z[zOffset] +=
                        x[b * xStride0 + c * xStride1 + xd * xStride2 +
                          xh * xStride3 + xw * xStride4];
            }
          }
        }
      }
...
void ConvolutionUtils::upsampling3dBP(sd::graph::Context& block,
                                      ...
                                      const bool isNCHW) {
  BUILD_SINGLE_SELECTOR(gradO.dataType(), upsampling3dBP_,
                        (gradO, gradI, isNCHW), FLOAT_TYPES);
}
