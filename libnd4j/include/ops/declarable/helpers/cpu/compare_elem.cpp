...
namespace helpers {
template <typename T>
static void _compare_elem(NDArray* input, bool isStrictlyIncreasing,
                          bool& output) {
...
void compare_elem(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(xType, _compare_elem,
                        (input, isStrictlyIncreasing, output), LIBND4J_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void _compare_elem,
                      (NDArray * A, bool isStrictlyIncreasing, bool& output);
                      , LIBND4J_TYPES);
}  // namespace helpers
}  // namespace ops
}  // namespace sd
