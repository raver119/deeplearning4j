...
template <typename T>
static void _range(const NDArray& start, const NDArray& delta,
                   NDArray& outVector) {
...
void range(sd::LaunchContext* context, const NDArray& start,
  ...
  BUILD_SINGLE_SELECTOR(outVector.dataType(), _range, (start, delta, outVector),
                        LIBND4J_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void _range,
                      (const NDArray& start, const NDArray& delta,
                       NDArray& outVector),
                      LIBND4J_TYPES);
