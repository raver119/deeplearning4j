...
static T continuedFraction(const T a, const T b, const T x) {
  ...
  for (uint i = 1; i <= maxIter; ++i) {
    ...
    result *= t2 * t1;
    val =
        -(a + i) * (aPlusb + i) * x / ((aPlus2i + static_cast<T>(1)) * aPlus2i);
    // t1
    t1 = static_cast<T>(1) + val * t1;
    if (math::nd4j_abs<T>(t1) < min) t1 = min;
    t1 = static_cast<T>(1) / t1;
...
///////////////////////////////////////////////////////////////////
// evaluates incomplete beta function for positive a and b, and x between 0
// and 1.
template <typename T>
static T betaIncCore(T a, T b, T x) {
  ...
  if (x <= (a + static_cast<T>(1)) / (a + b + static_cast<T>(2)))
    ...
    return static_cast<T>(1) -
           front * continuedFraction<T>(b, a, static_cast<T>(1) - x) / b;
}

///////////////////////////////////////////////////////////////////
template <typename T>
static void betaIncForArray(sd::LaunchContext* context, const NDArray& a,
...
void betaInc(sd::LaunchContext* context, const NDArray& a, const NDArray& b,
  ...
  BUILD_SINGLE_SELECTOR(xType, betaIncForArray, (context, a, b, x, output),
                        FLOAT_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void betaIncForArray,
                      (sd::LaunchContext * context, const NDArray& a,
                       const NDArray& b, const NDArray& x, NDArray& output),
                      FLOAT_TYPES);
