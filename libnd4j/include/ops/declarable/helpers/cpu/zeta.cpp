...
static FORCEINLINE T zetaScalarSlow(const T x, const T q) {
  const T precision = (T)1e-7;  // function stops the calculation of series when
                                // next item is <= precision
...
template <typename T>
static void zeta_(sd::LaunchContext* context, const NDArray& x,
                  const NDArray& q, NDArray& z) {
...
BUILD_SINGLE_TEMPLATE(template void zeta_,
                      (sd::LaunchContext * context, const NDArray& x,
                       const NDArray& q, NDArray& z),
                      FLOAT_TYPES);
