...
void cropAndResizeFunctor_(NDArray const *images, NDArray const *boxes,
  ...
  for (auto b = 0; b < numBoxes; ++b) {
    ...
    T heightScale = (cropHeight > 1)
                        ...
                        : T(0);
    T widthScale =
        (cropWidth > 1) ? (x2 - x1) * (imageWidth - 1) / (cropWidth - 1) : T(0);
      ...
      for (auto y = start; y < stop; y++) {
        ...
        if (method == 0 /* bilinear */) {
          ...
          for (auto x = 0; x < cropWidth; ++x) {
            ...
            for (auto d = 0; d < depth; ++d) {
              const float topLeft(
                  images->e<float>(bIn, topYIndex, left_x_index, d));
              const float topRight(
                  images->e<float>(bIn, topYIndex, right_x_index, d));
              const float bottomLeft(
                  images->e<float>(bIn, bottomYIndex, left_x_index, d));
              const float bottomRight(
                  images->e<float>(bIn, bottomYIndex, right_x_index, d));
          ...
          for (auto x = 0; x < cropWidth; ++x) {
            ...
            for (auto d = 0; d < depth; ++d) {
              crops->p(b, y, x, d,
                       images->e<T>(bIn, closestYIndex, closestXIndex, d));
            }
          }
        }
      }
