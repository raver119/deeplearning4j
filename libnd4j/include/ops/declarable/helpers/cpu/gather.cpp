  ...
  if (indices != nullptr) {
    ...
    if (indices->isScalar()) {
      ...
      if (input->rankOf() == 1 && output->rankOf() == 1) {
        ...
        if (shape::order(inTadShapeInfo) == shape::order(outTadShapeInfo) &&
            shape::order(inTadShapeInfo) == 'c' &&
            input->dataType() == output->dataType() &&
            shape::elementWiseStride(inTadShapeInfo) == 1 &&
            shape::elementWiseStride(outTadShapeInfo) == 1) {
          auto func = PRAGMA_THREADS_FOR {
            for (auto i = start; i < stop; i++) {
              auto inBuff = input->bufferWithOffset(
                  inTadPack.primaryOffsets()[indices->e<Nd4jLong>(i)]);
              auto outBuff =
                  output->bufferWithOffset(outTadPack.primaryOffsets()[i]);

              memcpy(outBuff, inBuff,
                     shape::length(inTadShapeInfo) * input->sizeOfT());
            }
          };
    ...
    if (numOfIntArgs == 2) {  // scalar case
      ...
      if (shape::order(inTadShapeInfo) == shape::order(outTadShapeInfo) &&
        ...
        auto func = PRAGMA_THREADS_FOR {
          for (auto i = start; i < stop; i++) {
            auto inBuff = input->bufferWithOffset(
                inTadPack.primaryOffsets()[intArgs[i + 1]]);
            void* outBuff =
                output->bufferWithOffset(outTadPack.primaryOffsets()[i]);

            std::memcpy(outBuff, inBuff,
                        shape::length(inTadShapeInfo) * input->sizeOfT());
          }
        };
        samediff::Threads::parallel_tad(func, 0, numOfSubArrs);
