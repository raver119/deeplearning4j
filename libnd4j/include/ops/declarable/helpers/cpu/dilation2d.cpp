...
static void dilation2d_(NDArray* input, NDArray* weights, NDArray* output,
          ...
          for (uint c = 0; c < iC; ++c) {
            ...
            for (uint kh = 0; kh < kH; ++kh) {
              ...
              for (uint kw = 0; kw < kW; ++kw) {
                ...
                uint xCoords[4] = {static_cast<uint>(b), static_cast<uint>(ih),
                                   static_cast<uint>(iw), c};
                uint yCoords[3] = {kh, kw, c};

                const X val = x[shape::getOffset(xShapeInfo, xCoords)] +
                              y[shape::getOffset(yShapeInfo, yCoords)];
                if (val > max) max = val;
              }
            }

            uint zCoords[4] = {static_cast<uint>(b), static_cast<uint>(oh), ow,
                               c};
...
void dilation2d(sd::LaunchContext* context, NDArray* input, NDArray* weights,
  ...
  BUILD_SINGLE_SELECTOR_TWICE(input->dataType(), dilation2d_,
                              (input, weights, output, sH, sW, pH, pW, dH, dW),
                              FLOAT_TYPES);
}
