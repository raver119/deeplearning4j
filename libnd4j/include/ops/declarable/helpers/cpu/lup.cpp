...
BUILD_SINGLE_TEMPLATE(template void swapRows_,
                      (NDArray * matrix, int theFirst, int theSecond),
                      FLOAT_TYPES);

template <typename T>
static void swapRows(T* matrixBuf, Nd4jLong const* matrixShape,
...
void swapRows(NDArray* matrix, int theFirst, int theSecond) {
  BUILD_SINGLE_SELECTOR(matrix->dataType(), swapRows_,
                        (matrix, theFirst, theSecond), FLOAT_TYPES);
}
...
static void invertLowerMatrix_(NDArray* inputMatrix, NDArray* invertedMatrix) {
  ...
  for (int i = 1; i < n; i++) {
    for (int j = 0; j < i - 1; j++)
      for (int k = 0; k < i; k++)
        invertedMatrix->t<T>(i, j) -=
            ((invertedMatrix->t<T>(k, j) * inputMatrix->t<T>(i, k) /
              inputMatrix->t<T>(i, i)));
  }
}

BUILD_SINGLE_TEMPLATE(template void invertLowerMatrix_,
                      (NDArray * inputMatrix, NDArray* invertedMatrix);
                      , FLOAT_TYPES);
...
void invertLowerMatrix(NDArray* inputMatrix, NDArray* invertedMatrix) {
  BUILD_SINGLE_SELECTOR(inputMatrix->dataType(), invertLowerMatrix_,
                        (inputMatrix, invertedMatrix), FLOAT_TYPES);
}
...
BUILD_SINGLE_TEMPLATE(template void _invertUpperMatrix,
                      (NDArray * inputMatrix, NDArray* invertedMatrix);
                      , FLOAT_TYPES);

void invertUpperMatrix(NDArray* inputMatrix, NDArray* invertedMatrix) {
  BUILD_SINGLE_SELECTOR(inputMatrix->dataType(), _invertUpperMatrix,
                        (inputMatrix, invertedMatrix), FLOAT_TYPES);
}

template <typename T, typename I>
static NDArray lup_(LaunchContext* context, NDArray* input, NDArray* compound,
                    NDArray* permutation) {
  ...
  NDArray permutationMatrix(
      input, false, context);  // has same shape as input and contiguous strides
    ...
    if (pivotValue > DataTypeUtils::min<T>()) {
      ...
      for (int j = i + 1; j < rowNum; j++) {
        ...
        for (int k = i + 1; k < rowNum; k++) {
          compoundMatrix.t<T>(j, k) -=
              compoundMatrix.t<T>(j, i) * compoundMatrix.t<T>(i, k);
        }
      }
    }
  }
  ...
  if (permutation != nullptr) {
    auto permutaionVector = NDArrayFactory::create(
        'c', {rowNum}, DataTypeUtils::fromT<I>(), input->getContext());
...
template <typename T>
void processColumns(int currentRow, int rowNum, T* compoundBuf,
                    Nd4jLong const* compoundShape) {
...
template <typename T>
static void doolitleLU(LaunchContext* context, NDArray* compound,
                       Nd4jLong rowNum) {
...
template <typename T, typename I>
static void luNN_(LaunchContext* context, NDArray* compound,
                  NDArray* permutation, Nd4jLong rowNum) {
  // const int rowNum = compound->rows();
  //        const int columnNum = output->columns();
  if (permutation) {  // LUP algorithm
    permutation->linspace(0);
    auto permutationBuf =
        permutation->bufferAsT<I>();  // dataBuffer()->primaryAsT<I>();
...
static void lu_(LaunchContext* context, NDArray* input, NDArray* output,
  ...
  auto loop = PRAGMA_THREADS_FOR {
    for (auto i = start; i < stop; i++) {
      luNN_<T, I>(context, &outputs.at(i),
                  permutationVectors ? &permutations.at(i) : nullptr, n);
    }
  };
...
void lu(LaunchContext* context, NDArray* input, NDArray* output,
        NDArray* permutation) {
  BUILD_DOUBLE_SELECTOR(input->dataType(),
                        permutation ? permutation->dataType() : DataType::INT32,
                        lu_, (context, input, output, permutation), FLOAT_TYPES,
                        INDEXING_TYPES);
}

//    BUILD_DOUBLE_TEMPLATE(template NDArray lu_, (LaunchContext *context,
//    NDArray* input, NDArray* output, NDArray* permutation), FLOAT_TYPES,
//    INDEXING_TYPES);

template <typename T>
static int determinant_(LaunchContext* context, NDArray* input,
                        NDArray* output) {
  Nd4jLong n = input->sizeAt(-1);
  Nd4jLong n2 = n * n;

  auto matrix =
      NDArrayFactory::create(input->ordering(), {n, n}, input->dataType(),
                             context);  //, block.workspace());

  for (int e = 0; e < output->lengthOf(); e++) {
    for (int k = e * n2, row = 0; k < (e + 1) * n2; ++k, ++row)
      matrix.p(row, input->e<T>(k));
    output->p(e, lup_<T, int>(context, &matrix, (NDArray*)nullptr,
                              (NDArray*)nullptr));
  }
...
int determinant(sd::LaunchContext* context, NDArray* input, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return determinant_,
                        (context, input, output), FLOAT_TYPES);
}
...
int logAbsDeterminant_(LaunchContext* context, NDArray* input,
  ...
  for (int e = 0; e < output->lengthOf(); e++) {
    ...
    NDArray det =
        lup_<T, int>(context, &matrix, (NDArray*)nullptr, (NDArray*)nullptr);
    if (det.e<T>(0) != 0.f)
...
static int inverse_(LaunchContext* context, NDArray* input, NDArray* output) {
  ...
  auto matrix = NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(),
                                       context);  //, block.workspace());
  auto compound = NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(),
                                         context);  //, block.workspace());
  auto permutation =
      NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(), context);
  auto lowerMatrix =
      NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(), context);
  auto upperMatrix =
      NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(), context);
  ...
  for (int e = 0; e < totalCount; e++) {
    ...
    T det = lup_<T, int>(context, &matrix, &compound, &permutation)
                .template e<T>(0);

    // FIXME: and how this is going to work on float16?
    if (sd::math::nd4j_abs<T>(det) < T(0.000001)) {
      nd4j_printf(
          "matrix_inverse: The matrix %i has no inverse due determinant is "
          "%lf. Quiting...\n",
          e, det);
    ...
    for (int k = 1; k < n;
         k++) {  // and then put all values under main diagonal on to it
      for (int j = 0; j < k; j++)
...
template <typename T>
static int lowerInverse_(LaunchContext* context, NDArray* input,
                         NDArray* output) {
  auto n = input->sizeAt(-1);
  auto n2 = n * n;
  auto totalCount = output->lengthOf() / n2;
...
template <typename T>
static int upperInverse_(LaunchContext* context, NDArray* input,
                         NDArray* output) {
  auto n = input->sizeAt(-1);
  auto n2 = n * n;

  output->nullify();  // fill up output tensor with zeros
  //        auto matrix = NDArrayFactory::create('c', {n, n},
  //        DataTypeUtils::fromT<T>(), context); //, block.workspace()); auto
  //        lowerMatrix = NDArrayFactory::create('c', {n, n},
  //        DataTypeUtils::fromT<T>(), context); auto upperMatrix =
  //        NDArrayFactory::create('c', {n, n}, DataTypeUtils::fromT<T>(),
  //        context);
...
int lowerInverseFunctor(sd::LaunchContext* context, NDArray* input,
                        NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return lowerInverse_,
                        (context, input, output), FLOAT_TYPES);
}

int upperInverseFunctor(sd::LaunchContext* context, NDArray* input,
                        NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return upperInverse_,
                        (context, input, output), FLOAT_TYPES);
}

template <typename T>
static bool checkCholeskyInput_(sd::LaunchContext* context,
                                NDArray const* input) {
  // std::unique_ptr<NDArray> matrix(NDArrayFactory::create_('c', {n, n},
  // input->dataType())); //, block.workspace());
  ResultSet lastMatrixList = input->allTensorsAlongDimension(
  ...
  for (size_t i = 0; i < lastMatrixList.size(); i++) {
    ...
    for (Nd4jLong r = 0; r < thisMatrix.rows(); r++)
      for (Nd4jLong c = 0; c < thisMatrix.columns(); c++)
        if (sd::math::nd4j_abs(thisMatrix.e<T>(r, c) -
                               lastMatrixList.at(i).e<T>(c, r)) >
            DataTypeUtils::min<T>())
          return false;

    NDArray output = NDArrayFactory::create<T>(0., context);
    if (ND4J_STATUS_OK != determinant(context, &thisMatrix, &output))
      return false;
    if (output.e<T>(0) <= T(0)) return 0;
    NDArray reversedMatrix = thisMatrix.dup();
    if (ND4J_STATUS_OK != inverse(context, &thisMatrix, &reversedMatrix))
      return false;
    if (ND4J_STATUS_OK != determinant(context, &reversedMatrix, &output))
      return false;
    if (output.e<T>(0) <= T(0)) return 0;
  }
...
bool checkCholeskyInput(sd::LaunchContext* context, NDArray const* input) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return checkCholeskyInput_,
                        (context, input), FLOAT_TYPES);
}
...
int cholesky_(LaunchContext* context, NDArray* input, NDArray* output,
  ...
  std::unique_ptr<NDArray> matrix(NDArrayFactory::create_(
      'c', {n, n}, input->dataType(), context));  //, block.workspace());
  std::unique_ptr<NDArray> lowerMatrix(
      NDArrayFactory::create_('c', {n, n}, input->dataType(), context));
  ...
  for (int e = 0; e < totalCount; e++) {
    ...
    for (Nd4jLong col = 0; col < n; col++) {
      for (Nd4jLong row = 0; row < col; row++) {
        ...
        lowerMatrix->p(col, row,
                       (matrix->e<T>(row, col) - rowSum) /
                           lowerMatrix->e<double>(row, row));
      }
      T diagonalSum = 0;
      ...
      lowerMatrix->p(
          col, col,
          sd::math::nd4j_sqrt<T, T>(matrix->e<T>(col, col) - diagonalSum));
      // nd4j_printf("%i: ", col);
...
int cholesky(sd::LaunchContext* context, NDArray* input, NDArray* output,
             bool inplace) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return cholesky_,
                        (context, input, output, inplace), FLOAT_TYPES);
}
...
int logdetFunctor_(LaunchContext* context, NDArray* input, NDArray* output) {
  ...
  if (res != ND4J_STATUS_OK) return res;
  auto n = input->sizeAt(-1);
  auto totalCount = output->lengthOf();
  std::vector<T> d(n);
  ResultSet matricies = tempOutput.allTensorsAlongDimension(
      {input->rankOf() - 2, input->rankOf() - 1});

  for (Nd4jLong e = 0; e < totalCount; e++) {
    for (size_t i = 0; i < n; ++i)
      output->t<T>(e) += sd::math::nd4j_log<T, T>(
          sd::math::nd4j_pow<T, T, T>(matricies.at(e).t<T>(i, i), T(2)));
  }
  return ND4J_STATUS_OK;
}

int logdetFunctor(sd::LaunchContext* context, NDArray* input, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return logdetFunctor_,
                        (context, input, output), FLOAT_TYPES);
}

int lup(sd::LaunchContext* context, NDArray* input, NDArray* compound,
        NDArray* permutation) {
