...
template <typename T>
Nd4jLong checkIndices_(const NDArray& indices, const NDArray& output,
                       const int axis) {
...
Nd4jLong checkIndices(sd::LaunchContext* context, const NDArray& indices,
  ...
  BUILD_SINGLE_SELECTOR(indices.dataType(), return checkIndices_,
                        (indices, output, axis), INDEXING_TYPES);
}
...
void scatter(sd::LaunchContext* context, pairwise::Ops op,
  ...
  if (outRank == 1) {
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        NDArray outSubArr =
            output(indices.e<Nd4jLong>(i), std::vector<int>({0}));
...
void scatterForLoss(sd::LaunchContext* context, const NDArray& indices,
  ...
  std::vector<int> dimsToExclude =
      ShapeUtils::evalDimsToExclude(updates.rankOf(), {-1});
