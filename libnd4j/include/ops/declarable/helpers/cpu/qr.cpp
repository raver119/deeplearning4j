...
NDArray vmul(NDArray const& v, int n) {
  NDArray res('c', {n, n}, v.dataType(),
              v.getContext());  // x = matrix_new(n, n);
...
template <typename T>
void qrSingle(NDArray* matrix, NDArray* Q, NDArray* R,
  ...
  NDArray e(
      'c', {M}, DataTypeUtils::fromT<T>(),
      Q->getContext());  // two internal buffers and scalar for squared norm

  for (Nd4jLong k = 0; k < N && k < M - 1;
       k++) {  // loop for columns, but not further then row number
    e.nullify();
    z = matrixMinor<T>(z, k);  // minor computing for current column with given
    ...
    if (matrix->t<T>(k, k) >
        T(0.f))         // negate on positive matrix diagonal element
      norm *= T(-1.f);  //.applyTransform(transform::Neg, nullptr, nullptr);
                        ////t<T>(0) = -norm.t<T>(0);
    // e.t<T>(k) = T(1.f); // e - is filled by 0 vector except diagonal element
    // (filled by 1) auto tE = e; tE *= norm;
    //            norm.printIndexedBuffer("Norm!!!");
    e.p(k, norm);
    e += currentColumn;  //  e += tE; // e[i] = x[i] + a * e[i] for each i from
                         //  0 to n - 1
...
template <typename T>
void qr_(NDArray const* input, NDArray* outputQ, NDArray* outputR,
         bool const fullMatricies) {
  ...
  auto batching = PRAGMA_THREADS_FOR {
    for (auto batch = start; batch < stop; batch++) {
      // qr here
      qrSingle<T>(&listInput.at(batch), &listOutQ.at(batch),
...
void qr(sd::LaunchContext* context, NDArray const* input, NDArray* outputQ,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), qr_,
                        (input, outputQ, outputR, fullMatricies), FLOAT_TYPES);
}
