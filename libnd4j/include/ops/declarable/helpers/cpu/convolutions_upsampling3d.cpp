...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void upsampling3d_(const NDArray& input, NDArray& output,
                          const int factorD, const int factorH,
                          const int factorW, const bool isNCDHW) {
  // input  has shape [bS, iC, iD, iH, iW] (NCDHW) or [bS, iD, iH, iW, iC]
  // (NDHWC) output has shape [bS, iC, factorD*iD, factorH*iH, factorW*iW ]
        ...
        for (uint d = start_z; d < stop_z; d += inc_z) {
          for (uint h = 0; h < oH; ++h) {
            for (uint w = 0; w < oW; ++w) {
              ...
              z[b * zStride0 + c * zStride1 + d * zStride2 + h * zStride3 +
                w * zStride4] =
                  x[b * xStride0 + c * xStride1 + xCoord2 * xStride2 +
                    xCoord3 * xStride3 + xCoord4 * xStride4];
            }
          }
        }
      }
...
void ConvolutionUtils::upsampling3d(sd::graph::Context& block,
                                    ...
                                    const int factorD, const int factorH,
                                    const int factorW, const bool isNCDHW) {
  BUILD_SINGLE_SELECTOR(input.dataType(), upsampling3d_,
                        (input, output, factorD, factorH, factorW, isNCDHW),
                        FLOAT_TYPES);
}
