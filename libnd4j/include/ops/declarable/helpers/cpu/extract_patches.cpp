...
template <typename T>
static void _extractPatches(NDArray* images, NDArray* output, int sizeRow,
                            int sizeCol, int strideRow, int strideCol,
  ...
  auto rowCast = 1;  //(sizeRow - 1)*rateRow < outRowDim/sizeRow  ?0:1;///(ksize
                     //* lastDim > rowDim * ksizeColsEffective + lastDim?1:0);
  auto colCast = 1;  // colDim / ksizeColsEffective +2 <= sizeCol?0:1;//(ksize *
                     // lastDim > ksizeRowsEffective * colDim + lastDim?1:0);
...
void extractPatches(sd::LaunchContext* context, NDArray* images,
  ...
  BUILD_SINGLE_SELECTOR(xType, _extractPatches,
                        (images, output, sizeRow, sizeCol, stradeRow, stradeCol,
                         rateRow, rateCol, theSame),
                        LIBND4J_TYPES);
}

BUILD_SINGLE_TEMPLATE(template void _extractPatches,
                      (NDArray * input, NDArray* output, int sizeRow,
                       int sizeCol, int stradeRow, int stradeCol, int rateRow,
