...
static void conv2d_(sd::graph::Context& block, const NDArray* input,
  ...
  if (isNCHW)
    ...
    input = new NDArray(input->permute(
        {0, 3, 1, 2}));  // [bS, iH, iW, iC] -> [bS, iC, iH, iW] if NHWC
  ...
  NDArray col('c', {bS, oH, oW, kH, kW, iC}, input->dataType(),
              input->getContext());
  NDArray colP = col.permute({0, 5, 3, 4, 1, 2});  // {bS, iC, kH, kW, oH, oW}
  NDArray mmulResult('f', {bS * oH * oW, oC}, output->dataType(),
                     output->getContext());
  ...
  MmulHelper::tensorDot(
      &col, weights, &mmulResult, {3, 4, 5}, wAxes,
...
void ConvolutionUtils::conv2d(sd::graph::Context& block, const NDArray* input,
                              const NDArray* weights, const NDArray* bias,
