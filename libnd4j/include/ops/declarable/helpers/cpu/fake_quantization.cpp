...
//
template <typename T>
static void nudge(T min, T max, int quantMin, int quantMax, T* scale,
                  T* nudgedMin, T* nudgedMax) {
  ...
  *scale = (max - min) / (quantMaxF - quantMinF);
  ...
  uint16_t const nudged_zero_point = [zeroPointFromMin, quantMin, quantMax,
                                      quantMaxF, quantMinF] {
...
template <typename T>
void fakeQuantWithMinMaxVarsPerChannel_(NDArray* input, NDArray* min,
                                        NDArray* max, int numBits,
  ...
  for (auto i = 0; i < channels; i++) {
    ...
    nudge<T>(min->t<T>(i), max->t<T>(i), lowIntBound, upperIntBound, &scale,
...
void fakeQuantWithMinMaxVars_(NDArray* input, NDArray* min, NDArray* max,
  ...
  nudge<T>(min->t<T>(0), max->t<T>(0), lowIntBound, upperIntBound, &scale,
           &nudgedMin, &nudgedMax);
  // quantization as one
  auto fakeQuantizationWithMinMax = LAMBDA_T(x, nudgedMin, nudgedMax, scale) {
    ...
    val -= nudgedMin;
    return (sd::math::nd4j_floor<T, T>(val / scale + T(0.5f)) * scale +
            nudgedMin);
  };
...
BUILD_SINGLE_TEMPLATE(template void fakeQuantWithMinMaxVars_,
                      (NDArray * input, NDArray* min, NDArray* max, int numBits,
                       bool narrowed, NDArray* output),
                      FLOAT_TYPES);
