...
// segment max
template <typename T>
static void segmentMaxFunctor_(NDArray* input, NDArray* indices,
                               NDArray* output) {
...
// segmen min
template <typename T>
static void segmentMinFunctor_(NDArray* input, NDArray* indices,
                               NDArray* output) {
  // int numClasses = output->sizeAt(0);
  // if input is a vector: (as if in doc sample)
  Nd4jLong idx = indices->e<Nd4jLong>(0);
    ...
    for (Nd4jLong i = 1; i < indices->lengthOf(); i++) {
      if (indices->e<Nd4jLong>(i) == idx) {
        for (Nd4jLong e = 0; e < minT.lengthOf(); e++) {
          minT.p(e,
                 sd::math::nd4j_min(minT.e<T>(e), listOfTensors.at(i).e<T>(e)));
        }
      } else {
        idx = indices->e<Nd4jLong>(i);
...
// segmen mean
template <typename T>
static void segmentMeanFunctor_(NDArray* input, NDArray* indices,
                                NDArray* output) {
  int numClasses = output->sizeAt(0);
  // if input is a vector: (as if in doc sample)
...
template <typename T>
static void segmentSumFunctor_(NDArray* input, NDArray* indices,
                               NDArray* output) {
  int numClasses = output->sizeAt(0);
  // if input is a vector: (as if in doc sample)
  int idx = indices->e<int>(0);
...
template <typename T>
static void segmentProdFunctor_(NDArray* input, NDArray* indices,
                                NDArray* output) {
  // int numClasses = output->sizeAt(0);
  // if input is a vector: (as if in doc sample)
  int idx = indices->e<int>(0);
...
//    template <typename T>
//    static bool segmentIndicesValidate_(NDArray* indices, NDArray& aexpected,
//    NDArray& anOutput) {
//      }

void segmentMaxFunctor(sd::LaunchContext* context, NDArray* input,
                       NDArray* indices, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), segmentMaxFunctor_,
                        (input, indices, output), LIBND4J_TYPES);
}

void segmentMinFunctor(sd::LaunchContext* context, NDArray* input,
                       NDArray* indices, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), segmentMinFunctor_,
                        (input, indices, output), LIBND4J_TYPES);
}

void segmentMeanFunctor(sd::LaunchContext* context, NDArray* input,
                        NDArray* indices, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), segmentMeanFunctor_,
                        (input, indices, output), LIBND4J_TYPES);
}

void segmentSumFunctor(sd::LaunchContext* context, NDArray* input,
                       NDArray* indices, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), segmentSumFunctor_,
                        (input, indices, output), LIBND4J_TYPES);
}

void segmentProdFunctor(sd::LaunchContext* context, NDArray* input,
                        NDArray* indices, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), segmentProdFunctor_,
                        (input, indices, output), LIBND4J_TYPES);
}
...
bool segmentIndicesValidate(sd::LaunchContext* context, NDArray* indices,
  ...
  for (Nd4jLong e = 1; e < indices->lengthOf(); e++) {
    output = indices->e(e);
    if (val.e<Nd4jLong>(0) > output.e<Nd4jLong>(0)) return false;
    val = indices->e(e);
  }
...
// BUILD_SINGLE_TEMPLATE(template bool segmentIndicesValidate_, (NDArray*,
// NDArray&, NDArray&), LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template void segmentProdFunctor_,
                      (NDArray * input, NDArray* indices, NDArray* output),
                      LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template void segmentSumFunctor_,
                      (NDArray * input, NDArray* indices, NDArray* output),
                      LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template void segmentMeanFunctor_,
                      (NDArray * input, NDArray* indices, NDArray* output),
                      LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template void segmentMinFunctor_,
                      (NDArray * input, NDArray* indices, NDArray* output),
                      LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template void segmentMaxFunctor_,
                      (NDArray * input, NDArray* indices, NDArray* output),
                      LIBND4J_TYPES);
// --------------------------------------------------------------------------------------------------------------
...
bool unsortedSegmentIndicesValidate(sd::LaunchContext* context,
                                    NDArray* indices, Nd4jLong expected,
                                    Nd4jLong& output) {
...
template <typename T>
static void unsortedSegmentMaxFunctor_(NDArray* input, NDArray* indices,
  ...
  if (input->isVector()) {  // 1D case
    ...
    for (auto fi = idxs.begin(); fi != idxs.end(); ++fi) {
      ...
      for (Nd4jLong idx = 1; idx < static_cast<Nd4jLong>(fi->second.size());
           ++idx) {
...
void unsortedSegmentMaxFunctor(sd::LaunchContext* context, NDArray* input,
                               ...
                               NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), unsortedSegmentMaxFunctor_,
...
template <typename T>
static void unsortedSegmentMinFunctor_(NDArray* input, NDArray* indices,
                                       Nd4jLong numOfClasses, NDArray* output) {
  // if input is a vector: (as if in doc sample)
  // int idx = static_cast<int>((*indices)(0.));
  MAP_IMPL<Nd4jLong, std::vector<Nd4jLong>> idxs;  //(indices->lengthOf());
...
template <typename T>
void unsortedSegmentProdFunctor_(NDArray* input, NDArray* indices,
                                 Nd4jLong numOfClasses, NDArray* output) {
  MAP_IMPL<Nd4jLong, std::vector<Nd4jLong>> idxs;  //(indices->lengthOf());
  for (Nd4jLong e = 0; e < indices->lengthOf(); ++e)
    idxs[indices->e<Nd4jLong>(e)].push_back(e);
...
void unsortedSegmentProdFunctor(sd::LaunchContext* context, NDArray* input,
                                NDArray* indices, Nd4jLong numOfClasses,
                                NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), unsortedSegmentProdFunctor_,
                        (input, indices, numOfClasses, output), NUMERIC_TYPES);
}
...
void unsortedSegmentSqrtNFunctor(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {  // 1D case
    for (auto fi = idxs.begin(); fi != idxs.end(); ++fi) {
      ...
      output->p(fi->first, sumValue / sd::math::nd4j_sqrt<Nd4jLong, double>(
                                          fi->second.size()));
    }
  } else {
    auto restDims = ShapeUtils::evalDimsToExclude(input->rankOf(), {0});
  ...
  if (input->isVector()) {
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto e = start; e < stop; e++) {
        ...
        if (sd::math::nd4j_abs(tempRes.e<T>(classNum) - input->e<T>(e)) <=
            T(1.e-6))
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        for (Nd4jLong e = 0; e < current.lengthOf(); e++) {
          if (sd::math::nd4j_abs(listOfBPTensors.at(classNum).e<T>(e) -
                                 current.e<T>(e)) <= T(1.e-6))
...
int segmentMaxFunctorBP(sd::LaunchContext* context, NDArray* input,
                        NDArray* indices, NDArray* gradOut, NDArray* output) {
  BUILD_SINGLE_SELECTOR(output->dataType(), return segmentMaxFunctorBP_,
                        (context, input, indices, gradOut, output),
                        NUMERIC_TYPES);
}
BUILD_SINGLE_TEMPLATE(template int segmentMaxFunctorBP_,
...
int segmentMinFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    auto func = PRAGMA_THREADS_FOR {
      for (auto e = start; e < stop; e++) {
        auto classNum = indices->e<Nd4jLong>(e);
        if (sd::math::nd4j_abs(tempRes.e<double>(classNum) -
                               input->e<double>(e)) < 1.e-5)
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        for (Nd4jLong e = 0; e < current.lengthOf(); e++) {
          if (sd::math::nd4j_abs(listOfBPTensors.at(classNum).e<double>(e) -
                                 current.e<double>(e)) < 1.e-5)
...
int segmentMeanFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    ...
    int pos = 0;
    // auto func = [&](uint64_t thread_id, uint64_t start, uint64_t stop,
...
int segmentProdFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    for (Nd4jLong e = 0; e < indices->lengthOf(); ++e) {
      Nd4jLong classNum = indices->e<Nd4jLong>(e);
      output->p(e, gradOut->e<double>(classNum) * tempRes.e<double>(classNum) /
                       input->e<double>(e));
    }
  } else {
    auto restDims = ShapeUtils::evalDimsToExclude(input->rankOf(), {0});
...
int unsortedSegmentMaxFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(
      output->dataType(), return unsortedSegmentMaxFunctorBP_,
      (context, input, indices, gradOut, numOfClasses, output), NUMERIC_TYPES);
}
BUILD_SINGLE_TEMPLATE(template int unsortedSegmentMaxFunctorBP_,
                      (sd::LaunchContext * context, NDArray* input,
                       NDArray* indices, NDArray* gradOut,
                       Nd4jLong numOfClasses, NDArray* output),
                      NUMERIC_TYPES);
...
int unsortedSegmentMinFunctorBP(sd::LaunchContext* context, NDArray* input,
                                ...
                                Nd4jLong numOfClasses, NDArray* output) {
  BUILD_SINGLE_SELECTOR(
      output->dataType(), return unsortedSegmentMinFunctorBP_,
      (context, input, indices, gradOut, numOfClasses, output), NUMERIC_TYPES);
}
BUILD_SINGLE_TEMPLATE(template int unsortedSegmentMinFunctorBP_,
                      (sd::LaunchContext * context, NDArray* input,
                       NDArray* indices, NDArray* gradOut,
                       Nd4jLong numOfClasses, NDArray* output),
                      NUMERIC_TYPES);
...
int unsortedSegmentProdFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    auto func = PRAGMA_THREADS_FOR {
      for (auto e = start; e < stop; e++) {
        auto classNum = indices->e<Nd4jLong>(e);
        output->p<double>(e, gradOut->e<double>(classNum) *
                                 tempRes.e<double>(classNum) /
                                 input->e<double>(e));
      }
    };
...
int unsortedSegmentSqrtNFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    ...
    for (Nd4jLong e = 0; e < indices->lengthOf(); e++) {
      auto classNum = indices->e<Nd4jLong>(e);
      output->p(e,
                gradOut->e<double>(classNum) /
                    sd::math::nd4j_sqrt<double, double>(classCount[classNum]));
    }
    //};
