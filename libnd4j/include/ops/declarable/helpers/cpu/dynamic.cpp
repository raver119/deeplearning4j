...
template <typename T>
static void _dynamicPartitionFunctor(NDArray const* input,
...
template <typename T>
static int _dynamicStitchFunctor(std::vector<NDArray*> const& inputs,
  ...
  if (output->isVector()) {
    for (int e = 0; e < numOfData; e++) {
      ...
      for (Nd4jLong i = 0; i < index->lengthOf(); i++) {
        ...
        if (pos < 0) {
          nd4j_printf(
              "dynamic_stitch: Index value should be non-negative. But %i was "
              "given",
              pos);
...
static void _dynamicPartitionFunctorBP(
  ...
  if (sourceDimsLen) {  // multidimensional case
    ...
    ResultSet listOfTensors =
        outputList[0]->allTensorsAlongDimension(sourceDims);
...
void dynamicPartitionFunctor(sd::LaunchContext* context, NDArray const* input,
  ...
  BUILD_SINGLE_SELECTOR(xType, _dynamicPartitionFunctor,
                        (input, indices, outputList), LIBND4J_TYPES);
}
...
int dynamicStitchFunctor(sd::LaunchContext* context,
  ...
  BUILD_SINGLE_SELECTOR(xType, return _dynamicStitchFunctor,
                        (inputs, indices, output), LIBND4J_TYPES);
}
...
BUILD_SINGLE_TEMPLATE(template void _dynamicPartitionFunctorBP,
                      (NDArray const* input, NDArray const* indices,
                       ...
                       std::vector<NDArray*>& outputList);
                      , LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template int _dynamicStitchFunctorBP,
                      (std::vector<NDArray*> const& inputs,
                       std::vector<NDArray*> const& indices,
                       NDArray const* gradInput,
                       std::vector<NDArray*>& outputList);
...
BUILD_SINGLE_TEMPLATE(template void _dynamicPartitionFunctor,
                      (NDArray const* input, NDArray const* indices,
                       std::vector<NDArray*>& outputList);
                      , LIBND4J_TYPES);
BUILD_SINGLE_TEMPLATE(template int _dynamicStitchFunctor,
                      (std::vector<NDArray*> const& inputs,
