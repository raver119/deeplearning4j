...
///////////////////////////////////////////////////////////////////
template <typename T>
void static _softMaxDerivForVector(sd::LaunchContext* context,
                                   const void* input,
                                   const Nd4jLong* inShapeInfo, void* output) {
...
void softmaxDerivative(sd::LaunchContext* context, const NDArray& input,
  ...
  if (shape::isCommonVector(input.shapeInfo(), temp)) {
    BUILD_SINGLE_SELECTOR(
        input.dataType(), _softMaxDerivForVector,
        (context, input.buffer(), input.shapeInfo(), output.buffer()),
        FLOAT_TYPES);
  } else {
...
///////////////////////////////////////////////////////////////////
template <typename T>
void logSoftMaxForVector_(void const* input, Nd4jLong const* inShapeInfo,
                          void* output, Nd4jLong const* outShapeInfo) {
...
///////////////////////////////////////////////////////////////////
void logSoftMaxForVector(sd::LaunchContext* context, const NDArray& input,
                         NDArray& output) {
  ...
  BUILD_SINGLE_SELECTOR(
      xType, logSoftMaxForVector_,
      (input.buffer(), input.shapeInfo(), output.buffer(), output.shapeInfo()),
      FLOAT_TYPES);
}
...
void prelu(sd::LaunchContext* context, const NDArray& input,
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto i = start; i < stop; i++) {
      ...
      if (x < 0.0) {
        // FIXME: double
        output.p(i, (x * alpha.e<double>(shape::subArrayIndex(
                             i, inputShapeInfo, alphaShapeInfo))));
      } else
        output.p(i, x);
    }
  };
...
void preluBP(sd::LaunchContext* context, const NDArray& input,
  ...
  for (Nd4jLong i = 0; i < inputLen; ++i) {
    ...
    if (x < 0.0) {
      Nd4jLong alphaInd =
          shape::subArrayIndex(i, inputShapeInfo, alphaShapeInfo);
...
bool checkAlphaShapeLen(std::vector<Nd4jLong> const& expectedShape,
                        Nd4jLong shapeLen) {
  Nd4jLong expectedAlphaLen =
      std::accumulate(expectedShape.cbegin(), expectedShape.cend(), 1,
                      std::multiplies<Nd4jLong>());
  return expectedAlphaLen == shapeLen;
}
template <typename T>
static void thresholdRelu_(NDArray const& input, double threshold,
                           NDArray& output) {
...
template <typename T>
static void thresholdReluDerivative_(sd::LaunchContext* context, NDArray* input,
                                     double theta, NDArray* dLdO,
                                     NDArray* output) {
...
void thresholdReluDerivative(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), thresholdReluDerivative_,
                        (context, input, threshold, dLdO, output), FLOAT_TYPES);
}
  ...
  if (input.isVector()) {
    if (rank == 1 || input.sizeAt(dimension) != 1) {
      BUILD_SINGLE_SELECTOR(input.dataType(), logSoftMaxForVector_,
                            (input.buffer(), input.shapeInfo(), output.buffer(),
                             output.shapeInfo()),
                            FLOAT_TYPES);
    } else
      output = 0.;
...
BUILD_SINGLE_TEMPLATE(template void logSoftMaxForVector_,
                      (void const* input, Nd4jLong const* inShapeInfo,
                       void* output, Nd4jLong const* outShapeInfo),
                      FLOAT_TYPES);
BUILD_SINGLE_TEMPLATE(template void _softMaxDerivForVector,
                      (sd::LaunchContext * context, const void* input,
                       const Nd4jLong* inShapeInfo, void* output),
                      FLOAT_TYPES);
