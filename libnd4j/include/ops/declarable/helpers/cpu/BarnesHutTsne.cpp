...
static void barnes_symmetrize_(const NDArray* rowP, const NDArray* colP,
  ...
  // std::vector<int> symRowP =
  // rowCounts->asVectorT<int>();//NDArrayFactory::create<int>('c',
  // {numElements}); NDArray symValP = NDArrayFactory::create<double>('c',
  // {numElements}); symRowP.insert(symRowP.begin(),0); symRowP(1, {0}) =
  // *rowCounts;
  int const* pRows = reinterpret_cast<int const*>(rowP->buffer());
...
template <typename T>
static void barnes_gains_(NDArray* input, NDArray* gradX, NDArray* epsilon,
                          NDArray* output) {
  //        gains =
  ...
  auto gainsInternal = LAMBDA_TTT(x, grad, eps) {
    //            return T((x + 2.) * sd::math::nd4j_sign<T,T>(grad) !=
    //            sd::math::nd4j_sign<T,T>(eps)) + T(x * 0.8 *
    //            sd::math::nd4j_sign<T,T>(grad) !=
    //            sd::math::nd4j_sign<T,T>(eps));
    // return T((x + 2.) * sd::math::nd4j_sign<T,T>(grad) ==
    // sd::math::nd4j_sign<T,T>(eps)) + T(x * 0.8 *
    // sd::math::nd4j_sign<T,T>(grad) == sd::math::nd4j_sign<T,T>(eps));
...
void barnes_gains(NDArray* input, NDArray* gradX, NDArray* epsilon,
  ...
  //        auto leftPartBool =
  //        NDArrayFactory::create<bool>(leftPart.ordering(),
  //        leftPart.getShapeAsVector());
  //
  //        leftPart.applyPairwiseTransform(pairwise::NotEqualTo, &signEpsilon,
  //        &leftPartBool, nullptr); auto rightPart = *input * 0.8 * signGradX;
  //        auto rightPartBool =
  //        NDArrayFactory::create<bool>(rightPart.ordering(),
  //        rightPart.getShapeAsVector());
  //        rightPart.applyPairwiseTransform(pairwise::NotEqualTo, &signEpsilon,
  //        &rightPartBool, nullptr); leftPart.assign(leftPartBool);
