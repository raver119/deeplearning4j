...
void fillRandomGamma_(LaunchContext* context, graph::RandomGenerator& rng,
                      NDArray* alpha, NDArray* beta, NDArray* output) {
  auto broadcasted = alpha->shapeInfo();
  if (beta != nullptr) {
    const Nd4jLong* broadcastedShape = nullptr;
    ShapeUtils::evalBroadcastShapeInfo(*alpha, *beta, true, broadcastedShape,
  ...
  for (Nd4jLong k = 0; k < shift; k++) {
    ...
    for (Nd4jLong e = 0; e < step; e++)
      if (directOutput) {
        outputBuf[pos + e] = copyAlpha->t<T>(e) <= 1? gammaLess(rng, copyAlpha->t<T>(e), beta?copyBeta->t<T>(e):T(
            1.f)):gammaGreat(rng,copyAlpha->t<T>(e), beta ?copyBeta->t<T>(e):T(1.f));
      } else {
        output->r<T>(pos + e) = copyAlpha->t<T>(e) <= 1? gammaLess(rng, copyAlpha->t<T>(e), beta?copyBeta->t<T>(e):T(
            1.f)):gammaGreat(rng,copyAlpha->t<T>(e), beta ?copyBeta->t<T>(e):T(1.f));
      }
  }
...
/*
 * algorithm Poisson generator based upon the inversion by sequential
search:[48]:505 init: Let x ← 0, p ← e−λ, s ← p. Generate uniform random number
u in [0,1]. while u > s do: x ← x + 1. p ← p * λ / x. s ← s + p. return x.
 * */
template <typename T>
void fillRandomPoisson_(LaunchContext* context, graph::RandomGenerator& rng,
...
template <typename T>
void fillRandomUniform_(LaunchContext* context, graph::RandomGenerator& rng,
...
void fillRandomMultiNomial_(LaunchContext* context, graph::RandomGenerator& rng,
  ...
  auto func = PRAGMA_THREADS_FOR_2D {
    for (auto nBatchIndex = start_x; nBatchIndex < stop_x;
         nBatchIndex += inc_x) {
      for (auto nSampleIndexInBatch = start_y; nSampleIndexInBatch < stop_y;
           nSampleIndexInBatch += inc_y) {
        ...
        for (Nd4jLong nClass = 0; nClass < numOfClassX; nClass += 1) {
          ...
          auto unifornLog =
              sd::math::nd4j_log<Tx, Tx>(-sd::math::nd4j_log<Tx, Tx>(
                  rng.relativeT<Tx>(nIndex, minVal, maxVal)));
...
void fillRandomMultiNomial(LaunchContext* context, graph::RandomGenerator& rng,
  ...
  BUILD_DOUBLE_SELECTOR(input.dataType(), output.dataType(),
                        fillRandomMultiNomial_,
                        (context, rng, input, output, numOfSamples, dimC),
