...
static void gatherND_(NDArray& input, NDArray& indices, NDArray& output) {
  ...
  const int maxRank =
      sd::math::nd4j_max<int>(yRank, sd::math::nd4j_max<int>(xRank, zRank));
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    for (auto i = start; i < stop; i++) {
      ...
      for (uint j = 0; j < yLastDim; ++j)
        xCoords[j] =
            y[yOffset + j * indices.stridesOf()[yRank - 1]];  // last stride
...
void gatherND(sd::LaunchContext* context, NDArray& input, NDArray& indices,
              NDArray& output) {
  BUILD_DOUBLE_SELECTOR(input.dataType(), indices.dataType(), gatherND_,
...
////////////////////////////////////////////////////////////////////////
template <typename T>
static void gather_(NDArray* input, const NDArray* indices, NDArray* output,
  ...
  if (indices != nullptr) {
    for (Nd4jLong i = 0; i < indices->lengthOf(); ++i)
      if (indices->e<Nd4jLong>(i) >= input->sizeAt(axis))
        throw std::runtime_error(
            "helpers::gather function: indices array contains wrong elements, "
            "each element must be smaller than corresponding dimension of "
            "input array !");
    ...
    if (indices->isScalar()) {
      if (input->rankOf() <= 1) {
        ...
        auto tadArr =
            NDArray(reinterpret_cast<void*>(
                        reinterpret_cast<T*>(input->buffer()) +
                        tadPack.primaryOffsets()[indices->e<Nd4jLong>(0)]),
      ...
      std::iota(dimsOut.begin(), dimsOut.end(),
                axis);  // fill with axis, axis+1, ... indices->rankOf()-1
      const Nd4jLong numOfSubArrs =
          ShapeUtils::getNumOfSubArrs(output->shapeInfo(), dimsOut);
    ...
    for (int i = 1; i < numOfIntArgs; ++i)
      if (intArgs[i] >= input->sizeAt(axis))
        throw std::runtime_error(
            "helpers::gather function: some of input indexes is larger than "
            "corresponding shape of input array !");
    ...
    if (numOfIntArgs == 2) {  // scalar case
      ...
      const Nd4jLong numOfSubArrs =
          ShapeUtils::getNumOfSubArrs(output->shapeInfo(), {axis});
...
void gather(NDArray* input, const NDArray* indices, NDArray* output,
            const std::vector<int>& intArgs) {
  BUILD_SINGLE_SELECTOR(input->dataType(), gather_,
                        (input, indices, output, intArgs), LIBND4J_TYPES);
}
