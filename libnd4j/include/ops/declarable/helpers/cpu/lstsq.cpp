...
int leastSquaresSolveFunctor_(sd::LaunchContext* context,
  ...
  if (fast) {  // Cholesky decomposition approach
    ...
    // 3. due l2Regularizer = 0, skip regularization ( indeed A' = A2 -
    // l2Regularizer * I)
  ...
  https:  // mangapark.net/
    ...
    auto err = helpers::cholesky(context, &leftOutput, &leftOutput,
                                 true);  // inplace decomposition
    if (err) return err;
    // alternate moment: inverse lower triangular matrix to solve equation A'x =
    // b' => L^Tx = L^-1 * b' solve one upper triangular system (to avoid float
    // problems)
    ...
    helpers::triangularSolveFunctor(context, &leftOutput, &rightOutput, true,
                                    false, &rightB);
    helpers::adjointMatrix(context, &leftOutput, true,
