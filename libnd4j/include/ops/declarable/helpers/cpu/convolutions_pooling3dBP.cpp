...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void pooling3dBP_(sd::graph::Context& block, const NDArray& input,
  ...
  const bool sameStrides = iStride0 == gIStride0 && iStride1 == gIStride1 &&
                           iStride2 == gIStride2 && iStride3 == gIStride3 &&
                           iStride4 == gIStride4;
            ...
            for (int oh = 0; oh < oH; ++oh) {
              for (int ow = 0; ow < oW; ++ow) {
                ...
                if (sameStrides) {
                  ...
                  for (Nd4jLong kd = dstart; kd < dend; kd += dD)
                    for (Nd4jLong kh = hstart; kh < hend; kh += dH)
                      for (Nd4jLong kw = wstart; kw < wend; kw += dW) {
                        T valIn =
                            pIn[kd * iStride2 + kh * iStride3 + kw * iStride4];
                        if (valIn > sum) {
                          sum = valIn;
                          maxKD = kd;
                          maxKH = kh;
                  ...
                  gI[b * gIStride0 + c * gIStride1 + maxKD * gIStride2 +
                     maxKH * gIStride3 + maxKW * gIStride4] += valO;
                }
              }
            }
          }
            ...
            for (int oh = 0; oh < oH; ++oh) {
              for (int ow = 0; ow < oW; ++ow) {
                ...
                if (extraParam0 == 0)  // Exclude padding
                  valO /= sd::math::nd4j_ceil<double, T>(
                              static_cast<double>(dend - dstart) /
                              static_cast<double>(gIStep2)) *
                          sd::math::nd4j_ceil<double, T>(
                              static_cast<double>(hend - hstart) /
                              static_cast<double>(gIStep3)) *
                          sd::math::nd4j_ceil<double, T>(
                              static_cast<double>(wend - wstart) /
                              static_cast<double>(
                                  gIStep4));  // Accounts for dilation
  ...
  /*************************************************************************/
                    ...
                    for (Nd4jLong kh = hstart; kh < hend; kh += iStep3)
                      for (Nd4jLong kw = wstart; kw < wend; kw += iStep4)
                        sum += sd::math::nd4j_pow<T, T, T>(
                            sd::math::nd4j_abs<T>(pIn[kd + kh + kw]),
                            extraParam0);

                  valO *= sd::math::nd4j_pow<T, T, T>(
                      sum, ((T)1.f - extraParam0) / extraParam0);
                      ...
                      for (Nd4jLong kw = wstart; kw < wend; kw += iStep4)
                        pgI[kd + kh + kw] +=
                            ...
                            sd::math::nd4j_pow<T, T, T>(
                                sd::math::nd4j_abs<T>(pIn[kd + kh + kw]),
                                extraParam0 - (T)1.f) *
                            sd::math::nd4j_sgn<T, T>(pIn[kd + kh + kw]);
                } else {
                  for (Nd4jLong kd = dstart; kd < dend; kd += dD)
                    for (Nd4jLong kh = hstart; kh < hend; kh += dH)
                      for (Nd4jLong kw = wstart; kw < wend; kw += dW)
                    ...
                    for (Nd4jLong kh = hstart; kh < hend; kh += dH)
                      for (Nd4jLong kw = wstart; kw < wend; kw += dW) {
                        ...
                        pgI[kd * gIStride2 + kh * gIStride3 + kw * gIStride4] +=
                            valO *
                            sd::math::nd4j_pow<T, T, T>(
                                sd::math::nd4j_abs<T>(inVal),
                                extraParam0 - 1.f) *
                            sd::math::nd4j_sgn<T, T>(inVal);
                      }
                }
              }
            }
    ...
    nd4j_printf(
        "ConvolutionUtils::pooling3dBP: pooling mode argument can take three "
        "values only: 0, 1, 2, but got %i instead !\n",
        poolingMode);
    throw "";
  }
}
...
void ConvolutionUtils::pooling3dBP(sd::graph::Context& block,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), pooling3dBP_,
                        (block, input, gradO, gradI, kD, kH, kW, sD, sH, sW, pD,
                         pH, pW, dD, dH, dW, poolingMode, extraParam0),
                        FLOAT_TYPES);
}
}  // namespace ops
}  // namespace sd
