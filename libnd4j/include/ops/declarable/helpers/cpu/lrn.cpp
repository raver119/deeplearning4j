...
template <typename T>
static int lrnFunctor_(sd::graph::Context& block, NDArray* input,
                       NDArray* output, int depth, float bias, float alpha,
                       float beta) {
  ...
  const Nd4jLong inTadEws =
      shape::elementWiseStride(inTadPack.primaryShapeInfo());
  const Nd4jLong outTadEws =
      shape::elementWiseStride(outTadPack.primaryShapeInfo());
  ...
  if (inTadEws == 1 && outTadEws == 1) {
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        // calculate squared sum of elements per each j-th element range [j -
        // depth, j + depth + 1] we store each squared sum in corresponding
        // element of y array
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          ...
          if (j == 0) {
            for (uint s = begin; s < end; ++s) prev = prev + x[s] * x[s];
            y[j] = prev;
          ...
          y[j] =
              x[j] / sd::math::nd4j_pow<T, T, T>(tbias + alpha * prev, tbeta);
        }
      }
    };
      ...
      for (Nd4jLong i = 0; i < numOfTads; ++i) {
        ...
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          ...
          if (j == 0) {
            ...
            y[j * outTadEws] =
                prev + x[(end - 1) * inTadEws] * x[(end - 1) * inTadEws];
          else if (begin > 0 && last <= tadLen)
            y[j * outTadEws] =
                prev + x[(end - 1) * inTadEws] * x[(end - 1) * inTadEws] -
            ...
            y[j * outTadEws] =
                prev - x[(begin - 1) * inTadEws] * x[(begin - 1) * inTadEws];
          else
            y[j * outTadEws] = prev;

          if (j != 0) prev = y[j * outTadEws];

          y[j * outTadEws] = x[j * inTadEws] / sd::math::nd4j_pow<T, T, T>(
                                                   tbias + alpha * prev, tbeta);
        }
      }
    };
...
BUILD_SINGLE_TEMPLATE(template int lrnFunctor_,
                      (sd::graph::Context & block, NDArray* input,
                       NDArray* output, int depth, float bias, float alpha,
                       float beta),
                      FLOAT_TYPES);

int lrnFunctor(sd::graph::Context& block, NDArray* input, NDArray* output,
               int depth, double bias, double alpha, double beta) {
  BUILD_SINGLE_SELECTOR(input->dataType(), return lrnFunctor_,
                        (block, input, output, depth, bias, alpha, beta),
                        FLOAT_TYPES);
}

//////////////////////////////////////////////////////////////////////////
template <typename X, typename Y>
static void lrnBP_(const NDArray& input, const NDArray& gradO, NDArray& gradI,
                   const int depth, const float bias, const float alpha,
                   const float beta) {
  const int rank = input.rankOf();
  ...
  const Nd4jLong inTadEws =
      shape::elementWiseStride(inTadPack.primaryShapeInfo());
  const Nd4jLong gradITadEws =
      shape::elementWiseStride(gradITadPack.primaryShapeInfo());
  ...
  if (inTadEws == 1 && gradITadEws == 1) {
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        // this loop calculates squared sum of elements per each j-th element
        // range [j - depth, j + depth + 1] we store each squared sum in
        // corresponding element of y array
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          const uint begin = sd::math::nd4j_max<int>(0, j - depth);
          const uint last = depth + j + 1;
          const uint end = sd::math::nd4j_min<int>(last, tadLen);
          ...
          if (j == 0) {
            y[0] = 0;
            for (uint s = begin; s < end; ++s) y[0] = y[0] + x[s] * x[s];
          } else if (begin == 0 && last <= tadLen)
        ...
        Y prev = 0;
        // second loop calculates derivatives using information gained in first
        // loop above
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          const uint begin = sd::math::nd4j_max<int>(0, j - depth);
          const uint last = depth + j + 1;
          ...
          if (j == 0) {
            for (uint s = begin; s < end; ++s) {
              factor[s] = sd::math::nd4j_pow<Y, Y, Y>(tbias + talpha * y[s],
                                                      -tbeta - 1);
              prev = prev + x[s] * factor[s];
            }
            y[0] = prev;
          } else if (begin == 0 && last <= tadLen) {
            factor[end - 1] = sd::math::nd4j_pow<Y, Y, Y>(
                tbias + talpha * y[end - 1], -tbeta - 1);
            y[j] = prev + x[end - 1] * factor[end - 1];
          } else if (begin > 0 && last <= tadLen) {
            factor[end - 1] = sd::math::nd4j_pow<Y, Y, Y>(
                tbias + talpha * y[end - 1], -tbeta - 1);
            y[j] = prev + x[end - 1] * factor[end - 1] -
                   x[begin - 1] * factor[begin - 1];
          } else if (begin > 0 && last > tadLen)
      ...
      for (auto i = start; i < stop; i++) {
        ...
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          ...
          if (j == 0) {
            ...
            y[j * gradITadEws] =
                y[(j - 1) * gradITadEws] +
                x[(end - 1) * inTadEws] * x[(end - 1) * inTadEws];
          else if (begin > 0 && last <= tadLen)
            y[j * gradITadEws] =
                y[(j - 1) * gradITadEws] +
                x[(end - 1) * inTadEws] * x[(end - 1) * inTadEws] -
        ...
        for (Nd4jLong j = 0; j < tadLen; ++j) {
          ...
          if (j == 0) {
            for (uint s = begin; s < end; ++s) {
              factor[s] = sd::math::nd4j_pow<Y, Y, Y>(
                  tbias + talpha * y[s * gradITadEws], -tbeta - 1);
              prev = prev + x[s * inTadEws] * factor[s];
            }
            y[0] = prev;
          } else if (begin == 0 && last <= tadLen) {
            factor[end - 1] = sd::math::nd4j_pow<Y, Y, Y>(
                tbias + talpha * y[(end - 1) * gradITadEws], -tbeta - 1);
            y[j * gradITadEws] =
                prev + x[(end - 1) * inTadEws] * factor[end - 1];
          } else if (begin > 0 && last <= tadLen) {
            factor[end - 1] = sd::math::nd4j_pow<Y, Y, Y>(
                tbias + talpha * y[(end - 1) * gradITadEws], -tbeta - 1);
            y[j * gradITadEws] = prev +
                                 x[(end - 1) * inTadEws] * factor[end - 1] -
                                 x[(begin - 1) * inTadEws] * factor[begin - 1];
          } else if (begin > 0 && last > tadLen)
            y[j * gradITadEws] =
                prev - x[(begin - 1) * inTadEws] * factor[begin - 1];
          else
            y[j * gradITadEws] = prev;
          ...
          y[j * gradITadEws] =
              factor[j] * init - 2 * x[j * inTadEws] * coeff * prev;
        }
...
void lrnBP(sd::graph::Context& block, const NDArray& input,
  ...
  BUILD_DOUBLE_SELECTOR(input.dataType(), gradO.dataType(), lrnBP_,
                        (input, gradO, gradI, depth, bias, alpha, beta),
