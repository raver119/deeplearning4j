...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void pooling2d_(sd::graph::Context& block, const NDArray& input,
        ...
        for (int c = start_y; c < stop_y; c += inc_y) {
          for (int oh = 0; oh < oH; ++oh) {
            for (int ow = 0; ow < oW; ++ow) {
              ...
              out[b * oStride0 + c * oStride1 + oh * oStride2 + ow * oStride3] =
                  max;
            }
          }
        }
          ...
          for (int oh = 0; oh < oH; ++oh) {
            for (int ow = 0; ow < oW; ++ow) {
              ...
              if (extraParam0 == 0) {  // Exclude padding
                int a = (hend - hstart) / iStep2 +
                        ((hend - hstart) % iStep2 == 0 ? 0 : 1);
                int r = (wend - wstart) / iStep3 +
                        ((wend - wstart) % iStep3 == 0 ? 0 : 1);
  ...
  /*************************************************************************/
        ...
        for (int c = start_y; c < stop_y; c += inc_y) {
          for (int oh = 0; oh < oH; ++oh) {
            for (int ow = 0; ow < oW; ++ow) {
              ...
              sum = sd::math::nd4j_pow<T, T, T>(
                  sum, static_cast<T>((T)1.f) / extraParam0);
    ...
    nd4j_printf(
        "ConvolutionUtils::pooling2d: pooling mode argument can take three "
...
void ConvolutionUtils::pooling2d(sd::graph::Context& block,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), pooling2d_,
                        (block, input, output, kH, kW, sH, sW, pH, pW, dH, dW,
