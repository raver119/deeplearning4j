...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void _percentile(const NDArray& input, NDArray& output,
                        std::vector<int>& axises, const float q,
  ...
  if (axises.empty())
    ...
    shape::checkDimensions(inputRank,
                           axises);  // check, sort dimensions and remove
  ...
  auto flattenedArr = NDArrayFactory::create(
      'c', shapeOfSubArr, input.dataType(), input.getContext());
  ...
  switch (interpolation) {
    case 0:  // lower
      position = static_cast<Nd4jLong>(
          math::nd4j_ceil<float, T>((len - 1) * fraction));
      break;
    case 1:  // higher
      position = static_cast<Nd4jLong>(
          math::nd4j_floor<float, T>((len - 1) * fraction));
      break;
    case 2:  // nearest
      position = static_cast<Nd4jLong>(
          math::nd4j_round<float, T>((len - 1) * fraction));
      break;
  }
  position = len - position - 1;

  // FIXME: our sort impl should be used instead, so this operation might be
...
void percentile(sd::LaunchContext* context, const NDArray& input,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), _percentile,
                        (input, output, axises, q, interpolation),
                        LIBND4J_TYPES);
}
