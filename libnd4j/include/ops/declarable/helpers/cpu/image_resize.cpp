...
inline void computeInterpolationWeights(
    const Scaler scaler, Nd4jLong outSize, Nd4jLong inSize, double scale,
 ...
 */
//    static void
//    resizeImage(NDArray const *images, Nd4jLong batchSize, Nd4jLong inHeight,
...
static void resizeImage_(T const* pInputBuf, Nd4jLong batchSize,
                         Nd4jLong inHeight, Nd4jLong inWidth,
  ...
  //        T const *pInputBuf = images->getDataBuffer()->primaryAsT<T>(); //
  //        this works only with 'c' direction
  BilinearInterpolationData const* xsPtr = xs.data();

  //        T* pOutputBuf = output->dataBuffer()->primaryAsT<T>();
  auto computeBilinear = [](double topLeft, double topRight, double bottomLeft,
                            double bottomRight, double xVal, double yVal) {
      ...
      for (Nd4jLong y = 0; y < outHeight; ++y) {
        ...
        for (Nd4jLong x = 0; x < outWidth; ++x) {
          ...
          for (Nd4jLong c = 0; c < channels; ++c) {
            ...
            pOutput[x * channels + c] = computeBilinear(
                topLeft, topRight, bottomLeft, bottomRight, xVal, yVal);
          }
        }
      }
    }
...
static int resizeBilinearFunctor_(NDArray const* images, int const width,
  ...
  if (halfPixelCenter) {
    computeInterpolationWeights(HalfPixelScaler(), outHeight, inHeight,
                                st.heightScale, ys.data());
    computeInterpolationWeights(HalfPixelScaler(), outWidth, inWidth,
                                st.widthScale, xs.data());

  } else {
    // Compute the cached interpolation weights on the x and y dimensions.
    computeInterpolationWeights(LegacyScaler(), outHeight, inHeight,
                                st.heightScale, ys.data());
    computeInterpolationWeights(LegacyScaler(), outWidth, inWidth,
                                st.widthScale, xs.data());
  }
  int xsSize = xs.size();
  ...
  resizeImage_<X, Z>(images->getDataBuffer()->primaryAsT<X>(), batchSize,
                     inHeight, inWidth, outHeight, outWidth, channels, xs, ys,
                     output->dataBuffer()->primaryAsT<Z>());
  return Status::OK();
}

template <class Scaler, typename T>
void resizeNeighbor(ImageResizerState const& st, NDArray const* images,
  ...
  auto func = PRAGMA_THREADS_FOR_2D {
    for (auto b = start_x; b < stop_x; b += inc_x) {
      for (auto y = start_y; y < stop_y; y += inc_y) {
        auto posY =
            alignCorners
                ? static_cast<Nd4jLong>(
                      sd::math::p_round<float>(scaler(y, st.heightScale)))
                : static_cast<Nd4jLong>(
                      sd::math::p_floor<float>(scaler(y, st.heightScale)));
        ...
        for (Nd4jLong x = 0; x < outWidth; ++x) {
          auto posX =
              alignCorners
                  ? static_cast<Nd4jLong>(
                        sd::math::p_round<float>(scaler(x, st.widthScale)))
                  : static_cast<Nd4jLong>(
                        sd::math::p_floor<float>(scaler(x, st.widthScale)));
...
template <typename T>
int resizeNeighborFunctor_(NDArray const* images, int const width,
                           int const height, bool const alignCorners,
                           bool const halfPixelCenter, NDArray* output) {
  ImageResizerState st(alignCorners, halfPixelCenter);
  st.validateAndCalculateOutputSize(images, width, height);
  ...
  // Handle no-op resizes efficiently.
  if (output->sizeAt(1) == images->sizeAt(1) &&
      output->sizeAt(2) == images->sizeAt(2)) {
    output->assign(images);
    return Status::OK();
  }

  if (halfPixelCenter)
    resizeNeighbor<HalfPixelScalerNN, T>(st, images, alignCorners, true,
                                         output);
  else
...
//    void resizeImage(NDArray const *images, Nd4jLong batchSize, Nd4jLong
//    inHeight, Nd4jLong inWidth, Nd4jLong outHeight,
//                     Nd4jLong outWidth, Nd4jLong channels,
//                     std::vector<BilinearInterpolationData> const &xs,
//                     std::vector<BilinearInterpolationData> const &ys,
//                     NDArray *output) {
...
//        resizeImage_,
//                              (images, batchSize, inHeight, inWidth,
//                              outHeight, outWidth, channels, xs, ys, output),
...
int resizeBilinearFunctor(sd::LaunchContext* context, NDArray const* images,
  ...
  BUILD_DOUBLE_SELECTOR(
      ...
      (images, width, height, alignCorners, halfPixelCenter, output),
      NUMERIC_TYPES, FLOAT_TYPES);
  return Status::OK();
}
...
static float compute(float values[4], const float xW0, const float xW1,
...
inline void getWeightsAndIndices(const float scale, const Nd4jLong out_loc,
                                 const Nd4jLong limit, WeightsAndIndices* out) {
  const Scaler scaler;
  ...
  if (half_pixel_centers) {
    auto func = PRAGMA_THREADS_FOR {
      for (auto x = start; x < stop; ++x) {
        ...
        auto& x_wai = (*x_wais)[x];
        x_wai._advance = calc.Advance(x_wai._index0, x_wai._index1,
                                      x_wai._index2, x_wai._index3);
      }
    };
...
template <typename T>
static FORCEINLINE float computeYInterpolation(int which, int channelNum,
                                               const WeightsAndIndices& yWai,
                                               const T* pY0, const T* pY1,
                                               const T* pY2, const T* pY3,
...
static void bicubicInterpolateWithCaching(NDArray const* image,
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    F* pOutputY =
        output->dataBuffer()->primaryAsT<F>();  // output is float anyway
...
int resizeBicubicFunctorA_(sd::LaunchContext* context, NDArray const* image,
                           int const width, int const height,
                           bool const alignCorners, bool const halfPixelAlign,
                           NDArray* output) {
  ImageResizerState st(alignCorners,
                       halfPixelAlign);  // align_corners, half_pixel_align
...
int resizeBicubicFunctorA(sd::LaunchContext* context, NDArray const* image,
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (context, image, width, height, alignCorners, halfPixelAlign, output),
      NUMERIC_TYPES);
}
// ------------------------------------------------------------------------------------------------------------------
// //
...
template <typename T>
static void computePatchSumOf3Channels(float scale, ImageResizerState const& st,
...
template <typename T>
static void resizeArea(ImageResizerState const& st,
                       std::vector<CachedInterpolation> const& caches,
                       NDArray const* input, NDArray* output) {
  T const* inputPtr = input->bufferAsT<T>();
  float scale = 1.f / (st.heightScale * st.widthScale);
  auto outputPtr =
      output->bufferAsT<float>();  // output is always float. TO DO: provide
    ...
    for (auto batch = start; batch < stop; batch++) {
      for (auto y = 0; y < st.outHeight; ++y) {
        ...
        for (auto i = yStart, k = 0LL; i < yEnd; ++i, ++k) {
          ...
          scaleCache.yPtr =
              inputPtr + (batch * st.inHeight * st.inWidth * st.channels +
                          bound(i, st.inHeight) * st.inWidth * st.channels);
          yCaches[k] = scaleCache;
        }
        float* output =
            outputPtr + (batch * st.outHeight + y) * st.channels * st.outWidth;
  ...
  if (Status::OK() == res) {
    ...
    auto cachingProcedure = PRAGMA_THREADS_FOR {
      for (auto x = start; x < stop; x++) {
        ...
        xCache.start = v;
        xCache.startScale = v < inX
                                ? (v + 1 > inX1 ? st.widthScale : v + 1 - inX)
                                : (v + 1 > inX1 ? inX1 - v : 1.f);
        v = math::nd4j_ceil<float, Nd4jLong>(inX1);
        xCache.end = v--;
        xCache.endMinusOneScale =
            v < inX ? (v + 1 > inX1 ? st.widthScale : v + 1 - inX)
                    : (v + 1 > inX1 ? inX1 - v : 1.f);
        xCache.needsBounding =
            bound(xCache.start, st.inWidth) != xCache.start ||
...
// ------------------------------------------------------------------------------------------------------------------
// //
int resizeFunctor(sd::LaunchContext* context, NDArray const* image,
                  int const width, int const height, ImageResizeMethods method,
  ...
  switch (method) {
    ...
    case kResizeBicubic:
      ...
      break;
    case kResizeArea:
      return resizeAreaFunctor(context, image, width, height,
