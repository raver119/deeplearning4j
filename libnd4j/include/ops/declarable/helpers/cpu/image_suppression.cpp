...
template <typename T>
static void nonMaxSuppressionV2_(NDArray* boxes, NDArray* scales, int maxSize,
  ...
  std::sort(indices.begin(), indices.end(), [scales](int i, int j) {
    return i >= 0 && j >= 0 ? scales->e<T>(i) > scales->e<T>(j) : (i > j);
  });

  //        std::vector<int> selected(output->lengthOf());
  std::vector<int> selectedIndices(output->lengthOf(), 0);
  auto needToSuppressWithThreshold = [](NDArray& boxes, int previousIndex,
                                        int nextIndex, T threshold) -> bool {
    if (previousIndex < 0 || nextIndex < 0) return true;
    T minYPrev = sd::math::nd4j_min(boxes.t<T>(previousIndex, 0),
                                    boxes.t<T>(previousIndex, 2));
    T minXPrev = sd::math::nd4j_min(boxes.t<T>(previousIndex, 1),
                                    boxes.t<T>(previousIndex, 3));
    T maxYPrev = sd::math::nd4j_max(boxes.t<T>(previousIndex, 0),
                                    boxes.t<T>(previousIndex, 2));
    T maxXPrev = sd::math::nd4j_max(boxes.t<T>(previousIndex, 1),
                                    boxes.t<T>(previousIndex, 3));
    T minYNext =
        sd::math::nd4j_min(boxes.t<T>(nextIndex, 0), boxes.t<T>(nextIndex, 2));
    T minXNext =
        sd::math::nd4j_min(boxes.t<T>(nextIndex, 1), boxes.t<T>(nextIndex, 3));
    T maxYNext =
        sd::math::nd4j_max(boxes.t<T>(nextIndex, 0), boxes.t<T>(nextIndex, 2));
    T maxXNext =
        sd::math::nd4j_max(boxes.t<T>(nextIndex, 1), boxes.t<T>(nextIndex, 3));
  ...
  for (int i = 0; i < numBoxes; ++i) {
    ...
    for (int j = numSelected - 1; j >= 0; --j) {
      if (shouldSelect)
        if (needToSuppressWithThreshold(*boxes, indices[i],
                                        indices[selectedIndices[j]],
                                        T(overlapThreshold))) {
          shouldSelect = false;
        }
    }
...
static inline T similirityV3_(NDArray const& boxes, Nd4jLong i, Nd4jLong j) {
  ...
  const T intersectionArea =
      math::nd4j_max(intersectionY, zero) * math::nd4j_max(intersectionX, zero);
  return intersectionArea / (areaI + areaJ - intersectionArea);
}

template <typename T>
static inline T similiratyOverlaps_(NDArray const& boxes, Nd4jLong i,
                                    Nd4jLong j) {
...
static NDArray similiratyOverlaps(NDArray const& boxes, Nd4jLong i,
                                  Nd4jLong j) {
  NDArray res(boxes.dataType(),
              boxes.getContext());  // = NDArrayFactory::create(0.);
  BUILD_SINGLE_SELECTOR(boxes.dataType(), res = similiratyOverlaps_,
                        (boxes, i, j), FLOAT_TYPES);
  return res;
}
...
static NDArray similiratyV3(NDArray const& boxes, Nd4jLong i, Nd4jLong j) {
  NDArray res(boxes.dataType(),
              boxes.getContext());  // = NDArrayFactory::create(0.);
  BUILD_SINGLE_SELECTOR(boxes.dataType(), res = similirityV3_, (boxes, i, j),
                        FLOAT_TYPES);
  return res;
}
...
static Nd4jLong nonMaxSuppressionGeneric_(sd::LaunchContext* context,
                                          ...
                                          float scoreThreshold, NDArray* output,
                                          SimiliratyFunc f) {
  ...
  while (selected.size() < outputSize && !candidatePriorityQueue.empty()) {
    ...
    for (int j = static_cast<int>(selected.size()) - 1;
         j >= nextCandidate._suppressBeginIndex; --j) {
      auto similarityA =
          f(*boxes, nextCandidate._boxIndex,
            selected[j]);  // boxes->t<T>(nextCandidate._boxIndex, selected[j]);
      similarity = similarityA.template t<T>(0);
      nextCandidate._score *= T(similarity <= overlapThreshold
                                    ? 1.0
                                    : 0.);  // suppressWeightFunc(similarity);
  ...
  if (output) {
    DataBuffer buf(selected.data(), selected.size() * sizeof(I),
...
Nd4jLong nonMaxSuppressionGeneric(sd::LaunchContext* context, NDArray* boxes,
  ...
  BUILD_DOUBLE_SELECTOR(
      boxes->dataType(),
      output == nullptr ? DataType::INT32 : output->dataType(),
      return nonMaxSuppressionGeneric_,
      (context, boxes, scores, maxSize, overlapThreshold, scoreThreshold,
       output, similiratyOverlaps),
...
BUILD_DOUBLE_TEMPLATE(template Nd4jLong nonMaxSuppressionGeneric_,
                      (sd::LaunchContext * context, NDArray* boxes,
                       NDArray* scores, int maxSize, float overlapThreshold,
                       float scoreThreshold, NDArray* output,
                       SimiliratyFunc similiratyFunc),
                      FLOAT_TYPES, INTEGER_TYPES);
...
void nonMaxSuppression(sd::LaunchContext* context, NDArray* boxes,
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (boxes, scales, maxSize, overlapThreshold, scoreThreshold, output),
      NUMERIC_TYPES);
}
BUILD_SINGLE_TEMPLATE(template void nonMaxSuppressionV2_,
                      (NDArray * boxes, NDArray* scales, int maxSize,
                       double overlapThreshold, double scoreThreshold,
                       NDArray* output),
                      NUMERIC_TYPES);
