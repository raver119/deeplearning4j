...
template <typename T>
static void adjustSaturation_(const NDArray *input,
...
void adjustSaturation(sd::LaunchContext *context, const NDArray *input,
                      const NDArray *factorScalarArr, NDArray *output,
                      const int dimC) {
  BUILD_SINGLE_SELECTOR(input->dataType(), adjustSaturation_,
                        (input, factorScalarArr, output, dimC), FLOAT_TYPES);
}

/*
template <typename T>
static void adjust_saturation_single_(sd::LaunchContext * context, NDArray
*array, NDArray *output, float delta, bool isNHWC) {
    ...
    if (isNHWC) {
        ...
        for (int e = 0; e < tuples; e++) {
            ...
            s = sd::math::nd4j_min<T>((T) 1.0f, sd::math::nd4j_max<T>((T) 0.0f,
s * delta));
...
*output, NDArray* delta, bool isNHWC) { auto xType = array->dataType();
    ...
    if (array->rankOf() == 4) {
        ...
        for (int e = 0; e < tSize; e++) {
            BUILD_SINGLE_SELECTOR(xType, adjust_saturation_single_, (context,
tadsIn->at(e), tadsOut->at(e), d, isNHWC);, FLOAT_TYPES);
        }
        ...
        BUILD_SINGLE_SELECTOR(xType, adjust_saturation_single_, (context, array,
output, d, isNHWC);, FLOAT_TYPES);
    }
}

BUILD_SINGLE_TEMPLATE(template void adjust_saturation_single_,
(sd::LaunchContext * context, NDArray *array, NDArray *output, float delta, bool
isNHWC), FLOAT_TYPES);
*/
