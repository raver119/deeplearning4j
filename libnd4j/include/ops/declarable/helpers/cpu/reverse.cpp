...
template <typename T>
static void reverseArray(sd::LaunchContext* context, void const* vinArr,
                         Nd4jLong const* inShapeBuffer, void* voutArr,
                         Nd4jLong const* outShapeBuffer,
                         int numOfElemsToReverse = 0) {
  ...
  if (inArr == outArr) {
    ...
    if (inEWS == 1 && outEWS == 1 && inOrder == outOrder) {
      auto func = PRAGMA_THREADS_FOR {
        for (Nd4jLong e = start; e < stop; e++) outArr[sLength - e] = inArr[e];
      };
...
static void reverseSequence_(sd::LaunchContext* context, const NDArray* input,
  ...
  if (input->isVector() ||
    ...
    if ((seqDim == 0 && input->sizeAt(0) == 1) ||
      ...
      helpers::reverseArray<T>(context, const_cast<NDArray*>(input)->buffer(),
                               const_cast<NDArray*>(input)->shapeInfo(),
                               output->buffer(), output->shapeInfo(),
                               seqLengths->e<int>(0));
  } else {
    ...
    for (int i = 0; i < inSubArrsSet.size(); ++i) {
      ...
      if (numOfElemsToReverse == 0 || numOfElemsToReverse == 1) {
        ...
        for (int j = 0; j < inInnerSet.size(); ++j)
          helpers::reverseArray<T>(
              context, inInnerSet.at(j).buffer(), inInnerSet.at(j).shapeInfo(),
              outInnerSet.at(j).buffer(), outInnerSet.at(j).shapeInfo(),
              numOfElemsToReverse);
      }
    }
  }
...
void reverse(sd::LaunchContext* context, const NDArray* input, NDArray* output,
  ...
  for (int i = 0; i < listIn.size(); ++i) {  // listIn.size() = listOut.size()
    ...
    BUILD_SINGLE_SELECTOR(input->dataType(), helpers::reverseArray,
                          (context, subArrIn.buffer(), subArrIn.shapeInfo(),
                           subArrOut.buffer(), subArrOut.shapeInfo()),
                          LIBND4J_TYPES);
  }
}
...
BUILD_SINGLE_TEMPLATE(template void reverseArray,
                      (sd::LaunchContext * context, void const* inArr,
                       Nd4jLong const* inShapeBuffer, void* outArr,
                       Nd4jLong const* outShapeBuffer, int numOfElemsToReverse),
                      LIBND4J_TYPES);
