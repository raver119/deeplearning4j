...
//     // maxRank = xRank = zRank, minRank = meanRank = varianceRank = gammaRank
//     = betaRank
...
// 		const auto meanOffset     = shape::getIndexOffset(i,
// meanShapeInfo);
//     	const auto varianceOffset = shape::getIndexOffset(i, varianceShapeInfo);

//     	T sigmaInvGam = 1. / sd::math::nd4j_sqrt<T, T>(variance[varianceOffset]
//     + epsilon);
...
//     		zTad[zTadOffset] = (xTad[xTadOffset] - mean[meanOffset]) *
//     sigmaInvGam;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void batchnormCuda2(
    const void* vx, const Nd4jLong* xShapeInfo, const void* vMean,
    const Nd4jLong* meanShapeInfo, const void* vVariance,
    const Nd4jLong* varianceShapeInfo, const void* vGamma,
    const Nd4jLong* gammaShapeInfo, const void* vBeta,
    const Nd4jLong* betaShapeInfo, void* vz, const Nd4jLong* zShapeInfo,
  ...
  __shared__ int xRank, minRank;  // xRank == zRank, minRank = meanRank =
                                  // varianceRank = gammaRank = betaRank
  ...
  for (uint i = tid; i < xLen; i += totalThreads) {
    ...
    T sigmaInvGam =
        1. / sd::math::nd4j_sqrt<T, T>(variance[varianceOffset] + epsilon);
...
//     xShapeInfo, vMean, meanShapeInfo, vVariance, varianceShapeInfo, vGamma,
//     gammaShapeInfo, vBeta, betaShapeInfo, vz, zShapeInfo, xTadShapeInfo,
//     xTadOffsets, zTadShapeInfo, zTadOffsets, static_cast<T>(epsilon));
// }

///////////////////////////////////////////////////////////////////
template <typename T>
__host__ static void batchnormCudaLauncher2(
    const int blocksPerGrid, const int threadsPerBlock,
    const cudaStream_t* stream, const void* vx, const Nd4jLong* xShapeInfo,
    const void* vMean, const Nd4jLong* meanShapeInfo, const void* vVariance,
    const Nd4jLong* varianceShapeInfo, const void* vGamma,
    const Nd4jLong* gammaShapeInfo, const void* vBeta,
    const Nd4jLong* betaShapeInfo, void* vz, const Nd4jLong* zShapeInfo,
  ...
  batchnormCuda2<T><<<blocksPerGrid, threadsPerBlock, 512, *stream>>>(
      ...
      vGamma, gammaShapeInfo, vBeta, betaShapeInfo, vz, zShapeInfo, numDims,
      dims, static_cast<T>(epsilon));
}

//////////////////////////////////////////////////////////////////////////
void batchnorm(const NDArray* input, const NDArray* mean,
               const NDArray* variance, const NDArray* gamma,
               const NDArray* beta, NDArray* output,
  ...
  //    const int blocksPerGrid = (mean->lengthOf() + threadsPerBlock - 1) /
  //    threadsPerBlock;

  //    PointersManager manager(input->getContext(), "batchnorm");

  //    NDArray::prepareSpecialUse({output}, {input, mean, variance, gamma,
  //    beta}); BUILD_SINGLE_SELECTOR(input->dataType(), batchnormCudaLauncher,
  //    (blocksPerGrid, threadsPerBlock, input->getContext()->getCudaStream(),
  //    input->specialBuffer(), input->specialShapeInfo(),
  //    mean->specialBuffer(), mean->specialShapeInfo(),
  //    variance->specialBuffer(), variance->specialShapeInfo(), gamma ?
  ...
  const int* dims = reinterpret_cast<int*>(
      manager.replicatePointer(axes.data(), axes.size() * sizeof(int)));

  NDArray::prepareSpecialUse({output}, {input, mean, variance, gamma, beta});
  BUILD_SINGLE_SELECTOR(
      input->dataType(), batchnormCudaLauncher2,
      (blocksPerGrid, threadsPerBlock, input->getContext()->getCudaStream(),
       input->specialBuffer(), input->specialShapeInfo(), mean->specialBuffer(),
       mean->specialShapeInfo(), variance->specialBuffer(),
       variance->specialShapeInfo(), gamma ? gamma->specialBuffer() : nullptr,
       gamma ? gamma->specialShapeInfo() : nullptr,
       beta ? beta->specialBuffer() : nullptr,
       beta ? beta->specialShapeInfo() : nullptr, output->specialBuffer(),
       output->specialShapeInfo(), axes.size(), dims, epsilon),
      FLOAT_TYPES);
