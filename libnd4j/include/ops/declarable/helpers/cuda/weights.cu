...
template <typename T>
static __device__ void adjustWeightsKernelD(
    void* inputBuffer, Nd4jLong const* inputShape, void* weightsBuffer,
    Nd4jLong const* weightsShape, void* outputBuffer, Nd4jLong inputLength,
  ...
  for (Nd4jLong e = tid; e < inputLength; e += blockDim.x) {
    ...
    if (current == val) {
      ...
      if (weightsBuffer != nullptr) {
        ...
        // atomicAdd();
        //*reinterpret_cast<int *>(outputBuffer) +=  reinterpret_cast<int
        //*>(weightsBuffer)[yOffset];
        sd::math::atomics::nd4j_atomicAdd(
            reinterpret_cast<T*>(outputBuffer),
            reinterpret_cast<T*>(
                weightsBuffer)[yOffset]);  // output->p(val, output->e<T>(val) +
                                           // 1);
        //                    atomicAdd(reinterpret_cast<int *>(outputBuffer),
        //                    reinterpret_cast<int *>(weightsBuffer)[yOffset]);
        ...
        //                    atomicAdd(reinterpret_cast<int *>(outputBuffer),
        //                    int(1)); //output->p(val, output->e<T>(val) + 1);
  ...
  //            reinterpret_cast<T *>(outputBuffer)[threadIdx.x + offset] =
  //            outputPart[threadIdx.x];
}

template <typename T>
static __global__ void adjustWeightsKernel(
    void* inputBuffer, Nd4jLong const* inputShape, void* weightsBuffer,
    Nd4jLong const* weightsShape, void* outputBuffer,
    Nd4jLong const* outputShape, int minLength, int maxLength) {
  // auto tid = blockIdx.x * blockDim.x + threadIdx.x; // * blockDim.x; // +
  // threadIdx.x;
  int threadCount = gridDim.x * blockDim.x;
  ...
  for (Nd4jLong e = blockIdx.x; e < outputLength; e += threadCount) {
    ...
    adjustWeightsKernelD<T>(inputBuffer, inputShape, weightsBuffer,
                            weightsShape, (void*)outputBufferZ, inputLength,
                            outputLength, (int)zOffset);
  }
}

template <typename T>
static void adjustWeights_(sd::LaunchContext* context, NDArray* input,
                           NDArray* weights, NDArray* output, int minLength,
                           int maxLength) {
  ...
  adjustWeightsKernel<T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
      ...
      weights ? weights->specialBuffer() : nullptr,
      weights ? weights->specialShapeInfo() : nullptr, output->specialBuffer(),
      output->specialShapeInfo(), minLength, maxLength);
}
...
void adjustWeights(sd::LaunchContext* context, NDArray* input, NDArray* weights,
  ...
  BUILD_SINGLE_SELECTOR(output->dataType(), adjustWeights_,
                        (context, input, weights, output, minLength, maxLength),
