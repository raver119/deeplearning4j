...
template <typename T>
static __global__ void invertKernelLow(void *invertedBuf,
                                       const Nd4jLong *invertedShape,
                                       const void *inputBuf,
                                       const Nd4jLong *inputShape, Nd4jLong n) {
  ...
  for (int i = start + 1; i < n; i += step) {
    ...
    //            math::atomics::nd4j_atomicAdd(&inverted[zIndex], -
    //            input[xIndex] * inverted[iIndex] / input[dIndex]);
  }
}
// ------------------------------------------------------------------------------------------------------------------
// // invert diagonal vals to upper diagonal matrix
template <typename T>
static __global__ void upvertKernel(void *invertedBuf,
                                    const Nd4jLong *invertedShape,
                                    const void *inputBuf,
                                    const Nd4jLong *inputShape, Nd4jLong n) {
  auto inverted = reinterpret_cast<T *>(invertedBuf);
...
template <typename T>
static __global__ void upvertKernelUp(void *invertedBuf,
                                      const Nd4jLong *invertedShape,
                                      const void *inputBuf,
                                      const Nd4jLong *inputShape, Nd4jLong n) {
  ...
  for (int i = start; i < n - 1; i += step) {
    ...
    math::atomics::nd4j_atomicAdd(
        &inverted[zIndex],
        -input[xIndex] * inverted[iIndex]);  // / input[yIndex]);
    // inputMatrix->t<T>(i, i + 1) * invertedMatrix->t<T>(i + 1, i + 1) /
    // inputMatrix->t<T>(i, i)
  }
}
...
// //
template <typename T>
static __global__ void invertLowKernel(void *invertedBuf,
                                       const Nd4jLong *invertedShape,
                                       const void *inputBuf,
                                       const Nd4jLong *inputShape, Nd4jLong n) {
  auto input = reinterpret_cast<const T *>(inputBuf);
  ...
  for (int i = tid + 2; i < n; i += step) {
    for (int j = i - 2; j >= 0; --j)
      for (int k = 0; k < i; k++) {
        ...
        math::atomics::nd4j_atomicAdd(
            &inverted[zIndex],
            -inverted[yIndex] * input[xIndex] / input[dIndex]);
      }
  }
}

// ------------------------------------------------------------------------------------------------------------------
// // Invertion of upper triangular matrix non-diagonal elements when main and
// second diagonals already processed
template <typename T>
static __global__ void invertUpKernel(void *invertedBuf,
                                      const Nd4jLong *invertedShape,
                                      const void *inputBuf,
                                      const Nd4jLong *inputShape, Nd4jLong n) {
  auto inverted = reinterpret_cast<T *>(invertedBuf);
  ...
  for (int i = (int)n - tid - 2; i >= 0; i -= step) {
    for (int j = i + 2; j < (int)n; j++)
      for (int k = i; k < (int)n; k++) {
        ...
        math::atomics::nd4j_atomicAdd(&inverted[zIndex],
                                      -inverted[yIndex] * input[xIndex]);
      }
  }
}

// ------------------------------------------------------------------------------------------------------------------
// // procedure to invert lower-triangular matrix. In current case lower
// triangular matrix has main diagonal with general values
//
template <typename T>
static void invertLowerMatrix_(LaunchContext *context, NDArray *inputMatrix,
                               NDArray *invertedMatrix) {
...
void invertLowerMatrix(LaunchContext *context, NDArray *inputMatrix,
                       NDArray *invertedMatrix) {
  NDArray::prepareSpecialUse({invertedMatrix}, {inputMatrix});
  BUILD_SINGLE_SELECTOR(inputMatrix->dataType(), invertLowerMatrix_,
                        (context, inputMatrix, invertedMatrix), FLOAT_NATIVE);
  NDArray::registerSpecialUse({invertedMatrix}, {inputMatrix});
}

// ------------------------------------------------------------------------------------------------------------------
// // procedure to invert upper-triangular matrix. In current case upper
// triangular matrix has main diagonal with all ones on it.
template <typename T>
static void invertUpperMatrix_(LaunchContext *context, NDArray *inputMatrix,
                               NDArray *invertedMatrix) {
  int n = inputMatrix->rows();
  invertedMatrix->setIdentity();
  auto stream = context->getCudaStream();
  ...
  // invert upper matrix
  // invert the second diagonal
  upvertKernelUp<T><<<1, n, 512, *stream>>>(
      invertedMatrix->specialBuffer(), invertedMatrix->specialShapeInfo(),
      inputMatrix->specialBuffer(), inputMatrix->specialShapeInfo(), n);

  // invert other elements
  invertUpKernel<T><<<n, n, 512, *stream>>>(
      invertedMatrix->specialBuffer(), invertedMatrix->specialShapeInfo(),
      inputMatrix->specialBuffer(), inputMatrix->specialShapeInfo(), n);
}
...
void invertUpperMatrix(LaunchContext *context, NDArray *inputMatrix,
                       NDArray *invertedMatrix) {
  NDArray::prepareSpecialUse({invertedMatrix}, {inputMatrix});
  BUILD_SINGLE_SELECTOR(invertedMatrix->dataType(), invertUpperMatrix_,
                        (context, inputMatrix, invertedMatrix), FLOAT_NATIVE);
  NDArray::prepareSpecialUse({invertedMatrix}, {inputMatrix});
}

// ------------------------------------------------------------------------------------------------------------------
// // determinant kernel - accumulation product of all values on the main
// diagonal
template <typename T>
static __global__ void determinantKernel(T *compound, T *result, Nd4jLong len) {
  ...
  for (auto i = start; i < len; i += step) {
    auto pos = i * len + i;  // shape::getOffset(0, shape::shapeOf(shape),
                             // shape::stride(shape), di, 2);
...
// ------------------------------------------------------------------------------------------------------------------
// // determinant logarithm - accumulation sum of all logarithm values on the
// main diagonal. All in logarithic values should be positive
template <typename T>
static __global__ void determinantLogKernel(T *compound, T *result,
                                            Nd4jLong len) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;
  for (auto i = start; i < len; i += step) {
    ...
    math::atomics::nd4j_atomicAdd(
        result, math::nd4j_log<T, T>(math::nd4j_abs(compound[pos])));
  }
}

// ------------------------------------------------------------------------------------------------------------------
// // kernel to copy matrix with given shape to compound tensor with given pos
// output - a N-D tensor buffer with rank not less than 2, input - 2D square n x
// n matrix with n = rowLen
template <typename T, typename F>
static __global__ void fillMatrix(void *output, const Nd4jLong *outShape,
                                  const void *input, const Nd4jLong *inputShape,
                                  Nd4jLong pos, Nd4jLong rowLen) {
  __shared__ F *matrix;
...
template <typename T>
static __global__ void returnMatrix(void *output, const Nd4jLong *outputShape,
                                    const void *input,
                                    const Nd4jLong *inputShape, Nd4jLong pos,
                                    Nd4jLong rowLen) {
...
// ------------------------------------------------------------------------------------------------------------------
// // fill up permutaion matrix kernel. Permutation matrix filled with zeros and
// ones
template <typename F>
static __global__ void fillUpPermutation(void *output, const Nd4jLong *shape,
                                         int *source, int rowNum) {
...
// ------------------------------------------------------------------------------------------------------------------
// // LUP decomposition runner - using CUBLAS SOLVER if permutation is given,
// then using LUP decomposition, LU decomposition otherwise L - lower
// triangular, U - upper triangular, P - permutation matricies PA = LU
//
// input - A matrix nxn
// compound - C matrix L + U - I, or main diagonal and lower - L matrix, from
// the 2nd diagonal - U matrix
template <typename T, typename I>
static void lup_(LaunchContext *context, NDArray *input, NDArray *compound,
                 NDArray *permutation) {
  auto stream = context->getCudaStream();
  auto n = input->rows();
  ...
  cusolverDnHandle_t *cusolverH =
      (cusolverDnHandle_t *)context->getCusolverHandle();  // nullptr;
  // create solver handle
  cusolverStatus_t status;  // cusolverDnCreate(&cusolverH);
  //        if (CUSOLVER_STATUS_SUCCESS != status) {
  //            throw cuda_exception::build("Cannot create cuSolver handle",
  //            status);
  //        }
  // set solver stream
  ...
  if (err) {
    throw cuda_exception::build(
        "helpers::lup_: Cannot allocate memory for solver info buffer", err);
  }

  DataType dtype = input->dataType();
  switch (dtype) {  // there are two implementations with cublas for LUP
                    // decomposition - double and float
    ...
    case DataType::DOUBLE: {
      ...
      if (CUSOLVER_STATUS_SUCCESS != status) {
        throw cuda_exception::build(
            "helpers::lup_: Cannot create cuSolver handle", status);
      }

      err = cudaMalloc((void **)&d_work, sizeof(float) * lwork);
      if (err) {
        throw cuda_exception::build(
            "helpers::lup_: Cannot allocate memory for solver data buffer",
            err);
      }

      if (permutation == nullptr) {
        status = cusolverDnDgetrf(*cusolverH, n, n, matrix, n, d_work, nullptr,
                                  d_info);

        if (status != CUSOLVER_STATUS_SUCCESS) {
          throw cuda_exception::build(
              "helpers::lup_: LU factorization is failed due ", status);
        }
      } else {
        NDArray permutVector('c', {n}, sd::DataType::INT32, context);
        int *permutationBuf = permutVector.dataBuffer()->specialAsT<int>();
        status = cusolverDnDgetrf(*cusolverH, n, n, matrix, n, d_work,
                                  permutationBuf, d_info);
        if (status != CUSOLVER_STATUS_SUCCESS) {
          throw cuda_exception::build(
              "helpers::lup_: LU factorization is failed due ", status);
        }
        ...
        if (permutation->rankOf() == 2) {
          fillUpPermutation<double><<<n, n, 1024, *stream>>>(
              permutation->specialBuffer(), permutation->specialShapeInfo(),
              permutationBuf, n);
        } else {
      ...
      if (err) {
        throw cuda_exception::build(
            "helpers::lup_: Cannot deallocate memory for solver data buffer",
            err);
      }
    } break;
    case DataType::FLOAT32: {
      ...
      if (permutation == nullptr)
        status = cusolverDnSgetrf(*cusolverH, n, n, matrix, n, d_work, nullptr,
                                  d_info);
      else {
        NDArray permutVector('c', {n}, DataType::INT32, context);
        int *permutationBuf =
            reinterpret_cast<int *>(permutVector.specialBuffer());
        status = cusolverDnSgetrf(*cusolverH, n, n, matrix, n, d_work,
                                  permutationBuf, d_info);
        if (permutation->rankOf() == 2) {
  ...
  if (CUSOLVER_STATUS_SUCCESS != status) {
    throw cuda_exception::build("helpers::lup_: Cannot make LU decomposition",
                                status);
  }
  err = cudaFree(d_info);
  if (err) {
    throw cuda_exception::build(
        "helpers::lup_: Cannot deallocate memory for solver info buffer", err);
  }
...
BUILD_DOUBLE_TEMPLATE(template void lup_,
                      (LaunchContext * context, NDArray *input, NDArray *output,
                       NDArray *permutation),
                      FLOAT_NATIVE, INDEXING_TYPES);

template <typename T>
static __device__ void swapRows(T *matrix, const Nd4jLong *shape,
                                Nd4jLong theFirst, Nd4jLong theSecond,
                                Nd4jLong n) {
...
template <typename T>
static __device__ void processColumns(Nd4jLong currentRow, Nd4jLong rowNum,
                                      T *compoundBuf,
                                      const Nd4jLong *compoundShape) {
...
template <typename T>
__device__ Nd4jLong argmaxCol(Nd4jLong column, T *compoundBuffer,
                              const Nd4jLong *compoundShape) {
  ...
  for (auto rowCounter = column; rowCounter < rowNum; rowCounter++) {
    ...
    if (sd::math::nd4j_abs(compoundBuffer[xIndex]) > maxValue) {
      maxValue = sd::math::nd4j_max(maxValue,
                                    sd::math::nd4j_abs(compoundBuffer[xIndex]));
      result = rowCounter;
    }
  }
  return result;
...
template <typename T, typename I>
static __device__ int luNN(T *matrix, const Nd4jLong *shape, I *permutation,
                           const Nd4jLong *permuShape, Nd4jLong n) {
  for (auto i = 0; i < n - 1; i++) {
    auto pivotIndex = argmaxCol(i, matrix, shape);
    if (pivotIndex < 0) {
      return -1;  // throw std::runtime_error("helpers::luNN_: input matrix is
                  // singular.");
    }
    math::nd4j_swap(permutation[shape::getIndexOffset(i, permuShape)],
...
static __global__ void luBatchedKernel(
    T *outputBuf, const Nd4jLong *outputShape, I *permutations,
    const Nd4jLong *permuShape, const Nd4jLong *outputTadShape,
    const Nd4jLong *outputTadOffsets, const Nd4jLong *permuTadShape,
    const Nd4jLong *permuTadOffsets, Nd4jLong batchNum) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;
  ...
  for (auto b = start; b < batchNum; b += step) {
    ...
    if (0 != luNN(matrix, outputTadShape, permutation, permuTadShape,
                  shape::length(permuTadShape)))
      break;
  }
}

template <typename T, typename I>
static void lu_(LaunchContext *context, NDArray *input, NDArray *output,
                NDArray *permutationVectors) {
  auto n = input->sizeAt(-1);
  auto stream = context->getCudaStream();
  NDArray iota('c', {n}, permutationVectors->dataType(),
               context);  // = NDArrayFactory::create(); // <int>('c', {n});
  iota.linspace(0);
  iota.syncToDevice();
  ...
  luBatchedKernel<T, I><<<batchNum, 256, 1024, *stream>>>(
      ...
      permutationVectors->specialShapeInfo(), tads.specialShapeInfo(),
      tads.specialOffsets(), permutaionTads.specialShapeInfo(),
      permutaionTads.specialOffsets(), batchNum);
}

void lu(LaunchContext *context, NDArray *input, NDArray *output,
        NDArray *permutations) {
  NDArray::prepareSpecialUse({output, permutations}, {input});
  BUILD_DOUBLE_SELECTOR(input->dataType(), permutations->dataType(), lu_,
                        (context, input, output, permutations), FLOAT_NATIVE,
                        INDEXING_TYPES);
...
// //
template <typename T>
static int determinant_(sd::LaunchContext *context, NDArray *input,
                        NDArray *output) {
  ...
  auto matrix = NDArrayFactory::create(input->ordering(), {n, n},
  ...
  for (int e = 0; e < output->lengthOf(); e++) {
    ...
    fillMatrix<T, T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
        matrix.specialBuffer(), matrix.specialShapeInfo(),
        input->specialBuffer(), input->specialShapeInfo(), pos, n);
    //            else
    ...
    //            else
    //                determinantKernel<T, float><<<launchDims.x, launchDims.y,
    //                launchDims.z, *stream >>> (inputBuf, outputBuf, n);
  }
...
int determinant(sd::LaunchContext *context, NDArray *input, NDArray *output) {
  NDArray::prepareSpecialUse({output}, {input});
  BUILD_SINGLE_SELECTOR(input->dataType(), return determinant_,
                        (context, input, output), FLOAT_NATIVE);
  NDArray::registerSpecialUse({output}, {input});
}

template <typename T>
int logAbsDeterminant_(LaunchContext *context, NDArray *input,
                       NDArray *output) {
  Nd4jLong n = input->sizeAt(-1);
  Nd4jLong n2 = n * n;
  std::vector<int> dims();
  ...
  for (int e = 0; e < output->lengthOf(); e++) {
    ...
    auto outputBuf = reinterpret_cast<T *>(output->specialBuffer()) + offset;
    //            if (matrix.dataType() == input->dataType())
    determinantLogKernel<T>
        <<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(inputBuf,
                                                                outputBuf, n);
    //            else
    //                determinantLogKernel<T, float><<<launchDims.x,
    //                launchDims.y, launchDims.z, *stream >>> (inputBuf,
    //                outputBuf, n);
  }
  NDArray::registerSpecialUse({output}, {input});
...
int logAbsDeterminant(sd::LaunchContext *context, NDArray *input,
                      NDArray *output) {
  NDArray::prepareSpecialUse({output}, {input});
  BUILD_SINGLE_SELECTOR(input->dataType(), return logAbsDeterminant_,
                        (context, input, output), FLOAT_NATIVE);
  NDArray::registerSpecialUse({output}, {input});
}

template <typename T>
static __global__ void fillLowerUpperKernel(
    void *lowerBuf, const Nd4jLong *lowerShape, void *upperBuf,
    const Nd4jLong *upperShape, void *matrixBuf, const Nd4jLong *matrixShape,
    Nd4jLong n) {
  ...
  for (int k = blockIdx.x; k < n;
       k +=
       gridDim.x) {  // and then put all values under main diagonal on to it
...
template <typename T>
static int inverse_(sd::LaunchContext *context, NDArray *input,
                    NDArray *output) {
  auto n = input->sizeAt(-1);
  auto n2 = n * n;
  ...
  for (auto i = 0LL; i < packX.numberOfTads(); i++) {
    ...
    fillLowerUpperKernel<T><<<n, n, 1024, *stream>>>(
        lower.specialBuffer(), lower.specialShapeInfo(), upper.specialBuffer(),
        upper.specialShapeInfo(), matrix.specialBuffer(),
        matrix.specialShapeInfo(), n);
...
int inverse(sd::LaunchContext *context, NDArray *input, NDArray *output) {
  NDArray::prepareSpecialUse({output}, {input});
  BUILD_SINGLE_SELECTOR(input->dataType(), return inverse_,
                        (context, input, output), FLOAT_NATIVE);
  NDArray::registerSpecialUse({output}, {input});
}
...
template <typename F>
__global__ void fillBatchKernel(F **dArrayBatch, F *buf,
                                const Nd4jLong *offsets, Nd4jLong batchSize) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;
...
template <typename F>
__global__ void adjustResultsKernel(F *dArray, const Nd4jLong *shape,
                                    const Nd4jLong *offsets, Nd4jLong batchSize,
                                    Nd4jLong n) {
  ...
  for (auto i = blockIdx.x; i < batchSize; i += gridDim.x) {
    ...
    for (auto r = threadIdx.x; r < n; r += blockDim.x) {
      for (auto c = r + 1; c < n; c++) {
        Nd4jLong posRC[] = {r, c};
        auto pos =
            r * n + c;  // shape::getOffset(0, shapeOf, strideOf, posRC, 2);
        current[pos] = 0.;
      }
    }
  }
...
template <typename F>
int cholesky__(LaunchContext *context, NDArray *input, NDArray *output,
               bool inplace) {
  ...
  if (err) {
    throw cuda_exception::build(
        "helpers::cholesky_: Cannot allocate memory for solver batch data "
        "buffer",
        err);
  }
  ...
  if (err) {
    throw cuda_exception::build(
        "helpers::cholesky_: Cannot allocate memory for solver errors buffer",
        err);
  }
  auto stream = context->getCudaStream();
  fillBatchKernel<F><<<1, batchSize, 128, *stream>>>(
      dArrayBatch, reinterpret_cast<F *>(tempOutput.specialBuffer()),
      packX.specialOffsets(), batchSize);

  status = cusolverDnSetStream(handle, *stream);
  if (CUSOLVER_STATUS_SUCCESS != status) {
    throw cuda_exception::build(
        "helpers::cholesky_: Cannot set stream to solver handle", status);
  }
  const cublasFillMode_t uplo = CUBLAS_FILL_MODE_UPPER;
  if (input->dataType() == DataType::DOUBLE)
    status = cusolverDnDpotrfBatched(handle, uplo, n, (double **)dArrayBatch, n,
                                     dInfoArray, batchSize);
  else
    status = cusolverDnSpotrfBatched(handle, uplo, n, (float **)dArrayBatch, n,
                                     dInfoArray, batchSize);

  if (CUSOLVER_STATUS_SUCCESS != status) {
    throw cuda_exception::build(
        "helpers::cholesky_: Cholesky factorization failed for batch", status);
  }
  adjustResultsKernel<F><<<batchSize, n2, 128, *stream>>>(
      reinterpret_cast<F *>(tempOutput.specialBuffer()),
      packX.specialShapeInfo(), packX.specialOffsets(), batchSize, n);

  err = cudaFree(dArrayBatch);
  if (err) {
    throw cuda_exception::build(
        "helpers::cholesky_: Cannot deallocate memory for solver batch data "
        "buffer",
        err);
  }
...
//    template <typename T>
int cholesky_(LaunchContext *context, NDArray *input, NDArray *output,
              bool inplace) {
  NDArray::prepareSpecialUse({output}, {input});
  if (input->dataType() == DataType::DOUBLE)
    ...
    std::unique_ptr<NDArray> tempOutput(NDArrayFactory::create_(
        'c', input->getShapeAsVector(), DataType::FLOAT32, context));
...
int cholesky(sd::LaunchContext *context, NDArray *input, NDArray *output,
             bool inplace) {
  //        BUILD_SINGLE_SELECTOR(input->dataType(), return cholesky_, (context,
  //        input, output, inplace), FLOAT_TYPES);
  return cholesky_(context, input, output, inplace);
}
//    BUILD_SINGLE_TEMPLATE(template int cholesky_, (LaunchContext* context,
//    NDArray* input, NDArray* output, bool inplace), FLOAT_TYPES);
BUILD_SINGLE_TEMPLATE(template int inverse_,
                      (sd::LaunchContext * context, NDArray *input,
                       NDArray *output),
                      FLOAT_NATIVE);

template <typename T>
__global__ void logDetKernel(const T *inputBuf, const Nd4jLong *inputShape,
                             Nd4jLong batchNum, const Nd4jLong *tadShape,
                             const Nd4jLong *tadOffsets, T *outputBuf,
...
template <typename T>
int logdetFunctor_(sd::LaunchContext *context, NDArray *input,
                   NDArray *output) {
  NDArray::prepareSpecialUse({output}, {input});
  ...
  logDetKernel<T><<<128, 512, 256, *stream>>>(
      inputBuf, tempOutput.specialShapeInfo(), packX.numberOfTads(),
...
int logdetFunctor(sd::LaunchContext *context, NDArray *input, NDArray *output) {
  BUILD_SINGLE_SELECTOR(output->dataType(), return logdetFunctor_,
                        (context, input, output), FLOAT_NATIVE);
}
 ...
 * */
int lup(LaunchContext *context, NDArray *input, NDArray *compound,
        NDArray *permutation) {
  BUILD_DOUBLE_SELECTOR(input->dataType(), permutation->dataType(), lup_,
                        (context, input, compound, permutation), FLOAT_NATIVE,
                        INDEXING_TYPES);
  return Status::OK();
}
