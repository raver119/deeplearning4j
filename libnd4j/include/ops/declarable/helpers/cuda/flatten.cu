...
void _CUDA_G flattenKernel(void **xBuffers, Nd4jLong **xShapeInfos,
                           Nd4jLong *offsets, Nd4jLong numInputs, void *zBuffer,
                           const Nd4jLong *zShapeInfo, char order) {
  int xCoord[MAX_RANK];
...
template <typename T>
void flatten_(sd::LaunchContext *context, std::vector<NDArray *> &inputs,
              NDArray *output, char order) {
  ...
  auto dBuffers = (void **)pm.replicatePointer(hdBuffers.data(),
                                               inputs.size() * sizeof(void *));
  auto dShapes = (Nd4jLong **)pm.replicatePointer(
      hdShapes.data(), inputs.size() * sizeof(Nd4jLong *));
  auto dOffsets = (Nd4jLong *)pm.replicatePointer(
      hOffsets.data(), inputs.size() * sizeof(Nd4jLong));

  flattenKernel<T><<<256, 512, 8192, *context->getCudaStream()>>>(
      dBuffers, dShapes, dOffsets, inputs.size(), output->specialBuffer(),
      output->specialShapeInfo(), order);

  pm.synchronize();
}

void flatten(sd::LaunchContext *context, std::vector<NDArray *> &inputs,
             NDArray *output, char order) {
  // FIXME: we want NDArrayFactory::prepareSpecialUse here eventually
  for (auto v : inputs) v->syncToDevice();

  BUILD_SINGLE_SELECTOR(output->dataType(), flatten_,
                        (context, inputs, output, order), LIBND4J_TYPES);
