...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void concatCuda(void* pVx, void* pxShapeInfo, void* vz,
                                  const Nd4jLong* zShapeInfo, const int axis) {
  ...
  for (uint64_t i = tid; i < zLen; i += totalThreads) {
    ...
    const auto* x =
        reinterpret_cast<T*>(reinterpret_cast<void**>(pVx)[inArrIdx]);
...
///////////////////////////////////////////////////////////////////
template <typename T>
__host__ static void concatCudaLauncher(
    ...
    const Nd4jLong* zShapeInfo, const int axis) {
  concatCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      pVx, pxShapeInfo, vz, zShapeInfo, axis);
}
...
void concat(sd::LaunchContext* context,
  ...
  if (luckCase1) {  // for example {1,10} + {2,10} + {3,10} = {6, 10} order c;
    ...
    for (uint i = 0; i < numOfInArrs; ++i) {
      ...
      cudaMemcpyAsync(
          z, reinterpret_cast<const int8_t*>(inArrs[i]->specialBuffer()),
          memAmountToCopy, cudaMemcpyDeviceToDevice, *context->getCudaStream());
  ...
  //         void* z = static_cast<int8_t*>(output.specialBuffer()) + zStep *
  //         iShift;

  //         for (uint j = 0; j < numOfInArrs; ++j) {
  //             const auto xDim = inArrs[j]->sizeAt(axis);
  //             void* x = static_cast<int8_t*>(inArrs[j]->specialBuffer()) +
  //             strideOfContigStride[j] * iShift; const auto memSizeToCopy =
  //             xDim * sizeofT; cudaMemcpyAsync(z, x, memSizeToCopy,
  //             cudaMemcpyDeviceToDevice, *context->getCudaStream()); z =
  //             static_cast<int8_t*>(z) + memSizeToCopy;
  //         }
  //     }
  ...
  void* dInShapeInfo = manager.replicatePointer(
      hInShapeInfo.data(), hInShapeInfo.size() * sizeof(Nd4jLong*));

  BUILD_SINGLE_SELECTOR(
      inArrs[0]->dataType(), concatCudaLauncher,
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(),
       dInBuffers, dInShapeInfo, output.specialBuffer(),
