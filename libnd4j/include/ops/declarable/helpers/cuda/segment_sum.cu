...
static __global__ void segmentSumLinearKernel(
...
template <typename T, typename I>
static __global__ void unsortedSegmentSumLinearKernel(
...
static __global__ void segmentSumTadKernel(
    ...
    const Nd4jLong* inputTadOffsets, const I* indices, int* starts,
    int* lengths, Nd4jLong numOfClasses, void* outputBuf,
    const Nd4jLong* outputShape, const Nd4jLong* outputTads,
...
template <typename T, typename I>
static void segmentSumFunctor_(sd::LaunchContext* context, NDArray* input,
...
void segmentSumFunctor(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(input->dataType(), indices->dataType(),
                        segmentSumFunctor_, (context, input, indices, output),
...
// //
template <typename T, typename I>
static void unsortedSegmentSumFunctor_(sd::LaunchContext* context,
...
static __global__ void segmentSumBPLinearKernel(
    const void* inputBuf, const Nd4jLong* inputShape, const void* eps,
    const Nd4jLong* epsShape, const void* indicesBuf,
    const Nd4jLong* indicesShape, void* outputBuf,
...
static __global__ void segmentSumBPTadKernel(
    const void* inputBuf, const Nd4jLong* inputShape, const void* eps,
    const Nd4jLong* epsShape, const void* indicesBuf,
    const Nd4jLong* indicesShape, void* outputBuf, const Nd4jLong* outputShape,
...
// //
template <typename T, typename I>
int segmentSumFunctorBP_(sd::LaunchContext* context, NDArray* input,
...
int segmentSumFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(
      output->dataType(), indices->dataType(), return segmentSumFunctorBP_,
      (context, input, indices, gradOut, output), FLOAT_TYPES, INDEXING_TYPES);
  NDArray::registerSpecialUse({output}, {input, indices, gradOut});
}

template <typename T, typename I>
static int unsortedSegmentSumFunctorBP_(sd::LaunchContext* context,
                                        NDArray* input, NDArray* indices,
...
int unsortedSegmentSumFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(
      ...
      return unsortedSegmentSumFunctorBP_,
