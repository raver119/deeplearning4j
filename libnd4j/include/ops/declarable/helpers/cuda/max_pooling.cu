...
template <typename Z>
static _CUDA_G void indicesFiller(void* vz, Nd4jLong const* zShapeInfo,
                                  Nd4jLong part, Nd4jLong bSize) {
...
static void maxPoolingFunctor_(sd::graph::Context& block, NDArray* input,
                               NDArray* values, std::vector<int> const& params,
                               NDArray* indices) {
  int kY = params[0];
  int kX = params[1];
  ...
  if (isSameMode)
    ConvolutionUtils::calcPadding2D(pY, pX, oY, oX, inY, inX, params[0],
  ...
  ConvolutionUtils::pooling2d(block, *input, *values, kY, kX, sY, sX, pY, pX,
                              dY, dX, PoolingType::MAX_POOL, 1);
  ...
  if (nullptr != indices) {
    ...
    indicesFiller<Y>
        <<<256, 256, 1024, *block.launchContext()->getCudaStream()>>>(
