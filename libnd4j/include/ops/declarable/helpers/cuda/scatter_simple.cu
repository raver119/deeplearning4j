...
static _CUDA_G void scatterSimpleKernel(
    void* vx, const Nd4jLong* xTadShape, const Nd4jLong* xTadOffsets,
    Nd4jLong xLength, Nd4jLong numTads, const void* vi,
    const Nd4jLong* iShapeInfo, Nd4jLong iLength, const void* vu,
    const Nd4jLong* uShapeInfo, Nd4jLong uLength) {
  ...
  for (int i = tid; i < iLength; i += blockDim.x * gridDim.x) {
    ...
    x[shape::getIndexOffset(idx, xTadShape)] =
        u[shape::getIndexOffset(i, uShapeInfo)];
  }
}

template <typename X, typename Y>
void scatterSimple_(sd::LaunchContext* context, const int opId, NDArray& input,
  ...
  scatterSimpleKernel<X, Y><<<256, 256, 1024, *context->getCudaStream()>>>(
      ...
      xLength, packX.numberOfTads(), indices.specialBuffer(),
      indices.specialShapeInfo(), iLength, updates.specialBuffer(),
      updates.specialShapeInfo(), uLength);
}
...
void scatterSimple(sd::LaunchContext* context, const int opId, NDArray& input,
  ...
  BUILD_DOUBLE_SELECTOR(xType, yType, scatterSimple_,
                        (context, opId, input, updates, indices, dimensions),
