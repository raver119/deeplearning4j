...
template <typename T>
static void ismax_(sd::LaunchContext* context, const NDArray* input,
  ...
  if (dimensions.size() == 0) {
    /**
     * In case of vector-input for IsMax, it just turns into IndexReduce call +
     * subsequent filler call
     */
    ...
    BUILD_SINGLE_SELECTOR(
        ...
        (launchDims, stream, output->specialBuffer(),
         output->specialShapeInfo(), output->lengthOf(), targetIdx),
        LIBND4J_TYPES);
    manager.synchronize();
    ...
    auto packZ = sd::ConstantTadHelper::getInstance().tadForDimensions(
        output->shapeInfo(), copy.data(), copy.size());

    // we launch legacy IndexMax op, to get indices of max values along
    // dimension
    auto indexMaxArr =
        input->applyIndexReduce(indexreduce::IndexMax, dimensions);

    dim3 launchDims(256, 256, 16384);
    dimension = (int*)manager.replicatePointer(dimensions.data(),
                                               dimensions.size() * sizeof(int));

    // at this point, all IMax indexes are gathered, and we execute filler
    BUILD_SINGLE_SELECTOR(zType, fillDimensionalIsMaxGeneric,
                          (launchDims, stream, indexMaxArr.specialBuffer(),
                           output->specialBuffer(), output->specialShapeInfo(),
                           packZ.specialShapeInfo(), dimension, dimensionLength,
...
void ismax(sd::LaunchContext* context, const NDArray* input, NDArray* output,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), ismax_,
                        (context, input, output, dimensions), LIBND4J_TYPES);
