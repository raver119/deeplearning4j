...
template <typename T>
static _CUDA_G void lrnKernel(void* vx, Nd4jLong const* xTadShapeInfo,
                              Nd4jLong const* xTadOffsets, void* vz,
                              Nd4jLong const* zTadShapeInfo,
                              Nd4jLong const* zTadOffsets, Nd4jLong numTads,
                              Nd4jLong tadLength, int depth, double bias,
  ...
  for (uint i = blockIdx.x; i < numTads; i += gridDim.x) {
    ...
    // load everything into shared memory, so we'll operate on shared memory
    // from now on
    ...
    z[threadIdx.x * zEws] =
        shared[threadIdx.x] /
        sd::math::nd4j_pow<T, T, T>(tbias + alpha * prev, tbeta);
  }
}

template <typename X, typename Z>
static _CUDA_G void lrnBPKernel(void const* vx, Nd4jLong const* xTadShapeInfo,
                                Nd4jLong const* xTadOffsets, void* vz,
                                Nd4jLong const* zTadShapeInfo,
                                Nd4jLong const* zTadOffsets, Nd4jLong numTads,
                                Nd4jLong tadLength, int depth, double bias,
  ...
  for (uint i = blockIdx.x; i < numTads; i += gridDim.x) {
    ...
    for (uint s = begin; s < end; ++s) {
      factor[s] =
          sd::math::nd4j_pow<Z, Z, Z>(tbias + talpha * sharedY[s], -tbeta - 1);
      prev = prev + sharedX[s] * factor[s];
    }

    z[threadIdx.x * zEws] =
        factor[threadIdx.x] * init - 2 * sharedX[threadIdx.x] * coeff * prev;
  }
}

template <typename X, typename Z>
static void lrnBP_(sd::graph::Context& block, const NDArray& input,
  ...
  const int numBlocks =
      sd::math::nd4j_min<Nd4jLong>(1024, packX.numberOfTads());
  ...
  lrnBPKernel<X, Z><<<numBlocks, numThreads,
                      numThreads * sizeof(X) + numThreads * sizeof(Z) + 1024,
                      *block.launchContext()->getCudaStream()>>>(
      input.specialBuffer(), packX.platformShapeInfo(), packX.platformOffsets(),
      gradI.specialBuffer(), packZ.platformShapeInfo(), packZ.platformOffsets(),
      packX.numberOfTads(), tadLength, depth, bias, alpha, beta);
...
void lrnBP(sd::graph::Context& block, const NDArray& input,
  ...
  BUILD_DOUBLE_SELECTOR(input.dataType(), gradO.dataType(), lrnBP_,
                        (block, input, gradO, gradI, depth, bias, alpha, beta),
...
template <typename T>
static void lrnFunctor_(sd::graph::Context& block, NDArray* input,
  ...
  lrnKernel<T><<<numBlocks, numThreads, numThreads * sizeof(T),
                 *block.launchContext()->getCudaStream()>>>(
      input->specialBuffer(), packX.platformShapeInfo(),
      packX.platformOffsets(), output->specialBuffer(),
      packZ.platformShapeInfo(), packZ.platformOffsets(), packX.numberOfTads(),
      tadLength, depth, bias, alpha, beta);
}
...
int lrnFunctor(sd::graph::Context& block, NDArray* input, NDArray* output,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), lrnFunctor_,
