...
namespace helpers {
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// count rows kernel - count input pRows and pCols and put result onto
// pRowCounts pRowCounts - array of ints, with length N pRows - array of ints
// with length N, vals from 0 to N-1 pCols - array of ints with length < N and
// vals between 0 and max(pRows)
//
static __global__ void countRowsKernel(int* pRowCounts, int const* pRows,
                                       int const* pCols, Nd4jLong N) {
...
Nd4jLong barnes_row_count(const NDArray* rowP, const NDArray* colP, Nd4jLong N,
  ...
  NDArray numElementsArr =
      rowCounts.sumNumber();  // reduceAlongDimension(reduce::Sum, {});
...
//
static __global__ void fillUpsymRow(int const* pRowCounts, int* symRowP,
                                    int N) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;

  for (int n = start; n < N + 1;
       n += step) {  // to avoid race condition use shift only for given index
    symRowP[n] = 0;
...
//
template <typename T>
static __global__ void symmetrizeKernel(int const* pRows, int const* pCols,
                                        T const* pVals, int* symRowP,
                                        int* symColP, int* offset, T* pOutput,
                                        int N) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;
...
//
template <typename T>
static void barnes_symmetrize_(const NDArray* rowP, const NDArray* colP,
                               const NDArray* valP, Nd4jLong N,
                               NDArray* outputRows, NDArray* outputCols,
                               NDArray* outputVals, NDArray* rowCounts) {
  int const* pRows = reinterpret_cast<int const*>(rowP->specialBuffer());
  ...
  // symmetrize itself
  symmetrizeKernel<T><<<1, 1, 1024, *stream>>>(pRows, pCols, pVals, symRowP,
                                               symColP, offset, pOutput, N);
}
...
void barnes_symmetrize(const NDArray* rowP, const NDArray* colP,
  ...
  BUILD_SINGLE_SELECTOR(
      valP->dataType(), barnes_symmetrize_,
      (rowP, colP, valP, N, outputRows, outputCols, outputVals, rowCounts),
      NUMERIC_TYPES);

  *outputVals /= 2.0;
}
BUILD_SINGLE_TEMPLATE(template void barnes_symmetrize_,
                      (const NDArray* rowP, const NDArray* colP,
                       const NDArray* valP, Nd4jLong N, NDArray* outputRows,
                       NDArray* outputCols, NDArray* outputVals,
                       NDArray* rowCounts),
                      NUMERIC_TYPES);
...
//
template <typename T>
static __global__ void edgeForcesKernel(int const* pRows, int const* pCols,
                                        T const* dataP, T const* vals,
                                        T* outputP, int N, int colCount,
                                        int rowSize) {
  ...
  for (int n = start; n < N; n += step) {
    ...
    for (int i = start; i < end; i++) {
      ...
      for (int k = 0; k < colCount; k++) {
        ...
        res +=
            valTemp *
            valTemp;  // (dataP[shift + k] * dataP[shift + k] - 2 * dataP[shift
                      // + k] * thisSlice[k] + thisSlice[k] * thisSlice[k])
      }
      res = vals[i] / res;
      for (int k = 0; k < colCount; k++)
        math::atomics::nd4j_atomicAdd(
            &outputP[shift + k], T((dataP[shift + k] - thisSlice[k]) * res));
    }
  }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
template <typename T>
static void barnes_edge_forces_(const NDArray* rowP, NDArray const* colP,
                                NDArray const* valP, int N, NDArray const* data,
                                NDArray* output) {
  ...
  edgeForcesKernel<T><<<1, 128, 1024, *stream>>>(pRows, pCols, dataP, vals,
                                                 outputP, N, colCount, rowSize);
...
void barnes_edge_forces(const NDArray* rowP, NDArray const* colP,
  ...
  BUILD_SINGLE_SELECTOR(output->dataType(), barnes_edge_forces_,
                        (rowP, colP, valP, N, &data, output), FLOAT_TYPES);
}
BUILD_SINGLE_TEMPLATE(template void barnes_edge_forces_,
                      (const NDArray* rowP, NDArray const* colP,
                       NDArray const* valP, int N, NDArray const* data,
                       NDArray* output),
                      FLOAT_TYPES);

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// gains - run a function T((x + 2.) * sd::math::nd4j_sign<T,T>(grad) !=
// sd::math::nd4j_sign<T,T>(eps)) + T(x * 0.8 * sd::math::nd4j_sign<T,T>(grad)
// != sd::math::nd4j_sign<T,T>(eps)); for all members in input and put all in
// output
//
template <typename T>
void barnes_gains_(NDArray* input, NDArray* gradX, NDArray* epsilon,
                   NDArray* output) {
