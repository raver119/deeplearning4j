...
// FIXME -> we should optimize these helpers for the case when input matrices
// have c order (perform transpositions appropriately)

template <typename T>
__global__ static void inverseColumnSignCuda(void* vu,
                                             const Nd4jLong* uShapeInfo,
                                             void* vv,
                                             const Nd4jLong* vShapeInfo) {
  ...
  for (Nd4jLong i = ind; i < uLen; i += gridDim.x * blockDim.x) {
    ...
    if (coords[rank - 1] == 0 ||
        coords[rank - 1] == uLastButOneColumn)  // do not change sign in first
                                                // and last but one columns
      continue;
  ...
  for (Nd4jLong i = ind; i < vLen; i += gridDim.x * blockDim.x) {
    ...
    if (coords[rank - 2] == 0 ||
        coords[rank - 2] == vLastButOneColumn)  // do not change sign in first
                                                // and last but one columns
      continue;
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void inverseColumnSignCudaLauncher(const int blocksPerGrid,
                                          const int threadsPerBlock,
                                          const int sharedMem,
                                          const cudaStream_t* stream, void* vu,
                                          const Nd4jLong* uShapeInfo, void* vv,
                                          const Nd4jLong* vShapeInfo) {
  inverseColumnSignCuda<T>
      <<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(vu, uShapeInfo,
                                                               vv, vShapeInfo);
}
BUILD_SINGLE_TEMPLATE(template void inverseColumnSignCudaLauncher,
                      (const int blocksPerGrid, const int threadsPerBlock,
                       ...
                       void* vu, const Nd4jLong* uShapeInfo, void* vv,
                       const Nd4jLong* vShapeInfo),
                      FLOAT_TYPES);

//////////////////////////////////////////////////////////////////////////
static void svdQR(sd::LaunchContext* context, const NDArray* A, NDArray* S,
                  NDArray* U, NDArray* VT, const bool fullUV,
                  const bool calcUV) {
  // since cusa api cusolverDnDgesvd/cusolverDnSgesvd have following constrain
  // on input matrix A: A_rows >= A_columns && A_order = 'f' we make this
  // function to have deal with 2 valid cases only: 1) A_rows >= A_columns and
  // A_corder = 'f' 2) A_rows <= A_columns and A_corder = 'c'    - int this case
  // perform transposition to get f order if 1) or 2) are not met then throw
  // exception
  ...
  if (m < n)
    throw std::runtime_error(
        "svdQR: due to cuda api input constrains given shape of A array are "
        "not valid !");
  ...
  if (A->dataType() == DataType::DOUBLE) {
    status = cusolverDnDgesvd(
        *handle, jobu, jobvt, m, n,
        reinterpret_cast<double*>(pA->specialBuffer()), lda,
        reinterpret_cast<double*>(pS->specialBuffer()),
        calcUV ? reinterpret_cast<double*>(pU->specialBuffer()) : nullptr, ldu,
        calcUV ? reinterpret_cast<double*>(pVT->specialBuffer()) : nullptr,
        ldvt, reinterpret_cast<double*>(dWork), lwork,
        reinterpret_cast<double*>(rWork), devInfo);
  } else if (A->dataType() == DataType::FLOAT32) {
    status = cusolverDnSgesvd(
        *handle, jobu, jobvt, m, n,
        reinterpret_cast<float*>(pA->specialBuffer()), lda,
        ...
        reinterpret_cast<float*>(dWork), lwork, reinterpret_cast<float*>(rWork),
        devInfo);
  } else
    throw std::invalid_argument("svdQR: given data type is unsupported !");
...
//////////////////////////////////////////////////////////////////////////
static void svdJcb(sd::LaunchContext* context, const NDArray* A, NDArray* S,
                   NDArray* U, NDArray* V, const bool fullUV,
                   const bool calcUV) {
  // A [m, n]
  // S [n]
  // U [m, m] or [m, n] if fullUV = false and m > n
  ...
  if (calcUV) {
    ...
    if (fullUV && std::vector<Nd4jLong>({n, n}) != V->getShapeAsVector())
      throw std::runtime_error("svdJcb: wrong shape of V array !");
             ...
             std::vector<Nd4jLong>({n, minDim}) != V->getShapeAsVector())
      throw std::runtime_error("svdJcb: wrong shape of V array !");
  }
  ...
  // create cusolverDn handle
  cusolverDnHandle_t* handle =
      (cusolverDnHandle_t*)context->getCusolverHandle();
  // cusolverStatus_t status = cusolverDnCreate(&handle);
  if (handle == nullptr)
  ...
  const cusolverEigMode_t jobz =
      calcUV ? CUSOLVER_EIG_MODE_VECTOR : CUSOLVER_EIG_MODE_NOVECTOR;
  const int econ = !fullUV;

  if (transA) math::nd4j_swap<int>(m, n);
  ...
  if (!calcUV && m != n) {
    int maxDim = m > n ? m : n;
    arrToAvoidBugInAPI =
        new NDArray('c', {maxDim, maxDim}, pA->dataType(), context);
  ...
  if (A->dataType() == DataType::DOUBLE)
    status = cusolverDnDgesvdj_bufferSize(
        *handle, jobz, econ, m, n,
        reinterpret_cast<double*>(pA->specialBuffer()), lda,
        reinterpret_cast<double*>(pS->specialBuffer()),
        calcUV ? reinterpret_cast<double*>(pU->specialBuffer())
               : reinterpret_cast<double*>(nullPtr),
        ldu,
        calcUV ? reinterpret_cast<double*>(pV->specialBuffer())
               : reinterpret_cast<double*>(nullPtr),
        ldv, &lwork, gesvdjParams);
  else if (A->dataType() == DataType::FLOAT32)
    status = cusolverDnSgesvdj_bufferSize(
        *handle, jobz, econ, m, n,
        reinterpret_cast<float*>(pA->specialBuffer()), lda,
        reinterpret_cast<float*>(pS->specialBuffer()),
        calcUV ? reinterpret_cast<float*>(pU->specialBuffer())
               : reinterpret_cast<float*>(nullPtr),
        ldu,
        calcUV ? reinterpret_cast<float*>(pV->specialBuffer())
               : reinterpret_cast<float*>(nullPtr),
        ldv, &lwork, gesvdjParams);
  else
  ...
  if (A->dataType() == DataType::DOUBLE) {
    status = cusolverDnDgesvdj(
        *handle, jobz, econ, m, n,
        ...
        calcUV ? reinterpret_cast<double*>(pV->specialBuffer())
               : reinterpret_cast<double*>(nullPtr),
        ldv, reinterpret_cast<double*>(dWork), lwork, devInfo, gesvdjParams);
  } else if (A->dataType() == DataType::FLOAT32) {
    status = cusolverDnSgesvdj(
        *handle, jobz, econ, m, n,
        ...
        calcUV ? reinterpret_cast<float*>(pV->specialBuffer())
               : reinterpret_cast<float*>(nullPtr),
        ldv, reinterpret_cast<float*>(dWork), lwork, devInfo, gesvdjParams);
  } else
    throw std::invalid_argument("svdJcb: given data type is unsupported !");
...
//////////////////////////////////////////////////////////////////////////
static void svdBatched(sd::LaunchContext* context, const NDArray* A, NDArray* S,
                       NDArray* U, NDArray* V, const bool fullUV,
                       const bool calcUV) {
  // A [..., m, n]
  // S [..., n]
  ...
  if (m > 32 || n > 32)
    throw std::runtime_error(
        "svdBatched: numbers of rows and columns should be <= 32 !");
  ...
  if (A->dataType() == DataType::DOUBLE)
    status = cusolverDnDgesvdjBatched_bufferSize(
        handle, jobz, m, n, reinterpret_cast<double*>(pA->specialBuffer()), lda,
        reinterpret_cast<double*>(pS->specialBuffer()),
        calcUV ? reinterpret_cast<double*>(pU->specialBuffer()) : nullptr, ldu,
...
void svd(sd::LaunchContext* context, const NDArray* x,
  ...
  if (x->rankOf() == 2) {
    ...
    auto tadsX =
        x->allTensorsAlongDimension({x->rankOf() - 2, x->rankOf() - 1});
    auto tadsS = S->allTensorsAlongDimension({S->rankOf() - 1});

    if (calcUV) {
      tadsU = new ResultSet(
          U->allTensorsAlongDimension({U->rankOf() - 2, U->rankOf() - 1}));
      tadsV = new ResultSet(
          V->allTensorsAlongDimension({V->rankOf() - 2, V->rankOf() - 1}));
    }
