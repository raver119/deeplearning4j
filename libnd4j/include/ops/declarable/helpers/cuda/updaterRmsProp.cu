...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ void rmsPropUpdaterCuda(const void *vx, const Nd4jLong *xShapeInfo,
                                   const void *vin, const Nd4jLong *inShapeInfo,
                                   void *vz, const Nd4jLong *zShapeInfo,
                                   void *vst, const Nd4jLong *stShapeInfo,
                                   const T lr, const T rmsDecay,
                                   const T epsilon) {
  ...
  if (threadIdx.x == 0) {
    ...
    bEWS = 1 == shape::elementWiseStride(xShapeInfo) &&
           ...
           1 == shape::elementWiseStride(stShapeInfo) &&
           1 == shape::elementWiseStride(inShapeInfo);
  ...
  for (Nd4jLong i = blockIdx.x * blockDim.x + threadIdx.x; i < xLen;
    ...
    st[stOffset] =
        init[initOffset] * rmsDecay + x[xOffset] * x[xOffset] * (1 - rmsDecay);
    up[zOffset] =
        (lr * x[xOffset]) / (math::nd4j_sqrt<T, T>(st[stOffset]) + epsilon);
  }
}
...
linkage void rmsPropUpdaterCudaLauncher(
    ...
    const void *vin, const Nd4jLong *inShapeInfo, void *vz,
    const Nd4jLong *zShapeInfo, void *vst, const Nd4jLong *stShapeInfo,
  ...
  rmsPropUpdaterCuda<T><<<blocksPerGrid, threadsPerBlock, 256, *stream>>>(
      vx, xShapeInfo, vin, inShapeInfo, vz, zShapeInfo, vst, stShapeInfo, lr,
      rmsDecay, epsilon);
}
...
void updaterRmsProp(sd::LaunchContext *context, const NDArray &gradient,
  ...
  BUILD_SINGLE_SELECTOR(gradient.dataType(), rmsPropUpdaterCudaLauncher,
                        (blocksPerGrid, threadsPerBlock,
                         context->getCudaStream(), gradient.specialBuffer(),
                         gradient.specialShapeInfo(), initState.specialBuffer(),
                         initState.specialShapeInfo(), update.specialBuffer(),
                         update.specialShapeInfo(), stateG.specialBuffer(),
                         stateG.specialShapeInfo(), dLr, dRmsDecay, dEpsilon),
                        FLOAT_TYPES);
