...
//
template <typename T>
static __host__ __device__ void nudge(T min, T max, int quantMin, int quantMax,
                                      T* scale, T* nudgedMin, T* nudgedMax) {
  ...
  *scale = (max - min) / (quantMaxF - quantMinF);
  ...
  uint16_t const nudgedZeroPoint = [zeroPointFromMin, quantMin, quantMax,
                                    quantMaxF, quantMinF] {
...
template <typename T>
void fakeQuantWithMinMaxVars_(NDArray* input, NDArray* min, NDArray* max,
  ...
  nudge(min->t<T>(0), max->t<T>(0), lowIntBound, upperIntBound, &scale,
        &nudgedMin, &nudgedMax);
  ...
  auto wiseMinMaxAndSoOn = LAMBDA_T(x, nudgedMin, nudgedMax, scale) {
    ...
    if (x < nudgedMin) {
      ...
      val = x;
    return (math::nd4j_floor<T, T>((val - nudgedMin) / scale + T(0.5)) * scale +
            nudgedMin);
  };
...
template <typename T>
static __global__ void fakeQuantWithMinMaxKernel(
    const T* input, const Nd4jLong* inputShape, T* min, T* max, int lowIntBound,
    int upperIntBound, Nd4jLong channels, T* output,
    const Nd4jLong* outputShape, Nd4jLong length) {
  ...
  for (auto i = blockIdx.x; i < (int)channels; i += gridDim.x) {
    T scale, nudgedMin, nudgedMax;
    nudge(min[i], max[i], lowIntBound, upperIntBound, &scale, &nudgedMin,
          &nudgedMax);
    ...
    for (auto b = threadIdx.x; b < block; b += blockDim.x) {
      ...
      output[shape::getIndexOffset(b * channels + i, outputShape)] =
          (math::nd4j_floor<T, T>((val - nudgedMin) / scale + T(0.5f)) * scale +
           nudgedMin);
    };
  }
}
...
template <typename T>
void fakeQuantWithMinMaxVarsPerChannel_(LaunchContext* context, NDArray* input,
  ...
  fakeQuantWithMinMaxKernel<<<128, 256, 256, *stream>>>(
      inputBuf, input->specialShapeInfo(), minBuf, maxBuf, lowIntBound,
      upperIntBound, channels, outputBuf, output->specialShapeInfo(), length);
  NDArray::registerSpecialUse({output}, {min, max, input});
}

void fakeQuantWithMinMaxVars(NDArray* input, NDArray* min, NDArray* max,
                             int numBits, bool narrowed, NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), fakeQuantWithMinMaxVars_,
                        (input, min, max, numBits, narrowed, output),
                        FLOAT_TYPES);
}
void fakeQuantWithMinMaxVarsPerChannel(LaunchContext* context, NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), fakeQuantWithMinMaxVarsPerChannel_,
                        (context, input, min, max, numBits, narrowed, output),
                        FLOAT_TYPES);
}
}  // namespace helpers
}  // namespace ops
