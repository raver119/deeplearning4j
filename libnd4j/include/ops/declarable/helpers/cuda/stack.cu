...
///////////////////////////////////////////////////////////////////
template <typename T>
static __global__ void stackScalarsCuda(void* pVx, void* vz,
...
///////////////////////////////////////////////////////////////////
template <typename T>
__host__ static void stackScalarsCudaLauncher(const int blocksPerGrid,
                                              const int threadsPerBlock,
                                              const cudaStream_t* stream,
                                              void* pVx, void* vz,
                                              const Nd4jLong* zShapeInfo) {
  stackScalarsCuda<T>
      <<<blocksPerGrid, threadsPerBlock, 256, *stream>>>(pVx, vz, zShapeInfo);
}

///////////////////////////////////////////////////////////////////
template <typename T>
static void stack_(sd::LaunchContext* context,
  ...
  if (inArrs[0]->rankOf() == 0) {
    ...
    void* dInBuffers = manager.replicatePointer(
        hInBuffers.data(), hInBuffers.size() * sizeof(void*));

    const int threadsPerBlock = MAX_NUM_THREADS / 2;
    const int blocksPerGrid =
        (output.lengthOf() + threadsPerBlock - 1) / threadsPerBlock;

    stackScalarsCudaLauncher<T>(
    ...
    for (uint i = 0; i < numOfSubArrs; ++i) {
      void* zBuff =
          output.specialBufferWithOffset(zTadPack.primaryOffsets()[i]);

      NativeOpExecutioner::execTransformAny(
          context, transform::Assign, nullptr, inArrs[i]->shapeInfo(),
...
////////////////////////////////////////////////////////////////////////
void stack(sd::LaunchContext* context,
           const std::vector<const NDArray*>& inArrs, NDArray& output,
           const int dim) {
  BUILD_SINGLE_SELECTOR(output.dataType(), stack_,
                        (context, inArrs, output, dim), LIBND4J_TYPES);
}
...
///////////////////////////////////////////////////////////////////
template <typename T>
static __global__ void unstackScalarsCuda(const void* vx,
...
///////////////////////////////////////////////////////////////////
template <typename T>
__host__ static void unstackScalarsCudaLauncher(const int blocksPerGrid,
                                                const int threadsPerBlock,
                                                const cudaStream_t* stream,
                                                const void* vx,
                                                const Nd4jLong* xShapeInfo,
                                                void* pVz) {
  unstackScalarsCuda<T>
      <<<blocksPerGrid, threadsPerBlock, 256, *stream>>>(vx, xShapeInfo, pVz);
}

///////////////////////////////////////////////////////////////////
template <typename T>
static void unstack_(sd::LaunchContext* context, const NDArray& input,
  ...
  if (outArrs[0]->rankOf() == 0) {
    ...
    void* dOutBuffers = manager.replicatePointer(
        hOutBuffers.data(), hOutBuffers.size() * sizeof(void*));

    const int threadsPerBlock = MAX_NUM_THREADS / 2;
    const int blocksPerGrid =
        (input.lengthOf() + threadsPerBlock - 1) / threadsPerBlock;

    unstackScalarsCudaLauncher<T>(
    ...
    for (uint i = 0; i < numOfSubArrs; ++i) {
      ...
      NativeOpExecutioner::execTransformAny(
          input.getContext(), transform::Assign, nullptr, xTadShapeInfo, xBuff,
...
////////////////////////////////////////////////////////////////////////
void unstack(sd::LaunchContext* context, const NDArray& input,
             const std::vector<NDArray*>& outArrs, const int dim) {
  BUILD_SINGLE_SELECTOR(input.dataType(), unstack_,
                        (context, input, outArrs, dim), LIBND4J_TYPES);
}
BUILD_SINGLE_TEMPLATE(template void unstack_,
                      (sd::LaunchContext * context, const NDArray& input,
                       const std::vector<NDArray*>& outArrs, const int dim),
                      LIBND4J_TYPES);

///////////////////////////////////////////////////////////////////
// template <typename T>
// static __global__ void unstackCuda(const void* vx, const Nd4jLong*
...
//         T *z = reinterpret_cast<T*>(reinterpret_cast<void
//         **>(pVz)[coords[axis]]);

//         for (uint j = axis; j < xRank - 1; ++j)	// shift coords staring
//         from axis position 	coords[j] = coords[j + 1];
...
// }
// BUILD_SINGLE_TEMPLATE(template void unstackCudaLauncher, (const int
// blocksPerGrid, const int threadsPerBlock, const cudaStream_t *stream, const
// void* vx, const Nd4jLong* xShapeInfo, void* pVz, const Nd4jLong*
// zTadShapeInfo, const int axis), LIBND4J_TYPES);

// ///////////////////////////////////////////////////////////////////
// void unstack(sd::LaunchContext* context, const NDArray& input, const
// std::vector<const NDArray*>& outArrs, const int axis) {

// 	const int threadsPerBlock = MAX_NUM_THREADS / 2;
// 	const int blocksPerGrid = (input.lengthOf() + threadsPerBlock - 1) /
// threadsPerBlock;
...
//     void* dOutBuffers = manager.replicatePointer(hOutBuffers.data(),
//     hOutBuffers.size() * sizeof(void*));
...
// ///////////////////////////////////////////////////////////////////
// template <typename T>
// static __global__ void stackCuda(void* pVx, const Nd4jLong* xTadShapeInfo,
// void* vz, const Nd4jLong* zShapeInfo, const int axis) {
...
//         const T *x = reinterpret_cast<const
//         T*>(reinterpret_cast<void**>(pVx)[coords[axis]]);

//         for (uint j = axis; j < zRank - 1; ++j)	// shift coords staring
//         from axis position 	coords[j] = coords[j + 1];
...
// }
// BUILD_SINGLE_TEMPLATE(template void stackCudaLauncher, (const int
// blocksPerGrid, const int threadsPerBlock, const cudaStream_t *stream, void*
// pVx, const Nd4jLong* xTadShapeInfo, void* vz, const Nd4jLong* zShapeInfo,
// const int axis), LIBND4J_TYPES);

// ///////////////////////////////////////////////////////////////////
// void stack(sd::LaunchContext* context, const std::vector<const NDArray*>&
// inArrs, NDArray& output, const int axis) {

// 	const int threadsPerBlock = MAX_NUM_THREADS / 2;
// 	const int blocksPerGrid = (output.lengthOf() + threadsPerBlock - 1) /
// threadsPerBlock;
...
//     void* dInBuffers = manager.replicatePointer(hInBuffers.data(),
//     hInBuffers.size() * sizeof(void*));
