...
static __global__ void segmentMaxLinearKernel(
  ...
  if (threadIdx.x == 0) {
    ...
    //                    numOfClasses; segment = blockIdx.x /
    //                    threadsPerSegment;
...
template <typename T, typename I>
static __global__ void unsortedSegmentMaxLinearKernel(
...
// //
template <typename T, typename I>
static __global__ void segmentMaxTadKernel(
...
template <typename T, typename I>
static void segmentMaxFunctor_(LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    segmentMaxLinearKernel<T, I>
        <<<numOfClasses, input->lengthOf(), numOfClasses * 32 + 32, *stream>>>(
...
void segmentMaxFunctor(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(input->dataType(), indices->dataType(),
                        segmentMaxFunctor_, (context, input, indices, output),
...
template <typename T, typename I>
static void unsortedSegmentMaxFunctor_(sd::LaunchContext* context,
...
// //
template <typename T, typename I>
static __global__ void segmentMaxBPLinearKernel(
...
// //
template <typename T, typename I>
static __global__ void segmentMaxBPTadKernel(
    void* inputBuf, Nd4jLong const* inputShape, void* forwardOutput,
    Nd4jLong const* forwardShape, void* eps, Nd4jLong const* epsShape,
    void* indicesBuf, Nd4jLong const* indicesShape, void* outputBuf,
...
// //
template <typename T, typename I>
int segmentMaxFunctorBP_(sd::LaunchContext* context, NDArray* input,
  ...
  if (input->isVector()) {
    ...
    segmentMaxBPLinearKernel<T, I>
        <<<1 + gradOut->lengthOf(), input->lengthOf(), 256, *stream>>>(
            input->specialBuffer(), input->specialShapeInfo(),
...
int segmentMaxFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(
      output->dataType(), indices->dataType(), return segmentMaxFunctorBP_,
...
// //
template <typename T, typename I>
static int unsortedSegmentMaxFunctorBP_(sd::LaunchContext* context,
                                        NDArray* input, NDArray* indices,
  ...
  NDArray tempRes(gradOut->ordering(), gradOut->getShapeAsVector(),
                  DataTypeUtils::fromT<T>(),
                  context);  //->shapeInfo(), context);
  unsortedSegmentMaxFunctor_<T, I>(context, input, indices, numOfClasses,
                                   &tempRes);
  NDArray::prepareSpecialUse({output}, {input, indices, gradOut, &tempRes});
  if (input->isVector()) {
    Nd4jLong loop_size = input->lengthOf();
...
int unsortedSegmentMaxFunctorBP(sd::LaunchContext* context, NDArray* input,
  ...
  BUILD_DOUBLE_SELECTOR(
      ...
      return unsortedSegmentMaxFunctorBP_,
