...
// reference: Lentz, W.J. 1976, “Generating Bessel Functions in Mie Scattering
...
__device__ T continuedFractionCuda(const T a, const T b, const T x) {
  ...
  T t2 = coeffs[1];
  T t1 = coeffs[0];
  if (math::nd4j_abs<T>(t1) < min) t1 = min;
  ...
  for (uint i = 1; i <= maxIter; ++i) {
    ...
    if (math::nd4j_abs<T>(t2) < min) t2 = min;
    // result
    result *= t2 * t1;
    // t1
  ...
  return DataTypeUtils::infOrMax<T>();  // no convergence, more iterations is
                                        // required, return infinity
}
...
__global__ void betaIncForArrayCuda(const void* va, const Nd4jLong* aShapeInfo,
  ...
  if (threadIdx.x == 0) {
    a = *(reinterpret_cast<const T*>(va) +
          shape::getIndexOffset(j, aShapeInfo));
    b = *(reinterpret_cast<const T*>(vb) +
          shape::getIndexOffset(j, bShapeInfo));
    x = *(reinterpret_cast<const T*>(vx) +
          shape::getIndexOffset(j, xShapeInfo));
  ...
  if (threadIdx.x != 0) {
    ...
    sharedMem[2 * i] =
        i * (b - i) * x / ((aPlus2i - static_cast<T>(1)) * aPlus2i);
    sharedMem[2 * i + 1] =
        -(a + i) * (a + b + i) * x / ((aPlus2i + static_cast<T>(1)) * aPlus2i);
  }
  ...
  if (threadIdx.x == 0) {
    ...
    const T front =
        math::nd4j_exp<T, T>(math::nd4j_log<T, T>(x) * a +
                             math::nd4j_log<T, T>(1.f - x) * b - gammaPart);
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void betaIncForArrayCudaLauncher(
    const int blocksPerGrid, const int threadsPerBlock, const int sharedMem,
    const cudaStream_t* stream, const void* va, const Nd4jLong* aShapeInfo,
    const void* vb, const Nd4jLong* bShapeInfo, const void* vx,
    const Nd4jLong* xShapeInfo, void* vz, const Nd4jLong* zShapeInfo) {
  betaIncForArrayCuda<T>
      <<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
          va, aShapeInfo, vb, bShapeInfo, vx, xShapeInfo, vz, zShapeInfo);
}
...
void betaInc(sd::LaunchContext* context, const NDArray& a, const NDArray& b,
  ...
  BUILD_SINGLE_SELECTOR(
      xType, betaIncForArrayCudaLauncher,
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(),
       a.specialBuffer(), a.specialShapeInfo(), b.specialBuffer(),
       b.specialShapeInfo(), x.specialBuffer(), x.specialShapeInfo(),
