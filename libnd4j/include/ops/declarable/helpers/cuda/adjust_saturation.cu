...
///////////////////////////////////////////////////////////////////
template <typename T>
static void _CUDA_G adjustSaturationCuda(
...
///////////////////////////////////////////////////////////////////
template <typename T>
static _CUDA_H void adjustSaturationCudaLauncher(
    ...
    const Nd4jLong* zTadOffsets, const Nd4jLong numOfTads,
...
void adjustSaturation(sd::LaunchContext* context, const NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(
      input->dataType(), adjustSaturationCudaLauncher,
      (blocksPerGrid, threadsPerBlock, context->getCudaStream(),
       ...
       output->specialShapeInfo(), packZ.platformOffsets(), numOfTads,
       factorScalarArr, dimC),
      FLOAT_TYPES);
...
/*
template <typename T>
static void _CUDA_G adjustSaturationSingleNHWCKernel(void *xBuffer, Nd4jLong
*xShapeInfo,  void *zBuffer, Nd4jLong *zShapeInfo, Nd4jLong tuples, float delta)
{ int numChannels = 3; auto tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    for (Nd4jLong e = tid; e < tuples; e += blockDim.x * gridDim.x) {
        ...
        s = sd::math::nd4j_min<T>((T) 1.0f, sd::math::nd4j_max<T>((T) 0.0f, s *
delta));
...
template <typename T>
static void _CUDA_G adjustSaturationSingleNCHWKernel(void *xBuffer, Nd4jLong
*xTadShapeInfo, Nd4jLong *xOffsets, void *zBuffer, Nd4jLong *zTadShapeInfo,
Nd4jLong *zOffsets, Nd4jLong tadLength, Nd4jLong tuples, float delta) { int
numChannels = 3; auto tid = threadIdx.x + blockIdx.x * blockDim.x; static const
int kChannelRange = 6;
...
template <typename T>
static void _adjust_saturation_single(sd::LaunchContext * context, NDArray
*array, NDArray *output, float delta, bool isNHWC) {
...
template <typename T>
static void _adjust_saturation_batch(sd::LaunchContext * context, NDArray
*array, NDArray *output, float delta, bool isNHWC) { auto xType =
array->dataType();
    ...
    if (isNHWC) {
        // in case of nhwc batch, we don't really care about examples: it's
still bunch of RGB values BUILD_SINGLE_SELECTOR(xType,
_adjust_saturation_single, (context, array, output, delta, isNHWC);,
FLOAT_TYPES); } else {
        // TODO: check this one
        auto packX =
...
void adjust_saturation(sd::LaunchContext * context, NDArray *array, NDArray
*output, NDArray* delta, bool isNHWC) { auto xType = array->dataType();

    float d = delta->e<float>(0);
    if (array->rankOf() == 4) {
        BUILD_SINGLE_SELECTOR(xType, _adjust_saturation_batch, (context, array,
output, d, isNHWC);, FLOAT_TYPES); } else { BUILD_SINGLE_SELECTOR(xType,
_adjust_saturation_single, (context, array, output, d, isNHWC);, FLOAT_TYPES);
    }
}
*/
