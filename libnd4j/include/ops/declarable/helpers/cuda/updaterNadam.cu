...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ void nadamUpdaterCuda(const void* vx, const Nd4jLong* xShapeInfo,
                                 const void* vinv, const Nd4jLong* invShapeInfo,
                                 const void* vinm, const Nd4jLong* inmShapeInfo,
                                 void* vz, const Nd4jLong* zShapeInfo,
                                 void* vstV, const Nd4jLong* stvShapeInfo,
                                 void* vstM, const Nd4jLong* stmShapeInfo,
  ...
  for (Nd4jLong i = blockIdx.x * blockDim.x + threadIdx.x; i < xLen;
    ...
    auto xOffset = i, zOffset = i, initMOffset = i, initUOffset = i,
         stMOffset = i, stUOffset = i;
    ...
    stV[stUOffset] =
        beta2 * initV[initUOffset] + grad[xOffset] * grad[xOffset] * mbeta2;

    up[zOffset] =
        (lr * ((stM[stMOffset] * beta1 + oneMinusBeta1Grad) / mbeta1T)) /
        (sd::math::nd4j_sqrt<T, T>(stV[stUOffset]) + epsilon);
  }
}

///////////////////////////////////////////////////////////////////
template <typename T>
linkage void nadamUpdaterCudaLauncher(
    ...
    const Nd4jLong* inmShapeInfo, void* vz, const Nd4jLong* zShapeInfo,
    void* vstV, const Nd4jLong* stvShapeInfo, void* vstM,
    const Nd4jLong* stmShapeInfo, const double dLr, const double dBeta1,
  ...
  nadamUpdaterCuda<T><<<blocksPerGrid, threadsPerBlock, 256, *stream>>>(
      vx, xShapeInfo, vinv, invShapeInfo, vinm, inmShapeInfo, vz, zShapeInfo,
      vstV, stvShapeInfo, vstM, stmShapeInfo, lr, beta1, beta2, epsilon,
      iteration);
}
...
void updaterNadam(sd::LaunchContext* context, const NDArray& gradient,
  ...
  NDArray::prepareSpecialUse({&update, &stateV, &stateM},
                             {&gradient, &initStateV, &initStateM});
  BUILD_SINGLE_SELECTOR(
      gradient.dataType(), nadamUpdaterCudaLauncher,
      (blocksPerGrid, threadsPerBlock, context->getCudaStream(),
       ...
       stateM.specialBuffer(), stateM.specialShapeInfo(), dLr, dBeta1, dBeta2,
       dEpsilon, nIteration),
      FLOAT_TYPES);
  NDArray::registerSpecialUse({&update, &stateV, &stateM},
                              {&gradient, &initStateV, &initStateM});
