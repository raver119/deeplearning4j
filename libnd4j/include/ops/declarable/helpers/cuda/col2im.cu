...
//////////////////////////////////////////////////////////////////////////
// columns [bS, iC, kH, kW, oH, oW] to be de-convoluted to image [bS, iC, iH,
// iW]
template <typename T>
static __global__ void col2imCuda(const void* columns,
                                  const Nd4jLong* colShapeInfo, void* image,
                                  const Nd4jLong* imShapeInfo, const int sH,
  ...
  for (Nd4jLong i = tid; i < imLen; i += gridDim.x * blockDim.x) {
    ...
    const auto bSiCoffset =
        coords[0] * colShapeInfo[7] + coords[1] * colShapeInfo[8];
    ...
    for (coords[4] = colHstart; coords[4] < colHend; ++coords[4]) {
      ...
      for (coords[5] = colWstart; coords[5] < colWend; ++coords[5]) {
        ...
        val += col[bSiCoffset + (coords[2] / dH) * colShapeInfo[9] +
                   (coords[3] / dW) * colShapeInfo[10] +
...
// iW]
template <typename T>
__global__ static void col2imCuda2(const void* columns, void* image,
                                   const Nd4jLong* colShapeInfo,
                                   const Nd4jLong* imShapeInfo, const int sH,
                                   const int sW, const int pH, const int pW,
                                   const int dH, const int dW) {
    ...
    for (int colH = h_col_start; colH < h_col_end; colH += 1) {
      for (int colW = w_col_start; colW < w_col_end; colW += 1) {
        ...
        if (kRow % dH == 0 && kCol % dW == 0) {
          ...
          int data_col_index = b * colStride0 + c * colStride1 +
                               kRow * colStride2 + kCol * colStride3 +
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void col2imCudaLauncher(const int blocksPerGrid,
                               const int threadsPerBlock, const int sharedMem,
                               const cudaStream_t* stream, const void* columns,
                               const Nd4jLong* colShapeInfo, void* image,
                               const Nd4jLong* imShapeInfo, const int sH,
                               const int sW, const int pH, const int pW,
                               const int dH, const int dW) {
  // col2imCuda2<T><<<512, 512, 1024, *stream>>>(columns, image, colShapeInfo,
  // imShapeInfo, sH, sW, pH, pW, dH, dW);
  col2imCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      columns, colShapeInfo, image, imShapeInfo, sH, sW, pH, pW, dH, dW);
}

//////////////////////////////////////////////////////////////////////////
void col2im(sd::LaunchContext& context, const NDArray& col, NDArray& im,
            const int sH, const int sW, const int pH, const int pW,
            const int iH, const int iW, const int dH, const int dW) {
  PointersManager manager(&context, "col2im");

  const int threadsPerBlock = MAX_NUM_THREADS / 2;
  const int blocksPerGrid =
      (im.lengthOf() + threadsPerBlock - 1) / threadsPerBlock;
  const int sharedMem = col.rankOf() * sizeof(uint) * threadsPerBlock + 256;

  NDArray::prepareSpecialUse({&im}, {&col});
  BUILD_SINGLE_SELECTOR(
      im.dataType(), col2imCudaLauncher,
      (blocksPerGrid, threadsPerBlock, sharedMem, context.getCudaStream(),
       col.specialBuffer(), col.specialShapeInfo(), im.specialBuffer(),
       im.specialShapeInfo(), sH, sW, pH, pW, dH, dW),
      FLOAT_TYPES);
