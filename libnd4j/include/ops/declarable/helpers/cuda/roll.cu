...
template <typename T>
static void _CUDA_D rollKernelLinearStage1Dev(
    const void *vx, const Nd4jLong *xShapeInfo, void *vz,
    const Nd4jLong *zShapeInfo, Nd4jLong fullLength, int actualShift) {
...
template <typename T>
static void _CUDA_G rollKernelLinearStage1(const void *vx,
                                           const Nd4jLong *xShapeInfo, void *vz,
                                           const Nd4jLong *zShapeInfo,
                                           Nd4jLong fullLength,
                                           int actualShift) {
  rollKernelLinearStage1Dev<T>(vx, xShapeInfo, vz, zShapeInfo, fullLength,
                               actualShift);
}

template <typename T>
static void _CUDA_G rollKernelLinearStage2(const void *vx,
                                           const Nd4jLong *xShapeInfo, void *vz,
                                           const Nd4jLong *zShapeInfo,
                                           Nd4jLong fullLength, int actualShift,
                                           int shiftCount) {
  auto x = reinterpret_cast<const T *>(vx);
  auto z = reinterpret_cast<T *>(vz);
...
template <typename T>
static void _CUDA_G rollKernelLinearStage3(const void *vx,
                                           const Nd4jLong *xShapeInfo, void *vz,
                                           const Nd4jLong *zShapeInfo,
                                           Nd4jLong fullLength, int actualShift,
                                           int remainShift) {
...
template <typename T>
static void _CUDA_D swapTadsKernel(void *vx, void *vz,
                                   const Nd4jLong *zShapeInfo,
                                   Nd4jLong tadLength) {
...
template <typename T>
static void _CUDA_G rollKernelFullAnyDimensionStage1(
    ...
    int numTads, Nd4jLong tadLength, int dim, Nd4jLong sizeAt, int theShift) {
  auto x = reinterpret_cast<const T *>(vx);
  auto z = reinterpret_cast<T *>(vz);
  ...
  for (int e = blockIdx.x + theShift; e < sizeAt - theShift; e += gridDim.x) {
    ...
    swapTadsKernel<T>(z + xTadOffsets[sourceIndex],
                      z + xTadOffsets[targetIndex], zTadShapeInfo, tadLength);
  }
}

template <typename T>
static void _CUDA_G rollKernelFullAnyDimensionStage2(
    void *vx, const Nd4jLong *xTadShapeInfo, const Nd4jLong *xTadOffsets,
    void *vz, const Nd4jLong *zTadShapeInfo, const Nd4jLong *zTadOffsets,
    int numTads, Nd4jLong tadLength, int dim, Nd4jLong sizeAt, int theShift) {
  auto x = reinterpret_cast<const T *>(vx);
  auto z = reinterpret_cast<T *>(vz);
  ...
  for (int e = blockIdx.x; e < theShift; e += gridDim.x) {
    ...
    swapTadsKernel<T>(z + zTadOffsets[sourceIndex],
                      z + zTadOffsets[targetIndex], zTadShapeInfo, tadLength);
  }
}

template <typename T>
static void rollFunctorFull_(NDArray *input, NDArray *output,
    ...
    if (axe == input->rankOf() - 1) {  // last dimension
      ...
      if (theShift) {
        for (int dim = 0; dim < numTads / sizeAt; ++dim) {
          rollKernelFullAnyDimensionStage1<T>
              <<<1, 256, 1024, *(output->getContext()->getCudaStream())>>>(
                  output->specialBuffer(), packZ.platformShapeInfo(),
                  packZ.platformOffsets(), output->specialBuffer(),
                  packZ.platformShapeInfo(), packZ.platformOffsets(), numTads,
                  tadLength, dim, sizeAt, theShift);

          rollKernelFullAnyDimensionStage2<T>
              <<<1, 256, 1024, *(output->getContext()->getCudaStream())>>>(
                  output->specialBuffer(), packZ.platformShapeInfo(),
                  packZ.platformOffsets(), output->specialBuffer(),
                  packZ.platformShapeInfo(), packZ.platformOffsets(), numTads,
...
template <typename T>
static void rollFunctorLinear_(NDArray *input, NDArray *output, int shift,
                               bool inplace) {
  if (!inplace) output->assign(input);
  ...
  if (actualShift) {
    ...
    rollKernelLinearStage1<T>
        <<<1, 1, 1024, *(output->getContext()->getCudaStream())>>>(
            output->specialBuffer(), output->specialShapeInfo(),
            output->specialBuffer(), output->specialShapeInfo(), fullLen,
            actualShift);

    // stage 2) swap swapped actualShift elements with rest remainShiftCount
    // times.
    rollKernelLinearStage2<T>
        <<<1, 1, 1024, *(output->getContext()->getCudaStream())>>>(
            output->specialBuffer(), output->specialShapeInfo(),
            output->specialBuffer(), output->specialShapeInfo(), fullLen,
            actualShift, shiftCount);

    // FIXME: no parallelism here :(
    // stage 3) swap remainer of items.
    if (remainShift && shiftCount)
      rollKernelLinearStage3<T>
          <<<1, 1, 1024, *(output->getContext()->getCudaStream())>>>(
              output->specialBuffer(), output->specialShapeInfo(),
              output->specialBuffer(), output->specialShapeInfo(), fullLen,
              actualShift, remainShift);
  }
}
...
void rollFunctorFull(sd::LaunchContext *context, NDArray *input,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), rollFunctorFull_,
                        (input, output, shifts, axes, inplace), LIBND4J_TYPES);

  output->tickWriteDevice();
}

void rollFunctorLinear(sd::LaunchContext *context, NDArray *input,
                       NDArray *output, int shift, bool inplace) {
  input->syncToDevice();

  BUILD_SINGLE_SELECTOR(input->dataType(), rollFunctorLinear_,
                        (input, output, shift, inplace), LIBND4J_TYPES);

  output->tickWriteDevice();
}

BUILD_SINGLE_TEMPLATE(template void rollFunctorLinear_,
                      (NDArray * input, NDArray *output, int shift,
                       bool inplace),
                      LIBND4J_TYPES);
