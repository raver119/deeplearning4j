...
//////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void pooling3dCuda(const void* vx, const Nd4jLong* xShapeInfo,
  ...
  switch (poolingMode) {
    ...
    case 1: {
      ...
      if (extraParam0 == 0) {  // Exclude padding
        ...
        sum /= static_cast<T>(
            a * b *
            c);  //  /= sd::math::nd4j_ceil<double,T>(static_cast<double>(dend -
                 //  dstart) / static_cast<double>(dD)) *
                 //  sd::math::nd4j_ceil<double,T>(static_cast<double>(hend -
                 //  hstart) / static_cast<double>(dH)) *
                 //  sd::math::nd4j_ceil<double,T>(static_cast<double>(wend -
                 //  wstart) / static_cast<double>(dW));   //Accounts for
                 //  dilation
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void pooling3dCudaLauncher(
    ...
    void* vz, const Nd4jLong* zShapeInfo, const int kD, const int kH,
    const int kW, const int sD, const int sH, const int sW, const int pD,
  ...
  pooling3dCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, vz, zShapeInfo, kD, kH, kW, sD, sH, sW, pD, pH, pW, dD,
      dH, dW, poolingMode, extraParam0);
}

//////////////////////////////////////////////////////////////////////////
void ConvolutionUtils::pooling3d(sd::graph::Context& block,
  ...
  const int sharedMem =
      output.rankOf() * sizeof(Nd4jLong) * threadsPerBlock + 128;
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem,
       ...
       output.specialShapeInfo(), kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH,
       dW, poolingMode, extraParam0),
      FLOAT_TYPES);
