...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void splitCuda(const void* vx, const Nd4jLong* xShapeInfo,
                                 void* pVz, const Nd4jLong* zTadShapeInfo,
                                 const int axis) {
  ...
  for (uint64_t i = tid; i < xLen; i += totalThreads) {
    ...
    auto* z = reinterpret_cast<T*>(
        reinterpret_cast<void**>(pVz)[coords[axis] / zDim]);
...
///////////////////////////////////////////////////////////////////
template <typename T>
__host__ static void splitCudaLauncher(
    const int blocksPerGrid, const int threadsPerBlock,
    const cudaStream_t* stream, const void* vx, const Nd4jLong* xShapeInfo,
    void* pVz, const Nd4jLong* zTadShapeInfo, const int axis) {
  splitCuda<T><<<blocksPerGrid, threadsPerBlock, 256, *stream>>>(
      vx, xShapeInfo, pVz, zTadShapeInfo, axis);
}
BUILD_SINGLE_TEMPLATE(template void splitCudaLauncher,
                      (const int blocksPerGrid, const int threadsPerBlock,
                       const cudaStream_t* stream, const void* vx,
                       const Nd4jLong* xShapeInfo, void* pVz,
                       const Nd4jLong* zTadShapeInfo, const int axis),
                      LIBND4J_TYPES);
...
void split(sd::LaunchContext* context, const NDArray& input,
  ...
  if (luckCase1) {  // for example {1,10} + {2,10} + {3,10} = {6, 10} order c;
    ...
    for (uint i = 0; i < numOfSubArrs; ++i) {
      ...
      cudaMemcpyAsync(static_cast<int8_t*>(outArrs[i]->specialBuffer()), x,
                      memAmountToCopy, cudaMemcpyDeviceToDevice,
  ...
  // here axis 1 shoud have stride = 1 for all inputs arrays and input array

  //     const auto xStep = shape::strideOverContigAxis(axis,
  //     input.shapeInfo()); const auto zDim = outArrs[0]->sizeAt(axis);     //
  //     same for all outArrs

  //     for (uint i = 0; i < input.lengthOf() / input.sizeAt(axis); ++i) {

  //         const auto iShift = i * sizeofT;
  //         void* x = static_cast<int8_t*>(input.specialBuffer()) + xStep *
  //         iShift;

  //         for (uint j = 0; j < numOfSubArrs; ++j) {
  //             void* z = static_cast<int8_t*>(outArrs[j]->specialBuffer()) +
  //             strideOfContigStride[j] * iShift; const auto memSizeToCopy =
  //             zDim * sizeofT; cudaMemcpyAsync(z, x, memSizeToCopy,
  //             cudaMemcpyDeviceToDevice, *context->getCudaStream()); x =
  //             static_cast<int8_t*>(x) + memSizeToCopy;
  //         }
  //     }
