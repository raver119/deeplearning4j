...
static __global__ void computeInterpolationWeights(
    Nd4jLong outSize, Nd4jLong inSize, double scale, Nd4jLong channels,
  ...
  for (Nd4jLong i = outSize - tid; i >= 0; i -= step) {
    ...
    //            interpolationData[i].topIndex =
    //            sd::math::nd4j_min(interpolationData[i].bottomIndex + 1,
    //            inSize - 1); interpolationData[i].interpolarValue = in -
    //            interpolationData[i].bottomIndex;
    double const in_f = sd::math::p_floor<double>(in);
    double const in_c = sd::math::p_ceil<double>(in);
    interpolationData[i].bottomIndex =
        sd::math::nd4j_max(static_cast<Nd4jLong>(in_f),
                           (Nd4jLong)0LL);  // static_cast<Nd4jLong>(in);
    interpolationData[i].topIndex =
        sd::math::nd4j_min(static_cast<Nd4jLong>(in_c), inSize - 1);
    interpolationData[i].interpolarValue = in - in_f;

    if (channels) {
      math::atomics::nd4j_atomicMul(&interpolationData[i].bottomIndex,
                                    channels);
...
//
static void resizeImage(sd::LaunchContext* context, NDArray const* images,
                        Nd4jLong batchSize, Nd4jLong inHeight, Nd4jLong inWidth,
                        Nd4jLong outHeight, Nd4jLong outWidth,
                        Nd4jLong channels, BilinearInterpolationData* xs_,
...
static __global__ void resizeImageKernel(
    T const* input, Nd4jLong const* inputShape, Z* outputYptr,
    Nd4jLong const* outputShape, Nd4jLong batchSize, Nd4jLong outWidth,
    Nd4jLong outHeight, Nd4jLong channels, Nd4jLong inRowSize,
    Nd4jLong outRowSize, Nd4jLong inBatchNumValues,
    BilinearInterpolationData* xs_, BilinearInterpolationData* ys_) {
  for (auto batch = blockIdx.x; batch < batchSize;
       batch += gridDim.x) {  // blockIdx.x as batch index
...
static void resizeImage_(sd::LaunchContext* context, NDArray const* images,
                         Nd4jLong batchSize, Nd4jLong inHeight,
                         Nd4jLong inWidth, Nd4jLong outHeight,
  ...
  resizeImageKernel<T, F><<<256, 256, 256, *stream>>>(
      pInput, images->specialShapeInfo(), pOutput, output->specialShapeInfo(),
      batchSize, outWidth, outHeight, channels, inRowSize, outRowSize,
      inBatchNumValues, xs_, ys_);

  auto err = cudaStreamSynchronize(*stream);
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::resizeImage_: Cannot synchronize kernel execution", err);
  }
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T, typename F>
static int resizeBilinearFunctor_(sd::LaunchContext* context,
                                  NDArray const* images, int const width,
  ...
  cudaError_t err =
      cudaMalloc(&xs_, sizeof(BilinearInterpolationData) * (outWidth + 1));
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::resize_image: Cannot allocate memory for vertical parts "
        "rectangulars",
        err);
  }
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::resize_image: Cannot allocate memory for horizontal parts "
        "rectangulars",
        err);
  }
  auto stream = context->getCudaStream();
  ...
  if (halfPixelCenter) {
    computeInterpolationWeights<HalfPixelScaler>
        <<<256, 512, 512, *stream>>>(outHeight, inHeight, heightScale, 0, ys_);
    computeInterpolationWeights<HalfPixelScaler><<<256, 512, 512, *stream>>>(
        outWidth, inWidth, widthScale, channels, xs_);
  } else {
    computeInterpolationWeights<LegacyScaler>
        <<<256, 512, 512, *stream>>>(outHeight, inHeight, heightScale, 0, ys_);
    computeInterpolationWeights<LegacyScaler><<<256, 512, 512, *stream>>>(
        outWidth, inWidth, widthScale, channels, xs_);
  }
  printf("Input is %dx%d, Output is %dx%d\n", inHeight, inWidth, outHeight,
         outWidth);
  NDArray::prepareSpecialUse({output}, {images});
  resizeImage_<T, F>(context, images, batchSize, inHeight, inWidth, outHeight,
                     outWidth, channels, xs_, ys_, output);
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::resize_image: Cannot deallocate memory for vertical parts "
        "rectangulars",
        err);
  }
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::resize_image: Cannot deallocate memory for horizontical "
        "parts rectangulars",
        err);
  }
...
//
template <typename T>
static __global__ void resizeNeighborKernel(
    T const* input, Nd4jLong const* inputShape, T* output,
    Nd4jLong const* outputShape, Nd4jLong batchSize, Nd4jLong inWidth,
    Nd4jLong inHeight, Nd4jLong outWidth, Nd4jLong outHeight, Nd4jLong channels,
    double widthScale, double heightScale, bool alignCorners,
    bool halfPixelCenters) {
  // for (int b = blockIdx.x; b < batchSize; b += gridDim.x)
  if (blockIdx.x < batchSize) {
    auto b = blockIdx.x;
    for (int y = threadIdx.x; y < outHeight; y += blockDim.x) {
      auto posY = alignCorners
                      ? static_cast<Nd4jLong>(sd::math::p_round<float>(
                            halfPixelCenters ? ((float)y + 0.5f) * heightScale
                                             : (float)y * heightScale))
                      : static_cast<Nd4jLong>(sd::math::p_floor<float>(
                            halfPixelCenters ? ((float)y + 0.5f) * heightScale
                                             : (float)y * heightScale));
      ...
      for (int x = threadIdx.y; x < outWidth; x += blockDim.y) {
        auto posX = alignCorners
                        ? static_cast<Nd4jLong>(sd::math::p_round<float>(
                              halfPixelCenters ? ((float)x + 0.5f) * widthScale
                                               : (float)x * widthScale))
                        : static_cast<Nd4jLong>(sd::math::p_floor<float>(
                              halfPixelCenters ? ((float)x + 0.5f) * widthScale
                                               : (float)x * widthScale));
...
//
template <typename T>
int resizeNeighborFunctor_(sd::LaunchContext* context, NDArray const* images,
                           int const width, int const height,
                           bool const alignCorners, bool const halfPixelCenters,
                           NDArray* output) {
  const Nd4jLong batchSize = images->sizeAt(0);
  const Nd4jLong inHeight = images->sizeAt(1);
  const Nd4jLong inWidth = images->sizeAt(2);
  ...
  //        if ((alignCorners && inHeight < 2) || (inHeight < 1) || (outHeight <
  //        1) || (alignCorners && outHeight < 2) ||
  //            (alignCorners && inWidth < 2) || (inWidth < 1) || (outWidth < 1)
  //            || (center && outWidth < 2)) {
  //            // wrong input data
  //            nd4j_printf("image.resize_nearest_neighbor: Wrong input or
  //            output size to resize\n", ""); return ND4J_STATUS_BAD_ARGUMENTS;
  //        }
  //        float heightScale = alignCorners ? (inHeight - 1.f) /
  //        float(outHeight - 1.f) : (inHeight / float(outHeight)); float
  //        widthScale = alignCorners ? (inWidth - 1.f) / float(outWidth - 1.f)
  ...
  auto imagesBuffer =
      images->getDataBuffer()
          ->specialAsT<T>();  // reinterpret_cast<T
                              // const*>(images->specialBuffer());
  auto outputBuffer =
      output->dataBuffer()
  ...
  NDArray::prepareSpecialUse({output}, {images});
  resizeNeighborKernel<T><<<batchSize, outHeight * outWidth, 512, *stream>>>(
      imagesBuffer, images->specialShapeInfo(), outputBuffer,
      output->specialShapeInfo(), batchSize, inWidth, inHeight, outWidth,
      outHeight, channels, widthScale, heightScale, alignCorners,
      halfPixelCenters);
  NDArray::registerSpecialUse({output}, {images});
...
//
void resizeImage(sd::LaunchContext* context, NDArray const* images,
                 Nd4jLong batchSize, Nd4jLong inHeight, Nd4jLong inWidth,
                 Nd4jLong outHeight, Nd4jLong outWidth, Nd4jLong channels,
                 BilinearInterpolationData* xs_, BilinearInterpolationData* ys_,
                 NDArray* output) {
  BUILD_DOUBLE_SELECTOR(images->dataType(), output->dataType(), resizeImage_,
                        (context, images, batchSize, inHeight, inWidth,
                         outHeight, outWidth, channels, xs_, ys_, output),
                        NUMERIC_TYPES, FLOAT_TYPES);
}

BUILD_DOUBLE_TEMPLATE(template void resizeImage_,
                      (sd::LaunchContext * context, NDArray const* images,
                       Nd4jLong batchSize, Nd4jLong inHeight, Nd4jLong inWidth,
                       Nd4jLong outHeight, Nd4jLong outWidth, Nd4jLong channels,
                       BilinearInterpolationData* xs_,
                       BilinearInterpolationData* ys_, NDArray* output),
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int resizeBilinearFunctor(sd::LaunchContext* context, NDArray const* images,
                          int width, int height, bool const alignCorners,
                          bool const halfPixelCenter, NDArray* output) {
  BUILD_DOUBLE_SELECTOR(
      images->dataType(), output->dataType(), return resizeBilinearFunctor_,
      (context, images, width, height, alignCorners, halfPixelCenter, output),
      NUMERIC_TYPES, FLOAT_TYPES);
}
//    BUILD_SINGLE_TEMPLATE(template int resizeBilinearFunctor_,
//    (sd::LaunchContext* context,
//            NDArray const* images, int const width, int const height, bool
//            const alignCorners, bool const halfPixelCenter, NDArray* output),
//            LIBND4J_TYPES);

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int resizeNeighborFunctor(sd::LaunchContext* context, NDArray const* images,
                          int const width, int const height,
                          bool const alignCorners, bool const halfPixelCenter,
                          NDArray* output) {
  BUILD_SINGLE_SELECTOR(
      images->dataType(), return resizeNeighborFunctor_,
      (context, images, width, height, alignCorners, halfPixelCenter, output),
      LIBND4J_TYPES);
}
//    BUILD_SINGLE_TEMPLATE(template int resizeNeighborFunctor_,
//    (sd::LaunchContext* context, NDArray const* images,
//            int width, int height, bool const alignCorners, bool const
//            halfPixelCenter, NDArray* output), LIBND4J_TYPES);
...
struct ImageResizerState {
  ...
  int validateAndCalculateOutputSize(NDArray const* input, int const width,
                                     int const height) {
    //
    ...
    if (ceilf((outHeight - 1) * heightScale) >
        static_cast<float>(DataTypeUtils::max<int>())) {
      nd4j_printf(
          "resize_bicubic: Upper overflow occurs for resize height (%f)\n",
          ceilf((outHeight - 1) * heightScale));
      return Status::CODE(
          ND4J_STATUS_BAD_INPUT,
          "resize_bicubic: Upper overflow occurs for resize height");
    }
    if (ceilf((outWidth - 1) * heightScale) >
        static_cast<float>(DataTypeUtils::max<int>())) {
      nd4j_printf(
          "resize_bicubic: Upper overflow occurs for resize height (%f)\n",
          ceilf((outHeight - 1) * heightScale));
      return Status::CODE(
          ND4J_STATUS_BAD_INPUT,
          "resize_bicubic: Upper overflow occurs for resize width");
    }
  ...
  int validateAndCreateOutput(NDArray const* input, int const width,
                              int const height) {
...
class CachedInterpolationCalculator {
 public:
  ...
  inline _CUDA_HD int Advance(const Nd4jLong x0, const Nd4jLong x1,
                              const Nd4jLong x2, const Nd4jLong x3) {
...
static __global__ void initCoefTableKernel(const double a, float* table,
                                           Nd4jLong tableSize) {
...
float* initCoeffsTable(const double a, cudaStream_t* stream) {
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::initCoeffsTable: Cannot allocate memory for vertical parts "
        "rectangulars",
        err);
  }
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::initCoeffsTable: Cannot syncronize kernel", err);
  }
...
//                stream); return sCoeffsTableHalf;
//            } else {
//                //static const float* coeffs_table = initCoeffsTable(-0.75f,
//                stream); return sCoeffsTableThreeFourth;
//            }
//        }
...
inline _CUDA_HD float interpolate1D(const float weight0, const float weight1,
                                    const float weight2, const float weight3,
                                    const T value0, const T value1,
                                    const T value2, const T value3) {
...
static _CUDA_HD float compute(float values[4], const float xW0, const float xW1,
                              const float xW2, const float xW3) {
  return interpolate1D(xW0, xW1, xW2, xW3, values[0], values[1], values[2],
                       values[3]);
}

template <typename Scaler, bool use_keys_cubic>
inline _CUDA_HD void getWeightsAndIndices(float const* coeffs_table,
                                          const float scale,
                                          const Nd4jLong out_loc,
                                          const Nd4jLong limit,
...
static __global__ void accumulateChannelsKernel(WeightsAndIndices* pXWais,
                                                Nd4jLong outWidth,
                                                Nd4jLong channels) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  auto step = blockDim.x * gridDim.x;
...
static __global__ void advaceWeightsAndIndicesKernel(
    float const* cacheTable, CachedInterpolationCalculator* calc,
    WeightsAndIndices* pXWais, Nd4jLong inWidth, float widthScale,
  ...
  for (auto x = start; x < outWidth; x += step) {
    if (halfPixelCenters)
      getWeightsAndIndices<HalfPixelScaler, true>(cacheTable, widthScale, x,
                                                  inWidth, &pXWais[x]);
    else
      getWeightsAndIndices<LegacyScaler, false>(cacheTable, widthScale, x,
                                                inWidth, &pXWais[x]);
    pXWais[x]._advance = calc->Advance(pXWais[x]._index0, pXWais[x]._index1,
                                       pXWais[x]._index2, pXWais[x]._index3);
  }
}
// resizerState and xWais are device allocated
static void computeXWeightsAndIndices(float const* coeffsTable,
                                      const ImageResizerState& resizerState,
  ...
  if (err != 0) {
    cuda_exception::build(
        "helpers::computeXWeightsAndIndices: Cannot allocated device memory "
        "for interpolate calculator",
        err);
  }
  err = cudaMemcpyAsync(pCalcD, &calc, sizeof(CachedInterpolationCalculator),
                        cudaMemcpyHostToDevice, *stream);
  if (err != 0) {
    cuda_exception::build(
        "helpers::computeXWeightsAndIndices: Cannot set up device memory for "
        "interpolate calculator",
        err);
  }

  advaceWeightsAndIndicesKernel<<<128, 128, 128, *stream>>>(
      coeffsTable, pCalcD, pXWais, resizerState.inWidth,
      resizerState.widthScale, outWidth, resizerState.channels,
      halfPixelCenters);
  err = cudaFree(pCalcD);
  if (err != 0) {
    cuda_exception::build(
        "helpers::computeXWeightsAndIndices: Cannot deallocated device memory "
        "for interpolate calculator",
        err);
  }
  err = cudaStreamSynchronize(*stream);
  if (err != 0) {
    cuda_exception::build(
        "helpers::computeXWeightsAndIndices: Cannot synchronize stream after "
        "advance weights and indicers",
        err);
  }
  // Scale the values so they can be used as offsets into buffers.
  accumulateChannelsKernel<<<128, 128, 512, *stream>>>(pXWais, outWidth,
                                                       resizerState.channels);
  err = cudaStreamSynchronize(*stream);
  if (err != 0) {
    cuda_exception::build(
        "helpers::computeXWeightsAndIndices: Cannot synchronize stream after "
...
static _CUDA_HD FORCEINLINE float computeYInterpolation(
    int which, int channelNum, const WeightsAndIndices& yWai, const T* pY0,
    const T* pY1, const T* pY2, const T* pY3, const WeightsAndIndices& xWai) {
  int xIndex;
  switch (which) {
    case 0:
      xIndex = xWai._index0;
...
static __global__ void bicubicInterpolateWithCachingKernel(
    float const* cachedTable, T const* inputPtr,
    ImageResizerState* pResizerState, WeightsAndIndices* xWais,
    bool halfPixelCenters, Nd4jLong inBatchWidth, Nd4jLong inRowWidth,
    float* outputPtr) {
  ...
  for (Nd4jLong b = blockIdx.x; b < pResizerState->batchSize; b += gridDim.x) {
    ...
    float* cachedValue;
    for (Nd4jLong y = threadIdx.x; y < pResizerState->outHeight;
         y += blockDim.x) {
      if (threadIdx.x == 0) {
        extern __shared__ char sharedChar[];
        cachedValue = reinterpret_cast<float*>(sharedChar);
      }
      auto pos = (b * pResizerState->outHeight + y) * pResizerState->outWidth *
                 pResizerState->channels;
      auto pOutput = &outputPtr[pos];
      struct WeightsAndIndices yWai;
      if (halfPixelCenters) {
        getWeightsAndIndices<HalfPixelScaler, true>(
            cachedTable, pResizerState->heightScale, y, pResizerState->inHeight,
            &yWai);
      } else {
      ...
      if (pResizerState->channels == 3) {
        ...
        for (Nd4jLong x = 0; x < pResizerState->outWidth; ++x) {
          ...
          for (Nd4jLong c = 0; c < pResizerState->channels; ++c) {
            pOutput[x * pResizerState->channels + c] =
...
template <typename T>
static void bicubicInterpolateWithCaching(NDArray const* image,
                                          ImageResizerState const& resizerState,
                                          bool const halfPixelCenters,
                                          NDArray* output) {
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::bicubicInterpolateWithCaching: Cannot allocate memory for "
        "resizerState",
        err);
  }
  err = cudaMemcpyAsync(resizerStateD, &resizerState, sizeof(ImageResizerState),
                        cudaMemcpyHostToDevice, *stream);
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::bicubicInterpolateWithCaching: Cannot set up memory for "
        "resizerState",
        err);
  }

  //        float* cachedValue = nullptr;
  //        size_t cachedSize = sizeof(float) * (numChannels == 3 ? 0 : 4 *
  //        numChannels); if (cachedSize) {
  //            err = cudaMalloc(reinterpret_cast<void**>(&cachedValue),
  //            cachedSize); if (err != 0) {
  //                throw cuda_exception::build(
  //                        "helpers::bicubicInterpolateWithCaching: Cannot
  //                        allocate memory for cached values", err);
  //            }
  //            err = cudaMemset(cachedValue, 0, cachedSize);
  //            if (err != 0) {
  //                throw cuda_exception::build(
  //                        "helpers::bicubicInterpolateWithCaching: Cannot set
  //                        up memory for cached values", err);
  //            }
  //        }
  ...
  auto coeffsTable = halfPixelCenters ? initCoeffsTable(-0.5, stream)
                                      : initCoeffsTable(-0.75, stream);
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::bicubicInterpolateWithCaching: computeXWeigtsAndInidces "
        "finished with error",
        err);
  }
  ...
  bicubicInterpolateWithCachingKernel<T><<<128, 1, 512, *stream>>>(
      coeffsTable, pInput, resizerStateD, xWais, halfPixelCenters, inBatchWidth,
      inRowWidth, pOutput);
  err = cudaStreamSynchronize(*stream);
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::bicubicInterpolateWithCaching: Kernels finished with error",
        err);
  }
  ...
  if (err != 0) {
    throw cuda_exception::build(
        "helpers::bicubicInterpolateWithCaching: Cannot deallocate memory for "
        "resizerState",
        err);
  }
  //        if (cachedSize)
  ...
  //        if (err != 0) {
  //            throw
  //            cuda_exception::build("helpers::bicubicInterpolateWithCaching:
  //            Cannot deallocate memory for cached values", err);
  //        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
int resizeBicubicFunctor_(sd::LaunchContext* context, NDArray const* image,
                          int width, int height, bool preserveAspectRatio,
                          bool antialias, NDArray* output) {
  return Status::OK();
}

int resizeBicubicFunctor(sd::LaunchContext* context, NDArray const* image,
                         int width, int height, bool preserveAspectRatio,
                         bool antialias, NDArray* output) {
  BUILD_SINGLE_SELECTOR(
      image->dataType(), return resizeBicubicFunctor_,
      (context, image, width, height, preserveAspectRatio, antialias, output),
      NUMERIC_TYPES);
}
...
static __global__ void fillInterpolationCache(CachedInterpolation* xCached,
                                              Nd4jLong cacheLen,
                                              Nd4jLong inWidth,
                                              float widthScale) {
  auto start = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (auto x = start; x < cacheLen; x += increment) {
    ...
    xCache.start = v;
    xCache.startScale = v < inX ? (v + 1 > inX1 ? widthScale : v + 1 - inX)
                                : (v + 1 > inX1 ? inX1 - v : 1.f);
    v = math::nd4j_ceil<float, Nd4jLong>(inX1);
    xCache.end = v--;
    xCache.endMinusOneScale = v < inX
                                  ? (v + 1 > inX1 ? widthScale : v + 1 - inX)
                                  : (v + 1 > inX1 ? inX1 - v : 1.f);
    xCache.needsBounding = bound(xCache.start, inWidth) != xCache.start ||
                           bound(xCache.end - 1, inWidth) != (xCache.end - 1);
  }
}
...
template <typename T>
static __device__ void computePatchSumOf3Channels(
    float scale, const ImageResizerState& st, ScaleCache<T> const* yScaleCache,
    Nd4jLong ptrsLen, const CachedInterpolation& xCache, float* outputPtr) {
...
static __device__ void computePatchSum(float scale, const ImageResizerState& st,
  ...
  for (Nd4jLong c = 0; c < numChannels; ++c) {
    ...
    for (int i = 0; i < ptrsLen; ++i) {
      ...
      float sumY =
          static_cast<float>(
              ptr[numChannels * boundIfNeeded(xCache.start, st.inWidth) + c]) *
          scaleX;
      if (xCache.start + 1 != xCache.end) {
        for (Nd4jLong x = xCache.start + 1; x < xCache.end - 1; ++x) {
        ...
        sumY +=
            static_cast<float>(
                ptr[numChannels * boundIfNeeded(xCache.end - 1, st.inWidth) +
                    c]) *
            scaleX;
      }
...
template <typename T>
static __global__ void resizeAreaKernel(
    ImageResizerState const* pSt, CachedInterpolation const* caches,
    float scale, T const* inputPtr, Nd4jLong const* inputShape,
    float* outputPtr, Nd4jLong const* outputShape,
  ...
  for (auto batch = blockIdx.x; batch < pSt->batchSize; batch += gridDim.x) {
    for (auto y = threadIdx.x; y < pSt->outHeight; y += blockDim.x) {
      ...
      auto yScaleCache = cachePool + (batch * pSt->outWidth + y) * scalesDim *
                                         sizeof(ScaleCache<T>);
      ...
      float* output = outputPtr + (batch * pSt->outHeight + y) * pSt->channels *
                                      pSt->outWidth;
      // int k = 0;
      for (Nd4jLong i = yStart, k = 0; i < yEnd; ++i, ++k) {
        ...
        yScaleCache[k].yPtr =
            inputPtr + (batch * pSt->inHeight * pSt->inWidth * pSt->channels +
                        bound(i, pSt->inHeight) * pSt->inWidth * pSt->channels);
      }

      if (pSt->channels == 3) {
        for (Nd4jLong x = 0; x < pSt->outWidth; ++x) {
          const CachedInterpolation& xCache = caches[x];
          computePatchSumOf3Channels<T>(scale, *pSt, yScaleCache, scalesDim,
                                        xCache, output);
          output += pSt->channels;
        }
      } else {
        for (Nd4jLong x = 0; x < pSt->outWidth; ++x) {
          const CachedInterpolation& xCache = caches[x];
          computePatchSum<T>(scale, *pSt, yScaleCache, scalesDim, xCache,
                             output);
          output += pSt->channels;
        }
      }
...
template <typename T>
static void resizeArea(cudaStream_t* stream, ImageResizerState const& st,
                       CachedInterpolation* cache, NDArray const* input,
                       NDArray* output) {
  T const* inputPtr = reinterpret_cast<T const*>(input->specialBuffer());
  //        float* yScales;
  //        T const** yPtrs;
  float scale = 1.f / (st.heightScale * st.widthScale);
  auto outputPtr = reinterpret_cast<float*>(
      output->specialBuffer());  // output is always float. TO DO: provide
                                 // another float types also with  template
                                 // <typename X, typename Z> declaration
  ...
  err = cudaMalloc(&cachePool, sizeof(ScaleCache<T>) * st.batchSize *
                                   st.outWidth * st.outHeight);
  resizeAreaKernel<T><<<128, 2, 2048, *stream>>>(
      pSt, cache, scale, inputPtr, input->specialShapeInfo(), outputPtr,
...
// //
template <typename T>
int resizeAreaFunctor_(sd::LaunchContext* context, NDArray const* image,
                       int const width, int const height,
  ...
  if (Status::OK() == res) {
    ...
    fillInterpolationCache<<<128, 128, 256, *stream>>>(
        xCached, st.outWidth, st.inWidth, st.widthScale);
...
int resizeAreaFunctor(sd::LaunchContext* context, NDArray const* image,
  ...
  BUILD_SINGLE_SELECTOR(image->dataType(), return resizeAreaFunctor_,
                        (context, image, width, height, alignCorners, output),
                        NUMERIC_TYPES);
}
...
//
template <typename T>
int resizeBicubicFunctorA_(sd::LaunchContext* context, NDArray const* image,
                           int width, int height, bool const alignCorners,
                           bool const halfPixelCenters, NDArray* output) {
  ImageResizerState st(alignCorners,
                       halfPixelCenters);  // align_corners, half_pixel_align
...
int resizeBicubicFunctorA(sd::LaunchContext* context, NDArray const* image,
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (context, image, width, height, alignCorners, halfPixelCenters, output),
      NUMERIC_TYPES);
}
BUILD_SINGLE_TEMPLATE(template int resizeBicubicFunctorA_,
                      (sd::LaunchContext * context, NDArray const* image,
                       int width, int height, bool const alignCorners,
                       bool const halfPixelCenters, NDArray* output),
                      NUMERIC_TYPES);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int resizeFunctor(sd::LaunchContext* context, NDArray const* image, int width,
                  int height, ImageResizeMethods method,
                  bool preserveAspectRatio, bool antialias, NDArray* output) {
  switch (method) {
    case kResizeBilinear:
      return resizeBilinearFunctor(context, image, width, height, false, false,
                                   output);
      break;
    case kResizeNearest:
      return resizeNeighborFunctor(context, image, width, height, false, false,
                                   output);
      break;
    case kResizeBicubic:
      return resizeBicubicFunctor(context, image, width, height,
                                  preserveAspectRatio, antialias, output);
      break;
...
static __global__ void cropAndResizeKernel(
    T const* images, Nd4jLong const* imagesShape, Z const* boxes,
    Nd4jLong const* boxesShape, I const* indices, Nd4jLong const* indexShape,
    I const* cropSize, Nd4jLong const* cropShape, int method,
    double extrapolationVal, T* output, Nd4jLong const* outputShape,
    int numBoxes, int cropHeight, int cropWidth, int batchSize, int imageHeight,
    int imageWidth, int depth) {
  for (int b = blockIdx.x; b < numBoxes; b += gridDim.x) {
    ...
    Z heightScale = (cropHeight > 1)
                        ? (y2 - y1) * (imageHeight - 1) / Z(cropHeight - 1)
                        : Z(0);
    Z widthScale = (cropWidth > 1)
                       ? (x2 - x1) * (imageWidth - 1) / Z(cropWidth - 1)
                       : Z(0);
      ...
      if (method == 0 /* bilinear */) {
        ...
        for (int x = 0; x < cropWidth; ++x) {
          ...
          for (int d = start; d < depth; d += step) {
            ...
            const T topLeft(images[shape::getOffset(
                imagesShape,
...
//
template <typename T, typename Z, typename I>
void cropAndResizeFunctor_(sd::LaunchContext* context, NDArray const* images,
  ...
  int threadsPerBlock =
      math::nd4j_max(imageHeight * imageWidth, cropHeight * cropWidth);
  ...
  cropAndResizeKernel<T, Z, I><<<batchSize, threadsPerBlock, 256, *stream>>>(
      imagesBuf, images->specialShapeInfo(), boxesBuf,
      boxes->specialShapeInfo(), indexBuf, indices->specialShapeInfo(),
      cropSizes, cropSize->specialShapeInfo(), method, extrapolationVal, outBuf,
      crops->specialShapeInfo(), numBoxes, cropHeight, cropWidth, batchSize,
...
void cropAndResizeFunctor(sd::LaunchContext* context, NDArray const* images,
                          ...
                          NDArray const* cropSize, int method,
                          double extrapolationVal, NDArray* crops) {
  BUILD_TRIPLE_SELECTOR(images->dataType(), boxes->dataType(),
                        indices->dataType(), cropAndResizeFunctor_,
                        (context, images, boxes, indices, cropSize, method,
                         extrapolationVal, crops),
