...
__global__ static void polyGammaCuda(const void *vn, const Nd4jLong *nShapeInfo,
  ...
  for (int i = tid; i < len; i += totalThreads) {
    ...
    const auto xOffset =
        sameOffsetNX ? nOffset : shape::getIndexOffset(i, xShapeInfo);
    const auto zOffset =
        sameOffsetNZ ? nOffset : shape::getIndexOffset(i, zShapeInfo);
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void polyGammaCudaLauncher(const int blocksPerGrid,
                                  const int threadsPerBlock,
                                  const cudaStream_t *stream, const void *vn,
                                  const Nd4jLong *nShapeInfo, const void *vx,
                                  const Nd4jLong *xShapeInfo, void *vz,
                                  const Nd4jLong *zShapeInfo) {
  polyGammaCuda<T><<<blocksPerGrid, threadsPerBlock, 1024, *stream>>>(
      vn, nShapeInfo, vx, xShapeInfo, vz, zShapeInfo);
}
...
void polyGamma(sd::LaunchContext *context, const NDArray &n, const NDArray &x,
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, context->getCudaStream(),
       n.specialBuffer(), n.specialShapeInfo(), x.specialBuffer(),
