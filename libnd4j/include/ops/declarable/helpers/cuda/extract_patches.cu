...
static __global__ void globalExtractPatchesKernel(
    bool theSame, int batchCount, int sizeRow, int sizeCol, int rowDim,
    ...
    const T* input, const Nd4jLong* patchShape, const Nd4jLong* inputOffsets,
    T* output, const Nd4jLong* outTadShape, const Nd4jLong* outputOffsets) {
  auto start = threadIdx.x + blockIdx.x * blockDim.x;

  auto step = blockDim.x * gridDim.x;
  // batch  input by 3 last dims and extrapole input onto output with
  // outColDim/outRowDim
  for (Nd4jLong batch = start; batch < batchCount; batch += step) {
    ...
    for (Nd4jLong i = 0; i < outRowDim; i++) {
      for (Nd4jLong j = 0; j < outColDim; j++) {
        ...
        if (!theSame) {
          rowEnd =
              math::nd4j_min(rowStart + sizeRow * rateRow, Nd4jLong(rowDim));
          colEnd =
              math::nd4j_min(colStart + sizeCol * rateCol, Nd4jLong(colDim));
        }
          ...
          for (auto col = colStart; col < colEnd; col += rateCol) {
            for (auto pixel = 0; pixel < lastDim; pixel++) {
              ...
              if (setUp) {  // VALID or SAME cases
                outMatrix[shape::getOffset(outTadShape, zPos)] =
                    patch[shape::getOffset(patchShape, xPos)];
              }
              pos++;
            }
          }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
static void _extractPatches(sd::LaunchContext* context, NDArray* images,
                            NDArray* output, int sizeRow, int sizeCol,
                            int strideRow, int strideCol, int rateRow,
                            int rateCol, bool theSame) {
  ...
  globalExtractPatchesKernel<T><<<128, 128, 1024, *stream>>>(
      theSame, batchCount, sizeRow, sizeCol, rowDim, colDim, outRowDim,
      outColDim, strideRow, strideCol, rateRow, rateCol, rowCast, colCast,
      lastDim, imagesBuffer, packX.specialShapeInfo(), packX.specialOffsets(),
      outputBuffer, packZ.specialShapeInfo(), packZ.specialOffsets());
...
BUILD_SINGLE_TEMPLATE(template void _extractPatches,
                      (sd::LaunchContext * context, NDArray* input,
                       NDArray* output, int sizeRow, int sizeCol, int stradeRow,
                       int stradeCol, int rateRow, int rateCol, bool theSame),
                      LIBND4J_TYPES);
...
void extractPatches(sd::LaunchContext* context, NDArray* images,
  ...
  BUILD_SINGLE_SELECTOR(xType, _extractPatches,
                        (context, images, output, sizeRow, sizeCol, stradeRow,
                         stradeCol, rateRow, rateCol, theSame),
                        LIBND4J_TYPES);
}
}  // namespace helpers
}  // namespace ops
