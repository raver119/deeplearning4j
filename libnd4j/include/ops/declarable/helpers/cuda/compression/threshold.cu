...
namespace ops {
namespace helpers {
void prescanArrayRecursive(int **g_scanBlockSums, int *dZ, int *dX,
  ...
  if (numBlocks > 1) {
    ...
    // recursive (CPU) call
    prescanArrayRecursive(g_scanBlockSums, g_scanBlockSums[level],
...
static void encodeThresholdP3_(void *dx, const Nd4jLong *hXShapeInfo,
                               int *offsets, Nd4jLong N, int *dz) {
  auto stream = LaunchContext::defaultContext()->getCudaStream();
  ...
  BUILD_SINGLE_SELECTOR(xType, encoderKernelP3Generic,
                        (launchDims, stream, dx, offsets, N, dz), FLOAT_TYPES);

  sd::DebugHelper::checkErrorCode(stream, "encodeThresholdP3Float(...) failed");
}

static NDArray thresholdEstimate_(const NDArray &updates,
                                  const float threshold) {
  const int numThreads = 512;
  const int numBlocks = updates.lengthOf() / numThreads +
                        (updates.lengthOf() % numThreads ? 1 : 0);
  ...
  BUILD_SINGLE_SELECTOR(
      xType, encoderKernelP1Generic,
      (launchDims, LaunchContext::defaultContext()->getCudaStream(),
       updates.specialBuffer(), updates.lengthOf(), tmp.specialBuffer(),
       threshold),
      FLOAT_TYPES);
...
void thresholdEncode(NDArray &updates, NDArray &encoded, float threshold) {
  // we need these blocks in order to know, how many "updates" will be processed
  // by each GPU block
  ...
  encodeThresholdP3_(updates.specialBuffer(), updates.shapeInfo(),
                     reinterpret_cast<int *>(offsets.specialBuffer()),
                     updates.lengthOf(),
...
void thresholdDecode(const NDArray &encoded, NDArray &updates) {
  ...
  BUILD_SINGLE_SELECTOR(
      xType, decoderKernelGeneric,
      (launchDims, LaunchContext::defaultContext()->getCudaStream(),
       encoded.specialBuffer(), updates.lengthOf(), updates.specialBuffer()),
      FLOAT_TYPES);
