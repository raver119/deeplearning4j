...
void decodeBitmap(sd::LaunchContext* context, const NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(xType, cudaDecodeBitmapGeneric,
                        (launchDims, stream, input->specialBuffer(),
                         output->lengthOf(), output->specialBuffer()),
                        FLOAT_TYPES);
...
Nd4jLong encodeBitmap(sd::LaunchContext* context, NDArray* input,
  ...
  int* resultPointer = reinterpret_cast<int*>(
      LaunchContext::defaultContext()->getScalarPointer());
  int* reductionPointer = reinterpret_cast<int*>(
  ...
  BUILD_SINGLE_SELECTOR(
      xType, cudaEncodeBitmapGeneric,
      (launchDims, stream, input->specialBuffer(), input->lengthOf(),
       reinterpret_cast<int*>(output->specialBuffer()), resultPointer,
       reductionPointer, threshold),
      FLOAT_TYPES);
