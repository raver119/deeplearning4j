...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void prefixPerBlockCuda(
    scalar::Ops op, const void* vx, const Nd4jLong* xTadShapeInfo,
    const Nd4jLong* xTadOffsets, void* vz, const Nd4jLong* zTadShapeInfo,
    const Nd4jLong* zTadOffsets, const Nd4jLong numTads, const Nd4jLong tadLen,
  ...
  for (uint i = 0; i < numTadChunks; ++i) {
    ...
    if (leftArrInd < tadLen)
      shared[sharedInd] = xLeft =
          xTad[shape::getIndexOffset(leftArrInd, xTadShapeInfo)];
    // else
    //     shared[sharedInd] = (op == scalar::Add) ? 0 : 1;

    if (rightArrInd < tadLen)
      shared[sharedInd + 1] = xRight =
          xTad[shape::getIndexOffset(rightArrInd, xTadShapeInfo)];
    // else
    ...
    for (uint d = blockDim.x; d > 0; d /= 2) {
      ...
      if (threadIdx.x < d) {
        ...
        shared[right] = (op == scalar::Add) ? (shared[right] + shared[left])
    ...
    for (uint d = 1; d < blockDim2; d *= 2) {
      ...
      if (threadIdx.x < d) {
        ...
        shared[right] = (op == scalar::Add) ? (shared[right] + temp)
                                            : (shared[right] * temp);
      }
    }
    ...
    if (leftArrInd < tadLen) {
      ...
      if (i > 0)
        result = (op == scalar::Add) ? result + lastElemInChunk
                                     : result * lastElemInChunk;
    ...
    if (rightArrInd < tadLen) {
      ...
      if (i > 0)
        result = (op == scalar::Add) ? result + lastElemInChunk
                                     : result * lastElemInChunk;
      if (i < numTadChunks - 1 &&
          threadIdx.x == blockDim.x - 1)  // last element in chunk
        lastElemInChunk = !exclusive ? result
                                     : (op == scalar::Add) ? result + xRight
                                                           : result * xRight;
...
///////////////////////////////////////////////////////////////////
template <typename X>
static void prefixPerBlockCudaLauncher(
    ...
    const Nd4jLong* xTadShapeInfo, const Nd4jLong* xTadOffsets, void* vz,
    const Nd4jLong* zTadShapeInfo, const Nd4jLong* zTadOffsets,
    const Nd4jLong numTads, const Nd4jLong tadLen, const bool exclusive,
    const bool reverse) {
  prefixPerBlockCuda<X><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      op, vx, xTadShapeInfo, xTadOffsets, vz, zTadShapeInfo, zTadOffsets,
      numTads, tadLen, exclusive, reverse);
}

///////////////////////////////////////////////////////////////////
void prefix(sd::LaunchContext* context, scalar::Ops op, const NDArray* x,
            NDArray* z, const std::vector<int>& dims, bool exclusive,
            bool reverse) {
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), op,
       x->specialBuffer(), packX.platformShapeInfo(), packX.platformOffsets(),
       z->specialBuffer(), packZ.platformShapeInfo(), packZ.platformOffsets(),
       numTads, tadLen, exclusive, reverse),
      NUMERIC_TYPES);
...
///////////////////////////////////////////////////////////////////
void prefix(sd::LaunchContext* context, scalar::Ops op, const NDArray* x,
            NDArray* z, bool exclusive, bool reverse) {
