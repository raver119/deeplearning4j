...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void matrixSetDiagCuda(const void* vx,
  ...
  if (threadIdx.x == 0) {
    ...
    areSameOffsets = shape::haveSameShapeAndStrides(
        xShapeInfo,
        zShapeInfo);  // shapes are definitely the same, but strides might not
  ...
  auto coords =
      sharedMem +
      threadIdx.x * xRank;  // we provide (xRank * sizeof(int) * threadIdx.x)
                            // amount of shared memory per each thread
  ...
  for (Nd4jLong i = tid; i < xLen; i += gridDim.x * blockDim.x) {
    ...
    const auto zOffset =
        areSameOffsets ? xOffset : shape::getOffset(zShapeInfo, coords);
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void matrixSetDiagCudaLauncher(
    ...
    const Nd4jLong* zShapeInfo, const bool zeroPad) {
  matrixSetDiagCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, vy, yShapeInfo, vz, zShapeInfo, zeroPad);
}

///////////////////////////////////////////////////////////////////
void matrixSetDiag(sd::LaunchContext* context, const NDArray& input,
                   const NDArray& diagonal, NDArray& output,
                   const bool zeroPad) {
