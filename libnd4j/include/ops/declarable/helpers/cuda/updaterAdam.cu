...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ void adamUpdaterCuda(const void* vx, const Nd4jLong* xShapeInfo,
  ...
  for (Nd4jLong i = blockIdx.x * blockDim.x + threadIdx.x; i < xLen;
    ...
    stU[stUOffset] = beta2 * initU[initUOffset] +
                     grad[xOffset] * grad[xOffset] * (1 - beta2);

    up[zOffset] = (stM[stMOffset] * epsilonT) /
                  (sd::math::nd4j_sqrt<T, T>(stU[stUOffset]) + epsilon);
  }
}

///////////////////////////////////////////////////////////////////
template <typename T>
linkage void adamUpdaterCudaLauncher(
...
void updaterAdam(sd::LaunchContext* context, const NDArray& gradient,
  ...
  NDArray::prepareSpecialUse({&update, &stateU, &stateM},
                             {&gradient, &initStateU, &initStateM});
