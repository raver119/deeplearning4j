...
__global__ static void diGammaCuda(const void *vx, const Nd4jLong *xShapeInfo,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < len;
    ...
    const auto zOffset =
        sameOffset ? xOffset : shape::getIndexOffset(i, zShapeInfo);
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void diGammaCudaLauncher(const int blocksPerGrid,
                                const int threadsPerBlock,
                                const cudaStream_t *stream, const void *vx,
                                const Nd4jLong *xShapeInfo, void *vz,
                                const Nd4jLong *zShapeInfo) {
  diGammaCuda<T><<<blocksPerGrid, threadsPerBlock, 1024, *stream>>>(
...
void diGamma(sd::LaunchContext *context, const NDArray &x, NDArray &z) {
  ...
  BUILD_SINGLE_SELECTOR(
      x.dataType(), diGammaCudaLauncher,
      (blocksPerGrid, threadsPerBlock, context->getCudaStream(),
       x.specialBuffer(), x.specialShapeInfo(), z.specialBuffer(),
       z.specialShapeInfo()),
      FLOAT_TYPES);
