...
//
template <typename T>
static __global__ void matrixBandKernel(
    const void* inputBuffer, const Nd4jLong* inputShape, void* outputBuffer,
    const Nd4jLong* outputShape, Nd4jLong lowerBand, Nd4jLong upperBand,
    ...
    Nd4jLong numTads, Nd4jLong inputLength) {
  ...
  for (Nd4jLong e = blockIdx.x; e < numTads; e += gridDim.x) {
    ...
    for (Nd4jLong i = blockIdx.y; i < rows; i += gridDim.y) {
      for (Nd4jLong j = threadIdx.x; j < cols; j += totalThreads) {
        ...
        Nd4jLong tadOffsetOut =
            shape::getOffset(tadOnlyOutputShapeInfo, coords);
...
//
template <typename T>
void matrixBandPart_(sd::LaunchContext* context, NDArray* input,
                     NDArray* output, Nd4jLong lowerBand, Nd4jLong upperBand) {
  ...
  matrixBandKernel<T><<<launchDims.x, launchDims.y, launchDims.z, *stream>>>(
      ...
      output->specialBuffer(), output->specialShapeInfo(), lowerBand, upperBand,
      packX.specialShapeInfo(), packX.specialOffsets(),
      packZ.specialShapeInfo(), packZ.specialOffsets(), numTads,
      input->lengthOf());
...
void matrixBandPart(sd::LaunchContext* context, NDArray* input, NDArray* output,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), matrixBandPart_,
                        (context, input, output, lowerBand, upperBand),
                        FLOAT_TYPES);
}
}  // namespace helpers
}  // namespace ops
