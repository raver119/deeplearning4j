...
///////////////////////////////////////////////////////////////////
template <typename X, typename Y>
static void gatherNDCudaLauncher(const int blocksPerGrid,
  ...
  gatherNDCuda<X, Y><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
...
void gatherND(sd::LaunchContext *context, NDArray &input, NDArray &indices,
              NDArray &output) {
  const int maxRank = sd::math::nd4j_max<int>(
      indices.rankOf(),
      sd::math::nd4j_max<int>(input.rankOf(), output.rankOf()));
  ...
  BUILD_DOUBLE_SELECTOR(
      xType, yType, gatherNDCudaLauncher,
