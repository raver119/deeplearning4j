...
__global__ static void crossCuda(const void* vx, const Nd4jLong* xShapeInfo,
  ...
  if (threadIdx.x == 0) {
    ...
    lenWithoutLastDim = shape::length(xShapeInfo) /
                        xShapeInfo[rank];  //  shape::length(xShapeInfo) / 3;
  }
  __syncthreads();
...
template <typename T>
__host__ static void crossCudaLauncher(
  ...
  crossCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
...
void crossBatched(sd::LaunchContext* context, NDArray* x, NDArray* y,
                  NDArray* z) {
  const int threadsPerBlock = MAX_NUM_THREADS / 4;
  const int blocksPerGrid =
      (x->lengthOf() / x->sizeAt(-1) + threadsPerBlock - 1) / threadsPerBlock;
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(),
       ...
       y->specialShapeInfo(), z->specialBuffer(), z->specialShapeInfo()),
      NUMERIC_TYPES);
