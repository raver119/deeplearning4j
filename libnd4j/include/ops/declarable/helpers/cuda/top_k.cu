...
__global__ static void inTopKCuda(const void* vx, const Nd4jLong* xShapeInfo,
                                  ...
                                  const Nd4jLong* xTadOffsets, const uint k) {
  ...
  if (threadIdx.x == 0) {
    ...
    idx = y[shape::getIndexOffset(
        blockIdx.x, yShapeInfo)];  // shape::length(yShapeInfo) == numTads
...
///////////////////////////////////////////////////////////////////
template <typename X, typename Y>
static void inTopKCudaLauncher(const int blocksPerGrid,
                               ...
                               const Nd4jLong* xTadShapeInfo,
                               const Nd4jLong* xTadOffsets, const uint k) {
  inTopKCuda<X, Y><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, vy, yShapeInfo, vz, zShapeInfo, xTadShapeInfo,
      xTadOffsets, k);
}
...
int inTopKFunctor(sd::LaunchContext* context, const NDArray* predictions,
  ...
  BUILD_DOUBLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(),
       predictions->specialBuffer(), predictions->specialShapeInfo(),
       targets->specialBuffer(), targets->specialShapeInfo(),
       output->specialBuffer(), output->specialShapeInfo(),
       packX.specialShapeInfo(), packX.specialOffsets(), k),
...
template <typename X, typename Y>
static _CUDA_G void topValuesMover(
    void const* vx, Nd4jLong const* xTadShapeInfo, Nd4jLong const* xTadOffsets,
    void const* vi, Nd4jLong const* iTadShapeInfo, Nd4jLong const* iTadOffsets,
    void* vz, Nd4jLong const* zTadShapeInfo, Nd4jLong const* zTadOffsets,
    Nd4jLong tadLength, int numTads, int k) {
  for (int t = blockIdx.x; t < numTads; t += gridDim.x) {
    ...
    for (int e = threadIdx.x; e < k; e += blockDim.x) {
      ...
      z[shape::getIndexOffset(e, zTadShapeInfo)] =
          x[shape::getIndexOffset(idx, xTadShapeInfo)];
    }
  }
}

template <typename X, typename Y>
static _CUDA_G void indicesAlongDimension(
    void const* vx, Nd4jLong const* xTadShapeInfo, Nd4jLong const* xTadOffsets,
    void* vi, Nd4jLong const* iTadShapeInfo, Nd4jLong const* iTadOffsets,
    void* vz, Nd4jLong const* zTadShapeInfo, Nd4jLong const* zTadOffsets,
    Nd4jLong tadLength, int numTads, int k, int scanWidth, bool needSort) {
  extern __shared__ char _shmem[];

  X* tempValues = reinterpret_cast<X*>(_shmem) + threadIdx.x * scanWidth;
  Y* tempIndices = reinterpret_cast<Y*>(reinterpret_cast<X*>(_shmem) +
                                        blockDim.x * scanWidth) +
                   threadIdx.x * scanWidth;
  ...
  for (int t = blockIdx.x; t < numTads; t += gridDim.x) {
    ...
    for (int p = 0; p < k; p += scanWidth) {
      ...
      // at this point we have local part ready for merge and define global
      // maximum for this iteration, and local maximum for next iteration
      for (uint activeThreads = blockDim.x / 2; activeThreads > 0;
           activeThreads /= 2) {
        if (threadIdx.x < activeThreads) {
    ...
    __syncthreads();
    if (!needSort) {
      // if we don't need sort, we need to return values based on their indices
      // (ascending)
      for (int m = 0; m < k; m++) {
        if (m % 2 == 0) {
...
static int topKFunctor_(sd::LaunchContext* context, const NDArray* input,
  ...
  if (k == 1) {
    input->applyIndexReduce(indexreduce::IndexMax, *indices,
                            {input->rankOf() - 1});
    ...
    topValuesMover<X, Y><<<256, 256, 1024, *context->getCudaStream()>>>(
        ...
        packX.platformOffsets(), indices->specialBuffer(),
        packI.platformShapeInfo(), packI.platformOffsets(),
        values->specialBuffer(), packZ.platformShapeInfo(),
        packZ.platformOffsets(), tadLength, packX.numberOfTads(), k);
  } else {
    int scanWidth = 1;
    int numTreads = 256;
    int shMemSize = (numTreads * sizeof(X) * scanWidth) +
                    (numTreads * sizeof(Y) * scanWidth) + 512;

    indicesAlongDimension<X, Y>
        <<<256, numTreads, shMemSize, *context->getCudaStream()>>>(
            input->specialBuffer(), packX.platformShapeInfo(),
            packX.platformOffsets(), indices->specialBuffer(),
            packI.platformShapeInfo(), packI.platformOffsets(),
            values->specialBuffer(), packZ.platformShapeInfo(),
...
int topKFunctor(sd::LaunchContext* context, const NDArray* input,
                NDArray* values, NDArray* indices, const uint k,
                bool needSort) {
  input->syncToDevice();

  BUILD_DOUBLE_SELECTOR(input->dataType(), indices->dataType(), topKFunctor_,
                        (context, input, values, indices, k, needSort),
