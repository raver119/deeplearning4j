...
__global__ static void padCuda(const int mode, const void* vx,
  ...
  __shared__ Nd4jLong zLen, totalThreads, *coords, *xShape, *zShape, shift1,
      shift2, yStride0;
  ...
  auto xzCoord =
      coords +
      threadIdx.x * rank;  // we use xzCoord storage both for x and z arrays
  ...
  if (mode == 0) {  // CONSTANT case
    ...
    for (Nd4jLong i = tid; i < zLen; i += totalThreads) {
      ...
      for (int j = rankMinusOne; j >= 0; --j) {
        ...
        xzCoord[j] =
            xzCoord[j] -
            y[shape::getIndexOffset(
                yStride0 * j, yShapeInfo)];  // are ready to fill middle (within
                                             // input dimension range)
        if (xzCoord[j] < 0)
          xzCoord[j] = -xzCoord[j] - shift1;  // means fill from left
        else if (xzCoord[j] >= xShape[j])
          xzCoord[j] =
              2 * xShape[j] - xzCoord[j] - shift2;  // means fill from right
      }
...
///////////////////////////////////////////////////////////////////
template <typename X, typename Y>
static void padCudaLauncher(const int blocksPerGrid, const int threadsPerBlock,
                            const int sharedMem, const cudaStream_t* stream,
                            const int mode, const void* vx,
                            const Nd4jLong* xShapeInfo, const void* vy,
                            const Nd4jLong* yShapeInfo, void* vz,
                            const Nd4jLong* zShapeInfo, const void* padVal) {
  padCuda<X, Y><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      mode, vx, xShapeInfo, vy, yShapeInfo, vz, zShapeInfo, padVal);
}

///////////////////////////////////////////////////////////////////
void pad(sd::LaunchContext* context, const int mode, const NDArray& input,
         const NDArray& paddings, NDArray& output, const NDArray& padValue) {
  ...
  BUILD_DOUBLE_SELECTOR(xType, yType, padCudaLauncher,
                        (blocksPerGrid, threadsPerBlock, sharedMem,
                         context->getCudaStream(), mode, input.specialBuffer(),
                         input.specialShapeInfo(), paddings.specialBuffer(),
                         paddings.specialShapeInfo(), output.specialBuffer(),
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
static __global__ void mirrorPadLinearKernel(
    void const* vx, const Nd4jLong* xShape, void* vz, const Nd4jLong* zShape,
    Nd4jLong leftSide, Nd4jLong leftSideCorrected, Nd4jLong xLen, Nd4jLong len,
    Nd4jLong zLen) {
...
static __global__ void mirrorPadKernel(void const* vx, const Nd4jLong* xShape,
                                       void* vz, const Nd4jLong* zShape,
                                       Nd4jLong outLen, void const* paddings,
                                       const Nd4jLong* paddingShape,
                                       int reflBorder) {
  ...
  for (Nd4jLong i = start; i < outLen; i += step) {
    ...
    for (int j = 0; j < rank; j++) {
      ...
      auto padOffset =
          shape::getOffset(paddingShape, coords);  // padding already has rank 2
      ...
      else if (xzCoord[j] >= leftSide &&
               xzCoord[j] < leftSide + inLen)  // middle
...
template <typename F, typename I>
static void mirrorPad_(sd::LaunchContext* context, const NDArray& input,
                       const NDArray& paddings, NDArray& output,
                       const int mode) {
  ...
  if (rank <= 1) {
    ...
    mirrorPadLinearKernel<F><<<256, 512, 256, *stream>>>(
        input.specialBuffer(), input.specialShapeInfo(), output.specialBuffer(),
        output.specialShapeInfo(), leftSide, leftSideCorrected, inLen, len,
        outLen);
    sd::DebugHelper::checkErrorCode(
        stream, "helpers::mirrorPadLinearKernel(...) failed");
  } else {
    mirrorPadKernel<F, I><<<256, 256, 8192, *stream>>>(
        input.specialBuffer(), input.specialShapeInfo(), output.specialBuffer(),
        output.specialShapeInfo(), outLen, paddings.specialBuffer(),
        paddings.specialShapeInfo(), reflBorder);
    sd::DebugHelper::checkErrorCode(stream,
                                    "helpers::mirrorPadKernel(...) failed");
  }
...
void mirrorPad(sd::LaunchContext* context, const NDArray& input,
  ...
  BUILD_DOUBLE_SELECTOR(input.dataType(), paddings.dataType(), mirrorPad_,
                        (context, input, paddings, output, mode), LIBND4J_TYPES,
                        INDEXING_TYPES);
}
