...
static __global__ void reverseTadKernel(
    const void* vinput, const Nd4jLong* inputShape, void* voutput,
    const Nd4jLong* outputShape, const Nd4jLong* inputTadShape,
    const Nd4jLong* inputTadOffsets, const Nd4jLong* outputTadShape,
    const Nd4jLong* outputTadOffsets, uint64_t limit,
  ...
  for (uint64_t e = tid; e < rlimit; e += step) {
    ...
    auto lOffset =
        shape::getIndexOffset(numOfElemsToReverse - idx - 1, inputTadShape);
    ...
    auto zlOffset =
        shape::getIndexOffset(numOfElemsToReverse - idx - 1, outputTadShape);
  ...
  if (odd && threadIdx.x == 0) {
    for (uint64_t e = blockIdx.x; e < numTads; e += gridDim.x) {
      ...
      auto xOffset =
          shape::getIndexOffset(numOfElemsToReverse / 2, inputTadShape);
      auto zOffset =
          shape::getIndexOffset(numOfElemsToReverse / 2, outputTadShape);
...
template <typename T>
static __global__ void reverseArrayKernel(const void* input,
                                          const Nd4jLong* inputShape,
                                          void* output,
                                          const Nd4jLong* outputShape,
                                          Nd4jLong numOfElemsToReverse) {
  const auto tid = blockIdx.x * blockDim.x + threadIdx.x;
  const auto step = gridDim.x * blockDim.x;
  ...
  for (uint64_t e = tid; e < limit; e += step) {
    ...
    auto lOffset =
        shape::getIndexOffset(numOfElemsToReverse - e - 1, inputShape);
    ...
    auto zlOffset =
        shape::getIndexOffset(numOfElemsToReverse - e - 1, outputShape);
...
template <typename T>
static void reverseTad(sd::LaunchContext* context, const NDArray* input,
                       NDArray* output, const Nd4jLong* inputTadShape,
                       const Nd4jLong* inputTadOffsets,
                       const Nd4jLong* outputTadShape,
                       const Nd4jLong* outputTadOffsets, uint64_t tadLength) {
  auto stream = context->getCudaStream();
  reverseTadKernel<T><<<256, 512, 8192, *stream>>>(
...
template <typename T>
static void reverseArray(sd::LaunchContext* context, const NDArray* input,
                         NDArray* output, Nd4jLong numOfElemsToReverse) {
  auto stream = context->getCudaStream();
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void reverseSequence_(sd::LaunchContext* context, const NDArray* input,
                             const NDArray* seqLengths, NDArray* output,
  ...
  if (input->isVector() ||
      shape::isLikeVector(input->shapeInfo(), posOfNonUnityDim) ||
      seqLengths->lengthOf() == 1) {
    int numOfElemsToReverse = seqLengths->e<int>(0);
    if ((seqDim == 0 && input->sizeAt(0) == 1) ||
        (batchDim == posOfNonUnityDim))
      output->assign(input);
    else
      reverseArrayKernel<T><<<256, 512, 8192, *stream>>>(
          input->specialBuffer(), input->specialShapeInfo(),
          output->specialBuffer(), output->specialShapeInfo(),
          numOfElemsToReverse);  // helpers::reverseArray<T>(context,
                                 // const_cast<NDArray*>(input), output,
                                 // numOfElemsToReverse);
  } else {
    if (seqDim > batchDim) --seqDim;

    std::vector<int> dimensions =
        ShapeUtils::evalDimsToExclude(input->rankOf(), {batchDim});
...
void reverseSequence(sd::LaunchContext* context, const NDArray* input,
  ...
  BUILD_SINGLE_SELECTOR(input->dataType(), reverseSequence_,
                        (context, input, seqLengths, output, seqDim, batchDim),
                        LIBND4J_TYPES);
...
void reverse(sd::LaunchContext* context, const NDArray* input, NDArray* output,
  ...
  if (packX.numberOfTads() == 1) {
    ...
    BUILD_SINGLE_SELECTOR(
        ...
        (context, input, output, packX.platformShapeInfo(),
         packX.platformOffsets(), packZ.platformShapeInfo(),
         packZ.platformOffsets(),
         (uint64_t)(input->lengthOf() / packX.numberOfTads())),
        LIBND4J_TYPES);
  }
