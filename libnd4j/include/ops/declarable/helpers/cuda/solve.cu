...
template <typename T>
static __global__ void oneOnDiagonalKernel(T* ioBuf, Nd4jLong const* ioShape,
                                           Nd4jLong const* tadShape,
                                           Nd4jLong const* tadOffsets,
                                           Nd4jLong batchNum, Nd4jLong rowNum) {
...
template <typename T>
static __global__ void restorePermutationsKernel(
    T* PBuf, Nd4jLong const* PShapeInfo, int const* permutationsBuf,
    Nd4jLong const* PTadShapeInfo, Nd4jLong const* PTadSOffsets,
    Nd4jLong const* permutationsTadShapeInfo,
    Nd4jLong const* permutationsTadOffsets, Nd4jLong batchNum,
    Nd4jLong rowNum) {
...
template <typename T>
static int solveFunctor_(sd::LaunchContext* context, NDArray* leftInput,
                         NDArray* rightInput, bool adjoint, NDArray* output) {
  ...
  oneOnDiagonalKernel<T><<<128, 256, 256, *stream>>>(
      leftLower.dataBuffer()->specialAsT<T>(), leftLower.specialShapeInfo(),
      leftLowerTad.specialShapeInfo(), leftLowerTad.specialOffsets(),
  ...
  restorePermutationsKernel<T><<<128, 256, 256, *stream>>>(
      ...
      permutations.dataBuffer()->specialAsT<int>(), PTad.specialShapeInfo(),
      PTad.specialOffsets(), permutationsTad.specialShapeInfo(),
      permutationsTad.specialOffsets(), permutationsTad.numberOfTads(),
      permutations.sizeAt(-1));
  P.tickWriteDevice();
  ...
  helpers::triangularSolveFunctor(context, &leftLower, &rightPart, true, false,
                                  &rightOutput);
...
int solveFunctor(sd::LaunchContext* context, NDArray* leftInput,
                 NDArray* rightInput, bool adjoint, NDArray* output) {
  BUILD_SINGLE_SELECTOR(leftInput->dataType(), return solveFunctor_,
                        (context, leftInput, rightInput, adjoint, output),
                        FLOAT_TYPES);
}

template <typename T>
static __global__ void adjointKernel(T* output, Nd4jLong batchSize,
                                     Nd4jLong rows, Nd4jLong columns,
                                     Nd4jLong const* outputTads,
...
static void adjointMatrix_(sd::LaunchContext* context, NDArray const* input,
  ...
  output->assign(input);
  adjointKernel<T><<<128, 256, 256, *stream>>>(
...
void adjointMatrix(sd::LaunchContext* context, NDArray const* input,
                   NDArray* output) {
  BUILD_SINGLE_SELECTOR(input->dataType(), adjointMatrix_,
