...
//////////////////////////////////////////////////////////////////////////
// columns [bS, iC, kD, kH, kW, oD, oH, oW] to be de-convoluted to volume [bS,
// iC, iD, iH, iW]
template <typename T>
static __global__ void col2volCuda(const void* columns,
                                   const Nd4jLong* colShapeInfo, void* volume,
                                   const Nd4jLong* volShapeInfo, const int sD,
  ...
  for (Nd4jLong i = tid; i < volLen; i += gridDim.x * blockDim.x) {
    ...
    const auto bSiCoffset =
        coords[0] * colShapeInfo[9] + coords[1] * colShapeInfo[10];
    ...
    for (uint colD = colDstart; colD < colDend; ++colD) {
      ...
      for (uint colH = colHstart; colH < colHend; ++colH) {
        ...
        for (uint colW = colWstart; colW < colWend; ++colW) {
          ...
          val += col[bSiCoffset + (coords[2] / dD) * colShapeInfo[11] +
                     (coords[3] / dH) * colShapeInfo[12] +
                     (coords[4] / dW) * colShapeInfo[13] +
                     colD * colShapeInfo[14] + colH * colShapeInfo[15] +
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void col2volCudaLauncher(const int blocksPerGrid,
                                ...
                                const int pH, const int pW, const int dD,
                                const int dH, const int dW) {
  col2volCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      columns, colShapeInfo, volume, volShapeInfo, sD, sH, sW, pD, pH, pW, dD,
      dH, dW);
}

//////////////////////////////////////////////////////////////////////////
void ConvolutionUtils::col2vol(sd::graph::Context& block, const NDArray& col,
                               NDArray& vol, const int sD, const int sH,
  ...
  const int blocksPerGrid =
      (vol.lengthOf() + threadsPerBlock - 1) / threadsPerBlock;
  ...
  BUILD_SINGLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem,
       block.launchContext()->getCudaStream(), col.specialBuffer(),
       col.specialShapeInfo(), vol.specialBuffer(), vol.specialShapeInfo(), sD,
       sH, sW, pD, pH, pW, dD, dH, dW),
      FLOAT_TYPES);
