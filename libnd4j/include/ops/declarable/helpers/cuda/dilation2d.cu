...
__global__ static void dilation2dCuda(const void* vx,
                                      const Nd4jLong* xShapeInfo,
                                      const void* vy,
                                      const Nd4jLong* yShapeInfo, void* vz,
                                      const Nd4jLong* zShapeInfo, const int sH,
  ...
  for (yCoords[0] = 0; yCoords[0] < kH; ++yCoords[0]) {
    ...
    for (yCoords[1] = 0; yCoords[1] < kW; ++yCoords[1]) {
      ...
      const X val = x[shape::getOffset(xShapeInfo, xzCoords)] +
                    y[shape::getOffset(yShapeInfo, yCoords)];
      if (val > max) max = val;
    }
  }
...
//////////////////////////////////////////////////////////////////////////
template <typename X, typename Z>
static void dilation2dCudaLauncher(
    ...
    const Nd4jLong* zShapeInfo, const int sH, const int sW, const int pH,
    const int pW, const int dH, const int dW) {
  dilation2dCuda<X, Z><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, vy, yShapeInfo, vz, zShapeInfo, sH, sW, pH, pW, dH, dW);
}

void dilation2d(sd::LaunchContext* context, NDArray* input, NDArray* weights,
  ...
  const int sharedMem =
      (weights->rankOf() + output->rankOf()) * sizeof(int) * threadsPerBlock +
      128;
  ...
  BUILD_SINGLE_SELECTOR_TWICE(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(),
       ...
       weights->specialBuffer(), weights->specialShapeInfo(),
       output->specialBuffer(), output->specialShapeInfo(), sH, sW, pH, pW, dH,
       dW),
      FLOAT_TYPES);
