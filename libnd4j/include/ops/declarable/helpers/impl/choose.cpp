...
namespace helpers {
template <typename T>
static sd::NDArray* processCondition_(int mode, sd::NDArray* arg,
                                      sd::NDArray* comp,
...
template <typename T>
sd::NDArray* processCondition_(int mode, sd::NDArray* arg, sd::NDArray* comp,
                               sd::NDArray* output, sd::NDArray* numResult,
                               sd::NDArray& compScalar) {
  ...
  if (comp != nullptr) {
    ...
    for (Nd4jLong i = 0; i < arg->lengthOf(); i++) {
      T result2 =
          processElementCondition(mode, arg->e<T>(i), compScalar.e<T>(0));
      if (result2 > static_cast<T>(0)) {
        if (output != nullptr) output->p(numResults, arg->e<T>(i));
        numResults++;
      }
...
sd::NDArray* processCondition(sd::LaunchContext* context, int mode,
                              sd::NDArray* arg, sd::NDArray* comp,
                              sd::NDArray* output, sd::NDArray* numResult,
                              sd::NDArray& compScalar) {
  arg->syncToHost();
  ...
  BUILD_SINGLE_SELECTOR(arg->dataType(), return processCondition_,
                        (mode, arg, comp, output, numResult, compScalar),
                        FLOAT_TYPES);
...
BUILD_SINGLE_TEMPLATE(template NDArray* processCondition_,
                      (int mode, sd::NDArray* arg, sd::NDArray* comp,
                       sd::NDArray* output, sd::NDArray* numResult,
                       sd::NDArray& compScalar),
                      FLOAT_TYPES);
...
void chooseFunctorArray(sd::LaunchContext* context, NDArray* arg, NDArray* comp,
...
void chooseFunctorScalar(sd::LaunchContext* context, NDArray* arg,
                         double scalar, int mode, NDArray* result,
                         NDArray* numResults) {
