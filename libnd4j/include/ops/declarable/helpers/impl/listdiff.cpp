...
Nd4jLong listDiffCount(sd::LaunchContext* context, NDArray* values,
  ...
  BUILD_SINGLE_SELECTOR(xType, return listDiffCount_, (values, keep),
                        LIBND4J_TYPES);

  NDArray::registerPrimaryUse({}, {values, keep});
}

BUILD_SINGLE_TEMPLATE(template Nd4jLong listDiffCount_,
                      (NDArray * values, NDArray* keep);
                      , LIBND4J_TYPES);

template <typename T>
static int listDiffFunctor_(NDArray* values, NDArray* keep, NDArray* output1,
                            NDArray* output2) {
  std::vector<T> saved;
  ...
  for (Nd4jLong e = 0; e < values->lengthOf(); e++) {
    ...
    ExtraArguments extras({v, 0.0, 10.0});
    NDArray idxScalar =
        keep->indexReduceNumber(indexreduce::FirstIndex, &extras);
  ...
  if (saved.size() == 0) {
    //            if (sd::ops::conditionHelper(__FILE__, __LINE__, false, 0,
    //            "ListDiff: search returned no results") != 0)
    nd4j_printf("ListDiff: search returned no results", "");
    throw std::invalid_argument("Op validation failed");
  } else {
    auto z0 = output1;  // OUTPUT_VARIABLE(0); //new NDArray<T>('c', {(int)
                        // saved.size()});
    auto z1 = output2;  // OUTPUT_VARIABLE(1); //new NDArray<T>('c', {(int)
                        // saved.size()});
...
int listDiffFunctor(sd::LaunchContext* context, NDArray* values, NDArray* keep,
  ...
  if (DataTypeUtils::isR(xType)) {
    BUILD_SINGLE_SELECTOR(xType, result = listDiffFunctor_,
                          (values, keep, output1, output2), FLOAT_TYPES);
  } else if (DataTypeUtils::isZ(xType)) {
    BUILD_SINGLE_SELECTOR(xType, result = listDiffFunctor_,
                          (values, keep, output1, output2), INTEGER_TYPES);
  } else {
    throw std::runtime_error(
        "ListDiff: Only integer and floating point data types are supported");
  }
...
BUILD_SINGLE_TEMPLATE(template int listDiffFunctor_,
                      (NDArray * values, NDArray* keep, NDArray* output1,
                       NDArray* output2);
                      , FLOAT_TYPES);
BUILD_SINGLE_TEMPLATE(template int listDiffFunctor_,
                      (NDArray * values, NDArray* keep, NDArray* output1,
