...
static void fill_(const void *vvalues, const void *vindices, void *voutput,
                  const Nd4jLong *zShapeInfo, uint8_t rank, uint64_t length) {
...
void compat_sparse_to_dense(const NDArray &values, const NDArray &indices,
  ...
  if (output.isS()) {
    // string case is not so trivial, since elements might, and probably will,
    ...
    auto bufferLength =
        defaultLength * (output.lengthOf() - numValues) + valuesLength;
    ...
    auto dataBuffer =
        reinterpret_cast<uint8_t *>(offsetsBuffer + output.lengthOf());

    offsetsBuffer[0] = 0;

    // getting initial value coords
    for (int e = 0; e < rank; e++) valueCoords[e] = indices.e<Nd4jLong>(e);
    ...
    // write out values
    BUILD_DOUBLE_SELECTOR(values.dataType(), indices.dataType(), fill_,
                          (values.buffer(), indices.buffer(), output.buffer(),
                           output.shapeInfo(), rank, values.lengthOf()),
