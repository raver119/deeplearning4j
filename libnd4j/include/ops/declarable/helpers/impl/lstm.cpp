...
/////////////////////////////////////////////////////////////////////////////
void lstmBlockTimeLoop(const NDArray* maxSeqLength, const NDArray* xSeq,
                       const NDArray* c0, const NDArray* y0, const NDArray* W,
                       const NDArray* Wci, const NDArray* Wcf,
                       const NDArray* Wco, const NDArray* b,
                       const NDArray* iSeq, const NDArray* cSeq,
                       const NDArray* fSeq, const NDArray* oSeq,
                       const NDArray* zSeq, const NDArray* hSeq,
                       const NDArray* ySeq, const std::vector<double>& params,
  ...
  for (int t = 0; t < seqLen; ++t) {
    ...
    helpers::lstmBlockCell(&xt, c_t1, y_t1, W, Wci, Wcf, Wco, b, &it, &ct, &ft,
                           &ot, &zt, &ht, &yt, params);
...
void lstmTimeLoop(sd::LaunchContext* context, const NDArray* x,
  ...
  // h0 initial cell output (at time step = 0) [bS x numProj], in case of
  // projection=false -> numProj == numUnits !!! c0 initial cell state  (at time
  // step = 0) [bS x numUnits],
  ...
  for (int t = 0; t < time; ++t) {
    ...
    helpers::lstmCell(context, &xt, &currentH, &currentC, Wx, Wh, Wc, Wp, b,
                      &ht, &ct, params);
    currentH.assign(ht);
    currentC.assign(ct);
  }
