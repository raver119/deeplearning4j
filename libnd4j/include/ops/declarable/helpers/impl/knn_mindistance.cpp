...
namespace helpers {
template <typename T>
void mindistance_(const void *vinput, const void *vlow, const void *vhigh,
                  int32_t length, void *vout) {
...
void knn_mindistance(const NDArray &input, const NDArray &lowest,
  ...
  BUILD_SINGLE_SELECTOR(input.dataType(), mindistance_,
                        (input.buffer(), lowest.buffer(), highest.buffer(),
                         input.lengthOf(), output.buffer()),
                        FLOAT_TYPES);
