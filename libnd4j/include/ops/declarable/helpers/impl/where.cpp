...
namespace helpers {
template <typename T>
static void __where(NDArray &condition, NDArray &output,
  ...
  for (Nd4jLong e = 0; e < condition.lengthOf(); e++) {
    ...
    if (condition.e<bool>(offset)) {
      auto array = NDArrayFactory::create(
          'c', {1, condition.rankOf()}, output.dataType(), output.getContext());
...
BUILD_SINGLE_TEMPLATE(template void __where,
                      (NDArray & condition, NDArray &output,
                       memory::Workspace *workspace),
                      LIBND4J_TYPES);
...
void _where(sd::LaunchContext *context, NDArray &condition, NDArray &output,
  ...
  BUILD_SINGLE_SELECTOR(output.dataType(), __where,
                        (condition, output, workspace), LIBND4J_TYPES);
  output.syncToDevice();
}
}  // namespace helpers
}  // namespace ops
