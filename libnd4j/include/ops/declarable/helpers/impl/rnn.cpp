...
// function nnCell implements an Elman RNN cell: output = activation(Wx*x + bx
// +  Wh*ht  + bh)
...
void rnnCell(sd::LaunchContext* context, const NDArray* xt, const NDArray* Wx,
             const NDArray* Wh, const NDArray* b, const NDArray* hPrev,
             NDArray* ht) {
  // xt    input [bS x iS]
  // Wx    input-to-hidden weights, [iS  x nU]
  // Wh    hidden-to-hidden weights, [nU x nU]
  // b     biases, [2*nU]: {0, nU} are input-to-hidden biases and {nU, 2*nU} are
  // hidden-to-hidden biases hPrev previous cell output [bS x nU],  that is at
  // previous time step t-1, in case of projection=false -> nU=nU!!!

  const int nU = hPrev->sizeAt(1);

  // ht is current cell output [bS x nU], that is at current time step t
  ht->assign(
      mmul(*xt, *Wx) + (*b)({{0, nU}}) + mmul(*hPrev, *Wh) +
      (*b)({{nU,
             2 * nU}}));  // [bS x nU] + [nU]  +  [bS x nU] + [nU] = [bS x nU]
...
void rnnTimeLoop(sd::LaunchContext* context, const NDArray* x,
  ...
  // maxTimeStep vector [bS] containing integer values within [0,time), each
  // element of this vector set max time step per each input in batch, this
  // means there are no calculations for time >= maxTimeStep
