...
//////////////////////////////////////////////////////////////////////
static void xwPlusBiasMKLDNN(const NDArray* x, const NDArray* weights,
                             const NDArray* bias, NDArray* z,
  ...
  dnnl::memory::data_type wType = (weights->dataType() == DataType::FLOAT32)
                                      ? wType = dnnl::memory::data_type::f32
  ...
  dnnl::memory::desc x_mkl_md =
      dnnl::memory::desc(xShape, xType, dnnl::memory::format_tag::any);
  dnnl::memory::desc x_user_md = dnnl::memory::desc(xShape, xType, mkldnnUtils::getFormat(*x));
  mkldnnUtils::setBlockStrides(*x, x_user_md);

  // weights
  dnnl::memory::desc weights_mkl_md =
      dnnl::memory::desc(wShape, wType, dnnl::memory::format_tag::any);
  dnnl::memory::desc weights_user_md =
      dnnl::memory::desc(wShape, wType, mkldnnUtils::getFormat(*weights));
      mkldnnUtils::setBlockStrides(*
          weights,
      weights_user_md, bShouldTransp ? std::vector<int>({1,0}) : std::vector<int>());

  // bias
  dnnl::memory::desc bias_mkl_md =
      dnnl::memory::desc(bShape, bType, dnnl::memory::format_tag::a);
  dnnl::memory::desc bias_user_md =
      dnnl::memory::desc(bShape, bType, dnnl::memory::format_tag::a);
  mkldnnUtils::setBlockStrides(*bias, bias_user_md);

  // z
  dnnl::memory::desc z_mkl_md =
      dnnl::memory::desc(zShape, zType, dnnl::memory::format_tag::any);
  ...
  dnnl::inner_product_forward::desc op_desc(dnnl::prop_kind::forward_inference,
                                            x_mkl_md, weights_mkl_md,
                                            bias_mkl_md, z_mkl_md);
  ...
  // weights
  mkldnnUtils::loadDataToMklStream(*weights, engine, stream, weights_user_md,
                                   op_prim_desc.weights_desc(),
                                   args[DNNL_ARG_WEIGHTS]);

  // bias
  auto bias_mkl_mem =
      dnnl::memory(bias_mkl_md, engine, const_cast<void*>(bias->buffer()));
...
//////////////////////////////////////////////////////////////////////
static void xwPlusBiasBp(const NDArray* x, const NDArray* weights,
                         const NDArray* bias, const NDArray* dLdz,
                         NDArray* dLdx, NDArray* dLdw, NDArray* dLdb,
                         const bool bShouldTransp) {
  // mkl works with following
  // [M,K]     x [N,K]^T  + [N]   = [M,N]
  const auto xRank = x->rankOf();
  ...
  // x
  dnnl::memory::desc x_mkl_md =
      dnnl::memory::desc(xShape, dataType, dnnl::memory::format_tag::any);
  dnnl::memory::desc x_user_md = dnnl::memory::desc(xShape, dataType, mkldnnUtils::getFormat(*x));
  mkldnnUtils::setBlockStrides(*x, x_user_md);

  // weights
  dnnl::memory::desc weights_mkl_md =
      dnnl::memory::desc(wShape, dataType, dnnl::memory::format_tag::any);
  dnnl::memory::desc weights_user_md =
      dnnl::memory::desc(wShape, dataType, mkldnnUtils::getFormat(*weights));
      mkldnnUtils::setBlockStrides(*
          weights,
      weights_user_md, bShouldTransp ? std::vector<int>({1,0}) : std::vector<int>());
  ...
  // bias
  dnnl::memory::desc bias_mkl_md =
      dnnl::memory::desc(bShape, dataType, dnnl::memory::format_tag::any);
  dnnl::memory::desc bias_user_md =
      dnnl::memory::desc(bShape, dataType, mkldnnUtils::getFormat(*bias));
  mkldnnUtils::setBlockStrides(*bias, bias_user_md);

  // dLdz
  dnnl::memory::desc dLdz_mkl_md =
      dnnl::memory::desc(dLdzShape, dataType, dnnl::memory::format_tag::any);
  dnnl::memory::desc dLdz_user_md =
      dnnl::memory::desc(dLdzShape, dataType, mkldnnUtils::getFormat(*dLdz));
  ...
  dnnl::memory::desc dLdw_user_md =
      dnnl::memory::desc(wShape, dataType, mkldnnUtils::getFormat(*dLdw));
      mkldnnUtils::setBlockStrides(*dLdw,
      dLdw_user_md, bShouldTransp ? std::vector<int>({1,0}) : std::vector<int>());

  // dLdb
  dnnl::memory::desc dLdb_mkl_md =
      dnnl::memory::desc(bShape, dataType, dnnl::memory::format_tag::any);
  dnnl::memory::desc dLdb_user_md =
      dnnl::memory::desc(bShape, dataType, mkldnnUtils::getFormat(*dLdb));
  ...
  // backprob
  // dLdw
  auto op_bpdw_desc = inner_product_backward_weights::desc(
      x_mkl_md, dLdw_mkl_md, dLdb_mkl_md, dLdz_mkl_md);
  auto op_bpdw_prim_desc = inner_product_backward_weights::primitive_desc(
      op_bpdw_desc, engine, op_ff_prim_desc);

  // backprob
  // dLdx
  auto op_bpdx_desc = inner_product_backward_data::desc(
      dLdx_mkl_md, weights_mkl_md, dLdz_mkl_md);
  auto op_bpdx_prim_desc = inner_product_backward_data::primitive_desc(
      op_bpdx_desc, engine, op_ff_prim_desc);
  ...
  mkldnnUtils::loadDataToMklStream(*dLdz, engine, stream, dLdz_user_md,
                                   op_bpdw_prim_desc.diff_dst_desc(),
                                   argsDw[DNNL_ARG_DIFF_DST]);

  // dLdz - dx
  mkldnnUtils::loadDataToMklStream(*dLdz, engine, stream, dLdz_user_md,
                                   op_bpdx_prim_desc.diff_dst_desc(),
                                   argsDx[DNNL_ARG_DIFF_DST]);

  // input x for dw
  mkldnnUtils::loadDataToMklStream(*x, engine, stream, x_user_md,
                                   op_bpdw_prim_desc.src_desc(),
                                   argsDw[DNNL_ARG_SRC]);

  // weights - dx
  mkldnnUtils::loadDataToMklStream(*weights, engine, stream, weights_user_md,
                                   op_bpdx_prim_desc.weights_desc(),
                                   argsDx[DNNL_ARG_WEIGHTS]);

  // dLdw
  auto dLdw_user_mem = mkldnnUtils::loadDataToMklStream(*dLdw, engine, stream, dLdw_user_md,
      op_bpdw_prim_desc.diff_weights_desc() ,
  argsDw[DNNL_ARG_DIFF_WEIGHTS] );

  // dLdx
  auto dLdx_user_mem = mkldnnUtils::loadDataToMklStream(*dLdx, engine, stream, dLdx_user_md,
      op_bpdx_prim_desc.diff_src_desc() ,
  argsDx[DNNL_ARG_DIFF_SRC] );

  // dLdb
  auto dLdb_user_mem = mkldnnUtils::loadDataToMklStream(*dLdb, engine, stream, dLdb_user_md,
      op_bpdw_prim_desc.diff_bias_desc() ,
  argsDw[DNNL_ARG_DIFF_BIAS] );

  // run calculations dw
  dnnl::inner_product_backward_weights(op_bpdw_prim_desc)
      .execute(stream, argsDw);
  // run calculations dx
  ...
  if (op_bpdx_prim_desc.diff_src_desc() != dLdx_user_mem.get_desc())
    dnnl::reorder(argsDx[DNNL_ARG_DIFF_SRC], dLdx_user_mem)
        .execute(stream, argsDx[DNNL_ARG_DIFF_SRC], dLdx_user_mem);
  ...
  if (op_bpdw_prim_desc.diff_bias_desc() != dLdb_user_mem.get_desc())
    dnnl::reorder(argsDw[DNNL_ARG_DIFF_BIAS], dLdb_user_mem)
        .execute(stream, argsDw[DNNL_ARG_DIFF_BIAS], dLdb_user_mem);
...
PLATFORM_IMPL(xw_plus_b, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(xRank == 2, 0,
               "xw_plus_b MKL: Input x array should have rank equal 2, but got "
               "instead %i!",
               xRank);
  REQUIRE_TRUE(wRank == 2, 0,
               "xw_plus_b MKL: Input weights array should have rank equal 2, "
               "but got instead %i!",
               wRank);
  REQUIRE_TRUE(zRank == 2, 0,
               "xw_plus_b MKL: Output array should have rank equal 2, but got "
               "instead %i!",
               zRank);

  REQUIRE_TRUE(1 == b->rankOf() && b->lengthOf() == z->sizeAt(-1), 0,
               "xw_plus_b MKL: Input bias vector should be 1D and have proper "
               "dimension 1x%i."
...
PLATFORM_IMPL(xw_plus_b_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(x->rankOf() == 2, 0,
               "xw_plus_b BP MKL: Input x array should have rank equal 2, but "
               "got instead %i!",
               x->rankOf());
  REQUIRE_TRUE(w->rankOf() == 2, 0,
               "xw_plus_b BP MKL: Input weights array should have rank equal "
               "2, but got instead %i!",
               w->rankOf());
  REQUIRE_TRUE(dLdz->rankOf() == 2, 0,
               "xw_plus_b BP MKL: Output array should have rank equal 2, but "
               "got instead %i!",
               dLdz->rankOf());
  REQUIRE_TRUE(1 == b->rankOf() && b->lengthOf() == dLdz->sizeAt(1), 0,
               "xw_plus_b BP MKL: Input bias vector should be 1D and have "
               "proper dimension 1x%i."
               " But got rank %i, and got length %i instead %i.",
