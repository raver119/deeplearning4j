...
static void xwPlusBiasMKLDNN(const NDArray* x, const NDArray* weights,
  ...
  dnnl::memory::desc weights_user_md =
      dnnl::memory::desc(wShape, wType, format);
  if (weights->ews() != 1 || weights->ordering() != 'c' || bShouldTransp) {
    weights_user_md.data.format_kind = dnnl_blocked;  // overrides format
    if (bShouldTransp) {
      weights_user_md.data.format_desc.blocking.strides[0] =
          weights->strideAt(1);
      weights_user_md.data.format_desc.blocking.strides[1] =
          weights->strideAt(0);
    } else {
      weights_user_md.data.format_desc.blocking.strides[0] =
          weights->strideAt(0);
  ...
  dnnl::memory::desc bias_mkl_md =
      dnnl::memory::desc(bShape, bType, dnnl::memory::format_tag::x);
...
static void xwPlusBiasBp(const NDArray* x, const NDArray* weights,
  ...
  // bias
  dnnl::memory::desc bias_mkl_md =
      dnnl::memory::desc(bShape, dataType, dnnl::memory::format_tag::x);
  dnnl::memory::desc bias_user_md =
      dnnl::memory::desc(bShape, dataType, dnnl::memory::format_tag::x);
  mkldnnUtils::setBlockStrides(bias, bias_user_md);
  ...
  dnnl::memory::desc dLdz_user_md =
      dnnl::memory::desc(dLdzShape, dataType, format);
  ...
  const bool dLdwReorder =
      op_bpdw_prim_desc.diff_weights_desc() != dLdw_user_mem.get_desc();
  auto dLdw_mkl_mem =
      dLdwReorder ? dnnl::memory(op_bpdw_prim_desc.diff_weights_desc(), engine)
                  : dLdw_user_mem;
  ...
  const bool dLdxReorder =
      op_bpdx_prim_desc.diff_src_desc() != dLdx_user_mem.get_desc();
  auto dLdx_mkl_mem =
      dLdxReorder ? dnnl::memory(op_bpdx_prim_desc.diff_src_desc(), engine)
                  : dLdx_user_mem;
  argsDx[DNNL_ARG_DIFF_SRC] = dLdx_mkl_mem;

  // dLdb
  auto dLdb_user_mem = dnnl::memory(dLdb_user_md, engine, dLdb->buffer());
  const bool dLdbReorder =
      op_bpdw_prim_desc.diff_bias_desc() != dLdb_user_mem.get_desc();
  auto dLdb_mkl_mem =
      dLdbReorder ? dnnl::memory(op_bpdw_prim_desc.diff_bias_desc(), engine)
                  : dLdb_user_mem;
  ...
  if (dLdwReorder)
    dnnl::reorder(dLdw_mkl_mem, dLdw_user_mem)
        .execute(stream, dLdw_mkl_mem, dLdw_user_mem);

  if (dLdbReorder)
    dnnl::reorder(dLdb_mkl_mem, dLdb_user_mem)
