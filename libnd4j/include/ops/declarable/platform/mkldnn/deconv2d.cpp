...
//////////////////////////////////////////////////////////////////////////
static void deconv2dMKLDNN(const NDArray* input, const NDArray* weights,
  ...
  dnnl::memory::dims padding_r = {(iH - 1) * sH - oH + kH - pH,
                                  (iW - 1) * sW - oW + kW - pW};
  ...
  dnnl::memory::format_tag xFormatMkl =
...
//////////////////////////////////////////////////////////////////////////
static void deconv2dBpMKLDNN(const NDArray* input, const NDArray* weights,
...
PLATFORM_IMPL(deconv2d, ENGINE_CPU) {
  ...
  auto weights = INPUT_VARIABLE(
      1);  // [kH, kW, oC, iC], [iC, oC, kH, kW], [iC, kH, kW, oC]
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "CUSTOM DECONV2D_MKLDNN OP: rank of input array must be equal "
               "to 4, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(weights->rankOf() == 4, 0,
               "CUSTOM DECONV2D_MKLDNN OP: rank of weights array must be equal "
               "to 4, but got %i instead !",
               weights->rankOf());
  ...
  int wFormat =
      block.numI() > 10 ? INT_ARG(10) : 0;  // 0 - [kH, kW, oC, iC], 1 - [iC,
                                            // oC, kH, kW], 2 - [iC, kH, kW, oC]
  ...
  std::vector<Nd4jLong> expectedWeightsShape =
      ConvolutionUtils::expectWeightsShape(wFormat, kH, kW, oC, iC);
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DECONV2D_MKLDNN OP: wrong shape of weights array, "
  ...
  if (bias)
    REQUIRE_TRUE(bias->rankOf() <= 2 && oC == bias->lengthOf(), 0,
                 "CUSTOM DECONV2D_MKLDNN OP: wrong shape of array with biases, "
  ...
  if (paddingMode) {  // SAME
    ...
    ConvolutionUtils::calcPadding2D(pH, pW, iH, iW, oH, oW, kH, kW, sH, sW, dH,
                                    dW);
  }

  deconv2dMKLDNN(input, weights, bias, output, kH, kW, sH, sW, pH, pW, dH, dW,
...
PLATFORM_IMPL(deconv2d_bp, ENGINE_CPU) {
  auto input =
      INPUT_VARIABLE(0);  // [bS, iH, iW, iC] (NHWC) or [bS, iC, iH, iW] (NCDHW)
  auto weights = INPUT_VARIABLE(
      1);  // [kH, kW, oC, iC], [iC, oC, kH, kW], [iC, kH, kW, oC]
  ...
  auto gradI = OUTPUT_VARIABLE(
      0);  // [bS, iH, iW, iC] (NHWC) or [bS, iC, iH, iW] (NCDHW), gradI
  auto gradW = OUTPUT_VARIABLE(
      1);  // [kH, kW, oC, iC], [iC, oC, kH, kW], [iC, kH, kW, oC]
  auto gradB = block.width() > 3 ? OUTPUT_VARIABLE(2) : nullptr;  // [oC]

  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "CUSTOM DECONV2D_MKLDNN_BP OP: rank of input array must be "
               "equal to 4, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(weights->rankOf() == 4, 0,
               "CUSTOM DECONV2D_MKLDNN_BP OP: rank of weights array must be "
               "equal to 4 , but got %i instead !",
               weights->rankOf());
  REQUIRE_TRUE(gradO->rankOf() == 4, 0,
               "CUSTOM DECONV2D_MKLDNN_BP OP: rank of output gradients (next "
  ...
  ConvolutionUtils::calcOutSizeDeconv2D(trueoH, trueoW, kH, kW, sH, sW, pH, pW,
  ...
  REQUIRE_TRUE(gradO->isSameShape(expectedGradOShape), 0,
               "CUSTOM DECONV2D_MKLDNN_BP OP: wrong shape of output gradients "
  ...
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DECONV2D_MKLDNN_BP OP: wrong shape of weights array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(expectedWeightsShape).c_str(),
               ShapeUtils::shapeAsString(weights).c_str());
  if (bias)
    REQUIRE_TRUE(
        ...
        "CUSTOM DECONV2D_MKLDNN_BP OP: wrong shape of array with biases, "
        "expected rank, length: <=2, %i, but got %i, %i instead !",
        oC, bias->rankOf(), bias->lengthOf());
  ...
  deconv2dBpMKLDNN(input, weights, gradO, gradI, gradW, gradB, kH, kW, sH, sW,
                   pH, pW, dH, dW, paddingMode, isNCHW, wFormat);
