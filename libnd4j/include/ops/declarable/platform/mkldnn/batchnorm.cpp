...
//////////////////////////////////////////////////////////////////////////
static void batchnormMKLDNN(const NDArray* x, const NDArray* mean,
                            const NDArray* variance, const NDArray* weights,
                            NDArray* z, const float epsilon,
                            const bool isNCHW) {
  // unfortunately mkl dnn doesn't support any format
  // (dnnl::memory::format_tag::any) for x

  // x -> 2D:nc, 4D:nchw/nhwc, 5D:ncdhw/ndhwc
  // mean -> 1D [c]
  // variance -> 1D [c]
  // weights 2D [2, c], weights({0,1, 0,0}) contains gamma and weights({1,2,
  // 0,0}) contains beta z(output) - same shape as x
  ...
  auto flags =
      dnnl::normalization_flags::use_global_stats;  // don't calculate the mean
  ...
  if (xRank == 2) {
    ...
    iW = x->sizeAt(indHW + 1);
    dims = {bS, iC, iH, iW};
    format = isNCHW ? dnnl::memory::format_tag::nchw
    ...
    iW = x->sizeAt(indHW + 2);
    dims = {bS, iC, iD, iH, iW};
    format = isNCHW ? dnnl::memory::format_tag::ncdhw
  ...
  dnnl::memory::desc z_mkl_md =
      dnnl::memory::desc(dims, type, dnnl::memory::format_tag::any);
  ...
  dnnl::batch_normalization_forward::desc op_ff_desc(
      dnnl::prop_kind::forward_inference, x_mkl_md, epsilon, flags);
  dnnl::batch_normalization_forward::primitive_desc op_ff_prim_desc(op_ff_desc,
                                                                    engine);
  ...
  auto z_mkl_mem =
      zReorder ? dnnl::memory(op_ff_prim_desc.dst_desc(), engine) : z_user_mem;
  if (zReorder)
    dnnl::reorder(z_user_mem, z_mkl_mem).execute(stream, z_user_mem, z_mkl_mem);
  args[DNNL_ARG_DST] = z_mkl_mem;

  // mean
  auto mean_mkl_mem = dnnl::memory(op_ff_prim_desc.mean_desc(), engine,
                                   const_cast<void*>(mean->buffer()));
  args[DNNL_ARG_MEAN] = mean_mkl_mem;

  // variance
  auto var_mkl_mem = dnnl::memory(op_ff_prim_desc.variance_desc(), engine,
                                  const_cast<void*>(variance->buffer()));
  args[DNNL_ARG_VARIANCE] = var_mkl_mem;

  // gamma and beta (and their gradients) if they are present
  if (weights != nullptr) {
    auto w_mkl_mem = dnnl::memory(op_ff_prim_desc.weights_desc(), engine,
                                  const_cast<void*>(weights->buffer()));
...
//////////////////////////////////////////////////////////////////////////
static void batchnormBackPropMKLDNN(const NDArray* x, const NDArray* mean,
                                    const NDArray* variance,
                                    const NDArray& dLdO, const NDArray* weights,
                                    NDArray* dLdI, NDArray* dLdW,
  ...
  // weights 2D [2, c], weights({0,1, 0,0}) contains gamma and weights({1,2,
  // 0,0}) contains beta dLdI - same shape as x dLdW - same shape as weights,
  // dLdW({0,1, 0,0}) contains grad_gamma and dLdW({1,2, 0,0}) contains
  // grad_beta
  ...
  dnnl::batch_normalization_backward::desc op_bp_desc(
      dnnl::prop_kind::backward, dLdO_mkl_md, x_mkl_md, epsilon, flags);
  dnnl::batch_normalization_backward::primitive_desc op_bp_prim_desc(
      op_bp_desc, engine, op_ff_prim_desc);
  ...
  // x
  mkldnnUtils::loadDataToMklStream(x, engine, stream, x_user_md,
                                   op_bp_prim_desc.src_desc(),
                                   args[DNNL_ARG_SRC]);

  // dLdO
  mkldnnUtils::loadDataToMklStream(&dLdO, engine, stream, dLdO_user_md,
                                   op_bp_prim_desc.diff_dst_desc(),
                                   args[DNNL_ARG_DIFF_DST]);

  // mean
  auto mean_mkl_mem = dnnl::memory(op_bp_prim_desc.mean_desc(), engine,
                                   const_cast<void*>(mean->buffer()));
  args[DNNL_ARG_MEAN] = mean_mkl_mem;

  // variance
  auto var_mkl_mem = dnnl::memory(op_bp_prim_desc.variance_desc(), engine,
                                  const_cast<void*>(variance->buffer()));
  args[DNNL_ARG_VARIANCE] = var_mkl_mem;

  // dLdI
  auto dLdI_user_mem = dnnl::memory(dLdI_user_md, engine, dLdI->buffer());
  const bool dLdIReorder =
      op_bp_prim_desc.diff_src_desc() != dLdI_user_mem.get_desc();
  auto dLdI_mkl_mem =
      dLdIReorder ? dnnl::memory(op_bp_prim_desc.diff_src_desc(), engine)
                  : dLdI_user_mem;
  args[DNNL_ARG_DIFF_SRC] = dLdI_mkl_mem;

  // gamma and beta (and their gradients) if they are present
  if (weights != nullptr) {
    auto w_mkl_mem = dnnl::memory(op_bp_prim_desc.weights_desc(), engine,
                                  const_cast<void*>(weights->buffer()));
    args[DNNL_ARG_WEIGHTS] = w_mkl_mem;

    auto dLdW_mkl_mem =
        dnnl::memory(op_bp_prim_desc.weights_desc(), engine, dLdW->buffer());
  ...
  // reorder outputs if necessary
  if (dLdIReorder)
    dnnl::reorder(dLdI_mkl_mem, dLdI_user_mem)
        .execute(stream, dLdI_mkl_mem, dLdI_user_mem);
  ...
  // notations:
  // f = g * (gamma * ((x - m) / (v + eps)^0.5) + beta) -> means dLdO *
  // ff_output g = dLdO stdInv = 1 / (v + eps)^0.5 N - batch size (product of
  // spatial dimensions)

  // formula for full derivative with respect to input (x)
  // dLdI = dfdx + dfdm*dmdx + dfdv*(dvdm*dmdx + dvdx)

  // !!! MKL CALCULATES ONLY FIRST TERM dfdx, SO WE SHOULD CALCULATE TERM
  // (dfdm*dmdx + dfdv*(dvdm*dmdx + dvdx)) BY OURSELF !!!

  // dfdm = -gamma*stdInv*g_sum;
  // dmdx  = 1/N;
  // dvdx  = 2 *  (x - m) / N
  // dvdm  = -2 * [(x - m)]_sum / N
  // dfdv  = -0.5 * [g*(x - m)]_sum * stdInv^3, drop gamma here for calc
  // convenience
  ...
  std::vector<int> axes =
      isNCHW ? std::vector<int>{1} : std::vector<int>{xRank - 1};
  ...
  const_cast<NDArray*>(x)->applyBroadcast(sd::broadcast::Subtract, axes, *mean,
                                          xMinusMean);

  // stdInv
  NDArray stdInv = *variance + epsilon;
  stdInv.applyTransform(transform::Reciprocal,
                        stdInv);  // 1 / (variance + epsilon)
  stdInv.applyTransform(transform::Sqrt,
...
PLATFORM_IMPL(batchnorm, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(numOfAxes == 1, 0,
               "BATCHNORM_MKLDNN op: mkl dnn library supports only one axis "
               "which represents channel dimension, but got %i axes instead!",
               numOfAxes);
  REQUIRE_TRUE(inRank == 2 || inRank == 4 || inRank == 5, 0,
               "BATCHNORM_MKLDNN op: possible values for rank of input array "
               "are 2, 4 or 5, but got %i instead!",
               inRank);
  REQUIRE_TRUE(mean->rankOf() == 1 && mean->sizeAt(0) == input->sizeAt(axes[0]),
               0,
               "BATCHNORM_MKLDNN op: wrong shape of mean array, expected is "
               "[%lld], but got %s instead !",
               input->sizeAt(axes[0]), ShapeUtils::shapeAsString(mean).c_str());
  REQUIRE_TRUE(
      variance->rankOf() == 1 && variance->sizeAt(0) == input->sizeAt(axes[0]),
      0,
      "BATCHNORM_MKLDNN op: wrong shape of variance array, expected is [%lld], "
      "but got %s instead !",
      input->sizeAt(axes[0]), ShapeUtils::shapeAsString(variance).c_str());
  if (gamma != nullptr)
    REQUIRE_TRUE(
        gamma->rankOf() == 1 && gamma->sizeAt(0) == input->sizeAt(axes[0]), 0,
        "BATCHNORM_MKLDNN op: wrong shape of gamma array, expected is [%lld], "
        "but got %s instead !",
        input->sizeAt(axes[0]), ShapeUtils::shapeAsString(gamma).c_str());
  if (beta != nullptr)
    REQUIRE_TRUE(
        beta->rankOf() == 1 && beta->sizeAt(0) == input->sizeAt(axes[0]), 0,
        "BATCHNORM_MKLDNN op: wrong shape of beta array, expected is [%lld], "
        "but got %s instead !",
  ...
  if (applyScale || applyOffset) {
    weights = new NDArray(input->ordering(), {2, input->sizeAt(axes[0])},
                          input->dataType());
...
PLATFORM_CHECK(batchnorm, ENGINE_CPU) {
  ...
  return block.isUseMKLDNN() && axes.size() == 1 &&
         (axes[0] == 1 || axes[0] == inRank - 1) &&
         (inRank == 2 || inRank == 4 || inRank == 5) &&
         (inputType == DataType::FLOAT32 && meanType == DataType::FLOAT32 &&
...
//     NDArray weights = NDArrayFactory::create<float>('c', shape,
//     block.launchContext()); weights({0, 1, 0, 0}).assign(1.0f); weights({1,
//     2, 0, 0}).assign(0.0f);

//     mkldnn_memory_desc_t empty;
//     dnnl::memory::desc batchnorm_src_md(empty), batchnorm_dst_md(empty),
//     user_src_md(empty), user_dst_md(empty);
...
//     dnnl::batch_normalization_forward::primitive_desc(batchnorm_desc,
//     engine); auto user_src_memory = dnnl::memory(user_src_md, engine,
//     input->buffer()); auto user_dst_memory = dnnl::memory(user_dst_md,
//     engine, output->buffer()); auto batchnorm_mean_memory =
//     dnnl::memory(batchnorm_prim_desc.mean_desc(), engine,
//                                                 mean->buffer());
//     auto batchnorm_variance_memory =
//     dnnl::memory(batchnorm_prim_desc.variance_desc(), engine,
//                                                     variance->buffer());
...
//         dnnl::reorder(user_src_memory, batchnorm_src_memory).execute(stream,
//         user_src_memory,
//                                                                batchnorm_src_memory);
//     }
//     auto batchnorm_dst_memory = user_dst_memory;
//     if (batchnorm_prim_desc.dst_desc() != user_dst_memory.get_desc()) {
//         batchnorm_dst_memory = dnnl::memory(batchnorm_prim_desc.dst_desc(),
//         engine);
//     }
//     if (applyScale || applyOffset) {
//         if (gamma != nullptr) {
...
//         auto batchnorm_weights_memory =
//         dnnl::memory(batchnorm_prim_desc.weights_desc(), engine,
//         weights.buffer());
//         dnnl::batch_normalization_forward(batchnorm_prim_desc).execute(stream,
//                                                                  {{MKLDNN_ARG_SRC,
//                                                                  batchnorm_src_memory},
//                                                                   {MKLDNN_ARG_MEAN,
//                                                                   batchnorm_mean_memory},
//                                                                   {MKLDNN_ARG_VARIANCE,
//                                                                   batchnorm_variance_memory},
//                                                                   {MKLDNN_ARG_WEIGHTS,
//                                                                   batchnorm_weights_memory},
//                                                                   {MKLDNN_ARG_DST,
...
//     }
//     if (batchnorm_prim_desc.dst_desc() != user_dst_memory.get_desc()) {
//         dnnl::reorder(batchnorm_dst_memory, user_dst_memory).execute(stream,
//         batchnorm_dst_memory,
//                                                                user_dst_memory);
//     }
//     stream.wait();
...
//            sd::MKLDNNStream::isSupported({input, mean, variance, gamma, beta,
//            output}) && axes.size() == 1;
// }
...
PLATFORM_IMPL(batchnorm_bp, ENGINE_CPU) {
  ...
  const int numOfAxes = axes.size();
  REQUIRE_TRUE(numOfAxes == 1, 0,
               "BATCHNORM_BP_MKLDNN op: mkl dnn library supports only one axis "
               "which represents channel dimension, but got %i axes instead!",
               numOfAxes);
  REQUIRE_TRUE(inRank == 2 || inRank == 4 || inRank == 5, 0,
               "BATCHNORM_BP_MKLDNN op: possible values for rank of input "
               "array are 2, 4 or 5, but got %i instead!",
               inRank);
  REQUIRE_TRUE(input->isSameShape(dLdO), 0,
               "BATCHNORM_BP_MKLDNN op: wrong shape of gradients array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(input).c_str(),
               ShapeUtils::shapeAsString(dLdO).c_str());
  REQUIRE_TRUE(mean->rankOf() == 1 && mean->sizeAt(0) == input->sizeAt(axes[0]),
               0,
               "BATCHNORM_BP_MKLDNN op: wrong shape of mean array, expected is "
               "[%lld], but got %s instead !",
               input->sizeAt(axes[0]), ShapeUtils::shapeAsString(mean).c_str());
  REQUIRE_TRUE(
      variance->rankOf() == 1 && variance->sizeAt(0) == input->sizeAt(axes[0]),
      0,
      "BATCHNORM_BP_MKLDNN op: wrong shape of variance array, expected is "
      "[%lld], but got %s instead !",
      input->sizeAt(axes[0]), ShapeUtils::shapeAsString(variance).c_str());
  if (gamma != nullptr)
    REQUIRE_TRUE(
        gamma->rankOf() == 1 && gamma->sizeAt(0) == input->sizeAt(axes[0]), 0,
        "BATCHNORM_BP_MKLDNN op: wrong shape of gamma array, expected is "
        "[%lld], but got %s instead !",
        input->sizeAt(axes[0]), ShapeUtils::shapeAsString(gamma).c_str());
  if (beta != nullptr)
    REQUIRE_TRUE(
        beta->rankOf() == 1 && beta->sizeAt(0) == input->sizeAt(axes[0]), 0,
        "BATCHNORM_BP_MKLDNN op: wrong shape of beta array, expected is "
        "[%lld], but got %s instead !",
        input->sizeAt(axes[0]), ShapeUtils::shapeAsString(beta).c_str());
  ...
  for (int i = 1; i < block.width() - 1; ++i)
    REQUIRE_TRUE(INPUT_VARIABLE(0)->dataType() == INPUT_VARIABLE(i)->dataType(),
                 0,
                 "BATCHNORM_BP_MKLDNN op: types of all input arrays should be "
                 "the same !");
  ...
  if (applyScale || applyOffset) {
    weights = new NDArray(input->ordering(), {2, input->sizeAt(axes[0])},
                          input->dataType());
    dLdW = new NDArray(input->ordering(), {2, input->sizeAt(axes[0])},
                       input->dataType());
    if (applyScale)
      (*weights)({0, 1, 0, 0}).assign(gamma);
    else
  ...
  if (shape::strideDescendingCAscendingF(dLdO->shapeInfo()))
    batchnormBackPropMKLDNN(input, mean, variance, *dLdO, weights, dLdI, dLdW,
                            epsilon, isNCHW);
  else
    batchnormBackPropMKLDNN(input, mean, variance, dLdO->dup(), weights, dLdI,
                            dLdW, epsilon, isNCHW);
