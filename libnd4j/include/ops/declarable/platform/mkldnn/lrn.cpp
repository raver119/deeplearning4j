...
PLATFORM_IMPL(lrn, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "lrn: Input rank of 4 expected, but got %i instead",
               input->rankOf());
  ...
  dnnl::memory::desc lrn_src_md(empty), lrn_dst_md(empty), user_src_md(empty),
      user_dst_md(empty);

  mkldnnUtils::getMKLDNNMemoryDescLrn(
      input, nullptr, output, &lrn_src_md, nullptr, &lrn_dst_md, &user_src_md,
      nullptr, &user_dst_md, input->rankOf() - 1);

  auto lrn_desc = lrn_forward::desc(
      prop_kind::forward_inference, algorithm::lrn_across_channels, lrn_src_md,
  ...
  if (lrn_prim_desc.src_desc() != user_src_memory.get_desc()) {
    ...
    reorder(user_src_memory, lrn_src_memory)
        .execute(stream, user_src_memory, lrn_src_memory);
  }
  ...
  lrn_forward(lrn_prim_desc)
      .execute(stream, {{DNNL_ARG_SRC, lrn_src_memory},
                        {DNNL_ARG_DST, lrn_dst_memory}});

  if (lrn_prim_desc.dst_desc() != user_dst_memory.get_desc()) {
    reorder(lrn_dst_memory, user_dst_memory)
        .execute(stream, lrn_dst_memory, user_dst_memory);
  }
