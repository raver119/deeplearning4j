...
//////////////////////////////////////////////////////////////////////////
static void deconv3dMKLDNN(const NDArray* input, const NDArray* weights,
                           const NDArray* bias, NDArray* output, const int kD,
                           const int kH, const int kW, const int sD,
                           const int sH, const int sW, const int pD,
                           const int pH, const int pW, const int dD,
                           const int dH, const int dW, const bool isNCDHW,
                           const int wFormat) {
  ...
  dnnl::memory::dims padding_r = {(iD - 1) * sD - oD + kD - pD,
                                  (iH - 1) * sH - oH + kH - pH,
                                  (iW - 1) * sW - oW + kW - pW};
  ...
  if (0 == wFormat) {
    ...
    i2 = 0;
    i3 = 1;
    i4 = 2;  // [kD, kH, kW, oC, iC] -> [oC, iC, kD, kH, kW]
  } else if (1 == wFormat) {
    i0 = 1;
    i1 = 0;
    i2 = 2;
    i3 = 3;
    i4 = 4;  // [iC, oC, kD, kH, kW] -> [oC, iC, kD, kH, kW]
  } else {
    i0 = 4;
    i1 = 0;
    i2 = 1;
    i3 = 2;
    i4 = 3;  // [iC, kD, kH, kW, oC] -> [oC, iC, kD, kH, kW]
  }
  ...
  dnnl::memory::format_tag xFormatMkl = isNCDHW
  ...
  dnnl::deconvolution_forward::desc op_desc(
...
//////////////////////////////////////////////////////////////////////////
static void deconv3dBackPropMKLDNN(const NDArray* input, const NDArray* weights,
                                   const NDArray* gradO, NDArray* gradI,
                                   NDArray* gradW, NDArray* gradB, const int kD,
                                   const int kH, const int kW, const int sD,
                                   const int sH, const int sW, const int pD,
                                   const int pH, const int pW, const int dD,
  ...
  dnnl::memory::desc gradO_user_md =
      dnnl::memory::desc(zDims, gradOType, xFormatMkl);
  ...
  dnnl::memory::desc gradI_user_md =
      dnnl::memory::desc(xDims, gradIType, xFormatMkl);
  ...
  dnnl::deconvolution_forward::desc op_ff_desc(
      dnnl::prop_kind::forward_inference, dnnl::algorithm::deconvolution_direct,
      x_mkl_md, w_mkl_md, gradB_mkl_md, gradO_mkl_md, strides, dilation,
      padding, padding_r);
  dnnl::deconvolution_forward::primitive_desc op_ff_prim_desc(op_ff_desc,
                                                              engine);

  // backward data primitive description
  dnnl::deconvolution_backward_data::desc op_data_bp_desc(
      dnnl::algorithm::deconvolution_direct, gradI_mkl_md, w_mkl_md,
      gradO_mkl_md, strides, dilation, padding, padding_r);
  dnnl::deconvolution_backward_data::primitive_desc op_data_bp_prim_desc(
      op_data_bp_desc, engine, op_ff_prim_desc);

  // backward weights primitive description
  dnnl::deconvolution_backward_weights::desc op_weights_bp_desc(
      dnnl::algorithm::deconvolution_direct, x_mkl_md, gradW_mkl_md,
      gradB_mkl_md, gradO_mkl_md, strides, dilation, padding, padding_r);
  dnnl::deconvolution_backward_weights::primitive_desc op_weights_bp_prim_desc(
  ...
  dnnl::deconvolution_backward_weights(op_weights_bp_prim_desc)
      .execute(stream, args);
...
PLATFORM_IMPL(deconv3d, ENGINE_CPU) {
  ...
  auto weights = INPUT_VARIABLE(
      1);  // [kD, kH, kW, oC, iC], [iC, oC, kD, kH, kW], [iC, kD, kH, kW, oC]
  ...
  REQUIRE_TRUE(input->rankOf() == 5, 0,
               "CUSTOM DECONV3D_MKLDNN OP: rank of input array must be equal "
               "to 5, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(weights->rankOf() == 5, 0,
               "CUSTOM DECONV3D_MKLDNN OP: rank of weights array must be equal "
               "to 5, but got %i instead !",
               weights->rankOf());
  ...
  int wFormat = block.numI() > 14
                    ? INT_ARG(14)
                    : 0;  // 0 - [kD, kH, kW, oC, iC], 1 - [iC, oC, kD, kH, kW],
                          // 2 - [iC, kD, kH, kW, oC]
  ...
  std::vector<Nd4jLong> expectedWeightsShape =
      ConvolutionUtils::expectWeightsShape(wFormat, kD, kH, kW, oC, iC);
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DECONV3D_MKLDNN OP: wrong shape of weights array, "
  ...
  if (bias)
    REQUIRE_TRUE(bias->rankOf() <= 2 && oC == bias->lengthOf(), 0,
                 "CUSTOM DECONV3D_MKLDNN OP: wrong shape of array with biases, "
                 "expected rank, length: <=2, %i, but got %i, %i instead !",
                 oC, bias->rankOf(), bias->lengthOf());

  if (isSameMode) {  // SAME
    // Note: we're intentionally swapping iH and oH, to calculated the padding
    // for a"normal" conv (not deconv) forward pass
    ConvolutionUtils::calcPadding3D(pD, pH, pW, iD, iH, iW, oD, oH, oW, kD, kH,
                                    kW, sD, sH, sW, dD, dH, dW);
  }

  deconv3dMKLDNN(input, weights, bias, output, kD, kH, kW, sD, sH, sW, pD, pH,
                 pW, dD, dH, dW, isNCDHW, wFormat);
...
PLATFORM_IMPL(deconv3d_bp, ENGINE_CPU) {
  ...
  auto gradI = OUTPUT_VARIABLE(0);  // [bS, iD, iH, iW, iC] (NDHWC) or [bS, iC,
                                    // iD, iH, iW] (NCDHW), gradI
  auto gradW = OUTPUT_VARIABLE(
      1);  // [kD, kH, kW, oC, iC], [iC, oC, kD, kH, kW], [iC, kD, kH, kW, oC]
  auto gradB = block.width() > 3 ? OUTPUT_VARIABLE(2) : nullptr;  // [oC]

  REQUIRE_TRUE(input->rankOf() == 5, 0,
               "CUSTOM DECONV3D_MKLDNN_BP OP: rank of input array must be "
               "equal to 5, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(weights->rankOf() == 5, 0,
               "CUSTOM DECONV3D_MKLDNN_BP OP: rank of weights array must be "
               "equal to 5 , but got %i instead !",
               weights->rankOf());
  REQUIRE_TRUE(gradO->rankOf() == 5, 0,
               "CUSTOM DECONV3D_MKLDNN_BP OP: rank of output gradients (next "
  ...
  ConvolutionUtils::calcOutSizeDeconv3D(trueoD, trueoH, trueoW, kD, kH, kW, sD,
  ...
  REQUIRE_TRUE(gradO->isSameShape(expectedGradOShape), 0,
               "CUSTOM DECONV3D_MKLDNN_BP OP: wrong shape of output gradients "
  ...
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DECONV3D_MKLDNN_BP OP: wrong shape of weights array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(expectedWeightsShape).c_str(),
               ShapeUtils::shapeAsString(weights).c_str());
  if (bias)
    REQUIRE_TRUE(
        ...
        "CUSTOM DECONV3D_MKLDNN_BP OP: wrong shape of array with biases, "
        "expected rank, length: <=2, %i, but got %i, %i instead !",
        oC, bias->rankOf(), bias->lengthOf());

  if (isSameMode)  // Note: we're intentionally swapping iH and oH, to
                   // calculated the padding for a"normal" conv (not deconv)
                   // forward pass
    ConvolutionUtils::calcPadding3D(pD, pH, pW, iD, iH, iW, oD, oH, oW, kD, kH,
                                    kW, sD, sH, sW, dD, dH, dW);

  deconv3dBackPropMKLDNN(input, weights, gradO, gradI, gradW, gradB, kD, kH, kW,
                         sD, sH, sW, pD, pH, pW, dD, dH, dW, isNCDHW, wFormat);
...
PLATFORM_CHECK(deconv3d_bp, ENGINE_CPU) {
  auto input = INPUT_VARIABLE(
      0);  // [bS, iD, iH, iW, iC] (NHWC) or [bS, iD, iC, iH, iW] (NCDHW)
  auto weights = INPUT_VARIABLE(
      1);  // [kD, kH, kW, oC, iC], [iC, oC, kD, kH, kW], [iC, kD, kH, kW, oC]
  auto bias = block.width() > 3 ? INPUT_VARIABLE(2) : nullptr;  // [oC]
  auto gradO =
      block.width() > 3
          ? INPUT_VARIABLE(3)
          : INPUT_VARIABLE(2);  // [bS, oD, oH, oW, oC] (NHWC) or [bS, oC, oD,
                                // oH, oW] (NCDHW), epsilon_next

  auto gradI = OUTPUT_VARIABLE(
      0);  // [bS, iD, iH, iW, iC] (NHWC) or [bS, iC, iD, iH, iW] (NCDHW), gradI
  auto gradW = OUTPUT_VARIABLE(
      1);  // [kD, kH, kW, oC, iC], [iC, oC, kD, kH, kW], [iC, kD, kH, kW, oC]
  auto gradB = block.width() > 3 ? OUTPUT_VARIABLE(2) : nullptr;  // [oC]
