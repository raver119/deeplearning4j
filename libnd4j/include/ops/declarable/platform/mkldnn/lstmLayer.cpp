...
static void lstmLayerMKLDNN(const NDArray* x, const NDArray* Wx,
  ...
  // directionMode:  0 = forward, 1 = backward, 2 = bidirectional sum, 3 =
  ...
  dnnl::memory::desc x_user_md, wx_user_md, wr_user_md, b_user_md, hI_user_md,
      cI_user_md, h_user_md, hL_user_md, cL_user_md, x_lstm_md, wx_lstm_md,
      wr_lstm_md, b_lstm_md, hI_lstm_md, cI_lstm_md, h_lstm_md, hL_lstm_md,
      cL_lstm_md;
  ...
  const bool hReorder =
      lstm_prim_desc.dst_layer_desc() != h_user_mem.get_desc();
  auto h_lstm_mem = hReorder
                        ? dnnl::memory(lstm_prim_desc.dst_layer_desc(), engine)
                        : h_user_mem;
  ...
  if (hL) {
    ...
    hL_lstm_mem = hLReorder
                      ? dnnl::memory(lstm_prim_desc.dst_iter_desc(), engine)
                      : hL_user_mem;
  ...
  if (cL) {
    ...
    cL_lstm_mem = cLReorder
                      ? dnnl::memory(lstm_prim_desc.dst_iter_c_desc(), engine)
                      : cL_user_mem;
...
PLATFORM_IMPL(lstmLayer, ENGINE_CPU) {
  ...
  const Nd4jLong bS =
      dataFormat == 1 || dataFormat == 2 ? x->sizeAt(0) : x->sizeAt(-2);
...
PLATFORM_CHECK(lstmLayer, ENGINE_CPU) {
  ...
  DataType bType = b != nullptr
                       ? b->dataType()
                       : (xType == DataType::HALF ? xType : DataType::FLOAT32);
  ...
  auto featuresSupported =
      (cellClip == 0)    // Cell clipping not supported
  ...
  return block.isUseMKLDNN() && featuresSupported &&
         ((xType == DataType::FLOAT32 && WxType == DataType::FLOAT32 &&
           WrType == DataType::FLOAT32 && bType == DataType::FLOAT32 &&
           hIType == DataType::FLOAT32 && cIType == DataType::FLOAT32 &&
           hType == DataType::FLOAT32 && hLType == DataType::FLOAT32 &&
           cLType == DataType::FLOAT32) ||
          (xType == DataType::HALF && WxType == DataType::HALF &&
           WrType == DataType::HALF && bType == DataType::HALF &&
           hIType == DataType::HALF && cIType == DataType::HALF &&
           hType == DataType::HALF && hLType == DataType::HALF &&
           cLType == DataType::HALF) ||
          (xType == DataType::UINT8 && WxType == DataType::INT8 &&
           WrType == DataType::INT8 && bType == DataType::FLOAT32 &&
           hIType == DataType::UINT8 && cIType == DataType::UINT8 &&
           (hType == DataType::FLOAT32 && hLType == DataType::FLOAT32 &&
                cLType == DataType::FLOAT32 ||
            hType == DataType::UINT8 && hLType == DataType::UINT8 &&
                cLType == DataType::UINT8)));
}
