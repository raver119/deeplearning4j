...
PLATFORM_IMPL(avgpool3dnew, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(input->rankOf() == 5, 0,
               "AVGPOOL3DNEW MKLDNN OP: rank of input array must be equal to "
               "5, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(dD != 0 && dH != 0 && dW != 0, 0,
               "AVGPOOL3DNEW MKLDNN OP: dilation must not be zero, but got "
               "instead {%i, %i, %i}",
               dD, dH, dW);
...
PLATFORM_IMPL(avgpool3dnew_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(input->rankOf() == 5, 0,
               "AVGPOOL3DNEW_BP MKLDNN op: input should have rank of 5, but "
               "got %i instead",
               input->rankOf());
  REQUIRE_TRUE(dD != 0 && dH != 0 && dW != 0, 0,
               "AVGPOOL3DNEW_BP MKLDNN op: dilation must not be zero, but got "
               "instead {%i, %i, %i}",
               dD, dH, dW);
  ...
  REQUIRE_TRUE(gradO->isSameShape(expectedGradOShape), 0,
               "AVGPOOL3DNEW_BP MKLDNN op: wrong shape of output's gradients "
