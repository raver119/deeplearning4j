...
//////////////////////////////////////////////////////////////////////////
static void depthwiseConv2dMKLDNN(const NDArray* input, const NDArray* weights,
  ...
  // input [bS, iC, iH, iW] nchw or [bS, iH, iW, iC] nhwc, since mkl doesn't
  // support nhwc format we'll permute when nhwc is given weights {iC, mC, 1,
  // kH, kW} bias [oC], may be nullptr output [bS, oC, oH, oW] nchw or [bS, oH,
  // oW, oC] nhwc oC = iC*mC
  ...
  dnnl::memory::format_tag xzFormatMkl =
  ...
  dnnl::memory::desc w_mkl_md =
      dnnl::memory::desc(wDims, wType, dnnl::memory::format_tag::any);
  ...
  w_user_md.data.format_desc.blocking.strides[0] =
      weights->strideAt(i0);  // permute
...
static void depthwiseConv2dBpMKLDNN(
  ...
  // input, gradI [bS, iC, iH, iW] nchw or [bS, iH, iW, iC] nhwc, since mkl
  // doesn't support nhwc format we'll permute when nhwc is given weights/gradW
  // {iC, mC, 1, kH, kW} gradB [oC], may be nullptr gradO [bS, oC, oH, oW] nchw
  // or [bS, oH, oW, oC] nhwc oC = iC*mC
  ...
  // input type
  dnnl::memory::data_type xType = input->dataType() == DataType::FLOAT32
  ...
  dnnl::memory::data_type wType = weights->dataType() == DataType::FLOAT32
                                      ...
                                      : dnnl::memory::data_type::bf16;
  // gradO type
  dnnl::memory::data_type gradOType = gradO->dataType() == DataType::FLOAT32
                                          ? dnnl::memory::data_type::f32
                                          : dnnl::memory::data_type::bf16;
  // gradI type
  dnnl::memory::data_type gradIType = gradI->dataType() == DataType::FLOAT32
                                          ? dnnl::memory::data_type::f32
                                          : dnnl::memory::data_type::bf16;
  // gradW type
  dnnl::memory::data_type gradWType = gradW->dataType() == DataType::FLOAT32
                                          ? dnnl::memory::data_type::f32
                                          : dnnl::memory::data_type::bf16;
  // gradB type
  dnnl::memory::data_type gradBType =
      gradB != nullptr ? (gradB->dataType() == DataType::FLOAT32
                              ? dnnl::memory::data_type::f32
                              : dnnl::memory::data_type::bf16)
                       : dnnl::memory::data_type::f32;
  ...
  dnnl::memory::desc gradW_mkl_md =
      dnnl::memory::desc(wDims, gradWType, dnnl::memory::format_tag::any);
  dnnl::memory::desc gradW_user_md =
      dnnl::memory::desc(wDims, gradWType, wFormatMkl);
  gradW_user_md.data.format_kind = dnnl_blocked;  // overrides format
  gradW_user_md.data.format_desc.blocking.strides[0] =
      gradW->strideAt(i0);  // permute
  ...
  if (gradB != nullptr)
    gradB_mkl_md =
        dnnl::memory::desc({oC}, gradBType, dnnl::memory::format_tag::x);
...
PLATFORM_IMPL(depthwise_conv2d, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DEPTHWISECONV2D MKL OP: wrong shape of weights array, "
  ...
  REQUIRE_TRUE(output->sizeAt(indIOioC) == iC * mC, 0,
               "CUSTOM DEPTHWISECONV2D MKL OP: the output_channels must be "
               "equal to input_channels * channels_multiplier = %i !",
               iC * mC);
  if (bias)
    REQUIRE_TRUE(
        bias->rankOf() <= 2 && oC == bias->lengthOf(), 0,
        "CUSTOM DEPTHWISECONV2D MKL OP: wrong shape of array with biases, "
        "expected rank, length: <=2, %i, but got %i, %i instead !",
        oC, bias->rankOf(), bias->lengthOf());

  depthwiseConv2dMKLDNN(input, weights, bias, output, kH, kW, sH, sW, pH, pW,
...
PLATFORM_IMPL(depthwise_conv2d_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "CUSTOM DEPTHWISECONV2D_BP MKL OP: rank of input array must be "
               "equal to 4, but got %i instead !",
               input->rankOf());
  REQUIRE_TRUE(weights->rankOf() == 4, 0,
               "CUSTOM DEPTHWISECONV2D_BP MKL OP: rank of weights array must "
               "be equal to 4, but got %i instead !",
               weights->rankOf());
  REQUIRE_TRUE(gradO->rankOf() == 4, 0,
               "CUSTOM DEPTHWISECONV2D_BP MKL OP: rank of output gradients "
               "(next epsilon) array must be equal to 4, but got %i instead !",
               gradO->rankOf());
  ...
  REQUIRE_TRUE(
      ...
      "CUSTOM DEPTHWISECONV2D_BP MKL OP: wrong shape of output gradients (next "
  ...
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DEPTHWISECONV2D_BP MKL OP: wrong shape of weights "
               "array, expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(expectedWeightsShape).c_str(),
               ShapeUtils::shapeAsString(weights).c_str());
  if (bias)
    REQUIRE_TRUE(
        bias->rankOf() <= 2 && oC == bias->lengthOf(), 0,
        "CUSTOM DEPTHWISECONV2D_BP MKL OP: wrong shape of array with biases, "
        "expected rank, length: <=2, %i, but got %i, %i instead !",
        oC, bias->rankOf(), bias->lengthOf());
...
PLATFORM_CHECK(depthwise_conv2d_bp, ENGINE_CPU) {
  ...
  const DataType gradBType =
      gradB != nullptr ? gradB->dataType() : DataType::FLOAT32;

  const int mC = weights->sizeAt(3);

  return block.isUseMKLDNN() && mC == 1 &&
         ((xType == DataType::FLOAT32 || xType == DataType::BFLOAT16) &&
