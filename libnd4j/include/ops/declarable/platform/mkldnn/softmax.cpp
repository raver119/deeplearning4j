...
dnnl::memory::format_tag xFormat = mkldnnUtils::getFormat(*x);
  ...
  dnnl::softmax_forward::desc op_desc(dnnl::prop_kind::forward_inference,
                                      x_mkl_md, axis);
...
PLATFORM_IMPL(softmax, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(
      dim < rank && dim >= 0, 0,
      "SOFTMAX_MKLDNN OP: the value of input integer parameter (dimension) "
      "must be less than input array rank %i, but got dimension = %i instead !",
      rank, dim);

  REQUIRE_TRUE(rank <= 6, 0,
               "SOFTMAX_MKLDNN OP: the rank of input must be less or qual 6, "
               "but got rank = %i instead !",
               rank);
...
PLATFORM_CHECK(softmax, ENGINE_CPU) {
  ...
  f32 	    f32
  */
  return !x->isEmpty() && block.isUseMKLDNN() && bSupportedRanks &&
         (xType == DataType::FLOAT32 && zType == DataType::FLOAT32);
}

//////////////////////////////////////////////////////////////////////
static void softmaxBpMKLDNN(const NDArray* x, const NDArray* dLdz,
                            NDArray* dLdx, const int axis) {
  ...
  // x
   x_mkl_md =
       x_user_md =
      dnnl::memory::desc(x->getShapeAsFlatVector(), dnnl::memory::data_type::f32, mkldnnUtils::getFormat(*x));
  mkldnnUtils::setBlockStrides(*x, x_user_md);

  // dLdx
   dLdx_mkl_md =
       dLdx_user_md =
      dnnl::memory::desc(dLdx->getShapeAsFlatVector(), dnnl::memory::data_type::f32, mkldnnUtils::getFormat(*dLdx));
  ...
  // dLdz
   dLdz_mkl_md =
       dLdz_user_md =
      dnnl::memory::desc(dLdz->getShapeAsFlatVector(), dnnl::memory::data_type::f32, mkldnnUtils::getFormat(*dLdz));
  ...
  // operation primitive description
  // forward description
  dnnl::softmax_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference,
                                         x_mkl_md, axis);
  ...
  dnnl::softmax_backward::primitive_desc op_bp_prim_desc(op_bp_desc, engine,
                                                         op_ff_prim_desc);
  ...
  // dLdx
  auto dLdx_user_mem = mkldnnUtils::loadDataToMklStream(*dLdx, engine, stream, dLdx_user_md, op_ff_prim_desc.src_desc(),
  argsff[DNNL_ARG_DST] );
  ...
  if (op_ff_prim_desc.dst_desc() != dLdx_user_mem.get_desc())
    dnnl::reorder(argsff[DNNL_ARG_DST], dLdx_user_mem)
        .execute(stream, argsff[DNNL_ARG_DST], dLdx_user_mem);
...
PLATFORM_IMPL(softmax_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(
      dim < rank && dim >= 0, 0,
      "SOFTMAX_MKLDNN_BP OP: the value of input integer parameter (dimension) "
      "must be less than input array rank %i, but got dimension = %i instead !",
      rank, dim);

  REQUIRE_TRUE(
      rank <= 6 && dLdzRank <= 6, 0,
      "SOFTMAX_MKLDNN_BP OP: the rank of input and dLdz must be less or qual "
      "6, but got input rank = %i and dLdz rank rank = %i instead !",
      rank, dLdzRank);
...
PLATFORM_CHECK(softmax_bp, ENGINE_CPU) {
  ...
  bool bSupportedRanks =
      xRank < 7 && dLdzRank == xRank && (!x->isEmpty() && !dLdz->isEmpty());
  ...
  return block.isUseMKLDNN() && bSupportedRanks &&
         (xType == DataType::FLOAT32 && dLdzType == DataType::FLOAT32 &&
          dLdxType == DataType::FLOAT32);
}
