...
static void softmaxBpMKLDNN(const NDArray* x, const NDArray* dLdz,
  ...
  dnnl::memory::desc dLdx_mkl_md =
      dnnl::memory::desc(dLdxShape, dnnl::memory::data_type::f32, format);
  ...
  dnnl::memory::desc dLdz_mkl_md =
      dnnl::memory::desc(dLdzShape, dnnl::memory::data_type::f32, format);
  ...
  const bool dLdxReorder =
      op_ff_prim_desc.dst_desc() != dLdx_user_mem.get_desc();
  auto dLdx_mkl_mem = dLdxReorder
                          ? dnnl::memory(op_ff_prim_desc.dst_desc(), engine)
                          : dLdx_user_mem;
