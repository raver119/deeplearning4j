...
dnnl::memory::format_tag get_format_tag(const sd::NDArray& array) {
  switch (array.rankOf()) {
    ...
    case 2:
      return array.ordering() == 'c' ? dnnl::memory::format_tag::ab
                                     : dnnl::memory::format_tag::ba;
    case 3:
      return array.ordering() == 'c' ? dnnl::memory::format_tag::abc
...
//////////////////////////////////////////////////////////////////////////
static void matmulMKLDNN(const NDArray* x, const NDArray* y, NDArray* z,
  ...
  // possible input cases not supported by mkl, however we'll perform
  // permut/reshape procedures in order to fit requirements [4]          x [4]
  // = [1]          --> [1,4]     x [4,1]     = [1,1] [4]          x [4,5] = [5]
  // --> [1,4]     x [4,5]     = [1,5] [4,5]        x [5]          = [4] -->
  // [4,5]     x [5,1]     = [4,1] [2,3, 4,5]   x [2,3, 5,4]   = [2,3, 4,4] -->
  // [6, 4,5]  x [6, 5,4]  = [6, 4,4] [2,2,3, 4,5] x [2,2,3, 5,4] = [2,2,3, 4,4]
  // --> [12, 4,5] x [12, 5,4] = [12, 4,4]
  ...
  const NDArray* xT =
      (transX && xRank > 1) ? new NDArray(x->permute(permut)) : x;
  const NDArray* yT =
      (transY && yRank > 1) ? new NDArray(y->permute(permut)) : y;

  const NDArray* xTR =
      xRank <= 3 ? xT
                 : new NDArray(xT->reshape(
                       xT->ordering(),
                       {xT->lengthOf() / (xT->sizeAt(-2) * xT->sizeAt(-1)),
                        xT->sizeAt(-2), xT->sizeAt(-1)}));
  const NDArray* yTR =
      xRank <= 3 ? yT
                 : new NDArray(yT->reshape(
                       yT->ordering(),
                       {yT->lengthOf() / (yT->sizeAt(-2) * yT->sizeAt(-1)),
                        yT->sizeAt(-2), yT->sizeAt(-1)}));
  NDArray* zR =
      xRank <= 3
          ? z
          : new NDArray(z->reshape(
                z->ordering(), {z->lengthOf() / (z->sizeAt(-2) * z->sizeAt(-1)),
  ...
  const int64_t bS =
      (xRank > 2) ? xTR->sizeAt(0) : 1;  // [bS, M,K] x [bS, K,N] = [bS, M,N]

  dnnl::memory::dims xShape =
      xRank < 3 ? dnnl::memory::dims({M, K}) : dnnl::memory::dims({bS, M, K});
  dnnl::memory::dims yShape =
      xRank < 3 ? dnnl::memory::dims({K, N}) : dnnl::memory::dims({bS, K, N});
  dnnl::memory::dims zShape =
      xRank < 3 ? dnnl::memory::dims({M, N}) : dnnl::memory::dims({bS, M, N});
  ...
  dnnl::memory::desc x_mkl_md =
      dnnl::memory::desc(xShape, xType, get_format_tag(*xTR));
  dnnl::memory::desc x_user_md =
      dnnl::memory::desc(xShape, xType, get_format_tag(*xTR));
  if (xTR->ews() != 1) {
    x_user_md.data.format_kind = dnnl_blocked;  // overrides format
    x_user_md.data.format_desc.blocking.strides[0] =
        xRank == 1 ? 1 : xTR->strideAt(0);
    x_user_md.data.format_desc.blocking.strides[1] =
        xRank == 1 ? xTR->strideAt(0) : xTR->strideAt(1);
    if (xRank > 2)
      x_user_md.data.format_desc.blocking.strides[2] = xTR->strideAt(2);
  }

  // y
  dnnl::memory::desc y_mkl_md =
      dnnl::memory::desc(yShape, yType, get_format_tag(*yTR));
  dnnl::memory::desc y_user_md =
      dnnl::memory::desc(yShape, yType, get_format_tag(*yTR));
  if (yTR->ews() != 1) {
    y_user_md.data.format_kind = dnnl_blocked;  // overrides format
    y_user_md.data.format_desc.blocking.strides[0] =
        yRank == 1 ? 1 : yTR->strideAt(0);
    y_user_md.data.format_desc.blocking.strides[1] =
        yRank == 1 ? yTR->strideAt(0) : yTR->strideAt(1);
    if (yRank > 2)
  ...
  dnnl::memory::desc z_mkl_md =
      dnnl::memory::desc(zShape, zType, get_format_tag(*zR));
  dnnl::memory::desc z_user_md =
      dnnl::memory::desc(zShape, zType, get_format_tag(*zR));
  if (zR->ews() != 1) {
    z_user_md.data.format_kind = dnnl_blocked;  // overrides format
    z_user_md.data.format_desc.blocking.strides[0] =
        zRank == 1 ? 1 : zR->strideAt(0);
    z_user_md.data.format_desc.blocking.strides[1] =
        zRank == 1 ? zR->strideAt(0) : zR->strideAt(1);
    if (zRank > 2)
  ...
  dnnl::primitive_attr attr;  // it is empty since we have usual values for
                              // alpha (=1) and beta (=0)
  ...
  // input
  mkldnnUtils::loadDataToMklStream(xTR, engine, stream, x_user_md,
  ...
  auto x_mkl_mem = xReorder ? dnnl::memory(op_prim_desc.src_desc(), engine) :
  x_user_mem; if (xReorder) dnnl::reorder(x_user_mem, x_mkl_mem).execute(stream,
  x_user_mem, x_mkl_mem); args[DNNL_ARG_SRC] = x_mkl_mem;
*/
  // y
  mkldnnUtils::loadDataToMklStream(yTR, engine, stream, y_user_md,
  ...
  auto y_mkl_mem = yReorder ? dnnl::memory(op_prim_desc.weights_desc(), engine)
  : y_user_mem; if (yReorder) dnnl::reorder(y_user_mem,
  y_mkl_mem).execute(stream, y_user_mem, y_mkl_mem); args[DNNL_ARG_WEIGHTS] =
  y_mkl_mem;
*/
  // z
...
PLATFORM_IMPL(matmul, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(
      xRank > 0 && yRank > 0, 0,
      "MATMUL MKLDNN OP: input arrays must have rank bigger than 0 (should not "
      "be scalars), but got instead: x rank = %i, y rank = %i !",
      xRank, yRank);

  if (xRank == 1 &&
      yRank == 1) {  // dot case, output is scalar (or vector with length = 1)
    REQUIRE_TRUE(x->lengthOf() == y->lengthOf(), 0,
                 "MATMUL MKLDNN OP: since input arrays are vectors they must "
                 "have the same length, but got x length = %i, y length = %i !",
                 x->lengthOf(), y->lengthOf());
  } else if (xRank == 1 && yRank == 2) {  // vector x matrix, i.e. [4] x [4,5] =
                                          // [5], output is vector
    REQUIRE_TRUE(x->lengthOf() == y->sizeAt(yLastButOneDim), 0,
                 "MATMUL MKLDNN OP: input arrays have inconsistent shapes for "
                 "vector-matrix product: x %s, y %s !",
                 ShapeUtils::shapeAsString(x).c_str(),
                 ShapeUtils::shapeAsString(y).c_str());
  } else if (xRank == 2 && yRank == 1) {  // matrix x vector , i.e. [4,5] x [5]
                                          // = [4], output is vector
    REQUIRE_TRUE(x->sizeAt(xLastDim) == y->lengthOf(), 0,
                 "MATMUL MKLDNN OP: input arrays have inconsistent shapes for "
                 "matrix-vector product: x %s, y %s !",
                 ShapeUtils::shapeAsString(x).c_str(),
                 ShapeUtils::shapeAsString(y).c_str());
  } else {
    REQUIRE_TRUE(
        xRank == yRank && yRank == zRank, 0,
        "MATMUL MKLDNN OP: input and output arrays must have the same rank, "
        "but got instead: x rank = %i, y rank = %i, z rank = %i !",
        xRank, yRank, zRank);
    REQUIRE_TRUE(x->sizeAt(xLastDim) == y->sizeAt(yLastButOneDim) &&
                     x->sizeAt(xLastButOneDim) == z->sizeAt(-2) &&
                     y->sizeAt(yLastDim) == z->sizeAt(-1),
                 0,
                 "MATMUL MKLDNN OP: input/output arrays have inconsistent "
                 "shapes for matrix product: x %s, y %s, z %s !",
                 ShapeUtils::shapeAsString(x).c_str(),
                 ShapeUtils::shapeAsString(y).c_str(),
    ...
    if (xRank > 2)  // outer dims must be the same
      for (int i = 0; i < xRank - 2; ++i)
        REQUIRE_TRUE(
            ...
            "MATMUL MKLDNN OP: input/output arrays have inconsistent shapes "
            "for matrix product: x %s, y %s, z %s !",
            ShapeUtils::shapeAsString(x).c_str(),
            ShapeUtils::shapeAsString(y).c_str(),
            ShapeUtils::shapeAsString(z).c_str());
  }
