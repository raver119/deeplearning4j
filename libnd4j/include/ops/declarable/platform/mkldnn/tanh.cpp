...
static void tanhMKLDNN(const NDArray* x, NDArray* z) {
  ...
  dnnl::eltwise_forward::desc op_desc(dnnl::prop_kind::forward_inference,
                                      algorithm::eltwise_tanh, x_mkl_md, 0, 0);
...
PLATFORM_IMPL(tanh, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(rank <= 6, 0,
               "TANH_MKLDNN OP: the rank of input must be less or qual 6, but "
               "got rank = %i instead !",
               rank);
...
static void tanhBpMKLDNN(const NDArray* x, const NDArray* dLdz, NDArray* dLdx) {
  ...
  // operation primitive description
  // forward
  dnnl::eltwise_forward::desc op_ff_desc(dnnl::prop_kind::forward_inference,
                                         algorithm::eltwise_tanh, x_mkl_md, 0,
                                         0);
  dnnl::eltwise_forward::primitive_desc op_ff_prim_desc(op_ff_desc, engine);

  // backward description
  dnnl::eltwise_backward::desc op_desc(algorithm::eltwise_tanh, dLdz_mkl_md,
                                       x_mkl_md, 0, 0);
  dnnl::eltwise_backward::primitive_desc op_prim_desc(op_desc, engine,
                                                      op_ff_prim_desc);
...
PLATFORM_IMPL(tanh_bp, ENGINE_CPU) {
  ...
  REQUIRE_TRUE(
      rank <= 6 && dLdzRank <= 6, 0,
      "TANH_BP_MKLDNN OP: the rank of input and dLdz must be less or qual 6, "
      "but got input rank = %i and dLdz rank rank = %i instead !",
      rank, dLdzRank);
...
PLATFORM_CHECK(tanh_bp, ENGINE_CPU) {
  ...
  bool bSupportedRanks = xRank < 7 && xRank > 0 && dLdzRank == xRank &&
                         (!x->isEmpty() && !dLdz->isEmpty());
  bSupportedRanks &=
