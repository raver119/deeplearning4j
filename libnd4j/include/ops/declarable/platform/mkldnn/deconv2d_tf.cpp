...
//////////////////////////////////////////////////////////////////////////
static void deconv2TFdBackPropMKLDNN(const NDArray* weights,
                                     const NDArray* gradO, NDArray* gradI,
                                     const int bS, const int iC, const int iH,
                                     const int iW, const int oC, const int oH,
  ...
  // weights [oC, iC, kH, kW] always, mkl doesn't support weights format [kH,
  // kW, iC, oC] gradO [bS, oH, oW, oC]
  ...
  dnnl::memory::desc x_mkl_md =
      dnnl::memory::desc(xDims, gradOType, dnnl::memory::format_tag::any);
  ...
  w_user_md.data.format_desc.blocking.strides[0] =
      weights->strideAt(3);  // permute [kH, kW, iC, oC] -> [oC, iC, kH, kW]
  ...
  mkldnnUtils::loadDataToMklStream(gradO, engine, stream, gradO_user_md,
                                   op_data_bp_prim_desc.diff_dst_desc(),
...
PLATFORM_IMPL(deconv2d_tf, ENGINE_CPU) {
  ...
  auto gradIShape = INPUT_VARIABLE(
      0);  // [4] - shape of input of conv2d (that is shape of gradI)
  ...
  REQUIRE_TRUE(weights->rankOf() == rank, 0,
               "CUSTOM DECONV2D_TF MKLDNN OP: rank of weights array must be "
               "equal to 4, but got %i instead !",
               weights->rankOf());
  REQUIRE_TRUE(gradIShape->rankOf() == 1, 0,
               "CUSTOM DECONV2D_TF MKLDNN OP: rank of array with output shape "
               "must be equal to 1, but got %i instead !",
               gradIShape->rankOf());
  REQUIRE_TRUE(gradIShape->lengthOf() == rank, 0,
               "CUSTOM DECONV2D_TF MKLDNN OP: length of array with output "
               "shape must be equal to 4, but got %i instead !",
               gradIShape->lengthOf());
  ...
  std::vector<Nd4jLong> gradIShapeVector =
      gradIShape->template asVectorT<Nd4jLong>();
  ...
  REQUIRE_TRUE(
      ...
      "CUSTOM DECONV2D_TF MKLDNN OP: wrong shape of input array, basing on "
      "array with output shape expected is %s, but got %s instead !",
  ...
  REQUIRE_TRUE(weights->isSameShape(expectedWeightsShape), 0,
               "CUSTOM DECONV2D_TF MKLDNN OP: wrong shape of weights array, "
  ...
  // weights = new NDArray(weights->permute({3,2,0,1}));        // [kH, kW, iC,
  // oC] -> [oC, iC, kH, kW]

  // // mkl supports NCHW format only
  // if(!isNCHW) {
  //     gradI = new NDArray(gradI->permute({0,3,1,2}));    // [bS, iH, iW, iC]
  //     -> [bS, iC, iH, iW] gradO = new NDArray(gradO->permute({0,3,1,2})); //
  //     [bS, oH, oW, oC] -> [bS, oC, oH, oW]
  // }

  deconv2TFdBackPropMKLDNN(weights, gradO, gradI, bS, iC, iH, iW, oC, oH, oW,
                           kH, kW, sH, sW, pH, pW, dH, dW, isNCHW, wFormat);
  ...
  // nullptr, nullptr, kH,kW,sH,sW,pH,pW,dH,dW,isSameMode,isNCHW);
...
PLATFORM_CHECK(deconv2d_tf, ENGINE_CPU) {
  ...
  auto gradO = INPUT_VARIABLE(
      2);  // [bS, oH, oW, oC] (NHWC) or [bS, oC, oH, oW] (NCDHW), epsilon_next
  auto gradI = OUTPUT_VARIABLE(
