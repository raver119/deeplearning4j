 ...
 * IArgs[0]       - order of output identity matrix, 99 -> 'c'-order, 102 ->
 * 'f'-order IArgs[1]       - the number of rows in output inner-most 2D
 * identity matrix IArgs[2]       - optional, the number of columns in output
 * inner-most 2D identity matrix, if this argument is not provided then it is
 * taken to be equal to number of rows IArgs[3,4,...] - optional, shape of
 * batch, output matrix will have leading batch dimensions of this shape
 */
 ...
 * - range array with first element being bottom limit and second element being
 top limit of histogram, please note that input_value <= range[0] will be mapped
 to histogram[0], input_value >= range[1] will be mapped to histogram[-1]
 *
 * Input integer arguments:
