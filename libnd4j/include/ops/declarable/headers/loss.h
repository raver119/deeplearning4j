 ...
 *    1: weights - is used for weighting (multiplying) of loss values, type
 * float. Can be single scalar or has the same rank as labels and must be
 * broadcastable to labels. 2: labels - ground truth vales, expected to be 0.
 * or 1., type float. Must have the same shape as logits.
 *
 *  Input integer arguments:
 *    0: type of reduction to apply to loss
 *       0 - "none", unreduced weighted losses with the same shape as logits.
 *       1 - "weighted_sum", output is scalar and equal to sum of all elements
 * of weightedLosses array 2 - "weighted_mean", output is scalar and equal to
 * sum of all elements of weightedLosses array divided by sum of all elements of
 * weightsBroad array 3 - "weighted_sum_by_nonzero_weights", output is scalar
 * and equal to scalar sum of all elements of weightedLosses array divided by
 * number of non-zero weights
 *
 * Output array:
 *    0: loss values, type float.
 *       Can be an array with the same shape as logits or just single scalar,
 ...
 *    0.5 * (labels-predictions)^2                                if
 * |labels-predictions| <= delta 0.5 * delta^2 + delta * (|labels-predictions| -
 * delta)      if |labels-predictions| >  delta
 *
 * Input arrays:
 *    0: predictions - the predicted values, type float
 *    1: weights - is used for weighting (multiplying) of loss values, type
 * float. Can be single scalar or has the same rank as labels, and must be
 * broadcastable to labels. 2: labels - ground truth vales, type float. Must
 * have the same shape as predictions.
 *
 *  Input integer arguments:
 *    0: type of reduction to apply to loss
 *       0 - "none", unreduced weighted losses with the same shape as
 * predictions 1 - "weighted_sum", output is scalar and equal to sum of all
 * elements of weightedLosses array 2 - "weighted_mean", output is scalar and
 * equal to sum of all elements of weightedLosses array divided by sum of all
 * elements of weightsBroad array 3 - "weighted_sum_by_nonzero_weights", output
 * is scalar and equal to scalar sum of all elements of weightedLosses array
 ...
 * Output array:
 *    0: loss values, type float.
 *       Can be an array with the same shape as predictions or just single
 * scalar, depending on reduction mode (see input integer argument)
 */
...
//////////////////////////////////////////////////////////////////////////
/**
 * Implementation of logarithmic loss function ( y_i * log(p_i) + (1 - y_i) *
 * log(1 - p_i) )
 *
 * Input arrays:
 *    0: predictions - the predicted values, type float
 ...
 * weightsBroad array 3 - "weighted_sum_by_nonzero_weights", output is scalar
 * and equal to scalar sum of all elements of weightedLosses array divided by
 * number of non-zero weights 1: optional - boolean value compute_full_loss: 0
 * (default) or 1 (compute)
 *
 * Output array:
 *    0: loss values, type float.
 *       Can be an array with the same shape as log_predictions or just single
 * scalar, depending on reduction mode (see input integer argument)
 */
...
//////////////////////////////////////////////////////////////////////////
/**
 * Implementation of Sum-of-Squares loss function 1/N *
 * sum_{i}^{N}(predictions_i - labels_i)^2
 *
 * Input arrays:
 *    0: predictions - the predicted values, type float
 *    1: weights - is used for weighting (multiplying) of loss values, type
...
//////////////////////////////////////////////////////////////////////////
/**
 * Implementation of sigmoid cross-entropy loss function max(logits, 0.) -
 * logits * labels + log(1. + exp(-abs(logits)));
 *
 * Input arrays:
 *    0: logits - logits, type float
 *    1: weights - is used for weighting (multiplying) of loss values, type
 ...
 *
 *  Input float arguments:
 *    0: smoothing value, if it is greater than 0 then apply smoothing to the
 * labels (smooth the labels towards 1/2): new_labels = labels * (1 -
 * labelsSmoothing)+ 0.5 * labelsSmoothing
 *
 * Output array:
 *    0: loss values, type float.
 *       Can be an array with the same shape as logits or just single scalar,
...
//////////////////////////////////////////////////////////////////////////
/**
 * Implementation of softmax cross-entropy loss function max(logits, 0.) -
 * logits * labels + log(1. + exp(-abs(logits)));
 *
 * Input arrays:
 *    0: logits - logits, type float
 ...
 *  Input float arguments:
 *    0: smoothing value, if it is greater than 0 then apply smoothing to the
 * labels (smooth the labels towards 1/numClasses):  new_labels = labels * (1 -
 * labelsSmoothing) + labelsSmoothing / numClasses
 *
 * Output array:
 *    0: loss values, type float.
 *       Can be an array with shape as in logits except last dimension is equal
 * to unity or just single scalar, depending on reduction mode (see input
 * integer argument)
 */
...
//////////////////////////////////////////////////////////////////////////
/**
 * Implementation of cosine-distance loss function 1. - (predictions *
 * labels).reduce_sum_along(dimension)
 *
 * Input arrays:
 *    0: predictions - the predicted values, type float
 *    1: weights - is used for weighting (multiplying) of loss values, type
 ...
 * elements of weightsBroad array 3 - "weighted_sum_by_nonzero_weights", output
 * is scalar and equal to scalar sum of all elements of weightedLosses array
 * divided by number of non-zero weights 1: dimension along which the cosine
 ...
 *
 * Output array:
 *    0: loss values, type float. An array with shape resulting from reducing of
 * logits shape along dimension with classes
 */
...
DECLARE_CUSTOM_OP(softmax_cross_entropy_loss_with_logits_grad, 2, 2, false, 0,
                  0);
#endif
 ...
 *
 * Input arrays:
 *    0: labels - ground truth vales, expected to be within range [0,
 * num_classes), type float. Must have rank equal logits rank minus 1. 1: logits
 * - logits, type float
 *
 * Output array:
...
DECLARE_CUSTOM_OP(sparse_softmax_cross_entropy_loss_with_logits, 2, 1, false, 0,
                  0);
DECLARE_CUSTOM_OP(sparse_softmax_cross_entropy_loss_with_logits_grad, 2, 1,
                  false, 0, 0);
#endif
