...
namespace ops {
/**
 * This operation returns index of max element in a given NDArray (optionally:
 * along given dimension(s)) Expected input: 0: N-dimensional array 1: optional
 * axis vector
 *
 * Int args:
 * 0: optional axis
...
/**
 * This operation returns index of min element in a given NDArray (optionally:
 * along given dimension(s)) Expected input: 0: N-dimensional array 1: optional
 * axis vector
 *
 * Int args:
...
/**
 * Inserts elements provided by diagonal array into the main diagonal of
 * innermost matrices of input array
 *
 * Input arrays:
 *  0: input array, considered as batch of matrices
 *  1: diagonal array containing elements to be inserted into input array,
 *     following rank condition should be satisfied: diagonal_rank = input_rank
 * - 1, the shapes of diagonal and input arrays must be equal except last
 * dimension of input array, for example if input_shape = [A,B,C,D] then
 * diagonal_shape = [A,B,C], also last dimension of diagonal array should be
 * equal to smaller of last and last but one input dimensions that is:
 * diagonal_shape[-1] = min(input_shape[-1], input_shape[-2])
 *
 * Output array:
 *  0: has the same shape as input, corresponding diagonal elements are
 * substituted
 */
...
/**
 * Inserts elements provided by diagonal array into the main diagonal of
 * innermost matrices of output array, rest output elements are set to zeros
 *
 * Input array:
 *    diagonal: array containing elements to be inserted into output array,
 *              following rank condition is present: diagonal_rank = ouput_rank
 * - 1
 *
 * Output array:
 *   0: is considered as batch of matrices, if for example diagonal array has
 * shape [A,B,C] then output array has shape [A,B,C,C]
 */
DECLARE_CUSTOM_OP(matrix_diag, 1, 1, false, 0, 0);

/**
 * This op calculates regularized incomplete beta integral Ix(a, b).
 * Implementation is based on two algorithms depending on input values of a and
 * b:
 * - when a and b are both >  maxValue (3000.), then Gauss-Legendre quadrature
 * method is applied
 * - when a and b are both <= maxValue (3000.), then modified Lentz’s algorithm
 * for continued fractions is applied
 *
 * Input arrays:
 *    a: defines power t^{a-1}, must be > 0, type float.
 *    b: defines power (1-t)^{b-1}, must be > 0, type float.
 *    x: defines upper limit of integration, must be within (0 <= x <= 1) range,
 * type float.
 *
 * Output array:
 *    0: values of  regularized incomplete beta integral that corresponds to
 * variable upper limit x, type float
 *
...
/**
 * Returns a diagonal tensor with a given diagonal values. Given a diagonal,
 * this operation returns a tensor with the diagonal and everything else padded
 * with zeros.
 */
 ...
 * Using input tensor as batched 2D diagonals flat them to vector (1D) with
 * diagonal values.
 *
...
/**
 * QR decomposition: A = QR, where Q is ortogonal (Q * QT = I) and R is upper
 * triangular. For A (MxN) Q is M x M and R is (NxN).
 *
 * Input :
 *    0 - float (or complex float) tensor with shape {.,..,...,M,N} - batch of
 * float matricies
 *
 * Output:
 *    0 - float tensor with shape {.,..,...,MxN} - batch of ortogonal matricies
 * {Qs} 1 - float tensor with shape {.,..,...,NxN} - batch of upper triangular
 * matricies {Rs}
 */
 ...
 * And returns 2 arrays: values left after exclusion, and indices in original
 * array for surivals. Expected arguments: 0: vector with original values 1:
 * vector with values to exclude
 */
...
/**
 * This operation scatter "updates" elements into new output array according to
 * given "indices" Expected arguments: indices: array containing elements/slices
 * indexes of output array to put "updates" elements into, the rest output
 * elements will be zeros updates: array containing elements to be inserted into
 * output array shape: contains shape of output array
 */
...
/**
 * This operation scatter "updates" elements into input array along given
 * "indices" Expected arguments: input: array to be updated indices: array
 * containing elements/slices indexes of input array to put "updates" elements
 * into updates: array containing elements to be inserted into input array
 */
 ...
 * Expected arguments:
 * input: array to be updated
 * indices: array containing elements/slices indexes of input array to add
 * "updates" elements to updates: array containing elements to be interfered
 * with input
 */
#if NOT_EXCLUDED(OP_scatter_add)
...
/**
 * This operation subtract "updates" elements from input array along given
 * "indices" Expected arguments: input: array to be updated indices: array
 * containing elements/slices indexes of input array to subtract "updates"
 * elements from updates: array containing elements to be interfered with input
 */
#if NOT_EXCLUDED(OP_scatter_sub)
...
/**
 * This operation takes input's shape, and returns new NDArray filled with
 * specified value Expected arguments: input: N-dimensional array
 *
 * T args:
...
/**
 * This operation returns unique elements from input array as vector, and their
 * original indices in input array Expected input: input: N-dimensional array
 */
...
/**
 * This operation returns 3 1D arrays for given 1D array with unique element
 * count and indexes input: 0 - 1D array
 *
 * output:
...
DECLARE_CUSTOM_OP(strided_slice, 1, 1, false, 0,
                  5);  // TODO: new op type needed. that returns VIEW
...
/**
 * This operation generate sequences. Basically from......to, with step used as
 * increment. Expected arguments: start: optional scalar with starting value
...
/**
 * This op calculates Hurwitz zeta function zeta(x, q) = sum_{n=0}^{inf} (q +
 * n)^{-x} Implementation is based on Euler-Maclaurin summation formula
 *
 *   Input arrays:
...
/**
 * This op calculates polygamma function psi^(n)(x). Implementation is based on
 * serial representation written in terms of the Hurwitz zeta function:
 * polygamma = (-1)^{n+1} * n! * zeta(n+1, x).
 *
 * Input arrays:
 *    0: n - define derivative order (n+1), type integer (however currently is
 * implemented as float casted to integer) 1: x - abscissa points where to
 * evaluate the polygamma function, type float
 *
 * Output array:
...
/**
 * This operation takes shape as first argument, and returns new NDArray filled
 * with specific scalar value. Input arrays: 0 - shape vector 1 - optional
 * scalar NDArray
 *
 * T arguments:
...
/**
 * This operation splits given NDArray into chunks of specific size, along given
 * dimension Input arrays: 0 - input array 1 - array of sizes 2 - optional axis
 *
 * Integer arguments:
 * 0 - optional axis
 *
...
/**
 * This operation splits given NDArray into chunks of specific size, along given
 * dimension 0 - input array 1 - optional axis
 *
 * Integer arguments:
 * 0 - number of splits
 * 1 - optional axis
 ...
 * Input arrays:
 * 0 - input array with rank >= 3, must have at least one dimension equal 3,
 * that is dimension containing channels. 1 - optional argument, input
 * scalar-array containing delta
 *
 * T arguments:
 ...
 * 0 - input array with rank >= 3, must have at least one dimension equal 3,
 * that is dimension containing channels. 1 - optional argument, input
 * scalar-array containing saturation factor
 *
 * T arguments:
...
/**
 * This operation adjusts image contrast by given factor ( z = (x - mean) *
 * factor + mean ) Input arrays: 0 - input array with rank >= 3, must have last
 * one dimension equal 3, that is dimension containing channels. 1 - optional
 * argument, input scalar-array containing saturation contrast factor
 *
 * T arguments:
...
/**
 * This operation rearranges data from depth into blocks of spatial data. This
 * is the reverse transformation of space_to_depth op. This op output is a copy
 * of the input tensor where values from the depth dimension are moved in
 * spatial blocks to the height and width dimensions. Int attr 0 indicates the
 * input block size and how the data is moved. Input: 0 - 4D tensor on given
 * type Output: 0 - 4D tensor of given type and proper shape
 *
 * Int arguments:
 *     0 - block size
 *     1 - output data format: 0 ("NHWC"): shape{ batch, height, width, channels
 * } 1 ("NCHW"): shape{ batch, channels, height, width } 2 ("NCHW_VECT_C"): int8
 * shape{ batch, channels / 4, height, width, 4 } optional (default 0)
 */
...
/**
 * This operation rearranges blocks of spatial data, into depth.This op output
 * is a copy of the input tensor where values from the height and width
 * dimensions are moved to the depth dimension. Int attr 0 indicates the input
 * block size.
 *
 * Input:
 ...
 * from the height and width dimensions are moved to the batch dimension. After
 ...
 *  The third is boolean value(default is true) (0 - as is, 1 - sorted by value)
 * optional
 */
 ...
 * Int args:
 *      0 - optional switcher of weights format, if int arg == 1 - mkldnn, else
 * mmul
 */
...
/**
 * sufficient_statistics operation return calculated mean and variation with
 * data count. this operation is invert for moments accordingly to shift and
 * count. input params:
 *  - input tensor
 *  - axes vector
 *
 ...
 *  - weights - the same shape tensor with integer weights for element
 * (optional) default weight - 1,1,1..,1 for all values in the tensor
 *
 *  optional ints:
...
/**
 * matrix_solve_ls op (lstsq) - solves one or more linear least-squares
 * problems.
 *
 * input params:
 *    0 - the tensor with dimension (x * y * z * ::: * M * N) - left parts of
 * equations 1 - the tensor with dimension (x * y * z * ::: * M * K) - right
 * parts of equations
 *
 * float args:
 ...
 *
 * boolean args:
 *    0 - fast - default is true (optional) - use Cholesky decomposition instead
 * QR decomposition of matricies.
 *
 * return value:
 ...
 *    tensor with dimension (x * y * z * ::: * N * K) with solutions
 *
 * Note: if fast is false - then l2_regularizer arg is ignored and used lstsq
 * method due QR decomposition
 * */
...
/**
 * matrix_inverse op. - make inverse for all 2D square matricies found in the
 * input tensor
 *
 * input params:
 *    0 - the tensor with dimension (x * y * z * ::: * M * M)
 *
 * return value:
 *    tensor with dimension (x * y * z * ::: * M * M) with inverse M x M
 * matricies in it
 */
...
/**
 * triangular_solve op. - reverse Gaussian method for solve systems of linear
 * equations.
 *
 * input params:
 *    0 - the tensor with dimension (x * y * z * ::: * M * M) - left parts of
 * equations 1 - the tensor with dimension (x * y * z * ::: * M * K) - right
 * parts of equations
 *
 * boolean args:
 *    0 - lower - default is true (optional) - left part is lower triangular
 * matrix 1 - adjoint - default is false (optional) - indicate input matrix or
 * its adjoint (hermitian addition) should be used
 *
 * return value:
 ...
 *
 * boolean args:
 *    0 - adjoint - default is false (optional) - indicate input matrix or its
 * adjoint (hermitian addition) should be used
 *
 * return value:
 *    tensor with dimension (x * y * z * ::: * M * K) with solutions
 *
 ...
 *
 * return value:
 *    0 - float tensor with dimension (x * y * z * ::: * M * M) with LU M x M
 * matricies in it 1 - int (32 or 64) batched vector of permutations with length
 * M - shape (x * y * z * ::: * M)
 *
 * int argument:
 *    0 - data type of output permutaion vector (int32 or int64), optional,
 * default INT32
 */
...
/**
 * sequence_mask op. - make mask for given tensor filled by (j > x[i_1,
 * i_2,...,i_n]) -> z[i_1, i_2,...,i_n,j]
 *
 * input params:
 *    0 - the ND-tensor filled by integer-like values
 *
 * optional int param - maxlength (maxlength >= max(x)). By default maxlength =
 * max(x). return value: (N+1)D tensor filled by 0 and 1 accordingly the mask
 */
#if NOT_EXCLUDED(OP_sequence_mask)
DECLARE_CUSTOM_OP(sequence_mask, 1, 1, false, 0, 0);
#endif
/**
 * segment_max op. - make a tensor filled by max values according to index
 * tensor given.
 *
 * input params:
...
/**
 * segment_min op. - make a tensor filled by min values according to index
 * tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * segment_sum op. - make a tensor filled by sum of values according to index
 * tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * segment_prod op. - make a tensor filled by product of values according to
 * index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
#endif
/**
 * segment_mean op. - make a tensor filled by average of values according to
 * index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * unsorted_segment_max op. - make a tensor filled by max values according to
 * index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * unsorted_segment_min op. - make a tensor filled by min values according to
 * index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * unsorted_segment_sum op. - make a tensor filled by sum of values according to
 * index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * unsorted_segment_prod op. - make a tensor filled by product of values
 * according to index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
 *    1 - the tensor with indices.
...
/**
 * unsorted_segment_mean op. - make a tensor filled by average of values
 * according to index tensor given.
 *
 * input params:
 *    0 - the tensor with data;
...
/**
 * unsorted_segment_sqrt_n op. - computes the sum along segments of a tensor
...
/**
 * extract_image_patches op - Extract patches from images and put them in the
...
/**
 * draw_bounding_boxes op - modified input image with given colors exept given
 * boxes.
 *
 * input params:
 *    0 - images tensor (4D) with shape {batch, width, height, channels}, where
 * channes is 1 (BW image), 3 (RGB) or 4 (RGBA) 1 - boxes tensor (3D) with shape
 * {batch, number_of_boxes, 4} where last dimension encoded as (y_min, x_min,
 * y_max, x_max), all values in between 0. and 1. 2 - colours tensor (2D) with
 * shape {number_of_boxes, channels} -- bordering color set (palette)
 *
 * output:
...
/**
 * roll - op porting from numpy
 * (https://docs.scipy.org/doc/numpy-1.14.0/reference/generated/numpy.roll.html)
 *
 * input params:
 *    0 - NDArray
 *
 ...
 *    N - axe N
 *
 *    All axes are optional and should be between 0 and input->rankOf(). Of
 * course, all axes can be repeated.
 *
 * output:
 ...
 *    2 - numOfElements
 * output:
 *    0 - 1D NDArray with the same type as input and length as given with
 * numOfElements param.
 */
 ...
 *
 * output:
 *    0 - NDArray with reduces shape accordingly to axes (the scalar in default
 * case).
 */
 ...
 *
 * float arguments:
 *   keepDims: if non zero, then keep reduced dimensions with length = 1,
 * default value is zero
 *
 * int arguments:
 *    list of integers - dimensions to calculate min along, default corresponds
 * to empty list in which case calculation is performed for all dimensions and
 * scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate max along, default corresponds
 * to empty list in which case calculation is performed for all dimensions and
 * scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate norm1 along, default
 * corresponds to empty list in which case calculation is performed for all
 * dimensions and scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate norm2 along, default
 * corresponds to empty list in which case calculation is performed for all
 * dimensions and scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate squared norm along, default
 * corresponds to empty list in which case calculation is performed for all
 * dimensions and scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate norm max along, default
 * corresponds to empty list in which case calculation is performed for all
 * dimensions and scalar is returned
 *
 * output array:
 ...
 *
 * int arguments:
 *    list of integers - dimensions to calculate mean along, default corresponds
 * to empty list in which case calculation is performed for all dimensions and
 * scalar is returned
 *
 * output array:
 ...
 * float arguments:
 *   keepDims: if non zero, then keep reduced dimensions with length = 1,
 * default value is zero biasCorrected -  if non zero, then bias correction will
 * be applied, default value is zero
 *
 * int arguments:
 *    list of integers - dimensions to calculate mean along, default corresponds
 * to empty list in which case calculation is performed for all dimensions and
...
/**
 * This op calculates sample standard deviation of elements along given
 * dimensions
 *
 * input array:
 *    x: tensor to calculate mean for
 ...
 *    N-1 axe N
 *
 *  CAUTION: All axes are optional and should be between 0 and input->rankOf() -
 * 1 and put either with second param or as integers but not both
 *
 * output:
 *    0 - NDArray with reduces shape accordingly to axes (the scalar in default
 * case).
...
/**
 * This op make bilinear or nearest neighbor interpolated resize for given
 * tensor
 *
 * input array:
 ...
 *
 * output array:
 *   the 4D-Tensor with resized image (shape is {batch, newWidth, newHeight,
 * channels})
 *
 * CAUTION: either size tensor or a pair of int params should be provided.
 */
...
/**
 * This op make area interpolated resize (as OpenCV INTER_AREA algorithm) for
 * given tensor
 *
 * input array:
 *    0 - images - 4D-Tensor with shape (batch, sizeX, sizeY, channels)
 *    1 - size -   1D-Tensor with 2 values (newWidth, newHeight) (if missing a
 * pair of integer args should be provided).
 *
 ...
 * Need to implement to full compatibility with TF: lanczos5, gaussian, area and
 * mitchellcubic
 *
 * input array:
 *    0 - 4D-Tensor with shape (batch, sizeX, sizeY, channels)
 ...
 *
 * output array:
 *   the 4D-Tensor with resized by given algorithm image (shape is {batch,
 ...
 *
 * input array:
 *    x: given tensor with shape {..., M, N} - as vector (matrix) of matricies
 * MxN
 *
 * int arguments:
 *   lower band
 ...
 * float args:
 *     0 - overlap_threshold - threshold value for overlap checks (optional, by
 * default 0.5) 1 - score_threshold - the threshold for deciding when to remove
 * boxes based on score (optional, by default -inf) int args: 0 - output_size -
 * as arg 2 used for same target. Eigher this or arg 2 should be provided.
 *
 * output:
 ...
 *
 * output:
 *     0 - 1D integer tensor with shape [M], epresenting the selected indices
 * from the overlaps tensor, where M <= max_output_size
 * */
...
/*
 * cholesky op - decomposite positive square symetric matrix (or matricies when
 * rank > 2). input: 0 - matricies - tensor with shape (..., N, N) by float type
 *
 * output - lower triangular matrix (matricies when rank > 2) with the same
 * shape as input.
 * */
 ...
 * input array:
 *     0 - input array
 *     1 - scalar tensor with n for operation. n should be less than last
 * dimension
 *
 * output:
 *    0 - NDArray with the same shape as input
...
/**
 * This op checks for Inf/NaN values within input array, and throws exception if
...
/**
 * fake_quant_with_min_max_vals_per_channel -
...
DECLARE_CONFIGURABLE_OP(fake_quant_with_min_max_vars_per_channel, 3, 1, true, 0,
                        -2);
#endif
