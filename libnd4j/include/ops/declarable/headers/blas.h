...
/**
 * This op is general matmum implementation. Depending on inputs dimensionality
 * output result might be different. matrix x matrix = BLAS gemm vector x matrix
 * = BLAS gemm vector x vector = BLAS dot vector x scalar = element-wise mul
 ...
 *
 * PLEASE NOTE: M, N, K, ldA, ldB, ldC should be equal for all matrices within
 * batch.
 */
