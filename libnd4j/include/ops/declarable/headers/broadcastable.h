...
/**
 * This is one of auto-broadcastable operations. It accepts 2 operands, and
 * operation is applied based on their shapes: 1) if shapes are equal that's
 * pairwise operation, result will have the same shape. 2) if shape X is scalar
 * and shape Y is array - result will have shape equal to Y. 3) if shape X is
 * array and shape Y is scalar - result will have shape equal to X. 4) if shape
 * X and Y are both arrays, but shapes aren't equal - result shape will be
 * broadcast result.
 *
...
/**
 * This op takes 2 equally shaped arrays as input, and provides binary matrix as
 * output. Math is: _x == _y ? (T) 1.0f : (T) 0.0f;
 *
 */
...
/**
 * This operation performs calculation of percentile of input array along given
 * axises
 *
 * Input - tensor with rank N > 0
 * Output - tensor with rank (N - length(axis)) or scalar if number of Integer
 * arguments is zero Float arguments: 0: percentile (scalar) in range [0,100]
 * (inclusively) 1: interpolation (optional), possible values are 0-"lower",
 * 1-"higher", 2-"nearest"(default) 2: keepDims (optional), if it is non zero,
 * then unities are kept in reduced resulting shape of output array, default is
 * 0 Integer arguments - axis - the sequence of axises to calculate percentile
 * along, if sequence is empty then calculate percentile for whole input tensor
 * and return result as scalar
 *
 */
