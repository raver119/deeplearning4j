 ...
 * array of the same shape, with elements, either from x or y, depending on the
 * condition.
 */
 ...
 *
 *  The output of the op is dynamic in size and returns a flat vector of
 * elements that return true on the given condition. In numpy parlance, most
 * people might understand: a[a > 2] where a is a numpy array and the condition
 * is true when an element is > 2. Libnd4j already implements a number of pre
 * defined conditions.
 * @tparam T
 */
...
/**
 * This op takes 1 n-dimensional array as input, and returns true if for every
 * adjacent pair we have x[i] <= x[i+1].
 */
...
/**
 * This op takes 1 n-dimensional array as input, and returns true if input is a
 * numeric array.
 */
