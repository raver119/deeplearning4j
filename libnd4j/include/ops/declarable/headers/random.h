...
#endif
/*
 * multinomial (categorical) random generator draws samples from a multinomial
 * distribution
 *
 * Input array:
 *    0 - 2D ndarray with unnormalized log-probabilities with shape [batch_size
 * (N), num_classes (K)] 1 - array with one int value of samples number, number
 * of independent samples to draw for each experiment 1,N. Int arguments: 0 -
 * optional argument, corresponds to dimension with batch_size 1 - optional
