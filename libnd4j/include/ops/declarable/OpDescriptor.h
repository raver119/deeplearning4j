...
class SD_EXPORT OpDescriptor {
 protected:
  ...
  // special flag for divergent ops - ops that CAN and WILL modify graph
  // behavior. Literally: IF, CASE.
  ...
  bool checkDataTypesMatch(sd::DataType needle,
                           std::vector<sd::DataType>& haystack) const;
 ...
 public:
  ...
  OpDescriptor(int numInputs, int numOutputs, const char* opName,
               bool allowsInplace, bool divergent, int tArgs, int iArgs);
  ...
  OpDescriptor* setAllowedInputTypes(
      const std::initializer_list<sd::DataType>& dtype);
  OpDescriptor* setAllowedOutputTypes(
      const std::initializer_list<sd::DataType>& dtype);
  OpDescriptor* setAllowedInputTypes(int index,
                                     const std::vector<sd::DataType>& dtype);
  OpDescriptor* setAllowedOutputTypes(int index,
                                      const std::vector<sd::DataType>& dtype);
