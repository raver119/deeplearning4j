...
class BroadcastHelper {
 public:
  static FORCEINLINE NDArray* broadcastApply(
      sd::BroadcastOpsTuple op, NDArray* x, NDArray* y, NDArray* z,
      ExtraArguments* extraArgs = nullptr) {
    if (x->isEmpty() || y->isEmpty()) {
      if (!z->isEmpty())
        throw std::runtime_error(
            "BroadcastHelper::broadcastApply: when some of input arrays (or "
            "both) is empty, output array must be empty as well !");
      return z;
    }
    ...
    if (!x->isScalar() && !y->isScalar() && x->isSameShape(y)) {
      ...
      if (z->isSameShape(y)) {
        ...
        } else if (op.s == scalar::MaxPairwise || op.s == scalar::MinPairwise ||
                   op.s == scalar::AMaxPairwise ||
        ...
        return tZ;
               ...
               y->isScalar()) {  // x->isScalar() && y->isScalar()
      x->applyScalarArr(op.s, const_cast<const NDArray&>(*y), *z);
      ...
      nd4j_printf(
          "Broadcast: shapes should be equal, or broadcastable. But got %s vs "
          "%s instead\n",
          sx.c_str(), sy.c_str());
      return nullptr;
    }
  ...
  static FORCEINLINE NDArray* broadcastApply(
      sd::BroadcastBoolOpsTuple op, NDArray* x, NDArray* y, NDArray* z,
      ExtraArguments* extraArgs = nullptr) {
    if (x->isEmpty() || y->isEmpty()) {
      if (!z->isEmpty())
        throw std::runtime_error(
