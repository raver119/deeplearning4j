...
CUSTOM_OP_IMPL(solve, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(shape::shapeEquals(a->rankOf() - 2, a->shapeInfo(),
                                  b->rankOf() - 2, b->shapeInfo()),
               0, "solve: Input shapes should be alike.");
  REQUIRE_TRUE(a->rankOf() >= 2, 0,
               "solve: The rank of input left tensor should not be less than "
               "2, but %i is given",
               a->rankOf());
  REQUIRE_TRUE(b->rankOf() >= 2, 0,
               "solve: The rank of input right tensor should not be less than "
               "2, but %i is given",
               b->rankOf());

  REQUIRE_TRUE(a->sizeAt(-1) == a->sizeAt(-2), 0,
               "solve: The last two dimmensions should be equal, but %i and %i "
               "are given",
               a->sizeAt(-1), a->sizeAt(-2));
  REQUIRE_TRUE(a->sizeAt(-1) == b->sizeAt(-2), 0,
               "solve: The last dimmension of left part should be equal to "
  ...
  auto res =
      helpers::solveFunctor(block.launchContext(), input, b, useAdjoint, z);
...
DECLARE_SHAPE_FN(solve) {
  ...
  auto luShape =
      ShapeBuilders::copyShapeInfoAndType(in1, in0, true, block.workspace());
