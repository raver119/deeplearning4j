...
CONFIGURABLE_OP_IMPL(polygamma, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(n->isSameShape(x), 0,
               "POLYGAMMA op: two input arrays n and x must have the same "
               "shapes, but got n=%s and x=%s instead !",
               ShapeUtils::shapeAsString(n).c_str(),
  ...
  REQUIRE_TRUE(nPositiveFlag, 0,
               "POLYGAMMA op: all elements of n array must be >= 0 !");
  REQUIRE_TRUE(xPositiveFlag, 0,
               "POLYGAMMA op: all elements of x array must be > 0 !");
