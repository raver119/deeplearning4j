...
CUSTOM_OP_IMPL(triu, 1, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      input->rankOf() > 0, 0,
      "TRIU OP: the rank of input array must be > 0, but got %i instead !",
      input->rankOf());

  const int diag = block.numI() > 0 ? INT_ARG(0) : 0;

  BUILD_SINGLE_SELECTOR(input->dataType(), input->fillAsTriangular,
                        (0, diag, 0, *output, 'l'), LIBND4J_TYPES);
...
DECLARE_SHAPE_FN(triu) {
  ...
  ALLOCATE(outShapeInfo, block.workspace(), shape::shapeInfoLength(rank),
           Nd4jLong);
  memcpy(
      outShapeInfo, inShapeInfo,
      (1 + rank) * sizeof(Nd4jLong));  // copy rank and dimensions values only
  ...
  ShapeUtils::updateStridesAndType(outShapeInfo, inShapeInfo,
...
CUSTOM_OP_IMPL(triu_bp, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      input->rankOf() > 0, 0,
      "TRIU_BP OP: the rank of input array must be > 0, but got %i instead !",
      input->rankOf());
