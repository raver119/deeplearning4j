...
CUSTOM_OP_IMPL(lu, 1, 2, false, 0, 0) {
  ...
  if (block.numI()) {
    ...
    REQUIRE_TRUE(dtype == sd::DataType::INT32 || dtype == sd::DataType::INT64,
                 0,
                 "lu: Permutation data type should be 32bit or 64bit int only, "
                 "but '%s' given.",
  ...
  REQUIRE_TRUE(
      input->rankOf() >= 2, 0,
      "lu: The rank of input array should not less than 2, but %i is given",
      input->rankOf());
  REQUIRE_TRUE(
      input->sizeAt(-1) == input->sizeAt(-2), 0,
      "lu: The last two dimmensions should be equal, but %i and %i are given",
...
DECLARE_SHAPE_FN(lu) {
  ...
  auto luShape =
      ShapeBuilders::copyShapeInfoAndType(in, in, true, block.workspace());
  ...
  auto luP = ShapeBuilders::createShapeInfo(
      dtype, shape::order(in), shapeVector.size() - 1, shapeVector.data(),
      block.workspace());
