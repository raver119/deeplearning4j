...
CUSTOM_OP_IMPL(matrix_diag, 1, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(!diagonal->isScalar(), 0,
               "CUSTOM_OP matrix_diag: input diagonal array must be at list a "
               "vector, but scalar was given!");

  helpers::matrixSetDiag(block.launchContext(), *output, *diagonal, *output,
                         true);
...
DECLARE_SHAPE_FN(matrix_diag) {
  ...
  //  if for example diagonal array has shape [A,B,C] then output array has
  //  shape [A,B,C,C]
