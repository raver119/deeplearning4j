...
CUSTOM_OP_IMPL(matrix_diag_part, 1, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(inRank >= 2, 0,
               "CUSTOM_OP matrix_diag_part: input array must have rank >= 2, "
               "but %i given!",
               inRank);
...
DECLARE_SHAPE_FN(matrix_diag_part) {
  ...
  int lastDimension =
      sd::math::nd4j_min(shape::sizeAt(in, -1), shape::sizeAt(in, -2));
  if (outRank == 1) {
    // output shape is a vector with size min(sizeAt(0), sizeAt(1))
    outShapeInfo = ConstantShapeHelper::getInstance().vectorShapeInfo(
        lastDimension, ArrayOptions::dataType(in));
  } else {
    Nd4jLong* anShapeInfo;
    ALLOCATE(anShapeInfo, block.workspace(), shape::shapeInfoLength(outRank),
             Nd4jLong);
