...
CUSTOM_OP_IMPL(svd, 1, 1, false, 0, 3) {
  ...
  REQUIRE_TRUE(
      rank >= 2, 0,
      "SVD OP: the rank of input array must be >=2, but got %i instead!", rank);
  ...
  helpers::svd(block.launchContext(), x,
               {OUTPUT_VARIABLE(0), calcUV ? OUTPUT_VARIABLE(1) : nullptr,
                calcUV ? OUTPUT_VARIABLE(2) : nullptr},
...
DECLARE_SHAPE_FN(svd) {
  ...
  const int diagSize = inShapeInfo[rank] < inShapeInfo[rank - 1]
                           ? inShapeInfo[rank]
                           : inShapeInfo[rank - 1];

  Nd4jLong* sShapeInfo(nullptr);
  if (rank == 2) {
    ALLOCATE(sShapeInfo, block.workspace(), shape::shapeInfoLength(1),
             Nd4jLong);
    sShapeInfo[0] = 1;
    sShapeInfo[1] = diagSize;
  } else {
    ALLOCATE(sShapeInfo, block.workspace(), shape::shapeInfoLength(rank - 1),
             Nd4jLong);
    sShapeInfo[0] = rank - 1;
  ...
  ShapeUtils::updateStridesAndType(sShapeInfo, inShapeInfo,
  ...
  if (calcUV) {
    ...
    auto result = SHAPELIST(ConstantShapeHelper::getInstance()->createShapeInfo(
                                ShapeDescriptor(sShapeInfo)),
                            ConstantShapeHelper::getInstance()->createShapeInfo(
                                ShapeDescriptor(uShapeInfo)),
                            ConstantShapeHelper::getInstance()->createShapeInfo(
                                ShapeDescriptor(vShapeInfo)));
