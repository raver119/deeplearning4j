...
CUSTOM_OP_IMPL(lstsq, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(a->rankOf() >= 2, 0,
               "lstsq: The rank of input left tensor should not be less than "
               "2, but %i is given",
               a->rankOf());
  REQUIRE_TRUE(b->rankOf() >= 2, 0,
               "lstsq: The rank of input right tensor should not be less than "
               "2, but %i is given",
               b->rankOf());

  //            REQUIRE_TRUE(a->sizeAt(-1) == a->sizeAt(-2), 0, "lstsq: The last
  //            two dimmensions should be equal, but %i and %i are given",
  //            a->sizeAt(-1), a->sizeAt(-2));
  REQUIRE_TRUE(a->sizeAt(-2) == b->sizeAt(-2), 0,
               "lstsq: The last dimmension of left part should be equal to "
               "prelast of right part, but %i and %i are given",
               a->sizeAt(-1), b->sizeAt(-2));
  // REQUIRE_TRUE(l2_factor == 0., 0, "lstsq: Implementation of operation is not
  // finished for factor difference from 0.");
  if (a->isEmpty() || b->isEmpty() || z->isEmpty()) return Status::OK();

  auto res = helpers::leastSquaresSolveFunctor(block.launchContext(), a, b,
                                               l2_factor, fastFlag, z);
...
CUSTOM_OP_IMPL(solve_ls, 2, 1, false, 0, 0) {
  ...
  res = helpers::leastSquaresSolveFunctor(block.launchContext(), a, b,
                                          l2_factor, fastFlag, z);
...
DECLARE_SHAPE_FN(lstsq) {
  ...
  auto resShape = ConstantShapeHelper::getInstance().createShapeInfo(
      ...
      shapeOf);  // ShapeBuilders::copyShapeInfoAndType(in1, in0, true,
                 // block.workspace());
  if (shapeOf[rank - 1] == 0) {
    //                ArrayOptions::setPropertyBit(resShape, ARRAY_EMPTY);
    resShape = ConstantShapeHelper::getInstance().emptyShapeInfo(
        ArrayOptions::dataType(in0));
  }
