...
CUSTOM_OP_IMPL(huber_loss, 3, 1, false, 1, 1) {
  ...
  REQUIRE_TRUE(labels->isSameShape(predictions), 0,
               "HUBER_LOSS OP: labels and predictions arrays must have the "
  ...
  REQUIRE_TRUE(weights->isScalar() || weights->rankOf() == labels->rankOf(), 0,
               "HUBER_LOSS OP: weights array should be scalar or have the same "
  ...
  REQUIRE_TRUE(weights->isScalar() ||
               ...
               0,
               "HUBER_LOSS OP: shapes of weights and labels arrays should be "
  ...
  REQUIRE_TRUE(reductionMode == 0 || reductionMode == 1 || reductionMode == 2 ||
                   reductionMode == 3,
               0,
               "HUBER_LOSS OP: reduction mode value is not acceptable, "
  ...
  NDArray quadratic(error.shapeInfo(), block.workspace());
...
DECLARE_SHAPE_FN(huber_loss) {
  ...
  REQUIRE_TRUE(
      ...
      0,
      "HUBER_LOSS OP: weights array should be scalar or have the same rank as "
...
CUSTOM_OP_IMPL(huber_loss_grad, 3, 3, false, 1, 1) {
  ...
  // inputs validation
  REQUIRE_TRUE(labels->isSameShape(predictions), 0,
               "HUBER_LOSS_GRAD OP: labels and predictions arrays must have "
  ...
  // be broadcastable to labels
  REQUIRE_TRUE(weights->isScalar() || weights->rankOf() == labels->rankOf(), 0,
               "HUBER_LOSS_GRAD OP: weights array should be scalar or have the "
  ...
  REQUIRE_TRUE(
      weights->isScalar() ||
          ShapeUtils::areShapesBroadcastable(*weights, *labels),
      0,
      "HUBER_LOSS_GRAD OP: shapes of weights and labels arrays should be "
      "broadcastable, but got weights = %s and labels = %s instead!",
      ShapeUtils::shapeAsString(weights).c_str(),
      ShapeUtils::shapeAsString(labels).c_str());
  // only 4 possible reduction modes exist
  REQUIRE_TRUE(reductionMode == 0 || reductionMode == 1 || reductionMode == 2 ||
                   reductionMode == 3,
               0,
               "HUBER_LOSS_GRAD OP: reduction mode value is not acceptable, "
               "possible values are 0, 1, 2, 3, but got %i instead!",
               reductionMode);
...
DECLARE_SHAPE_FN(huber_loss_grad) {
  ...
  REQUIRE_TRUE(
      shape::isScalar(weightsShapeInfo) ||
          shape::rank(weightsShapeInfo) == shape::rank(labelsShapeInfo),
      0,
      "HUBER_LOSS_GRAD OP: weights array should be scalar or have the same "
      "rank as labels array, but got %i and %i correspondingly!",
      shape::rank(weightsShapeInfo), shape::rank(labelsShapeInfo));
  // check whether broadcast operation is possible for weights array
  REQUIRE_TRUE(
