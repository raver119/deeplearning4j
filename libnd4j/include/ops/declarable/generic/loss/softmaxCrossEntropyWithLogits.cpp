...
CUSTOM_OP_IMPL(softmax_cross_entropy_loss_with_logits, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      ...
      "SOFTMAX_CROSS_ENTROPY_LOSS_WITH_LOGITS OP: labels and logits arrays "
  ...
  REQUIRE_TRUE(
      classesDim < logits->rankOf(), 0,
      "SOFTMAX_CROSS_ENTROPY_LOSS_WITH_LOGITS OP: class dimension must be "
      "smaller than rank of logits, but got %i and %i correspondingly !",
      classesDim, logits->rankOf());

  std::vector<int> dimension = {classesDim};

  auto maxAlongDim =
      logits->reduceAlongDimension(reduce::Max, {classesDim}, true);
  auto logExp = (*logits - maxAlongDim).transform(transform::Exp);
  auto logSoftMax =
      (logExp / logExp.reduceAlongDimension(reduce::Sum, {classesDim}, true))
          .transform(transform::Log);

  (-(*labels) * logSoftMax)
      .reduceAlongDimension(reduce::Sum, *output, dimension);
...
DECLARE_SHAPE_FN(softmax_cross_entropy_loss_with_logits) {
  ...
  REQUIRE_TRUE(
      shape::shapeEquals(logitsShapeInfo, labelsShapeInfo), 0,
      "SOFTMAX_CROSS_ENTROPY_LOSS_WITH_LOGITS OP: labels and logits arrays "
      "must have the same shapes, but got %s and %s correspondingly!",
  ...
  auto reducedShapeInfo = ShapeUtils::evalReduceShapeInfo(
      shape::order(labelsShapeInfo), dimensions, labelsShapeInfo, outType,
      false, false, block.workspace());
...
CUSTOM_OP_IMPL(softmax_cross_entropy_loss_with_logits_grad, 2, 2, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      labels->isSameShape(logits), 0,
      "SOFTMAX_CROSS_ENTROPY_LOSS_WITH_LOGITS_GRAD OP: labels and logits "
      "arrays must have the same shapes, but got %s and %s correspondingly !",
      ShapeUtils::shapeAsString(labels).c_str(),
      ShapeUtils::shapeAsString(logits).c_str());
  REQUIRE_TRUE(
      classesDim < logits->rankOf(), 0,
      "SOFTMAX_CROSS_ENTROPY_LOSS_WITH_LOGITS_GRAD OP: class dimension must be "
      "smaller than rank of logits, but got %i and %i correspondingly !",
      classesDim, logits->rankOf());
  ...
  dLdp->assign(softmax *
                   labels->reduceAlongDimension(reduce::Sum, dimension, true) -
               *labels);
