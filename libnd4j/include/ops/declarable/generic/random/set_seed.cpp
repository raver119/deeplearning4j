...
CUSTOM_OP_IMPL(set_seed, -2, 1, false, 0, -2) {
  ...
  if (block.numI() > 0) {
    ...
    REQUIRE_TRUE(input->isScalar(), 0,
                 "SetSeed: Seed operand should be scalar");
    seed = input->e<Nd4jLong>(0);
  } else {
    REQUIRE_TRUE(false, 0,
                 "SetSeed: either IArg or scalr input should be provided");
  }

  // FIXME: this approach isn't really good for cuda, since it'll assume that
  // CUDA might get nullptr instead of stream
