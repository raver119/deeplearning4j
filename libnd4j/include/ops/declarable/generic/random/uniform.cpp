...
CUSTOM_OP_IMPL(randomuniform, 1, 1, true, 0, 0) {
  ...
  if (block.numI()) dtype = (DataType)INT_ARG(0);
  ...
  REQUIRE_TRUE(output->dataType() == dtype, 0,
               "RandomUniform: data type of output should be equals to given.");
...
DECLARE_SHAPE_FN(randomuniform) {
  ...
  auto dtype = DataType::FLOAT32;  // ArrayOptions::dataType(inputShape->at(1));
                                   // // output type is by given min

  if (block.numI()) dtype = (DataType)INT_ARG(0);
  if (block.width() > 1)
    REQUIRE_TRUE(dtype == INPUT_VARIABLE(1)->dataType(), 0,
                 "RandomUniform: data type of output and min/max args should "
                 "be the same");
