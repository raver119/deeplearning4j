...
CUSTOM_OP_IMPL(random_gamma, 2, 1, false, 0, 0) {
  ...
  if (block.width() > 2) {
    ...
    REQUIRE_TRUE(
        ShapeUtils::areShapesBroadcastable(*alpha, *beta), 0,
...
DECLARE_SHAPE_FN(random_gamma) {
  ...
  if (inputShape->size() > 2) {
    ...
    REQUIRE_TRUE(
        ShapeUtils::areShapesBroadcastable(alphaShape, rest), 0,
        "random_gamma: alpha and beta shapes should be broadcastable.");
    const Nd4jLong* additionalShapeBroadcasted = nullptr;
    ShapeUtils::evalBroadcastShapeInfo(
        alphaShape, rest, true, additionalShapeBroadcasted, block.workspace());
