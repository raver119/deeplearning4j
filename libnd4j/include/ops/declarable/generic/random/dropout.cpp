...
CONFIGURABLE_OP_IMPL(dropout, 1, 1, true, 1, 1) {
  ...
  REQUIRE_TRUE(probValue > 0.f && probValue <= 1.f, 0,
               "dropout: Probability should be with range 0 to 1.");
  ...
  return helpers::dropOutFunctor(block, input, output, reduceShape, seed,
                                 probValue);
}
...
CONFIGURABLE_OP_IMPL(dropout_bp, 2, 1, false, 1, 1) {
  ...
  REQUIRE_TRUE((probValue > 0. && probValue <= 1.), 0,
               "dropout_bp: Probability should be with range 0 to 1.");
  if (probValue == 1.0) {
  ...
  REQUIRE_TRUE(
      helpers::dropOutFunctorBP(block, input, gradOut, output, reduceShape,
                                seed, probValue) == ND4J_STATUS_OK,
...
CONFIGURABLE_OP_IMPL(alpha_dropout_bp, 2, 1, false, 4, 1) {
  ...
  return helpers::alphaDropOutFunctorBP(block, input, gradOut, output,
                                        reduceShape, seed, probValue,
                                        alphaValue, alpha1Value, betaValue);
}
