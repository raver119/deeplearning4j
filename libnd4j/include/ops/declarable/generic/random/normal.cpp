...
CUSTOM_OP_IMPL(random_normal, 1, 1, true, 2, 0) {
  ...
  /*
              REQUIRE_TRUE(rng != nullptr, 0, "RNG isn't defined for this Graph
     instance");
     ...
     execTransform<randomOps::GaussianDistribution<T>>(block.getRNG(),
     z->buffer(), z->shapeInfo(), z->buffer(), z->shapeInfo(), z->buffer(),
     z->shapeInfo(), block.getTArguments()->data());
  */

  RandomLauncher::fillGaussian(block.launchContext(), rng, OUTPUT_VARIABLE(0),
                               T_ARG(0), T_ARG(1));
