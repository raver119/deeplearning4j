 ...
 * and array with one scalar value of samples number, number of independent
 * samples to draw for each experiment 1,N. represents the unnormalized
 * log-probabilities for all classes. Int arguments: 0 - optional argument,
 * corresponds to dimension with batch_size Int arguments: 1 - optional
 * argument, integer type to use for the output. Default int64.
 */
...
CUSTOM_OP_IMPL(random_multinomial, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      !input->isEmpty(), 0,
      "RANDOM_MULTINOMIAL OP: Have to be provided at least one logits. ");

  REQUIRE_TRUE(
      inputSamples->lengthOf() == 1, 0,
  ...
  REQUIRE_TRUE(numOfSamples > 0, 0,
               "RANDOM_MULTINOMIAL OP: Number of samples should be greater "
               "then 0, got %i. ",
               numOfSamples);

  const int rank = input->rankOf();
  REQUIRE_TRUE(rank == 2, 0,
               "RANDOM_MULTINOMIAL OP: Logits should be a matrix with rank = "
               "2, but got instead rank = %i.",
               rank);

  const int argSize = block.numI();
  const int dimC = argSize > 0
                       ? (INT_ARG(0) >= 0 ? INT_ARG(0) : INT_ARG(0) + rank)
                       : rank - 1;
  ...
  helpers::fillRandomMultiNomial(block.launchContext(), rng, *input, *output,
                                 numOfSamples, dimC);
  return Status::OK();
}
