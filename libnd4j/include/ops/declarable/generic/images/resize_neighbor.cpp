...
CUSTOM_OP_IMPL(resize_nearest_neighbor, 1, 1, false, 0, -2) {
  ...
  if (block.width() > 1) {
    ...
    REQUIRE_TRUE(
        newImageSize->lengthOf() == 2, 0,
        "resize_nearest_neighbor: Resize params is a pair of values, not %i.",
        newImageSize->lengthOf());
    REQUIRE_TRUE(block.numI() <= 1, 0,
                 "resize_nearest_neighbor: Resize params already given by the "
                 "second param. Int params are expensive.");
    height = newImageSize->e<int>(0);
    width = newImageSize->e<int>(1);
  } else {
    REQUIRE_TRUE(block.numI() == 2, 0,
                 "resize_nearest_neighbor: Neither resize width nor height are "
                 "provided.");
    height = INT_ARG(0);
    width = INT_ARG(1);
  }
  ...
  REQUIRE_TRUE(
      width <= (1 << 24) || height <= (1 << 24), 0,
      "resize_nearest_neighbor: the image resize should be limited to 2^24 "
      "pixels both for height and width, but %d and %d were given.",
      height, width);
  REQUIRE_TRUE(inRank == 4 || inRank == 3, 0,
               "resize_nearest_neighbor: Input should be 4D tensor, but rank "
               "%i occured");
  REQUIRE_TRUE(inRank == output->rankOf(), 0,
               "resize_nearest_neighbor: Input and output ranks should be "
               "equals, but %i and %i occured.",
               inRank, output->rankOf());
  REQUIRE_TRUE(image->dataType() == output->dataType(), 0,
               "resize_nearest_neighbor: Input and output types should be the "
               "same, but `%s' occured instead.",
               DataTypeUtils::asString(output->dataType()).c_str());
  REQUIRE_TRUE(!halfPixelCenter || (halfPixelCenter && !alignCorners), 0,
               "resize_nearest_neighbor: `half_pixel_centers' should be false "
               "or true only when `align_corners' is false");
  REQUIRE_TRUE(((alignCorners && height > 2) || (height > 0)) &&
                   ((alignCorners && width > 1) || (width > 0)),
               0,
               "resize_nearest_neighbor: Wrong input or output size to resize "
               "(width = %d, height = %d)",
               width, height);
  ...
  return helpers::resizeNeighborFunctor(
      block.launchContext(), inRank == 4 ? image : &source, width, height,
