...
OP_IMPL(draw_bounding_boxes, 3, 1, true) {
  ...
  if (block.width() >
      2)  // TF v.1.x ommits color set for boxes, and use color 1.0 for fill up
  ...
  REQUIRE_TRUE(
      images->rankOf() == 4, 0,
      "draw_bounding_boxes: Images input should be 4D tensor, but %i occured.",
      images->rankOf());
  REQUIRE_TRUE(
      boxes->rankOf() == 3, 0,
      "draw_bounding_boxes: Boxes should be 3D tensor, but %i occured.",
      boxes->rankOf());
  if (colors) {
    REQUIRE_TRUE(
        colors->rankOf() == 2, 0,
        "draw_bounding_boxes: Color set should be 2D matrix, but %i occured.",
        colors->rankOf());
