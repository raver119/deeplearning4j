...
CUSTOM_OP_IMPL(resize_bilinear, 1, 1, false, 0, -2) {
  ...
  REQUIRE_TRUE(inRank == output->rankOf(), 0,
               "resize_bilinear: Input and output ranks should be equals, but "
               "%i and %i occured.",
  ...
  if (block.width() > 1) {
    ...
    REQUIRE_TRUE(newImageSize->lengthOf() == 2, 0,
                 "resize_bilinear: Resize params is a pair of values, not %i.",
                 newImageSize->lengthOf());
    REQUIRE_TRUE(block.numI() <= 1, 0,
                 "resize_bilinear: Resize params already given by the second "
    ...
    REQUIRE_TRUE(
        block.numI() > 1, 0,
        "resize_bilinear: Neither resize width nor height are provided.");
    height = INT_ARG(0);
    width = INT_ARG(1);
  }
  ...
  REQUIRE_TRUE(!halfPixelCenter || (halfPixelCenter && !alignCorners), 0,
               "resize_bilinear: `half_pixel_centers' should be false or true "
               "only when `align_corners' is false");

  return helpers::resizeBilinearFunctor(
