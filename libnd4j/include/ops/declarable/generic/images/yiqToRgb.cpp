...
CONFIGURABLE_OP_IMPL(yiq_to_rgb, 1, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(rank >= 1, 0,
               "YIQtoRGB: Fails to meet the rank requirement: %i >= 1 ", rank);
  if (arg_size > 0) {
    REQUIRE_TRUE(
        dimC >= 0 && dimC < rank, 0,
  ...
  REQUIRE_TRUE(
      input->sizeAt(dimC) == 3, 0,
      "YIQtoRGB: operation expects 3 channels (Y, I, Q), but got %i instead",
      input->sizeAt(dimC));
