...
CUSTOM_OP_IMPL(crop_and_resize, 4, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(output->dataType() == image->dataType(), 0,
               "crop_and_resize: Source images and output should have the same "
               "data type.");
  REQUIRE_TRUE(newImageSize->lengthOf() == 2, 0,
               "crop_and_resize: Resize params is a pair of values, not %i.",
               newImageSize->lengthOf());
  // REQUIRE_TRUE(block.numI() <= 1, 0, "crop_and_resize: Resize params already
  // given by the second param. Int params are expensive."); width =
  // int(newImageSize->getScalar(0)); height = int(newImageSize->getScalar(1));
  if (block.numI() == 1) {
    method = INT_ARG(0);
  }
  ...
  helpers::cropAndResizeFunctor(block.launchContext(), image, boxes, boxIndexes,
                                newImageSize, method, extrapolationVal, output);
...
DECLARE_SHAPE_FN(crop_and_resize) {
  ...
  REQUIRE_TRUE(newImageSize->lengthOf() == 2, 0,
               ...
               newImageSize->lengthOf());
  // REQUIRE_TRUE(block.numI() <= 1, 0, "crop_and_resize: Resize params already
  // given by the second param. Int params are expensive.");
