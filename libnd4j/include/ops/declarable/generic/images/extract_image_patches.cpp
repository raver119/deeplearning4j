...
CUSTOM_OP_IMPL(extract_image_patches, 1, 1, false, 0, 7) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "extract_image_patches: The rank of input array should be 4, "
               "but %i is given",
               input->rankOf());
  //
  if (output->isSameShape(input))
    ...
    output->nullify();
    helpers::extractPatches(block.launchContext(), input, output, ksizeRows,
                            ksizeCols, kstrideRows, kstrideCols, krateRows,
                            krateCols, isSame);
  }
  return Status::OK();
}
...
DECLARE_SHAPE_FN(extract_image_patches) {
  ...
  auto outputDepthDim = shape::sizeAt(in, 3) * INT_ARG(0) *
                        INT_ARG(1);  // last dim * ksizeRows * ksizeCols
