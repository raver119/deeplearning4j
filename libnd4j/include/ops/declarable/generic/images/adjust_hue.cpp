...
CONFIGURABLE_OP_IMPL(adjust_hue, 1, 1, true, 0, 0) {
  ...
  const int dimC = arg_size > 0
                       ? (INT_ARG(0) >= 0 ? INT_ARG(0) : INT_ARG(0) + rank)
                       : rank - 1;

  REQUIRE_TRUE(block.numT() > 0 || block.width() > 1, 0,
               "ADJUST_HUE: delta factor is required !");
  REQUIRE_TRUE(rank >= 3, 0,
               "ADJUST_HUE: op expects rank of input array to be >= 3, but got "
               "%i instead",
               rank);
  if (arg_size > 0) {
    REQUIRE_TRUE(
  ...
  REQUIRE_TRUE(input->sizeAt(dimC) == 3, 0,
               "ADJUST_HUE: operation expects image with 3 channels (R, G, B), "
               "but got %i instead",
               input->sizeAt(dimC));
  ...
  REQUIRE_TRUE(-1. <= delta->e<double>(0) && delta->e<double>(0) <= 1., 0,
               "ADJUST_HUE: parameter delta must be within [-1, 1] interval, "
               "but got %f instead",
               delta);
