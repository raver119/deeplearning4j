...
CONFIGURABLE_OP_IMPL(adjust_saturation, 1, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(rank >= 3, 0,
               "ADJUST_SATURATION: op expects rank of input array to be >= 3, "
               "but got %i instead",
               rank);
  if (arg_size > 0) {
    REQUIRE_TRUE(
  ...
  REQUIRE_TRUE(input->sizeAt(dimC) == 3, 0,
               "ADJUST_SATURATION: operation expects image with 3 channels (R, "
               "G, B), but got %i instead",
               input->sizeAt(dimC));
  REQUIRE_TRUE(block.numT() > 0 || block.width() > 1, 0,
               "ADJUST_SATURATION: scale factor is required !");
