...
CUSTOM_OP_IMPL(image_resize, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(size->lengthOf() == 2, 0,
               "resize_bilinear: Resize params is a pair of values, not %lld.",
               size->lengthOf());
  width = size->e<int>(0);
  height = size->e<int>(1);
  if (block.numB()) {
    preserveAspectRatio = B_ARG(0);
    if (block.numB() > 1) antialias = B_ARG(1);
  }
  ...
  return helpers::resizeFunctor(block.launchContext(), image, width, height,
                                method, preserveAspectRatio, antialias, output);
}
