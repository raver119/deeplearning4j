...
CUSTOM_OP_IMPL(knn_mindistance, 3, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(input->lengthOf() == lowest->lengthOf() &&
                   input->lengthOf() == highest->lengthOf(),
               0, "knn_mindistance: all input arrays must have same length");
  REQUIRE_TRUE(input->dataType() == lowest->dataType() &&
                   input->dataType() == highest->dataType() &&
