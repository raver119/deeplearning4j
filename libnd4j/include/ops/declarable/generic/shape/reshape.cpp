...
DECLARE_SHAPE_FN(reshape) {
  ...
  // if(x->isEmpty()) {
  //     xLen = 1;
  //     for (uint i = 0; i < x->rankOf(); ++i)                            //
  //     take into account possible empty shapes
  //         if(x->sizeAt(i) != 0)
  //             xLen *= x->sizeAt(i);
  // }
  ...
  //             REQUIRE_TRUE(reshapeArgs[j] != -1, 0, "Reshape : Only one
  //             unknown dimension (-1) is allowed."); if(reshapeArgs[j] != 0)
  //                 shapeLength *= reshapeArgs[j];
  //             else
  //                 ++numOfZeros;
  //         }
