...
CUSTOM_OP_IMPL(flatten, -1, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(
      xType == zType, 0,
      "Flatten: output array must have same data type as input arrays");
  std::vector<NDArray*> arrays(block.width());
  for (int e = 0; e < block.width(); e++) {
    auto input = INPUT_VARIABLE(e);

    REQUIRE_TRUE(xType == input->dataType(), 0,
                 "Flatten: all input arrays must have the same data type");
...
DECLARE_TYPES(flatten) {
  getOpDescriptor()->setAllowedInputTypes(
      {ALL_INTS, ALL_FLOATS, sd::DataType::BOOL});
  getOpDescriptor()->setAllowedOutputTypes(
      0, {ALL_FLOATS, ALL_INTS, sd::DataType::BOOL});
}
