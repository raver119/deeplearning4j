...
CUSTOM_OP_IMPL(expand_dims, 1, 1, false, 0, -2) {
  ...
  REQUIRE_TRUE(axis >= 0 && axis <= input->rankOf() + 1, 0,
               "ExpandDims: axis should be in range of 0...%i in this case, "
               "but got %i instead",
               input->rankOf() + 1, axis);
  ...
  if (input->ews() == 1 && output->ews() == 1 &&
      input->ordering() == output->ordering()) {
    output->dataBuffer()->copyBufferFrom(
        *input->dataBuffer().get(),
        output->lengthOf() * DataTypeUtils::sizeOfElement(output->dataType()),
        0, input->bufferOffset());
  } else {
