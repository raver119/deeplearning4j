...
CUSTOM_OP_IMPL(broadcast_to, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(shapeRank <= 1, 0,
               "BROADCAST_TO op: rank of shape array should be <= 1, bot got "
               "%i instead !",
               shapeRank);
  REQUIRE_TRUE(inputRank <= shapeLen, 0,
               "BROADCAST_TO op: rank of input shape array should be <= length "
               "of shape array, bot got %i and %i correspondingly !",
               inputRank, shapeLen);
  ...
  for (int i = 1; i <= inputRank; ++i)
    REQUIRE_TRUE(input->sizeAt(inputRank - i) == outShape[shapeLen - i] ||
                     input->sizeAt(inputRank - i) == 1,
                 0,
                 "BROADCAST_TO op: shape of input array %s can't be "
                 "broadcasted to the shape %s !",
...
DECLARE_TYPES(broadcast_to) {
  getOpDescriptor()->setAllowedInputTypes(DataType::ANY)->setSameMode(true);
}
...
DECLARE_SHAPE_FN(broadcast_to) {
  ...
  REQUIRE_TRUE(shapeRank <= 1, 0,
               "BROADCAST_TO op: rank of input shape array should be <= 1, bit "
               "got %i instead !",
               shapeRank);
  REQUIRE_TRUE(inputRank <= shapeLen, 0,
               "BROADCAST_TO op: rank of input shape array should be <= length "
  ...
  for (int i = 1; i <= inputRank; ++i)
    REQUIRE_TRUE(inputShapeInfo[inputRank + 1 - i] == outShape[shapeLen - i] ||
                     inputShapeInfo[inputRank + 1 - i] == 1,
                 0,
                 "BROADCAST_TO op: shape of input array %s can't be "
                 "broadcasted to the shape %s !",
