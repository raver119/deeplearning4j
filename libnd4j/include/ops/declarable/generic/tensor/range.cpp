...
CUSTOM_OP_IMPL(range, -2, 1, false, -2, -2) {
  ...
  if (numInArrs > 0) {
    ...
    REQUIRE_TRUE(false, 0,
                 "CUSTOM RANGE OP: op should have inputs defined in any "
                 "possible way: T_args, INT_args, or INPUT variables!");
  }
  ...
  if (numInArrs > 0) {
    ...
    if (isR) {
      ...
      REQUIRE_TRUE(delta != 0, 0,
                   "CUSTOM RANGE OP: delta should not be equal to zero !");
    ...
    if (math::nd4j_abs<Nd4jLong>(start + steps * delta) <
        math::nd4j_abs<Nd4jLong>(limit))
      ++steps;
  } else if (numTArgs > 0) {
    double start(0), limit, delta(1);
