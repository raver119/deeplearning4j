    ...
    this->shrink_axis_mask = 0;
    {
      ...
      for (int e = 0; e < sparse_spec.dims; e++) {
        if ((1 << e) & sparse_spec.ellipsis_mask) {
          int next_index = sd::math::nd4j_min<int>(
              this->dims - (sparse_spec.dims - e) + 1 +
...
bool _preprocess_strided_slice(
    std::vector<Nd4jLong>* indicesList, std::vector<Nd4jLong>* final_shape,
    std::vector<Nd4jLong>& input_shape, std::vector<int>& begin,
    std::vector<int>& end, std::vector<int>& strides, int begin_mask,
    int ellipsis_mask, int end_mask, int new_axis_mask, int shrink_axis_mask,
  ...
  StridedSliceDenseSpec dense_spec = {
      (int)input_shape.size(), 0, 0, false, false, begin, end, strides};
  ...
  for (int e = 0; e < (int)input_shape.size(); e++) {
    ...
    const std::array<int, 2> masks = {
        {dense_spec.begin_mask & (1 << e), dense_spec.end_mask & (1 << e)}};
    const std::array<int, 2> valid_range = {
        {stride_idx > 0 ? 0 : -1, stride_idx > 0 ? size_idx : size_idx - 1}};

    auto canonical = [stride_idx, e, size_idx, masks, valid_range](int x,
                                                                   int c) {
      if (masks[c]) {
    ...
    if (known_interval) {
      ...
      if (interval_length == 0 || ((interval_length < 0) != (stride_idx < 0))) {
        ...
        size_i = interval_length / stride_idx +
                 (interval_length % stride_idx != 0 ? 1 : 0);
      }
...
CUSTOM_OP_IMPL(strided_slice, 1, 1, false, 0, 5) {
  ...
  if (block.numI() > 5) {
    ...
    for (int e = 5; e < block.numI(); e++) args.emplace_back(INT_ARG(e));

    REQUIRE_TRUE(delta == 0, 0,
                 "StridedSlice: Number of Integer arguments should be equal to "
                 "input rank x 3 = %i, but got %i instead",
    ...
    REQUIRE_TRUE(v_begin->lengthOf() == v_end->lengthOf(), 0,
                 "StridedSlice: Length of begin/end should match, but got %i "
                 "vs %i instead",
                 (int)v_begin->lengthOf(), (int)v_end->lengthOf());
    REQUIRE_TRUE(
        (v_begin->rankOf() == 1) && (v_begin->rankOf() == v_end->rankOf()), 0,
        "StridedSlice: Rank of begin and ends should be 1, but %i given "
        "instead",
        (int)v_end->rankOf());
    ...
    if (block.width() > 3) {
      ...
      REQUIRE_TRUE(v_stride->lengthOf() == v_begin->lengthOf(), 0,
                   "StridedSlice: Length of begin/end/stride should match, but "
                   "got %i vs %i vs %i instead",
                   (int)v_begin->lengthOf(), (int)v_end->lengthOf(),
                   (int)v_stride->lengthOf());
      REQUIRE_TRUE((v_begin->rankOf() == v_stride->rankOf()), 0,
                   "StridedSlice: Rank of begin and ends should be %i, but %i "
                   "given instead",
                   (int)v_begin->rankOf(), v_stride->rankOf());
    ...
    REQUIRE_TRUE(false, 0,
                 "StridedSlice: Can't find begin/end/stride information "
                 "neither in IArguments or in input arrays");
  }
  ...
  if (shrink_axis_mask == 0)
    for (int dim = 0, b = 0, e = 0; dim < x->rankOf(); ++dim) {
      ...
      if (b < begin.size() && !ignoreBegin[b] && !addAxes[dim]) {
        int first =
            strides[b] > 0 ? begin[b] : math::nd4j_abs<int>(begin[b]) - 1;
        REQUIRE_TRUE(
            first <= x->sizeAt(dim), 0,
            "StridedSlice: begin index should be <= corresponding dimension of "
            "input array, but got end_index = %i for dimension %i!",
            begin[b], dim);
      }
      if (e < end.size() && !ignoreEnd[e] && !addAxes[dim]) {
        ...
        REQUIRE_TRUE(
            last <= x->sizeAt(dim), 0,
            "StridedSlice: end index should be <= corresponding dimension of "
            "input array, but got end_index = %i for dimension %i!",
            end[e], dim);
      }
      ++b;
  ...
  REQUIRE_TRUE(_preprocess_strided_slice(
                   &indices, &final_shape, input_shape, begin, end, strides,
                   begin_mask, ellipsis_mask, end_mask, new_axis_mask,
                   shrink_axis_mask, &is_identity, &is_simple_slice, &is_dim0),
               0, "StridedSlice: shape calculation failed");
  //            if(z->lengthOf() == 1 && !z->isEmpty() && (input_shape.size() ==
  //            2 && input_shape[0] == 1)) { //(indices.size() == 6) &&
  ...
  if (indices.size()) {
    ...
    ALLOCATE(subArrShapeInfo, block.workspace(),
             shape::shapeInfoLength(x->rankOf()), Nd4jLong);
    Nd4jLong offset;
...
DECLARE_SHAPE_FN(strided_slice) {
  ...
  REQUIRE_TRUE(begin.size() > 0 && end.size() > 0 && strides.size() > 0, 0,
               "Strided_Slice: empty arguments");
  ...
  std::vector<Nd4jLong> indices;
  bool result = _preprocess_strided_slice(
      &indices, &shape, input_shape, begin, end, strides, begin_mask,
...
CUSTOM_OP_IMPL(strided_slice_bp, 2, 1, false, 0, 5) {
  ...
  if (block.numI() > 5) {
    ...
    REQUIRE_TRUE(delta == 0, 0,
                 "StridedSliceBP: Number of Integer arguments should be equal "
                 "to input rank x 3 = %i, but got %i instead",
                 (x->rankOf() * 3), dim_values);
    ...
    REQUIRE_TRUE(v_begin->lengthOf() == v_end->lengthOf(), 0,
                 "StridedSliceBP: Length of begin/end should match, but got %i "
                 "vs %i instead",
                 (int)v_begin->lengthOf(), (int)v_end->lengthOf());
    ...
    if (block.width() >= 4) {
      ...
      REQUIRE_TRUE(v_stride->lengthOf() == v_begin->lengthOf(), 0,
                   "StridedSliceBP: Length of begin/end/stride should match, "
                   "but got %i vs %i vs %i instead",
                   (int)v_begin->lengthOf(), (int)v_end->lengthOf(),
                   (int)v_stride->lengthOf());
    ...
    REQUIRE_TRUE(false, 0,
                 "StridedSliceBP: Can't find begin/end/stride information "
                 "neither in IArguments or in input arrays");
  }
  ...
  // REQUIRE_TRUE(epsNext->isSameShape(final_shape), 0, "StridedSlice_bp:
  // gradOut shape should be equals to output from strided_slice op."); Zero
  // output array, so unused elements have 0 gradient
  output->nullify();
  //
  // the first case: only for scalar gradient step
  if (epsNext->lengthOf() == 1 &&
      (indices.size() == 3 && (indices[1] - indices[0]) == 1 ||
       (indices[2] - indices[0] == 1))) {
