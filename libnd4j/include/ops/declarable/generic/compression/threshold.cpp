...
CUSTOM_OP_IMPL(encode_threshold, 1, 2, true, 1, 0) {
  ...
  REQUIRE_TRUE(x->lengthOf() <= DataTypeUtils::max<int>(), 0,
               "encode_threshold: gradients array must have length <= MAX_INT");
  REQUIRE_TRUE(encoded->lengthOf() >= 4, 0,
               "encode_threshold: array for encoded updates can't have less "
               "than 4 elements");
  //            REQUIRE_TRUE(x->platformBuffer() == updated->platformBuffer(),
  //            0, "encode_threshold: gradients array must be the same at input
  //            and output");
  ...
  if (encoded->lengthOf() == 4) return Status::OK();
...
DECLARE_SHAPE_FN(encode_threshold) {
  ...
  auto elements = sd::math::nd4j_min<int>(
      helpers::thresholdEstimate(*x, threshold), boundary);
...
CUSTOM_OP_IMPL(decode_threshold, 2, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(encoded->lengthOf() >= 4, 0,
               "decode_threshold: encoded array can't have length < 4");
  REQUIRE_TRUE(updates->lengthOf() == encoded->e<int>(1), 0,
               "decode_threshold: updates array must have length equal to [%i]",
               encoded->e<int>(1));
  REQUIRE_TRUE(
      encoded->e<int>(3) == 0, 0,
      "decode_threshold: encoded array doesn't look like threshold-encoded");
