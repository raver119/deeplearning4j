...
LIST_OP_IMPL(scatter_list, 1, 1, 0, -2) {
  ...
  REQUIRE_TRUE(indices->isVector() || indices->rankOf() == 1, 0,
               "ScatterList: Indices for Scatter should be a vector")
  REQUIRE_TRUE(indices->lengthOf() == array->sizeAt(0), 0,
               "ScatterList: Indices length should be equal number of TADs "
               "along dim0, but got %i instead",
               indices->lengthOf());
  ...
  for (int e = 0; e < tads.size(); e++) {
    ...
    if (idx >= tads.size()) return ND4J_STATUS_BAD_ARGUMENTS;
  ...
  if (!hasList) setupResultList(*list, block);
