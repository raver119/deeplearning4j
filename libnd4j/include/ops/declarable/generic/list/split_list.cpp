...
LIST_OP_IMPL(split_list, 2, 1, 0, -2) {
  ...
  REQUIRE_TRUE(sizes->isZ(), 0,
               "split_list: sizes array must have one of integer types");
  ...
  for (Nd4jLong e = 0; e < sizes->lengthOf(); e++) {
    ...
    REQUIRE_TRUE(c_size > 0, 0,
                 "Slice size should have postive value, but got %i instead",
                 c_size);
    REQUIRE_TRUE(
        cnt < array->sizeAt(0) && cnt + c_size <= array->sizeAt(0), 0,
        "Slices size should NOT be higher then number of TADs of source array. "
        "Source size: [%i]; Slice start: [%i]; Slice size: [%i]",
