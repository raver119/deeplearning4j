...
LIST_OP_IMPL(gather_list, 2, 1, 0, -2) {
  ...
  REQUIRE_TRUE(indices->isVector() || indices->rankOf() == 1, 0,
               "Indices for Gather operation should be a vector");
  REQUIRE_TRUE(
      list->height() > 0, 0,
      "Number of elements in list should be positive prior to Gather call");
  REQUIRE_TRUE(list->height() == indices->lengthOf(), 1,
               "Number of indicies should be equal to number of elements in "
               "list, but got [%i] indices instead",
               indices->lengthOf());
