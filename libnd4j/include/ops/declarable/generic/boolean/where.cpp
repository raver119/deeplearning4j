...
CUSTOM_OP_IMPL(Where, 1, 1, false, 0, 0) {
  ...
  if (block.width() == 3) {
    ...
    if (condition->isSameShape(x)) {
      // FIXME: for perf it might be better to issue memcpy here, and fill only
      // mismatched values from either X or Y
      for (int e = 0; e < condition->lengthOf(); e++) {
        if (y->isR()) {
          ...
          auto r =
              !condition->e<bool>(e) ? y->e<Nd4jLong>(e) : x->e<Nd4jLong>(e);
          z->p(e, r);
        }
      }
    } else {
      REQUIRE_TRUE(condition->lengthOf() == x->sizeAt(0), 0,
                   "Condition length should be equal to the dim0 of x/y to act "
                   "as TAD-mask, but got %d instead",
                   condition->lengthOf());
    ...
    // in this case we return 2D matrix, which basically contains coordinates fo
    // true
    REQUIRE_TRUE(
        block.width() == 1, 0,
        "Where op takes either 1 or 3 operands, But got %d operands instead",
        block.width());
    auto output = OUTPUT_VARIABLE(0);

    int width = condition->rankOf();
    if (z->isEmpty()) return ND4J_STATUS_OK;

    std::vector<int> dims = ShapeUtils::evalDimsToExclude(width, {0});

    helpers::_where(block.launchContext(), *condition, *output,
                    block.workspace());
  }
  return Status::OK();
}
...
DECLARE_SHAPE_FN(Where) {
  if (block.width() == 3) {
    ...
    Nd4jLong numOfTrue = 0;  // condition->reduceNumber(reduce::CountNonZero,
                             // nullptr).e<Nd4jLong>(0);
