...
CUSTOM_OP_IMPL(choose, -1, 2, false, -2, -1) {
  ...
  if (block.width() > 1) {
    ...
    helpers::chooseFunctorArray(block.launchContext(), arg, comp, mode, result,
                                numResults);
    ...
    auto arg = INPUT_VARIABLE(0);
    helpers::chooseFunctorScalar(block.launchContext(), arg, scalar, mode,
                                 result, numResults);
  }
...
DECLARE_SHAPE_FN(choose) {
  ...
  if (block.width() > 1) {
    ...
    helpers::chooseFunctorArray(block.launchContext(), first, second, mode,
                                nullptr, &numResults);
  } else {
    auto first = INPUT_VARIABLE(0);
    shape = first->shapeInfo();
    ...
    helpers::chooseFunctorScalar(block.launchContext(), first, scalar, mode,
                                 nullptr, &numResults);
  }
