...
CUSTOM_OP_IMPL(compat_string_split, 2, 2, false, 0, 0) {
  ...
  for (auto e = 0L; e < input->lengthOf(); e++) {
    ...
    auto cnt = StringUtils::countSubarrays(s.c_str(), s.length(), d.c_str(),
                                           d.length()) +
               1;
  ...
  auto blen = StringUtils::byteLength(tmp) +
              ShapeUtils::stringBufferHeaderRequirements(strings.size());
...
DECLARE_SHAPE_FN(compat_string_split) {
  ...
  for (auto e = 0L; e < input->lengthOf(); e++) {
    ...
    // each substring we see in haystack, splits string in two parts. so we
    // should add 1 to the number of subarrays
    cnt += StringUtils::countSubarrays(s.c_str(), s.length(), d.c_str(),
                                       d.length()) +
           1;
  }

  // shape calculations
  // virtual tensor rank will be N+1, for N rank input array, where data will be
  // located at the biggest dimension values tensor is going to be vector always
