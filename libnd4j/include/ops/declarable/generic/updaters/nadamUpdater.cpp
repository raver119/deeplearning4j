...
CONFIGURABLE_OP_IMPL(nadam_updater, 3, 3, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      gradient->isSameShape(initStateV), 0,
  ...
  REQUIRE_TRUE(bParamsSupply, 0,
               "NADAM UPDATER OP: learning rate, beta 1, beta 2 and epsilon "
               "were not provided!");
  ...
  if (block.width() > 3) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "NADAM UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(
        beta1->isScalar(), 0,
        "NADAM UPDATER OP: beta 1 has to be a scalar, but instead got rank %i!",
        beta1->rankOf());
    REQUIRE_TRUE(
        beta2->isScalar(), 0,
        "NADAM UPDATER OP: beta 2 has to be a scalar, but instead got rank %i!",
        beta2->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "NADAM UPDATER OP: Epsilon has to be a scalar, but instead "
                 "got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterNadam(block.launchContext(), *gradient, *initStateV,
                        *initStateM, *update, *stateV, *stateM, dLr, dBeta1,
