...
CONFIGURABLE_OP_IMPL(ada_grad_updater, 2, 2, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      bParamsSupply, 0,
      "ADA_GRAD UPDATER OP: learning rate and epsilon were not provided!");
  ...
  if (block.width() > 2) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "ADA_GRAD UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "ADA_GRAD UPDATER OP: Epsilon has to be a scalar, but instead "
                 "got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterAdaGrad(block.launchContext(), *gradient, *initState, *update,
                          *stateH, dLr, dEpsilon);
  return Status::OK();
}
