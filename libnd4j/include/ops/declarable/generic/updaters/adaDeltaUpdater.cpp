...
CONFIGURABLE_OP_IMPL(ada_delta_updater, 3, 3, true, 0, 0) {
  ...
  if (gradient->isEmpty() || initStateMsg->isEmpty() ||
      initStateMsdx->isEmpty())
    return Status::OK();

  REQUIRE_TRUE(gradient->isSameShape(initStateMsg), 0,
               "ADA_DELTA UPDATER OP: input state Msg must have the same shape "
               "as gradient,"
  ...
  REQUIRE_TRUE(gradient->isSameShape(initStateMsdx), 0,
               "ADA_DELTA UPDATER OP: input state Msdx must have the same "
               "shape as gradient,"
               "  expected shape %s, but got %s!",
               ShapeUtils::shapeAsString(gradient->shapeInfo()).c_str(),
  ...
  REQUIRE_TRUE(bParamsSupply, 0,
               "ADA_DELTA UPDATER OP: Rho and epsilon were not provided!");
  ...
  if (block.width() > 3) {
    ...
    REQUIRE_TRUE(rho->isScalar(), 0,
                 "ADA_DELTA UPDATER OP: Rho has to be a scalar, but instead "
                 "got rank %i!",
                 rho->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "ADA_DELTA UPDATER OP: Epsilon has to be a scalar, but "
                 "instead got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterAdaDelta(block.launchContext(), *gradient, *initStateMsg,
                           *initStateMsdx, *update, *stateMsg, *stateMsdx, dRho,
                           dEpsilon);
  return Status::OK();
}
