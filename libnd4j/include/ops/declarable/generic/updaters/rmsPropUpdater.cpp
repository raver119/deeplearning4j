...
CONFIGURABLE_OP_IMPL(rms_prop_updater, 2, 2, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      gradient->isSameShape(initState), 0,
  ...
  REQUIRE_TRUE(bParamsSupply, 0,
               "RSM_PROB UPDATER OP: learning rate, rsm decay and epsilon were "
               "not provided!");
  ...
  if (block.width() > 2) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "RSM_PROB UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(rmsDecay->isScalar(), 0,
                 "RSM_PROB UPDATER OP: Rms decay has to be a scalar, but "
                 "instead got rank %i!",
                 rmsDecay->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "RSM_PROB UPDATER OP: Epsilon has to be a scalar, but instead "
                 "got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterRmsProp(block.launchContext(), *gradient, *initState, *update,
                          *stateG, dLr, dRmsDecay, dEpsilon);
  return Status::OK();
}
