...
CONFIGURABLE_OP_IMPL(nesterovs_updater, 2, 2, true, 0, 0) {
  ...
  REQUIRE_TRUE(gradient->isSameShape(initState), 0,
               "NESTEROVS UPDATER OP: input state Msg must have the same shape "
               "as gradient,"
  ...
  REQUIRE_TRUE(
      bParamsSupply, 0,
      "NESTEROVS UPDATER OP: learning rate and momentum were not provided!");
  ...
  if (block.width() > 2) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "NESTEROVS UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(momentum->isScalar(), 0,
                 "NESTEROVS UPDATER OP: Momentum has to be a scalar, but "
                 "instead got rank %i!",
                 momentum->rankOf());
  ...
  helpers::updaterNesterovs(block.launchContext(), *gradient, *initState,
                            *update, *stateV, dLr, dMomentum);
  return Status::OK();
}
