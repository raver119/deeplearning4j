...
CONFIGURABLE_OP_IMPL(ada_max_updater, 3, 3, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      gradient->isSameShape(initStateU), 0,
  ...
  REQUIRE_TRUE(
      gradient->isSameShape(initStateM), 0,
  ...
  REQUIRE_TRUE(bParamsSupply, 0,
               "ADA_MAX UPDATER OP: learning rate, beta 1, beta 2 and epsilon "
               "were not provided!");
  ...
  if (block.width() > 3) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "ADA_MAX UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(beta1->isScalar(), 0,
                 "ADA_MAX UPDATER OP: beta 1 has to be a scalar, but instead "
                 "got rank %i!",
                 beta1->rankOf());
    REQUIRE_TRUE(beta2->isScalar(), 0,
                 "ADA_MAX UPDATER OP: beta 2 has to be a scalar, but instead "
                 "got rank %i!",
                 beta2->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "ADA_MAX UPDATER OP: Epsilon has to be a scalar, but instead "
                 "got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterAdaMax(block.launchContext(), *gradient, *initStateU,
                         *initStateM, *update, *stateU, *stateM, dLr, dBeta1,
                         dBeta2, dEpsilon, iteration);
  return Status::OK();
}
