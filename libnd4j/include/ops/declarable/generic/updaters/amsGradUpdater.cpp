...
CONFIGURABLE_OP_IMPL(ams_grad_updater, 4, 4, true, 0, 0) {
  ...
  REQUIRE_TRUE(gradient->isSameShape(initStateV), 0,
               "AMSGRAD UPDATER OP: input state Msg must have the same shape "
               "as gradient,"
  ...
  REQUIRE_TRUE(gradient->isSameShape(initStateM), 0,
               "AMSGRAD UPDATER OP: input state Msdx must have the same shape "
               "as gradient,"
               "  expected shape %s, but got %s!",
               ShapeUtils::shapeAsString(gradient->shapeInfo()).c_str(),
  ...
  REQUIRE_TRUE(bParamsSupply, 0,
               "AMSGRAD UPDATER OP: learning rate, beta 1, beta 2 and epsilon "
               "were not provided!");
  ...
  if (block.width() > 4) {
    ...
    REQUIRE_TRUE(lr->isScalar(), 0,
                 "AMSGRAD UPDATER OP: Learning rate has to be a scalar, but "
                 "instead got rank %i!",
                 lr->rankOf());
    REQUIRE_TRUE(beta1->isScalar(), 0,
                 "AMSGRAD UPDATER OP: beta 1 has to be a scalar, but instead "
                 "got rank %i!",
                 beta1->rankOf());
    REQUIRE_TRUE(beta2->isScalar(), 0,
                 "AMSGRAD UPDATER OP: beta 2 has to be a scalar, but instead "
                 "got rank %i!",
                 beta2->rankOf());
    REQUIRE_TRUE(epsilon->isScalar(), 0,
                 "AMSGRAD UPDATER OP: Epsilon has to be a scalar, but instead "
                 "got rank %i!",
                 epsilon->rankOf());
  ...
  helpers::updaterAmsGrad(block.launchContext(), *gradient, *initStateV,
                          *initStateM, *initStateH, *update, *stateV, *stateM,
                          *stateH, dLr, dBeta1, dBeta2, dEpsilon, iteration);
  return Status::OK();
}
