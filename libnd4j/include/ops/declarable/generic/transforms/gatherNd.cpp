...
CUSTOM_OP_IMPL(gather_nd, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(rankInd > 0, 0,
               "GATHER_ND op: array of indexes can't be single scalar, the "
               "requirement is: rank > 0, but got rank = %i instead!",
               rankInd);
  int lastIndDim = indices->sizeAt(-1);
  REQUIRE_TRUE(lastIndDim <= rankIn, 0,
               "GATHER_ND op: the last dimension of indices array must be <= "
               "rank of input array but got %i and %i correspondingly!",
               lastIndDim, rankIn);

  if (checkIndices) {
    const Nd4jLong numOfBadIndx =
        helpers::checkIndices(block.launchContext(), *indices, *input);
    REQUIRE_TRUE(numOfBadIndx == 0, 0,
                 "GATHER_ND OP: please check elements of indices-array, total "
...
DECLARE_SHAPE_FN(gather_nd) {
  ...
  ALLOCATE(outShapeInfo, block.workspace(), shape::shapeInfoLength(outRank),
           Nd4jLong);
  ...
  // ArrayOptions::setDataType(outShapeInfo,
  // ArrayOptions::dataType(inShapeInfoIn));
