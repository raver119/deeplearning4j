...
OP_IMPL(scatter_upd, 3, 1, true) {
  ...
  if (inRank == 1) {
    REQUIRE_TRUE(indices->isSameShape(updates), 0,
                 "SCATTER_UPD OP: when input array has rank = 1 then indices "
    ...
    REQUIRE_TRUE(expectedUpdShape == updShape, 0,
                 "SCATTER_UPD OP: wrong shape of updates array, expected is "
                 "%s, but got %s instead !",
    ...
    REQUIRE_TRUE(updRank == indRank + inRank - 1, 0,
                 "SCATTER_UPD OP: wrong rank of updates array, expected is %i, "
                 "but got %i instead !",
  ...
  if (!indices->isEmpty()) {
    if (checkIndices) {
      ...
      REQUIRE_TRUE(numOfBadIndx == 0, 0,
                   "SCATTER_UPD OP: please check elements of indices-array, "
                   "total number of wrong elements is %lld!",
                   numOfBadIndx);
    }

    // ScatterHelper<T>::template scatterApply<simdOps::Copy<T>>(output,
    // indices, updates);
    helpers::scatter(block.launchContext(), pairwise::CopyPws, *indices,
                     *updates, *output, lock);
  }
