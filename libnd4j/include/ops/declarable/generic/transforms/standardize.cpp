...
CONFIGURABLE_OP_IMPL(standardize, 1, 1, true, 0, -2) {
  ...
  auto stdev = input->varianceAlongDimension(
      variance::SummaryStatsStandardDeviation, false, axis);
  stdev.reshapei(means.getShapeAsVector());

  input->applyTrueBroadcast(sd::BroadcastOpsTuple::Subtract(), means, *output,
                            false);
  output->applyTrueBroadcast(sd::BroadcastOpsTuple::Divide(), stdev, *output,
                             false);
...
CUSTOM_OP_IMPL(standardize_bp, 2, 1, false, 0, -2) {
  ...
  eps->applyTrueBroadcast(sd::BroadcastOpsTuple::Divide(), stdev, *output,
                          false);
  ...
  *output += dldx_u;
  ...
  means.applyTrueBroadcast(sd::BroadcastOpsTuple::Subtract(), *input, tmp,
                           false);
  ...
  stdevBp.execute(stdevBpArgs, stdevBpOutput, stdevBpTArgs, longAxis,
                  stdevBpBArgs);
  *output += dldx_s;
