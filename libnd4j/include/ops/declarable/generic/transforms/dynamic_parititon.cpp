...
CUSTOM_OP_IMPL(dynamic_partition, 2, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(input->rankOf() >= indices->rankOf(), 0,
               "dynamic_partition: data tensor rank should be non-lesser than "
               "indices\' tensor, but %i < %i given,",
               input->rankOf(), indices->rankOf());
  for (int dim = 0; dim < indices->rankOf(); dim++) {
    REQUIRE_TRUE(input->sizeAt(dim) == indices->sizeAt(dim), 0,
                 "dynamic_partition: dimensions should be equals for data and "
                 "indices tensors, but at axis[%i] %i != %i given",
  ...
  helpers::dynamicPartitionFunctor(block.launchContext(), input, indices,
                                   outputList);
...
DECLARE_SHAPE_FN(dynamic_partition) {
  ...
  for (int i = 0; i < numPartition; i++) {
    for (int e = 0; e < indices->lengthOf(); ++e)
      if (indices->e<Nd4jLong>(e) == i) partitionSizes[i]++;
  }
  ...
  for (int e = 0; e < numPartition; e++) {
    Nd4jLong *newShape;
    ALLOCATE(newShape, block.workspace(), shape::shapeInfoLength(outRank),
             Nd4jLong);
...
CUSTOM_OP_IMPL(dynamic_partition_bp, 3, 2, false, 0, 1) {
  ...
  auto originalIndices =
      indices
          ->dup();  //->ordering(), indices->shapeInfo(), indices->dataType());
  originalIndices.linspace(0);
  ops::dynamic_partition op;
  auto res = op.evaluate({&originalIndices, indices}, {numPartition});
  REQUIRE_TRUE(res.status() == ND4J_STATUS_OK, 0,
               "dynamic_partition_bp: Error with dynamic partitioning.");
  ...
  REQUIRE_TRUE(result.status() == ND4J_STATUS_OK, 0,
               "dynamic_partition_bp: Error with dynamic partitioning.");
  result.at(0).reshapei(outputList[0]->getShapeAsVector());
  outputList[1]->assign(indices);
  outputList[0]->assign(result.at(0));

  //        helpers::dynamicPartitionFunctorBP(block.launchContext(), input,
  //        indices, gradOutList, outputList);
