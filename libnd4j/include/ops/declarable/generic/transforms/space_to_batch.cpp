...
CUSTOM_OP_IMPL(space_to_batch, 2, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(blockSize >= 2, 0,
               "SpaceToBatch: integer parameter block_size must be >= 2, but "
               "got %i instead",
               blockSize);

  REQUIRE_TRUE(
      input->rankOf() == 4, 0,
      "SpaceToBatch: rank of input array must be equal 4, but got %i instead",
      input->rankOf());
  REQUIRE_TRUE(
      output->rankOf() == 4, 0,
      "SpaceToBatch: rank of output array must be equal 4, but got %i instead",
      output->rankOf());

  if (padding->sizeAt(0) != 2 || padding->sizeAt(1) != 2)
    REQUIRE_TRUE(false, 0,
                 "SpaceToBatch: operation expects padding shape to be {2, 2}, "
                 "but got %s instead",
  ...
  REQUIRE_TRUE((input->sizeAt(1) + padBottom + padTop) % blockSize == 0 &&
  ...
  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::spaceToBatch(block.launchContext(), *input, *output, padBottom,
                          padTop, padLeft, padRight, blockSize);
  else
    helpers::spaceToBatch(block.launchContext(), input->dup(), *output,
                          padBottom, padTop, padLeft, padRight, blockSize);
...
DECLARE_SHAPE_FN(space_to_batch) {
  ...
  REQUIRE_TRUE((inputShapeInfo[2] + padBottom + padTop) % blockSize == 0 &&
  ...
  return SHAPELIST(ConstantShapeHelper::getInstance()->createShapeInfo(
      ...
      {inputShapeInfo[1] * blockSize * blockSize,
       ...
       (inputShapeInfo[3] + padLeft + padRight) / blockSize,
       inputShapeInfo[4]}));
}
