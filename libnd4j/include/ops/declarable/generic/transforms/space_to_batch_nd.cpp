...
CUSTOM_OP_IMPL(space_to_batch_nd, 3, 1, false, 0, 0) {
  ...
  // (iH + padBottom + padTop)/blockSize[0], (iW + padLeft +
  // padRight)/blockSize[1], iC]
  ...
  REQUIRE_TRUE(blockShape->rankOf() == 1, 0,
               "SpaceToBatchND: rank of blockShape array must be equal to one, "
               "but got %i instead !",
               blockShape->rankOf());
  ...
  REQUIRE_TRUE(input->rankOf() == output->rankOf(), 0,
               "SpaceToBatchND: rank of input and output array must be the "
               "same, but got %i and %i correspondingly !",
               input->rankOf(), output->rankOf());

  if (padding->sizeAt(0) != numOfSpatialDims || padding->sizeAt(1) != 2) {
    const std::string expectedpaddingShape = "[" +
                                             std::to_string(numOfSpatialDims) +
                                             ", 2]";  // [numOfSpatialDims, 2]
    REQUIRE_TRUE(false, 0,
                 "SpaceToBatchND: operation expects padding shape to be %s, "
                 "but got %s instead",
                 expectedpaddingShape.c_str(),
  ...
  for (uint i = 0; i < numOfSpatialDims; ++i) {
    ...
    REQUIRE_TRUE((input->sizeAt(i + 1) + padLeft + padRight) % blockSize == 0,
                 0,
                 "SpaceToBatchND: after padding, spatial dimensions of input "
                 "array must be divisible by blockSize !");
  }

  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::spaceToBatchND(block.launchContext(), *input, *blockShape,
                            *padding, *output);
  else
    helpers::spaceToBatchND(block.launchContext(), input->dup(), *blockShape,
                            *padding, *output);
...
DECLARE_SHAPE_FN(space_to_batch_nd) {
  ...
  for (uint i = 0; i < numOfSpatialDims; ++i)
    outShape[i + 1] = (outShape[i + 1] + INPUT_VARIABLE(2)->e<uint>(i, 0) +
