...
CUSTOM_OP_IMPL(dynamic_stitch, 2, 1, false, 0, 0) {
  ...
  // run helper
  return helpers::dynamicStitchFunctor(block.launchContext(), inputs, indices,
                                       output);
}
...
DECLARE_SHAPE_FN(dynamic_stitch) {
  ...
  // check up inputs to avoid non-int indices and calculate max value from
  // indices to output shape length
  for (int i = 0; i < numOfData; i++) {
    auto input = INPUT_VARIABLE(i);
    REQUIRE_TRUE(
        input->isZ(), 0,
        "dynamic_stitch: Indices should be integer, but %d type given.",
        (int)input->dataType());
  ...
  int outRank = shape::rank(restShape) - shape::rank(firstShape) +
                1;  // at least 1D tensor
  std::vector<Nd4jLong> outShape(outRank);
  // fill up output shape template: the first to max index, and rests - to vals
  // from the first data input
  outShape[0] = maxValue + 1;
  for (int i = 1; i < outRank; ++i) outShape[i] = shape::sizeAt(restShape, i);

  return SHAPELIST(ConstantShapeHelper::getInstance()->createShapeInfo(
      ShapeDescriptor(ArrayOptions::dataType(restShape),
                      shape::order(firstShape), outShape)));
}
}  // namespace ops
}  // namespace sd
