...
CUSTOM_OP_IMPL(tear, 1, -1, false, 0, -1) {
  ...
  REQUIRE_TRUE(!block.getIArguments().empty(), 0,
  ...
  for (auto &v : dims)
    REQUIRE_TRUE(v >= 0 && v < input->rankOf(), 0,
                 "Tear dimensions should be non-negative values, and lower "
                 "then input rank. Got %i instead",
                 v);
...
DECLARE_SHAPE_FN(tear) {
  ...
  auto tadPack =
      sd::ConstantTadHelper::getInstance().tadForDimensions(inShape, dims);
  ...
  for (Nd4jLong e = 0; e < numTads; e++) {
    auto newShape = ConstantShapeHelper::getInstance().createShapeInfo(
        ArrayOptions::dataType(inShape), shape::order(inShape),
