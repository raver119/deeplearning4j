...
CUSTOM_OP_IMPL(slice, 1, 1, false, 0, -2) {
  ...
  if (block.width() == 3) {
    ...
    REQUIRE_TRUE(block.numI() >= x_rank * 2, 0,
                 "Number of IArgs should be equal to [%i] but got [%i] instead",
                 x_rank * 2, block.numI());
  ...
  REQUIRE_TRUE(begin.size() == x_rank, 0,
               "begin array should have length of [%i] but got [%i] instead",
               x_rank, begin.size());
  REQUIRE_TRUE(sz.size() == x_rank, 0,
               "size array should have length of [%i] but got [%i] instead",
               x_rank, sz.size());
  ...
  for (int e = 0; e < x_rank; e++) {
    ...
    REQUIRE_TRUE(start <= input->sizeAt(e), 0,
                 "Index %i is invalid for dimension %i with size %i.", start, e,
                 input->shapeInfo()[e + 1]);
    if (size == -1) {
      size = input->sizeAt(e) - start;
    }
    REQUIRE_TRUE(size >= 0, 0,
                 "Slice: interval for dimension %i is less then 1");
    REQUIRE_TRUE(start + size <= input->sizeAt(e), 0,
                 "Slice: interval [%i, %i] is out of bounds for dimension %i "
                 "with size %i",
                 start, start + size, e, input->sizeAt(e));
  ...
  if (empty) {
    REQUIRE_TRUE(
        output->isEmpty(), 0,
        "Slice: empty array indices requested, but output array is not empty");
    return Status::OK();
  }

  Nd4jLong *subArrShapeInfo = nullptr;
  ALLOCATE(subArrShapeInfo, block.workspace(),
           shape::shapeInfoLength(input->rankOf()), Nd4jLong);

  Nd4jLong offset;

  shape::calcSubArrShapeInfoAndOffset(indices.data(), input->shapeInfo(),
...
DECLARE_SHAPE_FN(slice) {
  ...
  for (int e = 0; e < x_rank; e++) {
    ...
    REQUIRE_TRUE(start >= 0 && start <= inShape[e + 1], 0,
                 "Invalid begin[%i] value: Begin must satisfy 0 <= begin <= "
                 "size[i], got begin=%i for dimension size %i",
                 e, start, inShape[e + 1]);
    REQUIRE_TRUE(size == -1 || size >= 0, 0,
                 "Invalid size[%i] value: must be positive (or -1 for 'all "
                 "remaining'), got %i",
                 e, size, inShape[e + 1]);
    REQUIRE_TRUE(start >= 0 && start <= inShape[e + 1], 0,
                 "Invalid begin[%i] value: Begin must satisfy 0 <= begin <= "
    ...
    REQUIRE_TRUE(start + size <= inShape[e + 1], 0,
                 "Slice: interval [%i, %i] is out of bounds for dimension %i "
                 "with size %i",
                 start, start + size, e, inShape[e + 1]);
...
CUSTOM_OP_IMPL(slice_bp, 2, 1, false, 0, -2) {
  ...
  REQUIRE_TRUE(begin.size() == x_rank, 0,
               "begin array should have length of [%i] but got [%i] instead",
               x_rank, begin.size());
  REQUIRE_TRUE(end.size() == x_rank, 0,
               "end array should have length of [%i] but got [%i] instead",
               x_rank, end.size());
