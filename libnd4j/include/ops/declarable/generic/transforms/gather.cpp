...
CUSTOM_OP_IMPL(gather, 1, 1, false, 0, -2) {
  ...
  if (indices != nullptr && indices->isEmpty()) {
    REQUIRE_TRUE(output->isEmpty(), 0,
                 "Gather op: If indices are empty, output must also be empty");
  ...
  REQUIRE_TRUE(intArgs[0] < inputRank, 0,
               "GATHER op: input axis must be smaller than input array rank, "
               "but got %i and %i correspondingly!",
               intArgs[0], inputRank);
  REQUIRE_TRUE(indices != nullptr || numOfIntArgs > 1, 0,
               "GATHER op: indices should be provided either as additional "
               "input array or as IntArguments !");
    ...
    if (indices == nullptr)
      pIndices =
          new NDArray(input->ordering(), {static_cast<int>(intArgs.size()) - 1},
                      ...
                      DataType::INT64, block.launchContext());
    const Nd4jLong numOfBadIndx = helpers::checkIndices(
        block.launchContext(), *pIndices, *input, intArgs[0]);
    REQUIRE_TRUE(numOfBadIndx == 0, 0,
                 "GATHER OP: please check elements of indices-array, total "
...
DECLARE_SHAPE_FN(gather) {
  ...
  if (block.width() > 1) {
    ...
    ALLOCATE(outputShapeInfo, block.workspace(),
             shape::shapeInfoLength(outputRank), Nd4jLong);
  ...
  auto result = ConstantShapeHelper::getInstance().createShapeInfo(
      ShapeDescriptor(outputShapeInfo));
