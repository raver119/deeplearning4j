...
CUSTOM_OP_IMPL(batch_to_space_nd, 3, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(blockShape->rankOf() == 1, 0,
               "BatchToSpaceND: rank of blockShape array must be equal to one, "
               "but got %i instead !",
               blockShape->rankOf());
  ...
  const auto product =
      blockShape->reduceNumber(sd::reduce::Prod).e<Nd4jLong>(0);
  REQUIRE_TRUE(input->sizeAt(0) % product == 0, 0,
               "BatchToSpaceND: first dimension of input array must be "
               "divisible by product of blockShape array elements (= %lld), "
               "but got first dimension equal to %i",
               product, input->sizeAt(0));

  if (crop->sizeAt(0) != numOfSpatialDims || crop->sizeAt(1) != 2) {
    const std::string expectedCropShape = "[" +
                                          std::to_string(numOfSpatialDims) +
                                          ", 2]";  // [numOfSpatialDims, 2]
    REQUIRE_TRUE(false, 0,
                 "BatchToSpaceND: operation expects padding shape to be %s, "
                 "but got %s instead",
                 expectedCropShape.c_str(),
  ...
  for (uint i = 0; i < numOfSpatialDims; ++i) {
    ...
    const auto outSpatialDim =
        input->sizeAt(i + 1) * blockShape->e<Nd4jLong>(i) - cropLeft -
        cropRight;
    REQUIRE_TRUE(outSpatialDim >= 0, 0,
                 "BatchToSpaceND: crop left/right values are too big and cause "
                 "negative output spatial dimension/dimensions !");
  }

  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::batchToSpaceND(block.launchContext(), *input, *blockShape, *crop,
                            *output);
  else
    helpers::batchToSpaceND(block.launchContext(), input->dup(), *blockShape,
                            *crop, *output);
...
DECLARE_SHAPE_FN(batch_to_space_nd) {
  ...
  const auto product =
      INPUT_VARIABLE(1)->reduceNumber(sd::reduce::Prod).e<Nd4jLong>(0);
  ...
  std::vector<Nd4jLong> outShape(inputShapeInfo + 1,
                                 inputShapeInfo + 1 + inputShapeInfo[0]);

  outShape[0] /= product;

  for (uint i = 0; i < numOfSpatialDims; ++i)
    outShape[i + 1] = outShape[i + 1] * INPUT_VARIABLE(1)->e<Nd4jLong>(i) -
