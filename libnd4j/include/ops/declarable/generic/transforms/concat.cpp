...
CUSTOM_OP_IMPL(concat, -1, 1, false, 0, 0) {
  REQUIRE_TRUE(block.width() > 0, 0,
               "CONCAT op: No input arrays were provided");
  ...
  auto typeOfFirstArr =
      block.width() > 0 ? INPUT_VARIABLE(0)->dataType() : DataType::FLOAT32;

  for (int i = 0; i < numOfInArrs; ++i) {
    auto input = INPUT_VARIABLE(i);
    auto currentRank = input->rankOf();

    // TODO: follow two lines are in accordance to current tf.concat spec.
    // Commented for compatibility with legacy
    //        REQUIRE_TRUE(currentRank > 0, 0, "Rank of input variable %i must
    //        be greater 0, but is %lld instead.", i, currentRank);
    //        REQUIRE_TRUE(rankOfFirstArr == currentRank, 0, "Number of
    //        dimensions in concat should be equals, but for %i input variable
    //        %lld != %lld appears.", i, currentRank, rankOfFirstArr);
    if (!input->isEmpty()) {
      allOfSameType &= (typeOfFirstArr == input->dataType());

      if (input->rankOf() == 0) {
        auto vec =
            new NDArray('c', {1}, input->dataType(), block.launchContext());
        vec->assign(input);
  ...
  if (numOfNonEmptyArrs == 0) {
    // All inputs are empty arrays -> return empty, mainly for TF import
    // compatibility (no op)
    REQUIRE_TRUE(
        OUTPUT_VARIABLE(0)->isEmpty(), 0,
        "CONCAT op: If all input variables are empty, output must be empty");
    return Status::OK();
  }

  const int rank =
      nonEmptyArrs[0]->rankOf();  //  look up to first non-empty array
  int axis = isAxisInLastArr ? INPUT_VARIABLE(block.width() - 1)->e<int>(0)
                             : INT_ARG(0);
  if (axis < 0) {
    axis += rank;
  }
  ...
  for (int i = 1; i < numOfNonEmptyArrs; ++i)
    REQUIRE_TRUE(nonEmptyArrs[i]->rankOf() == rank, 0,
  ...
  for (int i = 1; i < numOfNonEmptyArrs; ++i) {
    for (int dim = 0; dim < rank; ++dim)
      if (dim != axis)
        REQUIRE_TRUE(
            nonEmptyArrs[i]->sizeAt(dim) == nonEmptyArrs[0]->sizeAt(dim), 0,
            "CONCAT op: all input arrays must have the same dimensions (except "
            "those on input axis) !");
  }
...
DECLARE_SHAPE_FN(concat) {
  ...
  for (int i = 1; i < numOfNonEmptyArrs; ++i)
    REQUIRE_TRUE(shape::rank(arrShapes.at(i)) == rank, 0,
                 "CONCAT op: all input arrays must have the same rank !");
    ...
    for (int dim = 0; dim < rank; ++dim)
      if (dim != axis)
        REQUIRE_TRUE(arrShapes.at(i)[dim + 1] == arrShapes.at(0)[dim + 1], 0,
                     "CONCAT op: all input arrays must have the same "
                     "dimensions (except those on input axis) !");
  }
  // ******** end of input validation ******** //
  ...
  if (numOfNonEmptyArrs == 1) {
    ShapeUtils::updateStridesAndType(outShapeInfo, arrShapes.at(0),
                                     shape::order(arrShapes.at(0)));
...
//     REQUIRE_TRUE(first != nullptr, 0, "Concat: at least 1 non-empty input
//     required!");

//     // it's possible to get into situation when your input has only 1 input.
//     That's just assign if (elements == 1) {
//         output->assign(first);
//         return Status::OK();
//     }
...
//     sd::SpecialMethods<T>::concatCpuGeneric(_dimension, elements, buffers,
//     shapes, output->buffer(), output->shapeInfo());
...
//         // all scalars
//         if (allScalars) {
//             ALLOCATE(newShape, block.workspace(), shape::shapeInfoLength(1),
//             Nd4jLong);
...
//         if (hasScalars) {
//             ALLOCATE(newShape, block.workspace(), shape::shapeInfoLength(1),
//             Nd4jLong); Nd4jLong length = shape::length(inp); for (int i = 1;
//             i < block.width(); i++) {
...
//     ALLOCATE(newShape, block.workspace(),
//     shape::shapeInfoLength(first->shapeInfo()), Nd4jLong);

//     if (_dimension < 0)
//         _dimension += first->rankOf();

//     std::memcpy(newShape, first->shapeInfo(),
//     shape::shapeInfoByteLength(first->shapeInfo())); for (int i = 0; i <
//     inputShape->size(); i++) {
//         auto s = INPUT_VARIABLE(i);

//         // FIXME: s == first is bad, but fast. alternatively we can subtract
//         first size out of result if (s->isEmpty() || s == first)
//             continue;

//         newShape[_dimension + 1] +=
//         shape::shapeOf(inputShape->at(i))[_dimension];
//     }
...
CUSTOM_OP_IMPL(concat_bp, -1, -1, false, 0, 0) {
  ...
  const int axis =
      isAxisInLastArr
          ? INPUT_VARIABLE(block.width() - 1)->e<int>(0)
          : (INT_ARG(0) >= 0 ? INT_ARG(0)
                             : INT_ARG(0) + INPUT_VARIABLE(0)->rankOf());
