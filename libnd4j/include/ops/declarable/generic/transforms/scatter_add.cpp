...
OP_IMPL(scatter_add, 3, 1, true) {
  ...
  if (inRank == 1) {
    REQUIRE_TRUE(indices->isSameShape(updates), 0,
                 "SCATTER_ADD OP: when input array has rank = 1 then indices "
                 "and updates must have the same shapes, but got %s and %s "
                 "correspondingly !",
                 ShapeUtils::shapeAsString(indices).c_str(),
                 ShapeUtils::shapeAsString(updates).c_str());
    ...
    expectedUpdShape.insert(expectedUpdShape.end(), inShape.begin() + 1,
                            inShape.end());

    REQUIRE_TRUE(expectedUpdShape == updShape, 0,
                 "SCATTER_ADD OP: wrong shape of updates array, expected is "
                 "%s, but got %s instead !",
    ...
    REQUIRE_TRUE(updRank == indRank + inRank - 1, 0,
                 "SCATTER_ADD OP: wrong rank of updates array, expected is %i, "
                 "but got %i instead !",
    ...
    expectedUpdShape.insert(expectedUpdShape.end(),
                            inShape.begin() + Nd4jLong(1L), inShape.end());
  ...
  if (!indices->isEmpty()) {
    if (checkIndices) {
      const Nd4jLong numOfBadIndx =
          helpers::checkIndices(block.launchContext(), *indices, *output, 0);
      REQUIRE_TRUE(numOfBadIndx == 0, 0,
                   "SCATTER_ADD OP: please check elements of indices-array, "
                   "total number of wrong elements is %lld!",
                   numOfBadIndx);
    }

    helpers::scatter(block.launchContext(), pairwise::Add, *indices, *updates,
                     *output, lock);
  }
