...
CUSTOM_OP_IMPL(batch_to_space, 2, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(blockSize >= 2, 0,
               "BatchToSpace: integer parameter block_size must be >= 2, but "
               "got %i instead",
               blockSize);
  ...
  REQUIRE_TRUE(
      rank == 4, 0,
      "BatchToSpace: rank of input array must be equal 4, but got %i instead",
      rank);
  REQUIRE_TRUE(dim0 % (blockSize * blockSize) == 0, 0,
               "BatchToSpace: first dimension of input array must be divisible "
               "by blockSize * blockSize (that is by %i), but got first "
               "dimension equal to %i",
               blockSize * blockSize, dim0);

  if (crop->sizeAt(0) != 2 || crop->sizeAt(1) != 2)
    REQUIRE_TRUE(false, 0,
                 "BatchToSpace: operation expects crop shape to be {2, 2}, but "
                 "got %s instead",
  ...
  const int oH =
      input->sizeAt(1) * blockSize - cropBottom - cropTop;  // top and bottom
  const int oW =
      input->sizeAt(2) * blockSize - cropLeft - cropRight;  // left and right
  REQUIRE_TRUE(oH >= 0, 0,
               "BatchToSpace: crop top/bottom values are too big and cause "
               "negative output height dimension !");
  REQUIRE_TRUE(oW >= 0, 0,
               "BatchToSpace: crop left/right values are too big and cause "
               "negative output width dimension !");

  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::batchToSpace(block.launchContext(), *input, *output, cropBottom,
                          cropTop, cropLeft, cropRight, blockSize);
  else
    helpers::batchToSpace(block.launchContext(), input->dup(), *output,
                          cropBottom, cropTop, cropLeft, cropRight, blockSize);
...
DECLARE_SHAPE_FN(batch_to_space) {
  ...
  const int oH =
      inputShapeInfo[2] * blockSize - cropTop - cropBottom;  // top and bottom
  const int oW =
      inputShapeInfo[3] * blockSize - cropLeft - cropRight;  // left and right
  REQUIRE_TRUE(oH >= 0, 0,
               "BatchToSpace: crop top/bottom values are too big and cause "
               "negative output height dimension !");
  REQUIRE_TRUE(oW >= 0, 0,
