...
CUSTOM_OP_IMPL(depth_to_space, 1, 1, false, 0, 2) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "DepthToSpace: input should be 4D array, but got %f instead",
               input->rankOf());
  ...
  REQUIRE_TRUE(iD % (block_size * block_size) == 0, 0,
               "DepthToSpace: input number of channels should be divisible by "
               "square(block_size)");

  auto output = OUTPUT_VARIABLE(0);

  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::_depthToSpace(block.launchContext(), *input, output, block_size,
                           isNHWC);
  else
    helpers::_depthToSpace(block.launchContext(), input->dup(), output,
                           block_size, isNHWC);
