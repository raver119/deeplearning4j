...
CUSTOM_OP_IMPL(unstack, 1, -1, false, 0, 1) {
  ...
  REQUIRE_TRUE(dim < input->rankOf(), 0,
               "Unstack dimension should be lower then rank of input %i, but "
               "got dimension=%i !",
               input->rankOf(), dim);
  REQUIRE_TRUE(dim >= 0, 0,
               "Unstack dimension should be non-negative value, but got %i !",
               dim);

  if (input->isEmpty()) return Status::OK();

  std::vector<NDArray*> outArrs(input->sizeAt(dim));
  for (uint i = 0; i < outArrs.size(); ++i) outArrs[i] = OUTPUT_VARIABLE(i);
...
DECLARE_SHAPE_FN(unstack) {
  ...
  REQUIRE_TRUE(dim < inShapeInfo[0], 0,
               "UNSTACK op: dimension should be lower then rank of input %i, "
               "but got dimension=%i !",
               inShapeInfo[0], dim);
  REQUIRE_TRUE(
      dim >= 0, 0,
      "UNSTACK op: dimension should be non-negative value, but got %i !", dim);
  ...
  if (dims.size() == 0 &&
      shape::rank(inShapeInfo) == 1) {  // split vector into lenthOf scalars
