...
OP_IMPL(scatter_mul, 3, 1, true) {
  ...
  if (inRank == 1) {
    REQUIRE_TRUE(indices->isSameShape(updates), 0,
                 "SCATTER_MUL OP: when input array has rank = 1 then indices "
    ...
    REQUIRE_TRUE(expectedUpdShape == updShape, 0,
                 "SCATTER_MUL OP: wrong shape of updates array, expected is "
                 "%s, but got %s instead !",
    ...
    REQUIRE_TRUE(updRank == indRank + inRank - 1, 0,
                 "SCATTER_MUL OP: wrong rank of updates array, expected is %i, "
                 "but got %i instead !",
  ...
  if (!indices->isEmpty()) {
    if (checkIndices) {
      ...
      REQUIRE_TRUE(numOfBadIndx == 0, 0,
                   "SCATTER_MUL OP: please check elements of indices-array, "
                   "total number of wrong elements is %lld!",
                   numOfBadIndx);
    }

    helpers::scatter(block.launchContext(), pairwise::Multiply, *indices,
                     *updates, *output, lock);
  }
