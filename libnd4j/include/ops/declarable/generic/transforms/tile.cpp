...
CUSTOM_OP_IMPL(tile, 1, 1, false, 0, -2) {
  ...
  if (block.numI() == inRank) {
    ...
    REQUIRE_TRUE(reps_vector->lengthOf() == inRank, 0,
                 "TILE op: repeats vector length should be equal to input "
                 "rank, but got %i and %i correspondingly !",
    ...
    REQUIRE_TRUE(
        false, 0,
        "TILE op: this op requires repeats vector, either as IArgs or second "
        "array with length equal to rank of input array to be tiled !");
  }
...
CUSTOM_OP_IMPL(tile_bp, 2, 1, false, 0, -2) {
  ...
  if (block.numI() == inRank) {
    ...
    auto reps_vector = INPUT_VARIABLE(1);
    REQUIRE_TRUE(reps_vector->lengthOf() == inRank, 0,
                 "TILE_BP op: repeats vector length should be equal to input "
                 "rank, but got %i and %i correspondingly !",
                 reps_vector->lengthOf(), inRank);
    ...
    REQUIRE_TRUE(
        false, 0,
        "TILE_BP op: this op requires repeats vector, either as IArgs or "
        "second array with length equal to rank of input array to be tiled !");
  }

  REQUIRE_TRUE(
      inRank == gradO->rankOf(), 0,
      "TILE_BP op: the ranks of input array and output's gradients array (next "
      "epsilon) must be equal, but got %i and %i correspondingly !",
      inRank, gradO->rankOf());

  for (int i = 0; i < inRank; ++i)
    REQUIRE_TRUE(gradO->sizeAt(i) == gradI->sizeAt(i) * reps[i], 0,
                 "TILE_BP op: shapes of input array and output's gradients "
                 "array (next epsilon) are inconsistent !");
...
DECLARE_SHAPE_FN(tile_bp) {
  ...
  for (int i = 0; i < inRank; ++i)
    REQUIRE_TRUE(
        shape::sizeAt(gradOShape, i) == shape::sizeAt(inShape, i) * reps[i], 0,
        "TILE_BP op: shapes of input array and output's gradients array (next "
        "epsilon) are inconsistent !");
