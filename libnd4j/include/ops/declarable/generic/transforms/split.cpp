...
CUSTOM_OP_IMPL(split, 1, -1, false, 0, 1) {
  ...
  // Edge case: splitting empty array (mainly for TF import compatibility) ->
  // return N empty arrays
  if (input->isEmpty()) {
    for (int i = 0; i < num_splits; i++) {
      REQUIRE_TRUE(
          OUTPUT_VARIABLE(i)->isEmpty(), 0,
  ...
  REQUIRE_TRUE(input->sizeAt(axis) % num_splits == 0, 0,
               "Split: num_splits has wrong value, remainder of division "
