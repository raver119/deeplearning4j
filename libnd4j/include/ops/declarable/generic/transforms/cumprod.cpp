...
CONFIGURABLE_OP_IMPL(cumprod, 1, 1, true, 0, 2) {
  ...
  if (block.numI() == 2 && block.width() == 1) {
    // all at once case
    sd::ops::helpers::prefix(block.launchContext(), scalar::Multiply, input,
                             output, exclusive, reverse);
  } else {
...
DECLARE_TYPES(cumprod_bp) {
  getOpDescriptor()
      ...
      ->setAllowedInputTypes(
          1,
          {ALL_INTS, ALL_FLOATS})  // there is a case when axes given as IArgs
...
CUSTOM_OP_IMPL(cumprod_bp, 2, 1, false, 0, 2) {
  ...
  if (!exclusive && !reverse) {
    if (dims.size())
      sd::ops::helpers::prefix(block.launchContext(), scalar::Add, &val, output,
                               dims, true, false);
    else
      sd::ops::helpers::prefix(block.launchContext(), scalar::Add, &val, output,
                               false, true);
