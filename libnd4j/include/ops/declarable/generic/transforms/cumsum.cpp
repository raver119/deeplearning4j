...
CONFIGURABLE_OP_IMPL(cumsum, 1, 1, true, 0, 2) {
  ...
  REQUIRE_TRUE(input->dataType() == output->dataType(), 0,
               "CumSum: input and output data types must be equal");
  ...
  if (block.numI() == 2 && block.width() == 1) {
    // all at once case
    sd::ops::helpers::prefix(block.launchContext(), scalar::Add, input, output,
                             exclusive, reverse);
  } else {
...
CUSTOM_OP_IMPL(cumsum_bp, 2, -1, true, 0, 2) {
  ...
  if (!exclusive && !reverse) {
    if (dims.size())
      sd::ops::helpers::prefix(block.launchContext(), scalar::Add, gradOut,
                               output, dims, false, true);
    else
      sd::ops::helpers::prefix(block.launchContext(), scalar::Add, gradOut,
                               output, false, true);
...
DECLARE_TYPES(cumsum_bp) {
  ...
  getOpDescriptor()->setAllowedInputTypes(
      1, {ALL_FLOATS, ALL_INTS});  // axes can be set as the second param
