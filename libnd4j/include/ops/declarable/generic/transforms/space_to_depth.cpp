...
CUSTOM_OP_IMPL(space_to_depth, 1, 1, false, 0, 2) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "SpaceToDepth: input should be 4D array, but got %f instead",
               input->rankOf());
  ...
  REQUIRE_TRUE(
      iH % block_size == 0 && iW % block_size == 0, 0,
      "SpaceToDepth: input Height & Width should be divisible by block_size");

  auto output = OUTPUT_VARIABLE(0);

  if (shape::strideDescendingCAscendingF(input->shapeInfo()))
    helpers::_spaceTodepth(block.launchContext(), *input, output, block_size,
                           isNHWC);
  else
    helpers::_spaceTodepth(block.launchContext(), input->dup(), output,
                           block_size, isNHWC);
