...
//////////////////////////////////////////////////////////////////////////
// here iArgs is int vector of repeats at the beginning and last element in
// iArgs is dimension
CUSTOM_OP_IMPL(repeat, 1, 1, true, 0, -1) {
  ...
  const int axis =
      repeats.back() < 0 ? repeats.back() + input->rankOf() : repeats.back();

  repeats.pop_back();

  REQUIRE_TRUE(0 <= axis && axis < input->rankOf(), 0,
               "CUSTOM REPEAT OP: wrong axis argument it should be less then "
               "input array rank %i, but got %i instead !",
               input->rankOf(), axis);

  REQUIRE_TRUE(repeats.size() == 1 || repeats.size() == input->sizeAt(axis), 0,
               "CUSTOM REPEAT OP: wrong axis argument, size of repeats vector "
               "must be 1 or equal to dimension at given axis, but got "
               "repeats.size = %i and axis = %i !",
               repeats.size(), axis);
...
DECLARE_SHAPE_FN(repeat) {
  ...
  auto repeats = block.getIArguments();
