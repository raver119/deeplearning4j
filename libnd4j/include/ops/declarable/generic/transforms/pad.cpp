...
CUSTOM_OP_IMPL(pad, 2, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(expectedPaddingsShape == currentPaddingsShape, 0,
               "PAD op: wrong shape of paddings array, expected is %s, but got "
               "%s instead !",
  ...
  // in case of REFLECT and SYMMETRIC modes paddings must obey additional shape
  // requirements
  if (INT_ARG(0) == 0) {  // CONSTANT mode
    if (block.width() > 2) {
      REQUIRE_TRUE(input->dataType() == INPUT_VARIABLE(2)->dataType(), 0,
                   "PAD op: data types of input and padValue arrays should be "
                   "the same but got %i and %i correspondingly !",
    ...
    for (int dim = 0; dim < rank; ++dim)
      REQUIRE_TRUE(
          paddings->e<Nd4jLong>(dim, 0) <= (input->shapeOf()[dim] - 1) &&
              paddings->e<Nd4jLong>(dim, 1) <= (input->shapeOf()[dim] - 1),
          0, "PAD op: wrong content of paddings array for REFLECT mode !");
  }
  if (INT_ARG(0) == 2) {  // SYMMETRIC mode
    for (int dim = 0; dim < rank; ++dim)
      REQUIRE_TRUE(
          paddings->e<Nd4jLong>(dim, 0) <= input->shapeOf()[dim] &&
              paddings->e<Nd4jLong>(dim, 1) <= input->shapeOf()[dim],
          0, "PAD op: wrong content of paddings array for SYMMETRIC mode !");
  }

  // CONSTANT->0, REFLECT->1, SYMMETRIC->2
  REQUIRE_TRUE(INT_ARG(0) >= 0 && INT_ARG(0) <= 2, 0,
               "PAD op: unknown padding mode, there are only three possible "
               "legal values -> 0,1,2, but got %i instead !",
               INT_ARG(0));
  ...
  // helpers::recursiveLoopForPad(INT_ARG(0), *input, *paddings, *output,
  // dimensions, 0, 0, 0, padValue);
  helpers::pad(block.launchContext(), INT_ARG(0), *input, *paddings, *output,
               padValue);
...
DECLARE_TYPES(pad) {
  getOpDescriptor()
      ...
      ->setAllowedInputTypes(
          1, {DataType::INT32, DataType::INT64})  // INT32 with TF
      //    	->setAllowedInputTypes(1, {DataType::INT32, DataType::INT64}) //
      //    INT32 with TF, but used also INT64 due long shapes
      ->setSameMode(true);
}
...
DECLARE_SHAPE_FN(pad) {
  ...
  const std::vector<Nd4jLong> currentPaddingsShape =
      paddings->getShapeAsVector();
  REQUIRE_TRUE(expectedPaddingsShape == currentPaddingsShape, 0,
               "PAD op: wrong shape of paddings array, expected is %s, but got "
               "%s instead !",
               ShapeUtils::shapeAsString(expectedPaddingsShape).c_str(),
  ...
  for (int i = 1; i <= rank; ++i)
    outShapeInfo[i] = inputShapeInfo[i] + paddings->e<Nd4jLong>(i - 1, 0) +
                      paddings->e<Nd4jLong>(i - 1, 1);
