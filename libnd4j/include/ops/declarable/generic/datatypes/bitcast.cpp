...
CUSTOM_OP_IMPL(bitcast, 1, 1, false, 0, 1) {
  ...
  if (inputSize < outputSize) {
    REQUIRE_TRUE(
        input->sizeAt(-1) == lastSize, 0,
        "BITCAST: %llu > %llu. So last dimension should be %i, but %i given.",
        inputSize, outputSize, lastSize, input->sizeAt(-1));
  }
  if (input->isEmpty()) {
    REQUIRE_TRUE(
        output->isEmpty(), 0,
        "BITCAST: If input is empty, output array must also be empty.");
    return Status::OK();
  }
...
DECLARE_SHAPE_FN(bitcast) {
  ...
  if (inputSize == outputSize) {
    ...
    // range of output increased by 1 with inputSize / outputSize as last
    // dimension
    ...
    auto outputShape = ConstantShapeHelper::getInstance()->createShapeInfo(
        newType, shape::order(inShape), shapeOf);
    return SHAPELIST(outputShape);
  }
  REQUIRE_TRUE(
      shape::sizeAt(inShape, -1) == outputSize / inputSize, 0,
      "BITCAST: %llu > %llu. So last dimension should be %i, but %i given.",
