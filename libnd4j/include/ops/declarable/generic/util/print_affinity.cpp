...
CUSTOM_OP_IMPL(print_affinity, 1, 1, true, 0, 0) {
  ...
  nd4j_printf(
      "<Node %i>: Actuality: [HOST: %s; DEVICE: %s]; affinity: [%i]; Pointers: "
      "[HOST: %p; DEVICE: %p]; DataBuffer length: %lld\n",
      block.nodeId(), input->isActualOnHostSide() ? "true" : "false",
      input->isActualOnDeviceSide() ? "true" : "false",
      input->dataBuffer()->deviceId(), input->buffer(), input->specialBuffer(),
