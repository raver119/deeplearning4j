...
CUSTOM_OP_IMPL(barnes_edge_forces, 4, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(rowP->isVector(), 0,
               "barnes_edge_force: row input must be a vector, but its rank is "
               "%i instead !",
               rowP->rankOf());
  REQUIRE_TRUE(colP->isVector(), 0,
               "barnes_edge_force: col input must be a vector, but its rank is "
               "%i instead !",
               colP->rankOf());
  REQUIRE_TRUE(dataP->dataType() == output->dataType() &&
                   dataP->dataType() == valP->dataType(),
               0,
               "barnes_edge_force: data type of dataP, valP and output must be "
               "the same");
...
DECLARE_SHAPE_FN(barnes_edge_forces) {
  ...
  outShapeInfo = ShapeBuilders::copyShapeInfoAndType(
      inputShape->at(3), inputShape->at(3), false, block.workspace());
