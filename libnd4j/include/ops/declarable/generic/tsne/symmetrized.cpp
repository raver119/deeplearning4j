...
DECLARE_SHAPE_FN(barnes_symmetrized) {
  ...
  auto dataType =
      rowP->dataType();  // ArrayOptions::dataType(inputShape->at(0));
  NDArray* rowCounts = NDArrayFactory::create_<int>(
      'c', {N}, block.launchContext());  // rowP->dup();
  // srowCounts->assign(0);
  ...
  // ALLOCATE(outShapeInfo, block.workspace(), shape::shapeInfoLength(2),
  // Nd4jLong);
  //            outShapeInfo[1] = 1;
  //            outShapeInfo[2] = len;
  // ShapeUtils::updateStridesAndType(outShapeInfo,
  // ArrayOptions::dataType(valPShapeInfo), 'c');
  // outShapeInfo =
  // ShapeBuilders::createVectorShapeInfo(ArrayOptions::dataType(valPShapeInfo),
  // len, block.workspace());
  outShapeInfo = sd::ShapeBuilders::createShapeInfo(
      ArrayOptions::dataType(valPShapeInfo), 'c', {1, len}, block.workspace());
  auto outColsShapeInfo = sd::ShapeBuilders::createShapeInfo(
      dataType, 'c', {1, len}, block.workspace());
  auto outRowsShapeInfo = sd::ShapeBuilders::createShapeInfo(
      dataType, 'c', {1, N + 1}, block.workspace());
  return SHAPELIST(CONSTANT(outRowsShapeInfo), CONSTANT(outColsShapeInfo),
                   CONSTANT(outShapeInfo));
}
