...
OP_IMPL(toggle_bits, -1, -1, true) {
  for (int i = 0; i < block.width(); i++) {
    ...
    REQUIRE_TRUE(x->dataType() == z->dataType(), 0,
                 "Toggle bits requires input and output to have same type");
    REQUIRE_TRUE(x->isZ(), 0,
                 "Toggle bits requires input and output to be integer type "
