...
CUSTOM_OP_IMPL(tensormmul, 2, 1, false, 0, -1) {
  ...
  REQUIRE_TRUE(a->dataType() == b->dataType(), 0,
               "tensormmul: A, B and C data types must be the same");
...
DECLARE_SHAPE_FN(tensormmul) {
  ...
  REQUIRE_TRUE(
      ArrayOptions::dataType(aShapeInfo) == ArrayOptions::dataType(bShapeInfo),
      0, "tensormmul: A and B data types must be the same");
  ...
  auto outShape = sd::ShapeUtils::evalShapeForTensorDot(
...
CUSTOM_OP_IMPL(tensormmul_bp, 3, 2, false, 0, -1) {
  ...
  REQUIRE_TRUE(
      (A->dataType() == B->dataType() && (dLdC->dataType() == A->dataType())),
  ...
  REQUIRE_TRUE(
      (Arank >= axe0Size), 0,
      "tensormmul_bp: A rank must be the higher or same as input axes 0");

  REQUIRE_TRUE(
      (Brank >= axe1Size), 0,
      "tensormmul_bp: B rank must be the higher or same as input axes 1");

  // building axes
  std::vector<int> axes0(axe0Size), axes1(axe1Size);
  for (uint e = 0; e < axe0Size; e++) axes0[e] = (int)INT_ARG(e + 1);
  for (uint e = 0; e < axe1Size; e++) axes1[e] = (int)INT_ARG(e + axe0Size + 2);

  std::vector<int> permutAt, permutBt;
  std::vector<Nd4jLong> shapeAt, shapeBt;

  ShapeUtils::evalShapeForTensorDot(A, B, axes0, axes1, permutAt, permutBt,
                                    shapeAt, shapeBt);
...
DECLARE_SHAPE_FN(tensormmul_bp) {
  ...
  REQUIRE_TRUE((ArrayOptions::dataType(aShapeInfo) ==
                    ArrayOptions::dataType(bShapeInfo) &&
                (ArrayOptions::dataType(dLShapeInfo) ==
                 ArrayOptions::dataType(aShapeInfo))),
               0, "tensormmul_bp: A, B and dLdC data types must be the same");
...
DECLARE_TYPES(tensormmul_bp) {
  getOpDescriptor()
      ->setAllowedInputTypes(0, {DataType::FLOAT32, DataType::DOUBLE,
                                 DataType::HALF})  // maybe better ALL_FLOATS
      ->setAllowedInputTypes(
          1, {DataType::FLOAT32, DataType::DOUBLE, DataType::HALF})
      ->setAllowedInputTypes(
