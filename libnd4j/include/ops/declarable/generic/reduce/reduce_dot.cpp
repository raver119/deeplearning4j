...
CUSTOM_OP_IMPL(reduce_dot_bp, 3, 2, false, 0, 0) {
  ...
  REQUIRE_TRUE(x->isSameShape(y), 0,
               "REDUCE_DOT_BP OP: both input arrays x and y should have same "
               "shapes, but got %s and %s correspondingly",
               ShapeUtils::shapeAsString(x).c_str(),
               ShapeUtils::shapeAsString(y).c_str());

  if (gradO->lengthOf() ==
      1) {  // scalar of reduced to scalar with keep dimensions
    ...
    REQUIRE_TRUE(dimensions.size() <= x->rankOf(), 0,
                 "REDUCE_DOT_BP OP: the number of dimensions to reduce along "
                 "must be <= input array rank, but got %i instead",
                 dimensions.size());

    for (const auto& item : dimensions)
      REQUIRE_TRUE(item >= -x->rankOf() && item < x->rankOf(), 0,
                   "REDUCE_DOT_BP OP: the input dimension to reduce along must "
                   "be in range [-%i, %i), but got %i instead !",
                   x->rankOf(), x->rankOf(), item);

    if (!keepDims) {
      auto gradOShapeKeepDims = ShapeUtils::evalReduceShapeInfo(
          gradO->ordering(), dimensions, *x, true, false, block.workspace());
