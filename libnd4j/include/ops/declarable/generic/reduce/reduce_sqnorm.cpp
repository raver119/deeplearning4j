...
DECLARE_SHAPE_FN(reduce_sqnorm) {
    ...
    Nd4jLong* outShapeInfo = ShapeUtils::evalReduceShapeInfo(shape::order(inputShape->at(0)), dimensions, inputShape->at(0), keepDims, false, block.workspace());
...
CUSTOM_OP_IMPL(reduce_sqnorm_bp, 2, 1, false, 0, 0) {
    ...
    if (gradO->lengthOf() == 1) {
        ...
        if(!keepDims) {
            auto gradOShapeKeepDims = ShapeUtils::evalReduceShapeInfo(gradO->ordering(), dimensions, *input, true, false, block.workspace());
