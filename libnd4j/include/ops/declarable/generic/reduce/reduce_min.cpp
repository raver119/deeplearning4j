...
CUSTOM_OP_IMPL(reduce_min, 1, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(dimensions.size() <= input->rankOf(), 0,
               "REDUCE_MIN OP: the number of dimensions to reduce along must "
  ...
  for (const auto& item : dimensions)
    REQUIRE_TRUE(item >= -input->shapeInfo()[0] && item < input->shapeInfo()[0],
                 0,
                 "REDUCE_MIN OP: the input dimension to reduce along must be "
...
CUSTOM_OP_IMPL(reduce_min_bp, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(dimensions.size() <= input->rankOf(), 0,
               "REDUCE_MIN_BP OP: the number of dimensions to reduce along "
  ...
  for (const auto& item : dimensions)
    REQUIRE_TRUE(item >= -input->shapeInfo()[0] && item < input->shapeInfo()[0],
                 0,
                 "REDUCE_MIN_BP OP: the input dimension to reduce along must "
  ...
  *gradI = 0;
  ...
  if (gradO->lengthOf() == 1) {
    ...
    auto indicesArr =
        input->applyIndexReduce(sd::indexreduce::IndexMin, dimensions);
