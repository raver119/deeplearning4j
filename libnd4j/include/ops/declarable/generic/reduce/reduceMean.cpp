...
CUSTOM_OP_IMPL(reduce_mean, 1, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(dimensions.size() <= input->rankOf(), 0,
               "REDUCE_MEAN OP: the number of dimensions to reduce along must "
               "be <= input array rank, but got %i instead",
               dimensions.size());

  for (const auto& item : dimensions)
    REQUIRE_TRUE(item >= -input->rankOf() && item < input->rankOf(), 0,
                 "REDUCE_MEAN OP: the input dimension to reduce along must be "
...
DECLARE_SHAPE_FN(reduce_mean) {
  ...
  REQUIRE_TRUE(dimensions.size() <= in[0], 0,
               "REDUCE_MEAN OP: the number of dimensions to reduce along must "
               "be <= input array rank, but got %i instead",
               dimensions.size());
...
CUSTOM_OP_IMPL(reduce_mean_bp, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(dimensions.size() <= input->rankOf(), 0,
               "REDUCE_MEAN_BP OP: the number of dimensions to reduce along "
               "must be <= input array rank, but got %i instead",
               dimensions.size());

  for (const auto& item : dimensions)
    REQUIRE_TRUE(item >= -input->rankOf() && item < input->rankOf(), 0,
                 "REDUCE_MEAN_BP OP: the input dimension to reduce along must "
...
DECLARE_SHAPE_FN(reduce_mean_bp) {
  ...
  for (const auto& item : dimensions)
    REQUIRE_TRUE(item >= -rank || item < rank, 0,
                 "REDUCE_MEAN_BP OP: the input dimension to reduce along must "
                 "be in range [-%i, %i), but got %i instead !",
                 rank, rank, item);
