...
CONFIGURABLE_OP_IMPL(skipgram, 12, 12, true, 0, 0) {
  ...
  REQUIRE_TRUE(block.isInplace(), 0,
               "SkipGram: this operation requires inplace execution only");

  REQUIRE_TRUE(syn0->dataType() == syn1->dataType() &&
                   syn0->dataType() == syn1neg->dataType(),
               0, "SkipGram: all syn tables must have the same data type");
  REQUIRE_TRUE(syn0->dataType() == expTable->dataType(), 0,
               "SkipGram: expTable must have the same data type as syn0 table");

  sd::ops::helpers::skipgram(*syn0, *syn1, *syn1neg, *expTable, *negTable,
                             *target, *ngStarter, nsRounds, *indices, *codes,
                             *alpha, *randomValue, *inferenceVector,
                             isPreciseMode, numWorkers);
...
DECLARE_SHAPE_FN(skipgram) {
    return SHAPELIST(ShapeBuilders::createScalarShapeInfo(DataType::INT8,
block.workspace()));
}
*/
}  // namespace ops
