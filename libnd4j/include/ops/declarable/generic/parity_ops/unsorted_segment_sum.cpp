...
CUSTOM_OP_IMPL(unsorted_segment_sum, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(idxSegments->isVector(), 0,
               "unsorted_segment_sum: segment indexes array should be a "
               "vector, but it rank is %i.",
               idxSegments->rankOf());
  REQUIRE_TRUE(idxSegments->lengthOf() == input->sizeAt(0), 0,
               "unsorted_segment_sum: segment indexes array length should be "
  ...
  REQUIRE_TRUE(helpers::unsortedSegmentIndicesValidate(
               ...
               0,
               "unsorted_segment_sum: segment indices should be in range [0, "
               "%ld), but %ld > %ld",
               numOfClasses, wrong, numOfClasses);

  helpers::unsortedSegmentSumFunctor(block.launchContext(), input, idxSegments,
...
CUSTOM_OP_IMPL(unsorted_segment_sum_bp, 3, 2, false, 0, 1) {
  return helpers::unsortedSegmentSumFunctorBP(
