...
CUSTOM_OP_IMPL(listdiff, 2, 2, false, 0, 0) {
  ...
  REQUIRE_TRUE(values->rankOf() == 1, 0,
               "ListDiff: rank of values should be 1D, but got %iD instead",
               values->rankOf());
  REQUIRE_TRUE(keep->rankOf() == 1, 0,
               "ListDiff: rank of keep should be 1D, but got %iD instead",
               keep->rankOf());
  REQUIRE_TRUE(keep->dataType() == values->dataType(), 0,
               "ListDiff: both inputs must have same data type");

  return helpers::listDiffFunctor(block.launchContext(), values, keep, output1,
                                  output2);
};
...
DECLARE_SHAPE_FN(listdiff) {
  ...
  auto shapeX = ConstantShapeHelper::getInstance().vectorShapeInfo(
      saved, values->dataType());
  auto shapeY = ConstantShapeHelper::getInstance().vectorShapeInfo(
      saved, DataType::INT64);
