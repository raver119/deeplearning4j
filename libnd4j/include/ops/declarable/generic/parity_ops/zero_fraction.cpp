...
CUSTOM_OP_IMPL(zero_fraction, 1, 1, false, 0, 0) {
  ...
  // nd4j_printf("Zero count is %f for %i elements.", countZero.e<double>(0),
  // input->lengthOf()); countZero /= double(input->lengthOf());
  output->p<double>(
      0, countZero.e<Nd4jLong>(0) /
             double(input->lengthOf()));  // printIndexedBuffer("Zero count");
