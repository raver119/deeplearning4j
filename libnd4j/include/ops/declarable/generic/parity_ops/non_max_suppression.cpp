...
CUSTOM_OP_IMPL(non_max_suppression, 2, 1, false, 0, 0) {
  ...
  if (block.width() > 2)
    ...
    REQUIRE_TRUE(false, 0,
                 "image.non_max_suppression: Max output size argument cannot "
                 "be retrieved.");
  ...
  REQUIRE_TRUE(boxes->sizeAt(1) == 4, 0,
               ...
               boxes->sizeAt(1));
  REQUIRE_TRUE(scales->rankOf() == 1 && scales->lengthOf() == boxes->sizeAt(0),
               0,
               "image.non_max_suppression: The rank of scales array should be "
               "1, but %i is given",
               boxes->rankOf());
  ...
  REQUIRE_TRUE(boxes->dataType() == scales->dataType(), 0,
               "image.non_max_suppression: Boxes and scores inputs should have "
               "the same data type, but %s and %s "
               "were given.",
               DataTypeUtils::asString(boxes->dataType()).c_str(),
               DataTypeUtils::asString(scales->dataType()).c_str());
  helpers::nonMaxSuppression(block.launchContext(), boxes, scales,
                             maxOutputSize, overlayThreshold, scoreThreshold,
                             output);
  return Status::OK();
}
...
CUSTOM_OP_IMPL(non_max_suppression_v3, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(scales->rankOf() == 1 && scales->lengthOf() == boxes->sizeAt(0),
               ...
               boxes->rankOf());
  REQUIRE_TRUE(overlayThreshold >= 0. && overlayThreshold <= 1., 0,
               "image.non_max_suppression_v3: The overlay threashold should be "
               "in [0, 1], but %lf given.",
               overlayThreshold);
  REQUIRE_TRUE(boxes->dataType() == scales->dataType(), 0,
               "image.non_max_suppression_v3: Boxes and scores inputs should "
               "have the same data type, but %s and %s "
               "were given.",
               DataTypeUtils::asString(boxes->dataType()).c_str(),
               DataTypeUtils::asString(scales->dataType()).c_str());

  helpers::nonMaxSuppressionV3(block.launchContext(), boxes, scales,
                               maxOutputSize, overlayThreshold, scoreThreshold,
                               output);
  return Status::OK();
}
...
DECLARE_SHAPE_FN(non_max_suppression_v3) {
  ...
  if (len > 0)
    len = helpers::nonMaxSuppressionV3(block.launchContext(), boxes, scales,
                                       maxOutputSize, overlayThreshold,
