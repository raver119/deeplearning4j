...
CUSTOM_OP_IMPL(segment_prod, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(idxSegments->isVector(), 0,
               "segment_prod: segment indexes array should be a vector, but it "
               "rank is %i.",
               idxSegments->rankOf());
  REQUIRE_TRUE(idxSegments->lengthOf() == input->sizeAt(0), 0,
               "segment_prod: segment indexes array length should be equal to "
  ...
  REQUIRE_TRUE(
      helpers::segmentIndicesValidate(block.launchContext(), idxSegments,
                                      expected, wrong),
      0, "segment_prod: segment indices should be arranged, but %2.1f > %2.1f",
      expected.e<float>(0), wrong.e<float>(0));

  segmentedOutput->nullify();
  helpers::segmentProdFunctor(block.launchContext(), input, idxSegments,
                              segmentedOutput);
...
CUSTOM_OP_IMPL(segment_prod_bp, 3, 2, false, 0, 0) {
  ...
  helpers::segmentProdFunctorBP(block.launchContext(), input, indices, gradOut,
                                output);
