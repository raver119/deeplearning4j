...
CUSTOM_OP_IMPL(unsorted_segment_min, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(idxSegments->lengthOf() == input->sizeAt(0), 0,
               "unsorted_segment_min: segment indexes array length should be "
  ...
  REQUIRE_TRUE(helpers::unsortedSegmentIndicesValidate(
               ...
               0,
               "unsorted_segment_min: segment indices should be in range [0, "
               "%ld), but %ld > %ld",
               numOfClasses, wrong, numOfClasses);

  helpers::unsortedSegmentMinFunctor(block.launchContext(), input, idxSegments,
...
CUSTOM_OP_IMPL(unsorted_segment_min_bp, 3, 2, false, 0, 1) {
  return helpers::unsortedSegmentMinFunctorBP(
