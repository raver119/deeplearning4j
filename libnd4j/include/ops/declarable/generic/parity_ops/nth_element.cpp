...
CUSTOM_OP_IMPL(nth_element, 2, 1, false, 0, 0) {
  ...
  bool reverse = false;
  if (block.numI() > 0) reverse = (bool)INT_ARG(0);

  auto output = OUTPUT_VARIABLE(0);
  Nd4jLong lastDim = input->sizeAt(-1);
  int nVal = n->e<int>(0);
  REQUIRE_TRUE(nVal < lastDim && nVal >= 0, 0,
               "nth_element: n should be non-negative and less than last "
               "dimension size (%lld), but %i was given.",
               lastDim, n);
  REQUIRE_TRUE(input->rankOf() > 0, 0,
               "nth_element: The rank of input array should be at least 1, but "
               "%i is given",
               input->rankOf());  //
  if (output->lengthOf() == input->lengthOf())
    ...
    helpers::nthElementFunctor(block.launchContext(), input, nVal, output,
                               reverse);
  }
...
DECLARE_SHAPE_FN(nth_element) {
  ...
  if (outRank > 1) {
    ...
    for (Nd4jLong e = 0; e < outRank; e++) outputShape[e + 1] = in[e + 1];
