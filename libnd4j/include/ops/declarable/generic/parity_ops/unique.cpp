...
CUSTOM_OP_IMPL(unique, 1, 2, false, 0, 0) {
  ...
  REQUIRE_TRUE(x->dataType() == values->dataType(), 0,
               "Unique: input and output data types must be the same");

  return helpers::uniqueFunctor(block.launchContext(), x, values, indices,
                                (NDArray*)nullptr);
}
...
CUSTOM_OP_IMPL(unique_with_counts, 1, 3, false, 0, 0) {
  ...
  return helpers::uniqueFunctor(block.launchContext(), input, values, indices,
                                counts);
}
