...
CUSTOM_OP_IMPL(sequence_mask, 1, 1, false, 0, 0) {
  ...
  // REQUIRE_TRUE(inRank >= 1, 0, "sequence_mask: input array must have rank >=
  // 1, but %i given!", inRank);
  Nd4jLong maxInd = input->argMax();
  float max = input->e<float>(maxInd);
  if (block.numI() > 0) {
    maxInd = INT_ARG(0);
    if (maxInd < max) maxInd = static_cast<Nd4jLong>(max);
  } else if (block.width() > 1) {
    auto maxlen = INPUT_VARIABLE(1);
    // REQUIRE_TRUE(maxlen->lengthOf() == 1, "sequence_mask: 2nd input (max
    // length) should be a scalar array.");
    float tmaxlen = maxlen->e<float>(0);
    if (tmaxlen > max) maxInd = static_cast<Nd4jLong>(tmaxlen);
  } else
