...
CUSTOM_OP_IMPL(bincount, 1, 1, false, 0, 0) {
  ...
  if (block.width() == 2) {  // the second argument is weights
    ...
    REQUIRE_TRUE(values->isSameShape(weights), 0,
                 "bincount: the input and weights shapes should be equals");
  } else if (block.width() ==
             3) {  // the second argument is min and the third is max
  ...
  helpers::adjustWeights(block.launchContext(), values, weights, result,
                         minLength, maxLength);
...
DECLARE_SHAPE_FN(bincount) {
  ...
  if (block.numI() > 0) outLength = sd::math::nd4j_max(maxLength, INT_ARG(0));

  if (block.numI() > 1) outLength = sd::math::nd4j_min(outLength, INT_ARG(1));
