...
CUSTOM_OP_IMPL(non_max_suppression_overlaps, 2, 1, false, 0, 0) {
  ...
  if (block.width() > 2)
    ...
    REQUIRE_TRUE(false, 0,
                 "image.non_max_suppression_overlaps: Max output size argument "
                 "cannot be retrieved.");
  REQUIRE_TRUE(boxes->rankOf() == 2, 0,
               "image.non_max_suppression_overlaps: The rank of boxes array "
               "should be 2, but %i is given",
               boxes->rankOf());
  REQUIRE_TRUE(boxes->sizeAt(0) == boxes->sizeAt(1), 0,
               "image.non_max_suppression_overlaps: The boxes array should be "
               "square, but {%lld, %lld} is given",
               boxes->sizeAt(0), boxes->sizeAt(1));
  REQUIRE_TRUE(scales->rankOf() == 1 && scales->lengthOf() == boxes->sizeAt(0),
               0,
               "image.non_max_suppression_overlaps: The rank of scales array "
               "should be 1, but %i is given",
               boxes->rankOf());
  ...
  if (block.numT() > 0) overlapThreshold = T_ARG(0);
  if (block.numT() > 1) scoreThreshold = T_ARG(1);

  // TODO: refactor helpers to multithreaded facility
  helpers::nonMaxSuppressionGeneric(block.launchContext(), boxes, scales,
                                    maxOutputSize, overlapThreshold,
...
DECLARE_SHAPE_FN(non_max_suppression_overlaps) {
  ...
  Nd4jLong boxSize = helpers::nonMaxSuppressionGeneric(
