...
CUSTOM_OP_IMPL(in_top_k, 2, 1, true, 0, 1) {
  ...
  REQUIRE_TRUE(block.numI() > 0, 0,
               "in_top_k: Parameter k is needed to be set");
  REQUIRE_TRUE(predictions->sizeAt(0) == target->lengthOf(), 0,
               "in_top_k: the number of predictions rows should be equal to "
               "target array length, but got %i and %i correspondingly !",
               predictions->sizeAt(0), target->lengthOf());
  REQUIRE_TRUE(
      predictions->rankOf() == 2, 0,
      "in_top_k: The predictions array should have rank 2, but %i given",
      predictions->rankOf());
  REQUIRE_TRUE(target->rankOf() == 1, 0,
               "in_top_k: The target should be a vector");

  int k = INT_ARG(0);
  return helpers::inTopKFunctor(block.launchContext(), predictions, target,
                                result, k);
}
