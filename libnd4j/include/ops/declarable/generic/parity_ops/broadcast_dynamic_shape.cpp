...
CUSTOM_OP_IMPL(broadcast_dynamic_shape, 2, 1, false, 0, 0) {
  ...
  REQUIRE_TRUE(x->rankOf() == 1, 0,
               "BROADCAST_DYNAMIC_SHAPE OP: the first input array must have "
               "rank = 1, but got %i instead!",
               x->rankOf());
  REQUIRE_TRUE(y->rankOf() == 1, 0,
               "BROADCAST_DYNAMIC_SHAPE OP: the second input array must have "
               "rank = 1, but got %i instead!",
               y->rankOf());
  REQUIRE_TRUE(x->dataType() == y->dataType(), 0,
               "BROADCAST_DYNAMIC_SHAPE OP: both input arrays must have the "
               "same integer type !");
  ...
  ArrayOptions::setDataType(
      xShapeInfo.data(),
      sd::DataType::INT64);  // fill with some data type, it doesn't matter what
                             // type exactly to choose
  ...
  const bool isBroadcastPossible = ShapeUtils::evalBroadcastShapeInfo(
      xShapeInfo.data(), yShapeInfo.data(), true, poinerOnOutShapeInfo,
      block.launchContext()->getWorkspace());

  REQUIRE_TRUE(isBroadcastPossible, 0,
               "BROADCAST_DYNAMIC_SHAPE OP: the shapes of two input arrays %s "
               "and %s are not suitable for broadcast operation !",
               ShapeUtils::shapeAsString(xShapeInfo.data()).c_str(),
               ShapeUtils::shapeAsString(yShapeInfo.data()).c_str());
