...
CUSTOM_OP_IMPL(top_k, 1, 2, false, 0, -1) {
  ...
  REQUIRE_TRUE(k <= x->sizeAt(-1), 0,
               "top_k: k should not be greater than last dimension");
  REQUIRE_TRUE(k > 0, 0, "top_k: k should be positive, but %i given.", k);

  int res = helpers::topKFunctor(block.launchContext(), x, values, indices, k,
                                 needSort);
  return res;
}
...
DECLARE_SHAPE_FN(top_k) {
  ...
  for (int e = 0; e < 2; e++) {  // 2 element tuple at output
    Nd4jLong* aShape;
    ALLOCATE(aShape, block.workspace(), shape::shapeInfoLength(shapeRank),
             Nd4jLong);
    aShape[0] = shapeRank;
    ...
    shapeList->push_back(ConstantShapeHelper::getInstance().createShapeInfo(
        ShapeDescriptor(aShape, (e == 0 ? ArrayOptions::dataType(in)
                                        : sd::DataType::INT64))));
