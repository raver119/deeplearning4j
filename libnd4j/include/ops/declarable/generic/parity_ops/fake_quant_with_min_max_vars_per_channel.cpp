...
namespace sd {
namespace ops {
CONFIGURABLE_OP_IMPL(fake_quant_with_min_max_vars_per_channel, 3, 1, true, 0,
                     0) {
  ...
  REQUIRE_TRUE(min->rankOf() == 1 && max->rankOf() == 1 &&
                   min->lengthOf() == max->lengthOf(),
               0,
               "fake_quant_with_min_max_vars_per_channel: Min and Max should "
               "be 1D tensors with the same length");
  ...
  REQUIRE_TRUE(x->dataType() == output->dataType(), 0,
               "fake_quant_with_min_max_vars_per_channel: input and output "
               "data types must be the same");

  int numBits = 8;
  if (block.numI()) numBits = INT_ARG(0);
  bool narrowed = false;
  // INT_ARG(1);
  if (block.numB()) {
    narrowed = B_ARG(0);
  ...
  REQUIRE_TRUE(numBits > 1 && numBits < 17, 0,
               ...
               "was given.",
               numBits);
  helpers::fakeQuantWithMinMaxVarsPerChannel(block.launchContext(), x, min, max,
                                             numBits, narrowed, output);
...
DECLARE_SYN(fake_quant_with_min_max_args_per_channel,
