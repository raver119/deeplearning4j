...
BROADCASTABLE_OP_IMPL(Pow, 0, 0) {
  ...
  auto tZ = BroadcastHelper::broadcastApply(
      {scalar::Pow, pairwise::Pow, broadcast::Pow}, x, y, z);
  if (tZ == nullptr)
...
CUSTOM_OP_IMPL(Pow_bp, 3, 2, false, 0, 0) {
  ...
  if (dLdy->isSameShape(*dLdz)) {
    ...
    dLdy->assign(temp.reduceAlongDimension(
        reduce::Sum, axesForY));  // dL/dy = sum(c * dL/dz)
  }

  // dL/dx = y*x^(y-1) * dL/dz
  x->applyTrueBroadcast(BroadcastOpsTuple::PowDerivative(), *y,
                        temp);  // a = y*x^(y-1)
  ...
  if (dLdx->isSameShape(*dLdz)) {
    ...
    dLdx->assign(
        temp.reduceAlongDimension(reduce::Sum, axesForX));  // dLdx = a*dL/dz
  }
