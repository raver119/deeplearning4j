...
BROADCASTABLE_OP_IMPL(multiply, 0, 0) {
  ...
  const bool areShapesBroadcastable = ShapeUtils::evalBroadcastShapeInfo(
      x->shapeInfo(), y->shapeInfo(), true, zShapeInfo, block.workspace());
  REQUIRE_TRUE(areShapesBroadcastable, 0,
               "MULTIPLY OP: the shapes of x %s and y %s are not suitable for "
               "broadcast !",
               ShapeUtils::shapeAsString(x).c_str(),
               ShapeUtils::shapeAsString(y).c_str());

  auto tZ = BroadcastHelper::broadcastApply(sd::BroadcastOpsTuple::Multiply(),
                                            x, y, z);
  if (tZ == nullptr)
...
CUSTOM_OP_IMPL(multiply_bp, 3, 2, false, 0, 0) {
  ...
  const bool areShapesBroadcastable = ShapeUtils::evalBroadcastShapeInfo(
      x->shapeInfo(), y->shapeInfo(), true, dLdzShapeInfo, block.workspace());
  REQUIRE_TRUE(areShapesBroadcastable, 0,
               "MULTIPLY_BP OP: the shapes of x %s and y %s are not suitable "
               "for broadcast !",
               ShapeUtils::shapeAsString(x).c_str(),
               ShapeUtils::shapeAsString(y).c_str());
  REQUIRE_TRUE(shape::equalsSoft(dLdz->shapeInfo(), dLdzShapeInfo), 0,
               "MULTIPLY_BP OP: wrong shape of next epsilon array (dLdOut), "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(dLdzShapeInfo).c_str(),
               ShapeUtils::shapeAsString(dLdz).c_str());
  ...
  if (x->isScalar() && y->isScalar()) {  // both are scalars
    ...
    y->tile(yTiled);
    std::vector<int> axesForY = ShapeUtils::evalBroadcastBackwardAxis(
        y->shapeInfo(), dLdz->shapeInfo());
    ...
    x->tile(xTiled);
    std::vector<int> axesForX = ShapeUtils::evalBroadcastBackwardAxis(
        x->shapeInfo(), dLdz->shapeInfo());
...
/*
        CUSTOM_OP_IMPL(multiply_bp, 3, 2, false, 0, 0) {
            ...
            if (x->isSameShape(y)) {
                ...
                auto axisX =
   ShapeUtils::evalBroadcastBackwardAxis(x->shapeInfo(), epsNext->shapeInfo());
                auto axisY =
   ShapeUtils::evalBroadcastBackwardAxis(y->shapeInfo(), epsNext->shapeInfo());

                if (axisX.size() > 0) {
                    auto sum = preX->template
   reduceAlongDimension<simdOps::Sum<T>>(axisX); gradX->assign(sum); delete sum;
                } else
                    gradX->assign(preX);

                if (axisY.size() > 0) {
                    auto sum = preY->template
   reduceAlongDimension<simdOps::Sum<T>>(axisY); gradY->assign(sum); delete sum;
                } else
                    gradY->assign(preY);
