...
CUSTOM_OP_IMPL(biasadd, 2, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(bias->rankOf() == 1, 0,
               "BIASADD CUSTOM_OP: bias array should have rank = 1, but got %i "
               "instead !",
               bias->rankOf());

  REQUIRE_TRUE(
      bias->sizeAt(0) == input->sizeAt(channelDim), 0,
      "BIASADD CUSTOM_OP: shapes of bias %s and input %s arrays are not "
      "suitable for broadcast operation along channel dimension %i !",
      ShapeUtils::shapeAsString(bias).c_str(),
      ShapeUtils::shapeAsString(input).c_str(), channelDim);

  REQUIRE_TRUE(output->isSameShape(input), 0,
               "BIASADD CUSTOM_OP: wrong shape of output array, expected is %s "
               "but got %s instead !",
...
CUSTOM_OP_IMPL(biasadd_bp, 3, 2, false, 0, 0) {
  ...
  gradO->reduceAlongDimension(
      sd::reduce::Sum, *gradB,
      ShapeUtils::evalDimsToExclude(gradO->rankOf(), {channelDim}));
