...
CUSTOM_OP_IMPL(batchnorm, 3, 1, false, 1, 2) {
  ...
  REQUIRE_TRUE(numOfAxes <= inRank, 0,
               "BATCHNORM op: too big number of input axes to normalize over, "
               "expected number should be less or equal to rank of input "
  ...
  // formula: output = gamma * ((input - mean) / sqrt(variance + epsilon)) +
  // beta auto v = input->varianceAlongDimension(variance::SummaryStatsVariance,
  // false, ShapeUtils::evalDimsToExclude(input->rankOf(), axes)); auto m =
  // input->reduceAlongDimension(sd::reduce::Mean,
  ...
  // stdInv.applyTransform(transform::Reciprocal);               // 1 /
  // (variance + epsilon) stdInv.applyTransform(transform::Sqrt); // 1 /
  // (variance + epsilon)^0.5 if(applyScale)
  //     stdInv *= *gamma;
...
DECLARE_SHAPE_FN(batchnorm) {
  ...
  DataType outType =
      DataTypeUtils::pickFloatingType(ArrayOptions::dataType(inShapeInfo));

  auto outShapeInfo = ShapeBuilders::copyShapeInfoAndType(
      inShapeInfo, outType, false,
      block.workspace());  // output shape is identical to input shape
...
CUSTOM_OP_IMPL(batchnorm_bp, 4, 3, false, 1, 2) {
  ...
  const uint numOfAxes = axes.size();
  REQUIRE_TRUE(numOfAxes <= inRank, 0,
               "BATCHNORM_BP op: too big number of input axes to normalize "
  ...
  REQUIRE_TRUE(mean->isSameShape(expShape), 0,
               "BATCHNORM_BP op: wrong shape of mean array, expected is %s, "
               "but got %s instead !",
               ShapeUtils::shapeAsString(expShape).c_str(),
               ShapeUtils::shapeAsString(mean).c_str());
  REQUIRE_TRUE(variance->isSameShape(expShape), 0,
               "BATCHNORM_BP op: wrong shape of variance array, expected is "
               "%s, but got %s instead !",
               ShapeUtils::shapeAsString(expShape).c_str(),
               ShapeUtils::shapeAsString(variance).c_str());
  if (gamma)
    REQUIRE_TRUE(gamma->isSameShape(expShape), 0,
                 "BATCHNORM_BP op: wrong shape of gamma array, expected is %s, "
                 "but got %s instead !",
                 ShapeUtils::shapeAsString(expShape).c_str(),
                 ShapeUtils::shapeAsString(gamma).c_str());
  if (beta)
    REQUIRE_TRUE(beta->isSameShape(expShape), 0,
                 "BATCHNORM_BP op: wrong shape of beta array, expected is %s, "
                 "but got %s instead !",
                 ShapeUtils::shapeAsString(expShape).c_str(),
                 ShapeUtils::shapeAsString(beta).c_str());

  REQUIRE_TRUE(input->isSameShape(dLdO), 0,
               "BATCHNORM_BP op: wrong shape of output gradients array, "
  ...
  for (unsigned long i = 1; i < block.width() - 2; ++i)
    REQUIRE_TRUE(INPUT_VARIABLE(0)->dataType() == INPUT_VARIABLE(i)->dataType(),
                 0,
                 "BATCHNORM_BP op: types of arrays (input, mean, variance, "
                 "gamma, beta) should be the same !");
  ...
  // finally:
  // dLdI = gamma * (  stdInv * (g - g_sum/N) + (2/N) * dfdv * (dvdm/2  + (x -
  // m))  )

  // dLdG = (g * (x - m))_sum * stdInv
  // dLdB = g_sum

  // variance = input->varianceAlongDimension(variance::SummaryStatsVariance,
  // false, ShapeUtils::evalDimsToExclude(input->rankOf(), axes)); mean =
  // input->reduceAlongDimension(sd::reduce::Mean,
  // ShapeUtils::evalDimsToExclude(input->rankOf(), axes));
  ...
  const float Ninv =
      1.f * shape::tadLength(input->shapeInfo(), axes.data(), axes.size()) /
      input->lengthOf();
  ...
  xMinusMean.reduceAlongDimension(sd::reduce::Sum, *dLdM, excludedAxes,
                                  keepUnitiesInShape);
  *dLdM *= -Ninv;

  // g_sum
  auto gSum = dLdO->reduceAlongDimension(sd::reduce::Sum, excludedAxes,
                                         keepUnitiesInShape);
  ...
  // dLdV <- [g*(x - m)]_sum
  (xMinusMean * *dLdO)
      .reduceAlongDimension(sd::reduce::Sum, *dLdV, excludedAxes,
                            keepUnitiesInShape);
  ...
  *dLdV = 0;  // put zeros so far
  ...
  // std.applyTransform(transform::Reciprocal);                           // 1 /
  // (variance + epsilon) std.applyTransform(transform::Sqrt); // 1 / (variance
  // + epsilon)^0.5 NDArray xMu(input);
  ...
  // temp.reduceAlongDimension(reduce::Sum, dLdV, excludedAxes,
  // keepUnitiesInShape); *dLdV *= -0.5f * std*std*std; NDArray* dxmu1 =
  // dxhat.reduceAlongDimension(reduce::Sum, excludedAxes, keepUnitiesInShape);
  // *dxmu1 *= -std;
  // NDArray* dxmu2 = xMu.reduceAlongDimension(reduce::Sum, excludedAxes,
  // keepUnitiesInShape); *dxmu2 *=  *dLdV * (-2.f/N); NDArray dLdmu = *dxmu1 +
  // *dxmu2; dLdmu *= (1.f /N); *dLdV *= (2.f/N);
  ...
  // delete  dxmu2;
  // xHat *= *dLdO;
  // xHat.reduceAlongDimension(reduce::Sum, dLdG, excludedAxes,
  // keepUnitiesInShape);
