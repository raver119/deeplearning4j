...
CUSTOM_OP_IMPL(fused_batch_norm, 3, 3, false, 0, 2) {
  ...
  REQUIRE_TRUE(x->rankOf() == 4, 0,
               "CUSTOM_OP fused_batch_norm: the rank of input x array must be "
  ...
  REQUIRE_TRUE(scale->rankOf() == 1 && scale->sizeAt(0) == iD, 0,
               "CUSTOM_OP fused_batch_norm: wrong shape of input scale array, "
               "expected is [%i], but got %s instead",
               iD, ShapeUtils::shapeAsString(scale).c_str());
  REQUIRE_TRUE(offset->rankOf() == 1 && offset->sizeAt(0) == iD, 0,
               "CUSTOM_OP fused_batch_norm: wrong shape of input offset array, "
               "expected is [%i], but got %s instead",
  ...
  if (!isTraining) {
    ...
    REQUIRE_TRUE(mean->rankOf() == 1 && mean->sizeAt(0) == iD, 0,
                 "CUSTOM_OP fused_batch_norm: wrong shape of input mean array, "
                 "expected is [%i], but got %s instead",
                 iD, ShapeUtils::shapeAsString(mean).c_str());
    REQUIRE_TRUE(variance->rankOf() == 1 && variance->sizeAt(0) == iD, 0,
                 "CUSTOM_OP fused_batch_norm: wrong shape of input variance "
                 "array, expected is [%i], but got %s instead",
                 iD, ShapeUtils::shapeAsString(variance).c_str());
  } else {
    // REQUIRE_TRUE(block.width() == 3, 0, "CUSTOM_OP fused_batch_norm: when
    // isTraining=true then number of input arrays must be equal to 3, but got
    // %i instead !", block.width());
    std::vector<Nd4jLong> shape = {iD};
    mean = NDArrayFactory::create_(scale->ordering(), shape, scale->dataType(),
                                   block.launchContext());
    variance = NDArrayFactory::create_(
        scale->ordering(), shape, scale->dataType(), block.launchContext());
  }
  ...
  auto xAffected = NDArrayFactory::create(
      x->ordering(), {restSize, iD}, mean->dataType(), block.launchContext());
  xAffected.assign(x);
  ...
  if (isTraining) {
    ...
    *batchVar = 0.;
  xAffected *=
      (*variance + epsilon).transform(transform::RSqrt) * (*scale) + (*offset);
  y->assign(xAffected);
...
DECLARE_SHAPE_FN(fused_batch_norm) {
  ...
  REQUIRE_TRUE(scaleShapeInfo[0] == 1 && scaleShapeInfo[1] == iD, 0,
               "CUSTOM_OP fused_batch_norm: wrong shape of input scale array, "
               "expected is [%i], but got %s instead",
               iD, ShapeUtils::shapeAsString(scaleShapeInfo).c_str());

  Nd4jLong *outShapeInfo(nullptr), *batchMeanShapeInfo(nullptr),
      *batchVarShapeInfo(nullptr);
  ...
  return SHAPELIST(CONSTANT(outShapeInfo), CONSTANT(batchMeanShapeInfo),
                   CONSTANT(batchVarShapeInfo));
}
