...
CONFIGURABLE_OP_IMPL(softmax, 1, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      dim < rank, 0,
      "SOFTMAX OP: the value of input integer parameter (dimension) must be "
      "less than input array rank %i, but got dimension = %i instead !",
      rank, dim);
...
CONFIGURABLE_OP_IMPL(softmax_bp, 2, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      dim < rank, 0,
      "SOFTMAX_BP OP: the value of input integer parameter (dimension) must be "
      "less than input array rank %i, but got dimension = %i instead !",
      rank, dim);
