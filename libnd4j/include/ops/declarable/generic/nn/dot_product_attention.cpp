...
CUSTOM_OP_IMPL(dot_product_attention, 3, -1, false, 0, 2) {
  ...
  if (outputWeights) {
    ...
    auto weightShape = ShapeUtils::evalShapeForMatmul(
        keys->shapeInfo(), queries->shapeInfo(), true, false);
    weights = new NDArray('c', weightShape, values->dataType(),
                          block.launchContext());
  }
  ...
  REQUIRE_TRUE(queries->rankOf() == 3 || queries->rankOf() == 4, 0,
               "dot_product_attention: Queries, Keys and Values must be rank 3 "
               "arrays for single headed attention "
               "or rank 4 arrays for multi headed attention. But got rank = %i",
               queries->rankOf());

  REQUIRE_TRUE(queries->sizeAt(0) == keys->sizeAt(0) &&
                   keys->sizeAt(0) == values->sizeAt(0),
               0,
               "dot_product_attention: Queries, Keys and Values must have the "
               "same mini batch size. "
  ...
  REQUIRE_TRUE(queries->sizeAt(-2) == keys->sizeAt(-2), 0,
               "dot_product_attention: Queries and Keys must have the same "
               "feature size. "
               "But got queries = %i, keys = %i",
               queries->sizeAt(-2), keys->sizeAt(-2));

  REQUIRE_TRUE(keys->sizeAt(-1) == values->sizeAt(-1), 0,
               "dot_product_attention: Keys and Values must have the same "
               "timestep length. "
  ...
  if (mask != nullptr && mask->defined()) {
    ...
    if (weights->rankOf() == 4) {
      reshapedMask = mask->reshape(mask->ordering(),
                                   {mask->sizeAt(0), 1, mask->sizeAt(1), 1});
    } else {
      reshapedMask = mask->reshape(mask->ordering(),
                                   {mask->sizeAt(0), mask->sizeAt(1), 1});
    }

    // the mask is 0 for positions we want to skip, and 1 for positions we want
    // to keep. By subtracting 1 from it we get -1 for those we want to skip and
    // 0 for those we want to keep. Multiplying it by 1e9 then turns all of
    // those we want to skip into very large negative values. By adding this to
    // the weights before going through the softmax, we effectively push all
    // masked positions to zero after softmax.
    //
  ...
  softmax.execute({weights}, std::vector<NDArray *>{weights}, {}, {-2}, {}, {},
                  true);
...
DECLARE_SHAPE_FN(dot_product_attention) {
  ...
  auto output_shape = ConstantShapeHelper::getInstance()->createShapeInfo(
      sd::ArrayOptions::dataType(values_shape), 'c',
      ShapeUtils::evalShapeForMatmul(values_shape, weights_shape, false,
                                     false));
...
CUSTOM_OP_IMPL(dot_product_attention_bp, 4, 3, false, 0, 1) {
  ...
  sd::ops::matmul mmul;
  NDArray preSoftmax('c', weightShape, values->dataType(),
                     block.launchContext());
  ...
  mmul_bp.execute({keys, queries, &dLds}, std::vector<NDArray *>{dLdk, dLdq},
                  {}, {1}, {});
