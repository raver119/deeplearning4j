...
CONFIGURABLE_OP_IMPL(layer_norm, 2, 1, false, 0, -1) {
  ...
  const bool isNCHW =
      block.numB() > 0 ? B_ARG(0) : true;  // INT_ARG(9): 0-NCHW,  1-NHWC
  const int dimC = isNCHW ? 1 : input->rankOf() - 1;

  REQUIRE_TRUE(gain->rankOf() == 1 && gain->sizeAt(0) == input->sizeAt(dimC), 0,
               "LAYER_NORM OP: wrong shape of gain array, expected is {%i}, "
               "but got %s instead !",
  ...
  if (block.width() > 2) {
    ...
    REQUIRE_TRUE(bias->rankOf() == 1 && bias->sizeAt(0) == input->sizeAt(dimC),
                 0,
                 "LAYER_NORM OP: wrong shape of bias array, expected is {%i}, "
                 "but got %s instead !",
  ...
  // output->applyTrueBroadcast(sd::BroadcastOpsTuple::Multiply(), gain,
  // output);
...
CUSTOM_OP_IMPL(layer_norm_bp, 3, -1, false, 0, -1) {
  ...
  REQUIRE_TRUE(gain->rankOf() == 1 && gain->sizeAt(0) == input->sizeAt(dimC), 0,
               "LAYER_NORM_BP OP: wrong shape of gain array, expected is {%i}, "
               "but got %s instead !",
               input->sizeAt(dimC), ShapeUtils::shapeAsString(gain).c_str());
  ...
  if (bias != nullptr) {
    REQUIRE_TRUE(bias->rankOf() == 1 && bias->sizeAt(0) == input->sizeAt(dimC),
                 0,
                 "LAYER_NORM_BP OP: wrong shape of bias array, expected is "
                 "{%i}, but got %s instead !",
                 input->sizeAt(dimC), ShapeUtils::shapeAsString(bias).c_str());
    // eps->reduceAlongDimension(sd::reduce::Sum, *dLdb, {0}, true);
    eps->reduceAlongDimension(
        sd::reduce::Sum, *dLdb,
        ShapeUtils::evalDimsToExclude(input->rankOf(), {dimC}));
  }
  ...
  standardizeOp.execute(inputs, outputs, targs, longAxis, bargs);
  standardized.applyPairwiseTransform(sd::pairwise::Multiply, *eps,
                                      standardized);
  standardized.reduceAlongDimension(
      sd::reduce::Sum, *dLdg,
      ShapeUtils::evalDimsToExclude(input->rankOf(), {dimC}));
  ...
  standardizeBp.execute(standardizeBpArgs, standardizeBpOut, targs, longAxis,
                        bargs);
