...
CONFIGURABLE_OP_IMPL(prelu, 2, 1, true, 0, 0) {
  ...
  //***** input validation *****//
  std::vector<Nd4jLong> expectedAlphaShape(&inputShape[1],
                                           &inputShape[inputRank]);

  REQUIRE_TRUE(inputRank > 1, 0,
               "PRELU OP: wrong rank of input array, expected rank should be > "
               "1, but got %i instead !",
               inputRank);
  ...
  for (int i = 0; i < numSharedAxes; ++i) {
    ...
    REQUIRE_TRUE(1 <= sharedAxes[i] && sharedAxes[i] <= inputRank - 1, 0,
                 "PRELU OP: wrong axis value %i in sharedAxes at position %i, "
                 "axis value must be within range [1, input_rank-1] !",
                 sharedAxes[i], i);
  ...
  REQUIRE_TRUE(product == alphaLen, 0,
               "PRELU OP: wrong shape of alpha array, expected is %s, but got "
               "%s instead !",
               ShapeUtils::shapeAsString(expectedAlphaShape).c_str(),
               ShapeUtils::shapeAsString(alphaShape).c_str());
  // ***** end of validation ***** //

  helpers::prelu(block.launchContext(), *input,
                 alphaShape != expectedAlphaShape
                     ? alpha->reshape(alpha->ordering(), expectedAlphaShape)
                     : *alpha,
                 *output);
...
CONFIGURABLE_OP_IMPL(prelu_bp, 3, 2, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      inputRank <= MAX_RANK / 2, 0,
      "rank of input array should be <= MAX_RANK/2, but got %i instead!",
      inputRank);
  REQUIRE_TRUE(
      input->lengthOf() / alpha->lengthOf() <= MAX_RANK * 2, 0,
      "the length of input array should be no more than MAX_RANK*2 times the "
      "alpha array length, but got %lld and %lld correspondingly!",
  ...
  REQUIRE_TRUE(inputRank > 1, 0,
               "PRELU_BP OP: wrong rank of input array, expected rank should "
               "be > 1, but got %i instead !",
               inputRank);

  for (int i = 0; i < numSharedAxes; ++i) {
    if (sharedAxes[i] <= 0) sharedAxes[i] += inputRank - 1;
    REQUIRE_TRUE(1 <= sharedAxes[i] && sharedAxes[i] <= inputRank - 1, 0,
                 "PRELU_BP OP: wrong axis value %i in sharedAxes at position "
                 "%i, axis value must be within range [1, input_rank-1] !",
                 sharedAxes[i], i);
    expectedAlphaShape[sharedAxes[i] - 1] = 1;
  }
  ...
  REQUIRE_TRUE(product == alphaLen, 0,
               "PRELU_BP OP: wrong shape of alpha array, expected is %s, but "
               "got %s instead !",
               ShapeUtils::shapeAsString(expectedAlphaShape).c_str(),
               ShapeUtils::shapeAsString(alphaShape).c_str());
  // ***** end of validation ***** //
...
DECLARE_TYPES(prelu_bp) {
  getOpDescriptor()
      ...
      ->setAllowedInputTypes(
          2, {DataType::FLOAT32, DataType ::DOUBLE, DataType::HALF})
