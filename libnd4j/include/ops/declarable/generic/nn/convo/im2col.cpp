...
CUSTOM_OP_IMPL(im2col, 1, 1, false, 0, 9) {
  ...
  REQUIRE_TRUE(x->rankOf() == 4, 0,
               "im2col input should be 4D, but got %i instead", x->rankOf());
  REQUIRE_TRUE(z->rankOf() == 6, 0,
               "im2col output should be 6D, but got %i instead", z->rankOf());
  ...
  if (block.numT() > 0) zeroPadVal = T_ARG(0);

  // FIXME: zeropad value is void
  LaunchContext* ctx = block.launchContext();
  sd::ops::helpers::im2col(
      *ctx, *x, *z, kernelHeight, kernelWidth, strideY, strideX, padHeight,
      padWidth, dY, dX,
...
CUSTOM_OP_IMPL(im2col_bp, 2, 1, false, 0, 9) {
  ...
  REQUIRE_TRUE(input->rankOf() == 4, 0,
               "im2col_bp input should be 4D, but got %i instead",
               input->rankOf());
  REQUIRE_TRUE(gradAtOutput->rankOf() == 6, 0,
               "im2col_bp gradient at output (input idx 1) should be 6D, but "
               "got %i instead",
               gradAtOutput->rankOf());
  REQUIRE_TRUE(
      z->rankOf() == 4, 0,
      "im2col_bp output (grad at input) should be 4D, but got %i instead",
      z->rankOf());
  ...
  ops::helpers::col2im(*ctx, *gradAtOutput, *z, strideY, strideX, pH, pW, imgH,
                       imgW, dY, dX);
