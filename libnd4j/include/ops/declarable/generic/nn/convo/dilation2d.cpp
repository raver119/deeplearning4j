...
CUSTOM_OP_IMPL(dilation2d, 2, 1, false, 0, 1) {
  ...
  REQUIRE_TRUE(input->sizeAt(3) == weights->sizeAt(2), 0,
               "Dilation2D: number of input channels doesn't match number of "
               "channels in weights: %i vs %i",
  ...
  if (block.width() > 2) {
    REQUIRE_TRUE(block.width() >= 4, 0,
                 "Dilation2D: number of input arrays should be 4 at least");
    ...
    REQUIRE_TRUE(block.numI() >= 9, 0,
                 "Dilation2D: number of Int arguments should be 9 at least");
  ...
  helpers::dilation_hw(block.launchContext(), input->shapeInfo(),
                       weights->shapeInfo(), strides, rates, isSameShape, &sH,
                       &sW, &pH, &pW, &dH, &dW, &oH, &oW);

  REQUIRE_TRUE(oH > 0 && oW > 0, 0,
               "Dilation2D: outY and outX should have positive values, but got "
               "[%i, %i] instead",
               oH, oW);

  helpers::dilation2d(block.launchContext(), input, weights, output, sH, sW, pH,
                      pW, dH, dW);
...
DECLARE_SHAPE_FN(dilation2d) {
  ...
  helpers::dilation_hw(block.launchContext(), input, weights, strides, rates,
