...
CUSTOM_OP_IMPL(col2im, 1, 1, false, 0, 9) {
  ...
  REQUIRE_TRUE(x->rankOf() == 6, 0,
               "col2im input should be 6D, but got %i instead", x->rankOf());
  REQUIRE_TRUE(z->rankOf() == 4, 0,
               "col2im output should be 4D, but got %i instead", z->rankOf());
  ...
  helpers::col2im(*ctx, *x, *z, strideY, strideX, padHeight, padWidth,
                  imgHeight, imgWidth, dY, dX);
