...
CUSTOM_OP_IMPL(embedding_lookup, 2, 1, false, 0, 1) {
  ...
  if (block.width() > 2) {  // multiple input
    ...
    REQUIRE_TRUE(
        block.width() > output->sizeAt(0), 0,
        "embedding_lookup: input list should be greater then %i, but %i given.",
    ...
    REQUIRE_TRUE(indexRank > 0, 0,
                 "embeded_lookup: input array of indexes can't be single "
                 "scalar, the requirement is: rank > 0 !");

    int inputRank = input->rankOf();
    int lastIndDim = indeces->lengthOf();
    int partition_mode =
        INT_ARG(0);  // partition_mode == 0 - i.e. 'mod' , 1 - 'div'

    sd::ops::gather op;

    auto result(op.evaluate({input, indeces}, {0}));
    REQUIRE_TRUE(result.status() == Status::OK(), 0,
                 "embedding_lookup: cannot retrieve results from gather op.");
    REQUIRE_TRUE(result.at(0).isSameShape(output), 0,
                 "embedding_lookup: wrong shape of return from gather op.");
