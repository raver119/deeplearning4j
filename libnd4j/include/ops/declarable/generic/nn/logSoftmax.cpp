...
CONFIGURABLE_OP_IMPL(log_softmax, 1, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      dim < rank, 0,
      "LOG_SOFTMAX OP: the value of input integer parameter (dimension) must "
      "be less than input array rank %i, but got dimension = %i instead !",
      rank, dim);
...
CONFIGURABLE_OP_IMPL(log_softmax_bp, 2, 1, true, 0, 0) {
  ...
  REQUIRE_TRUE(
      dim < rank, 0,
      "LOG_SOFTMAX_BP OP: the value of input integer parameter (dimension) "
  ...
  gradI->assign(
      *gradO -
      (*gradI * *gradO).reduceAlongDimension(reduce::Sum, {dim}, true));
