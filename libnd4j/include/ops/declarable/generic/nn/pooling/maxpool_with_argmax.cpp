...
CUSTOM_OP_IMPL(max_pool_with_argmax, 1, 2, false, 0, 9) {
  ...
  REQUIRE_TRUE(
      x->rankOf() == 4, 0,
      "max_pool_with_argmax: Input should have rank of 4, but got %i instead",
      x->rankOf());
...
DECLARE_SHAPE_FN(max_pool_with_argmax) {
  auto in = inputShape->at(0);auto dtype = block.numD() ? D_ARG(0) : sd::DataType::INT64;
  auto valuesShape =
