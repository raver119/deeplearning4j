...
CUSTOM_OP_IMPL(xw_plus_b, 3, 1, false, 0, 0) {
  ...
  auto w = bTranspose ? new NDArray(INPUT_VARIABLE(1)->transpose())
                      : INPUT_VARIABLE(1);

  REQUIRE_TRUE(
      x->rankOf() == 2, 0,
      "xw_plus_b: Input x array should have rank equal 2, but got instead %i!",
      x->rankOf());
  REQUIRE_TRUE(w->rankOf() == 2, 0,
               "xw_plus_b: Input weights array should have rank equal 2, but "
               "got instead %i!",
               w->rankOf());
  REQUIRE_TRUE(
      z->rankOf() == 2, 0,
      "xw_plus_b: Output array should have rank equal 2, but got instead %i!",
      z->rankOf());

  REQUIRE_TRUE(1 == b->rankOf() && b->lengthOf() == z->sizeAt(-1), 0,
               "xw_plus_b: Input bias vector should be 1D and have proper "
               "dimension 1x%i."
...
DECLARE_SHAPE_FN(xw_plus_b) {
  ...
  auto weightsShape = (1 == nWeightsFormat) ? ShapeUtils::evalTranspShapeInfo(
                                                  *weights, block.workspace())
                                            : inputShape->at(1);

  auto outputShape = ShapeUtils::matrixProductShape(
      inputShape->at(0), weightsShape, false, false,
...
CUSTOM_OP_IMPL(xw_plus_b_bp, 4, 3, false, 0, 0) {
  ...
  REQUIRE_TRUE(x->rankOf() == 2, 0,
               "xw_plus_b BP: Input x array should have rank equal 2, but got "
               "instead %i!",
               x->rankOf());
  REQUIRE_TRUE(w->rankOf() == 2, 0,
               "xw_plus_b BP: Input weights array should have rank equal 2, "
               "but got instead %i!",
               w->rankOf());
  REQUIRE_TRUE(dLdz->rankOf() == 2, 0,
               "xw_plus_b BP: Output array should have rank equal 2, but got "
               "instead %i!",
               dLdz->rankOf());
  REQUIRE_TRUE(1 == b->rankOf() && b->lengthOf() == dLdz->sizeAt(-1), 0,
               "xw_plus_b BP: Input bias vector should be 1D and have proper "
               "dimension 1x%i."
               " But got rank %i, and got length %i instead %i.",
               dLdz->sizeAt(-1), b->rankOf(), b->lengthOf(), dLdz->sizeAt(-1));

  auto dLdw = (bTranspose) ? new NDArray(OUTPUT_VARIABLE(1)->transpose())
                           : OUTPUT_VARIABLE(1);
...
DECLARE_SHAPE_FN(xw_plus_b_bp) {
  ...
  return SHAPELIST(CONSTANT(xShapeInfo), CONSTANT(wShapeInfo),
                   CONSTANT(bShapeInfo));
}
