...
CUSTOM_OP_IMPL(gruCell, 6, 4, false, 0, 0) {
  ...
  auto hLast =
      INPUT_VARIABLE(1);  // previous cell output [bS, nU],  that is at previous
                          // time step t-1, nU - number of units
  auto Wru = INPUT_VARIABLE(2);  // RU weights - [nIn+nU, 2*nU] - reset and
                                 // update gates (input/recurrent weights)
  auto Wc = INPUT_VARIABLE(
      3);  // C weights - [nIn+nU, nU] - cell gate (input/recurrent weights)
  auto bru = INPUT_VARIABLE(
      4);  // reset and update biases, [2*nU] - reset and update gates
  ...
  REQUIRE_TRUE(x->rankOf() == 2 && hLast->rankOf() == 2, 0,
               "gruCell: Input ranks must be 2 for inputs 0 and 1 (x, hLast) - "
               "got %i, %i",
               x->rankOf(), hLast->rankOf());
  ...
  REQUIRE_TRUE(x->sizeAt(0) == hLast->sizeAt(0), 0,
               "gruCell: Input minibatch sizes (dimension 0) must be same for "
               "x and hLast");
  REQUIRE_TRUE(
      Wru->rankOf() == 2 && Wc->rankOf() == 2, 0,
      "gruCell: weight arrays (Wru, Wc) arrays must be 2, got %i and %i",
      Wru->rankOf(), Wc->rankOf());
  REQUIRE_TRUE(Wru->sizeAt(0) == (nIn + nU) && Wc->sizeAt(0) == (nIn + nU), 0,
               "gruCell: Weights size(0) must be equal to nIn + nU, got %i",
               Wru->sizeAt(0));
  REQUIRE_TRUE(Wru->sizeAt(1) == (2 * nU), 0,
               "gruCell: Weights (reset and update) size(1) must be equal to "
               "2*nU, got %i",
               Wru->sizeAt(1));
  REQUIRE_TRUE(Wc->sizeAt(1) == nU, 0,
               "gruCell: Weights (cell) size(1) must be equal to nU, got %i",
               Wc->sizeAt(1));
  REQUIRE_TRUE(bru->rankOf() == 1 && bru->sizeAt(0) == (2 * nU), 0,
               "gruCell: reset/update biases must be rank 1, size 2*nU");
  REQUIRE_TRUE(bc->rankOf() == 1 && bc->sizeAt(0) == nU, 0,
               "gruCell: cell biases must be rank 1, size nU");
  REQUIRE_TRUE(
      r->rankOf() == 2 && u->rankOf() == 2 && c->rankOf() == 2 &&
          h->rankOf() == 2 && r->sizeAt(0) == bS && u->sizeAt(0) == bS &&
          c->sizeAt(0) == bS && h->sizeAt(0) == bS && r->sizeAt(1) == nU &&
          u->sizeAt(1) == nU && c->sizeAt(1) == nU && h->sizeAt(1) == nU,
      0,
      "gruCell: Output arrays must all be rank 2 with size(0) == batchSize and "
      "size(1) == nU");

  helpers::gruCell(block.launchContext(), x, hLast, Wru, Wc, bru, bc, r, u, c,
                   h);
...
DECLARE_SHAPE_FN(gruCell) {
  ...
  auto hLast = inputShape->at(
      1);  // previous cell output [bS x nU],  that is at previous time step t-1
  auto Wru = inputShape->at(2);  // RU weights - [(nIn+nU), 2*nU] - reset and
                                 // update gates (input/recurrent weights)
  auto Wc = inputShape->at(
      3);  // C weights - [(nIn+nU), nU] - cell gate (input/recurrent weights)
  auto bru = inputShape->at(
      4);  // reset and update biases, [2*nU] - reset and update gates
  ...
  REQUIRE_TRUE(x[1] == hLast[1], 0,
               "gruCell: Input minibatch sizes (dimension 0) must be same for "
               "x and hLast");
  REQUIRE_TRUE(
      shape::rank(Wru) == 2 && shape::rank(Wc) == 2, 0,
      "gruCell: weight arrays (Wru, Wc) arrays must be 2, got %i and %i",
      shape::rank(Wru), shape::rank(Wc));
  REQUIRE_TRUE(
      Wru[1] == (nIn + nU) && Wc[1] == (nIn + nU), 0,
      "gruCell: Weights size(0) must be equal to nIn + nU, got %i and %i",
      Wru[1], Wc[1]);
  REQUIRE_TRUE(Wru[2] == (2 * nU), 0,
               "gruCell: Weights (reset and update) size(1) must be equal to "
               "2*nU, got %i",
               Wru[2]);
  REQUIRE_TRUE(Wc[2] == nU, 0,
               "gruCell: Weights (cell) size(1) must be equal to nU, got %i",
               Wc[2]);
  REQUIRE_TRUE(shape::rank(bru) == 1 && bru[1] == (2 * nU), 0,
               "gruCell: reset/update biases must be rank 1, size 2*nU");
  REQUIRE_TRUE(shape::rank(bc) == 1 && bc[1] == nU, 0,
               "gruCell: cell biases must be rank 1, size nU");

  Nd4jLong *s0(nullptr);
  ALLOCATE(s0, block.workspace(), shape::shapeInfoLength(rank),
           Nd4jLong);  // [bS x nU]
  ...
  auto ts0 = ConstantShapeHelper::getInstance().createFromExisting(
      s0, block.workspace());
...
CUSTOM_OP_IMPL(gruCell_bp, 10, 6, false, 0, 0) {
  ...
  REQUIRE_TRUE(
      x->rankOf() == 2, 0,
      "GRU_CELL_BP: rank of input array x must be 2, but got %i instead",
      x->rankOf());
  ...
  REQUIRE_TRUE(hi->isSameShape(hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of previous cell output array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(hi).c_str());
  REQUIRE_TRUE(W->isSameShape(wCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of weights array, expected is %s, "
               "but got %s instead !",
               ShapeUtils::shapeAsString(wCorrectShape).c_str(),
               ShapeUtils::shapeAsString(W).c_str());
  REQUIRE_TRUE(Wc->isSameShape(wcCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of c weights array, expected is "
               "%s, but got %s instead !",
               ShapeUtils::shapeAsString(wcCorrectShape).c_str(),
               ShapeUtils::shapeAsString(Wc).c_str());
  REQUIRE_TRUE(b->isSameShape(bCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of biases array, expected is %s, "
               "but got %s instead !",
               ShapeUtils::shapeAsString(bCorrectShape).c_str(),
               ShapeUtils::shapeAsString(b).c_str());
  REQUIRE_TRUE(bc->isSameShape(bcCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of c biases array, expected is %s, "
               "but got %s instead !",
               ShapeUtils::shapeAsString(bcCorrectShape).c_str(),
               ShapeUtils::shapeAsString(bc).c_str());
  REQUIRE_TRUE(dLdr->isSameShape(hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdr array (gradient wrt reset "
               "gate), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdr).c_str());
  REQUIRE_TRUE(dLdu->isSameShape(hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdu array (gradient wrt update "
               "gate), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdu).c_str());
  REQUIRE_TRUE(dLdc->isSameShape(hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdc array (gradient wrt cell "
               "state), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdc).c_str());
  REQUIRE_TRUE(dLdh->isSameShape(hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdh array (gradient wrt "
               "current cell output), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdh).c_str());

  helpers::gruCellBp(block.launchContext(), x, hi, W, Wc, b, bc, dLdr, dLdu,
                     dLdc, dLdh, dLdx, dLdhi, dLdW, dLdWc, dLdb, dLdbc);
...
DECLARE_SHAPE_FN(gruCell_bp) {
  ...
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(hiShapeInfo, hiCorrectShape), 0,
               ...
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(hiShapeInfo).c_str());
  ...
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(wcShapeInfo, wcCorrectShape), 0,
               ...
               "%s, but got %s instead !",
               ShapeUtils::shapeAsString(wcCorrectShape).c_str(),
               ShapeUtils::shapeAsString(wcShapeInfo).c_str());
  ...
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(bcShapeInfo, bcCorrectShape), 0,
               ...
               "but got %s instead !",
               ShapeUtils::shapeAsString(bcCorrectShape).c_str(),
               ShapeUtils::shapeAsString(bcShapeInfo).c_str());
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(dLdrShapeInfo, hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdr array (gradient wrt reset "
               "gate), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdrShapeInfo).c_str());
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(dLduShapeInfo, hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdu array (gradient wrt update "
               "gate), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLduShapeInfo).c_str());
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(dLdcShapeInfo, hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdc array (gradient wrt cell "
               "state), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdcShapeInfo).c_str());
  REQUIRE_TRUE(ShapeUtils::areShapesEqual(dLdhShapeInfo, hiCorrectShape), 0,
               "GRU_CELL_BP op: wrong shape of dLdh array (gradient wrt "
               "current cell output), expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hiCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdhShapeInfo).c_str());
  ...
  return SHAPELIST(CONSTANT(dLdxShapeInfo), CONSTANT(dLdhiShapeInfo),
                   CONSTANT(dLdWShapeInfo), CONSTANT(dLdWcShapeInfo),
