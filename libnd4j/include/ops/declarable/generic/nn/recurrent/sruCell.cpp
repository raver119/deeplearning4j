...
CUSTOM_OP_IMPL(sruCell, 4, 2, false, 0, 0) {
  auto xt = INPUT_VARIABLE(
      0);  // input [bS x inSize], bS - batch size, inSize - number of features
  auto ct_1 = INPUT_VARIABLE(1);  // previous cell state ct  [bS x inSize], that
  ...
  auto ht = OUTPUT_VARIABLE(
      0);  // current cell output [bS x inSize], that is at current time step t
  auto ct = OUTPUT_VARIABLE(
      1);  // current cell state  [bS x inSize], that is at current time step t
  ...
  REQUIRE_TRUE(ct_1->isSameShape(correctCt_1Shape), 0,
               "SRUCELL operation: wrong shape of previous cell state, "
  ...
  REQUIRE_TRUE(w->isSameShape(correctWShape), 0,
               "SRUCELL operation: wrong shape of weights, expected is %s, but "
               "got %s instead !",
               ShapeUtils::shapeAsString(correctWShape).c_str(),
               ShapeUtils::shapeAsString(w).c_str());
  REQUIRE_TRUE(b->isSameShape(correctBShape), 0,
               "SRUCELL operation: wrong shape of biases, expected is %s, but "
               "got %s instead !",
...
DECLARE_SHAPE_FN(sruCell) {
  auto xtShapeInfo = inputShape->at(
      0);  // input [bS x inSize], bS - batch size, inSize - number of features
  auto ct_1ShapeInfo =
      inputShape->at(1);  // previous cell state ct  [bS x inSize], that is at
  ...
  ShapeUtils::updateStridesAndType(hShapeInfo, ct_1ShapeInfo,
