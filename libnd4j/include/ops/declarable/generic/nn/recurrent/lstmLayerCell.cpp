...
CUSTOM_OP_IMPL(lstmLayerCell, 5, 2, false, 1, 3) {
  ...
  const auto gateAct =
      INT_ARG(0);  // activation for input (i), forget (f) and output (o) gates
  ...
  const auto hasPH =
      B_ARG(1);  // indicates whether peephole connections are present
  ...
  REQUIRE_TRUE(cellClip >= 0, 0,
               "LSTM_LAYER_CELL operation: cell clipping value should be "
               "nonnegative (>=0) !");
  ...
  if (Wx->rankOf() != 2 || Wx->sizeAt(0) != nIn)
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL operation: wrong shape of input weights, "
  ...
  if (Wr->rankOf() != 2 || Wr->sizeAt(0) != nOut || Wr->sizeAt(1) != 4 * nOut)
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL operation: wrong shape of recurrent weights, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({nOut, 4 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(Wr).c_str());
  // initial output/cell validation
  std::vector<Nd4jLong> exphIcIShape = x->rankOf() == 1
                                           ? std::vector<Nd4jLong>{nOut}
                                           : std::vector<Nd4jLong>{bS, nOut};
  REQUIRE_TRUE(hI->isSameShape(exphIcIShape), 0,
               "LSTM_LAYER_CELL operation: wrong shape of initial output, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(exphIcIShape).c_str(),
               ShapeUtils::shapeAsString(hI).c_str());
  REQUIRE_TRUE(cI->isSameShape(exphIcIShape), 0,
               "LSTM_LAYER_CELL operation: wrong shape of initial cell state, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(exphIcIShape).c_str(),
  ...
  if (b != nullptr && (b->rankOf() != 1 || b->sizeAt(0) != 4 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL operation: wrong shape of biases, expected "
  ...
  if (Wp != nullptr && (Wp->rankOf() != 1 || Wp->sizeAt(0) != 3 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL operation: wrong shape of peephole weights, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({3 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(Wp).c_str());

  std::vector<float> params = {
      static_cast<float>(0) /*ignore*/, static_cast<float>(0) /*ignore*/,
...
CUSTOM_OP_IMPL(lstmLayerCellBp, 7, 5, false, 1, 3) {
  ...
  REQUIRE_TRUE(cellClip >= 0, 0,
               "LSTM_LAYER_CELL_BP operation: cell clipping value should be "
               "nonnegative (>=0) !");
  ...
  if (Wx->rankOf() != 2 || Wx->sizeAt(0) != nIn)
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of input weights, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({nIn, 4 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(Wx).c_str());
  // Wr validation
  if (Wr->rankOf() != 2 || Wr->sizeAt(0) != nOut || Wr->sizeAt(1) != 4 * nOut)
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of recurrent "
  ...
  std::vector<Nd4jLong> exphIcIShape = x->rankOf() == 1
                                           ...
                                           : std::vector<Nd4jLong>{bS, nOut};
  REQUIRE_TRUE(hI->isSameShape(exphIcIShape), 0,
               "LSTM_LAYER_CELL_BP operation: wrong shape of initial output, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(exphIcIShape).c_str(),
               ShapeUtils::shapeAsString(hI).c_str());
  REQUIRE_TRUE(cI->isSameShape(exphIcIShape), 0,
               "LSTM_LAYER_CELL_BP operation: wrong shape of initial cell "
               "state, expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(exphIcIShape).c_str(),
               ShapeUtils::shapeAsString(cI).c_str());
  REQUIRE_TRUE(dLdh->isSameShape(exphIcIShape), 0,
               "LSTM_LAYER_CELL_BP operation: wrong shape of dLdh gradient, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(exphIcIShape).c_str(),
  ...
  if (b != nullptr && (b->rankOf() != 1 || b->sizeAt(0) != 4 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of biases, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({4 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(b).c_str());
  if (dLdb != nullptr && (dLdb->rankOf() != 1 || dLdb->sizeAt(0) != 4 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of dLdb gradient, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({4 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(dLdb).c_str());
  // peephole weights validation
  if (Wp != nullptr && (Wp->rankOf() != 1 || Wp->sizeAt(0) != 3 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of peephole "
                 "weights, expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({3 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(Wp).c_str());
  if (dLdWp != nullptr &&
      (dLdWp->rankOf() != 1 || dLdWp->sizeAt(0) != 3 * nOut))
    REQUIRE_TRUE(false, 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of dLdWp gradient, "
                 "expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString({3 * nOut}).c_str(),
                 ShapeUtils::shapeAsString(dLdWp).c_str());
  ...
  std::vector<Nd4jLong> zShape = x->rankOf() == 1
                                     ? std::vector<Nd4jLong>({4 * nOut})
                                     : std::vector<Nd4jLong>({bS, 4 * nOut});
  ...
  helpers::lstmLayerCellBp(x, Wx, Wr, b, hI, cI, Wp, dLdh, nullptr, nullptr, &z,
                           &a, &c, params, dLdx, dLdWx, dLdWr, dLdhI, dLdcI,
                           dLdb, dLdWp);
