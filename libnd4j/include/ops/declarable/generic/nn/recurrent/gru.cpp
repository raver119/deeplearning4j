...
CUSTOM_OP_IMPL(gru, 5, 1, false, 0, 0) {
  ...
  auto hI =
      INPUT_VARIABLE(1);  // initial cell output (at time step = 0) [bS, nOut]
  ...
  auto h = OUTPUT_VARIABLE(
      0);  // cell outputs [time, bS, nOut], that is per each time step
  ...
  REQUIRE_TRUE(hI->isSameShape(h0CorrectShape), 0,
               "GRU operation: wrong shape of previous cell output array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(h0CorrectShape).c_str(),
               ShapeUtils::shapeAsString(hI).c_str());
  REQUIRE_TRUE(Wx->isSameShape(wxCorrectShape), 0,
               "GRU operation: wrong shape of input-to-hidden weights array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(wxCorrectShape).c_str(),
               ShapeUtils::shapeAsString(Wx).c_str());
  REQUIRE_TRUE(Wh->isSameShape(whCorrectShape), 0,
               "GRU operation: wrong shape of hidden-to-hidden weights array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(whCorrectShape).c_str(),
               ShapeUtils::shapeAsString(Wh).c_str());
  REQUIRE_TRUE(b->isSameShape(bCorrectShape), 0,
               "GRU operation: wrong shape of biases array, expected is %s, "
               "but got %s instead !",
...
CUSTOM_OP_IMPL(gru_bp, 6, 5, false, 0, 0) {
  ...
  auto dLdhI =
      OUTPUT_NULLIFIED(1);  // gradient vs. initial cell output, [bS, nOut]
  auto dLdWx = OUTPUT_NULLIFIED(
      2);  // gradient vs. input-to-hidden  weights, [nIn, 3*nOut]
  auto dLdWh = OUTPUT_NULLIFIED(
      3);  // gradient vs. hidden-to-hidden weights, [nOut, 3*nOut]
  ...
  REQUIRE_TRUE(hI->isSameShape(h0CorrectShape), 0,
               "GRU_BP operation: wrong shape of previous cell output array, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(h0CorrectShape).c_str(),
               ShapeUtils::shapeAsString(hI).c_str());
  REQUIRE_TRUE(Wx->isSameShape(wxCorrectShape), 0,
               "GRU_BP operation: wrong shape of input-to-hidden weights "
               "array, expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(wxCorrectShape).c_str(),
               ShapeUtils::shapeAsString(Wx).c_str());
  REQUIRE_TRUE(Wh->isSameShape(whCorrectShape), 0,
               "GRU_BP operation: wrong shape of hidden-to-hidden weights "
               "array, expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(whCorrectShape).c_str(),
               ShapeUtils::shapeAsString(Wh).c_str());
  REQUIRE_TRUE(b->isSameShape(bCorrectShape), 0,
               "GRU_BP operation: wrong shape of biases array, expected is %s, "
               "but got %s instead !",
               ShapeUtils::shapeAsString(bCorrectShape).c_str(),
               ShapeUtils::shapeAsString(b).c_str());
  REQUIRE_TRUE(dLdh->isSameShape(hCorrectShape), 0,
               "GRU_BP operation: wrong shape of gradient vs. ff output, "
               "expected is %s, but got %s instead !",
               ShapeUtils::shapeAsString(hCorrectShape).c_str(),
               ShapeUtils::shapeAsString(dLdh).c_str());

  helpers::gruTimeLoopBp(block.launchContext(), x, hI, Wx, Wh, b, dLdh, dLdx,
                         dLdhI, dLdWx, dLdWh, dLdb);
