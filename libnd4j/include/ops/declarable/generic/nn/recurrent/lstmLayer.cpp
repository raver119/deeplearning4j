...
CUSTOM_OP_IMPL(lstmLayer, 3, 1, false, 1, 5) {
  ...
  const auto dataFormat =
      INT_ARG(0);  // for unidirectional: 0 = [sL, bS, nIn], 1 = [bS, sL ,nIn],
                   // 2 = [bS, nIn, sL], for bidirectional: 3 = [sL, bS, nIn] &&
                   // [sL, 2, bS, nOut] (for ONNX)
  ...
  // integer numbers corresponding to activations: 0=tanh, 1=relu, 2=sigmoid,
  // 3=affine, 4=leaky relu, 5= thresholded relu, 6=scaled tanh, 7=hard sigmoid,
  // 8=ELU, 9=softsign, 10=softplus
  const auto gateAct =
      INT_ARG(2);  // activation for input (i), forget (f) and output (o) gates
  ...
  const auto retLastC = B_ARG(
      7);  // indicates whether to return cells state at last time step only

  const auto gateActHasAlpha = gateAct == 3 || gateAct == 4 || gateAct == 5 ||
                               gateAct == 6 || gateAct == 8;
  const auto cellActHasAlpha = cellAct == 3 || cellAct == 4 || cellAct == 5 ||
                               cellAct == 6 || cellAct == 8;
  const auto outActHasAlpha =
      outAct == 3 || outAct == 4 || outAct == 5 || outAct == 6 || outAct == 8;
  ...
  REQUIRE_TRUE(
      dataFormat < 3 || (dataFormat == 3 && directionMode == 4), 0,
      "LSTM_LAYER operation: if argument dataFormat = 3, then directionMode = "
      "4, but got dataFormat = %i and directionMode = %i instead !",
      dataFormat, directionMode);
  REQUIRE_TRUE(cellClip >= 0, 0,
               "LSTM_LAYER operation: cell clipping value should be "
               "nonnegative (>=0) !");
  REQUIRE_TRUE(
  ...
  if (directionMode < 2) {  // no bidirectional
    ...
    if (Wx->rankOf() != 2 || Wx->sizeAt(0) != nIn)
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong shape of input weights, "
    ...
    if (Wr->rankOf() != 2 || Wr->sizeAt(0) != nOut || Wr->sizeAt(1) != 4 * nOut)
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong shape of recurrent weights, "
                   "expected is %s, but got %s instead !",
    ...
    if (b != nullptr && (b->rankOf() != 1 || b->sizeAt(0) != 4 * nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong shape of biases, expected is "
                   "%s, but got %s instead !",
    ...
    if (hI != nullptr &&
      ...
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong shape of initial output, "
                   "expected is %s, but got %s instead !",
    ...
    if (cI != nullptr &&
      ...
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong shape of initial cell state, "
                   "expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({bS, nOut}).c_str(),
    ...
    if (Wp != nullptr && (Wp->rankOf() != 1 || Wp->sizeAt(0) != 3 * nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong peephole weights, expected is "
                   "%s, but got %s instead !",
                   ShapeUtils::shapeAsString({3 * nOut}).c_str(),
                   ShapeUtils::shapeAsString(Wp).c_str());
  } else {  // bidirectional
            // Wx validation
    ...
    // peephole weights validation
    if (Wp != nullptr &&
        (Wp->rankOf() != 2 || Wp->sizeAt(0) != 2 || Wp->sizeAt(1) != 3 * nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER operation: wrong peephole weights, expected is "
                   "%s, but got %s instead !",
                   ShapeUtils::shapeAsString({2, 3 * nOut}).c_str(),
                   ShapeUtils::shapeAsString(Wp).c_str());
  }

  std::vector<float> params = {
      static_cast<float>(dataFormat), static_cast<float>(directionMode),
      static_cast<float>(cellClip),   static_cast<float>(gateAct),
      static_cast<float>(gateAlpha),  static_cast<float>(gateBeta),
      static_cast<float>(cellAct),    static_cast<float>(cellAlpha),
      static_cast<float>(cellBeta),   static_cast<float>(outAct),
      static_cast<float>(outAlpha),   static_cast<float>(outBeta)};

  if (directionMode == 0) {  // forward

    helpers::lstmLayerTimeLoop(x, Wx, Wr, b, seqLen, hI, cI, Wp, params, true,
                               h, hL, cL);
  } else if (directionMode == 1) {  // backward

    helpers::lstmLayerTimeLoop(x, Wx, Wr, b, seqLen, hI, cI, Wp, params, false,
                               h, hL, cL);
  } else {  // bidirectional
    ...
    NDArray *WpFwd(nullptr), *WpBwd(nullptr), *bFwd(nullptr), *bBwd(nullptr),
        *hIFwd(nullptr), *hIBwd(nullptr), *cIFwd(nullptr), *cIBwd(nullptr),
        *hLFwd(nullptr), *hLBwd(nullptr), *cLFwd(nullptr), *cLBwd(nullptr),
    ...
    if (h) {
      if (directionMode == 2) {  // sum
        ...
        hFwd = new NDArray(dataFormat <= 1 ? (*h)({0, 0, 0, 0, 0, nOut})
                                           : (*h)({0, 0, 0, nOut, 0, 0}));
        hBwd =
            new NDArray(dataFormat <= 1 ? (*h)({0, 0, 0, 0, nOut, 2 * nOut})
                                        : (*h)({0, 0, nOut, 2 * nOut, 0, 0}));
    ...
    // FIXME - following two calls are independent and may run in different
    // streams
    helpers::lstmLayerTimeLoop(x, &WxFwd, &WrFwd, bFwd, seqLen, hIFwd, cIFwd,
                               WpFwd, params, true, hFwd, hLFwd, cLFwd);
    helpers::lstmLayerTimeLoop(x, &WxBwd, &WrBwd, bBwd, seqLen, hIBwd, cIBwd,
                               WpBwd, params, false, hBwd, hLBwd, cLBwd);
...
DECLARE_SHAPE_FN(lstmLayer) {
  const auto dataFormat = INT_ARG(
      0);  // for unidirectional: 0 = [sL, bS, nIn], 1 = [bS, sL ,nIn], 2 = [bS,
           // nIn, sL], for bidirectional: 3 = [sL, 2, bS, nIn] (for ONNX)
  const auto directionMode =
      INT_ARG(1);  // direction: 0 = fwd, 1 = bwd, 2 = bidirectional sum, 3 =
                   // bidirectional concat, 4 = bidirectional extra output dim

  const auto retFullSeq =
      B_ARG(5);  // indicates whether to return whole h {h_0, h_1, ... ,
                 // h_sL-1}, if true, format would be [sL,bS,nOut] (exact shape
...
CUSTOM_OP_IMPL(lstmLayer_bp, 4, 1, false, 1, 5) {
  ...
  // *******
  // gradient vs cell state at last time step dLdcL(same shape as in dLdhL),
  // optional: 1) [bS, nOut]    when directionMode <  2 2) [2, bS, nOut] when
  // directionMode >= 2
  ...
  // gradient vs. sequence length array dLdsL, optional (do not calculate
  // it!!!): 1) [bS] always
  ...
  const auto hasPH =
      B_ARG(4);  // indicates whether peephole connections are present
  const auto retFullSeq =
      B_ARG(5);  // indicates whether gradient vs. outputs is given for whole
                 // time sequence dLdh {dLdh_0, dLdh_1, ... , dLdh_sL-1}
  const auto retLastH = B_ARG(6);  // indicates whether gradient vs. output at
                                   // last time step (dLdhL) is given
  const auto retLastC = B_ARG(7);  // indicates whether gradient vs. cell state
                                   // at last time step (dLdcL) is given
  ...
  REQUIRE_TRUE(
      dataFormat < 3 || (dataFormat == 3 && directionMode == 4), 0,
      "LSTM_LAYER_BP operation: if argument dataFormat = 3, then directionMode "
      "= 4, but got dataFormat = %i and directionMode = %i instead !",
      dataFormat, directionMode);
  REQUIRE_TRUE(cellClip >= 0, 0,
               "LSTM_LAYER_BP operation: cell clipping value should be "
               "nonnegative (>=0) !");
  REQUIRE_TRUE(retFullSeq || retLastH || retLastC, 0,
               "LSTM_LAYER_BP operation: please specify at least one of three "
               "input gradient arrays: dLdh, dLdhL or dLdcL !");
  ...
  const auto Wp =
      hasPH ? INPUT_VARIABLE(count++) : nullptr;  // peephole weights
  const auto dLdh =
      retFullSeq ? INPUT_VARIABLE(count++) : nullptr;  // gradient vs. output
  const auto dLdhL = retLastH
                         ? INPUT_VARIABLE(count++)
                         : nullptr;  // gradient vs. output at last time step
  const auto dLdcL =
      retLastC ? INPUT_VARIABLE(count++)
               : nullptr;  // gradient vs. cell state at last time step
  ...
  auto dLdb =
      hasBiases ? OUTPUT_NULLIFIED(count++) : nullptr;  // gradient vs. biases
  auto dLdsL =
      hasSeqLen
          ? INPUT_VARIABLE(count++)
          : nullptr;  // gradient vs. seqLen vector, we don't calculate it !!!
  auto dLdhI = hasInitH ? OUTPUT_NULLIFIED(count++)
                        : nullptr;  // gradient vs. initial output
  auto dLdcI = hasInitC ? OUTPUT_NULLIFIED(count++)
                        : nullptr;  // gradient vs. initial cell state
  auto dLdWp = hasPH ? OUTPUT_NULLIFIED(count)
                     : nullptr;  // gradient vs. peephole weights
  ...
  if (directionMode < 2) {  // no bidirectional
    ...
    if (Wx->rankOf() != 2 || Wx->sizeAt(0) != nIn)
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of input weights, "
                   "expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({nIn, 4 * nOut}).c_str(),
                   ShapeUtils::shapeAsString(Wx).c_str());
    // Wr validation
    if (Wr->rankOf() != 2 || Wr->sizeAt(0) != nOut || Wr->sizeAt(1) != 4 * nOut)
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of recurrent weights, "
                   "expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({nOut, 4 * nOut}).c_str(),
                   ShapeUtils::shapeAsString(Wr).c_str());
    // biases validation
    if (b != nullptr && (b->rankOf() != 1 || b->sizeAt(0) != 4 * nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of biases, expected "
    ...
    if (hI != nullptr &&
        (hI->rankOf() != 2 || hI->sizeAt(0) != bS || hI->sizeAt(1) != nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of initial output, "
                   "expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({bS, nOut}).c_str(),
                   ShapeUtils::shapeAsString(hI).c_str());
    // initial cell  validation
    if (cI != nullptr &&
        (cI->rankOf() != 2 || cI->sizeAt(0) != bS || cI->sizeAt(1) != nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of initial cell "
    ...
    if (Wp != nullptr && (Wp->rankOf() != 1 || Wp->sizeAt(0) != 3 * nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong peephole weights, expected "
                   "is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({3 * nOut}).c_str(),
                   ShapeUtils::shapeAsString(Wp).c_str());
    // gradient vs. output at last time step validation
    if (dLdhL != nullptr && (dLdhL->rankOf() != 2 || dLdhL->sizeAt(0) != bS ||
                             dLdhL->sizeAt(1) != nOut))
      REQUIRE_TRUE(
          false, 0,
          "LSTM_LAYER_BP operation: wrong shape of gradient vs. output at last "
          "time step, expected is %s, but got %s instead !",
          ShapeUtils::shapeAsString({bS, nOut}).c_str(),
          ShapeUtils::shapeAsString(dLdhL).c_str());
    // gradient vs. cell state at last time step validation
    if (dLdcL != nullptr && (dLdcL->rankOf() != 2 || dLdcL->sizeAt(0) != bS ||
                             dLdcL->sizeAt(1) != nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of gradient vs. cell "
                   "state at last time, expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({bS, nOut}).c_str(),
                   ShapeUtils::shapeAsString(dLdcL).c_str());
  } else {  // bidirectional
            // Wx validation
    if (Wx->rankOf() != 3 || Wx->sizeAt(0) != 2 || Wx->sizeAt(1) != nIn)
      REQUIRE_TRUE(false, 0,
    ...
    // gradient vs. output at last time step validation
    if (dLdhL != nullptr &&
        (dLdhL->rankOf() != 3 || dLdhL->sizeAt(0) != 2 ||
         dLdhL->sizeAt(1) != bS || dLdhL->sizeAt(2) != nOut))
      REQUIRE_TRUE(
          false, 0,
          "LSTM_LAYER_BP operation: wrong shape of gradient vs. output at last "
          "time step, expected is %s, but got %s instead !",
    ...
    // gradient vs. cell state at last time step validation
    if (dLdcL != nullptr &&
        (dLdcL->rankOf() != 3 || dLdcL->sizeAt(0) != 2 ||
         dLdcL->sizeAt(1) != bS || dLdcL->sizeAt(2) != nOut))
      REQUIRE_TRUE(false, 0,
                   "LSTM_LAYER_BP operation: wrong shape of gradient vs. cell "
                   "state at last time, expected is %s, but got %s instead !",
                   ShapeUtils::shapeAsString({2, bS, nOut}).c_str(),
  ...
  if (dLdh) {
    ...
    if (dataFormat == 0)
      expdLdhShape = std::vector<Nd4jLong>{sL, bS, factor * nOut};
    else if (dataFormat == 1)
      expdLdhShape = std::vector<Nd4jLong>{bS, sL, factor * nOut};
    else if (dataFormat == 2)
      expdLdhShape = std::vector<Nd4jLong>{bS, factor * nOut, sL};
    else
      expdLdhShape = std::vector<Nd4jLong>{sL, 2, bS, nOut};
    REQUIRE_TRUE(dLdh->isSameShape(expdLdhShape), 0,
                 "LSTM_LAYER_CELL_BP operation: wrong shape of gradient vs. "
                 "output, expected is %s, but got %s instead !",
                 ShapeUtils::shapeAsString(expdLdhShape).c_str(),
  ...
  if (directionMode == 0) {  // forward
    ...
    helpers::lstmLayerTimeLoopBp(x, Wx, Wr, b, seqLen, hI, cI, Wp, dLdh, dLdhL,
                                 dLdcL, params, true, dLdx, dLdWx, dLdWr, dLdb,
                                 dLdhI, dLdcI, dLdWp);
  } else if (directionMode == 1) {  // backward

    helpers::lstmLayerTimeLoopBp(x, Wx, Wr, b, seqLen, hI, cI, Wp, dLdh, dLdhL,
                                 dLdcL, params, false, dLdx, dLdWx, dLdWr, dLdb,
                                 dLdhI, dLdcI, dLdWp);
  } else {  // bidirectional
    ...
    NDArray *WpFwd(nullptr), *WpBwd(nullptr), *bFwd(nullptr), *bBwd(nullptr),
        *hIFwd(nullptr), *hIBwd(nullptr), *cIFwd(nullptr), *cIBwd(nullptr),
        *dLdhFwd(nullptr), *dLdhBwd(nullptr), *dLdhLFwd(nullptr),
        *dLdhLBwd(nullptr), *dLdcLFwd(nullptr), *dLdcLBwd(nullptr),
        *dLdWpFwd(nullptr), *dLdWpBwd(nullptr), *dLdbFwd(nullptr),
        *dLdbBwd(nullptr), *dLdhIFwd(nullptr), *dLdhIBwd(nullptr),
        *dLdcIFwd(nullptr), *dLdcIBwd(nullptr);
    ...
    if (dLdh) {
      if (directionMode == 2) {  // sum
        ...
        dLdhFwd = new NDArray(dataFormat <= 1 ? (*dLdh)({0, 0, 0, 0, 0, nOut})
                                              : (*dLdh)({0, 0, 0, nOut, 0, 0}));
        dLdhBwd = new NDArray(dataFormat <= 1
                                  ? (*dLdh)({0, 0, 0, 0, nOut, 2 * nOut})
    ...
    // FIXME - following two calls are independent and may run in different
    // streams
    helpers::lstmLayerTimeLoopBp(x, &WxFwd, &WrFwd, bFwd, seqLen, hIFwd, cIFwd,
                                 WpFwd, dLdhFwd, dLdhLFwd, dLdcLFwd, params,
                                 true, dLdx, &dLdWxFwd, &dLdWrFwd, dLdbFwd,
                                 dLdhIFwd, dLdcIFwd, dLdWpFwd);
    helpers::lstmLayerTimeLoopBp(x, &WxBwd, &WrBwd, bBwd, seqLen, hIBwd, cIBwd,
                                 WpBwd, dLdhBwd, dLdhLBwd, dLdcLBwd, params,
                                 false, &dLdxBwd, &dLdWxBwd, &dLdWrBwd, dLdbBwd,
                                 dLdhIBwd, dLdcIBwd, dLdWpBwd);
