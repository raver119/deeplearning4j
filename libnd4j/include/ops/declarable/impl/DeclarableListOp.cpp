...
namespace sd {
namespace ops {
DeclarableListOp::DeclarableListOp(int numInputs, int numOutputs,
                                   const char* opName, int tArgs, int iArgs)
    : DeclarableOp::DeclarableOp(numInputs, numOutputs, opName, false, tArgs,
                                 iArgs) {
 ...
 * @return
 */
ShapeList* DeclarableListOp::calculateOutputShape(ShapeList* inputShape,
...
void DeclarableListOp::setupResultList(const NDArrayList& arrayList,
                                       Context& block) {
...
ResultSet DeclarableListOp::execute(const NDArrayList& list,
  ...
  // should be never used in practice, since in-graph NDArrayList should have id
  // set
  int cnt = -1;
  std::vector<int> in;

  // first input must be our NDArrayList, except create_list op. it creates list
  // itself.
