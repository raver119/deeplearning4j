...
template <typename OpName>
__registratorSynonym<OpName>::__registratorSynonym(const char* name,
                                                   const char* oname) {
  auto ptr = reinterpret_cast<OpName*>(
      OpRegistrator::getInstance()->getOperation(oname));
  if (ptr == nullptr) {
    std::string newName(name);
    std::string oldName(oname);

    OpRegistrator::getInstance()->updateMSVC(
        sd::ops::HashHelper::getInstance()->getLongHash(newName), oldName);
    return;
  }
...
OpRegistrator* OpRegistrator::getInstance() {
  if (!_INSTANCE) _INSTANCE = new sd::ops::OpRegistrator();
...
const char* OpRegistrator::getAllCustomOperations() {
  ...
  if (!isInit) {
    for (MAP_IMPL<std::string, std::shared_ptr<sd::ops::DeclarableOp>>::iterator
             it = _declarablesD.begin();
...
bool OpRegistrator::registerOperation(
    const std::string& opName, std::shared_ptr<sd::ops::DeclarableOp> op) {
  std::pair<std::string, std::shared_ptr<sd::ops::DeclarableOp>> pair(opName,
                                                                      op);
...
void OpRegistrator::registerHelper(sd::ops::platforms::PlatformHelper* op) {
  ...
  nd4j_debug("Adding helper for op \"%s\": [%lld - %i]\n", op->name().c_str(),
             op->hash(), (int)op->engine());

  std::pair<std::pair<std::string, samediff::Engine>,
            sd::ops::platforms::PlatformHelper*>
      pair({op->name(), op->engine()}, op);
  _helpersH.insert(pair);

  std::pair<std::pair<Nd4jLong, samediff::Engine>,
            sd::ops::platforms::PlatformHelper*>
      pair2(p, op);
 ...
 * @return
 */
std::shared_ptr<sd::ops::DeclarableOp> OpRegistrator::getOperation(
    Nd4jLong hash) {
  if (!_declarablesLD.count(hash)) {
    if (!_msvc.count(hash)) {
      ...
      std::pair<Nd4jLong, std::shared_ptr<sd::ops::DeclarableOp>> pair(oHash,
                                                                       op);
...
sd::ops::platforms::PlatformHelper* OpRegistrator::getPlatformHelper(
    Nd4jLong hash, samediff::Engine engine) {
...
namespace std {
size_t hash<std::pair<Nd4jLong, samediff::Engine>>::operator()(
    const std::pair<Nd4jLong, samediff::Engine>& k) const {
  using std::hash;
