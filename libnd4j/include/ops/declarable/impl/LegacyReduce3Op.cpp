...
Nd4jStatus LegacyReduce3Op::validateAndExecute(Context &block) {
  ...
  if (x->isSameShape(y) &&
    ...
    NativeOpExecutioner::execReduce3Scalar(
        ...
        extras.argumentsAsT(z->dataType()), y->buffer(), y->shapeInfo(),
    ...
    NativeOpExecutioner::execReduce3(
        ...
        xTadShape, xTadOffsets, yTadShape, yTadOffsets);
  }
 ...
 * dimensions
 */
ShapeList *LegacyReduce3Op::calculateOutputShape(ShapeList *inputShape,
  ...
  if (shape::equalsSoft(xShape, yShape) &&
    ...
    xShape = ShapeUtils::evalReduceShapeInfo('c', block.getIArguments(), *array,
                                             false, true);
