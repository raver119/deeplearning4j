  ...
  if (block.width() == 1) {
    ...
    if ((block.getIArguments()->size() == 0) ||
      ...
      NativeOpExcutioner::execReduceFloatScalar(
          opNum, x->buffer(), x->shapeInfo(), block.getTArguments()->data(),
      ...
      NativeOpExcutioner::execReduceFloat(
          opNum, x->buffer(), x->shapeInfo(), block.getTArguments()->data(),
    ...
    if ((block.getIArguments()->size() == 1 && INT_ARG(0) == MAX_INT) ||
        allAxes) {
      auto z = OUTPUT_VARIABLE(0);
      ...
      // scalar
      NativeOpExcutioner::execReduceFloatScalar(opNum, b, s, e, z->buffer(),
                                                z->shapeInfo());
    } else {
      // TAD
 ...
 * dimensions
 */
ShapeList *LegacyReduceOp::calculateOutputShape(ShapeList *inputShape,
  ...
  if (block.getIArguments()->size() == 0 ||
    ...
    if (block.getIArguments()->size() > 0 &&
      ...
      ALLOCATE(newShape, block.workspace(), shape::shapeInfoLength(0),
               Nd4jLong);
      newShape[0] = 0;
      newShape[1] = 0;
      newShape[2] = 1;
    ...
    newShape = ShapeUtils::evalReduceShapeInfo(shape::order(inShape),
                                               *block.getIArguments(), *array,
