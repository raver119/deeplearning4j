...
LegacyScalarBoolOp::LegacyScalarBoolOp(int opNum)
...
LegacyScalarBoolOp::LegacyScalarBoolOp(int opNum, NDArray &scalar)
    : LegacyOp::LegacyOp(1, opNum) {
  _scalar = new NDArray(scalar.dup(scalar.ordering()));
}

ShapeList *LegacyScalarBoolOp::calculateOutputShape(ShapeList *inputShape,
...
Nd4jStatus LegacyScalarBoolOp::validateAndExecute(Context &block) {
  ...
  if (block.width() > 1) {
    ...
    NativeOpExecutioner::execScalarBool(
        ...
        x->specialBuffer(), x->specialShapeInfo(), z->buffer(), z->shapeInfo(),
        z->specialBuffer(), z->specialShapeInfo(), y->buffer(), y->shapeInfo(),
    ...
    NativeOpExecutioner::execScalarBool(
        ...
        z->specialBuffer(), z->specialShapeInfo(), y.buffer(), y.shapeInfo(),
        y.specialBuffer(), y.specialShapeInfo(),
        extras.argumentsAsT(x->dataType(), 1));
    ...
    NativeOpExecutioner::execScalarBool(
        block.launchContext(), opNum, x->buffer(), x->shapeInfo(),
        x->specialBuffer(), x->specialShapeInfo(), z->buffer(), z->shapeInfo(),
        z->specialBuffer(), z->specialShapeInfo(), _scalar->buffer(),
        _scalar->shapeInfo(), _scalar->specialBuffer(),
        _scalar->specialShapeInfo(), extras.argumentsAsT(x->dataType()));
  }
  manager.synchronize();
  STORE_RESULT(*z);
