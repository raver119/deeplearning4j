...
Nd4jStatus LegacyBroadcastBoolOp::validateAndExecute(Context &block) {
  ...
  REQUIRE_TRUE(shape::length(packX.primaryShapeInfo()) == y->lengthOf(), 0,
               "Length of broadcast TAD should be equal to length of Y "
               "operand, but got [%i] vs [%i]",
               (int)shape::length(packX.primaryShapeInfo()),
               (int)y->lengthOf());
  ...
  if (x == z)
    NativeOpExecutioner::execBroadcast(
        ...
        y->specialBuffer(), y->specialShapeInfo(), z->buffer(), z->shapeInfo(),
        z->specialBuffer(), z->specialShapeInfo(), dims.data(), dims.size(),
        pTadShape, pTadOffsets, pTadShape, pTadOffsets);
  else {
    ...
    NativeOpExecutioner::execBroadcast(
        ...
        y->specialBuffer(), y->specialShapeInfo(), z->buffer(), z->shapeInfo(),
        z->specialBuffer(), z->specialShapeInfo(), dims.data(), dims.size(),
        pTadShape, pTadOffsets, zTadShape, zTadOffsets);
  }
...
LegacyBroadcastBoolOp::LegacyBroadcastBoolOp(int opNum)
    : LegacyOp::LegacyOp(2, opNum) {
  //
}
...
/**
 *   If external NDArray wasn't specified - the same shape is returned by all
 * broadcast ops.
 */
ShapeList *LegacyBroadcastBoolOp::calculateOutputShape(
