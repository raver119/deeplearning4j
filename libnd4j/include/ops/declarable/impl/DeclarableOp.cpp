...
namespace sd {
namespace ops {
Nd4jStatus conditionHelper(const char *file, int line, int condition,
                           int argNumber, const char *format, ...) {
  if (!condition) {
    va_list args;
...
DeclarableOp::DeclarableOp(int numInputs, int numOutputs, const char *opName,
                           bool allowsInplace) {
...
DeclarableOp::DeclarableOp(int numInputs, int numOutputs, const char *opName,
                           bool allowsInplace, bool divergent) {
  _descriptor =
      new OpDescriptor(numInputs, numOutputs, opName, allowsInplace, divergent);
  _name = opName;
}

DeclarableOp::DeclarableOp(int numInputs, int numOutputs, const char *opName,
                           bool allowsInplace, int tArgs, int iArgs) {
  _descriptor = new OpDescriptor(numInputs, numOutputs, opName, allowsInplace,
                                 tArgs, iArgs);
  _name = opName;
}
...
Nd4jLong DeclarableOp::getOpHash() const { return _descriptor->getHash(); }
...
sd::NDArray *sd::ops::DeclarableOp::getZ(Context &ctx, int inputId) {
  ...
  if (ctx.isFastPath()) {
    ...
    if (ctx.isInplace()) {
      ...
      // hypothetically it's possible to have no variable. chances are low, but
      // who knows. let's just create it for now
  ...
  if (z != nullptr && z->undefined()) return nullptr;
...
int DeclarableOp::prepareOutputs(Context &ctx) {
  ...
  std::chrono::time_point<std::chrono::system_clock> inputEnd, inputStart,
      shapeStart, shapeEnd, arrayStart, arrayEnd;
  ...
  if (Environment::getInstance()->isProfiling()) {
    ...
    prof->nodeById(ctx.nodeId());
    }
     */
    throw std::runtime_error(
        "DeclarableOp::prepareOutputs - Not implemented yet");
  }
    ...
    if (!fp) {
      ...
      for (auto p : ctx.inputs()) {
        ...
        if (var->variableType() == VariableType::NDARRAY) {
          ...
          // in case of this override we might need to update outputs in the
          // Graph VariableSpace as well
          if (vs != nullptr) {
        ...
        if (var->variableType() == VariableType::NDARRAY) {
          ...
          if (array.get() == nullptr)
            throw unresolved_input_exception::build(
                "Variable wasn't resolved prior shape calculation", p);
    ...
    for (auto out : *outSha->asVector()) {
      if (!fp) {
        ...
        if (!ctx.isValueAvailable(ctx.name(), ctx.nodeId(), pair.second)) {
          ...
          /*
           * FIXME: we want to uncomment this eventually, and check data types
          equality
          //checking out data type equality
          if (ArrayOptions::dataType(out) != ArrayOptions::dataType(shape)) {
              std::string msg = "Provided array [" +
          StringUtils::valueToString<int>(pair.second) + "] has unexpected data
          type"; throw sd::datatype_exception::build(msg,
          ArrayOptions::dataType(out), ArrayOptions::dataType(shape));
          }
           */
        }
      } else {
        ...
        if (fout.size() <= idx) {
          ...
          if (!shape::equalsSoft(out, array->shapeInfo()) ||
            ...
            nd4j_printf(
                "Expected vs provided shape mismatch %s vs %s at index %i\n",
...
void sd::ops::DeclarableOp::storeResult(Context &block, int outputNumber,
                                        NDArray *array) {
  this->storeResult(block, outputNumber, *array);
}

void sd::ops::DeclarableOp::storeResult(sd::graph::Context &ctx,
                                        int outputNumber, NDArray &array) {
...
bool sd::ops::DeclarableOp::allocateResult(Context &block, Nd4jLong *shape) {
  ...
  Nd4jLong *__shape;
  ALLOCATE(__shape, workspace, shape::shapeInfoLength(shape),
           Nd4jLong);  // new int[shape[0] * 2 + 4];
  ...
  if (var->getNDArray() == nullptr) {
    std::shared_ptr<DataBuffer> buffer = std::make_shared<DataBuffer>(
        len * sizeof(int8_t), ArrayOptions::dataType(__shape), workspace);
    var->setNDArray(std::make_shared<NDArray>(buffer, ShapeDescriptor(__shape),
                                              block.launchContext()));
...
bool sd::ops::DeclarableOp::allocateResult(
    Context &block, std::initializer_list<Nd4jLong> &shape, char order) {
  auto var = block.variable(block.getNodeId(), 0);
  auto workspace = block.workspace();
  ...
  // if that's first run - we probably have nothing here
  if (var->getNDArray() == nullptr) {
    var->setNDArray(std::make_shared<NDArray>(order, shape, DataType::FLOAT32,
                                              block.launchContext()));
  } else if (var->getNDArray()->lengthOf() != len) {
    // if length not match - lets reallocate array
    var->setNDArray(std::make_shared<NDArray>(order, shape, DataType::FLOAT32,
  ...
  if (block.isFastPath()) {
    ...
    for (auto array : block.fastpath_out()) {
      ...
      if (_descriptor->isSameMode()) {
        ...
        if (std::find(inputTypes.begin(), inputTypes.end(), cType) ==
            inputTypes.end()) {
          auto t = DataTypeUtils::asString(cType);
      ...
      if (varSpace != nullptr && varSpace->hasVariable(block.nodeId(), index)) {
        ...
        if (var != nullptr && var->hasNDArray()) {
          ...
          if (_descriptor->isSameMode()) {
            ...
            // in inherit mode, output type must be the same as one of input
            // types
            if (std::find(inputTypes.begin(), inputTypes.end(), cType) ==
                inputTypes.end()) {
              auto t = DataTypeUtils::asString(cType);
...
Nd4jStatus DeclarableOp::execute(Context *block) {
  ...
  std::chrono::time_point<std::chrono::system_clock> timeEnter, timeStart,
      timeEnd;
  Nd4jLong prepTime, outerTime;

  Nd4jLong memoryBefore = block->workspace() == nullptr
                              ? 0L
  ...
  if (block->isInplace() && !this->getOpDescriptor()->allowsInplace())
    throw std::runtime_error(
        "DeclarableOp::execute - trying to execute non-inplace op as inplace");
  ...
  if (Environment::getInstance()->isProfiling() &&
    ...
    if (fp != nullptr) {
        auto p = fp->profile();
        if (p != nullptr) {
            Nd4jLong memoryAfter = block->workspace() == nullptr ? 0L :
  ...
  if (sd::Environment::getInstance()->isDebugAndVerbose()) {
    ...
    for (int e = 0; e < numOutputs; e++) {
      ...
      auto array = block->isFastPath()
                       ? block->isInplace() ? block->fastpath_in()[e]
                                            : block->fastpath_out()[e]
                       : vs->getVariable(block->nodeId(), e)->getNDArray();

      auto shape = ShapeUtils::shapeAsString(array.get());
      auto first =
          array->isEmpty() ? std::string("Empty NDArray") : array->asString(32);
...
void DeclarableOp::overwriteResult(Context &block, int outputIdx,
                                   NDArray *array) {
...
Nd4jStatus sd::ops::DeclarableOp::validateArguments(Context &block) {
  /*
   * We're checking number of T and I arguments. If number of args is finite
   * number - we check strict equality If number of args is variable (-1), but
   * variables MUST be present - we check for non-zero number of arguments
   */
  if (_descriptor->getNumberOfTArgs() > 0) {
    if ((int)block.numT() < _descriptor->getNumberOfTArgs()) {
      nd4j_printf("%s: %i T args expected, but %i received\n",
                  this->getOpName().c_str(), _descriptor->getNumberOfTArgs(),
                  block.numT());
      return ND4J_STATUS_BAD_PARAMS;
    }
    ...
    if (block.numT() == 0) {
      nd4j_printf(
          "%s: Number of T arguments should be positive number, but got 0 "
          "arguments\n",
          this->getOpName().c_str());
      return ND4J_STATUS_BAD_PARAMS;
    }
  ...
  if (_descriptor->getNumberOfIArgs() > 0) {
    if ((int)block.numI() < _descriptor->getNumberOfIArgs()) {
      nd4j_printf("%s: %i int args expected, but %i received\n",
                  this->getOpName().c_str(), _descriptor->getNumberOfIArgs(),
                  block.numI());
      return ND4J_STATUS_BAD_PARAMS;
    }
    ...
    if (block.numI() == 0) {
      nd4j_printf(
          "%s: Number of Integer arguments should be positive number, but got "
          "0 arguments\n",
          this->getOpName().c_str());
      return ND4J_STATUS_BAD_PARAMS;
    }
...
Nd4jStatus sd::ops::DeclarableOp::validateInputDimensions(Context &block,
                                                          int rank) {
  ...
  for (auto p : block.inputs()) {
    ...
    if (aV == nullptr) return ND4J_STATUS_BAD_INPUT;

    if (aV->rankOf() != rank) return ND4J_STATUS_BAD_DIMENSIONS;
  }
...
Nd4jStatus sd::ops::DeclarableOp::validateNonEmptyInput(Context &block) {
  if (this->getOpDescriptor()->getNumberOfInputs() == -2 ||
      this->getOpDescriptor()->getNumberOfInputs() == 0)
    return Status::OK();

  if (block.width() < 1) {
    nd4j_printf("%s: no operands provided for the op",
                this->getOpName().c_str());
    ...
    if (v == nullptr) {
      if (!this->getOpName().empty()) {
        nd4j_printf("Node [%i:<%s>]: Variable [%i] (%i:%i) is NULL\n",
                    block.getNodeId(), this->getOpName().c_str(), cnt, p.first,
                    p.second);
      } else {
...
Nd4jStatus sd::ops::DeclarableOp::validateOrdersMatch(Context &block) {
  ...
  for (auto p : block.inputs()) {
    auto v = block.variable(p);
    NDArray *aV = v->getNDArray().get();
    if (a0->ordering() != aV->ordering()) return ND4J_STATUS_BAD_ORDER;
  }

  return ND4J_STATUS_OK;
}

Nd4jStatus sd::ops::DeclarableOp::execute(
    sd::graph::RandomGenerator &rng, const std::vector<NDArray *> &inputs,
  ...
  for (int e = 0; e < tArgs.size(); e++) block.appendT(tArgs.at(e));
  ...
  for (int e = 0; e < dArgs.size(); e++) block.appendD(dArgs.at(e));
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
                                 const std::vector<NDArray *> &outputs) {
  return execute(inputs, outputs, std::vector<double>(),
                 std::vector<Nd4jLong>(), std::vector<bool>(),
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
  ...
  return execute(inputs, outputs, tArgs, std::vector<Nd4jLong>(),
                 std::vector<bool>(), std::vector<sd::DataType>());
}
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
  ...
  return execute(inputs, outputs, std::vector<double>(),
                 std::vector<Nd4jLong>(), std::vector<bool>(), dArgs);
}
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
  ...
  return execute(inputs, outputs, realArgs, std::vector<Nd4jLong>(),
                 std::vector<bool>(), std::vector<sd::DataType>());
}
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
                                 const std::vector<NDArray *> &outputs,
                                 std::initializer_list<Nd4jLong> iArgs) {
  return execute(inputs, outputs, std::vector<double>(), iArgs,
                 std::vector<bool>(), std::vector<sd::DataType>());
}
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
  ...
  return execute(inputs, outputs, std::vector<double>(), realArgs,
                 std::vector<bool>(), std::vector<sd::DataType>());
}
...
Nd4jStatus DeclarableOp::execute(const std::vector<NDArray *> &inputs,
  ...
  return execute(inputs, outputs, std::vector<double>(),
                 std::vector<Nd4jLong>(), bArgs, std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs) {
  return evaluate(inputs, std::vector<double>(), std::vector<Nd4jLong>(),
                  std::vector<bool>(), std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
  ...
  return evaluate(inputs, std::vector<double>(), realArgs, std::vector<bool>(),
                  std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
                                     std::initializer_list<Nd4jLong> iArgs) {
  return evaluate(inputs, std::vector<double>(), iArgs, std::vector<bool>(),
                  std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
  ...
  return evaluate(inputs, realArgs, std::vector<Nd4jLong>(),
                  std::vector<bool>(), std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
                                     std::initializer_list<double> tArgs) {
  return evaluate(inputs, tArgs, std::vector<Nd4jLong>(), std::vector<bool>(),
                  std::vector<sd::DataType>());
}
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
                                     std::initializer_list<bool> bArgs) {
  return evaluate(inputs, std::vector<double>(), std::vector<Nd4jLong>(), bArgs,
                  std::vector<sd::DataType>());
}

template <>
sd::ResultSet DeclarableOp::evaluate(
    const std::vector<NDArray *> &inputs,
    std::initializer_list<sd::DataType> bArgs) {
  return evaluate(inputs, std::vector<double>(), std::vector<Nd4jLong>(),
...
sd::ResultSet DeclarableOp::evaluate(const std::vector<NDArray *> &inputs,
  ...
  for (int e = 0; e < iArgs.size(); e++) block.appendI(iArgs.at(e));

  for (int e = 0; e < bArgs.size(); e++) block.appendB(bArgs.at(e));
  ...
  if (status != ND4J_STATUS_OK) return arrayList;
...
sd::ResultSet sd::ops::DeclarableOp::execute(const sd::OpArgsHolder &holder,
                                             bool isInplace) {
  // FIXME: add DArgs to OpArgsHolder
  return evaluate(holder.getInArrs(), holder.getTArgs(), holder.getIArgs(),
                  holder.getBArgs(), std::vector<sd::DataType>(), isInplace);
}
...
Nd4jStatus sd::ops::DeclarableOp::validateInputLengthMatch(Context &block) {
  ...
  for (uint32_t e = 0; e < block.width(); e++) {
    if (l0 != block.array(e)->lengthOf()) return ND4J_STATUS_BAD_LENGTH;
  }
...
/*
template <typename T>
int* sd::ops::DeclarableOp::calculateOutputShape(int* inputShape,
sd::graph::Block& block) {
    // default implementation suits transform, so just returns the same shape

    int* newshape;
    ALLOCATE(newshape, block.workspace(), shape::shapeInfoLength(inputShape),
int); memcpy(newshape, inputShape, shape::shapeInfoByteLength(inputShape));
