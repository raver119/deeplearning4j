...
LegacyReduceBoolOp::LegacyReduceBoolOp(int opNum)
...
Nd4jStatus LegacyReduceBoolOp::validateAndExecute(Context &block) {
  ...
  if (block.width() == 1) {
    ...
    if ((axis.empty()) ||
        (axis.size() == 1 && axis[0] == sd::DataTypeUtils::max<int>()) ||
        allAxes) {
      // scalar
 ...
 * dimensions
 */
ShapeList *LegacyReduceBoolOp::calculateOutputShape(ShapeList *inputShape,
  ...
  return SHAPELIST(ShapeUtils::evalReduceShapeInfo(
      shape::order(inShape), axis, inShape, DataType::BOOL, keepDims,
