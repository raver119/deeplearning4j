...
LegacyPairwiseTransformOp::LegacyPairwiseTransformOp(int opNum)
...
Nd4jStatus LegacyPairwiseTransformOp::validateAndExecute(Context &block) {
  ...
  if (!x->isSameShape(y))
    REQUIRE_TRUE(x->isSameShape(y) || y->isScalar(), 0,
                 "Node_%i: For Pairwise transforms shapes of both operands "
                 "should be equal but got %s vs %s",
                 block.getNodeId(), ShapeUtils::shapeAsString(x).c_str(),
...
/**
 *   Output shape of PWT operations always the same as input[0] shape, no
 * exclusions.
 */
ShapeList *LegacyPairwiseTransformOp::calculateOutputShape(
