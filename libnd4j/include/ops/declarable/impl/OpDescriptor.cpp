...
OpDescriptor::OpDescriptor(int numInputs, int numOutputs, std::string opName,
                           bool allowsInplace)
    : OpDescriptor::OpDescriptor(numInputs, numOutputs, opName.c_str(),
                                 allowsInplace) {
  //
}

void OpDescriptor::setHash(Nd4jLong hash) { _hash = hash; }

// default constructor
OpDescriptor::OpDescriptor(int numInputs, int numOutputs, const char* opName,
                           bool allowsInplace) {
  _numInputs = numInputs;
...
OpDescriptor::OpDescriptor(int numInputs, int numOutputs, const char* opName,
                           bool allowsInplace, bool divergent, int tArgs,
                           int iArgs)
...
Nd4jLong OpDescriptor::getHash() { return _hash; }
...
OpDescriptor* OpDescriptor::setAllowedInputTypes(
    const std::initializer_list<sd::DataType>& dtypes) {
  _allowedIns = dtypes;
  return this;
}

OpDescriptor* OpDescriptor::setAllowedOutputTypes(
    const std::initializer_list<sd::DataType>& dtypes) {
  _allowedOuts = dtypes;
  return this;
}
...
OpDescriptor* OpDescriptor::setInputType(const int idx,
                                         const sd::DataType dtype) {
  _inputTypes[idx] = {dtype};
  return this;
}

OpDescriptor* OpDescriptor::setOutputType(const int idx,
                                          const sd::DataType dtype) {
...
OpDescriptor* OpDescriptor::setAllowedInputTypes(
    int index, const std::vector<sd::DataType>& dtype) {
...
OpDescriptor* OpDescriptor::setAllowedInputTypes(int index,
                                                 sd::DataType dtype) {
...
OpDescriptor* OpDescriptor::setAllowedOutputTypes(int index,
                                                  sd::DataType dtype) {
...
bool OpDescriptor::checkDataTypesMatch(
    sd::DataType needle, std::vector<sd::DataType>& haystack) const {
  ...
  if (std::find(haystack.begin(), haystack.end(), needle) == haystack.end()) {
    ...
    if (std::find(haystack.begin(), haystack.end(), sd::DataType::ANY) ==
        haystack.end())
      return false;
    else
      return true;
...
bool OpDescriptor::isInherit(int index) {
  if (std::find(_allowedOuts.begin(), _allowedOuts.end(),
                sd::DataType::INHERIT) != _allowedOuts.end())
    return true;
  if (_outputTypes.count(index) > 0) {
