...
LegacyReduceSameOp::LegacyReduceSameOp(int opNum)
  ...
  if (block.width() == 1) {
    ...
    if (axis.empty() || allAxes) {
      ...
      NativeOpExecutioner::execReduceSame(
          ...
          x->specialBuffer(), x->specialShapeInfo(),
          extras.argumentsAsT(z->dataType()), z->buffer(), z->shapeInfo(),
          x->specialBuffer(), x->specialShapeInfo(), dims.data(),
 ...
 * dimensions
 */
ShapeList *LegacyReduceSameOp::calculateOutputShape(ShapeList *inputShape,
