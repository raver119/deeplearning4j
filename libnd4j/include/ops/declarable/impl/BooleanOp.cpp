...
bool BooleanOp::verify(sd::graph::Context &block) {
  ...
  if (status == ND4J_STATUS_TRUE)
    ...
    nd4j_printf("Got error %i during [%s] evaluation: ", (int)status,
                this->getOpDescriptor()->getOpName()->c_str());
...
bool BooleanOp::prepareOutputs(Context &ctx) {
  ...
  for (int e = 0; e < this->getOpDescriptor()->getNumberOfOutputs(); e++) {
    ...
    if (!var->hasNDArray()) {
      var->setNDArray(std::make_shared<NDArray>(
          NDArrayFactory::create<bool>(false, ctx.launchContext())));
    }
  }
...
Nd4jStatus sd::ops::BooleanOp::execute(Context *block) {
  ...
  // basically we're should be putting 0.0 as FALSE, and any non-0.0 value will
  // be treated as TRUE
  std::pair<int, int> p(block->nodeId(), 0);
  auto var = block->isFastPath() ? block->fastpath_out()[0]
                                 : block->variable(p)->getNDArray();
  var->p(Nd4jLong(0), status == ND4J_STATUS_TRUE ? 1.0f : 0.0f);

  // for CPU backend that's nop, but for CUDA-like archs this will update
  // special buffer
  var->syncToDevice();
