...
LegacyIndexReduceOp::LegacyIndexReduceOp(int opNum)
...
ShapeList *LegacyIndexReduceOp::calculateOutputShape(
  ...
  if (block.getAxis().size() == 0 && block.width() == 1) {
    ...
    auto array =
        INPUT_VARIABLE(0);  // new NDArray(nullptr, inShape, block.workspace());

    auto newShape = ShapeUtils::evalReduceShapeInfo(
        'c', block.getAxis(), *array, DataType::INT64, false, true,
        block.workspace());
    ...
    if (allAxes) {
      Nd4jLong *newShape;
      // in this case we just return scalar
      ALLOCATE(newShape, block.workspace(), shape::shapeInfoLength(2),
               Nd4jLong);
      newShape[0] = 2;
      newShape[1] = 1;
      newShape[2] = 1;
      ...
      // in this case we're building proper shape for reduction
      auto array = INPUT_VARIABLE(
          0);  // new NDArray(nullptr, inShape, block.workspace());
      return SHAPELIST(ShapeUtils::evalReduceShapeInfo(
          'c', axis, *array, DataType::INT64, false, true, block.workspace()));
    }
  }
}
...
Nd4jStatus LegacyIndexReduceOp::validateAndExecute(Context &block) {
  ...
  if (z->dataType() != INT64) {
    throw std::runtime_error(
        "IndexReduce operations require output to be INT64");
  }
