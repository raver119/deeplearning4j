...
LegacyReduceFloatOp::LegacyReduceFloatOp(int opNum)
    : LegacyOp::LegacyOp(1, opNum) {
  // this->_opNum = opNum;
}
...
Nd4jStatus LegacyReduceFloatOp::validateAndExecute(Context &block) {
  ...
  if (block.width() == 1) {
    ...
    //                    (block.getIArguments()->size() == 1 && INT_ARG(0) ==
    //                    sd::DataTypeUtils::max<int>())
    if (block.getAxis().empty() || allAxes) {
      ...
      auto pTadShape =
          Environment::getInstance()->isCPU()
              ...
              : packX
                    .specialShapeInfo();  // manager.replicatePointer(tad.tadOnlyShapeInfo,
      ...
      NativeOpExecutioner::execReduceFloat(
          ...
          x->specialBuffer(), x->specialShapeInfo(),
          extras.argumentsAsT(z->dataType()), z->buffer(), z->shapeInfo(),
          z->specialBuffer(), z->specialShapeInfo(), dims.data(),
          (int)dims.size(), reinterpret_cast<Nd4jLong const *>(pTadShape),
          reinterpret_cast<Nd4jLong const *>(pTadOffsets));
    }
    ...
    if ((block.numI() == 1 && INT_ARG(0) == sd::DataTypeUtils::max<int>()) ||
        allAxes) {
      // scalar
      NativeOpExecutioner::execReduceFloatScalar(
          block.launchContext(), opNum, x->buffer(), x->shapeInfo(),
          x->specialBuffer(), x->specialShapeInfo(),
          extras.argumentsAsT(x->dataType()), z->buffer(), z->shapeInfo(),
          z->specialBuffer(), z->specialShapeInfo());
    } else {
      // TAD
      REQUIRE_TRUE(dims.size() > 0, 0,
      ...
      NativeOpExecutioner::execReduceFloat(
          ...
          extras.argumentsAsT(z->dataType()), z->buffer(), z->shapeInfo(),
          z->specialBuffer(), z->specialShapeInfo(), dims.data(),
          (int)dims.size(), pTadShape, pTadOffsets);
    }
  }
...
/**
 *   For all reductions rules are simple: either you return scalar, or you
 * return reduced NDArray. It solely depends on input shape, and requested
 * dimensions
 */
ShapeList *LegacyReduceFloatOp::calculateOutputShape(
  ...
  auto axis =
      block.width() > 1 ? INPUT_VARIABLE(1)->asVectorT<int>() : block.getAxis();

  if (axis.size() == shape::rank(inShape)) allAxes = true;

  // in this case we're building proper shape for reduction
  auto newShape =
      ShapeUtils::evalReduceShapeInfo(shape::order(inShape), axis, inShape,
                                      keepDims, !newFormat, block.workspace());
