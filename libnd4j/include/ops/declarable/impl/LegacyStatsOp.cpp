...
Nd4jStatus LegacyStatsOp::validateAndExecute(Context &block) {
  ...
  // we assume that opNuk is either stored in block, or was provided via op
  // constructor
  int opNum = block.opNum() < 0 ? this->_opNum : block.opNum();

  // bias goes as first argument, unlike all other reductions
  bool biasCorrected = false;
  if (block.numI() > 0) biasCorrected = INT_ARG(0) > 0;

  ExtraArguments extras(block.getTArguments());
  PointersManager manager(block.launchContext(), "LegacyStatsOp");

  if (block.numI() == 1 ||
      (block.numI() == 2 && INT_ARG(1) == sd::DataTypeUtils::max<int>())) {
    // scalar
    ...
    // dimensions for TAD
    // we should skip first argument here, because it's addressing bias
    // correction
 ...
 * dimensions
 */
ShapeList *LegacyStatsOp::calculateOutputShape(ShapeList *inputShape,
                                               sd::graph::Context &block) {
  auto inShape = inputShape->at(0);

  Nd4jLong *newShape;
  if (block.numI() == 0 ||
      (block.numI() == 1 && INT_ARG(0) == sd::DataTypeUtils::max<int>())) {
    ...
    auto newShape = ShapeUtils::evalReduceShapeInfo('c', block.getIArguments(),
                                                    *array, false, true);
