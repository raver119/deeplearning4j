...
LegacyScalarOp::LegacyScalarOp(int opNum, NDArray &scalar)
    : LegacyOp::LegacyOp(1, opNum) {
  _scalar = new NDArray(scalar.dup(scalar.ordering()));
}

ShapeList *LegacyScalarOp::calculateOutputShape(ShapeList *inputShape,
...
Nd4jStatus LegacyScalarOp::validateAndExecute(Context &block) {
  ...
  if (block.width() > 1) {
    ...
    auto y =
        NDArrayFactory::create(x->dataType(), T_ARG(0), block.launchContext());
    ...
    NativeOpExecutioner::execScalar(
        ...
        _scalar->specialShapeInfo(), extras.argumentsAsT(z->dataType()));
