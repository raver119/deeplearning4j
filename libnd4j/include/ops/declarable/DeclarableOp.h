...
Nd4jStatus SD_EXPORT conditionHelper(const char* file, int line, int condition,
                                     int argNumber, const char* format, ...);

template <typename T>
Nd4jStatus resultHelper(T status, const char* func, const char* file,
                        int line) {
  if (status) {
    //  TODO: fill out error codes here
    fprintf(stderr, "Validation error at %s:%d code=%d(%s) \"%s\" \n", file,
            line, static_cast<unsigned int>(status), "", func);
...
/**
 * This class is the basic building block of Graph Operations. Any CustomOp out
 * there is built on top of this "abstract" class.
 *
 */
class SD_EXPORT DeclarableOp {
 ...
 protected:
  ...
  /**
   * This method executes this Op, and defined for most of individual ops
   * separately
   */
   ...
   */
  bool allocateResult(Context& block, std::initializer_list<Nd4jLong>& shape,
                      char order = 'c');
  ...
  /*
   * This method attaches array to specific Variable, identified by node ID and
   * outputNumber (which is output index for multi-output operations)
   */
 ...
 public:
  ...
  /**
   *   This method should be available in each implemented Op, and should return
   * Op output shape(s), for a given input shape(s)
   */
  virtual ShapeList* calculateOutputShape(ShapeList* inputShape,
                                          sd::graph::Context& block) = 0;
  ...
  Nd4jStatus execute(const std::vector<NDArray*>& inputs,
                     const std::vector<NDArray*>& outputs);

  template <class T, typename = std::enable_if<
                         DataTypeUtils::scalarTypesForExecution<T>::value>>
  Nd4jStatus execute(const std::vector<NDArray*>& inputs,
                     const std::vector<NDArray*>& outputs,
  ...
  Nd4jStatus execute(
      ...
      const std::vector<sd::DataType>& dArgs = std::vector<sd::DataType>(),
  ...
  sd::ResultSet evaluate(
      const std::vector<NDArray*>& inputs, const std::vector<double>& tArgs,
  ...
  Nd4jStatus execute(
      ...
      const std::vector<sd::DataType>& dArgs = std::vector<sd::DataType>(),
      bool isInplace = false, sd::DataType type = sd::DataType::FLOAT32);
  ...
  // this method checks if all input arrays have the same shapes (orders/strides
  // are NOT checked)
  ...
  /**
   *   This method pre-allocates NDArrays for Op output, in case they are not
   * available at op execution time
   */
