...
template <typename T>
void *transpose(int orderSource, int orderTarget, int rows, int cols,
                void *vsource) {
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto r = start; r < stop; r++) {
      for (int c = 0; c < cols; c++) {
        int zIdx = orderTarget == CblasRowMajor
                       ? linearIndexC(rows, cols, r, c)
                       : linearIndexF(rows, cols, r, c);
        int xIdx = orderSource == CblasColMajor
                       ? linearIndexF(rows, cols, r, c)
                       : linearIndexC(rows, cols, r, c);
...
void GEMM<X, Y, Z>::op(int Order, int TransA, int TransB, int M, int N, int K,
                       double alpha, void *vA, int lda, void *vB, int ldb,
                       double beta, void *vC, int ldc) {
    ...
    for (auto r = start_x; r < stop_x; r += inc_x) {
      for (auto c = start_y; c < stop_y; c += inc_y) {
        ...
        if (alpha != 0.0) {
          ...
          for (int k = 0; k < K; k++) {
            aIdx = (transAFlag ? linearIndexC(M, K, r, k)
                               : linearIndexF(M, K, r, k));
            bIdx = (transBFlag ? linearIndexC(K, N, k, c)
                               : linearIndexF(K, N, k, c));
            dot += static_cast<Z>(alpha) * static_cast<Z>(A[aIdx]) *
                   static_cast<Z>(B[bIdx]);  // A[aIdx]sd::math::nd4j_dot<T>(aX,
                                             // bX, K) * alpha;
          }
        }
...
template <typename X, typename Y, typename Z>
void GEMV<X, Y, Z>::op(int TRANS, int M, int N, double alpha, void *vX, int lda,
                       void *vY, int incx, double beta, void *vZ, int incy) {
  auto x = reinterpret_cast<X *>(vX);
  auto y = reinterpret_cast<Y *>(vY);
  auto z = reinterpret_cast<Z *>(vZ);

  auto aT = TRANS == CblasTrans ? reinterpret_cast<X *>(sd::blas::transpose<X>(
                                      CblasColMajor, CblasRowMajor, M, N,
  ...
  auto func = PRAGMA_THREADS_FOR {
    for (auto r = start; r < stop; r++) {
      ...
      auto dot =
          sd::math::nd4j_dot<X, Y, Z>(aX, y, lda) * static_cast<Z>(alpha);
...
// BUILD_TRIPLE_TEMPLATE(template class  GEMV, , LIBND4J_TYPES, FLOAT_TYPES,
// FLOAT_TYPES); BUILD_TRIPLE_TEMPLATE(template class  GEMM, , LIBND4J_TYPES,
