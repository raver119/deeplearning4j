 ...
 */
// template <typename T>
// void SpecialMethods<T>::concatCpuGeneric(const std::vector<NDArray*>& inArrs,
// NDArray& output, const int axis) {
...
//             const uint outEws = isOutputVector ? output.stridesOf()[outDim] :
//             output.ews();
...
//                 allVectorsOrScalars &= (inArrs[i]->lengthOf() == 1 ||
//                 inArrs[i]->isCommonVector(nonUnityDim[i]));
//                 if(!allVectorsOrScalars)
//                     break;
//                 if(i == 0)  zOffset[0] = 0;
//                 else        zOffset[i] = zOffset[i - 1] + outEws * inArrs[i -
//                 1]->lengthOf();
//             }
...
//                         const uint xEws = (arrLen == 1) ? 1 :
//                         inArrs[r]->stridesOf()[nonUnityDim[r]];
...
//         std::vector<std::vector<Nd4jLong>> indices(numOfArrs,
//         std::vector<Nd4jLong>(rank2,0));
...
//             indices[i][2 * axis]     = indices[i-1][2 * axis + 1]; // index
//             start from indices[i][2 * axis + 1] = indices[i-1][2 * axis + 1]
//             + inArrs[i]->sizeAt(axis);      // index end with (excluding)
//         }

//         auto func = PRAGMA_THREADS_FOR {
//             for (auto i = start; i < stop; i += increment) {
//                 auto temp = output(indices[i], true);
//                 sd::TransformLoops<T, T, T>::template
//                 loopTransform<simdOps::Assign<T, T>>(
//                 inArrs[i]->bufferAsT<T>(), inArrs[i]->shapeInfo(),
//                 temp.bufferAsT<T>(), temp.shapeInfo(), nullptr, 0, 1);
//             }
//         };
...
void SpecialMethods<T>::concatCpuGeneric(
  ...
  bool luckCase1 =
      ((axis == 0 && output.ordering() == 'c') ||
       (axis == output.rankOf() - 1 && output.ordering() == 'f')) &&
      output.ews() == 1;

  if (luckCase1) {
    for (uint i = 0; i < numOfInArrs; ++i) {
      luckCase1 &=
          inArrs[i]->ordering() == output.ordering() && inArrs[i]->ews() == 1;
      if (!luckCase1) break;
    }
  }

  if (luckCase1) {  // for example {1,10} + {2,10} + {3,10} = {6, 10} order c;
                    // or {10,1} + {10,2} + {10,3} = {10, 6} order f
  ...
  //             const T* x = inArrs[j]->bufferAsT<T>() +
  //             strideOfContigStride[j] * i; memcpy(z, x, xDim * sizeofT); z +=
  //             xDim;
  //         }
  //     }
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    for (auto i = start; i < stop; i += increment) {
      ...
      const auto xOffset =
          shape::getOffset(inArrs[inArrIdx]->shapeInfo(), coords);
 ...
 */
template <typename T>
void SpecialMethods<T>::concatCpuGeneric(int dimension, int numArrays,
                                         Nd4jPointer *data,
  ...
  for (int i = 0; i < numArrays; ++i)
    inputs[i] = new NDArray(static_cast<void *>(data[i]),
                            static_cast<Nd4jLong *>(inputShapeInfo[i]));
...
template <typename T>
void SpecialMethods<T>::splitCpuGeneric(const NDArray &input,
                                        const std::vector<NDArray *> &outArrs,
                                        const int axis) {
  ...
  bool luckCase1 = ((axis == 0 && input.ordering() == 'c') ||
                    (axis == input.rankOf() - 1 && input.ordering() == 'f')) &&
                   input.ews() == 1;

  if (luckCase1) {
    for (uint i = 0; i < numSplits; ++i) {
      luckCase1 &=
          outArrs[i]->ordering() == input.ordering() && outArrs[i]->ews() == 1;
      if (!luckCase1) break;
    }
  }
  ...
  //     const auto xStep = shape::strideOverContigAxis(axis,
  //     input.shapeInfo());
  ...
  //             T* z = outArrs[j]->bufferAsT<T>() + strideOfContigStride[j] *
  //             i; memcpy(z, x, zDim * sizeofT); x += zDim;
  //         }
  //     }
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    for (auto i = start; i < stop; i += increment) {
      ...
      const auto zOffset =
          shape::getOffset(outArrs[outArrIdx]->shapeInfo(), coords);
 ...
 */
template <typename T>
void SpecialMethods<T>::accumulateGeneric(void **vx, void *vz,
                                          Nd4jLong const *zShapeInfo, int n,
                                          const Nd4jLong length) {
 ...
 */
template <typename T>
void SpecialMethods<T>::averageGeneric(void **vx, void *vz,
                                       Nd4jLong const *zShapeInfo, int n,
                                       const Nd4jLong length, bool propagate) {
  auto z = reinterpret_cast<T *>(vz);
  auto x = reinterpret_cast<T **>(vx);
  ...
  if (z == nullptr) {
    ...
    // instead of doing element-wise propagation, we just issue memcpy to
    // propagate data
    for (Nd4jLong ar = 1; ar < n; ar++) {
...
template <typename T>
Nd4jLong SpecialMethods<T>::getPosition(Nd4jLong const *xShapeInfo,
                                        Nd4jLong index) {
...
template <typename T>
void SpecialMethods<T>::quickSort_parallel_internal(T *array,
...
template <typename T>
void SpecialMethods<T>::quickSort_parallel(void *varray,
                                           Nd4jLong const *xShapeInfo,
                                           Nd4jLong lenArray, int numThreads,
                                           bool descending) {
...
template <typename T>
void SpecialMethods<T>::sortGeneric(void *vx, Nd4jLong const *xShapeInfo,
                                    bool descending) {
  auto x = reinterpret_cast<T *>(vx);

  quickSort_parallel(x, xShapeInfo, shape::length(xShapeInfo),
                     omp_get_max_threads(), descending);
}

template <typename T>
void SpecialMethods<T>::sortTadGeneric(void *vx, Nd4jLong const *xShapeInfo,
  ...
  Nd4jLong xTadLength =
      shape::tadLength(xShapeInfo, dimension, dimensionLength);
...
template <typename T>
void SpecialMethods<T>::decodeBitmapGeneric(const void *dx, Nd4jLong N,
                                            void *vz,
...
template <typename T>
Nd4jLong SpecialMethods<T>::encodeBitmapGeneric(void *vx,
                                                Nd4jLong const *xShapeInfo,
                                                Nd4jLong N, int *dz,
                                                float threshold) {
