...
Nd4jLong const* NDArray::platformShapeInfo() const {
  return specialShapeInfo();
}
// Nd4jLong const* NDArray::platformShapeInfo()          { return
// specialShapeInfo(); }
...
bool NDArray::isActualOnDeviceSide() const {
  return _buffer->isSpecialActual();
}
...
///////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void fillAsTriangularCuda(
    const void* vx, const Nd4jLong* xShapeInfo, void* vz,
    const Nd4jLong* zShapeInfo, const T val, const int lower, const int upper) {
  const auto x = reinterpret_cast<const T*>(vx);
  auto z = reinterpret_cast<T*>(vz);

  __shared__ int zRank, xRank, areSameOffsets,
      *sharedMem;  // xRank == zRank always, except when xRank = 1, in this case
                   // zRank = 2
  __shared__ Nd4jLong zLen, totalThreads;  // xLen == zLen, except when xRank =
                                           // 1, in this case zLen = 2*xLen
  ...
  for (Nd4jLong i = tid; i < zLen; i += totalThreads) {
    ...
    if ((coords[zRank - 2] + upper < coords[zRank - 1]) ||
        (coords[zRank - 2] + lower > coords[zRank - 1]))
      z[zOffset] = val;
    else if (vx != vz) {  // when x and z are different arrays
      if (xRank != zRank) coords[0] = coords[1];
      const auto xOffset =
          areSameOffsets ? zOffset : shape::getOffset(xShapeInfo, coords);
      z[zOffset] = x[xOffset];
    }
  }
}
...
///////////////////////////////////////////////////////////////////
template <typename T>
void NDArray::fillAsTriangular(const float val, int lower, int upper,
                               NDArray& target, const char direction) {
  if (isS())
    throw std::runtime_error(
        "NDArray::fillAsTriangular: you can't use this method on String "
        "array!");

  if (!isSameShape(target) &&
      !(rankOf() == 1 && target.rankOf() == 2 &&
        sizeAt(0) == target.sizeAt(0) && sizeAt(0) == target.sizeAt(1)))
    throw std::string(
        "NDArray::fillAsTriangular method: wrong shape of target array !");
  ...
  const int blocksPerGrid =
      (target.lengthOf() + threadsPerBlock - 1) / threadsPerBlock;
  ...
  fillAsTriangularCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem,
                            *getContext()->getCudaStream()>>>(
      platformBuffer(), platformShapeInfo(), target.platformBuffer(),
      target.platformShapeInfo(), static_cast<T>(val), lower, upper);
  NDArray::registerSpecialUse({&target}, {this});

  manager.synchronize();
}
BUILD_SINGLE_TEMPLATE(template SD_EXPORT void NDArray::fillAsTriangular,
                      (const float val, int lower, int upper, NDArray& target,
                       const char direction),
                      LIBND4J_TYPES);

////////////////////////////////////////////////////////////////////////
template <typename T>
__global__ static void identityMatrixCuda(void* vx, const Nd4jLong* xShapeInfo,
                                          const T val) {
  auto x = reinterpret_cast<T*>(vx);

  __shared__ int rank, *sharedMem;
  __shared__ Nd4jLong len, totalThreads;  // xLen == zLen, except when xRank =
                                          // 1, in this case zLen = 2*xLen
...
///////////////////////////////////////////////////////////////////
template <typename T>
static void identityMatrixCudaLauncher(const int blocksPerGrid,
                                       const int threadsPerBlock,
                                       const int sharedMem,
                                       const cudaStream_t* stream, void* vx,
                                       const Nd4jLong* xShapeInfo,
                                       const float val) {
  identityMatrixCuda<T><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, static_cast<T>(val));
}
BUILD_SINGLE_TEMPLATE(template void identityMatrixCudaLauncher,
                      (const int blocksPerGrid, const int threadsPerBlock,
                       ...
                       void* vx, const Nd4jLong* xShapeInfo, const float val),
                      LIBND4J_TYPES);
...
void NDArray::setIdentity() {
  ...
  // if (rankOf() != 2)
  //     throw std::runtime_error("NDArray::setIdentity: method should work only
  //     for 2D tensors. But " + toStringValue(rankOf()) + " was given.");

  const int threadsPerBlock = MAX_NUM_THREADS / 4;
  const int blocksPerGrid =
      (lengthOf() + threadsPerBlock - 1) / threadsPerBlock;
  ...
  BUILD_SINGLE_SELECTOR(
      dataType(), identityMatrixCudaLauncher,
      (blocksPerGrid, threadsPerBlock, sharedMem, getContext()->getCudaStream(),
       platformBuffer(), platformShapeInfo(), 1.f),
      LIBND4J_TYPES);
  tickWriteDevice();
...
void NDArray::syncShape() const {
  cudaMemcpy(const_cast<Nd4jLong*>(specialShapeInfo()), shapeInfo(),
             shape::shapeInfoByteLength(shapeInfo()), cudaMemcpyHostToDevice);
}

//////////////////////////////////////////////////////////////////////////
void const* NDArray::specialBufferWithOffset(Nd4jLong offset) const {
  return specialBuffer() != nullptr
             ? static_cast<int8_t const*>(specialBuffer()) +
                   (offset * sizeOfT())
             : nullptr;
}
...
void* NDArray::specialBufferWithOffset(Nd4jLong offset) {
  return specialBuffer() != nullptr
             ? static_cast<int8_t*>(specialBuffer()) + (offset * sizeOfT())
             : nullptr;
}
...
NDArray NDArray::tile(const std::vector<Nd4jLong>& reps) const {
  ...
  if (product == 1) {  // in this case 2 possibilities are present: just reshape
                       // or nothing to do
    NDArray result(*this);
    if (diff < 0) {  // reshape to higher dimension
      std::vector<Nd4jLong> shapeNew =
          reps;  // need to have unities at first "diff" positions of new shape
      memcpy(
          &shapeNew[-diff], result.shapeInfo() + 1,
          rankOld *
              sizeof(Nd4jLong));  // put old shape numbers at rest of positions
  ...
  auto newShapeInfo =
      ShapeUtils::evalTileShapeInfo(*this, reps, getContext()->getWorkspace());
  // create new buffer, in any case the memory amount new buffer points to is
  // bigger then those for old _buffer
  std::shared_ptr<DataBuffer> newBuff = std::make_shared<DataBuffer>(
      shape::length(newShapeInfo) * sizeOfT(), dataType(),
      getContext()->getWorkspace(), true);
  // assign new shape and new buffer to resulting array
  NDArray result(newBuff, ShapeDescriptor(newShapeInfo), getContext());

  // fill newBuff, loop through all elements of newBuff
  // looping through buffer() goes automatically by means of getSubArrayIndex
  // applying
...
void NDArray::tile(const std::vector<Nd4jLong>& reps, NDArray& target) const {
  ...
  if (!shape::equalsSoft(newShapeInfo, target.shapeInfo())) {
    throw std::runtime_error(
        "NDArray::tile method - shapeInfo of target array is not suitable for "
        "tile operation !");
  }
  ...
  BUILD_SINGLE_SELECTOR_TWICE(
      ...
      (specialBuffer(), specialShapeInfo(), target.specialBuffer(),
       target.specialShapeInfo(), targetLen, ews, stream),
      LIBND4J_TYPES);
...
void NDArray::tile(NDArray& target) const {
  if (rankOf() > target.rankOf())
    throw std::runtime_error(
        "NDArray::tile method - rank of target array must be bigger or equal "
        "to the rank of this array !");
  ...
  // fill newBuff, loop through all elements of newBuff
  // looping through getBuffer() goes automatically by means of getSubArrayIndex
  // applying
...
//////////////////////////////////////////////////////////////////////////
template <typename X, typename Z>
static void repeatCudaLauncher(const int blocksPerGrid,
                               const int threadsPerBlock, const int sharedMem,
                               const cudaStream_t* stream, const void* vx,
                               const Nd4jLong* xShapeInfo, void* vz,
                               const Nd4jLong* zShapeInfo, const int* repeats,
                               const int repSize, const int axis) {
  repeatCuda<X, Z><<<blocksPerGrid, threadsPerBlock, sharedMem, *stream>>>(
      vx, xShapeInfo, vz, zShapeInfo, repeats, repSize, axis);
}
...
NDArray NDArray::repeat(const int axis, const std::vector<int>& repeats) const {
  NDArray output('c', ShapeUtils::evalRepeatShape(axis, repeats, *this),
                 dataType(), getContext());
  ...
  PointersManager manager(
      getContext(),
      "NDArray::repeat(const int axis, const std::vector<int>& repeats)");

  const int* reps = reinterpret_cast<int*>(
      manager.replicatePointer(repeats.data(), repeats.size() * sizeof(int)));

  prepareSpecialUse({&output}, {this});
  BUILD_SINGLE_SELECTOR_TWICE(
      dataType(), repeatCudaLauncher,
      (blocksPerGrid, threadsPerBlock, sharedMem, getContext()->getCudaStream(),
       specialBuffer(), specialShapeInfo(), output.specialBuffer(),
       output.specialShapeInfo(), reps, repeats.size(), axis),
      LIBND4J_TYPES);
  prepareSpecialUse({&output}, {this});
...
void NDArray::repeat(const int axis, const std::vector<int>& repeats,
  ...
  BUILD_DOUBLE_SELECTOR(
      ...
      (blocksPerGrid, threadsPerBlock, sharedMem, getContext()->getCudaStream(),
       specialBuffer(), specialShapeInfo(), target.specialBuffer(),
       target.specialShapeInfo(), reps, repeats.size(), axis),
      LIBND4J_TYPES, LIBND4J_TYPES);
  prepareSpecialUse({&target}, {this});
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
void NDArray::printCurrentBuffer(const bool host, const char* msg,
  ...
  if (host) {
    ...
    cudaError_t cudaResult =
        cudaStreamSynchronize(*getContext()->getCudaStream());
    if (cudaResult != 0)
      throw std::runtime_error(
...
template void NDArray::printCurrentBuffer<int>(const bool host, const char* msg,
                                               const int precision) const;
template void NDArray::printCurrentBuffer<float>(const bool host,
                                                 const char* msg,
                                                 const int precision) const;
template void NDArray::printCurrentBuffer<double>(const bool host,
                                                  const char* msg,
                                                  const int precision) const;
