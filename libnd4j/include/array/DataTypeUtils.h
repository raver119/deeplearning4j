...
class SD_EXPORT DataTypeUtils {
 public:
  ...
  FORCEINLINE static sd::DataType pickPairwiseResultType(sd::DataType typeX,
                                                         sd::DataType typeY);

  FORCEINLINE static sd::DataType pickPairwiseResultType(
      const Nd4jLong* shapeInfo1, const Nd4jLong* shapeInfo2);

  FORCEINLINE static sd::DataType pickFloatingType(sd::DataType typeX);

  template <typename T1, typename T2>
  FORCEINLINE static std::vector<T2> convertVector(
      const std::vector<T1>& vector);

  template <typename T>
  FORCEINLINE static bool castShapeInfo(const Nd4jLong* originalShapeInfo,
                                        T* newShapeInfo);

  template <typename T>
  // struct scalarTypesForNDarray { static bool const value =
  // std::is_same<double, T>::value || std::is_same<float, T>::value ||
  // std::is_same<int, T>::value || std::is_same<bfloat16, T>::value ||
  // std::is_same<float16, T>::value || std::is_same<long long, T>::value; };
  struct scalarTypesForNDarray {
    static bool const value =
        ...
        std::is_same<bool, T>::value || std::is_same<bfloat16, T>::value ||
  ...
  struct scalarTypesForExecution {
    static bool const value =
        std::is_same<double, T>::value || std::is_same<float, T>::value ||
        std::is_same<Nd4jLong, T>::value || std::is_same<int, T>::value ||
...
FORCEINLINE bool DataTypeUtils::isR(sd::DataType dataType) {
  return dataType == sd::DataType::FLOAT32 ||
         dataType == sd::DataType::BFLOAT16 || dataType == sd::DataType::HALF ||
...
FORCEINLINE bool DataTypeUtils::isS(sd::DataType dataType) {
  return dataType == sd::DataType::UTF8 || dataType == sd::DataType::UTF16 ||
...
FORCEINLINE bool DataTypeUtils::isU(sd::DataType dataType) {
  return dataType == sd::DataType::UINT8 || dataType == sd::DataType::UINT16 ||
         dataType == sd::DataType::UINT32 || dataType == sd::DataType::UINT64;
}

FORCEINLINE sd::DataType DataTypeUtils::pickPairwiseResultType(
    sd::DataType typeX, sd::DataType typeY) {
...
FORCEINLINE sd::DataType DataTypeUtils::pickPairwiseResultType(
    const Nd4jLong* shapeInfo1, const Nd4jLong* shapeInfo2) {
  return pickPairwiseResultType(ArrayOptions::dataType(shapeInfo1),
                                ArrayOptions::dataType(shapeInfo2));
}
...
template <typename T>
FORCEINLINE bool DataTypeUtils::castShapeInfo(const Nd4jLong* originalShapeInfo,
                                              T* newShapeInfo) {
...
template <typename T1, typename T2>
FORCEINLINE std::vector<T2> DataTypeUtils::convertVector(
    const std::vector<T1>& vector) {
  std::vector<T2> result(vector.size());
  Nd4jLong vecSize = vector.size();
  for (Nd4jLong e = 0; e < vecSize; e++) result[e] = static_cast<T2>(vector[e]);
