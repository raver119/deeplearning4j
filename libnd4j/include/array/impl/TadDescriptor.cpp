...
TadDescriptor::TadDescriptor(const Nd4jLong *originalShape,
...
TadDescriptor::TadDescriptor(const ShapeDescriptor &descriptor,
...
size_t hash<sd::TadDescriptor>::operator()(const sd::TadDescriptor &k) const {
  ...
  res ^= std::hash<sd::ShapeDescriptor>()(k.originalShapeConst()) + 0x9e3779b9 +
         (res << 6) + (res >> 2);
