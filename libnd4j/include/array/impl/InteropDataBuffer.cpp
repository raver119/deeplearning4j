...
namespace sd {
InteropDataBuffer::InteropDataBuffer(InteropDataBuffer& dataBuffer,
...
InteropDataBuffer::InteropDataBuffer(size_t elements, sd::DataType dtype,
                                     bool allocateBoth) {
  if (elements == 0) {
    ...
    _dataBuffer =
        std::make_shared<DataBuffer>(elements, dtype, nullptr, allocateBoth);
  }
}
...
void InteropDataBuffer::registerSpecialUse(
    const std::vector<const InteropDataBuffer*>& writeList,
...
void InteropDataBuffer::prepareSpecialUse(
    const std::vector<const InteropDataBuffer*>& writeList,
    const std::vector<const InteropDataBuffer*>& readList,
  ...
  for (const auto& v : writeList) {
    ...
    // special case for legacy ops - views can be updated on host side, thus
    // original array can be not updated
...
void InteropDataBuffer::registerPrimaryUse(
    const std::vector<const InteropDataBuffer*>& writeList,
    const std::vector<const InteropDataBuffer*>& readList) {
  for (const auto& v : writeList) {
    if (v == nullptr) continue;
...
void InteropDataBuffer::preparePrimaryUse(
    const std::vector<const InteropDataBuffer*>& writeList,
    const std::vector<const InteropDataBuffer*>& readList,
    bool synchronizeWritables) {
  for (const auto& v : readList) {
...
void InteropDataBuffer::expand(size_t newlength) {
  _dataBuffer->expand(newlength *
                      DataTypeUtils::sizeOf(_dataBuffer->getDataType()));
}
