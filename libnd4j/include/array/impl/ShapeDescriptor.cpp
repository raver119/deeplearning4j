...
bool ShapeDescriptor::operator==(const ShapeDescriptor &other) const {
  ...
  if (_strides != other._strides) return false;
...
bool ShapeDescriptor::operator<(const ShapeDescriptor &other) const {
  return std::tie(_empty, _rank, _dataType, _ews, _order, _shape, _strides) <
         std::tie(other._empty, other._rank, other._dataType, other._ews,
                  other._order, other._shape, other._strides);
}
...
Nd4jLong *ShapeDescriptor::toShapeInfo() const {
  ...
  switch (_rank) {
    ...
    case 1: {
      auto shapeInfo =
          ShapeBuilders::createVectorShapeInfo(_dataType, _shape[0]);
    ...
    default: {
      auto shapeInfo =
          ShapeBuilders::createShapeInfo(_dataType, _order, _shape);
...
ShapeDescriptor::ShapeDescriptor(const DataType type, const char order,
                                 const Nd4jLong *shape, const int rank)
    : _dataType(type), _order(order), _rank(rank), _ews(1) {
  _shape.resize(rank);
  _strides.resize(rank);

  for (int e = 0; e < rank; e++) _shape[e] = shape[e];
...
ShapeDescriptor::ShapeDescriptor(const DataType type, const char order,
                                 const Nd4jLong *shape, const Nd4jLong *strides,
                                 const int rank, Nd4jLong ews,
                                 const bool empty) {
  _shape.resize(rank);
  _strides.resize(rank);
  ...
  for (int e = 0; e < rank; e++) _strides[e] = strides[e];
...
ShapeDescriptor::ShapeDescriptor(const DataType type, const char order,
                                 const std::vector<Nd4jLong> &shape,
                                 const std::vector<Nd4jLong> &strides,
                                 const Nd4jLong ews)
    : ShapeDescriptor(type, order, shape, strides) {
  _ews = ews;
}

ShapeDescriptor::ShapeDescriptor(const DataType type, const Nd4jLong length)
    : _dataType(type), _ews(1), _order('c'), _rank(1), _empty(false) {
  _shape = {length};
  _strides = {1};
}
...
ShapeDescriptor::ShapeDescriptor(const Nd4jLong *shapeInfo, bool inheritDtype) {
  ...
  for (int e = 0; e < _rank; e++) {
    ...
    if (shapeInfo[e + 1] == 0) _empty = true;
  }
...
ShapeDescriptor::ShapeDescriptor(const Nd4jLong *shapeInfo,
                                 const sd::DataType dtypeOverride)
    : ShapeDescriptor::ShapeDescriptor(shapeInfo, false) {
  _dataType = dtypeOverride;
}

ShapeDescriptor::ShapeDescriptor(const Nd4jLong *shapeInfo,
                                 const Nd4jLong *dtypeOverride)
    : ShapeDescriptor::ShapeDescriptor(shapeInfo,
                                       ArrayOptions::dataType(dtypeOverride)) {
  //
}

ShapeDescriptor::ShapeDescriptor(const Nd4jLong *shapeInfo,
                                 const Nd4jLong *dtypeOverride,
                                 const Nd4jLong *orderOverride)
    : ShapeDescriptor::ShapeDescriptor(shapeInfo,
                                       ArrayOptions::dataType(dtypeOverride)) {
...
Nd4jLong ShapeDescriptor::ews() const { return _ews; }
...
bool ShapeDescriptor::isEmpty() const { return _empty; }

std::vector<Nd4jLong> &ShapeDescriptor::shape() { return _shape; }

std::vector<Nd4jLong> &ShapeDescriptor::strides() { return _strides; }
...
ShapeDescriptor ShapeDescriptor::vectorDescriptor(const Nd4jLong length,
                                                  const DataType type) {
  ShapeDescriptor descriptor;
  descriptor._dataType = type;
...
namespace std {
size_t hash<sd::ShapeDescriptor>::operator()(
    const sd::ShapeDescriptor &k) const {
