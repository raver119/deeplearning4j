...
void ExtraArguments::convertAndCopy(Nd4jPointer pointer, Nd4jLong offset) {
  ...
  cudaMemcpy(pointer, target,
             length * DataTypeUtils::sizeOf(DataTypeUtils::fromT<T>()),
             cudaMemcpyHostToDevice);
  delete[] target;
#endif
}
BUILD_SINGLE_TEMPLATE(template SD_EXPORT void ExtraArguments::convertAndCopy,
                      (Nd4jPointer pointer, Nd4jLong offset), LIBND4J_TYPES);
...
BUILD_SINGLE_TEMPLATE(template SD_EXPORT void *ExtraArguments::argumentsAsT,
                      (Nd4jLong offset), LIBND4J_TYPES);
