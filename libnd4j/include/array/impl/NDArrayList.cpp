...
namespace sd {
    ...
    int NDArrayList::elements() const {
        return (int) _state->_chunks.size();
    }
    ...
    NDArray NDArrayList::pick(const std::vector<int> &indices) {
        ...
        std::vector<int> axis = ShapeUtils::evalDimsToExclude(shape.size(), {_state->_axis});
    ...
    bool NDArrayList::equals(NDArrayList& other) {
        if (_state->_axis != other._state->_axis)
            return false;

        if (_state->_chunks.size() != other._state->_chunks.size())
            return false;

        for (auto const& v : _state->_chunks) {
            if (other._state->_chunks.count(v.first) == 0)
                return false;
