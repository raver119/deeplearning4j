...
ResultSet::ResultSet(const sd::graph::FlatResult* result) {
  for (int e = 0; e < result->variables()->size(); e++) {
    ...
    if (var->ndarray() != nullptr) {
      ...
      array = NDArray((char)shapeInfo.at(shapeInfo.size() - 1), shape,
                      DataTypeUtils::fromFlatDataType(var->dtype()));
    } else {
      nd4j_printf(
...
Nd4jStatus ResultSet::status() { return _status; }

void ResultSet::setStatus(Nd4jStatus status) { _status = status; }
