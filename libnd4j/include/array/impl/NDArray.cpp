...
//////////////////////////////////////////////////////////////////////////
template <>
std::string* SD_EXPORT NDArray::bufferAsT() const {
...
BUILD_SINGLE_UNCHAINED_TEMPLATE(template SD_EXPORT , * NDArray::bufferAsT() const, LIBND4J_TYPES);
...
ResultSet NDArray::allTensorsAlongDimension(const std::vector<int> &dimensions) const {
    ...
    for (Nd4jLong idx = 0; idx < numTads; idx++ ) {
        NDArray array(_buffer, ShapeDescriptor(pack.primaryShapeInfo()), getContext(), pack.primaryOffsets()[idx] + getBufferOffset());
        array._isView = true;
        result.push_back(array);
    }
