...
class SD_EXPORT DataTypeConversions {
 private:
  ...
  static FORCEINLINE void rconv(bool isBe, bool canKeep, T *buffer,
                                Nd4jLong length, void *src) {
 ...
 public:
  static FORCEINLINE void convertType(void *vbuffer, void *src,
                                      DataType dataType, ByteOrder order,
                                      Nd4jLong length) {
    auto buffer = reinterpret_cast<T *>(vbuffer);
    bool isBe = BitwiseUtils::isBE();
    bool canKeep =
        (isBe && order == ByteOrder::BE) || (!isBe && order == ByteOrder::LE);

    switch (dataType) {
      case BOOL: {
        DataTypeConversions<T>::template rconv<bool>(isBe, canKeep, buffer,
                                                     length, src);
      } break;
      case UINT8: {
        DataTypeConversions<T>::template rconv<uint8_t>(isBe, canKeep, buffer,
                                                        length, src);
      } break;
      case INT8: {
        DataTypeConversions<T>::template rconv<int8_t>(isBe, canKeep, buffer,
                                                       length, src);
      } break;
      case INT16: {
        DataTypeConversions<T>::template rconv<int16_t>(isBe, canKeep, buffer,
                                                        length, src);
      } break;
      case INT32: {
        DataTypeConversions<T>::template rconv<int>(isBe, canKeep, buffer,
                                                    length, src);
      } break;
      case INT64: {
        DataTypeConversions<T>::template rconv<Nd4jLong>(isBe, canKeep, buffer,
                                                         length, src);
      } break;
      case FLOAT32: {
