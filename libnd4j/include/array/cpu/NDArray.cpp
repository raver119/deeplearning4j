...
void NDArray::fillAsTriangular(const float val, int lower, int upper,
  ...
  if (isS())
    throw std::runtime_error(
        "NDArray::fillArrayAsTriangular: you can't use this method on String "
        "array!");
  ...
  if (!isSameShape(target) &&
    ...
    throw std::string(
        "NDArray::fillArrayAsTriangular method: wrong shape of target array !");
  ...
  const bool areSameOffsets =
      shape::haveSameShapeAndStrides(shapeInfo(), target.shapeInfo());
  ...
  auto func = PRAGMA_THREADS_FOR {
    ...
    for (auto i = start; i < stop; i++) {
      ...
      if ((coords[zRank - 2] + upper < coords[zRank - 1]) ||
        ...
        const auto xOffset =
            areSameOffsets ? zOffset : shape::getOffset(shapeInfo(), coords);
        z[zOffset] = x[xOffset];
...
static void templatedSwap(void* xBuffer, void* yBuffer, const Nd4jLong* xShapeInfo, const Nd4jLong* yShapeInfo, Nd4jLong length) {
  ...
  const bool isSameOrders = shape::order(xShapeInfo) == shape::order(xShapeInfo);
    ...
    const auto yEws = shape::elementWiseStride(yShapeInfo);auto func = PRAGMA_THREADS_FOR {
...
BUILD_SINGLE_TEMPLATE(template void templatedSwap,
                      (void* xBuffer, void* yBuffer, const Nd4jLong* xShapeInfo, const Nd4jLong* yShapeInfo, Nd4jLong length),
                      LIBND4J_TYPES);
...
void NDArray::swapUnsafe(NDArray& other) {
  ...
  BUILD_SINGLE_SELECTOR(xType, templatedSwap,
                        (buffer(), other.buffer(), shapeInfo(), other.shapeInfo(), this->lengthOf()),
                        LIBND4J_TYPES);
}
...
////////////////////////////////////////////////////////////////////////
void* NDArray::specialBufferWithOffset(Nd4jLong offset) { return nullptr; }
...
NDArray NDArray::tile(const std::vector<Nd4jLong>& reps) const {
  ...
  if (product == 1) {  // in this case 2 possibilities are present: just reshape
    ...
    if (diff < 0) {  // reshape to higher dimension
      std::vector<Nd4jLong> shapeNew =
          reps;  // there is requirement to have unities at first "diff"
                 // positions of new shape
      memcpy(
  ...
  std::shared_ptr<DataBuffer> newBuff =
      std::make_shared<DataBuffer>(shape::length(newShapeInfo) * sizeOfT(),
                                   dataType(), getContext()->getWorkspace());
  // assign new shape and new buffer to resulting array
  NDArray result(newBuff, ShapeDescriptor(newShapeInfo), getContext());

  // fill newBuff, loop through all elements of newBuff
  // looping through _buffer goes automatically by means of getSubArrayIndex
  // applying
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        BUILD_SINGLE_SELECTOR(xType, this->template templatedAssign,
                              (result.buffer(), i, this->buffer(), yOffset),
                              LIBND4J_TYPES);
      }
    };
    ...
    auto func = PRAGMA_THREADS_FOR {
      for (auto i = start; i < stop; i++) {
        ...
        BUILD_SINGLE_SELECTOR(
            xType, this->template templatedAssign,
            (result.buffer(), xOffset, this->buffer(), yOffset), LIBND4J_TYPES);
      }
    };
...
void NDArray::tile(const std::vector<Nd4jLong>& reps, NDArray& target) const {
  ...
  if (target.ordering() == 'c' && ews == 1) {  //  ews == 1 always here
    //#pragma omp parallel for simd if(targetLen >
    ...
    for (Nd4jLong i = 0; i < targetLen; ++i) {
      ...
      BUILD_DOUBLE_SELECTOR(target.dataType(), dataType(),
                            templatedDoubleAssign,
                            (target.buffer(), i, buffer(), yOffset),
    ...
    for (Nd4jLong i = 0; i < targetLen; ++i) {
      ...
      BUILD_DOUBLE_SELECTOR(target.dataType(), dataType(),
                            templatedDoubleAssign,
                            (target.buffer(), i * ews, buffer(), yOffset),
                            LIBND4J_TYPES, LIBND4J_TYPES);
    }
  } else {
    for (Nd4jLong i = 0; i < targetLen; ++i) {
      ...
      BUILD_DOUBLE_SELECTOR(target.dataType(), dataType(),
                            templatedDoubleAssign,
                            (target.buffer(), xOffset, buffer(), yOffset),
                            LIBND4J_TYPES, LIBND4J_TYPES);
    }
  }
}
...
static void repeat_(const NDArray& input, NDArray& output,
                    const std::vector<int>& repeats, const int axis) {
...
NDArray NDArray::repeat(const int axis, const std::vector<int>& repeats) const {
  ...
  BUILD_SINGLE_SELECTOR_TWICE(dataType(), repeat_,
                              (*this, output, repeats, axis), LIBND4J_TYPES);
...
void NDArray::repeat(const int axis, const std::vector<int>& repeats,
  ...
  BUILD_DOUBLE_SELECTOR(dataType(), target.dataType(), repeat_,
                        (*this, target, repeats, axis), LIBND4J_TYPES,
                        LIBND4J_TYPES);
}
