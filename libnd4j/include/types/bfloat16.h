...
struct bfloat16 {
 private:
  ...
  // std::decay<T>::type>::value || std::is_same<float16, T>::value;; };

 public:
  int16_t _data;

  local_def bfloat16() { _data = 0; }
  ...
  local_def operator float() const {
    int32_t temp = this->_data
                   << 16;  //((sign << 31) | (exponent << 23) | mantissa);
  ...
  local_def friend bool operator==(const bfloat16& a, const bfloat16& b) {
  ...
  local_def friend bfloat16 operator+(const bfloat16& a, const bfloat16& b) {
  ...
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bfloat16 operator+(const bfloat16& a, const T& b) {
    return a + static_cast<bfloat16>(b);
  }
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bfloat16 operator+(const T& a, const bfloat16& b) {
  ...
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bool operator==(const bfloat16& a, const T& b) {
    return a == static_cast<bfloat16>(b);
  }
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bool operator==(const T& a, const bfloat16& b) {
    return static_cast<bfloat16>(a) == b;
  }
  ...
  local_def bfloat16& operator+=(bfloat16 rhs) {
  ...
  local_def bfloat16 operator++(int) {
    *this = (float)*this + (float)1.f;
    return *this;
  }
  ...
  local_def bfloat16 operator-() const { return 0.f - (float)*this; }
...
// WARNING: this implementation only for avoid cyclic references between float16
// and bfloat16 types. local_def void float16::assign(const bfloat16& rhs) {
//   assign((float)rhs);
// }
