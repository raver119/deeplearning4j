...
struct ihalf : public __half {
 public:
  ...
  inline __host__ __device__ unsigned short* getXP() { return &this->__x; }

  inline __host__ __device__ unsigned short getX() const { return this->__x; }
...
struct ihalf : public __half {
 public:
  ...
  inline __host__ __device__ unsigned short* getXP() { return &this->x; }

  inline __host__ __device__ unsigned short getX() const { return this->x; }
...
struct float16 {
 private:
  ...
  struct isNumericType {
    static bool const value =
      ...
      // typename std::decay<T>::type>::value || std::is_same<float, typename
      // std::decay<T>::type>::value || std::is_same<int, typename
      // std::decay<T>::type>::value || std::is_same<unsigned int, typename
      // std::decay<T>::type>::value || std::is_same<long long, typename
  // std::decay<T>::type>::value || std::is_same<unsigned long long, typename
  // std::decay<T>::type>::value || std::is_same<long int, typename
  // std::decay<T>::type>::value || std::is_same<long unsigned int, typename
  // std::decay<T>::type>::value || std::is_same<int8_t, typename
  // std::decay<T>::type>::value || std::is_same<uint8_t, typename
  // std::decay<T>::type>::value || std::is_same<int16_t, typename
  // std::decay<T>::type>::value || std::is_same<uint16_t, typename
  // std::decay<T>::type>::value || std::is_same<bool, typename
 ...
 public:
  ...
  template <typename T, typename = typename std::enable_if<
                            ...
                            std::is_same<bfloat16, T>::value>::type>
  ...
  local_def explicit operator half() const { return data; }

  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
...
#ifdef NATIVE_HALFS
  local_def friend bool operator==(const float16& a, const float16& b) {
  ...
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend float16 operator+(const float16& a, const T& b) {
    return a + static_cast<float16>(b);
  }
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend float16 operator+(const T& a, const float16& b) {
  ...
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bool operator==(const float16& a, const T& b) {
    return a == static_cast<float16>(b);
  }
  template <typename T,
            typename = typename std::enable_if<isNumericType<T>::value>::type>
  local_def friend bool operator==(const T& a, const float16& b) {
    return static_cast<float16>(a) == b;
  }
  ...
  local_def float16& operator+=(float16 rhs) {
  ...
  local_def float16 operator++(int) {
    *this = *this + (float16)1.f;
    return *this;
  }
  ...
  local_def float16 operator-() const { return 0.f - (float)*this; }
...
#ifdef __CUDACC__
local_def int isnan(const float16& h) {
  return ishnan_(((ihalf)h.data).getX());
}

local_def int isinf(const float16& h) {
  return ishinf_(((ihalf)h.data).getX());
}
#endif
