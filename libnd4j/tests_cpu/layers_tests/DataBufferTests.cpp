...
TEST_F(DataBufferTests, test_alloc_limit_1) {
  ...
  MemoryCounter::getInstance().setGroupLimit(MemoryType::HOST,
                                              odLimit + limitSize);
  ...
  ASSERT_EQ(odUse + allocSize,
            MemoryCounter::getInstance().allocatedDevice(deviceId));
  ASSERT_EQ(ogUse + allocSize,
            MemoryCounter::getInstance().allocatedGroup(MemoryType::HOST));
