...
TEST_F(SortCudaTests, test_linear_sort_by_val_2) {
    auto k = NDArrayFactory::create<int>(  {6}, {0, 1, 2, 3, 4, 5});
//    auto v = NDArrayFactory::create<double>('c', {6}, {1.5, 3.5, 5.5, 9.5, 0.5, 2.5, 4.5, 6.5, 7.5, 8.5});
    NDArray v = NDArrayFactory::create<double>(  {6}, {0.9f, .75f, .6f, .95f, .5f, .3f});
    auto ek = NDArrayFactory::create<int>(  {6}, {3, 0, 1, 2, 4, 5});
    auto ev = NDArrayFactory::create<double>(  {6}, {0.95, 0.9, 0.75, 0.6, 0.5, 0.3});
