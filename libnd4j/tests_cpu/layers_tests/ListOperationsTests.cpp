...
TEST_F(ListOperationsTests, BasicTest_Write_1) {
    NDArrayList list(5);
    auto x = NDArrayFactory::create<double>(  {128});
    x.linspace(1);
...
TEST_F(ListOperationsTests, BasicTest_Stack_1) {
    NDArrayList list(10);
    auto exp = NDArrayFactory::create<double>(  {10, 100});
    auto tads = exp.allTensorsAlongDimension({1});
    for (int e = 0; e < 10; e++) {
        auto row = NDArrayFactory::vector<double>(100);
        row.assign((double) e);
...
TEST_F(ListOperationsTests, BasicTest_UnStackList_1) {
    ...
    auto x = NDArrayFactory::create<double>(  {10, 100});
...
TEST_F(ListOperationsTests, BasicTest_Read_1) {
    NDArrayList list(10);
    auto exp = NDArrayFactory::create<double>(  {1, 100});
    exp.assign(4.0f);
...
TEST_F(ListOperationsTests, BasicTest_Pick_1) {
    NDArrayList list(10);
    auto exp = NDArrayFactory::create<double>(  {4, 100});
...
TEST_F(ListOperationsTests, BasicTest_Create_1) {
    auto matrix = NDArrayFactory::create<double>(  {3, 2});
    matrix.linspace(1);
...
TEST_F(ListOperationsTests, BasicTest_Split_1) {
    ...
    auto exp0 = NDArrayFactory::create<double>(  {2, 5});
    auto exp1 = NDArrayFactory::create<double>(  {3, 5});
    auto exp2 = NDArrayFactory::create<double>(  {5, 5});

    auto matrix = NDArrayFactory::create<double>(  {10, 5});

    auto lengths = NDArrayFactory::vector<int>(3);
    lengths.p(0, 2);
    lengths.p(1, 3);
    lengths.p(2, 5);
    ...
    int cnt2 = 0;
    for (int e = 0; e < 10; e++) {
        auto row = NDArrayFactory::vector<double>(5);
        row.assign((double) e);
        tads.at(e)->assign(row);
...
TEST_F(ListOperationsTests, BasicTest_Scatter_1) {
    ...
    auto indices = NDArrayFactory::create<double>(  {1, 10});
...
TEST_F(ListOperationsTests, BasicTest_Gather_1) {
    NDArrayList list(0, true);
    for (int e = 0; e < 10; e++) {
        auto row = NDArrayFactory::vector<double>(3);
        row.assign((double) e);
        list.write(e, new NDArray(row));
    }

    auto exp = NDArrayFactory::create<double>(  {10, 3});
    auto tads = exp.allTensorsAlongDimension({1});
    for (int e = 0; e < 10; e++) {
...
TEST_F(ListOperationsTests, GraphTests_Sequential_1) {
    ...
    auto matrix  = NDArrayFactory::create<float>(  {3, 3});
    auto tads = matrix.allTensorsAlongDimension({1});
    ...
    auto indices = NDArrayFactory::valueOf<int>({3}, 1);
    //indices->linspace(0);
...
TEST_F(ListOperationsTests, GraphTests_Sequential_2) {
    ...
    auto matrix = NDArrayFactory::create<double>(  {3, 3});
    auto tads = matrix.allTensorsAlongDimension({1});
    for (int e = 0; e < tads.size(); e++) {
        tads.at(e)->assign((float) (e+1));
    }
    ...
    auto indices = NDArrayFactory::create<double>(  {1, 3});
    indices.linspace(0);
