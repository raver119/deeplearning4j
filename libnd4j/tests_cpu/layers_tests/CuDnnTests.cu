...
static void printer(
    std::initializer_list<sd::ops::platforms::PlatformHelper*> helpers) {
  for (auto v : helpers) {
...
TEST_F(CuDnnTests, helpers_includer) {
  // we need this block, to make sure all helpers are still available within
  // binary, and not optimized out by linker
#ifdef HAVE_CUDNN
  ...
  sd::ops::platforms::PLATFORM_depthwise_conv2d_bp_ENGINE_CUDA
      depthwise_conv2d_bp;
...
TEST_F(CuDnnTests, mixed_helpers_test_1) {
  ...
  int bS = 2, iH = 4, iW = 3, iC = 4, oC = 3, kH = 3, kW = 2, sH = 1, sW = 1,
      pH = 0, pW = 0, dH = 1, dW = 1;
  int oH = 2, oW = 2;
  ...
  auto expOutput = NDArrayFactory::create<float>(
      'c', {bS, oC, oH, oW},
      {61.f,   61.f,   61.f,   61.f,   177.2f, 177.2f, 177.2f, 177.2f,
       293.4f, 293.4f, 293.4f, 293.4f, 61.f,   61.f,   61.f,   61.f,
       177.2f, 177.2f, 177.2f, 177.2f, 293.4f, 293.4f, 293.4f, 293.4f});
