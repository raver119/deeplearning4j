...
TEST_F(EmptyTests, Test_Concat_2) {
  auto empty = NDArray(
      'c', {0}, sd::DataType::FLOAT32);  // NDArrayFactory::empty_<float>();
...
TEST_F(EmptyTests, Test_Concat_3) {
  auto empty =
      NDArrayFactory::empty<float>();  // NDArrayFactory::empty_<float>();
...
TEST_F(EmptyTests, test_shaped_empty_4) {
  const auto shape = ConstantShapeHelper::getInstance().vectorShapeInfo(
      0, sd::DataType::FLOAT32);
