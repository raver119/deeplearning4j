...
TEST_F(GraphAnalysisTests, basic_toposort_test_9) {
    ...
    // we expect that OpSequence has exactly 1 ops
    ASSERT_EQ(1, layer2[1].length());
    ASSERT_EQ(14, layer2[1].at(0).protoContext().nodeId());

    //sequence = layer2[2];
    // we expect that OpSequence has exactly 1 ops
    ASSERT_EQ(1, layer2[2].length());
    ASSERT_EQ(11, layer2[2].at(0).protoContext().nodeId());

    //sequence = layer2[3];
    // we expect that OpSequence has exactly 1 ops
    ASSERT_EQ(1, layer2[3].length());
    ASSERT_EQ(15, layer2[3].at(0).protoContext().nodeId());
    
    //sequence = layer2[4];
    // we expect that OpSequence has exactly 1 ops
    ASSERT_EQ(1, layer2[4].length());
    ASSERT_EQ(12, layer2[4].at(0).protoContext().nodeId());
    ...
    ASSERT_EQ(13, layer2[6].at(0).protoContext().nodeId());
...
TEST_F(GraphAnalysisTests, basic_toposort_test_11) {
    Graph graph;
...
TEST_F(GraphAnalysisTests, test_cond_1) {
    ...
    Node name: cond/switch_f; ID: 11; Input: 9, 0; Operation type: 21;  Operation class: -1719689536
    Node name: cond/switch_t; ID: 10; Input: 9, 1; Operation type: 21;  Operation class: -1719689536
    Node name: cond/Switch;   ID: 9;  Input: 1, 0; Operation type: 119; Operation class: -1719689536
    Node name: cond/Switch;   ID: 9;  Input: 6, 0; Operation type: 119; Operation class: -1719689536
    Node name: cond/Merge;    ID: 8;  Input: 5, 0; Operation type: 119; Operation class: -1719689536
    Node name: cond/Merge;    ID: 8;  Input: 7, 0; Operation type: 119; Operation class: -1719689536
    Node name: in_0/read;     ID: 6;  Input: 1, 0; Operation type: 21;  Operation class: -1719689536
    Node name: cond/LinSpace; ID: 7;  Input: 2, 0; Operation type: 21;  Operation class: -1719689536
    Node name: cond/LinSpace; ID: 7;  Input: 3, 0; Operation type: 21;  Operation class: -1719689536
    Node name: cond/LinSpace; ID: 7;  Input: 4, 0; Operation type: 21;  Operation class: -1719689536

    as it can be seen cond/LinSpace is not connected with any switch node(s) that causes wrong results of optimization.
    also maybe to cover all conditional operations will be need "Operation class", but this have to discovered deeper.
