...
TEST_F(PerformanceTests, test_matmul_c_f_1) {
  ...
  for (int e = 0; e < iterations; e++) {
    ...
    auto outerTimeF = std::chrono::duration_cast<std::chrono::nanoseconds>(
                          timeEndF - timeStartF)
                          .count();
    ...
    auto outerTimeC = std::chrono::duration_cast<std::chrono::nanoseconds>(
                          timeEndC - timeStartC)
                          .count();
  ...
  nd4j_printf("Median time C: [%lld]; Median time F: [%lld];",
              valuesC[valuesC.size() / 2], valuesF[valuesF.size() / 2]);
}
...
TEST_F(PerformanceTests, test_maxpooling2d_1) {
  ...
  for (int i = 0; i < numIterations; i++) {
    ...
    if ((i + 1) % 1000 == 0) nd4j_printf("Iteration %i finished...\n", i + 1);
  }
