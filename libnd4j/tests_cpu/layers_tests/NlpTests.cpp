...
TEST_F(NlpTests, basic_sg_hs_test_1) {
  ...
  auto result = op.evaluate(
      {&target, &ngStarter, &indices, &codes, &syn0, &syn1, &syn1Neg, &expTable,
       &negTable, &alpha, &randomValue, &inferenceVector, &neu1e},
      {}, {}, {false}, {}, true);
...
TEST_F(NlpTests, basic_sg_hs_test_3) {
  ...
  RandomLauncher::fillUniform(LaunchContext::defaultContext(), rng, &syn00, 0.0,
                              1.0);
  RandomLauncher::fillUniform(LaunchContext::defaultContext(), rng, &syn10, 0.0,
                              1.0);
  ...
  sd::ops::skipgram op;
  auto result0 = op.evaluate(
      {&target, &ngStarter, &indices0, &codes00, &syn00, &syn10, &syn1Neg,
       &expTable, &negTable, &alpha, &randomValue, &inferenceVector, &neu1e},
      {}, {}, {false}, {}, true);
  auto result1 = op.evaluate(
      {&target, &ngStarter, &indices1, &codes01, &syn01, &syn11, &syn1Neg,
       &expTable, &negTable, &alpha, &randomValue, &inferenceVector, &neu1e},
      {}, {}, {false}, {}, true);
...
TEST_F(NlpTests, test_cbow_hs_batch_1) {
  ...
  auto ngStarter = NDArrayFactory::empty<int>();
  auto context =
      NDArrayFactory::create<int>('c', {2, 3}, {0, 1, 2, 100, 101, 102});
