...
TEST_F(DeclarableOpsTests19, test_threshold_encode_1) {
  ...
  auto exp_encoded =
      NDArrayFactory::create<int>('c', {7}, {3, 3, 1056964608, 0, 1, 2, -3});
  auto exp_gradients =
      NDArrayFactory::create<double>('c', {3}, {1.0, 2.0, -3.0});
...
TEST_F(DeclarableOpsTests19, test_threshold_decode_1) {
  ...
  auto y =
      NDArrayFactory::create<int>('c', {7}, {3, 3, 1056964608, 0, 1, 2, -3});
  auto exp_gradients =
      NDArrayFactory::create<double>('c', {3}, {1.5, 2.5, -3.5});
...
TEST_F(DeclarableOpsTests19, test_bitmap_encode_1) {
  auto initial = NDArrayFactory::create<float>(
      'c', {6}, {0.0f, 0.0f, 1e-3f, -1e-3f, 0.0f, 0.0f});
...
TEST_F(DeclarableOpsTests19, test_bitmap_encode_decode) {
  ...
  for (int e = 0; e < initial.lengthOf(); e++) {
    ...
    if (f != 1.0f) nd4j_printf("initial[%i] = %f\n", e, f);
  }
