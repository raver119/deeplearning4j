...
TEST_F(ContextTests, Basic_Test_1) {
  ...
  ASSERT_NEAR(1.0f, block.variable(0)->getNDArray()->meanNumber().e<float>(0),
              1e-5);
  ASSERT_NEAR(2.0f, block.variable(1)->getNDArray()->meanNumber().e<float>(0),
              1e-5);
}
...
TEST_F(ContextTests, Basic_Test_9) {
  ...
  ctx.pushNDArrayToVariableSpace(1, 1, in);
}
...
TEST_F(ContextTests, test_short_context_1) {
  ...
  auto array1 = NDArrayFactory::create<float>(
      'c', {3, 2}, {-1.f, -2.f, -3.f, -4.f, -5.f, -6.f});
  Context ctx(1);
