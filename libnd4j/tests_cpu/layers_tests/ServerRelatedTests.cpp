...
TEST_F(ServerRelatedTests, Basic_Execution_Test_1) {
  flatbuffers::FlatBufferBuilder builder(4096);
  auto oGraph = GraphExecutioner::importFromFlatBuffers(
      "./resources/reduce_dim_false.fb");
  oGraph->printOut();
...
TEST_F(ServerRelatedTests, Basic_Execution_Test_2) {
  ...
  auto input0 = NDArrayFactory::create<float>(
      'c', {3, 3}, {2.f, 2.f, 2.f, 2.f, 2.f, 2.f, 2.f, 2.f, 2.f});
...
TEST_F(ServerRelatedTests, BasicExecutionTests_3) {
  ...
  auto flatResult =
      GraphHolder::getInstance()->execute(fir->id(), builder, fir);
