...
TEST_F(ThreadsTests, th_test_2) {
  // in this case we'll get better split over second loop - exactly 32 elements
  // per thread
  ASSERT_EQ(2, ThreadsHelper::pickLoop2d(32, 48, 1024));
  ASSERT_EQ(2, ThreadsHelper::pickLoop2d(6, 4, 16384));

  // in this case we'll get better split over first loop - 2 loops/2048 elements
  // per thread
  ASSERT_EQ(1, ThreadsHelper::pickLoop2d(32, 64, 1024));
  ASSERT_EQ(1, ThreadsHelper::pickLoop2d(6, 6, 16384));

  // in this case none of loops are good enough, but second loop is too small
  // for split
    ...
    for (int c = 1; c <= 64; c++) {
      for (int t = 1; t <= 64; t++) {
        ...
        if (t > 1 && threads == 1 && (b > 1 && c > 1)) {
          nd4j_printf(
              "Got 1 thread for [%i, %i] loop; initial max threads: %i\n", b, c,
              t)
        }
...
TEST_F(ThreadsTests, basic_test_1) {
    ...
    auto outerTimeThreads =
std::chrono::duration_cast<std::chrono::microseconds> (timeEndThreads -
timeStartThreads).count();
    ...
    auto outerTimeOmp = std::chrono::duration_cast<std::chrono::microseconds>
(timeEndOmp - timeStartOmp).count();
