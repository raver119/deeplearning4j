...
CHIP="${CHIP:-cpu}"
TEST_RESULTS_PATH="xml:../target/surefire-reports/TEST-${CHIP}-results.xml"
...
if [ -n "$BUILD_PATH" ]; then
    if which cygpath; then
        ...
        TEST_RESULTS_PATH=xml:..\\target\\surefire-reports\\TEST-${CHIP}-results.xml
    fi
    export PATH="$PATH:$BUILD_PATH"
fi

../blasbuild/${CHIP}/tests_cpu/layers_tests/runtests --gtest_output=${TEST_RESULTS_PATH}
