...
public class CpuNDArrayFactory extends BaseNativeNDArrayFactory {
    ...
    public INDArray accumulate(INDArray target, INDArray... arrays) {
        ...
        Pointer[] dataPointers = new Pointer[arrays.length];
        ...
        for (int i = 0; i < arrays.length; i++) {
            ...
            dataPointers[i] = arrays[i].data().addressPointer();
        }


        nativeOps.accumulate(null,
                new PointerPointer(dataPointers), (LongPointer) arrays[0].shapeInfoDataBuffer().addressPointer(),
                null, null,
    ...
    public void shuffle(List<INDArray> arrays, Random rnd, List<int[]> dimensions) {
        ...
        val dataPointers = new Pointer[arrays.size()];
        val shapePointers = new Pointer[arrays.size()];
        val tadPointers = new Pointer[arrays.size()];
        val offsetPointers = new Pointer[arrays.size()];
        ...
        for (int i = 0; i < arrays.size(); i++) {
            ...
            dataPointers[i] = array.data().addressPointer();
            shapePointers[i] = array.shapeInfoDataBuffer().addressPointer();
            offsetPointers[i] = offsets.addressPointer();
            tadPointers[i] = tadBuffers.getFirst().addressPointer();
        }


        nativeOps.shuffle(dummy,
                    new PointerPointer(dataPointers), new PointerPointer(shapePointers),
                null, null,
                    new PointerPointer(dataPointers), new PointerPointer(shapePointers),
                null, null,
                arrays.size(),
                    ptrMap, new PointerPointer(tadPointers), new PointerPointer(offsetPointers));
